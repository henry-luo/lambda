...
void render_rectangles(SDL_Surface *surface, int width) {
    int x = GAP, y = GAP;
    SDL_Rect rect;
    SDL_FillSurfaceRect(surface, NULL, 0xFFFFFFFF); // Clear surface with white
    ...
    for (int i = 0; i < NUM_RECTANGLES; i++) {
        ...
        rect.w = RECT_WIDTH;
        rect.h = RECT_HEIGHT;
        SDL_FillSurfaceRect(surface, &rect, 0xFF0000FF | (x << 8));
