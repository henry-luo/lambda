...
| **8 — Crypto** | +60 | 🟢 **Symmetric done, asymmetric missing** | `createCipheriv/Decipheriv` (AES-CBC/CTR/GCM) `js_crypto.cpp:908–938`; `pbkdf2(Sync)` `:945`; `scrypt(Sync)` `:1112`; `subtle.digest/encrypt/decrypt` `:1225–1440`; `getCiphers/getCipherInfo/getHashes/timingSafeEqual`; `hash()`, `randomUUIDv7()`, `randomFillSync`, callback-form `randomFill`, `getFips`, `hkdf(Sync)`, minimal `createSecretKey()`, and AES/HMAC `generateKey(Sync)` are now present. Missing: sign/verify, DH/ECDH, asymmetric keygen, full KeyObject, X509. |
...
Rows for `https`, `net`, `child_process`, `tls`, `stream`, `util`, and `dns` were refreshed with focused 2026-06-18 runs against the current local build; `buffer` and `crypto` were refreshed on 2026-06-20 after the C.3 iterator-surface and Track D P3 random-fill/HKDF/generate-key/getCipherInfo/hash/randomUUIDv7 slices. The remaining rows are from the 2026-06-16 release-baseline probe and should be refreshed before using their exact counts in a release decision.
...
| crypto | 120 | 23 | 19% | 0 | randomFillSync/randomFill/getFips, HKDF, AES/HMAC generateKey, AES getCipherInfo metadata, one-shot hash, and UUIDv7 now landed; asymmetric gap remains (sign/verify/asymmetric keygen/full KeyObject) |
