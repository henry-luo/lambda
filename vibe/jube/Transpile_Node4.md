...
| **14 — Test infra** | +25 | 🟡 **skip-list yes, common shims partial** | External skip-list (26 entries) + GTest loading done. `common/{index,tmpdir,fixtures,internet,wpt}` shimmed (`lambda/js/test_shim/`), and `make node-shim` installs the runner-facing copies under `ref/node/test/common/`. Missing: `common/crypto`, `common/dns`. |
...
Rows for `https`, `net`, `child_process`, `tls`, `stream`, `util`, and `dns` were refreshed with focused 2026-06-18 runs against the current local build; `buffer` was refreshed on 2026-06-20 after the C.3 iterator-surface slice. The remaining rows are from the 2026-06-16 release-baseline probe and should be refreshed before using their exact counts in a release decision.
