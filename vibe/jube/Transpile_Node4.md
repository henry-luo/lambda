...
| **14 — Test infra** | +25 | 🟡 **skip-list yes, common shims partial** | External skip-list (26 entries) + GTest loading done. `common/{index,tmpdir,fixtures,internet,wpt,dns}` shimmed (`lambda/js/test_shim/`), and `make node-shim` installs the runner-facing copies under `ref/node/test/common/`. Missing: `common/crypto`. |
