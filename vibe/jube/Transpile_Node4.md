...
- Node3's plan was executed **out of order**. The work that landed (crypto ciphers, Buffer, assert, a real TLS layer, module sub-path resolution, process/os) tracked Phases 6/8/9/10/13. **Phase 7 — the stream state-machine rebuild, identified in Node3 as the highest-reward item — was not rebuilt.** Narrow 2026-06-22 pipe/PassThrough/read-stream/eventNames/addAbortSignal/state-surface/disabled-Duplex slices have landed, but `js_stream.cpp` is still about 1,503 LOC of map-based scaffolding.
...
| **7 — Stream internals** | +100 | 🟡 **Narrow pipe/surface/state slices landed; core rebuild still open** | `js_stream.cpp` is still about 1,503 LOC of JS-map state (`__flowing__`, `__buffer__`, `_readableState`, `_writableState`), only option storage for `highWaterMark` rather than real backpressure, and `Symbol.asyncIterator` is only a narrow buffered/done shim. 2026-06-22 fixed zero-length internal buffers, basic future-chunk `pipe()` forwarding, PassThrough double-push/signature behavior, instance `eventNames()`, `stream.addAbortSignal()`/internal no-validate helper wiring, readable/writable state properties, disabled Duplex side errors, and late end/finish/close listener behavior; **the real state-machine rebuild remains the critical miss.** |
...
Rows for `https`, `net`, `child_process`, `tls`, `util`, and `dns` were refreshed with focused 2026-06-18 runs against the current local build; `buffer` was refreshed on 2026-06-20 after the C.3 iterator-surface slice; `stream` and `crypto` were refreshed on 2026-06-22 after the hash-stream pipe/read-stream/eventNames/addAbortSignal/state-surface slices. The remaining rows are from the 2026-06-16 release-baseline probe and should be refreshed before using their exact counts in a release decision.
...
| stream | 213 | 78 | 37% | 0 | Basic pipe/read-stream/PassThrough/eventNames/addAbortSignal/state-surface/disabled-Duplex fixes landed; **the linchpin still has 135 failing** |
...
- `push()`/`write()` mutate `__buffer__` directly; there is **no `highWaterMark`, no backpressure** — normal writes are not pressure-limited, and `drain` is still emitted unconditionally.
...
135 of 213 stream tests still fail directly on this after the narrow 2026-06-22 pipe/read-stream/eventNames/addAbortSignal/state-surface/disabled-Duplex slices, and it blocks everything in RC2.
...
Estimated ~2,000-2,500 LOC. **stream 78 -> ~120 (+42)**, and it unblocks Track B.
