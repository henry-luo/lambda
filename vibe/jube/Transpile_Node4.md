...
| **7 — Stream internals** | +100 | 🟡 **Narrow pipe/surface/state slices landed; core rebuild still open** | `js_stream.cpp` is still about 1,385 LOC of JS-map state (`__flowing__`, `__buffer__`, `_readableState`, `_writableState`), only option storage for `highWaterMark` rather than real backpressure, and there is no `Symbol.asyncIterator`. 2026-06-22 fixed zero-length internal buffers, basic future-chunk `pipe()` forwarding, PassThrough double-push/signature behavior, instance `eventNames()`, `stream.addAbortSignal()`/internal no-validate helper wiring, readable/writable state properties, and late end/finish/close listener behavior; **the real state-machine rebuild remains the critical miss.** |
...
| stream | 213 | 77 | 36% | 0 | Basic pipe/read-stream/PassThrough/eventNames/addAbortSignal/state-surface fixes landed; **the linchpin still has 136 failing** |
...
136 of 213 stream tests still fail directly on this after the narrow 2026-06-22 pipe/read-stream/eventNames/addAbortSignal/state-surface slices, and it blocks everything in RC2.
...
Estimated ~2,000-2,500 LOC. **stream 77 -> ~120 (+43)**, and it unblocks Track B.
