...
> **Status of this document.** This proposal supersedes the planning in `Transpile_Node3.md`. It re-measures the Node.js compatibility baseline against a **fresh `make release` build (2026-06-16)**, then refreshes key module claims against the current LambdaJS implementation with focused checks on 2026-06-18, 2026-06-20, 2026-06-21, and 2026-06-22. The locked full-suite baseline remains 1,462; the focused spot-checks below are drift indicators, not a replacement full release baseline. Every claim is grounded in the current `lambda/js/` code (`file:line` cited; line numbers drift -- confirm against the symbol).
...
Rows for `https`, `net`, `child_process`, `tls`, `util`, and `dns` were refreshed with focused 2026-06-18 runs against the current local build; `buffer` was refreshed on 2026-06-20 after the C.3 iterator-surface slice; `stream` and `crypto` were refreshed on 2026-06-22 after the hash-stream pipe/read-stream/eventNames/addAbortSignal/state-surface/inheritance/backpressure slices. The remaining rows are from the 2026-06-16 release-baseline probe and should be refreshed before using their exact counts in a release decision.
...
| stream | 213 | 90 | 42% | 0 | Basic pipe/read-stream/PassThrough/eventNames/addAbortSignal/state-surface/disabled-Duplex/constructor-options/inheritance/write-backpressure/writable-encoding fixes landed; **the linchpin still has 123 failing** |
...
- `push()`/`write()` still mutate `__buffer__` and state-map fields directly; a narrow write-side `highWaterMark`/`needDrain` callback path exists, but there is **no real Readable/Writable queue state machine** and no push-side backpressure.
...
- the static surface is `Readable/Writable/Duplex/Transform/PassThrough/pipeline/finished` + `Readable.from` + `addAbortSignal`. Missing: `compose` and a correct promise-returning `stream/promises.pipeline/finished`.

123 of 213 stream tests still fail directly on this after the narrow 2026-06-22 pipe/read-stream/eventNames/addAbortSignal/state-surface/disabled-Duplex/constructor-options/inheritance/backpressure/captureRejections/writable-encoding slices, and it blocks everything in RC2.
...
Estimated ~2,000-2,500 LOC. **stream 88 -> ~120 (+32)**, and it unblocks Track B.
...
Tests come from `ref/node/test/parallel/` (3,938 files); the runner filters by the enabled-module table in `test/test_node_gtest.cpp:110+` (dgram/http2/wasi disabled) and the external `test/node/skip_list.txt` (25 test entries). PASS = exit 0 and no `Uncaught` in output.
