...
| **7 — Stream internals** | +100 | 🟡 **Narrow pipe/surface/state slices landed; core rebuild still open** | `js_stream.cpp` is still about 1,998 LOC of JS-map state (`__flowing__`, `__buffer__`, `_readableState`, `_writableState`), with narrow write-side highWaterMark/needDrain callback handling but no real Readable/Writable queue state machine, and `Symbol.asyncIterator` is only a narrow buffered/done shim. 2026-06-22 fixed zero-length internal buffers, basic future-chunk `pipe()` forwarding, PassThrough double-push/signature behavior, instance `eventNames()`, `stream.addAbortSignal()`/internal no-validate helper wiring, readable/writable state properties, disabled Duplex side errors, constructor option surfaces/default encoding validation, constructor inheritance/prototype wiring, cork/bufferedRequestCount, write-side needDrain/drain callback behavior, captureRejections, and late end/finish/close listener behavior; **the real state-machine rebuild remains the critical miss.** |
...
| stream | 213 | 88 | 41% | 0 | Basic pipe/read-stream/PassThrough/eventNames/addAbortSignal/state-surface/disabled-Duplex/constructor-options/inheritance/write-backpressure fixes landed; **the linchpin still has 125 failing** |
...
125 of 213 stream tests still fail directly on this after the narrow 2026-06-22 pipe/read-stream/eventNames/addAbortSignal/state-surface/disabled-Duplex/constructor-options/inheritance/backpressure/captureRejections slices, and it blocks everything in RC2.
