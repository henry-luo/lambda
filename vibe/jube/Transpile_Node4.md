...
| **7 — Stream internals** | +100 | 🟡 **Narrow pipe/surface slices landed; core rebuild still open** | `js_stream.cpp` is still about 937 LOC of JS-map state (`__flowing__`, `__buffer__`), only option storage for `highWaterMark` rather than real backpressure, `_write` gets a noop callback, and there is no `Symbol.asyncIterator`. 2026-06-22 fixed zero-length internal buffers, basic future-chunk `pipe()` forwarding, PassThrough double-push/signature behavior, instance `eventNames()`, and `stream.addAbortSignal()`/internal no-validate helper wiring; **the real state-machine rebuild remains the critical miss.** |
...
Rows for `https`, `net`, `child_process`, `tls`, `util`, and `dns` were refreshed with focused 2026-06-18 runs against the current local build; `buffer` was refreshed on 2026-06-20 after the C.3 iterator-surface slice; `stream` and `crypto` were refreshed on 2026-06-22 after the hash-stream pipe/read-stream/eventNames/addAbortSignal slices. The remaining rows are from the 2026-06-16 release-baseline probe and should be refreshed before using their exact counts in a release decision.
...
| stream | 213 | 67 | 31% | 0 | Basic pipe/read-stream/PassThrough/eventNames/addAbortSignal fixes landed; **the linchpin still has 146 failing** |
...
146 of 213 stream tests still fail directly on this after the narrow 2026-06-22 pipe/read-stream/eventNames/addAbortSignal slices, and it blocks everything in RC2.
...
Estimated ~2,000-2,500 LOC. **stream 67 -> ~120 (+53)**, and it unblocks Track B.
