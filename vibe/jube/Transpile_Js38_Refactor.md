...
2. **A3 — `JsClass` enum.** T1+T2+T3a+T3b+T3c DONE: `JsClass` enum (50 entries covering all class names currently written via `__class_name__` in `lambda/js/`), `uint8_t js_class` byte on `TypeMap`, clone-aware writer (`js_class_stamp` + public `js_typemap_clone_for_mutation_pub`), pure-inline reader (`js_class_get`), name→enum lookup (`js_class_from_name`). T3a: built-in prototypes stamped at lazy-prototype creation in `js_runtime.cpp` (covers Array/String/Number/Boolean/Object/Function/Date/RegExp/Error*/Symbol/Promise) plus `%TypedArray%.prototype` in `js_globals.cpp`. T3b: instance constructors in `js_globals.cpp`/`js_runtime.cpp`/`js_clipboard.cpp`/`js_string_decoder.cpp`/`js_net.cpp`/`js_tls.cpp`/`js_http.cpp`/`js_url_module.cpp`/`js_events.cpp`/`js_event_loop.cpp`/`js_canvas.cpp`/`js_formdata.cpp`/`js_dom_events.cpp`/`js_cssom.cpp` all dual-write byte alongside legacy string. T3c: `js_proto_class_method_dispatch` refactored — class identity resolved once (typed byte first, legacy `__class_name__` strncmp as fallback for any unstamped prototype), then dispatched through unified `js_proto_class_dispatch_for_class(JsClass, key, len)` table covering Boolean/String/Array/Number/Date/RegExp. No more dual-table maintenance. Outstanding work (A3-T4+, multi-session):
