...
| §4 test262 build-flag gate (registry side only) | 15 framework helpers (`js_assert_*`, `js_test262_*`, `js_donotevaluate`, `js_verify_property`, `js_decimal_to_percent_hex_string`, `js_validate_native_function_source`, `js_compare_array`, `js_is_constructor`) wrapped in `#if JS_TEST262_FAST_PATHS`. Macro defaults to 1; `-DJS_TEST262_FAST_PATHS=0` (or `#undef`) gates them out. **Update:** the transpiler-side gate has since landed — every catalog symbol's definition and emit site now sits inside `#if JS_TEST262_FAST_PATHS`, and `js_mir_expression_lowering.cpp` compiles clean at both `=0` and `=1`. `js_validate_native_function_source` was dropped from the catalog entirely: its emit site had been removed, leaving a registered-but-never-called import. | 0 in default build; **−14 in `JS_TEST262_FAST_PATHS=0` build** |
