...
**Explicitly OUT of scope for this plan:** the tier-2 worker tier (K18/K29 wire), Stage A/B threading (K15), streams integration (K21–K28, P8), supervision, binary Mark. Each is a follow-on with its design already recorded.

> **Amendment 2026-07-26 (K31).** The follow-on formerly written here as "the process tier (`start process(...)`)" is now the **worker tier**: `start worker(spec, isolation: 'thread' | 'process')`, one API, user picks. Nothing in the tier-1 work recorded below changes — handles, mailboxes, `wait`/`send`/`receive`/`select`/`cancel`, and the K13 capture rule were already tier-neutral. The follow-on plan gains two items it did not have: the **per-isolate runtime-state audit** (gate for thread isolation) and the **JubeHostAPI thread-isolation-safety marker** (unretrofittable). See `Lambda_Design_Concurrency.md` §10.3.1–§10.3.2 and §10.6.
> **Amendment 2026-08-19 (K12v2/K31v2; S13.1.1v2).** The contextual keyword recorded below was retired. The current surface is the ordinary-call system `pn` `start(target, args = [], options = {})`; `{mode: 'task' | 'thread' | 'process'}` replaces the worker wrapper and isolation-specific syntax. The implementation still lowers the resolved builtin to `AstStartNode`, so the tier-1 ownership, K13 capture rule, and runtime recorded here are unchanged. Thread/process modes remain follow-on work and are rejected explicitly today.
...
**Design anchors:** K1 (`fn` never suspends) · K2-R (may-await state machines, Phase-6 family) · K12v2/S13.1.1v2 (`start` ordinary-call system `pn`; no `async`/`await` keywords) · K13 (capture rule) · K14 (state machines because of thread-portability) · K16 (no `async` in grammar; uniform-Promise membrane) · K17 (build Lambda transform first; extract shared core after two working clients) · K20a–e (mailbox semantics) · **K30a–f (scoped tasks, cancellation as `T^E` at park points, `cancel` as seventh builtin)** · K3/K10 (one libuv loop shared with JS) · O9/O10 (the two specs this plan must write before coding).
