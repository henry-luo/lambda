...
| **L2** | Statement & variable | declarations, assignment, control flow, loops/for, try/raise, yield/await — variables given particular care |
| **L3** | Binding & pattern | declarators, params, destructuring patterns (fills the level gap — binding targets cut across L2/L4/L5 and deserve their own layer) |
| **L4** | Function & closure | fn/pn, arrow/async/generator flags, methods (extending `AST_FUNC`), captures |
| **L5** | Class & interface | class declarations, fields, interfaces resolving to `TypeObject` |
...
**Governing rule (user decision, U19): semantically unified, syntax variance preserved as fields.** At the node-kind level the catalog is as few and as unified as possible — constructs that are semantically reducible to one another are **one core node kind**, with a *form field* preserving which surface syntax was written. Examples: one `AST_LOOP` for while / do-while / until / C-style for (`LoopForm` field); one `AST_MATCH` for match / switch / case-when (`MatchForm` field, per-arm `fallthrough`); one `AST_ASSIGN` whose `op` field carries all compound forms. This gets the small kernel *without* desugaring's rewrite hazards — nothing like `a[f()] += 1` is ever expanded in the tree; the form field keeps single-evaluation and codegen choices (jump tables, test-after loops) explicit in one node's lowering. Dump fidelity is retained because dumps print the form.

Per-language variance on core nodes is expressed in exactly three ways, in order of preference:

1. **Fields/flags on the shared struct** — including the *form fields* above, plus e.g. `FuncFlags{is_proc, is_arrow, is_async, is_generator, …}` covering fn/pn *and* JS function kinds in one node; `CatchNode.type_filter` is NULL for JS but carries Python's `except TypeError` / Ruby's `rescue TypeError`; `Declarator.error_name` carries Lambda's `let a^err`. Cheap, uniform, and shared passes see through it.
...
| `AST_ASSIGN` | `Operator op (incl. compound); target; value; lhs_parenthesized` *(var.)* — usable in expr (JS, Py walrus) and stmt position (§L2) | `AST_NODE_ASSIGN`, `ASSIGN_STAM`, `INDEX_ASSIGN_STAM`, `MEMBER_ASSIGN_STAM` (target = ident/member/pattern); since P1b, Lambda declarations use `AST_NODE_VARIABLE_DECLARATOR` rather than this tag | `ASSIGNMENT_EXPRESSION` | assignment, `:=` |
...
| `AST_MATCH` | `MatchForm {match, switch}` *(form)*`; scrutinee; arms; label?` — one node per U19; usable as expr or stmt | `AST_NODE_MATCH_EXPR` | `SWITCH_STATEMENT` (form=switch) | Py `match`, Rb `case/in` + `case/when`, Bash `case` |
| `AST_MATCH_ARM` | `pattern-or-test; guard` *(var.)*`; body; fallthrough` *(form=switch)* | `AST_NODE_MATCH_ARM` | `SWITCH_CASE` | arm with guard |
...
| `AST_TRY`                           | `block; AST_CATCH_ARM* arms; else_body?; finalizer` — **one multi-arm node mirroring `AST_MATCH`** (U23): block as the scrutinee, catch arms with type filters, plus Py/Rb `else` and `finally/ensure` slots; JS is the single-arm case | — (Lambda uses `T^E`; never produced) | `TRY`+`CATCH`+`FINALLY` (one arm)    | `try/except*N*/else/finally`, `begin/rescue*N*/else/ensure` |
| `AST_CATCH_ARM`                     | `param (L3 pattern); type_filter` *(var.— Py/Rb typed excepts)*`; guard?; body; next`                                                                                                           | —                                     | `CATCH_CLAUSE`                       | `except T as e` / `rescue T => e` |
| `AST_YIELD`                         | `argument; delegate` — statement-level per U24 (usable in expression position where the language allows, like `AST_ASSIGN`)                                                                     | — (Lambda `start`/streams lower differently per K-plan) | `YIELD_EXPRESSION`  | yield / yield from                |
| `AST_AWAIT`                         | `argument` — statement-level per U24, expression-position capable                                                                                                                               | —                                     | `AWAIT_EXPRESSION`                   | await                             |
| `AST_IF`, `AST_ASSIGN`, `AST_MATCH` | shared with L1 — same node in statement position (`AST_IF` carries an `IfForm {if, unless, ternary}` form field per U19; `unless` may alternatively be builder-normalized by swapping branches) |                                       |                                      |                                   |

**Labels are a core *field*, not a node** (user decision): `String* label` lives on the labelable core statements — `AST_LOOP`, `AST_FOR`, `AST_BLOCK`, `AST_MATCH` — and `break`/`continue` carry the target label. There is no `LABELED_STATEMENT` wrapper node; the JS builder folds `label: stmt` into the statement's field, and for the rare legal-but-exotic cases (labeled `if`, labeled expression statement) it normalizes by wrapping in a labeled `AST_BLOCK` — semantically equivalent, since `break label` continues after the labeled statement either way. Lambda doesn't have labels today but the core field is there if it ever wants them; Python/Ruby builders never set it.
...
| `AST_METHOD` **: AST_FUNC** | extends `AST_FUNC` with `key; MethodKind {method, ctor, getter, setter}; is_static; is_private; computed` *(var.)* — a method **is** a function node (U25), so every shared function pass (captures, inference, resumable transform) sees methods with zero special-casing; `AST_CLASS` members reference these directly | `TypeMethod` surface | `METHOD_DEFINITION` (flattened — no wrapper around a separate function) | def in class |
...
(`AST_YIELD`/`AST_AWAIT` are statement-level nodes — see the L2 table, per U24. Their per-function *counts* still live here in `FnAnalysis`, feeding the resumable-function transform.)
...
| Core node       | Fields                                                                                                                      | Lambda maps                                | JS maps                                | Py/Rb maps            |
| --------------- | --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ | -------------------------------------- | --------------------- |
| `AST_CLASS`     | `String* name; bases (list — single for JS, multiple for Py); members (`AST_METHOD`/`AST_FIELD`); clauses` *(tier 2 — decorators, Lambda constraints)* | `AST_NODE_OBJECT_TYPE` (with methods/base) | `CLASS_DECLARATION`/`CLASS_EXPRESSION` | class                 |
...
| Iteration protocols (JS Symbol.iterator vs Lambda ranges/collections vs Py `__iter__`) | `AST_FOR` core node, profile-dispatched iterator emission (`jm_emit_get_iterator` generalizes) |
...
- **Ruby:** blocks/procs map onto the shared closure/scope-env model; method dispatch is profile `emit_member_get`/`emit_call`; `case/in` and `case/when` both land on `AST_MATCH` (form field).
...
**Live status, 2026-08-28:** the numbered phases below are the historical structural migration plan. Common node storage, aliases, `FnAnalysis`, `AstIndex`, pass-fact scaffolding, `MirValue`, and `MirEmitter` are partially landed, but core tag layouts, binding identity, pass ownership, and expression contracts are not yet authoritative across both languages. `vibe/Lambda_Design_JS_Unified.md` is the active implementation proposal; `vibe/impl/Lambda_Impl_Tune_Ast (retired).md` preserves the earlier partial evidence. The old phase text remains design provenance, not the current checklist.
...
(Node names in earlier entries reflect the catalog at the time of decision — `AST_FOR_ITER`→`AST_FOR` and `AST_CATCH`→`AST_CATCH_ARM` were renamed by U22/U23; `AST_SWITCH` was absorbed into `AST_MATCH` by U19.)
...
| **U4**  | One superset `Operator` enum; operator *semantics* dispatch through the LangProfile                                                                                                                                                                                                                                                                                                                                                                                                                                                               | **confirmed** |
...
| **U11** | C2MIR/`transpile.cpp` **kept as-is** — Lambda-only debug backend, frozen during migration (only the mechanical enum update touches it)                                                                                                                                                                                                                                                                                                                                                                                                            | **confirmed** |
...
| **U14** | fn/pn purity is a **core flag** on `AST_FUNC` (guest builders mark all-effectful); the one-bit effect system rides the shared spine rather than staying a Lambda-range concept                                                                                                                                                                                                                                                                                                                                                                    | **confirmed** |
...
| **U19** | **Semantically unified, syntax variance as form fields** (Q1 resolution): semantically-reducible constructs are ONE core node kind with a *form field* preserving surface syntax — `AST_LOOP{LoopForm}` absorbs while/do-while/until/for(;;), `AST_MATCH{MatchForm}` absorbs match/switch/case-when (per-arm `fallthrough`), `AST_IF{IfForm}`, `AST_ASSIGN.op`. No tree rewriting (desugar hazards never arise); form keeps single-evaluation and codegen choices (jump tables, test-after) explicit in one node's lowering; dumps print the form | **confirmed** |
| **U20** | **`FnAnalysis` extension = union of typed pointers** (Q2 resolution): `union FnExt { JsFnExt* js; LambdaFnExt* lm; PyFnExt* py; }` with forward `typedef`s only — core header never sees profile struct contents (no rebuild-the-world, no size bloat: one pointer), code reads `fa->ext.js` with natural syntax; supplemented by an `ext_kind` debug tag + profile `dump_fn_ext` hook for dump completeness                                                                                                                                      | **confirmed** |
| **U21** | **Interleaved sequencing with concurrency** (Q3 resolution): Phase 0 (MirEmitter + G1 rooting) first — prerequisite of both tracks; then concurrency Stage A ∥ unified-AST Phases 1–3; Phase 4 extracts the resumable-function transform with two green clients (JS + Lambda Stage B) per K17; Stage B ships on the shared transform                                                                                                                                                                                                              | **confirmed** |
| **U22** | **`AST_FOR` is its own core node, separate from `AST_LOOP`**: other languages treat for/while as near-siblings, but in Lambda they differ fundamentally — different *color* (for-expr is pure/fn; condition loops are pn-only) and deep iteration semantics (iterator protocol, pattern target, clause family shared with `AST_FOR_EXPR`). `for_c` (C-style `for(;;)`) stays a `LoopForm` of `AST_LOOP` — it is a condition loop with init/update, and Lambda never produces it                                                                    | **confirmed** |
| **U23** | **`AST_TRY` is one multi-arm node mirroring `AST_MATCH`**: `{block; AST_CATCH_ARM* arms (param, type_filter, guard, body); else_body?; finalizer}` — Python `try/except*/else/finally` and Ruby `begin/rescue*/else/ensure` are natively multi-arm; JS is the single-arm case; Lambda never produces it (`T^E`)                                                                                                                                                                                                                                   | **confirmed** |
...
| **U26** | ~~**No AST-walking execution interpreter** (§12): MIR-interp mode (`g_mir_interp_mode`) stays the no-JIT execution path; a **const-folder over the pure L1 core subset** (profile-dispatched `fold_*` hooks, fn/pn purity as the soundness gate) is added to the shared analysis passes; a slow reference interpreter is **KIV** — future, and only for semantic analysis / executable-spec goals MIR cannot serve, never wired into `eval`/REPL execution~~ **Superseded 2026-08-15 (user ruling):** a boxed AST interpreter is now the default tier with per-function MIR promotion — `vibe/Lambda_Design_Ast_Interpreter.md` AI1–AI22, **D8.1.1v2**; the const-folder survives as that engine's CONST mode (AI16) and the KIV reference interpreter is subsumed by T0 as the continuously-gated executable spec (AI18) | ~~confirmed~~ **superseded** |
...
| **U34** | **D8.6.4v2 requires at least 2,000 net physical C/C++ runtime LOC deleted** from the anchored `lambda/runtime` + `lambda/js` scope (new shared code included; moving code out of scope invalidates the gate). The 2026-08-12 anchor is `e66e5b5c71bc7ee7fe2d1e2b2a9afe27dc6825a3`, measured at 319,606 lines; candidate must be ≤317,606. Reformatting/comment stripping is not accepted as the reduction rationale | **confirmed** |
| **U35** | **D8.6.4v2 optimizes measured work before cache complexity**: eliminate repeated walks, linear rediscovery, duplicate lowering, temporaries, boxing, and excess emission first. D8.5 caches/lazy compilation are optional only after phase attribution proves the remaining gate cannot be reached directly; cached/skipped samples are reported separately and cannot silently disappear from the corpus                                                                             | **confirmed** |
| **U36** | **D8.6.4v2 keeps finalized MIR volume numeric and review-visible without making it an exit gate**: Phase 0 freezes the discovered `lib_*` plus `underscore_lib` large-library manifest and the complete JS corpus; the batch protocol prints per-test module/function/instruction counts using MT7's instruction definition, reports aggregate and per-library deltas, and requires attribution of material growth                                                                 | **confirmed** |

## 10. Resolved Questions (decision record)

All major open questions are now resolved by user decision; the trade-space analysis is kept here as the rationale record.

### Q1 — resolved: **semantic unification with form fields** (U19)

The original dilemma was "preserve surface forms as distinct nodes vs desugar to a kernel." Preservation's virtues: desugaring is where semantic bugs live and is not language-independent (`a[f()] += 1` must evaluate `a[f()]` once; correct expansion needs temporaries whose interaction with evaluation order/TDZ/coercion differs per language); codegen prefers original shapes (`switch`→jump table, `do-while` without synthetic flags); dumps/goldens map 1:1 to source. The kernel's virtue: fewer node kinds in every shared pass.

**The user's hybrid takes both:** semantically-reducible constructs collapse into ONE node *kind* — but the surface variation is preserved as a **form field**, never by rewriting the tree. `AST_LOOP{LoopForm{while,do_while,until,for_c}}`, `AST_MATCH{MatchForm{match,switch}}` (+ per-arm `fallthrough`), `AST_IF{IfForm}`, `AST_ASSIGN.op`. The desugar hazards never arise (no expansion happens); the kernel benefit arrives (one loop case per pass, not four); codegen switches on form inside one node's lowering; dumps print the form. Builder-level *mapping* (Python comprehension → `AST_FOR_EXPR` clauses) remains sanctioned as before.

### Q2 — resolved: **union of typed pointers** (U20)

The dilemma was opaque `void*` (decoupled, unchecked) vs inline tagged union (checked, but core header depends on every profile's struct contents and is sized to the largest member — unacceptable given the size of the JS ext). **The user's hybrid:** a union of *pointers* with forward `typedef`s only:

```c
typedef struct JsFnExt JsFnExt;        // forward decl only — contents stay in the JS profile
typedef struct LambdaFnExt LambdaFnExt;
typedef struct PyFnExt PyFnExt;
union FnExt { JsFnExt* js; LambdaFnExt* lm; PyFnExt* py; };
```

Core header never sees struct contents (no rebuild-the-world; adding a guest = one forward typedef + one union member); size is one pointer regardless of profile-struct size; call sites read `fa->ext.js` with natural, compiler-visible typing. Supplemented by an `ext_kind` debug tag (asserts the right member is read — the members are type-distinct but bit-identical) and a `dump_fn_ext` profile hook so AST dumps stay complete.

### Q3 — resolved: **interleaved sequencing** (U21, per recommendation)

Phase 0 (MirEmitter + G1 honest rooting) first — the shared prerequisite of both this project and concurrency (Stage A runs JIT'd frames concurrently; it must not be built on the blanket-rooting hack). Then concurrency Stage A ∥ unified-AST Phases 1–3 (low file contention: Stage A works against the emitter API in the lowering file; Phases 1–3 live in ast-core/builders/analysis). Phase 4 extracts the resumable-function transform with two green clients (JS async/generators + Lambda's Stage-B transform), honoring K17's two-client rule; Stage B ships on the shared transform.

### Q4 — deferred by recommendation: the "views" fourth variance tier

From CodeQL (§11.3): language-range nodes implementing core *interfaces* (Lambda `PIPE` exposing a call-like view) so shared passes handle them without promotion. Adopt only when a real shared pass asks for it.
...
## 12. Considered and Rejected — a Unified AST Interpreter (U26)

> **Superseded 2026-08-15 (user ruling).** The rejection recorded below is reversed: a boxed AST-walking interpreter is now the **default execution tier (T0)** with per-function MIR promotion — see `vibe/Lambda_Design_Ast_Interpreter.md` (AI1–AI22) and **D8.1.1v2**, whose §1.1 answers §12.2's counter-arguments point-by-point with post-U26 evidence (MIR_link at 50–82% of compile on vendor libraries, generated code at 0.6% of working CPU, the REPL's O(n²) whole-history recompile, Radiant's forced-interp and skip-oversize budgets). §12.3's const-folder is absorbed as that engine's CONST mode (AI16); the KIV reference interpreter is subsumed by T0 as the continuously-gated executable spec (AI18). The analysis below is retained unchanged as the historical record.

With one common AST, the natural question arose: should we also build one AST-walking interpreter, replacing MIR-interp as the "interpret if needed" path? **Decision: no execution interpreter. Keep MIR-interp; add a const-folder to the shared passes; KIV a reference interpreter for non-execution goals only.**

### 12.1 The case for (arguments considered)

1. **Unification dividend** — a unified AST makes a unified interpreter *possible* for the first time; one tree-walker could serve every front-end.
2. **Skip lowering** — interpreted execution would not need to transpile to MIR at all.
3. **`eval("...")`** — dynamically-evaluated code could run without invoking the JIT pipeline.
4. **Compile-time const-expr evaluation** — the AST builder could evaluate constant expressions during transpiling.
5. **REPL** — interactive lines could be interpreted directly instead of compiled.

### 12.2 The counter-arguments

**The core objection: it reintroduces the duplication this design exists to eliminate.** The economic case of §7 is collapsing ~860 KB of parallel *lowering* code into one shared driver + LangProfile. An AST interpreter re-opens a parallel execution path — and it multiplies by language, because semantics live in the profile. `LangProfile` (§4.3) is defined entirely as MIR-emission hooks (`lower_binary`, `emit_truthy_test`, `emit_member_get`, `emit_error_check`); an interpreter needs a second, parallel vtable of *direct-execution* hooks for every one of those micro-decisions, per language (JS coercing `==`, prototype walks, TDZ, completion values; Lambda structural equality, `T^E` propagation, pipes/queries). Every future semantic change lands twice; every test corpus must run in both modes or divergence creeps in silently. That is the classic two-implementations-of-semantics trap, on a new axis (interpreter × JIT) just as the language × language axis is being closed. It is also mistimed: Phases 0–6 ∥ concurrency Stage A already keep multiple clients green through heavy churn; a third executor would tax every phase gate.

Point-by-point:

1. *Possible ≠ needed.* The unified AST makes an interpreter exactly as possible as the unified transpiler — which is the client the design was built for.
2. *Lowering is not the cost to avoid.* MIR-direct transpile is a single-pass tree walk emitting instructions — the same order of work as one interpretive pass. A no-JIT execution mode already exists: `g_mir_interp_mode` routes both Lambda and JS through `MIR_set_interp_interface` (transpile-mir.cpp:14030 and all `js_mir_*` link sites), covering the W^X/no-executable-memory platform case that classically justifies tree-walkers. After transpile, MIR-interp amortizes over every re-execution of a loop body; a tree-walker pays full price every pass. The AST interpreter would be the *slowest* of the three execution options for anything but run-once straight-line code.
3. *`eval` via the pipeline is a feature.* Eval'd code compiled through the same pipeline is guaranteed to mean the same thing as static code; JS eval already has working MIR lowering with env-frame access to enclosing scope (`js_mir_eval_lowering.cpp`, §6.4). An interpreter would additionally need visibility into JIT'd frames' natively-typed *registers* (not just heap scope-envs) — a deopt/materialization problem we currently don't have. The one real cost of eval-via-JIT — MIR module/code accumulation for ephemeral evals — is a lifecycle/disposal fix in one place.
4. *The genuine gap — but it wants a folder, not an interpreter.* See §12.3.
5. *The REPL's cost is elsewhere.* The REPL re-runs the entire accumulated history each line (main.cpp `run_repl` → `run_script_mir(repl_history)`); interpreting that history per line would be strictly worse. The real improvement is incremental top-level state (compile only the new line against persisted module state) — orthogonal to execution strategy, and a natural companion of the unified global-vars BSS work (§5.3/§6.3).
