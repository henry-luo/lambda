...
**Clarified (USER, 2026-09-26, spec 47.2.0).** Two points of the ruling's wording, neither a change of meaning:
- *"Literal operands decide" includes containers.* A container reads as the pattern its literal spells (S10.1.1v2), so `[1] & [2]`, `[int] & [int, int]` and `{a: 1} & {a: 2}` are `none` like `1 & 2`. The first implementation listed scalar values only and left them unreduced; that was a gap, not the ruling.
- *The `int & string` sentence is not a ruling.* It described what the implementation does — it eagerly reduces only operations that involve literals — not what the semantics requires: "a good impl should detect `int & string` and alarm and even reject such writing." The spec keeps it as an informative remark, and Appendix A carries the status.
...
**Implementation and residue** (spec Appendix A, S11.1.7 row). The reduction is one three-valued function: does one operand admit all, none or only some of a literal operand's values? A scalar literal is asked by value, a range by its bounds, and a container pattern part by part against a pattern of its kind, or by kind alone against any other type. Anything that needs a predicate, a string pattern or a run answers *unknown* and never reduces, and so does anything `is` could answer otherwise. Two older defects surfaced and were fixed with it: type equality compared a payload tag, so `number == integer` held; and `is` tested a bare numeric literal type by its tag. `!none` reduces to `any` in every position. Structural type equality beyond the compact meta types followed later the same day ([LR03-29](<Lambda_Issue_Ledger (fixed).md#lr03-29>)); container patterns and function types still compare by kind ([LR03-33](Lambda_Issue_Ledger.md#lr03-33)). A one-literal alias `type T = 1`, which was not a type value, was fixed the same day ([LR03-30](<Lambda_Issue_Ledger (fixed).md#lr03-30>)), and so was a bool literal type that carried no value ([LR03-31](<Lambda_Issue_Ledger (fixed).md#lr03-31>)).

*A wrong turn, recorded.* The first reduction asked a numeric literal's own value only, and so it was unsound: a literal admits every value `==` to it (S11.2.1), `1.0` included for `1`, so `1 ! int` is not empty, yet it reduced to `none`, and `t'2025-01-01' ! date` did the same across datetime precisions ([LR03-32](<Lambda_Issue_Ledger (fixed).md#lr03-32>)). A kind that holds only some of a literal's values now answers *unknown*. The validator had the mirror defect, and `1.0 is (1 | 2)` was `false` ([LR13-13](<Lambda_Issue_Ledger (fixed).md#lr13-13>)). Two validator facts bound what containers decide. It admits every array into `[]` ([LR13-14](Lambda_Issue_Ledger.md#lr13-14)). It never requires an element attribute, so `<p> is <p a: 1>` holds and `<p a: 1> & <p a: 2>` is not empty. Whether an attribute pattern should require its attribute is unruled.
