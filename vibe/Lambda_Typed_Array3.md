...
> **Status:** design proposal with Phase 3a, the Phase 3b substrate slice, Phase 3c, and Phase 3d implemented through structural storage-field deletion. `ELEM_UINT8_CLAMPED` now exists as the 15th `ArrayNum` kind, `ArrayNum` can describe a writable external byte-buffer view with a raw `Container*` lifetime base, and each JS `JsTypedArray` owns a GC-marked `ArrayNum* view` descriptor over its non-moving `JsArrayBuffer` bytes. JS number typed-array scalar reads/writes, non-Buffer same-type byte-preserving bulk operations, cross-type number conversions, and equal-width BigInt bulk copies all route through the current `ArrayNum` view when the descriptor is valid. The old raw-fast fallback flag and duplicate raw-storage dispatch paths are retired; external byte-oriented consumers now go through typed-array accessors instead of reading `JsTypedArray` storage fields directly. Standalone typed arrays now allocate a `JsArrayBuffer` substrate immediately, so `JsTypedArray::data`/`length`/`byte_length`/`byte_offset` have been deleted; stored geometry lives in `ArrayNumShape`, and live length/data/offset are derived from the descriptor plus backing buffer state. Node `Buffer` remains JS-observable and intentionally does not delegate its instance semantics to `ArrayNum`.

---

## 0. TL;DR

- The storage refactor is now in place inside the JS layer: `JsTypedArray` uses an `ArrayNum` view over `JsArrayBuffer` bytes. What still does **not** exist is the embedding interop bridge — no public Lambda `ArrayNum` value can yet be handed to JS as a typed array, and no JS typed array can yet be consumed by Lambda as an `ArrayNum`. That remaining bridge is the payoff phase and is not a JS-observable semantics change.
...
## 2. Current state — shared typed-array descriptor, interop still absent
...
```c
// lambda/js/js_typed_array.h:54-61
...
- **Allocation is malloc-backed and non-moving:** every typed array now has a `JsArrayBuffer` substrate. Standalone typed arrays allocate that substrate up front; `ArrayNum` is a mutable external view over the buffer bytes. The bytes are still `mem_alloc` / `mem_calloc` with `MEM_CAT_JS_RUNTIME`, **not** the GC data zone.
- **Finalization is explicit:** `gc_finalize_arraybuffer` frees the byte storage; `gc_finalize_typed_array` frees only the native typed-array shell. The typed-array native trace path marks both the identity-preserving `buffer_item` and the `ArrayNum* view`, so the descriptor and substrate survive with the JS wrapper.
- **Indexed access** routes through an inline fast path (`js_runtime.cpp:7045`) and a general "exotic gate" (`js_runtime.cpp:2894`) that enforces ECMAScript canonical-numeric-index rules; metadata (`length`/`byteLength`/`byteOffset`/`buffer`) is synthesized from the `ArrayNumShape` descriptor plus live backing-buffer state, not from cached fields on `JsTypedArray`.
...
- JS hands live raw `data` pointers to JIT code and to `memcpy`/`memmove` bulk fast paths. Those consumers are **not** updated by the GC. After a compaction, every cached JS pointer into ArrayNum-owned bytes would dangle.
...
- **`JsTypedArray`** — storage is now a single **`ArrayNum* view`** over the backing `JsArrayBuffer` bytes. Element typing, sizing, indexed get/set, and bulk copy/fill/slice delegate to `ArrayNum`; fixed byte offset/length live in `ArrayNumShape`, and length-tracking views refresh their descriptor from the live buffer. It keeps the **JS-only** fields: `element_type`, `buffer`, `buffer_item` (identity-preserving `.buffer`), `length_tracking`, and `is_buffer`. Because `JsTypedArray` is native data behind a Map, the GC marks the `ArrayNum* view` and `buffer_item` through the native trace path. A standalone (`new Uint8Array(8)`) typed array allocates a fresh non-moving `JsArrayBuffer` plus an owning-less `ArrayNum` view over it, so there is exactly one storage path.
...
What Phase 3d deleted from the JS layer: duplicated per-element-size raw load/store dispatch, duplicated same-type byte-copy/reverse/copyWithin/slice mechanics, cross-type numeric conversion loops that could be expressed through the `ArrayNum` lane helpers, and standalone typed-array byte ownership. The remaining JS code is the spec layer: coercion, exotic property behavior, detach/OOB rules, Atomics, DataView, species/prototype semantics, and Node `Buffer`.
...
**Descriptor, scalar-number, same-type bulk, and cross-type conversion paths implemented.** `JsTypedArray` carries an `ArrayNum* view`; standalone and ArrayBuffer-backed constructors create a writable external view over the same non-moving bytes, refresh its length/offset for length-tracking or detached buffers, and the GC marks the view through a JS-native Map trace callback. Number typed-array scalar reads (`js_typed_array_raw_get_item` / raw numeric load) and scalar writes (`raw_store_number`) delegate to `ArrayNum` numeric lane helpers when the current data pointer matches the descriptor; JS coercion remains in the JS layer before the lane write. Non-Buffer same-type byte-preserving paths (`set` fast path, raw same-type copy, raw reverse, reversed copy, in-range same-type `slice`, and same-type `copyWithin`) delegate to `ArrayNum` byte helpers, using raw byte copies/swaps rather than numeric get/set so float NaN payloads and signed-zero bytes are preserved. Cross-type constructor/set fast paths read numeric lanes from the source `ArrayNum` view and write through the destination `ArrayNum` view while keeping JS typed-array store coercion in `js_typed_array.cpp`; `BigInt64Array`/`BigUint64Array` conversion uses an equal-element-size `ArrayNum` byte copy, preserving lane bits. BigInt typed arrays keep the existing JS boundary conversion so `BigUint64Array` high-bit lanes still reconstruct positive BigInts. Node `Buffer` is intentionally excluded from this delegation for now because its observable behavior remains JS-layer and the parallel JS batch gate exposed teardown fragility when Buffer-heavy crypto tests used the new path. Phase 3d then deleted the duplicated raw dispatch and legacy storage fields around these descriptor-backed paths. *Test:* full `test/js/typed_arrays.*` + the `regression_js54_*` set + test262 typed-array pages.

**Phase 3d — delete the duplicated storage code.**
**Implemented, including structural field deletion.** The `LAMBDA_JS_TA_RAW_FAST` fallback gate is retired, and the raw helpers no longer fall back to their own element-size switch for scalar numeric load/store, dense numeric-array construction, same-type copy/reverse/reversed-copy, same-type `copyWithin`, same-type constructor copy, or `indexOf`; those paths now require a current `ArrayNum` descriptor and otherwise fall back to the existing JS element-by-element semantic path. Cross-type number conversion and equal-width BigInt copy remain `ArrayNum`-backed from Phase 3c. Byte-oriented callers outside `js_typed_array.cpp` (`Buffer`, fs, string decoder, zlib, clipboard, crypto, structuredClone, TextEncoder/TextDecoder, and typed-array-to-string helpers) now use public typed-array accessors instead of touching `JsTypedArray` storage fields directly, and the repeated prototype out-of-bounds checks in `js_runtime.cpp` use `js_typed_array_is_out_of_bounds_item()`.

Standalone typed arrays now allocate a `JsArrayBuffer` substrate up front and keep the identity-preserving `buffer_item` marked through the native trace path. The `JsTypedArray` struct no longer carries duplicated `data`/`length`/`byte_length`/`byte_offset` fields: fixed descriptor geometry is preserved in `ArrayNumShape`, length-tracking views refresh their descriptor length from the live backing buffer, detached/OOB views null their data pointer, and `gc_finalize_typed_array()` no longer owns byte storage. *Test:* `make build-test`; `test/js/typed_arrays`; `regression_js54_*`; exact Test262 slice same-buffer-offset regression; Buffer/crypto/zlib/string-decoder Node prelim smoke; direct `js-test-batch` smoke for `opt_p4_typed_reads`; `make test262-baseline` (40261/40261, 0 regressions).
...
- **The cached-pointer hazard (primary).** Any JS consumer caching a raw `data` pointer across a potential GC must be audited. Because the substrate stays malloc-backed (non-moving) and the `ArrayNum` is only a *view* (its data isn't relocated), the hazard is contained — but the migration must verify no path accidentally routes JS bytes through the moving data zone. Mitigation: standalone arrays now use the same `JsArrayBuffer` substrate as buffer-backed views, while the live pointer is still derived from non-moving buffer bytes.
...
- **Performance.** The same-type byte-preserving paths must remain bulk byte copies (now via `ArrayNum` helpers), and indexed `ta[i]` must stay on the inline helper path. Keep a small release-build perf gate for indexed numeric reads/writes and same-type bulk copy against the pre-migration baseline.
...
1. **Pin-clearing for Lambda→JS** — TA2 defers pin-clearing. If pinned interop proves common, do we revisit view-count tracking on the base to clear `is_pinned` when the last JS view dies? (Deferred until there's evidence.)

---

## 13. Success criteria

- `ArrayNum` covers all 11 JS element kinds (incl. clamped), and it can describe mutable external views over non-moving byte buffers; `make test-lambda-baseline` stays green through 3a–3b. **Achieved.**
- `JsTypedArray` storage is a single `ArrayNum` view over `JsArrayBuffer` bytes; duplicated raw-storage dispatch is removed from scalar, same-type bulk, cross-type conversion, and byte-consumer paths; `JsTypedArray::data`/`length`/`byte_length`/`byte_offset` are deleted with **zero** JS test regressions. **Achieved through Phase 3d.**
