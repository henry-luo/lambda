...
> **Status:** design proposal with Phase 3a, the Phase 3b substrate slice, and the Phase 3c descriptor, scalar-number, and byte-preserving same-type bulk slices implemented. `ELEM_UINT8_CLAMPED` now exists as the 15th `ArrayNum` kind, `ArrayNum` can describe a writable external byte-buffer view with a raw `Container*` lifetime base, and each JS `JsTypedArray` now owns a GC-marked `ArrayNum* view` descriptor over its existing byte storage. JS number typed-array scalar reads/writes use `ArrayNum` numeric lane helpers when the descriptor is current, and non-Buffer same-type bulk copy/reverse/slice/copyWithin paths now use `ArrayNum` byte helpers so float payload bytes are not canonicalized. BigInt typed arrays, Node `Buffer`, cross-type conversion loops, and the remaining JS storage fallbacks still stay on the JS raw-byte path. Full JS raw-path replacement is still pending. The Typed Array 2 work (N-D, views, `is_view`/`is_pinned`, the compactor view-skip, the `base: Container*` field) was built with this migration in mind and is the load-bearing prerequisite — it is now in place.
