...
> **Status:** design proposal with Phase 3a, the Phase 3b substrate slice, Phase 3c, and the Phase 3d duplicated-dispatch cleanup implemented. `ELEM_UINT8_CLAMPED` now exists as the 15th `ArrayNum` kind, `ArrayNum` can describe a writable external byte-buffer view with a raw `Container*` lifetime base, and each JS `JsTypedArray` now owns a GC-marked `ArrayNum* view` descriptor over its existing byte storage. JS number typed-array scalar reads/writes, non-Buffer same-type byte-preserving bulk operations, cross-type number conversions, and equal-width BigInt bulk copies all route through the current `ArrayNum` view when the descriptor is valid. The old raw-fast fallback flag and duplicate raw-storage dispatch paths are retired; external byte-oriented consumers now go through typed-array accessors instead of reading `JsTypedArray::data` directly. Node `Buffer` remains JS-observable and intentionally does not delegate its instance semantics to `ArrayNum`. The remaining structural cleanup is to choose the standalone typed-array ownership substrate and then delete the now mostly metadata-only legacy fields (`data`/`length`/`byte_length`/`byte_offset`) from `JsTypedArray`.
...
**Duplicated dispatch cleanup implemented; final ownership-field deletion split out.** The `LAMBDA_JS_TA_RAW_FAST` fallback gate is retired, and the raw helpers no longer fall back to their own element-size switch for scalar numeric load/store, dense numeric-array construction, same-type copy/reverse/reversed-copy, same-type `copyWithin`, same-type constructor copy, or `indexOf`; those paths now require a current `ArrayNum` descriptor and otherwise fall back to the existing JS element-by-element semantic path. Cross-type number conversion and equal-width BigInt copy remain `ArrayNum`-backed from Phase 3c. Byte-oriented callers outside `js_typed_array.cpp` (`Buffer`, fs, string decoder, zlib, clipboard, crypto, structuredClone, TextEncoder/TextDecoder, and typed-array-to-string helpers) now use public typed-array accessors instead of touching `JsTypedArray::data`/`byte_length` directly, and the repeated prototype out-of-bounds checks in `js_runtime.cpp` use `js_typed_array_is_out_of_bounds_item()`.

The remaining deletion is structural rather than behavioral: standalone typed arrays still allocate and own malloc-backed bytes through `ta->data`, and `gc_finalize_typed_array()` frees that storage when there is no backing `JsArrayBuffer`. Removing `data`/`length`/`byte_length`/`byte_offset` from the struct therefore requires the next prerequisite decision: make standalone typed arrays always use a `JsArrayBuffer` substrate, with the `ArrayNum` view becoming the only storage descriptor. *Test:* `make build-test`; `test/js/typed_arrays`; `regression_js54_*`; exact Test262 slice same-buffer-offset regression; Buffer/crypto/zlib/string-decoder Node prelim smoke; `make test262-baseline` (40261/40261, 0 regressions).
