...
**Descriptor, scalar-number, same-type bulk, and cross-type conversion paths implemented.** `JsTypedArray` now carries an `ArrayNum* view` alongside the legacy storage fields; standalone and ArrayBuffer-backed constructors create a writable external view over the same non-moving bytes, refresh its length/offset for length-tracking or detached buffers, and the GC now marks the view through a JS-native Map trace callback. Number typed-array scalar reads (`js_typed_array_raw_get_item` / raw numeric load) and scalar writes (`raw_store_number`) delegate to `ArrayNum` numeric lane helpers when the current data pointer matches the descriptor; JS coercion remains in the JS layer before the lane write. Non-Buffer same-type byte-preserving paths (`set` fast path, raw same-type copy, raw reverse, reversed copy, in-range same-type `slice`, and same-type `copyWithin`) now delegate to `ArrayNum` byte helpers, using raw byte copies/swaps rather than numeric get/set so float NaN payloads and signed-zero bytes are preserved. Cross-type constructor/set fast paths now read numeric lanes from the source `ArrayNum` view and write through the destination `ArrayNum` view while keeping JS typed-array store coercion in `js_typed_array.cpp`; `BigInt64Array`/`BigUint64Array` conversion uses an equal-element-size `ArrayNum` byte copy, preserving lane bits. BigInt typed arrays keep the existing JS boundary conversion so `BigUint64Array` high-bit lanes still reconstruct positive BigInts. Node `Buffer` is intentionally excluded from this delegation for now because its observable behavior remains JS-layer and the parallel JS batch gate exposed teardown fragility when Buffer-heavy crypto tests used the new path. Phase 3d follows by deleting the duplicated raw dispatch around these descriptor-backed paths. *Test:* full `test/js/typed_arrays.*` + the `regression_js54_*` set + test262 typed-array pages.
