...
| 12 | Ordinary arithmetic formerly normalized every compact sized integer to `int64`, kept `int64 + int` in `int64`, and selected `uint64 → int64/decimal` by comparing the runtime value with `INT64_MAX` | **Fixed — W10 landed 2026-07-20:** `lambda-number.hpp` supplies the shared type-directed classifier; runtime and static adapters preserve `integer` as a semantic kind; compact lanes enter `int`, every `i64/u64` enters `integer`, and full-width integer/float mixes enter `decimal`. `normalize_sized` and its magnitude branch are removed. Scalar, vector, reduction, AST, and MIR paths use the same decision. | §3.3.2, P1 |
...
- **W10** — **Done 2026-07-20:** magnitude-directed `u64` promotion is gone. Sized-integer × sized-integer integral operations stay in their selected Go-style lane; compact sized integers enter `int` and `i64/u64` enter `integer` for sized/non-sized operations and true `/`; full-width integer + float enters decimal. The evaluator, AST, MIR Direct, vectors/reductions, exact consumers, and tests share the landed classifier. The archived C2MIR source is not built or tested. Final symbols and gate evidence are recorded in `Lambda_Impl_Numbers.md`.
