...
| 7 | LambdaJS BigInt scaffolding is int64-leaning (`js_bigint_to_index`) | **Partially fixed:** BigInt non-decimal `toString(radix)` no longer truncates through `bigint_to_int64`; arbitrary-precision mpdec division now drives radix conversion, covering cases such as `(2^64-1n).toString(16)` and `BigInt.asUintN(64, -1n).toString(16)`. Wide `BigInt.asUintN`/`BigInt.asIntN` arithmetic now sizes mpdec contexts from operand digits, and non-decimal `toString(radix)` uses a sized quiet-divmod context, so widths past the default 2000-digit BigInt context are not rounded, returned as `null`, or trapped; `BigInt.asUintN(8000, -1n).toString(16)` is pinned as 2000 hex `f`s. Buffer `readBigInt64*`/`readBigUInt64*` now return JS BigInt instead of packed number values, and the matching writes consume BigInt through 64-bit modulo/signed narrowing; DataView and BigInt typed-array 64-bit edge reads/writes are pinned by the same fixture. BigInt typed-array `fill()` now wraps through the same 64-bit `BigInt.asIntN`/`BigInt.asUintN` path as indexed/DataView writes instead of clamping oversized values through `bigint_to_int64`. `fs.read*`/`readv*` BigInt positions now compare against the signed 64-bit host-position range before narrowing, so oversized positions reject instead of clamping. MIR type inference now keeps BigInt bitwise/shift results boxed instead of mislabeling them as JS Numbers, preserving cases such as `-(1n << 63n)` and the signed 64-bit minimum boundary. BigInt exponent sign checks now use BigInt negativity instead of `bigint_to_int64`, and `1n ** hugeBigInt` is pinned without host-width exponent extraction. Remaining work is the broader `js_bigint_to_index`/BigInt helper audit under W2. | §5.2/§2.2 (W2) |
...
- **W2** — Partially fixed: BigInt radix string conversion now stays on the mpdec-unlimited backing instead of truncating via `int64`, regression coverage pins the 64-bit `asUintN` edge, signed-minimum `asIntN`/bitwise-shift edge, `ToIndex` rejection for BigInt `bits`, mixed Number/BigInt throw paths, BigInt Atomics lane conversion, and a huge-exponent identity (`1n ** (1n << 63n)`) that must not consult host-width exponent conversion. Buffer/DataView/BigInt typed-array read/write APIs now preserve the JS BigInt face at the 64-bit boundary, and BigInt typed-array `fill()` now uses the same modulo-64 narrowing instead of clamp-through-`int64`. `fs.read*`/`readv*` BigInt positions now reject outside `[-1, INT64_MAX]` before host narrowing. Wider `asIntN`/`asUintN` probes above 64 bits pass in the current tree, including a >2000-decimal-digit radix-conversion probe (`BigInt.asUintN(8000, -1n)` -> 2000 hex `f`s). Remaining BigInt semantic coverage is the broader helper/conversion audit, riding the node baseline under the K28-style best-effort banner.
- **W3** — Partially fixed: the shared JS egress helper now routes raw `uint64` values to BigInt via decimal-string construction, so values above `INT64_MAX` have a lossless unsigned path. `fs.Stats`/`fs.StatFs` now make their Node API split explicit (`number` by default, `bigint` for `{ bigint: true }`) instead of letting packed host integers choose by magnitude. Node `os.totalmem()`/`freemem()` and `os.cpus()[].speed/times.*` now explicitly use JS `Number` construction instead of compact-int packing, so large host counters cannot surface as `ITEM_ERROR`. Remaining work is to route the rest of the FFI/lane boundary producers through explicit number-vs-BigInt helpers instead of ad hoc numeric casts.
...
- **W6** — Partially fixed: `js_to_number` now warns once per native caller when a regular Lambda `decimal` crosses into JS `Number` through the lossy `decimal_to_double` path, while JS BigInt (`DECIMAL_BIGINT`) still throws before any warning. The warning site table is mutex-protected, resets with the JS batch lifecycle, and the LambdaJS audit found no direct `decimal_to_double` bypass outside `js_to_number`. Remaining work: decide whether warning-by-default is strong enough in practice or promote decimal egress to error-by-default.
