...
- **W2** — Partially fixed: BigInt radix string conversion now stays on the mpdec-unlimited backing instead of truncating via `int64`, regression coverage pins the 64-bit `asUintN` edge, and Buffer/DataView/BigInt typed-array read/write APIs now preserve the JS BigInt face at the 64-bit boundary. Remaining BigInt semantic coverage (`js_bigint_to_index`, wider `asIntN`/`asUintN`, mix-throws, Atomics and less-common conversion edges) rides the node baseline under the K28-style best-effort banner.
