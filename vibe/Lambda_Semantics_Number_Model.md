...
| 6 | `int64 → JS` silent `(double)` casts (e.g. `js_fs.cpp:217`, `js_child_process.cpp`) | **Partially fixed:** raw Lambda `int64`/`uint64` values crossing the JS primitive boundary now report `typeof === "bigint"`, box with `BigInt.prototype`, reject `Number.*`/`JSON.stringify` Number paths, and canonicalize to mpdec-backed BigInt for arithmetic/equality/comparison. Remaining work is the API-by-API host audit for legacy call sites that still cast native 64-bit host fields to JS numbers instead of returning a value whose declared Lambda type matches the intended JS face. | §5.3 (int64 → BigInt always) |
| 7 | LambdaJS BigInt scaffolding is int64-leaning (`js_bigint_to_index`) | **Partially fixed:** BigInt non-decimal `toString(radix)` no longer truncates through `bigint_to_int64`; arbitrary-precision mpdec division now drives radix conversion, covering cases such as `(2^64-1n).toString(16)` and `BigInt.asUintN(64, -1n).toString(16)`. Remaining work is the broader `js_bigint_to_index`/BigInt helper audit under W2. | §5.2/§2.2 (W2) |
...
- **W2** — Partially fixed: BigInt radix string conversion now stays on the mpdec-unlimited backing instead of truncating via `int64`, and regression coverage pins the 64-bit `asUintN` edge. Remaining BigInt semantic coverage (`js_bigint_to_index`, wider `asIntN`/`asUintN`, mix-throws, typed-array/DataView edges) rides the node baseline under the K28-style best-effort banner.
- **W3** — Partially fixed: the shared JS egress helper now routes raw `uint64` values to BigInt via decimal-string construction, so values above `INT64_MAX` have a lossless unsigned path. Remaining work is to route all FFI/lane boundary producers through that helper instead of ad hoc numeric casts.
