...
**Phase 1 — Core factory (no call-site changes yet)**
- `lib/mem_context.h` / `lib/mem_context.c`: context, node slab, mutex, factory + destroy functions, cascade teardown, snapshot capture.
- Rename existing primitives to internal `*_raw` (or keep names but mark internal-linkage / move to a private header). Factory functions wrap them.
- Provide `MEM_CONTEXT_DISABLED` passthrough macros so a release build is byte-for-byte behavior-equivalent.
- Unit tests: concurrent create/destroy from N threads + concurrent snapshot (TSan clean); `child_count` invariant; cascade teardown ordering; node-slab free-list.

**Phase 2 — Pool & Arena migration**
- Add `mem_node` to `Pool`/`Arena`. Convert every `pool_create()` / `arena_create*()` call site to `mem_pool_create(ctx, role, label)` / `mem_arena_create(ctx, backing, role, label)`. Add `arena_get_stats`, pool reserved/mmap accessors.
- Forbid direct `pool_create` in application code via a grep check in `make check-*` (similar to `make check-int-cast`).

**Phase 3 — Heap, nursery, scratch, name/shape pools**
- Route `heap_init*`, `gc_nursery_create`, `scratch_init`, name/shape pool creation through the factory. Wire `EvalContext`, `LayoutContext`, `RenderContext`, `ViewTree` to a per-document/per-pass sub-context.
...
**Phase 5 — Consumers**
- JSON dumper + `--mem-dump` + teardown leak report; `--mem-top` TUI; optional inspector query surface.
