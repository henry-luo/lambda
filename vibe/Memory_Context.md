...
- **sysinfo**: the per-thread cache holds an `Input` created on the eval context's pool and Items cached from it (TTL up to an hour). A later runtime on the same thread reused both after the pool was gone. The cache now registers its own cleanup, which invalidates it. Regression: `SysInfoRuntimeTest.CacheIsRebuiltAfterEachRuntimeInOneProcess` runs one `sys.*` script three times in a `test-batch`; with the cleanup removed, the later runs print the first run's freed data.
