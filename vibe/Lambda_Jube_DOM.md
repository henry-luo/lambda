...
- DOM node wrappers now use branded, non-owning VMaps. The stale `MAP_KIND_DOM` node shell has been removed, and Range, Selection, inline style, computed style, CSSOM stylesheet/rule/declaration resources, document proxies, and foreign-document proxies have also moved to branded native VMaps.
...
- The post-Phase 7 cleanup items 1-4 are implemented, plus the document/foreign-document carrier cleanup: MIR method-call lowering no longer emits the stale map-DOM branch, non-node DOM resources use native VMap descriptors, Range/Selection use native VMap wrappers, CSSOM stylesheet/rule/declaration objects use native VMap wrappers, and document/foreign-document proxies no longer use `MAP_KIND_DOC_PROXY` / `MAP_KIND_FOREIGN_DOC` compatibility dispatch.
...
- Latest gate: `make build`, direct `dom_v12b`, `dom_jquery_lib`, `dom_module_props`, and `dom_style` runs passed, and focused `test_js_gtest.exe` over `dom_module_props` + `dom_jquery_lib` + `dom_style` + `dom_v12b` + `input_event_target_ranges_constructor` passed `5/5`. The preceding broad checkpoint also had `make build-test`, full `test_ui_automation_gtest.exe` (`236` with `2` skipped), and `git diff --check` green.
...
- 2026-07-08: Phase 2 foreign-document/window proxy dispatch moved. `radiant_dom_bridge.cpp` now owns foreign-document get/set/method routing: browsing-context aliases (`defaultView`, `document`, `window`, `self`, `getSelection`), active-document-swapped document proxy reads/writes/methods, and the window-global method fallback used by iframe `contentWindow`. This initially routed through the foreign-document map carrier and later moved to the branded VMap carrier.
...
- 2026-07-08 post-Phase 7 items 1-4 completed: MIR call lowering now sends map receivers through `js_map_method()` and VMap host receivers through the VMap dispatch path without the stale map-DOM branch; inline style and computed style resources use branded native VMaps; Range and Selection wrappers use branded native VMaps with side-table expandos preserved; CSSOM stylesheet/rule/declaration wrappers use branded native VMaps and route `in`, get/set, method, descriptor, and prototype paths through native resource predicates. The DOM unwrap guard now unwraps only Radiant-branded node VMaps so style/CSSOM VMaps cannot recurse through the Radiant fallback path. Verification: `make build`, `make build-test`, direct `dom_v12b`, `dom_jquery_lib`, `dom_module_props`, and `dom_style` runs passed; focused `test_js_gtest.exe` over `dom_module_props` + `dom_jquery_lib` + `dom_style` + `dom_v12b` + `input_event_target_ranges_constructor` passed `5/5`; full `test_ui_automation_gtest.exe` passed `236` with `2` skipped; `git diff --check` passed.
- 2026-07-08 document/foreign-document carrier cleanup completed: the main `document` proxy and foreign documents produced by `document.implementation.create*Document` now use branded native VMaps with `host_data` carrying the active `DomDocument`; document unwrap, get/set, `in`, method dispatch, and prototype lookup route through the VMap predicates; the stale `MAP_KIND_DOC_PROXY` / `MAP_KIND_FOREIGN_DOC` JS compatibility dispatch and marker state were removed. Verification: `make build`, direct `dom_module_props`, `dom_v12b`, `dom_jquery_lib`, and `dom_style` runs passed; focused `test_js_gtest.exe` over `dom_module_props` + `dom_jquery_lib` + `dom_style` + `dom_v12b` + `input_event_target_ranges_constructor` passed `5/5`.
...
- Post-Phase 7 update: Range/Selection native-object carrier cleanup is complete; broader Range/Selection conformance remains tracked outside the carrier migration.
...
The POC 1A deliverable is therefore complete as a compatibility checkpoint. POC 1B is complete through post-Phase 7 item 4 plus document/foreign-document carrier cleanup: DOM, Range/Selection, style, CSSOM, document proxy, and foreign-document host resources now use branded VMap carriers, the stale DOM-node map shell is gone, and MIR method-call lowering no longer emits the retired map-DOM compatibility path.
...
- Full CSSOM conformance work beyond the native-carrier migration.
- Full Range/Selection conformance work beyond the native-carrier migration.
- Node module refactor.
- Online registry and version resolution.
- Migrating any remaining non-JS-system host resources outside DOM/Range/Selection/style/CSSOM/document proxies/foreign-document proxies to native descriptors.

DOM-node `MAP_KIND_DOM` deletion, post-Phase 7 cleanup items 1-4, and document/foreign-document proxy carrier cleanup are complete. Remaining cleanup is broader conformance and descriptor unification outside these migrated carriers.
