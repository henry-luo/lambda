...
- DOM node wrappers now use branded, non-owning VMaps. The stale `MAP_KIND_DOM` node shell has been removed, and Range, Selection, inline style, computed style, CSSOM stylesheet/rule/declaration resources have also moved to branded native VMaps with legacy map predicates kept only as transition guards.
...
- The post-Phase 7 cleanup items 1-4 are implemented: MIR method-call lowering no longer emits the stale map-DOM branch, non-node DOM resources use native VMap descriptors, Range/Selection use native VMap wrappers, and CSSOM stylesheet/rule/declaration objects use native VMap wrappers.
...
- Latest gate: `make build`, `make build-test`, direct `dom_v12b`, `dom_jquery_lib`, `dom_module_props`, and `dom_style` runs passed, focused `test_js_gtest.exe` over `dom_module_props` + `dom_jquery_lib` + `dom_style` + `dom_v12b` + `input_event_target_ranges_constructor` passed `5/5`, full `test_ui_automation_gtest.exe` passed `236` with `2` skipped, and `git diff --check` passed.
...
The POC 1A deliverable is therefore complete as a compatibility checkpoint. POC 1B is complete through post-Phase 7 item 4: DOM, Range/Selection, style, and CSSOM host resources now use branded VMap carriers, the stale DOM-node map shell is gone, and MIR method-call lowering no longer emits the retired map-DOM compatibility path.
...
- Migrating any remaining non-JS-system host resources outside DOM/Range/Selection/style/CSSOM to native descriptors.

DOM-node `MAP_KIND_DOM` deletion and post-Phase 7 cleanup items 1-4 are complete. Remaining cleanup is broader conformance and descriptor unification outside these migrated carriers.
