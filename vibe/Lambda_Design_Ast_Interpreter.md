...
*Scoped here; a stage-2 revision (or sibling doc) firms up file-level detail after stage 1's P5 gate. JS_01 §6 (interpreter-vs-JIT selection), JS_15, and `vibe/jube/JS_Profiling_Helpers.md` are the anchor references.*
...
**What transfers unchanged from stage 1:** the side-stack frame discipline (`Lambda_Design_Stack_Frame_JS.md` already unified the frame shape); promotion cells and satellite lowering (JS lowering is also whole-module one-shot today, so the same table-persistence motion applies to `JsFuncCollected` and friends); `EvalSignal`; the debug/profiling hooks; and helper reuse — ~492 registered `js_*` helpers taking and returning plain `Item`s, each carrying `JitImportMetadata` (GC/reentry/exception effects), which is exactly the soundness metadata the walker's rooting decisions need. Notably, **JS throws are already a return-value protocol, not unwinding** (JS_04 §9: no C++ exceptions, no `longjmp` for ordinary JS throws — ERROR-tagged Items with compiler-inserted propagation branches): a check-and-propagate tree-walker is the natural consumer of that protocol, and the walker's propagation points are a transcription of where lowering emits `jm_emit_error_lane_propagate_check` today.

**JS-specific work stage 2 must own** (deltas Lambda stage 1 does not have):

1. **Environment records.** JS captures *variables*, not values: by-reference mutation visible across sibling closures, plus fresh per-iteration `let`/`const` bindings. Today's copied-env + write-back machinery is flagged in JS_05's Known Issues as a live bug source (per-iteration staleness; silent capture drop past index 16) — the interpreter's environment-record chain is a correctness *fix*, not just a port.
2. **Completion semantics.** Labeled break/continue must run IteratorClose for every for-of crossed, `finally` re-raises a saved in-flight error unless its own throw wins, `with` pushes dynamic scope objects — structured completion records in the walker, replacing the compiler's try-context stack (today depth-16 with silent truncation).
...
5. **Per-AST-node inline caches.** D8.4.1 keeps ICs for JS ("guard-based cache cells… never patched instructions"), but the existing ICs are per-MIR-call-site with raw pointers baked into the module — the walker needs its own IC slots, and `AstIndex`'s stable `AstNodeId` side tables are the designed home.
