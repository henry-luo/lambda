...
The stems `dom_node`/`dom_element`/`dom_lifecycle` are **reserved** — they belong to the data structures in `lambda/input/css/`, which stay put (user direction; `Radiant_Design_View_Struct.md` OQ5 reserves this same directory for their eventual move — coherent end state, deferred, ESO77). Build: the files stay in the `lambda-rt` target (new glob + include dirs); no new link target this pass (ESO72).
...
| ESO77 | **Struct co-location** (user-deferred): moving `dom_node`/`dom_element`/`dom_lifecycle` from `lambda/input/css/` into `lambda/dom/` — reconcile with `Radiant_Design_View_Struct.md` OQ5 when taken up; the stems are already reserved (ES32). |
...
| ESO79 | **The rest of the ES33 adapter split — in progress.** *Landed 2026-09-02: the realm-prototype seam.* `dom_realm_constructor` / `_constructor_prototype` / `_apply_prototype` (declared in `dom.h`, implemented in `js_dom_adapter.cpp`) replace the read-the-global-then-take-`.prototype` sequence that had been open-coded identically at eight sites across `dom.cpp`, `dom_selection.cpp` and `dom_cssom.cpp`. All three answer null without a realm, which is what keeps those construction paths usable from a Lambda-only document. `js_set_prototype` 7→2, `js_get_global_this` 24→16, `dom_cssom` and `dom_platform` clean. *Survey finding for (1), 2026-09-02 — the installers are not a contiguous region.* An early plan was to lift `dom.cpp:15888..16428` wholesale, since a dependency scan said only ~9 symbols would need promoting. Reading it function by function killed that: the realm plumbing (`_install_iface`, `_iface_proto`, `_link_iface_proto`, `_install_node_iface`, `dom_install_value_constructor`, `_set_ctor_int_constant`, `dom_install_collection_globals`, `dom_install_option_constructor`) is **interleaved** with code that is squarely core — the XPath evaluator's tree matching (`_xpath_element_matches`, `_xpath_collect_descendants`) and the Web Animations surface (`dom_element_animate`, `js_web_animation_parse_keyframes`), which DOM_Pkg §4.2 places at N. Cutting the range would have moved DOM algorithms into the JS adapter and made the layering worse than leaving it alone. So (1) is a **per-function** extraction, and its natural seam points are the three constructor callbacks the plumbing invokes — `_document_fragment_ctor`, `_xpath_evaluator_ctor`, `_option_ctor` — which stay in the core and get promoted, while only the publish-on-global wrappers leave. *Slice 2 landed 2026-09-02: the realm-installation cluster.* Extracted **function by function** into the new `lambda/js/js_dom_realm.cpp` (270 lines): the WebIDL plumbing (`_install_iface`, `_iface_proto`, `_link_iface_proto`, `_install_node_iface`, `_install_nodelist_for_each`, `dom_install_value_constructor`, `_set_ctor_int_constant`, `_set_iface_to_string_tag`, `_coll_illegal_constructor`), the two big installers (`dom_install_collection_globals`, `dom_install_option_constructor`), and the `getComputedStyle`/`prompt` window publications. What builds DOM objects stayed in the core and is reached through the new `lambda/dom/dom_realm_hooks.h` — the constructor callbacks (`dom_document_fragment_ctor`, `dom_xpath_evaluator_ctor`, `dom_option_ctor`, `dom_matrix_constructor`, `dom_point_constructor`), the `Element.prototype.querySelector`/`animate` bodies, `dom_window_prompt`, and the HTML interface table read out by index so the table itself never leaves. **`js_set_prototype` is now 0 across the entire core**, `js_get_global_this` 16→12; XPath matching and Web Animations verifiably did not follow the installers out. *Slice 1a landed 2026-09-02: the last realm writes.* A binding seam (`dom_realm_has_own` / `dom_realm_lookup` / `dom_realm_define`) lets HTML's named-property-access-on-Window keep its *policy* in the core — never clobber a user global, prefer a connected element, refresh a stale detached wrapper — while the adapter answers where a realm keeps bindings. The two split-shaped sites split along the same line: the core collects the frame windows (`dom_collect_frame_windows_array`) and the adapter publishes `window.frames`/`length`; the testdriver hooks, the DOMParser/XMLSerializer constructor globals, and `dom_selection_install_globals` moved to `js_dom_realm.cpp` with their bodies promoted through `dom_realm_hooks.h`. **`dom.cpp` and `dom_selection.cpp` are now free of all four markers**; the adapter is 407 + 80 lines.

*This fixed a real bug.* `JavaScriptRegression.DocumentExitCodeAfterContextRestoreDoesNotInternWithNullContext` had been failing since before this work. The old prototype lookups called `js_name_item(ctor_name)` — which interns — *before* checking the realm global was live, so a document exiting after context restore interned against a dead context (`heap_create_name called with invalid context or name_pool`) and then did `map_get` with the resulting null key. The seam checks the global first and returns early, so neither happens: **lambda is now 4072/4072**, and the test passes 3/3 standalone.

*Remaining:* **(1b) the Storage object builder** — `dom_platform.cpp`'s `storage_object()` still holds 6 `js_install_native_method` calls and caches the built JS object *inside the storage state*; splitting it means the core exposing entry get/set/remove/clear/key/length and the adapter building the object over them. **(2) the event-class cluster** (9 `JS_CLASS_` + 7 `js_get_global_this` in `dom_events.cpp`) — still blocked on ES24: the class id is a parameter of `js_create_event_init_with_class`, which the native factories call too, so it needs the record/wrapper separation first. **(3) residue**: `dom_register_named_elements` and `dom_set_document` each need one realm write; `dom_cssom`'s 2 `JS_CLASS_` stamps ride with (2). |
