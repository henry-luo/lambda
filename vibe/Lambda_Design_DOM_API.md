...
| ESO82 | **ES35's second clause is unshipped: 160 core-defined symbols still carry `js_`.** Split them before renaming — `js_get_computed_style`, `js_classlist_*`, `js_dataset_*`, `js_storage_*`, `js_match_media*`, `js_cssom_*` are core mechanism and should become `dom_*`; `js_create_event*`, `js_ctor_*`, `js_create_foreign_*` are ESO79's extraction targets and should keep `js_` until they leave. Renaming the second group now would be work undone later, so ESO79 sequences ahead of this. |
| ESO83 | **ES38 is only partly satisfied for `radiant.*`.** Three module functions delegate through ordinals (`set_attr`, `remove_attr`, `closest` — `radiant_module.cpp:1205,1211,1514`); the tree-navigation neighbours (`first_element_child`, `next_element_sibling`, `parent`, `root`, `document_root`, `attr`, `has_attr`) still walk `DomNode` links themselves, so `dom.first_element_child` and `radiant.first_element_child` are two implementations of one operation. Harmless today because both read the same tree through the same wrappers, and the duplicates are short — but ES38 is a standing invariant and this is the concrete debt against it. Fold on the core side, behind the ES34 seam; it is also exactly the inventory Phase 2's catalog (ESO78) has to reconcile, so the two are best done together. |
