...
> **Ledger series**: this doc extends the DOM-State area's existing `ES#` (decisions) and `ESO#` (open issues) series per `doc/Doc_Convention.md` §4 — it mints no new series. New open issues start at **ESO48**.
...
| `dragstart` / `dragover` / `drop` / `dragend` | HTML DnD; cancelable (`dragover`'s default is *rejecting* the drop — `preventDefault` enables it) | move/copy the dragged content | drag geometry and range tracking native; `<img>` and `<a href>` are draggable by default (`event.cpp:8891`); edits arrive as `deleteByDrag` + `insertFromDrop`, claimed by the same appliers as cut/paste. **Not claimed on contenteditable** (§4) | ✅ text controls · ❌ contenteditable |
...
| `summary` | toggle the parent `<details>` `open` attribute | absent — the only `<summary>` code is a list-marker style rule (`resolve_htm_style.cpp:1194`) | ❌ |
...
| ESO56 | **`<details>` / `<summary>` has no toggle behavior** and fires no `toggle` event; only the list-marker style rule exists | a `view <summary>` behavior template writing the parent's `open` attribute, once `~.attr = v` DOM writes from a template are settled (ESO3) |
...
5. `<details>` toggle (ESO56) and `:target` (ESO53) — both small, both frequently exercised by static documents.
