...
| --- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Cross-module `: float` type annotations break JIT**                  | MIR compilation fails when `pub fn` parameters or return types use `: float`                                                                                             | Removed all `: float` annotations from pub function signatures; use un-annotated params                                                        | ✅ **Engine fixed** — bug in `build_call_expr` (`build_ast.cpp`): aliased import function was overwritten by `else` branch; fixed with `else if (!is_aliased_import_call)` + `AstPrimaryNode` wrapping                   |
| 2   | **Tuple-pipe mapping broken cross-module**                             | `tuple \| module.fn(~)` fails at MIR compilation                                                                                                                         | Replaced with explicit `for (x in tuple) module.fn(x)` comprehensions                                                                          | ✅ **Engine fixed** — same root cause as #1; pipe-mapped calls now resolve correctly                                                                                                                                     |
| 3   | **`unique()` broken for string arrays**                                | Built-in `unique()` only returns the first element when given string arrays; works correctly for integer arrays                                                          | Implemented recursive `unique_vals()` in `util.ls` using linear-scan `has_val()` helper; replaced all 12 call sites across 5 modules           | No longer exists.                                                                                                                                                                                                       |
| 4   | **X-axis tick labels show `"<error>"`**                                | `util.fmt_num(tv)` called on string values from band scales (nominal categories)                                                                                         | Changed to `string(tv)` in `axis.ls` — works universally for both numeric and string tick values                                               | Expected                                                                                                                                                                                                                |
| 5   | **`if`/`else` blocks parsed as map literals**                          | `{ }` inside if-expressions treated as map constructor, not code blocks                                                                                                  | Rewrote all conditional logic as parenthesized if-expressions: `(if (cond) expr1 else expr2)`                                                  | Expected                                                                                                                                                                                                                |
| 6   | **Reserved words as map keys**                                         | `.type` field access fails because `type` is a reserved Lambda keyword                                                                                                   | Used bracket notation: `obj["type"]` instead of `obj.type`                                                                                     | ✅ **Engine fixed** — added `base_type` keywords to `member_expr`, `map_item`, and `attr_name` grammar rules; updated AST builder to handle `SYM_BASE_TYPE` in field access and key positions                            |
| 7   | **`for` comprehension concatenates string results**                    | When the body of a `for` expression returns strings, results are concatenated instead of collected into a tuple                                                          | Used recursive array-building pattern with `[*result, item]` instead of `for` comprehension for string-producing operations                    | Expected.<br>`for` expression returns strings, does construct an array of strings.<br><br>However, when the result is further embedded in list of element, or output at script top-level, the strings are concatenated. |
| 8   | **`let` with `if_expr` inside `for` comprehension causes MIR failure** | `let label = (if (cond) A else B)` as a `for`-local binding crashes MIR compilation                                                                                      | Avoided the pattern; used `string(tv)` universally instead of conditional formatting                                                           | No longer exists.                                                                                                                                                                                                       |
| 9   | **`input()` with `^err` destructuring breaks module resolution**       | `let vl^err = input(...)` in test scripts causes "Failed to find import item for module" error at MIR compilation, even though all imported modules compile individually | Switched test scripts to inline data (map literals) instead of file I/O; `input()` with `^err` appears to interfere with the JIT module linker | ✅ **Engine fixed** — `declare_global_var` now declares the error variable for `^err` destructuring; `assign_global_var` delegates to `transpile_assign_expr` for proper error split pattern |
| 10  | **Computed map keys not supported**                                    | `{[key_var]: val}` syntax does not exist in Lambda                                                                                                                       | Cannot dynamically construct maps with variable keys; must use spread `{*base, field: val}` with known field names                             | Workaround                                                                                                                                                                                                              |
| 11  | **Map spread segfault**                                                | `{...m}` causes a segfault                                                                                                                                               | Use `{*m}` instead                                                                                                                             | Workaround                                                                                                                                                                                                              |
| 12  | **Functions limited to 8 parameters**                                  | MIR JIT fails for functions with >8 parameters                                                                                                                           | Restructured functions to use context maps (`ctx`) instead of many individual parameters                                                       | Workaround                                                                                                                                                                                                              |
