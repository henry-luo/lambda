...
| 9   | **Band/point scale index lookup is O(n)**                                        | Uses linear scan `for` comprehension instead of hash map            | Acceptable for typical category counts (<100); would need optimization for large categorical datasets                       | Can use the new map() constructing function. |
