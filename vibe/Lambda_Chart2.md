...
| 14  | **`sort(data, comparator)` destroys map items**            | `fn_sort2` treats the 2nd argument as a direction string ("asc"/"desc"), not as a comparator lambda. Internally calls `item_to_double()` on every item, converting maps to NaN.                                                                                                                                        | **Workaround**: Use `for (d in data order by d[field]) d` syntax which transpiles to `fn_sort_by_keys` and preserves original Item types.<br>Expected.<br>Lambda does not have sort() function at the moment.                                                                                                                                                       |
| 15  | **C2MIR ternary type mismatch in `group_key_str`**         | `if-else` returning `String*` (from `fn_string()`) vs `Item` (from `fn_str_join()`) — incompatible types in conditional expression. Root cause: `transpile_if()` fast path skipped boxing when both branches had the same semantic type (e.g., STRING), but C-level return types could differ (`String*` vs `Item`).   | **Transpiler fix** in `transpile.cpp`: Restricted the fast path (no boxing) in `transpile_if()` to scalar types only (INT, INT64, FLOAT, BOOL) where C representation is always consistent. All other types (STRING, SYMBOL, BINARY, containers) now always use boxing, ensuring both ternary arms produce `Item`.                                                  |
| 16  | **C2MIR implicit pointer cast with `: string` annotation** | `pub fn compute_agg(data, op: string, field)` — `: string` makes the C parameter `String*` but callers pass `Item`, causing type error. Root cause: `transpile_call_argument()` had no STRING handler — the same-type fast path emitted raw `transpile_expr`, and the catch-all `else` boxed to `Item`, both wrong when param expects `String*`. Also: `define_func_unboxed()` generated broken `_u` variants with `String*` return type but `Item`-returning bodies. | **Transpiler fix** in `transpile.cpp`: (1) Added STRING/BINARY handler in `transpile_call_argument()` using `it2s(transpile_box_item(...))` to always produce proper `String*`. (2) Excluded STRING from the same-type fast path so it reaches the handler. (3) Added BOOL handler with `it2b()` for ANY→bool conversion. (4) Limited `define_func_unboxed()` to INT/INT64 returns only, preventing broken `_u` variants for STRING. Also added `it2b` to MIR function table and STRING unboxing in `transpile_assign_expr()`. |
...
| 18  | **`//` comments after element expressions**                | `// comment` can be parsed as division operator when following certain expressions. The Tree-sitter lexer didn't prioritize the comment token over `/`, and `\n` in extras allowed cross-line binary expression parsing.                                                                                               | **Grammar fix** in `grammar.js`: (1) Wrapped comment token in `token(prec(1, ...))` so `//` wins over `/` in lexer conflicts. (2) Added `'statement_end'` precedence and `token(prec(10, linebreak))` in `_statement` so `\n` terminates statements before binary operators can continue across lines. Also added defensive `strview_trim(&op)` in `build_ast.cpp`. |
