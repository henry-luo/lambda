...
> **Ledger series**: extends the DOM area's `ES#`/`ESO#`/`F#` per `doc/Doc_Convention.md` §4. Decisions **ES39–ES44**; open issues **ESO84–ESO95**; stages **F28–F33**.
...
| `JubeHostDomAPI` (`jube.h:676–928`) | 218 slots = ~173 live + 24 `NULL` + 21 dead | **only** `lambda/module/radiant/` — no guest, no interpreter path reads `host->dom` today |
...
| **Dead / retired** | 45 | 24 `NULL` slots, 21 wired-but-unreferenced slots |
...
|---|---|---|
| **F28 — catalog + tiering, no behavior change** ✅ **LANDED 2026-09-02** | `lambda/dom/dom_api.def` (113 rows: 73 CORE, 40 DERIVED, each DERIVED row carrying its derivation string); `dom_core.h/.cpp` = the uniform `Item f(Item×argc)` bodies — the F26 `fn_dom_*` bodies *moved* here as fast paths (not copied), plus new core bodies `create_node`, `set_node_value`, `attribute_names`, `scroll_state`, `range_boundaries`, `selection_boundaries`, `computed_style`, and `parse_fragment` in dom.cpp sharing the innerHTML setter's parse loop (`dom_parse_markup_into`, one loop, two callers); `dom_api_check.cpp` = compile-time uniqueness (one enumerator per name), arity (`static_assert` per row against the body's C signature) and tier/derivation consistency; `dom_module.cpp` now *expands the catalog* (rows with a body and `DOM_F_NEUTRAL`) instead of listing functions by hand, so `dom.*` speaks canonical names (`get_attribute`, `query_selector`, `append_child`, …) and gained 12 operations. Engine-provided rows (`DOM_F_ENGINE`, 40) and realm-dependent rows carry `DOM_NO_BODY`/no NEUTRAL flag and wait for F30. | Tests: `dom_api.ls`/`dom_api_read.ls` renamed to canonical names with goldens unchanged byte for byte; new `dom_api_core.ls` drives creation of all four node kinds, `parse_fragment`, `set_node_value`, snapshot child lists and `attribute_names` from a realm-less script. Gates: lambda 4072/4072; radiant 8092 passed / 1 failed — the Layout Page Suite, which **passes when rerun standalone**; it checks against `test/layout/snapshot/page.json`, a local unversioned snapshot, and its per-page numbers swing tens of points between runs on static pages with no script (clojure 100%→22.5%, bootstrap +38%), so it measures capture noise, not DOM behavior; WPT DOM nodes rerun against the new code (see ESO95). |
| **F29 — realm split + dead slots** | ES41 section; the 28 JS-shape slots move; 45 dead/`NULL` slots deleted; version → 2 | bridge builds against `->dom` + `->realm`; gates green |
| **F30 — core section + first oracles** | The table's core section expanded from the catalog; the derived-tier lint (ES42) introduced against a *new* `lambda/dom/dom_derived.cpp` that initially holds only the trivial derivations (§5.1–§5.3); `dom_derive_traversal.ls` and `dom_derive_chardata.ls` oracles pass against the existing bodies | core slots are the only non-wrapper DOM slots; two oracle fixtures green |
| **F31 — derive the rest; close the seam** | Each remaining derived operation re-expressed over core (§5.4–§5.9), native fast paths kept only where DOM_Pkg §3.3's frequency test says so and each one oracle-tested; the 8 module externs and 60 direct entries replaced by table calls; `set_text_content`/`set_inner_html` published to `dom.*` (ESO81/ESO86 close); `dom.load` (ESO80 closes) | zero `extern "C"` from `lambda/module/radiant/` into DOM bodies; every FP row has a passing oracle; `dom_api.ls` gains text/innerHTML writes and a `dom.load` round trip |
| **F32 — `dom.*` / `radiant.*` unification; package migration** | `dom.*` regenerated (typed, both tiers); `radiant.*` reduced to the engine set + aliases; `lambda/package/dom/*.ls` migrated `radiant.<op>` → `dom.<op>`; aliases deleted per ESO84 | package fixture outputs unchanged; `radiant_module.cpp` has no DOM rows |
...
| ID | Issue |
|---|---|
| ESO84 | **Alias retirement.** Delete `radiant.<op>` aliases after F32 (default), or keep indefinitely for external scripts. User call. |
