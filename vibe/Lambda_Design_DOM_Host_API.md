...
> **Ledger series**: extends the DOM area's `ES#`/`ESO#`/`F#` per `doc/Doc_Convention.md` §4. Decisions **ES39–ES44**; open issues **ESO84–ESO106** (ESO81, ESO93 and ESO101 resolved); stages **F28–F33**.
...
| ESO106 | **Crossing the dispatch entry points needs a rooted adapter.** `invoke`'s row takes an array where the executor takes `(op, args, argc)`, so the module can only cross it by building an array per call. Two things have to be true first, and neither is today: the allocation must not sit on the DOM's hottest path unamortised (every property access goes through these three), and the caller's `args` — raw `Item`s on the C stack — must be rooted across it, or a collection during the build frees them. `form_submission` failed on exactly that. The fix is a rooted, non-allocating adapter (a `RootFrame` over a fixed-size argument buffer, per D5's precise rooting), after which the ~95 rowless slots become a *class* that can cross rather than a queue of individual contract questions. |
