...
| **ESO110 — dispatch is one operation now; one of the two blockers is fixed, the other is located.** `dom_core_dispatch` has a single shape: a live event goes straight through, and a descriptor — a name or `{type, bubbles, cancelable}` — is **built into a real event** by `radiant_dom_event_create`, which is fully native (a VMap over a `RadiantDomEventRecord`, `vmap_new` + `mem_calloc`), so no realm is needed to make or read one. **Blocker 1 (the cascade) is fixed, but not by merging the paths.** Two attempts to route everything through the general path failed the same nine behaviour fixtures, because `radiant_dispatch_synthetic_dom_event` opens a **fresh `EventContext`** while the engine's descriptor entry passes the in-flight `ctx->evcon` — so an `input` raised while handling a keystroke joins that walk instead of starting a second one. The fix is to *choose*: a new `dom_engine_event_cascade_active()` seam reports whether a cascade is running, and dispatch takes the engine entry inside one and the general path outside. That is one operation with the right implementation in each context, rather than one implementation that is wrong in one of them. It also removes the defect ESO109 found: `dom.dispatch` no longer answers false at the top level, and can target a document or window. **Blocker 2 (the realm) is narrowed to one line.** A realm-free Lambda script still cannot dispatch, and the failure is now exactly `dom_dispatch_event`'s `dom_realm_set_name(event, "target", …)`: a VMap made outside a realm has no arena for `map_put`, so the store fails with `no js_input context`. Fixing it means the event's target living in the record rather than as a map property — which puts an `Item` in a `mem_calloc`'d struct the collector does not trace, so it is a rooting design question, not a rename. That is ES47 residue and the last thing between this operation and realm-neutrality. |
