...
> **Ledger series**: extends the DOM area's `ES#`/`ESO#`/`F#` per `doc/Doc_Convention.md` §4. Decisions **ES39–ES44**; open issues **ESO84–ESO104** (ESO81, ESO93 and ESO101 resolved); stages **F28–F33**.
...
| **F34 — close the JS backdoor (ES45)** | Route JS's DOM access through the API: the property/ordinal path (the main channel — the remaining member ordinals and the declared-interface entries join the 7 that already delegate), event construction (`create_event` becomes a real row, realm-conditional), and the four genuine DOM operations among the runtime's 49 direct symbols (`css_supports`, `css_escape`, `dataset` set, `event_handler_property_set`). Lifecycle and realm-seam calls are exempt per ES45. | `grep -E '\bdom_[a-z_]+\(' lambda/js/*.cpp` matches only lifecycle and realm seams; WPT dom/nodes and dom/events unchanged; the DOM UI fixtures unchanged |
...
| ID | Issue |
|---|---|
| **ESO84 — DECIDED 2026-09-03: keep the aliases.** ES44 expected F32 to delete them once the package no longer needed them. The package no longer does — 114 call sites moved, 49 remain and those are the engine set — but the aliases have **three other consumers**, and deleting them would break working code for no gain. (1) The radiant module's own tests exercise `radiant.attr`, `set_attr`, `get_state`, `set_state` directly (`radiant_state_prims.ls`, `radiant_value_state.ls`, `radiant_dom_mutate.ls`, `radiant_poc.ls`); those test the engine module's surface, not the package's, and should not be forced onto `dom.*`. (2) The oracles that prove each move — `dom_engine_rows.ls`, `dom_derive_forms.ls`, `dom_derive_urlencode.ls` — compare `dom.<op>` against `radiant.<op>`, and cannot do that if one side is deleted; they are the standing regression test for the unification. (3) `release/lmd/package/dom/*.ls` is a checked-in release snapshot of the older package that still calls them. So `radiant.*` keeps its DOM spellings as a **published compatibility surface with a test-backed equivalence guarantee**, rather than as scaffolding awaiting removal. Revisit if and when the snapshot is regenerated and the module's own tests are rewritten against `dom.*`. |
