...
| ESO102 | **The Document identity split is now fully characterised, and what remains is a semantics decision rather than a defect.** **Step 1 is landed** (readers accept either object; ESO101 segfaulted because the writers moved first). **Step 2 was carried much further and reverted twice, and the crash is now understood and fixed.** Flipping the writers needs four things, all found by doing it: (a) `radiant_dom_host_type_for_node` types the `#document` stub as an **element**, so the wrapper presented the Element interface and `document.createRange` did not exist; (b) `radiant_dom_is_node_host_type` **excludes** the document type, so unwrapping `document` answered null, though a Document is a Node; (c) **the crash**: three sites in `radiant_dom_bridge.cpp` — the Document interface's property get, property set and operation dispatch — pass `object.vmap->host_data` straight to `dom_swap_active_document`. The proxy carried a `DomDocument*` there and the node wrapper carries a `DomNode*`, so the active document became garbage and *every* script died. It surfaced as "recovered from crash in JS JIT code" with no exception, which is why it read as mysterious; resolving the document from the stub's `->doc` fixes it. (d) `ownerDocument` must keep asking the **core** which object a given document is, because the answer differs per document: unifying it in the module gave foreign documents a second identity and broke `foreignDoc.body.ownerDocument === foreignDoc`. With all four, **radiant is fully green** (8091, DOM UI 108/108, UI Automation 114/114) and the JS side passes. What blocks it is the **Lambda** surface: `radiant_dom_read.ls`, `radiant_dom_mutate.ls`, `proc_radiant_dom_set.ls` and the two custom-layout scripts read `owner_document.document_element`, `.query_selector(...)` and iterate its keys, because Lambda's document is the module's **keyed map** (`document_element`, `ready_state`, `node_name`) while JS's is a node wrapper that materialises no entries (S5.1.4). Unifying the Document therefore *necessarily* changes what a Lambda script sees. That is a ruling about the `dom.*` surface, not a refactor: **should Lambda's `document` become the node wrapper, with those five goldens updated?** Until that is answered the two objects stay. |
