...
> **Ledger series**: extends the DOM area's `ES#`/`ESO#`/`F#` per `doc/Doc_Convention.md` §4. Decisions **ES39–ES47**; open issues **ESO84–ESO107** (ESO81, ESO93, ESO101, ESO102, ESO106 and ESO107 resolved) (ESO81, ESO93 and ESO101 resolved); stages **F28–F33**.
...
|---|---|---|
| `radiant.*` (`radiant_module.cpp:2754–2952`) | 98 fns; **70** used by `lambda/dom/*.ls`, 28 test/POC-only | the UA behavior package; tests |
...
### ES46 (ruled by the user, 2026-09-03) — one DOM section, and a flag for what has no Lambda face

> "retire ->dom completely. for members that are not compatible with lambda, still
> migrate them under dom_catalog. just put a flag, so that they are not exposed to
> lambda, for the time being."

ES40 said the v1 per-method section should disappear as API structure, and the
A/B/C census answered that 37 of its members could never be rows — a row is a
Lambda function, and a `void` return over a raw `DomNode*` and two C strings is
not one. That was read as a limit on how far the section could shrink. It is
not. The catalog can carry an operation that has **no Lambda face yet**, so long
as it says so.

So the catalog has two row kinds. **`DOM_OP`** is the Lambda-expressible
operation as before: uniform `Item` arity, a Lambda signature, an operation id,
a compile-checked arity, and a `dom.*` face when it is realm-neutral.
**`DOM_RAW`** carries an operation's native C signature verbatim. It is in the
catalog, the module calls it there, and it is *structurally* unpublishable —
`dom_build_published_table` walks the `DOM_OP` rows alone, so a `DOM_RAW` row
cannot reach Lambda even by mistake. Every one carries **`DOM_F_NATIVE`**, which
states that policy explicitly rather than leaving it to be inferred from the
absence of `DOM_F_NEUTRAL` (a property, not a policy).

The consequence worth stating: **being in the catalog and having a Lambda face
are now separate things.** They were the same thing until ESO107 moved the
absence rule to the publication boundary, which is what makes this ruling
...
| **ESO108 — the "event split" is not a split; the live event is already one object.** Asked whether the JS Event could be unified into the Lambda event map, the answer is that they are not two representations of one thing. F17–F21 already unified the **record**: there is one native `RadiantDomEventRecord` and one author cascade, and both realms' listeners see the same live event. What differs is `dispatch`'s **argument**. A *descriptor* — a name, or `{type, bubbles, cancelable}` — is keyed data saying how to build an event. A *live event* is a record-backed wrapper carrying propagation state (`default_prevented`, `stop_propagation`, `event_phase`, `class_id`) that a listener mutates and the dispatcher reads back. A Lambda map cannot be the second: it has three fields where the record has thirteen, six of them written during propagation, and `class_id` selects a JS subclass, which is a realm concept. **What was fixed**: `dom_core_dispatch` inferred which argument it had by reading `"type"` and checking for a string — and a live event answers that as readily as a descriptor does, so JS's `dispatchEvent` took the construction path, rebuilt the event and dropped identity along with `preventDefault`. It asks the module's record predicate (`radiant_dom_event_is`) first now, so the two arguments are distinguished by what they are. **What still cannot cross**: routing the module's `dispatchEvent` onto the row was tried and reverted. JS's `dispatchEvent` accepts a plain descriptor object too — `button.dispatchEvent({type: "custom"})` — and builds a **JS** Event for it, running the JS listener path, where the row's descriptor path builds an engine event. Four listener counts in `dom_module_props` went from 1 to 0. So `dispatch` and `dispatchEvent` are two operations under one name, exactly like `click` and `check_validity`, and the remedy is naming rather than unification. |
| **ESO109 — `dispatch` and `dispatchEvent` differ about the *cascade*, not about the event.** Asked why the two cannot be unified, the measured answer is not the one ESO108 gave. The row's engine body reaches `radiant_dispatch_event_with_flags_from_script`, which fails closed unless `g_emit_handler_ctx` is live — *"only callable from a handler"* — and takes a `DomElement`, so **`dom.dispatch` from a plain Lambda script logs an error and returns false**, and can never target a document or window. That is a defect in a published `DOM_F_NEUTRAL` row, not a design. The general path is strictly larger, so unification should run *that* way: the descriptor path was pointed at `dom_dispatch_event_bridge`, which already builds an event from a plain descriptor and dispatches at any EventTarget. Lambda stayed green at 4090 and **nine radiant fixtures failed** — `ui_phase3_form_controls`, `ui_phase1d_interactive_elements`, `dom_pkg_static`, `dom_pkg_space_keyup` and friends: exactly the behaviour-package paths whose `dom.dispatch(~, {type: "input", …})` runs *inside* a handler. The engine path **reuses the in-flight `EventContext`**, so the `input`/`change` a control raises while handling a keystroke joins the same cascade; the general path starts a fresh one. So the obstacle is neither the event's representation (already one record) nor the argument's shape (settled by ESO108's predicate) but **what dispatch means about the cascade in progress**. Unifying needs the general path to be able to continue an active cascade rather than begin one — and separately needs `dom_dispatch_event` to stop being written against the JS realm (it reads the descriptor with `dom_realm_get_name`, throws JS `TypeError`s and maintains `window.event`, so a realm-free Lambda script fails with `no js_input context`). Both are the ES47 residue, not event work. |
| **ESO114 — stylesheets and rules are Lambda-facing (2026-09-04).** The CSSOM cluster had the range/selection shape — native `CssStylesheet*`/`CssRule*` behind a host brand, `vmap_new` builders, `make_string_item` projections — and one realm dependency in two places: the **ambient document**, once as the 0-arity `document_stylesheets` entry and once as `get_document_pool()` inside the serialisers, where a realm-free script did not crash but *degraded silently* (`css_text` answering `""`). Both are the ESO113 pattern. **`stylesheets(doc)`** is the explicit-document door (one `stylesheets_array_for(doc)` behind both the ambient and explicit entries), and `sheet_pool()` derives a sheet's pool from the sheet, then its owner element's document, with the ambient document last — the two sites that reached for it first (`stylesheet_rules`, `stylesheet_index`) go through it. **Published**: the 11 `DOM_RAW` `stylesheet_get_*`/`rule_get_*` rows became `DOM_OP` in place (each had exactly one module caller by the same name, so nothing was repointed); `stylesheet_get_css_rules` was the unpublished `stylesheet_rules` row's body under a second name and is retired onto it; **`stylesheet_rule_at(sheet, index)`** is the Item-uniform twin of the native `(Item, int64_t)` `stylesheet_index`, which the module keeps; and `stylesheet_rules`, the four `rule_style_*` rows and `set_selector_text` gained `DOM_F_NEUTRAL`. `computed_style_get_property` stays native: nothing yet hands a Lambda script a computed-style object. Catalog: **235 `DOM_OP` (209 published) + 50 `DOM_RAW`**. Test: `dom_stylesheet.ls` over a fixture with an inline `<style>` — sheet count, `href: null`, `type`, rule selectors, `css_text`, `rule_style_has_property` — realm-free, with a golden. **What the 50 remaining native rows are**: `host` 8 (`get_document`, `unwrap_element_impl`, `to_attribute_cstr`, document swapping), `select`/`notify`/`forms` 6 each, `textctl` 5, `style` 4, `document` 4, `tree` 3, and singletons — the census's classes B–D: `void*`-receiver operations the module unwraps once and reuses, the `after_*` notifiers, the `is_*` predicates, and the four that are realm-bound by nature (`invoke_raw`, inline event-attribute compilation, `replace_with`'s listener calls, `focus_first_invalid_form_control`'s throws). None can be a Lambda function as shaped, and that is now the honest boundary of `DOM_RAW`: not "not yet promoted" but "native by nature, or reached only by the module". |
| **ESO113 — the range/selection cluster is Lambda-facing, and ESO81's real root was the ambient document (2026-09-04).** **ESO81, finally**: `dom.create_range()` did not need the JS object model. It needed a *current document*, and that is realm state: `dom_get_document()` is `js_active_runtime_state ? _js_current_document : nullptr`, the storage is `JsDomState`, and `dom_set_document` — the only writer — returns early without a realm, so `dom.load` hands a script its document and nothing ever makes it current. A Lambda script names the document it means: **`document_create_range(doc)`** and **`document_selection(doc)`** (`dom_create_range_for`/`dom_get_selection_for`, state via `dom_document_from_item` + `dom_engine_document_ensure_state`). The ambient `create_range`/`selection` stay JS's, unpublished. **Published**: the 43 remaining `DOM_RAW` range/selection rows became `DOM_OP` with generated signatures, the 6 single-line rows and the 9 DERIVED rows (`range_to_string`, `range_collapse`, … — multi-line, so the first flip missed them) gained `DOM_F_NEUTRAL`. **63 range/selection rows are Lambda-facing; 0 remain native.** Bodies unchanged. A `DOM_OP` body is expanded by the publication trampolines and the arity check, so each promoted body had to be declared in `dom_core.h`. Catalog: **222 `DOM_OP` (190 published) + 62 `DOM_RAW`**, from 177/130 + 105. **Two more realm couplings found only by running it.** `range_boundaries` and `selection_boundaries` built their result map with `js_new_object` + OrdinarySet, and a realm-free script **segfaulted** (exit 139) — the ESO110 mechanism, a window-hook probe creating the global — while `scroll_state` already had ESO81's rect idiom inline. That was the third copy, so it is one helper now: `dom_result_map` builds a plain Lambda map from the document's `Input` with no realm and the JS object otherwise; all three rows use it. And the interpreter's planner capped Jube calls at `arg_count > 4` while its executor already handles five, so `set_base_and_extent` (the catalog's one 5-argument row) excluded any script that touched it to the JIT; the cap is 5. **Tests**: `dom_range_selection.ls` and `dom_range_selection_ops.ls` pin ranges and selections from a Lambda script with no realm — `range_to_string` answering `"lph"`, `select_all_children` moving the anchor off the text node — with goldens. **RESOLVED 2026-09-04 — two causes, neither a DOM-API defect.** (a) `mir: undefined variable 'dom'` was the window in which the DERIVED range/selection rows were unpublished (the multi-line rows the single-line flip missed): an unpublished `dom.X` is no sys func, so the JIT lowered it as a member access on a plain identifier; gone once the rows were published. (b) The real transpiler defect: `transpile-mir.cpp` emitted sys-func calls through literal 1/2/3/4-argument branches and dropped anything longer into its **vararg** prototype (`i64:a, ...`). On Darwin arm64 varargs travel on the stack, so `dom_pub_set_base_and_extent(Item×5)` read its trailing operands from unset registers — a flood of `js_set_key_default: no js_input context` and no result at all, JIT-only, and invisible to the harness because the default tier is `auto` = interpreter-first. Fixed by one loop over `em_call_with_args` for every `arg_count != -1` (the vararg prototype is reserved for descriptors declared variadic — only `set`), replacing the four copies. Guard: `test_interp_gtest` `FixedArityCallsBeyondFourUseFixedAbi` runs `dom_range_selection_ops.ls` under `LAMBDA_TIER=jit` against its golden. ⚠ The interpreter planner's cap is still a hardcoded `arg_count > 5`; a 6-argument row would silently push its script to the JIT. Also: `ident_three_code_points` still fails at HEAD. **Method**: three grep-based classifications of the 105 rows were each wrong; what found `range_to_string` unpublished, the boundaries segfault and the arity cap was a realm-free script exercising the rows. |
| **ESO112 — de-realming the `DOM_RAW` rows: what the census said, and the two moves that landed (2026-09-04).** Asked to make the native rows language-neutral, the first job was to find out what actually couples them to JavaScript. A depth-1 census of the 105 bodies said 56 were realm-free, 5 realm-bound and 44 unfound; the 44 were macro-generated (`JS_RANGE_GETTER`, `JS_SELECTION_GETTER`, `JS_FORWARD_ITEM`, …) and a transitive walk over their expansions gave the real shape: **the 44-row range/selection cluster is already native** — `range_from`/`selection_from` are host_data unwraps, `build_range_object` is `vmap_new` plus the host brand, and `range_sync_props`/`selection_sync_props` are *empty*. Its only realm coupling was the throw path. **Move 1 — a realm-aware raise.** `dom_raise`/`dom_raise_named`/`dom_raise_type_error` in `dom_core`: in a realm, the JS DOMException-shaped throw as before; with no realm there is nothing to throw into (building the Error object allocates in the realm's pool), so the row answers **`ItemError`** — Lambda's error value, what `^` consumes — and logs the name and message so the failure is not silent. `throw_dom_exception` in `dom_selection.cpp` funnels 23 sites through it; 14 direct `dom_realm_throw(dom_realm_new_error_named(…))`/`dom_realm_throw_type_error` sites were rewritten. `dom_absent_to_null` passes `ItemError` through, so the Lambda face sees it. **Move 2 — the per-control dirty flags become element bits.** Class D's "realm-bound" rows were not DOM-value mirrors: they were the spec's *dirty checkedness / dirty value / option dirty selectedness / select dirtiness* flags parked as JS expandos (`__chkDirty`, `__valueDirty`, `__optDirty`, `__selDirty`) on the wrapper, read through OrdinaryGet — which, per ESO110, allocates realm-free. `DomElement` already keeps option selectedness as native bits ("keep it on the native node"), so the four flags are `ELMT_FLAG_*_DIRTY` bits 23–26 with accessors; `dom_expando_flag_is/_set` map the four names onto them and keep the expando path for any other flag, and the ten sites that touched the expando map directly now go through the helpers. **What remains realm-bound, and why it should**: `invoke_raw` (the ordinal executor is the JS method surface; each ordinal is its own operation), `dom_compile_event_attr_to_expando` reached from `after_set_attribute` (compiling `onclick="…"` is JS by nature; a Lambda-only document has none), `replace_with_bridge`'s listener calls, and `range_get_bounding_client_rect`'s rect objects, already guarded on `dom_realm_active()` since ESO81. **Next**: promote the realm-free, Item-uniform rows to `DOM_OP` — but *empirically*, one row at a time from a realm-free script, because this census was corrected three times and `create_range`'s ESO81 note ("needs the JS object model") now looks stale: its builder is `vmap_new` and its state comes from an engine seam. **Verified**: `dom.dispatch` realm-free still answers `true`; `dom_event_native_record`, `dom_module_props` pass; form/radio/select/dom_pkg fixtures 36/36; the five DOM Lambda goldens byte-identical; lambda 4091/4091; radiant 8091 with UI Automation 114/114 and DOM UI 108/108, the one failure the pre-existing `ident_three_code_points`. The three `rte_*` fixtures fail when the gtest binary runs standalone and pass under the harness — the same standalone-environment pattern as `test_form_*` and `editable_*`; the raise helper never fired during them. |
| **ESO111 — the catalog stops advertising what nobody can call (2026-09-04).** A retirement pass after ESO102/ESO110. Removed: the **`dispatch` engine seam** (`dom_engine_dispatch`, its weak default and its provider), dead since ESO110 routed the name spelling through the same event-building path as a descriptor; **61 host-API aliases** in `radiant_dom_bridge.cpp` that no longer expand anywhere, the residue of moving call sites onto rows; and **38 `DOM_RAW` rows** nothing reaches, which took one transitively-dead wrapper in `jube_registry.cpp` with them. `JUBE_HOST_API_VERSION` is **4** — removing members is not additive. The catalog is **177 `DOM_OP` + 105 `DOM_RAW`**. **The method is the point.** A grep for `dom_catalog->NAME` said 93 rows were unreached; counting names handed to `RADIANT_GETTER`/`METHOD_n` cut that to 50; finding `RADIANT_GETTER_D` and `RADIANT_DOM_MEMBER_FROM_CATALOG` cut it to 38. Three corrections to one scan, each because a name only exists after preprocessing — the same blind spot that under-counted the `->dom` census three times. The number was still moving, so the answer came from deleting the rows and letting the build fail if any was reachable. It did not: **the compiler confirmed all 38**. A grep is a hypothesis; the compiler is the census. **Deliberately kept**: 17 unpublished `DOM_OP` rows that no caller reaches today, because several are documented decisions rather than residue — `invoke` is the array-shaped door kept binding-only (ESO106), `create_event` is JS's by nature (ESO104), and `report_validity`/`submit_form`/`request_submit` were added at slice 5 as the named operations the module *may* cross. A row with no caller is dead ABI only when nothing was ever meant to call it; these were named on purpose, and deleting them would undo a ruling rather than clean up after one. |
| **ESO110 — RESOLVED 2026-09-04: the event is a native record, and a Lambda script with no realm can dispatch.** The user's framing was right and mine was not: the dispatch fields point at things the DOM tree already owns, so they belong in `host_data` as raw pointers — exactly what every node wrapper's `host_data` is — and the collector never enters. `RadiantDomEventRecord` now carries `target` (`srcElement` is its alias), `current_target` and the frozen propagation path as **`RadiantEvtKey`** values: a kind (`NODE`, `WINDOW`, `DOCUMENT`, `CONTAINER`) and a pointer. The path was already native in the core (`void*` + `is_dom`, with window/document sentinels); it was only being *projected* to an Item array and cached in the backing store. The core maps its sentinels to kinds (`path_key_of`), and the module projects a kind back to an Item on read (`radiant_dom_event_key_to_item`) through the wrapper cache, so `event.target === event.target` still holds. `composedPath()` builds its array on demand from the record. **Why no tracing is needed**: a `NODE` is tree-owned; `WINDOW` and `DOCUMENT` are resolved from the realm at read time and carry no pointer; a `CONTAINER` (a plain `EventTarget` object) is rooted by the dispatch frame for the dispatch that stores it, and `current_target` and the path are cleared when dispatch ends. The one residual is `target` on a plain `EventTarget` *after* dispatch, which is valid for as long as the caller keeps that object alive — documented at the type. `detail` stays where it was: it is realm-owned and native never reads it, and the backing store is the traced place for a realm value the record cannot see. **Two shadowing bugs closed on the way.** The core-name table now claims `target`, so a key can no longer be answered by the record on one path and the backing store on another (which is what broke the first attempt: `srcElement` was in the table without a branch). And the event's fields are written and read through the module's own member protocol — `event_set_value`, `event_flag_get`, `event_get_item` — never through OrdinarySet/OrdinaryGet. **The realm coupling, finally named**: it was not the event at all. `radiant_dom_window_get_property` is probed by *every* ordinary property read, and its guard calls `script->global_this()`, which lazily **creates** the global — so in a realm-free script any OrdinaryGet on any object allocates, and OrdinaryGet on a host VMap also materialises an own-property descriptor (`radiant_dom_data_descriptor`, four `map_put`s). The last such read on the path was `cancelable` at the tail of `dom_dispatch_event`; `window.event` is guarded on `dom_realm_active()`, which requires `js_input`. A backtrace at the failing write found this in one run after three rounds of guessing had not — the compiler is the census. **Verified**: `dom.dispatch(el, {type: "custom"})` from a plain Lambda script answers `true`; `dom_event_native_record` and `dom_module_props` pass; the DOM fixtures pass 36/36; lambda 4090/4090; radiant 8091 with one failure, `WptCssSyntax/ident_three_code_points`, which **fails identically at HEAD** and is a computed-style check unrelated to events. |
| **ESO102 — RESOLVED 2026-09-03: one Document, and the proxy is retired.** `document` is the `#document` node's wrapper in every realm state; `js_get_document_object_value()` and `documentElement.parentNode` answer the same object because they are the same object. Six changes, each found by attempting the flip rather than by reading: (1) `js_get_document_object_value()` returns `doc_to_proxy_item(_js_main_document)`; (2) `radiant_dom_wrap_node` stops detouring the document node to a proxy; (3) `radiant_dom_host_type_for_node` maps the `#document` stub to the **Document** host type — it is a `DomElement`, so the element mapping gave it the Element interface and `document.createRange` did not exist; (4) `radiant_dom_is_node_host_type` includes the Document type, because a Document is a Node and unwrapping `document` was answering null; (5) **the crash that stopped ESO101 and two attempts here**: the Document interface's property get, property set and operation dispatch passed `object.vmap->host_data` straight to `dom_swap_active_document`. The proxy carried a `DomDocument*` there and a node wrapper carries a `DomNode*`, so the active document became garbage and *every* script died — reported only as "recovered from crash in JS JIT code" with no exception, which is why it read as mysterious. They resolve the document from the stub's `->doc` now; (6) `doc_to_proxy_item` asks for the **foreign** wrapper first and otherwise builds the node wrapper, which both keeps foreign-document identity (`foreignDoc.body.ownerDocument === foreignDoc`) and makes the whole path independent of a JS realm having bound a main document — which a Lambda-only script never does. **The predicted surface change did not happen.** The user ruled that Lambda's `document` should become the node wrapper with five goldens updated; in the event (6) removed the need, and `radiant_dom_read`, `radiant_dom_mutate`, `proc/radiant_dom_set` and the two custom-layout scripts all pass **unchanged**. The earlier breakage was an artefact of routing Lambda through the realm-dependent accessor, not an inherent consequence of unifying the identity. **The proxy is gone**: `js_document_proxy_vmap_marker`, the legacy branch that read it, the patch block in `dom_set_document` that kept its `host_data` current, and the `JsDomState::document_proxy` field itself. That field sat inside a **fixed-size root range** — `M(&state->dom.roots, &state->dom.implementation, 5, …)` — so removing it required dropping the count to 4; leaving it would have had the collector scan one slot past the end. |
...
| `lambda/dom/*.ls` | the 70-name usage census; migrated in F32 |
