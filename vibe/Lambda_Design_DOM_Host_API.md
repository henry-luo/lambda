...
> **Ledger series**: extends the DOM area's `ES#`/`ESO#`/`F#` per `doc/Doc_Convention.md` §4. Decisions **ES39–ES44**; open issues **ESO84–ESO101** (ESO81 and ESO93 resolved); stages **F28–F33**.
...
| ESO101 | **The document proxy is still not the document node.** ESO93 makes the document reachable and usable as a node; it does not merge the two representations. `dom_document_get_property` still answers off `_js_current_document`, so the proxy is inert without a realm, and `ownerDocument` hands back different objects to the two doors (the proxy inside a realm, the node outside). Merging them — one Document object that is a node and carries the document's own methods — is the real end state and belongs with the ES24 record-wrapper design. |
