...
| ESO102 | **The Document is still two objects across realm states — but the readers no longer care, and the remaining blocker is now measured rather than guessed.** `js_get_document_object_value()` hands back the proxy inside a realm and the node outside. **Step 1 landed**: `js_document_proxy_doc_from_item` accepts a wrapper around the document *node* as the same Document, so every reader is tolerant of both shapes before either writer moves. ESO101's attempt segfaulted precisely because the writers changed while the readers still keyed on the proxy's host type; doing it in this order removes that failure mode. **Step 2 was attempted and reverted, and it got much further than ESO101 — 28 of 29 fixtures rather than a segfault.** Flipping both writers (the singleton returns `wrap_node(doc_node)`; `wrap_node` stops detouring the document node to the proxy) needs two more changes that the attempt found and that are worth recording, because they are not obvious from the ESO101 note: (a) `radiant_dom_host_type_for_node` types the `#document` stub as an **element**, since that is what it is, so the wrapper presented the Element interface and `document.createRange` simply did not exist — the stub has to map to the Document host type; and (b) `radiant_dom_is_node_host_type` deliberately **excludes** the document type, so unwrapping `document` answered null — a Document is a Node and belongs in that predicate. With both fixed, one fixture still fails: `dom_pkg_edge_text` throws during script execution after `dom_set_document`, with the error surfacing as a bare `ItemError` rather than a TypeError. That is where the ES24 record-wrapper work actually bites, and it is the next thing to isolate. |
