...
- **The DOM bridge is monolithic and JS-only**: `dom.cpp` (12,415 lines) + `dom_events.cpp` (2,145) + `dom_selection.cpp` (1,825) + `dom_cssom.cpp` (1,518) ≈ **18k lines** wired through the central registry, with Radiant coupling flowing through `edit_bridge.cpp` relying on C linkage "matching sys_func_registry.c declarations" (`edit_bridge.cpp:822`). Lambda-side scripts have no modular access to the same engine.
...
**What**: carve the DOM/CSSOM bridge (`dom.cpp`, `dom_events.cpp`, `dom_selection.cpp`, `dom_cssom.cpp`, ~18k lines) into a Jube module exposing Radiant to **both** JS and Lambda.
...
1. **The performance objection is weaker than it looks.** The MapKind fast path matters for *plain JS objects* — shaped maps whose named accesses resolve to O(1) typed slot loads (through the stateless `NameId` head today; per-site inline caches were removed 2026-08-15 and are banned by D8.4.1v2/LC1v2). DOM nodes were never on that path: the DOM MapKind exists precisely to route DOM access **off** the IC path into C runtime calls (`js_dom_get_property` → per-property dispatch). Today's DOM access is already "kind check + C call per property"; a VMap access is "vtable-identity check + indirect call" — **the same dispatch structure, cost-parity by construction**. Unifying does not sacrifice a fast path; it replaces one C-dispatch route with another. Hot-property tricks the current code carries (interned-pointer key compares for `style`/`className`/`id`, the DOM wrapper cache at `dom.cpp:1508`) migrate *inside* the module's vtable, where every language benefits from them instead of only JS. And DOM-heavy workloads are dominated by layout/render cost, not property dispatch (document-mode JS already runs on the MIR interpreter for this reason). The existing baselines are still the proof: perf parity is asserted by measurement, not by this argument alone.
...
1. **Wrapper identity**: `el.parentNode === el.parentNode` must hold ⇒ exactly one VMap wrapper per `DomElement*`. Already solved once in the current code (thread-local wrapper cache, `dom.cpp:1508`) — the cache moves into the module/host, it is not new invention.
...
> **Status 2026-09-01 — the carve-out half landed** (`vibe/Lambda_Design_DOM_API.md`, stages F22–F26). The ~28k-line web-platform layer moved out of the monolith to `lambda/dom/` and dropped its `js_` prefix; the Lambda round-trip criterion is **met** — `test/lambda/dom_api.ls` queries, mutates and serializes a real DOM through `import dom`. Two criteria are **not** met and were re-scoped by user direction rather than quietly dropped: the `JubeHostDomAPI` table is **retained** (it is the dynamic DOM ABI — AST-interpreter dispatch and non-statically-linked modules such as Python), so "registry-driven with no host table" is superseded by ES34; and dynamic loading of the radiant module remains untested. The host DOM API's *grain* — ~216 meticulous slots, plus a second parallel DOM surface in `radiant_module.cpp` — is the subject of DOM API Phase 2 (ESO78), which unifies the two into one operation catalog.
