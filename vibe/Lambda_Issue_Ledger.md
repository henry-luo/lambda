...
**One runtime consumer was a real bug, not a migration.** The mapping pipe sized its traversal with `fn_len`, so `g |> ~["amount"]` gained one null row per attribute and poisoned `sum`. Its own comment already said elements pipe over content ("a group Element's attributes describe its key while its children are the rows"); both tiers now call a shared `fn_pipe_count`, so the rule cannot drift between them again.
