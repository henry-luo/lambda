...
**Measured production result (2026-08-19):** after `make generate-grammar`, contextual-start removal, navigation/member unification, and structural greedy namespace names, the landed external boundary is `parser.c` **6,852,463** bytes, `parser.o` **949,840** bytes, `scanner.o` **17,072** bytes, and `libtree-sitter-lambda.a` **967,456** bytes. It has 4,862 states, 798 large states, 218 symbols, 109 tokens, and seven external symbols. Removing `DOTTED_NAME_HEAD` saves 1,464 scanner-object bytes and one external symbol; replacing its contextual commitment with a structural namespace-dot token costs 56,006 generated-C bytes, 8,472 parser-object bytes, 20 states, and 14 large states. The symbol/token totals remain unchanged because the structural rule introduces a distinct lexical namespace-dot symbol. Focused static-path, provider-root, parent/root navigation, qualified-name, whitespace-qualified-name, and explicit relative-path-child fixtures pass. `base_type_token` remains deferred because its experimental fixed-vocabulary scanner collided with broader statement candidates. The differential/AST-shape work listed in GR5-P5/P6 remains deliberately outstanding follow-up.
