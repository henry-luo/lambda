...
- **Date:** 2026-08-18
- **Status:** **TYPE/ISLAND + RETURN/WHOLE-VIEW EXTERNAL SEAMS LANDED (2026-08-19); DIRECT PATH AST PARSER LANDED.** Production uses first-party external tokens for full type patterns, string/symbol islands, declaration return contracts, and whole view patterns. Static paths are parsed directly to AST in Lambda code after Tree-sitter retains their necessary `/` and `.` lexical shell. `make build` and the focused runtime fixtures are green; the full Lambda baseline is the remaining close-out gate. Differential full-grammar/span coverage, external path-token feasibility, and the optional base-type extraction remain outstanding.
...
- **Scope:** Lambda has three nested sub-DSLs whose interiors do not need Tree-sitter's general expression parser: **type patterns**, **string/symbol patterns**, and **paths**. Reduce5 moves the scanner-safe type/string extents into first-party external C tokens and parses every sub-DSL interior on the Lambda side. Paths retain their lexical shell because `/` also starts division and `.` also starts member access. Type patterns have three distinct entry forms: a full type pattern, a restricted view type pattern, and a restricted declaration return type.
...
- **ID series:** `SC0`–`SC7` records the landed type/island campaign. `GR5-P0`–`GR5-P7` below records the Reduce5 extension, with landed work and remaining close-out tasks separated explicitly.
...
**`grammar.js` — the trimmed production grammar.** Generates the shipped `src/parser.c`. Its full annotation type-patterns, value-position string/symbol islands, whole view patterns, and declaration return types are external tokens. Paths remain ordinary grammar at the lexical boundary; ordinary statements and expressions remain Tree-sitter grammar.
...
**`scanner.c` — first-party end-finder only.** The active tokens, beside contextual `_start`, are `TYPE_PATTERN`, `PRIMARY_TYPE_PATTERN`, `PATTERN_ISLAND`, `CONTENT_TYPE`, `VIEW_PATTERN`, and `RETURN_TYPE`. `VIEW_ATOM` was removed once whole-view parsing landed. It remains single-pass and stateless — scanner serialization stays empty.
...
Data flow after landing: `source → parser.c (scanner.c emits bounded type-form tokens; grammar preserves path lexical shape) → build_ast.cpp → Lambda-side parser reconstructs runtime-compatible AST nodes + attached Type* data from each token span or a direct AstPathNode from the path source → typed AST`. `AST_NODE_CONSTRAINED_TYPE` still wraps `{Type* base, AstNode* predicate}`.
...
**Current boundary, not a permanent exclusion.** Production `return_type` and `view_pattern` are external-token seams. Their historical inner rules (`return_type_pattern`, `return_occurrence_type`, `view_pattern_union`, `occurrence`, and `occurrence_count`) remain in the shared/full grammar but are unreachable from the shipped parser and no longer have production CST builders. `parse_path_expr.cpp` directly builds every static path AST, but `path_expr` stays grammar-level: an external token cannot distinguish a leading `/` from division or a leading `.` from member access. `base_type` remains ordinary grammar because it is also a value/member/key spelling; the attempted fixed-vocabulary token collided with broader statement candidates, so that optional extraction stays deferred. Statement-level `object_type`/`view_stam` bodies, expression defaults, dynamic `path[expr]`, dynamic `value.~~`/`value./`, and `that` predicates remain ordinary Lambda grammar.
...
| Path | ordinary `path_expr`/member/navigation lexical shell | direct `parse_path_expr_text` after CST identifies a path; external token deferred | Parse root/scheme, authority, and static segments directly into `AstPathNode`; leave genuinely dynamic indexing/navigation as outer AST wrappers | direct AST landed; scanner seam deferred |
...
| static path source selected by `path_expr` / member/navigation outer AST | grammar-level lexical shell | `parse_path_expr_text` | `AstPathNode` with static `AstPathSegment[]` |

The direct path parser represents a **complete static path**, not merely today's initial `path_expr` leaf. A future external-token proposal must first factor the equivalent reference rule so the mapping is visible and mechanical:
...
`GR5-P2` and `GR5-P3` are production external-token seams; `GR5-P4` has its direct-AST parser in production but deliberately retains Tree-sitter's lexical path boundary. Scanner branches are bounded and side-effect free on successful tokens; `parse_type_pattern.cpp` owns return/view interiors and `parse_path_expr.cpp` owns the path interior. The remaining work is parity automation, broader coverage, baseline close-out, and a viable path-token design.
...
### GR5-P4 — Direct path AST parser: landed; external token deferred

- Implement `parse_path_expr_text` as reusable rule functions plus a shared Path-AST builder. It creates `AstPathNode` and its compact segment array directly from the complete source span selected by the outer AST builder. Cover logical root `/.field`, relative `.field`, bare `/`, schemes/authority, identifiers, symbols, integer keys, `*`/`**`, base-type-named fields, `.~~`, and `./`.
...
**Implementation:** `parse_path_expr.cpp` maps root/scheme, quoted/identifier/integer/wildcard/parent/root segments and file authority directly to `AstPathNode`; `build_static_path_ast` is also the legacy CST finalizer. `AstPathNode::file_local` is removed. `build_member_expr`, `build_navigation_or_path_expr`, and `path[expr]` try the same parser before their legacy collectors, so provider paths also avoid an inner path CST walk. Generic navigation remains untouched. A complete `path_expr_token` was tested and reverted: `/` is also division (`1.0 / float(...)`) and `.` is also member access (`state.moves`); the external scanner has no prior-token context to distinguish either form. Any future scanner seam must prove those lexical conflicts impossible before it lands.
...
- Extend `make test-grammar-diff` to compare accepted/rejected files and source spans for the four production token seams: full type, string/symbol island, whole view pattern, and declaration return type; also compare direct-path AST shape against the full grammar.
...
**Measured production result (2026-08-19):** after `make generate-grammar` and a forced grammar-archive rebuild, the landed external boundary is `parser.c` **7,318,818** bytes, `parser.o` **1,015,848** bytes, `scanner.o` **15,496** bytes, and `libtree-sitter-lambda.a` **1,031,888** bytes. It has 5,003 states, 903 large states, 219 symbols, 108 tokens, and seven external symbols (including `_start`). That is −41,293 parser-source bytes and −17,560 parser-object bytes versus SC4, while the scanner object grew by 5,016 bytes. The focused runtime corpus, including the `else {` graph regression and `|>` return-boundary regression, is green; the final `make test-lambda-baseline` passed **3,812/3,812** combined Input and Lambda tests (Lambda 1,708/1,708, including 738/738 script goldens). Both `path_expr_token` and `base_type_token` remain deferred because their experimental scanner branches caused lexical regressions, so they were rolled back rather than broaden the committed scanner. The differential/AST-shape work listed in GR5-P5/P6 remains deliberately outstanding follow-up, not a condition withheld from this landed runtime slice.
...
| **GR5 production (2026-08-19)** | **7,318,818** | **1,015,848** | **5,003** | **903** | **219** | **2** |

GR5's final deltas versus the pre-campaign baseline are **parser.c −17.9%, parser.o −22.5%, states −21.8%**. Its archive is 1,031,888 bytes versus the same-build fresh SC4 archive of 1,044,432 bytes (−12,544 bytes, −1.2%); the archive includes a deliberate 5,016-byte scanner-object increase for the type/view/return end-finders, including the return-keyword and pipe-boundary guards. The runtime side now comprises the existing type parser plus the reusable direct path parser; production AST construction no longer rebuilds return/occurrence/view interiors from a Tree-sitter CST and no longer walks path interiors to construct `AstPathNode`.
