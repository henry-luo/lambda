...
- **Date:** 2026-08-20
- **Status:** **TYPE/ISLAND + RETURN/WHOLE-VIEW + PATH-BODY EXTERNAL SEAMS LANDED; CONTEXTUAL `start` RETIRED; NAVIGATION UNIFIED INTO MEMBERS; GREEDY STRUCTURAL NAMESPACE NAMES LANDED; QUERY POSTFIX PRECEDENCE EXPLICIT; ZERO DECLARED GLR CONFLICTS; BASELINE-GREEN (2026-08-19); LATEST TREE-SITTER SIZE POC FINDS NO MATERIAL WIN.** Production uses first-party external tokens only for the bounded type/pattern/path seams. `start(target, args, options)` now parses through ordinary call grammar and is recognized as a concurrency system `pn` by the AST builder; the `_start` external and `start_expr` grammar node are gone. `path_parent` / `path_root` are now `member_expr.field` alternatives rather than a parallel `nav_expr` chain. S2.4.3v3 makes namespaced `dotted_name` maximal; a relative-path element child is spelled `<svg \.rect>` with no delimiter at all (S16.9.4 respelling; the former `;` boundary is retired). The qualified-name scanner token and the former dotted-name/path conflict are gone. S7.6.3v2 puts query at the left-associative postfix/member tier, replacing the final `_expr` / `query_expr` GLR conflict with named precedence. Tree-sitter v0.26.11 ABI 14 produces the same state inventory and a parser object 272 bytes larger than the pinned v0.24.7 build; ABI 15 is 8,552 bytes larger. Differential full-grammar/span coverage and the optional base-type extraction remain outstanding.
- **Design authority:** `vibe/Lambda_Type_Pattern.md` §3 — ledger CT1v2–CT10, ALL decided (that-clause at statement-level slots only; value-annotation `^` dropped; nested map/element pure patterns; parens hold simple patterns; `type_pattern` embeds zero expressions); `vibe/Lambda_Type_Path.md` PTH16v3 (maximal namespace-qualified names; undelimited `\.` relative-path content). Formal: S2.4.3v3 (names versus paths), S7.6.3v2–S7.6.4 (query at the postfix/member tier), S11.1.2 (islands), S11.2.1 (type match), S7.5.1 (raised-channel acknowledgment), S10.1.1/S10.1.3 (unions and lexical `~`), S10.4.1–S10.4.3 (parent navigation), S10.5.1–S10.5.3 (root navigation), D8.1.1v3 (C parser → typed AST pipeline), D8.1.2v2 (reference grammar regeneration), and SO9.
- **Cutover note:** Reduce5 remains the Tree-sitter grammar/scanner reference and
  size record. Normal Lambda files/modules now use the C RD/Pratt parser and
  direct AST sink under D8.1.1v3; the reference archive remains for explicit
  rollback, REPL fragments, and editor/binding consumers.
...
> **Superseded 2026-08-28 (S2.4.3v3).** The `;` boundary above is the
> 2026-08-19 spelling and no longer parses. S16.9.4 respelled the relative
> path `\.a.b`, which dissolved the ambiguity outright, and S16.9.3 made the
> boundary comma a biconditional that forbids a comma here. The current form
> is `<svg \.rect>` — no delimiter; `<svg, \.rect>` and `<svg; .rect>` are
> both errors. The rest of this section's grammar/AST consequences still hold.
