...
- **OB12 — Answered: `obj is element` reports the declared kind** (was spec SO40; answered by the D2.6.6v2 representation flip, recorded here 2026-09-21 when the closed item left the spec). The question presupposed the superseded model in which a nominal value was always element-shaped and `map` was the odd case. Under S2.1.1v3 an object wears its DECLARED structural kind, so the structural test simply reports that kind: an attribute-only type yields maps, and `is map` is true while `is element` is false; a type with a content pattern yields elements, and the reverse holds. `is object` is true either way — the orthogonality S2.1.1v3 rules. Pinned by `test/lambda/object_nominal.ls`: `[p is P, p is object, p is map, p is element]` is `[true, true, true, false]`, and the content-bearing type gives the same shape with the last two swapped. S11.3.1v2 still keeps `is` nominal for the `object` type itself, matched by record identity and never by a tag.
...
- **OB23 — A nominal name is not an element tag (2026-09-25).** USER: *"for type P { ... }, the nominal type name P is not enforced on the data that it is check against"*; to enforce a tag, *"go with type e = <elmt ...>"*. So `type B { label: string, string* }` still builds elements tagged `B` (S2.1.3v2's tag-name output stands), but `is B` never compares the tag — S11.3.1v2 compares the record, which is also why a hierarchy needs no tag-matching rule: each instance carries its constructor's name and `is Base` holds by derivation. Kind selection stays as D2.6.6v3 has it — a content section makes the type element-kinded — and the declared content pattern lives in `TypeElmt::content_list`, matched as a sequence-pattern slot (S11.1.6v3). A declaration that also names a tag, `type <B> { … }`, is deferred as DO31; the options weighed were the type name as the enforced tag for every object type (rejected: attribute-only objects would grow from a 32-byte map to a 72-byte element, and `p is map` would flip) and the explicit `<B>` marker (parked).
