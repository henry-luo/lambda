...
- **Containers**: lists/tuples = Lambda `Array`, dicts/objects = Lambda `Map` (`py_runtime.cpp:1340–1418`). **Zero `->extra` uses** in `lambda/module/py/` — no companion-pointer hack, no SF15-J analog needed. Container wide-scalar re-homing (SF15/SF16) arrives automatically through the shared `fn_*`/container helpers once frames exist.
