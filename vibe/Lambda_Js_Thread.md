...
**P2.2 — convert the three users.** Batch (`batch_crash_handler` + install block) and Radiant (`js_exec_crash_handler`, `script_runner.cpp:90–112`, plus its `sigaction` save/restore pairs at `:2032–2087`) delete their handlers and arm/disarm the shared kit around script execution. The event-loop drain's nested SIGSEGV guard, which this step was to audit, was removed on 2026-07-31 (ER-S6): the drain has no signal handling of its own, so there is nothing to convert. `mir_error_jmp` is a `longjmp` hook, not a signal — untouched beyond P1.3's TLS move.
