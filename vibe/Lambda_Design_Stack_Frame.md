...
**SF20 addendum (2026-09-07, D5.1.1→v2 / JSCU26).** The carrier converges. LambdaJS built the same Items-then-tail frame as a GC object (`GC_TYPE_JS_ENV`: first half Items traced through whoever points at it, second half raw tail never scanned) for generators and async functions, while `LambdaAsyncFrame` stayed a `mem_calloc` array whose Item half the task root-registers and re-registers on every growth. Two forms of one ruling. Ratified: `lambda_async_frame_enter_current` allocates the GC carrier, the task retains it through its one rooted slot, growth is allocate-larger-and-copy with a single store into `task->async_cursor` at frame entry, and the twelve `lambda_async_frame_*` helpers keep their signatures so no generated MIR changes. A `start` task keeps **strong** registry retention (its frame is never collected under it); a JS async activation task is registered weakly and lives by script reachability (JSCU25). Everything else in SF20 — write-through, the suspension-barrier invariant, non-LIFO heap allocation, copy-out restore — is unchanged; only "root-registered" in D5.1.1 v1 stopped being the definition and became one owner's way of holding the container. Design and gates: `vibe/Lambda_Design_Structs_JS.md` item 1 (JSCU25, JSCU26).
...
| 3 | **Async stack frames** (`LambdaAsyncFrame`, and LambdaJS generator/async envs) | **non-LIFO**, task- or object-shaped — a GC-heap container, owned by the task or the suspended object, reused across suspensions | suspended state: async locals as Item slots + wide-scalar tail (SF20) | Item region traced through the owner (D5.1.1v2; was root-registered in v1); tail never scanned |
