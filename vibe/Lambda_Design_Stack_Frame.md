...
**SF15 — Unify the `extra` mechanism; JS arrays migrate off `extra`.** *(decided 2026-07-15; JS resolution amended 2026-07-15 — reserved tail slot; the JsArray-subtype preference is REJECTED)*
...
**SF15-J — JS props migrate into a flag-gated reserved tail slot** *(amended 2026-07-15; replaces the earlier "JsArray subtype" preference)*:
...
- **Implementation plan:** `vibe/Lambda_Impl_Stack_Frame_JS2.md`. This migration must complete before phase-2 J3d turns JS number watermarks on (that is when out-of-band doubles start homing into JS-array tails); polyglot Lambda→JS wide-scalar writes make it live even earlier.
