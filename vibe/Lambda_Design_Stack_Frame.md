...
**Status:** design SETTLED; Lambda phase-1 implementation COMPLETE — SF1–SF20 decided, OS1–OS11 closed for the Lambda MIR-Direct scope. The portable return-ABI and number-slot implementation refinements are recorded below and in `vibe/Lambda_Impl_Stack_Frame.md`.
**Date:** 2026-07-15
...
**OS5 — Region sizing policy.** CLOSED for phase 1 (2026-07-15). Separate reservations are 16 MiB for roots and 64 MiB for numbers. Static-slot telemetry is available through `LAMBDA_MIR_LOG_FRAME_SLOTS`; explicit frame checks report stack overflow; POSIX demand paging and Windows reserve/commit preserve stable addresses; the GC driver decommits unused pages above the live watermarks after collection.
A later profile matrix (fn vs. pn, Lambda vs. JS, or script vs. page/worker isolate) is an optional tuning refinement, not a phase-1 correctness dependency. The implemented fixed reservations are virtual-address budgets rather than committed-memory costs, and the full gates plus stack-overflow tripwire completed within them.
