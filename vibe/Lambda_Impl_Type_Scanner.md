...
**`lambda/runtime/parse_type_pattern.cpp` — the hand parser.** Recursive descent over token source text, producing the same `AstNode`/`Type` structures today's CST builders produce, by calling the same underlying type constructors. It effectively becomes the schema parser (validator `.ls` schemas are type-dominant).

Data flow after landing: `source → parser.c (tree-sitter, with scanner.c emitting opaque pattern tokens) → build_ast.cpp → parse_type_pattern.cpp for token leaves → typed AST`.
...
- Entry points: `AstNode* parse_type_pattern_token(Transpiler*, TSNode)`, `AstNode* parse_primary_type_token(Transpiler*, TSNode)`, `AstNode* parse_pattern_island_token(Transpiler*, TSNode)`. Dispatch from the type-expr switch in `build_ast.cpp` (~11760) on the three token syms.
