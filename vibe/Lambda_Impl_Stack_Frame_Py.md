...
- **Scalar-home/hidden-ABI machinery is live** for Lambda, JS, and Python (`MirScalarReturnMode`, `incoming_scalar_home`, colored homes, and discard scratch). Python public and generator entries carry a trailing caller home, so `push_d`-produced floats are activation-owned until adopted into that home or a destination-owned slot.
