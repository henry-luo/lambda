...
This archive contains **168 historical records**: 163 fixed or resolved entries,
...
<a id="lr03-15"></a>**LR03-15 · `5u8 is (u8 | string)` is false (S11.1) · FIXED 2026-09-26 (found 2026-09-25)**
...
*Fixed 2026-09-26, by [LR13-12](#lr13-12), which had the cause:* the validator's base-type walk read `kind` before it tested the tag, so a sized type's `NumSizedType` read as a type construct. `5u8 is (u8 | string)` and `5i16 is (i16 | string)` are `true` on every tier. Fixtures `type_set_operators_expr.ls` and `type_repr_equality.ls` (LR03-15 section).

<a id="lr03-16"></a>**LR03-16 · A literal type alias used as a value prints a pointer · FIXED 2026-09-26 (found 2026-09-25)**
...
*Fixed 2026-09-26, by [LR03-30](#lr03-30), which had the cause:* every bare literal payload is wrapped as a type value. With `type Three = 3`, `3 is Three` and `Three <: int` are `true` and `type(Three)` is `type`, and `[One]` no longer prints an address. Fixtures `type_literal_alias.ls` and `type_repr_equality.ls` (LR03-16 section).
...
*Residue:* `<:` tries each arm of a union whole, so `1 to 5 <: (1 to 3 | 4 to 5)` is `false` ([LR03-21](#lr03-21)). Found on the way, all older than this fix: [LR03-19](#lr03-19), [LR03-20](#lr03-20), [LR07-30](Lambda_Issue_Ledger.md#lr07-30), and two more symptoms of [LR03-16](#lr03-16).

<a id="lr03-19"></a>**LR03-19 · In a nominal object type, a union- or range-typed field that is not last reads back a wrong value (S1.6) · FIXED 2026-09-26 (found 2026-09-25, while fixing LR03-18)**
```
...
*Fixed 2026-09-26.* `resolve_object_field` and `resolver_object_copy_base` (`build_ast.cpp`) stride by the field's storage size (`shape_entry_storage_size`), as `resolve_field_shape` does for map types, so a union-, range-, `number`- or `integer`-typed field keeps its 9-byte slot and `byte_size` counts it. Reads, writes, inherited fields and printing are right on every tier. Fixtures `object_boxed_field_layout.ls` and `proc/object_boxed_field_write.ls`, pinned in `kTune27TierParity`.
...
Found on the way: a derived object type dropped its base fields' defaults (`resolver_object_copy_base`), so with `type Dog : Animal { … }`, `<Dog name: "Rex">` stored null in an inherited `legs: int = 4` on both tiers. The copy now keeps the default. Tracing the fixture also found [LR03-19](#lr03-19)'s cause.
...
<a id="lr03-21"></a>**LR03-21 · `<:` does not split a range across union arms (S11.1.4v2) · FIXED 2026-09-26 (found 2026-09-25, residue of LR03-18)**
...
*Fixed 2026-09-26.* When no arm of an expected union holds a range whole, `contract_type_is_subtype` (`type_contract.cpp`) asks whether the arms cover it together (`contract_range_split_covered`): a same-domain range arm covers its overlap, a literal its one member (`lambda_range_type_position`, which `lambda_range_type_contains` now shares), and a type admitting the range's whole domain all of it; any other arm covers nothing. `R <: OneTwo` and `1 to 5 <: (1 to 3) | (4 to 5)` are `true`, and `1 to 5 <: (1 to 2) | (4 to 5)` stays `false`. The S11.1.7 reduction uses the same cover, from range and literal arms only, so `(1 to 3) ! (1 | 2 | 3)` is `none`, one of that ruling's residue items. Fixture `range_subtype_union.ls`, pinned in `kTune27TierParity`.
...
*Residue:* the JIT reads an imported predicate's names in the importer ([LR03-27](Lambda_Issue_Ledger.md#lr03-27)); a predicate that names its own type crashes compilation on both tiers ([LR03-28](Lambda_Issue_Ledger.md#lr03-28)).
...
<a id="lr03-26"></a>**LR03-26 · An inline pattern island never compiles as a constrained base · FIXED 2026-09-26 (found 2026-09-25)**
...
*Fixed 2026-09-26.* `resolve_type_pattern` (`parse_type_pattern.cpp`) compiles an island where it resolves (`compile_runtime_pattern`), so an island in any type position has its regex: a constrained base, a union arm, a map field or an array element (`"1234" is Digits` is `true`). A pattern it names has resolved by then, and a failure to compile is left to the evaluation that reports it. Found on the way and fixed: the JIT tested a `case \(d+):` arm with `==`, because `mir_is_type_value_node` did not list the island, so the arm never matched while T0's did (S1.6); and `fn_is` raised an error for a non-text value against a pattern, where S11.1.2v2's domain check makes `5 is D` `false`. Fixture `pattern_island_type.ls`, pinned in `kTune27TierParity`. Found and filed: [LR03-34](Lambda_Issue_Ledger.md#lr03-34).

<a id="lr03-29"></a>**LR03-29 · Type equality compares a compound type's payload tag only (S5.5.2) · FIXED 2026-09-26 (found 2026-09-26, while implementing S11.1.7)**
...
*Fixed 2026-09-26.* `lambda_type_repr_equal` (`type_contract.cpp`) compares normalized forms. Wrappers, parameters and binders are peeled; a literal compares by value (`1 == 1.0` as types, S11.2.1); `|` and `&` compare as the sets of their flattened arms, with `none` no arm of a union (S11.1.7); an occurrence by operand and bounds (`int? == int{0,1}`, `int* == int{0+}`), an array type by operand and length, and `!` by its ordered operands; a range by domain and bounds; a pattern by its compiled regex; a constrained type by base and predicate site (S5.5.1); a nominal type by record; and the kinds that share a TypeId by what they are (the sized kind, `date`/`time`/`datetime`, `list`/`array`, `object`/`map`, the any-tops). `lambda_type_repr_hash` agrees with it, and `lambda_item_hash` and `lambda_item_compare` use both, so grouping and map keys see equal types as one (S5.6.2). S5.5.2 names only `|`; reading `&` as a set too is the normalization this fix chose. `type(x)` now wraps the kind singleton for an array, a map without a named shape and a function, where a bare prefix with that tag had been read as the extended struct; `fn_is` knows `list` and `array` by their payload, so `[1, 2] is type((3, 4))` no longer crashes. One answer changed: `type()` names `datetime` for a date value, so `type(t'2025-01-01') == date` is `false` (it was `true` by tag). Residue: [LR03-33](Lambda_Issue_Ledger.md#lr03-33). Fixtures `type_repr_equality.ls` and `type_of_kind_is.ls`, pinned in `kTune27TierParity`.
...
<a id="lr07-19"></a>**LR07-19 · Named arguments to an object method bind by position · FIXED 2026-09-26 (found 2026-09-25)**
...
*Fixed 2026-09-26.* A statically resolved method's named arguments are put in parameter order at build time, against `TypeMethod::ast_def` (`order_method_named_arguments`, `build_ast.cpp`), so the bound-member call both tiers make receives them by name, and T0 runs the call instead of refusing it. The ruling this record asked for was already implied by direct calls, which give a skipped optional parameter its default, or null, and keep an explicit `null` (`f(1, c: 3)` is `[1, 42, 3]`, `f(1, null, 3)` is `[1, null, 3]`); the call ABI's absent-argument marker (`ITEM_MISSING_ARGUMENT`) already carries that for a trailing omission. A skipped optional parameter's position now holds a placeholder (`ast_is_omitted_argument`) that both tiers pass as the marker (T0's method path, the JIT's `mir_box_call_argument`), so `t.n(1, c: 3)` is `[1, 42, 3, 100]`. A skipped required parameter is E206 on a method call and a direct call alike (`report_missing_named_parameter`); on a direct call it had split the tiers, T0 failing the parameter's `null` and the JIT binding `0`. Found on the way and fixed: `lambda_ast_validate_call_arguments` compared a direct call's named arguments by position, so `fn g(a: int, b: string)` rejected `g(b: "x", a: 1)` with two E207s; it now reads each parameter's argument by name. Found and filed: [LR07-39](Lambda_Issue_Ledger.md#lr07-39), [LR07-40](Lambda_Issue_Ledger.md#lr07-40). Fixtures `object_method_named_args.ls` (`kTune27TierParity`), `negative/semantic/named_arg_skips_required.ls` and `named_arg_method_skips_required.ls` (`NegativeScriptTest.NamedArgumentsCannotSkipRequiredParameter`).
