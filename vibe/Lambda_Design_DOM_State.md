...
| **F3 — constraint validation** ✅ **landed 2026-08-26** | `lambda/package/dom/validate.ls` owns the constraint pass on the **post-mutation** `input` and on blur: `required`/`optional`/`readonly` reflection, `minlength`, `maxlength`, `min`, `max`, `step`, number/email/url, custom-validity override, writing `:valid`/`:invalid`. Covers ground native never had (RAD_19 known-issue 6). Predicates unit-tested (`test/lambda/dom_validate.ls`); end-to-end in `test/ui/dom_pkg_validate.json`; **state-equivalent to native** under the parity harness | `custom_validity`; `value` became engine-backed state (ES4) | **`te_validate` retired behind the claim**: the package covers every point it did — `init` (attach-time dispatch), the post-mutation `input`, and blur. It remains the implementation when the package is absent |
...
| ESO29 | **No regex on the Lambda surface** — `pattern=` validation cannot be implemented in the dom package, and native's `te_validate` has the same gap for the same reason (`TODO(F5)`, "needs lazy-compiled regex"). `js_create_regex` and `bash_test_regex` exist but nothing is reachable from Lambda | add a regex system function, or expose the existing engine through the `radiant` module |
...
| ESO32 | **A validity change does not restyle** — `te_validate` writes `:valid`/`:invalid`/`:optional`/`:required`/`:read-only` without syncing the pseudo-class, so CSS never re-cascades on a validity change; an `input:invalid { … }` rule does not take effect in either the native or the package path (verified with the same fixture failing on both). The package deliberately matches this so the migration stays state-equivalent, rather than shipping an unverified behavior change inside a migration | fix in both paths together, with a test that shows the rule actually applying |
