...
| **F1 — checkbox & radio** | `form.ls`: checked/indeterminate toggle, radio-group exclusivity, `change` dispatch, disabled gating | `radio_group`, `form_of` primitives | checkbox/radio activation blocks in `event.cpp` (MOUSE_UP path incl. `radiant_uncheck_radio_group`) |
| **F2 — select** | selection commit policy: `selected_index` writes, `change` on commit, keyboard selection (arrows/Enter/Escape policy) | `select_options(elem)` read primitive | select-activation & option-commit code in `event.cpp`; dropdown open/close *overlay mechanics stay native* (geometry, paint, outside-click capture), policy of when-to-open/commit moves |
...
| ESO25 | **Script-less pages own no runtime** — verified during F0b: `execute_document_scripts` only creates a `Runtime` when a page actually has scripts, so a plain HTML page with no `<script>` has neither `lambda_runtime` nor `js.runtime`, and `dom_document_script_runtime()` returns null. Behavior templates need a runtime on *every* interactive document, so document setup must **ensure** one exists before loading the dom package. This does not weaken ES12 (still one shared runtime per document) — it adds a creation obligation the design assumed away | resolve as part of F0b package loading; decide the creation point (document setup vs first behavior dispatch) and who owns teardown |
