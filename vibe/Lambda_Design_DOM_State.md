...
| ESO34 | **A JS page gets no constraint validation** — surfaced 2026-08-26 by removing the native validator. The dom package defers on a document with a live JS DOM realm, so with no native fallback such a page now has no `:valid`/`:invalid` at all (`:required`/`:optional`/`:read-only` still reflect natively). Lifting the deferral no longer crashes — EO5v2 fixed that — but it fails for a newly identified reason: **module state ids are handed out from a `Runtime` counter (`lambda_module_state_reserve`) while the slabs they index live on the `EvalContext`, and a Lambda `Script` never assigns `module_state_id` at all**, so every Lambda script uses slot 0. That is harmless while their `var_count` is 0 and they share a fresh runtime, but a JS realm reserves slot 0 with `JS_MAX_MODULE_VARS`, so the package's modules trip "sealed layout changed for module 0" and it registers **zero** templates. The same one-`Runtime`-per-thread limit is what blocks a Lambda-script iframe under a page that owns a runtime (ESO27) | core-runtime fix (D8): give each Lambda `Script` a reserved module-state id; note this touches REPL variable persistence and module snapshot/restore, which rely on the stable id today |
