...
- Every LR13-10 reproducer now answers correctly on both tiers, and the validator CLI accepts three `<li>` against `<li>*`/`<li>+` and reports a per-position tag mismatch against `<li>, <li>, <li>` given `<p>` children.
...
- Docs at landing: S11.1.6 and S2.1.3 footnote rows, the D2.6.6 rows, LR13-10 to the fixed archive, LR_03 §4 and LR_11 §2.1.
