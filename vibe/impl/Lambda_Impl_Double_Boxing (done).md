...
- [x] **Raw `>> 56` sweep** — ~24 sites / 11 files: classify each as (a) downstream of `get_type_id` (fine), (b) tag-literal compare provably never seeing a float Item (fine), (c) needs the `DBL_MASK` test added at S1. Include `& 0x00FF…` mask-only sites and any private tag arithmetic in the Jube runtimes (`lambda/module/py/`, `lambda/module/rb/`, `lambda/module/bash/`, …).
