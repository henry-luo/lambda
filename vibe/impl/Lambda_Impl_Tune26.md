...
- Later T26-4 ownership slices distinguish a pure in-place var T[] callee
  from one that retains, forwards, captures, or rebinds its parameter.  The
  AST records those effects so direct native calls retain caller-home
  publication only where a callee can replace the caller-visible owner
  (S9.2.1, S9.2.2, D3.3.3v3).  A same-index non-null float[] store can use
...
| Check | Result | Scope |
|---|---:|---|
| `make build-test` | passed | fresh Lambda and test executable build |
| focused `test_lambda_gtest` ownership/typed-path filter | 22/22 passed | snapshots, retained aliases, `var` reborrows, and nullable float rejection |
| focused `test_mir_emission_gtest` Tune26 filter | 8/8 passed | direct-store, ownership, nullable-lane, comparison, and ratchet shapes |
| `git diff --check` | passed | current working-tree patch |

An earlier experimental relaxation of `var ArrayNum` COW was rejected.  In the
reproducer `let snapshot = values; values[0] = 2.0`, a raw store caused the
snapshot to observe `2.0` rather than its old value.  The expected result was
`12 2`; the trial produced `22 2`.  This establishes that call-boundary
preparation alone does not prove exclusive ownership after an in-body snapshot
capture.  The retained-effect analysis keeps that dynamic COW case guarded,
preserving S9.2.2 and D3.3.3v3.
...
The r20 artifact is
temp/tune26_r37_r20_compact_fallback_nbody2_100.json: identical Nbody
source hashes, JIT tier, 30-second timeout, and 100 valid alternating pairs.
Its Result37/candidate medians were 57.462/65.194 ms.  The compact fallback
reduced advance's MIR side-stack frame from 456 to 384 bytes, but that code
size reduction did not establish a causal speedup; the G2 residual remains.

### Work still outstanding

1. Run the required full Lambda, Test262, MIR GC-stress, T0/JIT, and sanitizer
   checks on a stable candidate.
2. Repeat the complete 43-row interleaved
