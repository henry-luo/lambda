...
**Date:** 2026-09-07 (rev 71 — **loop-bodied first-entry promotion + aggregate value params (D8.1.1v7, §3.0.59)**; rev 70 satellite write-back: indexed/member stores and untyped `var` parameters promote through the CW33 home transport (D8.1.1v6, §3.0.58)**; rev 69 widened admission to `any` parameters and local `var`/assignment)
...
### 3.0.57 Satellite admission widening — 2026-09-06 (rev 69, D8.1.1v6)

- **`any` parameters and local `var`/rebinding statements promote.** `interp_satellite_supported` no longer refuses a plain `any` parameter, and `interp_scan_satellite_node` no longer refuses `AST_NODE_VAR_STAM`/`AST_NODE_ASSIGN_STAM`; aggregate/structured contracts, `var` parameters, indexed/member stores and the other v5 pins are unchanged. Design record: `vibe/Lambda_Design_Ast_Interpreter.md` §5.2.1; ruling **D8.1.1v6**.
- **Two latent defects fixed on the way.** `compile_ast_function_satellite` rebinds the module state's static image after interning (the state's const pointer went stale on reallocation; pdf `path.ls` `apply_op` crashed in `fn_eq`), and the dynamic-call argument builder appends with the new verbatim `array_push_argument` (`array_push` spliced a content-list argument: `test_latex_includegraphics` reported `build_kv_pairs expects 4 arguments, got 5`).
- **Gates.** Auto-tier differential 751/751 exact against the pre-widening binary; forced tiers unchanged; baseline green under AUTO. Measurements in `vibe/impl/Lambda_Impl_Tune21.md` §T21-3.
- **Partition side effect.** `make interp-sweep` now reports **750 interpreted matches, 1 counted fallback** (was 737/14): the 13 task-backed `conc/*` rows were excluded only because their async satellite membrane failed closed on local `var`/assignment statements, which this widening admits. `test/lambda/interp_p0_subset.txt` and `interp_excluded.txt` were regenerated by the target.

### 3.0.62 DO30 was a collector defect, not inference — 2026-09-07 (rev 74, D4.3.4)

- **The premise.** After D8.1.1v9 splay ran 446 ms on the auto tier (= eager) where the one-definition satellites of t45 had run 256 ms, so DO30 blamed the eager tier's parameter inference. Two inference facts were real -- `splay_node`'s `key` takes a float lane whose every use is `fn_lt` against an untyped member read, so it is re-boxed per use and unboxed per recursive entry -- and a rule that counts a comparison as numeric use only when its partner can be native did remove the lane. It changed nothing on the eager tier (1.03) and cost binarytrees 1.24x and gcbench 1.26x (×7 pairs), so it was reverted.
- **The real cause.** `LAMBDA_GC_STATS`: same 5 collections, mark 18 ms (t45 auto) vs 90 ms (eager). Per-collection traces: the t45-style run traced 17,451 of 420,980 objects at its fourth collection and 10,817 of 774,376 at its fifth, and freed the rest -- the tree was unreachable to the collector yet the program kept using it (freed slabs keep their bytes). `LAMBDA_GC_POISON_FREED=1` turned that into "cow map mutation rejected non-map owner type 221" on every tier, and `LAMBDA_GC_FORCE_EVERY=1` made a 40-node splay fail deterministically on interp, jit and auto (t45's eager release included). A root-slot dump at the fourth collection showed node maps whose `right` field was still typed `null` in the shape but held a live child pointer at byte offset 17: `{key: any(9 bytes), left: null, right: null, value: any}`. `gc_trace_shape_field` skipped the `null` lane and `gc_trace_data_words` (eight-byte stride) never landed on offset 17. Whether a given instance's `left`/`right` got retagged to `map` depended on which store path touched it first, which is why the one-definition images (different store lowering) exposed it and the cluster/eager images mostly did not.
- **Fix.** `gc_trace_shape_field` marks a `null`-typed lane's word conservatively (D4.3.4). Companion: `fn_map_set`'s same-width `null` retag no longer types a shared literal shape's field as `error` (an error value assigned into a `null` field had made every later literal at that site fail its own construction -- "unknown map storage type error" -- which was how the interpreter tier's recursion-budget errors cascaded).
- **Pin.** `test/mir/lambda/gc_splay_null_lane.ls` (splay reduced to 40 nodes): the forced-collection stress sweep (`test_mir_gc_stress_gtest`, collection at every allocation, freed objects poisoned) failed it on every tier before the fix (interp "unknown map storage type error", jit E308 stack overflow from a cyclic tree) and passes after.
- **Measured:** auto e2e median of 5 (`t51` → `t53`, jit reference): splay 410 → 406 ms (jit 404), splay2 410 → 431 (406), richards 496 → 500 (498), deltablue 251 → 252 (254), havlak 176 → 177 (174), gcbench 27 → 27 (28), diviter 302 → 300 (300) -- every row at the eager tier's time. Eager A/B `t51 → t53` on 18 GC-sensitive rows 0.90–1.04 (havlak read 1.15 at ×3 and 1.00 at ×7; mark time 4.0 vs 4.1 ms), so the conservative `null`-lane mark costs nothing measurable.
- **Gates:** auto / jit / interp sweeps (764 scripts) 0 regressions; `make test-lambda-baseline` 4140 / 4140; `test_mir_gc_stress_gtest` 107 tests passed including the new pin (it failed on the pre-fix binary on every tier); `make interp-sweep` regenerated; benchmark golden check 111 / 113 on both the eager and the auto tier (cd2_orig, nbody pre-existing).
- **Also recorded.** Two cluster diagnostics used here: `LAMBDA_SATELLITE_CLUSTER_CAP=1` reproduces the one-definition images; `TEMP`-style per-collection root dumps were removed after the finding.

### 3.0.61 Satellite cluster co-compilation — 2026-09-07 (rev 73, D8.1.1v9)

- **Why.** After D8.1.1v8 nothing was pinned in richards/deltablue/diviter, yet auto ran 1.2x/1.4x/66x the eager tier. Dump census (auto | jit): richards `fn_call_borrowed_into` 40 | 0, deltablue 111 (+28 `fn_call1/2_into`) | 0 -- every call between satellites was boxed dynamic dispatch through the callee's wrapper; diviter's `_diviter_div` raw entry ran `fn_ge`/`fn_sub` because a one-definition root has no call sites to infer from (its callers are in `benchmark`).
- **Mechanism (transpile-mir).** `compile_ast_function_satellite` builds the temporary root from the target's direct-callee cluster (`mir_satellite_collect_cluster`: module-level, promotable, `FnPromotionCell::state == INTERP`, cap 64, module order); `MirTranspiler::satellite_cluster` is set before the prepasses; `mir_satellite_defines` replaces the `entry_node != target` test at both call-emission sites; `prepass_collect_call_sites` walks the owner's complete AST for a satellite; `mir_callsite_canonical_fn` maps a root copy to its original by shared `body` at every `key.fn` lookup; after linking, `interp_publish_satellite_member` (new, interp.cpp) publishes each member's `_b` entry.
- **Publication cost.** The first cut fetched every member's `_b` entry through `jit_gen_func`, which re-walks every module, reloads them and RE-LINKS the context per call: 47 members cost ~180 ms on deltablue -- more than the 115 ms compile -- and showed up as a 1.27x auto/jit gap with an identical self-timed workload. The link already generated every function (gen interface), so members resolve through `find_func` (the item address). Each image now logs `interp-tier: satellite image function=… members=N compile_ms=…` (transpile + link + publish), which is how the cost was attributed.
- **Module loads inline.** `mir_module_slot_load_item` and `emit_load_const` no longer call `lambda_module_var_at` / `lambda_module_const_at_state` under a satellite; they emit the eager path's loads from `state->vars[slot]` / `state->consts[index]`, re-reading the slab pointer from the module state at every use (a REPL-grown slab or a rebound const image is never stale). deltablue read 103 module `let` constants and 22 consts through calls in its hot loops, richards 118.
- **Diagnostics.** `LAMBDA_SATELLITE_CLUSTER_CAP=N` bounds the cluster (1 = the old one-definition image); `LAMBDA_SATELLITE_CLUSTER_SKIP=a,b` keeps named definitions out of it (they stay dynamic targets). Bisecting deltablue2's hang with them took six runs.
- **Two defects on the way.** (1) Copy-keyed call-site lookups: the collect rounds keyed entries by the original nodes, define time by the copies -- inference at define time saw no call-site evidence (`evidence=40 → any`). (2) Discovery order: with the target first, `warm`'s facts were pre-registered before `half`'s, whose return lane `warm` inherits; the direct edge passed a raw lane where the callee expected an Item (SIGSEGV in `fn_numeric_binary`). Module order matches the eager compile.
- **Measured (release, auto e2e wall ms, median of 3, `temp/lambda_t46_final.exe` → `t47_final`, jit reference):** diviter 19.96 s → 349 ms (jit 345; 66x → 1.01x), richards 786 → 620 ms (jit 614; 1.01x), deltablue 464 → 348 (jit 347; 1.00x), deltablue2 1039 → 365 (jit 370), havlak 274 → 237 (jit 238; 0.99x), json2 193 → 77 (jit 75; 1.02x), navier_stokes 292 (jit 288), jetstream/richards 72 → 77 (jit 74), base64 38 → 40 (jit 40), hashmap2 1093 → 162 (jit 160), cd 1122 → 973 (jit 974), cd2 1296 → 787 (jit 807), spectralnorm 99 → 41 (jit 41; its T0 divergence is gone because the module promotes as one image), nbody2 1348 → 97 (jit 112); the self-timed deltablue workload is 127 ms on both tiers, so every remaining auto/jit difference is startup plus the image compile. ⚠ splay 256 → 446 (jit 444): the uninferred per-function satellites had been 1.7x FASTER than the eager tier here; with the eager tier's call-site inference the auto tier now matches it (DO30 -- an eager-tier item: `splay_find(tree, key)` takes a float `key` lane and `next_random`/`insert_new_node` native float returns, and the inferred code is slower than the boxed bodies).
...
- **Fixture:** `test/lambda/proc/interp_satellite_cluster.ls`.

### 3.0.60 Typed `var` parameters in satellites — 2026-09-07 (rev 72, D8.1.1v8)

- **Why.** Every function the scan still pinned in navier_stokes (10), nbody2 (3) and json2 (5) was refused for one reason: a typed `var` parameter (`var x: float[]`, `var p: Parser`), plus their callers through the call-site gate. The raw-lane ABI has no CW33 home for them.
- **Admission.** `interp_satellite_supported` and the scan's `CALL_EXPR` case no longer refuse typed `var` signatures. Instead `interp_fn_rebinds_typed_var_param` (one walk per definition, `FnPromotionCell::typed_var_rebind`) pins a body that ASSIGNS to a typed `var` parameter, and pins any satellite that would call such a body directly -- T0 publishes the rebind through the home, a raw argument would not be reloaded.
- **Boxed edges (transpile-mir).** `mir_dynamic_call_var_param`: every `var` position of a statically known callee travels through `Context::mir_var_homes` on the dynamic edge (pre-detach of a shared root included); `mir_emit_var_home_reload` gained a `typed` flag that reloads the register but keeps the raw descriptor (S9.2.1 invariance). `emit_boxed_abi_wrapper`: for a typed `var` position it reads and clears the cell, `cow_prepare_write`s the argument (the CW33 callee-prologue prepare), admits it, and after the raw call (or the boxed slow body) stores the binding's boxed container back through the home.
- **What went wrong first.** Without the wrapper prepare, a container that had been marked shared by an earlier callee (T0 marks a mutated plain parameter's value; `let keep = x` marks it) made the raw `var` store COW-copy into a replacement the raw entry could not publish -- writes silently lost (probe `temp/t4/tv_c.ls`). Without transporting typed positions on the dynamic edge, a promoted caller never saw the T0 callee's coerced or rebound container. The eager direct edge has the first gap when the mark happens at runtime only, and the rebind gap always (DO29).
- **Correction (found by §3.0.61's gates).** The wrapper's prepare must run only when a home was transported. The eager tier enters typed-`var` wrappers too, with no cells set; an unconditional `cow_prepare_write` there detached a shared container into a replacement nobody could publish, and deltablue2's `vec_remove_first(todo)` shrank a private copy forever (`LAMBDA_TIER=jit` hung from t46 to t47 -- the jit A/B rows did not include deltablue2 and the golden check ran auto only). The prepare is now guarded by the home register, and the benchmark golden check runs on BOTH tiers.
- **Fixtures.** `test/lambda/proc/interp_typed_var_param.ls` (three tiers byte-identical; auto promotes `main`, `store_f`, `store_i`, `field`, `nested`, `scale`, `fromalias`, `shared_then_write`, `rec`) and `interp_typed_var_rebind.ls` (T0 golden; auto matches; eager differs -- DO29).
- **Measured (release, auto e2e wall ms, median of 3; `temp/lambda_t45_final.exe` → `t46_final`, jit reference):** navier_stokes 11.1 s → 281 ms (jit 264; 40x → 1.06x), awfy/nbody2 1230 → 83 ms (jit 84; parity), beng/nbody2 1207 → 87 ms (jit 83), json2 194 → 91 ms (jit 80; 2.4x → 1.13x); jetstream/nbody2 flat at 104 ms (it was never pinned), richards 702 → 684 ms and deltablue 371 → 376 ms unchanged (their residue is the map field protocol, not a scan pin). Eager JIT A/B `t45 → t46` (×3) on the `var`-heavy rows richards, deltablue, havlak, json2, nbody2, navier_stokes, quicksort, permute, towers, hashmap2, cd2, splay: 0.98–1.03, flat.
- **Gates:** auto / jit / interp sweeps (763 scripts) 0 regressions -- the jit sweep counts the new `interp_typed_var_rebind` fixture as its 5th known failure (DO29, T0 golden); `make test-lambda-baseline` 4137 / 4137; `make interp-sweep` regenerated; auto-tier golden check over all 113 benchmark rows 109 PASS with the four pre-existing T0 divergences (cd2_orig, fasta, nbody, spectralnorm).

### 3.0.59 First-entry promotion of loop-bodied procedures and aggregate parameter admission — 2026-09-07 (rev 71, D8.1.1v7)

- **Loop-bodied first entry.** `interp_promote_function` scans a definition's body once (`interp_body_has_loop`, `FnPromotionCell::loop_bodied`) and bypasses the entry threshold on the first entry when the body owns a loop; a once-called `main` promotes before its loop runs. Release auto e2e: mandelbrot 4.04 s → 75 ms (jit 77), matmul 2.26 s → 40 ms, brainfuck2 6.6 s → 346 ms (all at JIT parity).
- **Aggregate value parameters.** `interp_satellite_supported` no longer refuses `array`/`array[num]`/`map`/`element`/`vmap`/structured VALUE parameters (typed `var` parameters kept the CW33 refusal until §3.0.60). Release auto e2e: pnpoly2 1.6 s → 75 ms, gcbench2 1.65 s → 238 ms, deriv2 453 → 40 ms, array1 187 → 20 ms, base642 512 → 40 ms (JIT parity except pnpoly2 1.9x, ray2 2.0x).
- **DO28 found on the way.** base642's `b64_encode(bytes: int[])` read the module `let TABLE: string[]` as null once promoted: the index emitter assumed the eager init's Array-of-Items carrier for an annotated module `let`, but T0's declaration boundary rebuilds a pointer-lane contract into its native carrier. Under a satellite / T0-owned module the read now takes `fn_index`. Fixture: the auto-tier golden check (base642 PASS) and `temp`-free probes recorded in the Result37 analysis.
- **Reject reason logging.** `interp_satellite_supported` logs the scan's refusing node kind (`satellite scan refused function=… node=…`).
- **Still pinned after this:** navier_stokes (`vel_step`/`set_bnd`/`lin_solve`), nbody2 (`advance`/`offset_momentum`), json2 (`p_read_value`/`p_skip_ws`), gcbench2's typed `Node` recursion -- each by the scan, per the logged node kinds; and satellite call-site specialization (diviter's boxed arithmetic) for which the whole-script route is the measured stopgap.

### 3.0.58 Satellite write-back: indexed stores and untyped `var` parameters — 2026-09-06 (rev 70, D8.1.1v6)

- **Indexed/member stores promote.** `interp_scan_satellite_node` no longer refuses `AST_NODE_INDEX_ASSIGN_STAM`/`AST_NODE_MEMBER_ASSIGN_STAM`: a store through a local or value parameter replaces a MIR register the satellite roots itself, a store through a module binding goes to the shared slab, and a store through an untyped `var` parameter is published by the CW33 epilogue. Nothing new was emitted for this row; the pin had been a leftover of the pre-CW33 "no replacement channel" reasoning.
- **Untyped `var` parameters promote through the CW33 home transport on both tier edges** (`interp_var_parameters_transportable`: a `var` parameter with a declared `full_type` has no home in the raw-lane ABI and stays pinned). *T0 → satellite:* `eval_call`'s `var` branch now counts the entry and, once the callee is promoted, publishes each borrowed frame slot's address in `Context::mir_var_homes`, prepares chain-root borrows as the interpreted prologue would, and calls through the new `LAMBDA_DYNAMIC_CALL_BORROWED` mode (`fn_call_borrowed_into`, which lifts the E229 "dynamic dispatch of a function with `var` parameters is deferred" refusal only for that mode); the satellite's existing epilogue stores the final parameter value back through the cell. *Satellite → callee:* the dynamic-call emitter (`mir_dynamic_call_var_signature`) pre-detaches shared roots before argument evaluation, publishes slot addresses after it (`mir_emit_var_home_transport`, shared with the direct edge), calls through the borrowed mode and reloads the bindings (`mir_emit_var_home_reload`); the interpreted side consumes the cells in the new `interp_call_borrowed` entry (read, then clear — the plain `interp_call` must never read them, the cells are only clean around a borrowed-mode call) and treats each as a chain-root borrow, so `interp_call_internal`'s capture/write-back loops run for a caller-less borrowed call. *Declaration rule:* a `var` local passed to an untyped `var` parameter is bound boxed at declaration (`mir_binding_borrowed_as_var_argument`, folded into the T21 widen-at-declaration scan) — this also fixed a pre-existing eager-JIT divergence where `pn bump(var n) { n = n + 1 }` on an int-lane local left the caller's `n` unchanged (no root slot to transport).
- **Differential.** `test/lambda/proc/interp_var_writeback.ls` (array/map/scalar `var` parameters across T0, promoted and eager code; golden identical on `interp`/`auto`/`jit`).
- **Partition.** `make interp-sweep`: 754 scripts, **753 interpreted matches, 1 counted fallback**, 0 mismatches; the two new fixtures joined `test/lambda/interp_p0_subset.txt`. `make test-lambda-baseline` 4120/4120 under the unset AUTO default.
- **Merge regression fixed on the way (not a satellite defect).** The 2026-09-05 upstream "dynamic attr `[expr]:val`" change made `elmt_literal_begin` resolve a computed-key/spread element literal's `type_index` against `context->type_list` — the *running script's* list — instead of the defining module's, so every module-defined `<tag *:spread, …>` literal took a foreign type (50 `graph/*` scripts segfaulted in `fn_map_set` on all three tiers once the merge landed). `elmt()` is now a lookup over the new `elmt_with_type(TypeElmt*)`, which `elmt_literal_begin` calls directly. Fixture `test/lambda/elmt_literal_module.ls` (+ `mod_elmt_literal.ls`). The same merge had also broken `make release` (`lambda-error.cpp` write-only `total_frames_found` once `log_info` compiles away; the counter duplicated `depth` and was removed).
