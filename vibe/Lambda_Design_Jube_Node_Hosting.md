...
| JN3 | Manifest schema gains a non-language module kind: `"kind": "runtime-library"`, `"engine": "js"`, and a `"provides"` specifier list. ABI fields and `entry_symbol` remain supported; build-local `host_build_id` and `sha256_*` fields are deliberately optional and are not generated by normal builds. Builtin-ness must be decidable from manifests alone — **no dlopen at compile time**. |
...
| JN7 | The host API grows **one additive `JubeHostNodeAPI` parent** composing versioned service tables — async request/resource operations, binary (Buffer/typed arrays), promise, Node-error, and dependency providers such as filesystem/zlib — the Node analog of Python's compiler API (`JubeGuestExecutionAPI`), with v1 extracted empirically from what `fs`/`net`/`child_process` actually use (§9). JS value mechanics stay on the existing script/value/data tables (the `js_native_api.h` vs `node_api.h` split, §3.1). |
...
| JN8a | **The host is the sole owner of built-in dependency libraries.** Lambda/Radiant's filesystem, zlib, HTTP, TLS, and crypto dependencies remain statically linked into the host. A Node DSO never links, embeds, or imports `libz`, libuv, curl, mbedTLS, OpenSSL, or another host dependency. A module owns only Node-visible policy, validation, namespace construction, and callbacks; it consumes the host capability through a versioned `JubeHostNodeAPI` provider table. This rule is independent of whether a particular provider is optional at runtime. |
| JN8b | **A provider is not a namespace proxy.** A transitional module may use a host-owned primitive provider while its extraction is in flight, but a completed leaf must not call `resolve_host_namespace()` for any specifier it claims. The host exports transport/process/codec primitives and opaque resource ids only; the leaf owns Node option normalization, JS object state, events, promise/callback settlement, and Node-shaped errors. This prevents an image-only wrapper from falsely satisfying the dynamic-packaging gate while the legacy Node implementation remains resident in the host. |
...
### P9. Static dependencies stay with the host

Packaging changes whether a Node namespace is discoverable; it never moves
Lambda/Radiant's dependency ownership to a Node image. The host statically
links its filesystem, zlib, HTTP, TLS, and crypto implementation libraries
once, then exposes narrowly scoped, versioned providers through
`JubeHostNodeAPI`. A Node module may add Node-specific shaping and policy, but
must not link or embed a second copy. This prevents ABI drift, duplicated
global state, and accidental variation between the otherwise byte-identical
host bundles.
...
falls on the Node-visible heavy edges. It does not transfer ownership of the
underlying libraries out of the host:
...
| `node-zlib` | zlib namespace, validation, callbacks, stream facades | 1.4k | host zlib provider | node-core |
| `node-fs` | fs, fs/promises, FileHandle | 3.9k | host filesystem/async provider | node-core |
| `node-net` | net, dns (+cares_wrap shim) | 7.9k | host stream/DNS provider | node-core |
| `node-child-process` | child_process (+cluster-online glue) | 3.8k | host process provider | node-core, node-net (IPC) |
| `node-http` | http, https | 6.9k | host stream/TLS provider | node-core, node-net |
| `node-tls` | tls | 3.1k | host TLS/crypto provider | node-core, node-net |
| `node-crypto` | node:crypto after the WebCrypto split (JN14) | ≈6–7k | host crypto provider | node-core |
