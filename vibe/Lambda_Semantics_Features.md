...
- **R1 — `open()` is the resource acquisition, `pn`-only.** `open(src)` returns a *scoped resource*; the eager/lazy/value/resource split is completed by `input()` (eager value) and `stream()` (lazy; D9/D10 in `Lambda_Design_Data_Processing.md`). Precedent: C#'s `using var` declaration + Rust's drop-at-scope-exit, minus the keyword.
