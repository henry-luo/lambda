...
- **Reach consts through a runtime call**, e.g. `lambda_unit_const_at(unit_id,
  index)`. No new module plumbing, and still a clear win over the status quo —
  the call returns an existing pointer instead of allocating a GC String — but
  a call per const load rather than a load. This is what shipped; the key is
  the compilation *unit*, not the active module state, because a slab is shared
  by more than one unit while the index space is per unit (**RC-J7v2**).
...
| **RC-J7v2** | **The compilation unit owns a JS unit's const pool -- not the transpiler, and not the module state.** Lambda's pool is safe because `Script` is registered with the `Runtime` and outlives every tier; a JS unit's `JsTranspiler` is ephemeral -- `js_mir_compile_unit` destroys it while its compiled code stays live in **preamble mode** (harness function objects hold code pages) and in **hot-reload batch mode** (`jm_defer_mir_cleanup`). So JS const bodies are `mem_calloc`'d rather than pool-allocated. RC-J7 v1 handed them to `LambdaModuleState`, and that aliased: a literal index is assigned **per unit**, starting at zero, while a module slab is shared by more than one unit -- a direct `eval` compiles its own unit into its caller's slab, so whichever unit linked last owned every index (measured on `test/js/eval_basic.js`: `t2:hello` returned the script's filename; 9 `test_js_gtest` cases failed, `eval`, `new Function`, and the handlebars/highlight.js corpora among them). The pool is therefore keyed by **compilation unit**: `lambda_const_pool_acquire` hands a unit an empty pool and an integer `unit_id` at its first literal, `jm_intern_string_const` interns into that pool, and generated code loads `lambda_unit_const_at(unit_id, index)` -- bounds-checked, so a unit that failed to register faults visibly instead of casting an adjacent word to `String*`. The id keeps the emission relocatable and unpatched (**DI14**). The registry lives on `EvalContext` beside `module_states` and is released in `lambda_module_state_destroy`: **not** at slab release, because a retired slab can leave live code behind, and every unit's code dies with its context. `LambdaModuleState` is unchanged from before RC-J2, and Lambda still borrows its Script-owned pool through `bind_static`. |
