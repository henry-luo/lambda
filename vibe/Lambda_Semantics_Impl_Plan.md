...
| 2.7 | **`last` index keyword (C15)**: fully reserved word (identifier sweep first); within a subscript denotes `len(container) − 1` of the innermost enclosing subscript container (innermost-wins, like `~`); arithmetic and slices (`a[last - 1]`, `a[5 to last]`); all indexable sequences incl. strings; syntax error outside subscripts; NO `last()` fn / `.last` property / `first` keyword. Negative indices stay null (C5) — add lint: literal negative index → "use a[last]". **C15a additions**: `limit last N` for-clause modifier (desc-style clause syntax; tail keeps original order; count mandatory); the two-homes rule enforced (`last` legal ONLY in subscripts and limit clauses — never a value/range); `take`/`drop` reject negative counts; **ArrayNum audit: NumPy-style negative indexing, if present, is nonconforming — correct incl. views; `last` on N-D = leading axis** (`img[last]` = last row). | C15 | `grammar.js` (reserved word + subscript resolution), `build_ast.cpp`, transpilers; lint | `[1,2,3][last]` → 3; `[1,2,3][last - 1]` → 2; `[1,2,3][1 to last]` → `[2,3]`; `[][last]` → null; `a[b[last]]` inner binds to `b`; `let last = 1` → compile error; `for (x in 1 to 100 limit last 10) x` → 91..100; `take(v, -1)` → error; ArrayNum negative-index conformance test |
