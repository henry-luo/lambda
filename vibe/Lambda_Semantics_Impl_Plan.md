...
| 0.8 | **Shadow-proof `name(item)` accessor** (designer naming, 2026-07-06; was `tag(elem)`) — `.name` is shadowed by a `name:` attribute; models can't read tags reliably. Add `name(item)` system function: the shadow-proof *function form of the existing documented `.name` property* — one word, one concept — generalized across named values: `name(elem)` → tag symbol; `name(fn)` → declaration name (null for anonymous); `name(type)` → type name; `name(obj)` → its type name; unnamed values (numbers, strings, maps, …) → null (C5 absence). `attrs(elem)`/`children(elem)` **deferred** — covered by `for (k at elem)` / `for (x in elem)` iteration. | C9a probing                         | sys-func registry (`lambda-eval.cpp` / sys_func defs)                 | `name(<x name:'v'>)` → `'x'`; `name(42)` → null                                 |
