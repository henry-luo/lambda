...
**Status (2026-07-07)**: Implemented 4.1, 4.2, 4.6, and 4.7. Compact `int` now enforces the symmetric ±(2^53 − 1) semantic bound at packing and arithmetic boundaries; compact `int` `+`, `-`, and `*` promote to boxed `float` on overflow, and MIR routes compact-int arithmetic through the boxed helpers so `type(math.max_int + 1)` reports `float` instead of a widened tagged `int`. `math.max_int` is exposed as a math-module constant, documented in `Lambda_Sys_Func.md`, and covered in the existing math-constants fixture. Added focused fixtures `int53_boundary.ls` and `int_promotion.ls`. Verification: `make build`; direct runs for the new fixtures and `sys_func_math_constants.ls`; `./test/test_lambda_gtest.exe --gtest_filter='AutoDiscovered/LambdaScriptTest.ExecuteAndCompare/*int53_boundary*:AutoDiscovered/LambdaScriptTest.ExecuteAndCompare/*int_promotion*:AutoDiscovered/LambdaScriptTest.ExecuteAndCompare/*sys_func_math_constants*'` passed 3/3; nearby numeric cluster (`numeric_expr`, `mixed_numeric_ops`, `large_int_map`, `int64`, decimal, and sized numeric fixtures) passed 15/15. 4.3, 4.4, 4.5, 4.8, and 4.9 remain open.
