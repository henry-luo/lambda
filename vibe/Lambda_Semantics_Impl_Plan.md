...
- Verification: `make build-test` passed; focused Phase 0 tests passed; the
  escalated `make test-lambda-baseline` run passed input baseline plus
  `test_lambda_gtest` 424/424 and `test_lambda_repl_gtest` 39/39, but the full
  gate is still blocked by pre-existing `NodePrelimTests/NodeFileTest.Run/crypto_dh_constructor`
  timing out with exit 124.
  After 0.8, `make build` passed and
  `./test/test_lambda_gtest.exe --gtest_filter='*name_member*'` passed.
...
- Verification: `make build` passed; direct stdout diffs for
  `eq_phase1_remaining` and `eq_poison` passed; focused
  `./test/test_lambda_gtest.exe --gtest_filter='*eq_total*:*eq_fn*:*eq_poison*:*eq_decimal_float*:*eq_phase1_remaining*'`
  passed 5/5. Direct `./lambda.exe --no-log test/lambda/eq_poison.ls` still
  emits only the golden stdout; the stale memtrack leak diagnostic is gone.
