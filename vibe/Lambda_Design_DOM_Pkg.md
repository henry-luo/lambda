...
| Default actions / activation behavior (link follow, checkbox/radio toggle, submit-on-Enter, button/space activation, `details` toggle, label forwarding) | **L** | policy (user decision 2). **Per-behavior status: `Lambda_Design_DOM_Default.md` §3.9** — checkbox/radio/label/`select`, local submit/reset, link policy, and `details` activation have landed as templates; POST transport and popover remain open |
| Sequential focus navigation (Tab order, focus delegation, `autofocus` processing) | **L** | policy, per-keypress cold. `focus.ls` orders the native candidate snapshot and selects autofocus; native commits canonical state, emits the focus transition, resolves scroll geometry, and paints (`Lambda_Design_DOM_Default.md` §5.3/§5.5, ESO52/ESO60) |
