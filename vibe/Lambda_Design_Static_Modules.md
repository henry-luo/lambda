...
This is not a mechanical `git mv` until the active core/rt behavior split is complete. P1a changes active interfaces; P1b performs the mechanical moves after those boundaries compile. Moving builder/editor to io is itself mechanical apart from include-path/build-list updates because their current behavior already matches the io charter. **(Rev 28–29: those boundaries now compile, so the remaining top-level core/rt sources and headers listed above are relocated. Rev 28 used transitional top-level forwarding shims; rev 29 then migrated every active call site to a direct `core/`/`io/`/`runtime/` include and deleted all shims (the rev-28 set plus 7 pre-existing ones), leaving `lambda/` shim-free. See the rev-28 and rev-29 checkpoints.)**
...
| **P4** | **IMPLEMENTED EXCEPT CLASS F.** Active MIR-Direct `pn_emit` calls the lower-owned `runtime/radiant_event_hook`; the former `pn_set_selection` callback is retired in favor of revisioned `EditResult`/`dom.finish_model_edit` operations on the declared `radiant-dom` waist (D7.5.3). Counters, symbols, resource attachment/release, layout updates, and memory-stage hooks follow the same lower-owned-hook rule. The active boundary report has zero upward extern declarations. The only remaining rt→Radiant interface is the user-deferred Class-F JS DOM/CSSOM/editing/clipboard/history binding set, ratcheted by the rt boundary image. The frozen C2MIR compatibility surface remains out of scope under SM14. | active boundary baseline; `make check-module-boundary`; no C2MIR diff |
...
**Rev 28 physical-relocation checkpoint.** The core/rt translation units and headers that were still physically at `lambda/*.{c,cpp,h,hpp}` — referenced there by path while only the newer split files had been placed in subdirectories — are now moved to their owning directories, completing the P1b mechanical relocation for the value/engine originals. `lambda/core/` gains the pure value sources `binary`, `lambda-data`, `lambda-decimal`, `name_pool`, `path`, `print`, `shape_builder`, `shape_pool`, `utf_string` plus their `binary.h`/`utf_string.h`/`name_pool.hpp`/`shape_pool.hpp`/`shape_builder.hpp`/`lambda-decimal.hpp`/`lambda-path.h` headers; `lambda/runtime/` gains the active engine sources (`build_ast`, `parse`, `lambda-eval*`, `lambda-vector`, `lambda-data-runtime`, `lambda-error`, `mir`, `mir_dump`, `transpile-mir`, `transpile_shared`, `runner`, `module_registry`, `render_map`, `safety_analyzer`, `re2_wrapper`, `vmap`, `pack`, `sysinfo`, `concurrency`/`concurrency_js`, `edit_bridge`, `http_module`, `template_registry`/`template_state`, `sys_func_registry`) and their headers (`ast.hpp`/`ast-core.hpp`, `transpiler.hpp`, `lambda-error.h`, `lambda-number*.hpp`, `value_rep.h`, `mir_*`, `render_map.h`, `ts-enum.h`, …). Each relocated header leaves a top-level forwarding shim (`#include "core/…"`/`"runtime/…"`) matching the `mark_reader.hpp`/`lambda-stack.h` precedent, so the ~300 external include sites — and the frozen C2MIR translation units, which stay unedited under SM14 — resolve unchanged; only the moved files' own current-dir-relative includes were rewritten, and cross-layer references were routed through the top-level shims so the tier-1 include matrix records no new edges. The mixed aggregate headers (`lambda.h`, `lambda.hpp`, `lambda-data.hpp`), the shell (`main*.cpp`), the wasm entry, and the frozen C2MIR sources stay at top level by charter. Source paths were updated in `lambda-data`, `lambda-boundary-core`, `lambda-rt`, `lambda-runtime-full`, and `exclude_source_files`, plus the stale `cli` build variant's `source_dirs` (which predated the split and had never gained `lambda/core`/`io`/`runtime`). This is a pure relocation: no behavior, ownership, boundary baseline, or the still-pending rev-26 dead-body deletion changed. `make build`, `make build-lambda-static`, `make check-module-boundary` (lib/core/io/radiant fatal-undefined-clean; rt Class-F unchanged at 165 = baseline), and `make test-lambda-baseline` (3,594 tests across 18 suites, zero failures) all pass; the frozen C2MIR diff remains empty. One build-system note: premake flattens object names into a shared `OBJDIR`, so a first incremental build after the move failed on stale `.d` prerequisites still naming the old top-level paths — clearing the moved basenames' `.o`/`.d` (or a clean build) resolves it.

**Rev 29 direct-include checkpoint.** The transitional top-level forwarding shims created by rev 28 are removed in favor of direct includes, per the design's own end-state rule ("one implementation and one authoritative declaration … forwarding headers removed once call sites migrate", §9.4). Every active includer of a relocated header — ~171 sites across ~120 files, including the moved TUs themselves and the surviving mixed aggregate headers (`lambda.h`/`lambda.hpp`/`lambda-data.hpp`) — now names `core/<h>` or `runtime/<h>` (or the correct relative form) directly; cross-layer core→rt uses that were previously masked behind a top-level shim path (e.g. `lambda/core/lambda-data.cpp` → `runtime/ast.hpp`, `lambda/core/path.c` → `runtime/sysinfo.h`) are now explicit, which the report-only tier-1 inventory reflects without blocking. All 28 rev-28 shims are deleted, and so are the 7 transitional forwarding shims left by earlier P1b work (`mark_reader.hpp` → core; `mark_builder.hpp`/`mark_editor.hpp` → io; `lambda-stack.h`, `emit_sexpr.h`, `mem_factory_rt.h`, `lambda-number-runtime.hpp` → runtime) — `lambda/` is now shim-free. Four headers (`transpiler.hpp`, `safety_analyzer.hpp`, `re2_wrapper.hpp`, `lambda-number-types.hpp`) are included by the frozen C2MIR TUs `transpile.cpp`/`transpile-call.cpp` via same-dir paths; their shims are deleted too. Those two frozen TUs — together with `http_module_stub.cpp` (the stub alternative to the real `http_module.cpp`) — are then relocated out of `lambda/` top into `lambda/runtime/`: `git mv` only for the two C2MIR files, so their bytes are unchanged and rule 14 holds, while the stub's own relative includes are fixed because the monolithic build compiles it. Because `lambda-rt` globs `lambda/runtime/*.cpp`, all three are added to a new `lambda-rt` `exclude_patterns` (inherited by `lambda-boundary-rt` via `validation_source_target`) so the uncompilable C2MIR files and the conflicting stub stay out of that archive; the monolithic build keeps compiling the stub and excludes the real `http_module.cpp` and the two C2MIR TUs through `exclude_source_files`. Now that the frozen TUs live in `runtime/`, their `transpiler.hpp`/`safety_analyzer.hpp`/`re2_wrapper.hpp`/`lambda-number-types.hpp` includes resolve same-dir to the real headers; only their `../lib/*` includes are relatively stale, and that is inert because they are never compiled. Every *active* includer of all four headers resolves to the real `runtime/` header directly. Finally `headless_stubs.cpp` and `windows_compat.h` leave the top level too: `headless_stubs.cpp` (the frozen `LAMBDA_HEADLESS` radiant-stub enclave, whole body under `#ifdef LAMBDA_HEADLESS` which only the `cli` variant defines) is `git mv`'d to `lambda/runtime/` and added to the `lambda-rt` `exclude_patterns` — it compiles to an empty TU in the non-headless builds and references radiant, so it stays out of the rt archive; and `windows_compat.h` moves to `lib/` per §8.2 with its lone `format-xml.cpp` includer repointed. After this, `lambda/` top holds only the shell (`main*.cpp`), the wasm entry (`lambda-wasm*`), the mixed aggregate headers (`lambda.h`/`lambda.hpp`/`lambda-data.hpp`), and the generated frozen `lambda-embed.h`. (Caveat: the `cli`/`LAMBDA_HEADLESS` build would need `headless_stubs.cpp`'s relative radiant/lambda includes updated for the deeper path, which rule 14 forbids editing; that variant is separately stale and SM9 is retiring the file, so it is not an active-build concern.) The relocation had also staled several hardcoded tool paths, now fixed: `check_gc_effects.py` and `check_hosted_python_architecture.py` (which read `sys_func_registry.c`/`ast-core.hpp`), the `ts-enum.h` generator (`update_ts_enum.sh` + `Makefile TS_ENUM_H`, which must emit into `lambda/runtime/`), `js262_unused_registry_entries.sh`, and four `utils/lint/rules/c-cpp/*.yml` allow-lists. `make build`, `make check-module-boundary` (four strict DSOs clean; rt Class-F unchanged at 165 = baseline), and `make test-lambda-baseline` (2,104 input + 3,594 gtest, zero failures) pass; `make lint`'s `static-module-architecture` rule is green, and its two remaining failures (`gc-effects` NO_GC audit, `no-new-per-file-header` Radiant DD4) were confirmed pre-existing on clean `HEAD` and are unrelated to this change.
