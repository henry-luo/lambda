...
| Check | Criterion |
|---|---|
| G1.1 full differential | `make test-lambda-interp` runs the committed 687-row T0 subset with zero fallback and exact JIT/golden output; the three-way 687-row partition has zero excluded and zero inconclusive rows (SI3/D3.3.1/D8.1.1v2) |
...
| G1.3 non-regression | Direct premake build, 689-row subset/fallback gate, empty-exclusion accounting probe, and 35-test walker/frame-plan suite pass; default-tier behavior remains unchanged by the interpreter-only path |
