...
**Date:** 2026-08-20 (rev 45 — **nominal-object alignment and generic-`u64` fail-closed boundary**)
**Status:** **P0 complete and gated (2026-08-15)**; **P1 remains partial** (the global partition still awaits a clean full-corpus sweep). T0 covers the bounded P1 scalar/type, native-array, compact-`NumSized`, full-width `u64`, nullable/pointer-lane, numeric-mask, and fixed-coordinate N-D `ArrayNum` boundaries through the same conversion and checked-store helpers as MIR. A mixed generic container whose read may yield `u64` remains explicitly pinned to MIR until generic storage has a lane-preserving read carrier. It traverses complete Element content and uses the matching COW setter for direct Element writes. A direct local `m.set(key, value)` uses `vmap_set_cow` and republishes its replacement; a direct root with an open `any` / `any | error` contract, including input-derived Map or Element values, routes through the runtime-layout COW bridge. Declared structural maps now recursively convert a dynamic binding and validate a nested COW replacement before publish. A rejected deferred declaration never publishes its error binding and skips its declaring block; a rejected direct parameter returns through the caller before the callee body or an enclosing fallback expression can observe it. Nominal object construction, including inherited defaults, and a direct local `object.method()` whose type definition is imported use the declaring Script and a traced receiver closure; mutating `pn` methods publish their COW receiver replacement before entry. P1 comprehensions now include ordered streams, one-source `group by … into`, and chained inner/left equi-joins with multi-key, index/key, and cross-product stages. Pure system calls now admit named source labels, including a pipe-injected first argument; dynamic calls and procedural system calls remain pinned because they lack a safe formal or COW write-back ABI. Typed array contracts, dynamic/import VMap receivers, dynamic/captured object receivers, arbitrary non-mask computed index shapes, N-D row writes, joined grouping, and procedural stream windows remain pinned. The serialized `make test-lambda-interp` gate completed **414/415** in 27m50s before this unpartitioned follow-up: all 24 walker tests, all three frame-plan tests, and all 385 differential rows were exact. Its sole failure was the stale exclusion audit for `arraynum_transform_preserve.ls`, `compact_typed_arrays.ls`, `eq_total.ls`, `for_clauses_test.ls`, `for_group_test.ls`, `for_join_s3b_test.ls`, `for_join_test.ls`, `fuzzy_crash_regression.ls`, and `graph_layout.ls`. `LAMBDA_TIER=auto` runs T0 and promotes eligible definitions to Script-owned MIR satellites at the shared dynamic-call boundary; captures, cross-language/type imports, and named-property/key-table users remain pinned to T0. **P3 is in progress:** non-object constrained `is` checks and constrained match arms use pure, fuel-bounded `PREDICATE`, while the JIT folds admitted pure literal scalar subtrees at safe generic-value boundaries. **P4 has a persistent-REPL vertical slice:** `interp` and `auto` retain one Script, execute only completed appended fragments, geometrically grow module/root storage, and roll failed fragments back. Validator de-JIT and P5's default-tier flip remain unimplemented.
...
|---|---|---|
| P1.4 procedural + errors | **partial** | `EvalSignal` plumbing on `InterpFrame` (payload in the reserved slot); `VAR_STAM`, `ASSIGN_STAM` to a named binding, `WHILE_STAM`/`DO_WHILE_STAM`, `BREAK`/`CONTINUE`, explicit `RETURN`; `RAISE_STAM`/`RAISE_EXPR`; `AstCallNode::propagate` (`f(...)^`); root and nested `INDEX_ASSIGN_STAM`/`MEMBER_ASSIGN_STAM` through COW helpers, including full-root validation for nested declared-map replacements, direct supported typed-array replacement validation, and fixed-arity direct `var`-parameter write-back for a mutable binding; direct `push`/`splice` replacement publication; expression/statement handlers with `CURRENT_ERROR`; local native-fault recovery; self-tail-call iteration; variadic Lambda `varg()` scope; compact scalar declaration witnesses, recursive declared-map binding admission, canonical declared sized-numeric/`u64` binding stores, and direct sized-numeric/`u64` coercion calls; inferred compact `NumSized` array literals; and run-mode `pn main()`. **Not yet:** deferred declaration-boundary skip (S7.7.2), `PIPE_FILE_STAM`, N-D row writes, full-width `u64[]` literal/index lanes, nullable/pointer typed-array lanes, and the remaining procedural AST families. |
| P1.1 comprehensions | **partial** | `FOR_EXPR`/`FOR_STAM` core: nested `AstLoopNode` chains, key/value and index binding, `key_filter`, `key_only`, the single-binding `let` clause, `where`, the spreadable output stream, `PIPE`/`where`, and slot-backed `~`/`~#` contexts. `LAST_INDEX` is admitted for an ordinary subscript and uses the innermost rooted owner (S10.1.3); expression-form unordered `limit`, `limit last`, and `offset` are full-stream post-selections. Ordered outputs use the shared stable key sort before their window. One-source `group by … into` materializes the group Element before its aggregate body. Chained inner/left equi-joins materialize rooted tuples through the shared hash/cross helpers, including multi-key joins and index/key bindings (S14.1). **Not yet:** joined grouping and window clauses on a procedural `FOR_STAM`. |
...
| P1.2 types | **partial** | Type expressions as values (`TYPE`, `BINARY_TYPE`, `UNARY_TYPE`, `CONTENT_TYPE`, `LIST_TYPE`, `ARRAY_TYPE`, `MAP_TYPE`, `ELMT_TYPE`, `FUNC_TYPE`), `TYPE_STAM` declarations, type names used as values, recursive declared structural-map conversion plus nested checked updates, nominal `OBJECT_TYPE`/object-literal construction, and direct local receiver method calls with a traced snapshot receiver closure. Identity selection is shared with lowering through `lambda_type_node_singleton` (ast.hpp). **Not yet:** `MATCH_EXPR`/`MATCH_ARM`, string/symbol patterns, `CONSTRAINED_TYPE`, dynamic/captured/import object receivers, named/`var`-parameter method calls, and generic object dispatch. |
...
- **Evidence.** The current procedural directory sweep reports **145 scripts: 140 matches, 5 counted fallbacks, 0 mismatches, 0 timeouts**; the PDF directory reports **81/81 matches, 0 fallbacks, 0 mismatches, 0 timeouts**. A subsequent 685-script all-corpus run reached 350 rows but was inconclusive: the runner hit a sandbox `PermissionError` while reaping `test/lambda/editor/editor_api_basic.ls`; no partition manifest was changed. `make test-lambda-baseline` rebuilt successfully and passed **3861/3862** tests; the sole unrelated failure is `test_js_gtest`'s `tune4_global_callable_binding`, which cannot load the absent `modules/node-crypto/node-crypto.dylib`. P1 therefore remains partial and the clean full-corpus gate is still outstanding.
...
- **Evidence.** `test_interp_gtest.exe` focused promotion/fallback coverage passes (**23 tests** after the manifest refresh); the stale `object_constraint.ls` golden is retained as an inconclusive `golden_drift` row. The direct functional root slice remains **299 scripts: 277 matches, 20 counted fallbacks, 0 mismatches, 2 bounded renderer timeouts**; the clean 685-script gate is still outstanding. These changes do not claim P1 completion: joined procedural windows, remaining typed-array lanes, the full corpus, and the baseline's unrelated missing `node-crypto` dylib remain open.
...
- [ ] `FOR_EXPR`/`FOR_STAM` with the full clause set (`AstForNode`: `where/group/order/limit/limit_from_end/offset/then`; `AstLoopNode` joins incl. `join_keys`/`key_filter`/`key_only`/`optional`), `LOOP`, `ORDER_SPEC`, `GROUP_CLAUSE`, `JOIN_KEY`; accumulators/group tables through the same container/sort helpers as lowering (S6 total order lives in the helpers). The landed subset covers ordered expression streams, one-source groups, and tuple-stream inner/left joins; joined grouping and procedural windows remain open.
