...
**Date:** 2026-08-22 (rev 61 — **edit-template bridge and retained-event state guard**)
**Status:** **P0 and the bounded P1 construct set are complete and gated.** The AST walker now covers the P1 expression, comprehension, type/pattern, document/path/query, procedural/error, module, numeric-carrier, COW-mutation, joined-group, pipe-file, procedural-window, and view/edit-template slices through the same runtime helpers and representation boundaries as MIR. The committed differential corpus is explicit: **687 discovered scripts = 648 interpreted matches, 37 counted fallbacks, and 2 inconclusive rows; 0 unclassified and 0 divergent rows**. The 37 fallbacks are deliberate P1.6 policy boundaries (async/task/Jube/renderer-only or unsupported native-system families), not silent coverage gaps; the 2 inconclusive rows are named timeouts. The focused edit-template accounting and differential probes pass; the full gate is being refreshed for the new 648-row subset. P2 satellite promotion, P3 validator ownership, P4's retained REPL slice, and P5's default-tier flip remain subsequent phases.
...
- [x] Scripts with `START`, generator/async definitions (`FnAnalysis` `may_await`/`is_generator`/`needs_task_context`), mixed-tier/Jube imports, or unsupported native-system/renderer forms fall back to whole-module JIT. View and edit state/handler bodies are registered and evaluated by the interpreter bridge. The 37 affected golden rows are committed in `test/lambda/interp_excluded.txt`; the differential gate counts them and the 2 bounded inconclusive rows separately (no silent caps).

### P1 gate (G1)

| Check | Criterion |
|---|---|
| G1.1 full differential | `make test-lambda-interp` runs the committed 648-row T0 subset with zero fallback and exact JIT/golden output; the 37 P1.6 fallbacks and 2 inconclusive rows are explicit in the three-way 687-row partition (SI3/D3.3.1/D8.1.1v2) |
| G1.2 GC stress | Forced-GC/freed-memory differentials pass for the newly landed view/edit/state/handler, nullable/native, COW, N-D, sized, generic-`u64`, and comprehension frame-plan boundaries; the 648-row gate refresh is pending. A sanitizer-wide run remains a CI-strengthening task, not an unverified completion claim |
| G1.3 non-regression | `make build -j8`, focused interpreter gates, and the exclusion-accounting probe pass; the 648-row gate refresh is pending, and default-tier behavior remains unchanged by the interpreter-only path. The broader Lambda baseline is rerun below and any unrelated external-module failure is recorded explicitly |
| G1.4 **measurement report** | §6's historical performance report remains labeled to its 2026-08-15 corpus; rev61 adds the current 687-row correctness partition and exact focused edit/event evidence rather than silently rewriting old measurements |
