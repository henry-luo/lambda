...
Two slices are in, verified at the same bar as P0 (sweep 0 divergences, GC stress 0 divergences, `test_interp_gtest` 116/116, default tier 718/718). The `test/lambda` corpus moved from **81 → 104** interpreted scripts, **198 → 175** fallbacks.

| Slice | State | What landed |
|---|---|---|
| P1.4 procedural + errors | **partial** | `EvalSignal` plumbing on `InterpFrame` (payload in the reserved slot); `VAR_STAM`, `ASSIGN_STAM` to a named binding, `WHILE_STAM`/`DO_WHILE_STAM`, `BREAK`/`CONTINUE`, explicit `RETURN`; `RAISE_STAM`/`RAISE_EXPR`; `AstCallNode::propagate` (`f(...)^`). **Not yet:** `INDEX_ASSIGN_STAM`/`MEMBER_ASSIGN_STAM` (COW write paths), declaration-boundary skip (S7.7.2), `HANDLER_EXPR`/`HANDLER_STAM`, `CURRENT_ERROR`, self-tail-call iteration |
...
Three defects the differential caught while landing these, all fixed:
