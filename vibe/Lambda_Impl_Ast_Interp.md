...
**Date:** 2026-08-21 (rev 53 — **P1 closure and explicit corpus partition**)
**Status:** **P0 and the bounded P1 construct set are complete and gated.** The AST walker now covers the P1 expression, comprehension, type/pattern, document/path/query, procedural/error, module, numeric-carrier, COW-mutation, joined-group, pipe-file, and procedural-window slices through the same runtime helpers and representation boundaries as MIR. The committed differential corpus is explicit: **687 discovered scripts = 487 interpreted matches, 186 counted fallbacks, and 14 inconclusive rows; 0 unclassified and 0 divergent rows**. The 186 fallbacks are deliberate P1.6 policy boundaries (async/task/view/Jube/renderer-only or unsupported native-system families), not silent coverage gaps; the 14 inconclusive rows are named timeouts or the retained golden-drift ruling. `make test-lambda-interp` now runs **522 tests, all passing**. P2 satellite promotion, P3 validator ownership, P4's retained REPL slice, and P5's default-tier flip remain subsequent phases.
...
### 3.0 P1 progress — **complete 2026-08-21 (rev 53)**

The five P1 slices and the explicit P1.6 policy boundary are implemented. The committed corpus partition is complete and auditable: **687 discovered scripts with checked-in goldens, 487 interpreted matches, 186 counted fallbacks, 14 inconclusive rows, 0 unclassified rows, and 0 differential divergences**. Inconclusive is a first-class verdict under R4/D8.1.1v2: eleven bounded renderer rows, two existing renderer-transform rows, and one retained `object_constraint` golden-drift ruling are named in `interp_inconclusive.txt`; they are not silently promoted or counted as fallback. The full committed gate, `make test-lambda-interp`, runs **522 tests and passes all 522**.

The fallback rows are deliberate policy, not hidden P1 omissions: async/task/`START`, views/state/event handlers, Jube/JS import cones, renderer-only object/update forms, and native-system/PDF families without a safe boxed ABI remain in `interp_excluded.txt` with their first unsupported AST kind. This is the P1.6 contract from **D8.1.1v2** and **D5.3.3**; P2 may widen it through satellites without changing the default tier.
...
| P1.4 procedural/errors | **complete** | `EvalSignal`, declaration-boundary skip, explicit return/raise/propagate, handlers and local-fault recovery, COW writes, typed/native/nullable lanes, generic `u64` carriers, `PIPE_FILE_STAM`, self-tail iteration, `run`-mode `pn main`, and procedural windows are all covered by the 487-row differential gate. |
| P1.5 modules | **complete** | Import-cone recording, post-order transactional initialization, per-module slabs, declaring-Script lookup, and module-state cleanup are exact; mixed-tier and cross-language cones are explicit P1.6 exclusions. |
...
- [x] `FOR_EXPR`/`FOR_STAM` with the full clause set (`AstForNode`: `where/group/order/limit/limit_from_end/offset/then`; `AstLoopNode` joins incl. `join_keys`/`key_filter`/`key_only`/`optional`), `LOOP`, `ORDER_SPEC`, `GROUP_CLAUSE`, `JOIN_KEY`; accumulators/group tables use the same container/sort helpers as lowering (S6 total order lives in the helpers). Ordered streams, one-source groups, tuple-stream inner/left joins, joined grouping, and procedural windows are covered by the 522-test gate.
...
- [x] Retained-module (L1 cache) interaction: interp-mode Scripts carry no `jit_context`; retain/invalidate paths tolerate NULL JIT state, with mixed-tier and cross-language cones remaining explicit P1.6 policy boundaries.

### P1.6 Exclusions wired explicitly

- [x] Scripts with `START`, generator/async definitions (`FnAnalysis` `may_await`/`is_generator`/`needs_task_context`), `VIEW`/`STATE_ENTRY`/`EVENT_HANDLER`, mixed-tier/Jube imports, or unsupported native-system/renderer forms fall back to whole-module JIT. The 186 affected golden rows are committed in `test/lambda/interp_excluded.txt`; the differential gate counts them and the 14 bounded inconclusive rows separately (no silent caps).

### P1 gate (G1)

| Check | Criterion |
|---|---|
| G1.1 full differential | `make test-lambda-interp` runs the committed 487-row T0 subset with zero fallback and exact JIT/golden output; the 186 P1.6 fallbacks and 14 inconclusive rows are explicit in the three-way 687-row partition (SI3/D3.3.1/D8.1.1v2) |
| G1.2 GC stress | Forced-GC/freed-memory differentials pass for the newly landed nullable/native, COW, N-D, sized, and generic-`u64` boundaries; the full 522-test gate is clean. A sanitizer-wide run remains a CI-strengthening task, not an unverified completion claim |
| G1.3 non-regression | `make build -j8`, focused interpreter gates, and the 522-test gate pass; default-tier behavior is unchanged by the interpreter-only path. The broader Lambda baseline is rerun below and any unrelated external-module failure is recorded explicitly |
| G1.4 **measurement report** | §6's historical performance report remains labeled to its 2026-08-15 corpus; rev53 adds the current 687-row correctness partition and exact gate counts rather than silently rewriting old measurements |
