...
**Date:** 2026-08-22 (rev 63 — **async/task satellite and import-cone bridge**)
**Status:** **P0 and the bounded P1 construct set are implemented; focused gates are green and the serialized full gate is pending.** The AST walker now covers the P1 expression, comprehension, type/pattern, document/path/query, procedural/error, module, numeric-carrier, COW-mutation, joined-group, pipe-file, procedural-window, view/edit-template, hosted-JavaScript import, typed-object-update, and async/task-procedure slices through the same runtime helpers and representation boundaries as MIR. The committed differential corpus is explicit: **687 discovered scripts = 682 interpreted matches, 3 counted fallbacks, and 2 inconclusive rows; 0 unclassified and 0 divergent rows**. The three remaining fallbacks are compile-invalid/node:none fixtures; the two inconclusive rows are named renderer timeouts. Task-backed procedure bodies now delegate only their resumable continuation to a P2 MIR satellite while the module and synchronous code remain under T0. Lambda imports and hosted-JavaScript promise calls are admitted when their dependency ABI is sealed; all promoted async/import rows pass normal and forced-GC/poisoned-memory differential probes. P3 validator ownership, P4's retained REPL slice, and P5's default-tier flip remain subsequent phases.
...
- [x] Scripts with unsupported generators, stateful view/event/native-system forms, or compile-invalid source remain outside the interpreted subset: executable unsupported forms fall back to whole-module JIT, while the three source-invalid rows stay `node:none`. Task-backed procedures use a resumable MIR satellite for the suspension body while the owning module and synchronous definitions stay under T0; direct Lambda imports and hosted-JavaScript promise exports use the same sealed module/import ABI. The three remaining rows are committed in `test/lambda/interp_excluded.txt`; the differential gate counts them and the 2 bounded inconclusive rows separately (no silent caps).

### P1 gate (G1)

| Check | Criterion |
|---|---|
| G1.1 full differential | `make test-lambda-interp` runs the committed 682-row T0 subset with zero fallback and exact JIT/golden output; the 3 node:none rows and 2 inconclusive rows are explicit in the three-way 687-row partition (SI3/D3.3.1/D8.1.1v2) |
| G1.2 GC stress | Forced-GC/freed-memory differentials pass for all newly promoted async/task and Lambda/JavaScript import rows plus the prior typed-object-update, view/edit/state/handler, nullable/native, COW, N-D, sized, generic-`u64`, and comprehension frame-plan boundaries; the serialized 682-row gate refresh is pending. A sanitizer-wide run remains a CI-strengthening task, not an unverified completion claim |
| G1.3 non-regression | Direct premake build, focused 35-test interpreter gate, and the exclusion-accounting probe pass; the serialized 682-row gate refresh is pending, and default-tier behavior remains unchanged by the interpreter-only path. The broader Lambda baseline is rerun below and any unrelated external-module failure is recorded explicitly |
| G1.4 **measurement report** | §6's historical performance report remains labeled to its 2026-08-15 corpus; rev63 adds the current 687-row correctness partition and exact async/import evidence rather than silently rewriting old measurements |
