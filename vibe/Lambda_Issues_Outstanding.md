...
- **LR_02 (parse/AST)**: **#1 unknown binary operator silently defaults to `OPERATOR_ADD`**; #4 relational result type must stay in lockstep with vectorized codegen; #11 `~` in match-arm bodies can be missed; #13 `parse.c` ignores parse errors. *(#2 enum-order numeric promotion FIXED by the shared number classifier.)*
...
- **LR_04 (numbers)**: **#3 trapping `mpd_get_ssize` can SIGFPE**; **#4 `decimal_cmp` returns equal on conversion failure** (→ OI-1c); #1 "unlimited" decimal is a 200-digit cap; #5 float↔decimal via `%.17g` string detour. *(#6 magnitude/signedness normalization and #7 sized signed-overflow UB FIXED by the type-directed classifier and Go-style lane kernels.)*
...
- **LR_06 (C2MIR)**: retired from supported builds and tests. Its historical gaps are not runtime acceptance items; useful algorithms may be ported deliberately into MIR Direct without restoring backend parity.
