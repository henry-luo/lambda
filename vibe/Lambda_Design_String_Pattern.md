...
| **SP4 note (2026-09-22)** | Counted occurrence respelled to the regex form — `{n}`, `{n,m}`, `{n,}` — inside islands as everywhere else (S11.1.6, `Lambda_Type_Pattern.md` §1.3): `\(d{3} "-" d{4})`, `HexDigit{3}`. The `[n]`/`[n+]`/`[n, m]` spellings above are superseded and not yet migrated in the parser. |
