...
- **D0 — inventory + realm state.** Enumerate every per-call override-check site (the `arr.push` pattern and siblings across Array/String/Object builtins). Add realm-owned state: cached intrinsic-prototype Items + **monotonic pristine-tamper flags**, set on any mutation path that can touch an intrinsic prototype (`js_property_set`/`defineProperty`/`delete` targeting them). The in-code precedent is already there — `js_note_array_prototype_push_tamper` and the `g_array_sym_iter_ever_set` flag pattern — this generalizes it into one registry instead of ad-hoc globals.
- **D1 — replace the per-call checks**: fast path reads the realm's tamper flag (one load + branch, no interning, no lookup); tampered → today's slow path unchanged. **D1c fixtures**: two-realm isolation (the reverted process-global cache's failure mode — one realm's tamper or prototype must never leak into another), tamper-then-call correctness for every converted builtin (test262's prototype-tampering suites are the backstop), tamper-flag coverage from *all* mutation entry points (including `Reflect` and descriptor paths).
- **D2 — measure**: push-heavy loops, kostya array benchmarks, and the composite; record which builtins were converted.
