...
**Date:** 2026-09-25
**Status:** RATIFIED 2026-09-25 as **D3.4.3v2** (Formal Design 13.0.0). Maps: implemented. Elements: design settled 2026-09-25 (§7); E1–E6 landed the same day (plan phases P0–P2), including the editor's rebuilds; E7, the shape pool's retirement, not yet. Plan: [Impl_Element_Type_Sharing](impl/Lambda_Impl_Element_Type_Sharing.md).
...
Since P2 the editor no longer uses the pool either: a rebuilt type comes from the tree, or is a type the container owns with a chain of its own (§8). The pool's last caller is `elmt_finalize_shape`, for element types that stayed private (runtime-built, or past the element budget); P3 retires it.

## 7. Elements — design settled 2026-09-25; E1–E6 implemented
