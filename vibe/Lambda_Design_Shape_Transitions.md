...
**Date:** 2026-09-25
**Status:** RATIFIED 2026-09-25 as **D3.4.3v2** (Formal Design 13.0.0). Maps: implemented. Elements: design settled 2026-09-25 (§7), not implemented; the editor's map rebuilds and the shape pool's retirement are decided, not done. Plan: [Impl_Element_Type_Sharing](impl/Lambda_Impl_Element_Type_Sharing.md).
...
- **E7 — The shape pool retires** once E2 and E3 land: `shape_pool.cpp`/`.hpp`, the interning half of `ShapeBuilder`, `Input::shape_pool`, its memory-registry node and its tests, and D3.4.2's signature clause with them. Nothing compares shapes by pointer, and the transpiler's `shape_pool` field has no reader.
...
- **Suspected bug: inline map edits rewrite a shared type.** On an inline edit, `container_rebuild_with_new_shape` rewrites a map's registered `TypeMap` in place; it exempts only unregistered types and `EmptyMap`, and never checks `typemap_is_shared_shape`. On a tree or literal type that would change the layout under every map sharing it — the failure the element branch already guards against. Not yet reproduced.
- **Decided 2026-09-25: editor map rebuilds move onto the tree (not implemented).** `container_rebuild_with_new_shape` will take a map's new type from the tree instead of interning a chain in the pool. An added field follows one edge from the current node; a retype or delete replays the new field sequence from the root. Repeated edits to the same shape then hit existing nodes and allocate nothing, which is the cost the in-place rewrite above was written to avoid, so the move should also let that rewrite go.
