...
1. [TL;DR](#0-tldr)
2. [Scope and Non-Goals](#1-scope-and-non-goals)
3. [Reference Matrix — What We Take From Each](#2-reference-matrix--what-we-take-from-each)
4. [Data Model — `<drawing>` and Shape Records](#3-data-model--drawing-and-shape-records)
5. [Position, Selection, and the Mode Model](#4-position-selection-and-the-mode-model)
6. [Step Algebra and Commands](#5-step-algebra-and-commands)
7. [Tool State Machine](#6-tool-state-machine)
8. [Hit Testing](#7-hit-testing)
9. [Render Contract — `view`/`edit` Templates for Drawings](#8-render-contract)
10. [Connector Routing](#9-connector-routing)
11. [Snap, Align, and Smart Guides](#10-snap-align-and-smart-guides)
12. [Clipboard and Paste](#11-clipboard-and-paste)
13. [History Integration](#12-history-integration)
14. [Package Module Plan](#13-package-module-plan)
...
- **Infinite-canvas / edgeless / whiteboard mode** for the *whole* doc. Stage 4 is flow-doc-with-embedded-drawings; the whole-doc canvas mode is a future Stage 5 if and when needed.
...
## 4. Position, Selection, and the Mode Model

### 4.1 `SourcePos` inside drawings
...
### 4.2 New selection variant: `MultiNodeSelection`
...
### 4.4 Focus & the `data-editable` annotation
...
| `cmd_enter_canvas_mode(drawing_id)` | Mode switch (§4.3) |
| `cmd_exit_canvas_mode()` | Mode switch (§4.3) |
...
Unlike tldraw's nested `StateNode` hierarchy, ours is flat — one active tool at a time, each tool is self-contained, transitions go through `editor.set_tool(name)`. This is simpler and sufficient for Stage 4 scope; nested sub-states (e.g., "select tool → resizing handle 3") are handled by tool-internal flags rather than a state hierarchy.
...
For drag gestures producing a `set_attr(x)` per frame on the same shape, the full path is expensive. The optimization (Stage 4 §15 phase 4h):
...
| Internal: shape | onto flow doc | Convert to inline image (render shape to PNG/SVG) — Stage 4 *uses* SVG export |
...
```c
// radiant/canvas_bridge.hpp (Stage 4)
...
A new tree `test/lambda/editing/` (parallel to the existing `test/lambda/editor/`) — kept separate so Stage-4 work doesn't entangle with the Stage 1-3 baseline.
...
A *separate* tier (Stage 4 phase 4h) drives the same fixtures through the live Radiant pipeline to catch render/layout/hit-test bugs that the headless tier can't see. This is the existing pattern from `test/lambda/editor/` baseline tests, extended.
...
| Coordinate units (px vs pt) cross-format paste | Low | Stage 4 ships with `'px` only; other units deferred |
...
5. **What about `text-frame` nesting depth?** A `text-frame` can contain a `<drawing>` (recursive drawings) — *not* in Stage 4. Schema disallows it; we revisit if a use case appears.
