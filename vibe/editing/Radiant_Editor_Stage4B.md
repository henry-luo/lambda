...
**Done since:** dead-test-exe cleanup (removed `test_wpt_contenteditable_gtest` + `test_chrome_editing_gtest` build entries and their orphaned sources — they no-op'd after the Phase-5 engine deletion); Phase-0 formal sign-off recorded (§7); the four substrate→editing "edges" audited as already-moot (§1.4).
