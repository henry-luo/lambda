...
**Date:** 2026-07-02 · **Status:** **Phase A + Phase B GREEN, integrated under `make editor-4c`** — Phase A **1931/1931** (`make editor-4c-js`, headless `lambda.exe js`), Phase B **34/34** (`make editor-4c-view`, `lambda.exe view` + `event_sim`). All 5 LambdaJS runtime bugs + all 4 Phase-B substrate blockers fixed (arrow-caret nav, selectionchange nested-CE, Shift+Enter state_machine invariant, Tab routing). Gestures now include **HTML5 drag-reorder** (JS `DragEvent` dispatch with a session `dataTransfer`, decoupled from the native drag machinery — see "drag-reorder" below). `make test-radiant-baseline` 5934/5934, `test_js_gtest` 302/303 (pre-existing `vm_runincontext_cross_unit`; `dom_jquery_lib`+`lib_popper` also fail pre-existing on this branch — "no document set" in the JS harness, unrelated to Stage 4C). Remaining: Milestone-3 parity report + CI wiring; React `.test.tsx` stays out (§1.2). Two-phase plan + Lane B pick unchanged (§4.2).
...
- **Root-cause subtlety:** the editor mutates the DOM inside `dragover` (`showDropLine`), which triggers Radiant's "force full relayout by clearing view tree" → `doc_state_clear_drag_drop`, wiping the **native** drag mid-gesture (confirmed: `active` flipped to 0 after the first dragover). So the JS-DnD lifecycle is **deliberately decoupled** from `DragDropState` — driven by an own `g_jsdnd_active` flag + a source held as a DOM element (survives the view-tree rebuild; only layout views are freed). `event_sim`'s `drag_and_drop` draggable check was relaxed to match the img/a default. **This decoupling is a workaround, not the right fix** — the underlying defect (a localized DOM mutation forces a full view-tree teardown and discards in-flight interaction state) is filed as `vibe/radiant/Radiant_Issue4.md` for follow-up correction (re-anchor `DragDropState`/caret to DOM anchors; incremental relayout instead of full teardown).
...
- **table col-resize** — coordinate mouse drag on a column resizer; likely works now via the window-`mouseup` fix + `mouse_drag dx/dy` (same shape as image-resize), but the resizer overlay wiring hasn't been fixture-verified yet.
...
- **`make editor-4c-view`** → the Phase-B fixtures under `lambda.exe view` + `event_sim`. **34/34.**
...
| **B1 — Lane B subset** | ✅ **done (initial set)** | `make editor-4c-view` runs `test/ui/editor4b/*.json` (13) + `test/ui/editor4c/*.json` (21, incl. drag-reorder); all 4 former `_open_` blockers root-caused + fixed in the Radiant substrate (state_machine caret-projection guard, Tab keydown-first routing, `event_sim` Arrow* aliases, per-fixture `html`) | **34/34 pass**; **`make test-radiant-baseline` 5934/5934** (fully green — the substrate fixes also cleared the 3 form-drag + 1 view-cmd pre-existing failures) |
...
| **3 — Integration & parity** | ◑ **make targets landed** | `make editor-4c-js` (Phase A via `tools/run-phase-a.mjs`), `make editor-4c-view` (Phase B), `make editor-4c` (both) | **`make editor-4c` green: Phase A 1931/1931 + Phase B 34/34.** Parity report vs. vitest + CI wiring still TODO |
