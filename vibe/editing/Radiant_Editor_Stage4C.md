...
**Date:** 2026-07-08 · **Status:** **Phase A + Phase B GREEN, integrated and re-stamped under `make editor-4c`** — Phase A **1931/1931** (`make editor-4c-js`, headless `lambda.exe js`), Phase B **39/39** (`make editor-4c-view`, `lambda.exe view` + `event_sim`). All 5 LambdaJS runtime bugs + all Phase-B substrate blockers fixed (arrow-caret nav, selectionchange nested-CE, Shift+Enter state_machine invariant, Tab routing, table col-resize). Gestures include **HTML5 drag-reorder** (JS `DragEvent` dispatch, now riding on the retention-safe native `DragDropState` — Radiant_Issue4 fixed) and **table column resizing** (`Object.assign(elem.style, ...)` now writes through the inline-style host setter). **Milestone 3 COMPLETE:** `make editor-4c-parity` reconciles Radiant 1931/0 group-for-group against the vitest/jsdom oracle (1960 total − 29 React `.test.tsx` = 1931), **exact parity, zero unexplained divergence** (`vibe/editing/Stage4C_Parity_Report.md`); CI wired via `run-tests-linux.sh`. React `.test.tsx` stays out (§1.2).
...
- **Phase C (expanded depth):** add a second UI lane over high-value Phase-A cases that Phase B intentionally sampled lightly: table merge/split, gap cursor/block atoms, inline atom deletion, richer paste, advanced caret/selection, source-path/reconcile invariants, and mark/block toolbar breadth.
...
**Phase B — 28/28 (blockers) → 32/32 (gestures) → 39/39 (final re-stamp), all green (2026-07-08):** `make editor-4c-view` runs the 4B baseline + the Stage-4C fixture set (`test/ui/editor4c/`) under `lambda.exe view --event-file` + `event_sim`. **39/39 pass**. No `_open_` blockers remain — all were root-caused and fixed.
...
- **`make editor-4c-view`** → the Phase-B fixtures under `lambda.exe view` + `event_sim`. **39/39.**
...
**Future optional Lane-B expansion:** broader fixture translation remains possible (block-type-select, gap-cursor around images, deeper table cell selection, more command-file cases), but it is no longer blocking Stage 4C acceptance. The original `_open_` blockers have been diagnosed and fixed.
...
**Initial C1 implementation:** add `test/ui/editor4c_phase_c/*.json`, wire it into `make editor-4c-view`, and keep it intentionally small/high-signal: table merge/split, gap cursor insertion/deletion, mention atom deletion, richer paste, block type select, code/strikethrough/highlight, and blockquote/hr autoformat. Larger C2 work can add row/column table UI and structured editor-model assertions if needed.

**Phase C exit:** C1 fixtures green under `lambda.exe view --event-file`; `make editor-4c-view` includes Phase B + Phase C and remains green.
...
| **C1 — Phase-A-to-UI expansion** | ◑ **in progress** | `test/ui/editor4c_phase_c/*.json` promotes high-value Phase-A cases into `event_sim`: table merge/split, gap cursor, mention atom delete, richer paste, block/mark toolbar breadth, blockquote/hr autoformat | C1 fixtures green and included in `make editor-4c-view` |
| **C2 — optional deeper Lane-B files** | ⬚ optional | add row/column table controls if exposed by the demo, model-selection assertions for `dom-bridge`, and deeper reconcile/history fixtures | Not required for Stage 4C exit; current Lane-B subset is green under `lambda.exe view` + `event_sim` |
| **3 — Integration & parity** | ✅ **done, re-stamped 2026-07-08** | `make editor-4c-js` (Phase A), `make editor-4c-view` (Phase B), `make editor-4c-parity` (Radiant↔jsdom oracle reconciliation → `Stage4C_Parity_Report.md`), `make editor-4c` (parity + view); CI wired in `run-tests-linux.sh` | **`make editor-4c` green: Phase A 1931/1931 + Phase B 39/39; parity EXACT** (1931 non-React reconcile group-for-group; 29 React `.test.tsx` excluded → 1960 oracle total) |
...
Stage 4C has promoted the editor's own suite onto the runtime it ships on. **Phase A** is green: **1931/1931** non-React plain-DOM tests run headless under `lambda.exe js`, with exact group-for-group parity against the jsdom oracle. **Phase B** is green: **39/39** curated interaction fixtures run under `lambda.exe view` + `event_sim`, including paste/copy, toolbar commands, autoformat, image resize, drag-reorder, and table col-resize. React remains explicitly out of scope. `make editor-4c` is the acceptance gate and is green as of the 2026-07-08 re-stamp.
