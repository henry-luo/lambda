...
| **3 — Integration & parity** | ✅ **done** | `make editor-4c-js` (Phase A), `make editor-4c-view` (Phase B), `make editor-4c-parity` (Radiant↔jsdom oracle reconciliation → `Stage4C_Parity_Report.md`), `make editor-4c` (parity + view); CI wired in `run-tests-linux.sh` | **`make editor-4c` green: Phase A 1931/1931 + Phase B 34/34; parity EXACT** (1931 non-React reconcile group-for-group; 29 React `.test.tsx` excluded → 1960 oracle total) |
