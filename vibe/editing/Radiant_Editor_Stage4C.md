...
**Date:** 2026-07-08 · **Status:** **Phase A + Phase B + Phase C C2 GREEN, integrated and re-stamped under `make editor-4c`** — Phase A **1931/1931** (`make editor-4c-js`, headless `lambda.exe js`), original Phase B **39/39**, expanded Phase B+C2 **51/51** (`make editor-4c-view`, `lambda.exe view` + `event_sim`). All 5 LambdaJS runtime bugs + all Phase-B/C substrate blockers fixed (arrow-caret nav, selectionchange nested-CE, Shift+Enter state_machine invariant, Tab routing, table col-resize, JS-owned select selectedness, selector-list DOM APIs). Gestures include **HTML5 drag-reorder** (JS `DragEvent` dispatch, now riding on the retention-safe native `DragDropState` — Radiant_Issue4 fixed), **table column resizing** (`Object.assign(elem.style, ...)` now writes through the inline-style host setter), and the Phase C expansion for richer paste, block type select, blockquote autoformat, mark toolbar breadth, HR gap-cursor insert/delete, mention atom deletion, and paste history. **Milestone 3 COMPLETE:** `make editor-4c-parity` reconciles Radiant 1931/0 group-for-group against the vitest/jsdom oracle (1960 total − 29 React `.test.tsx` = 1931), **exact parity, zero unexplained divergence** (`vibe/editing/Stage4C_Parity_Report.md`); `make editor-4c` is green with expanded Phase B+C2 view coverage; CI wired via `run-tests-linux.sh`. React `.test.tsx` stays out (§1.2).
...
- **Phase C (expanded depth):** add a second UI lane over high-value Phase-A cases that Phase B intentionally sampled lightly. C1 is green for richer paste, pasted image HTML, block type select, blockquote autoformat, and mark toolbar breadth; C2 is green for HR gap-cursor insert/delete, mention atom deletion, multi-paragraph paste, and paste undo/redo. Table merge/split remains deferred because the demo command path is covered in Phase A but the current `event_sim` shift-click path does not yet expose a reliable multi-cell UI command harness.
...
- **`make editor-4c-view`** → the Phase-B fixtures plus Phase C C2 under `lambda.exe view` + `event_sim`. **51/51.**
...
**Phase C expansion (2026-07-08) — 39 → 51/51:** added `test/ui/editor4c_phase_c/*.json` and wired the directory into `make editor-4c-view`. C1 covers blockquote autoformat, block type select, code/strikethrough/highlight toolbar commands, multi-paragraph paste, and pasted image HTML. C2 adds HR gap-cursor delete/insert on a seeded `editor-gap.html`, paste undo/redo history, mention atom backspace deletion, and the user-visible multi-paragraph paste preservation assertion. Direct Phase C run: **12/12**. Integrated `make editor-4c-view`: **51/51**.
...
- **C2 table note:** table merge/split remains a documented optional follow-up. The command logic is green in Phase A; the current demo UI needs either a reliable shift-click multi-cell harness or a test-only editor-selection assertion before the merge/split toolbar path can be stamped under `event_sim` without brittle adaptation.

**Future optional Lane-B expansion:** broader fixture translation remains possible (table merge/split, gap cursor around images, HR autoformat, deeper table cell controls, source-path/model-selection assertions, and reconcile-history edge cases), but it is no longer blocking Stage 4C acceptance. The original `_open_` blockers have been diagnosed and fixed.
...
- **Phase B/C:** `make editor-4c-view` runs the Lane B subset (§4.2) plus the Phase C C2 expansion (§4.3) under `lambda.exe view` + `event_sim` and reports **0 failed**, exercising every distinct editing interaction end-to-end plus the promoted high-value Phase-A UI cases.
...
**C1/C2 implementation:** `test/ui/editor4c_phase_c/*.json` is wired into `make editor-4c-view` and kept intentionally small/high-signal: richer paste, block type select, code/strikethrough/highlight, pasted image HTML, blockquote autoformat, HR gap insert/delete, paste history, and mention atom deletion. The active Phase C fixture set is **12/12 green** standalone and green in the integrated **51/51** `editor-4c-view` run. Table merge/split remains deferred until the demo/harness exposes the multi-cell toolbar command path cleanly.

**Phase C C2 exit:** met. Phase C fixtures are green under `lambda.exe view --event-file`; `make editor-4c-view` includes Phase B + Phase C C2 and remains green at **51/51**.
...
| **C1 — Phase-A-to-UI expansion** | ✅ **done** | `test/ui/editor4c_phase_c/*.json` promotes high-value Phase-A cases into `event_sim`: richer paste, pasted image HTML, block/mark toolbar breadth, blockquote autoformat, block type select | Included in Phase C **12/12** and `make editor-4c-view` **51/51** |
| **C2 — deeper Lane-B files** | ✅ **done, table merge/split deferred** | HR gap-cursor insert/delete via seeded `editor-gap.html`, mention atom delete, paste history, multi-paragraph paste preservation, selector-list DOM API regression | **12/12 Phase C pass** standalone; `make editor-4c-view` **51/51 pass**. Table merge/split is documented as optional follow-up because Phase A covers command logic but current event-sim/demo shift-click does not reliably expose the toolbar command path |
| **3 — Integration & parity** | ✅ **done, re-stamped 2026-07-08** | `make editor-4c-js` (Phase A), `make editor-4c-view` (Phase B + Phase C C2), `make editor-4c-parity` (Radiant↔jsdom oracle reconciliation → `Stage4C_Parity_Report.md`), `make editor-4c` (parity + view); CI wired in `run-tests-linux.sh` | **`make editor-4c` green: Phase A 1931/1931 + parity EXACT + expanded view 51/51** (1931 non-React reconcile group-for-group; 29 React `.test.tsx` excluded → 1960 oracle total) |
...
- **Phase B/C:** `make editor-4c-view` runs the Lane B subset plus Phase C C2 under `lambda.exe view` + `event_sim` → **51/51 passed**, covering every distinct editing interaction (§4.2 checklist) and the promoted Phase-A UI cases (§4.3).
...
Stage 4C has promoted the editor's own suite onto the runtime it ships on. **Phase A** is green: **1931/1931** non-React plain-DOM tests run headless under `lambda.exe js`, with exact group-for-group parity against the jsdom oracle. **Phase B + Phase C C2** is green: **51/51** curated interaction fixtures run under `lambda.exe view` + `event_sim`, including paste/copy, toolbar commands, autoformat, image resize, drag-reorder, table col-resize, block type select, blockquote autoformat, richer paste, pasted image HTML, code, strikethrough, highlight, HR gap-cursor insert/delete, mention atom deletion, and paste undo/redo. React remains explicitly out of scope. `make editor-4c` is green as of the 2026-07-08 Phase C C2 re-stamp.
