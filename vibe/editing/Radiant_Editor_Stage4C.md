...
**Regression check:** `test_js_gtest` 302/303 (only pre-existing `vm_runincontext_cross_unit`); **`test_ui_automation_gtest` 219/219** (up from 216/219 — the 3 pre-existing form-drag failures are now green, incidentally cleared by the Phase-A tail DOM-fidelity work); `make test-radiant-baseline` remaining differences are pre-existing (verified via `git stash` + rebuild).
...
| **B1 — Lane B subset** | ◑ **pilot + first expansion** | `make editor-4c-view` runs `test/ui/editor4b/*.json` (12) + `test/ui/editor4c/*.json` (11 new); `_open_` prefix marks 4 known blockers (arrow-caret nav, selectionchange nested-CE, Shift+Enter state_machine assert, Tab not reaching handler) that run but are excluded from the pass count | 23/23 pass; **`test_ui_automation_gtest` 219/219** (up from 216/219 — the 3 pre-existing form-drag failures now green, incidentally cleared by the Phase-A tail DOM-fidelity work) |
