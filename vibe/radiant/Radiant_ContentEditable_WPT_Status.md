...
| **CE-5** — Drop into editable | ✅ Shipped (lowered) | Drop completion at `event.cpp` lowers to `beforeinput { insertFromDrop, data: <drag_data> }` on the target, paired with `beforeinput { deleteByDrag }` on the source when source was also editable. Full `DataTransfer` object (multi-MIME + files) deferred — uses the same clipboard Phase 9 transport. |
...
| **CE-7** — JS API surface + status doc | ✅ Shipped (this doc) | `HTMLElement.contentEditable` / `isContentEditable` / `inputMode` / `enterKeyHint` exposed (CE-1 + CE-4). `InputEvent` constructor already in place pre-CE-3. `StaticRange` constructor registered (`JS_CTOR_STATIC_RANGE`) — stub copies the dictionary fields and computes `collapsed`. Status doc published here. |
...
| `input-events/input-events-get-target-ranges*` (non-tentative) | blocked on real `StaticRange` snapshot (CE-3 follow-up) |
| `input-events/input-events-cut-paste.html` | not yet wired — CE-3 dispatch path ready, `dataTransfer` blocked |
...
| ~~`dataTransfer` population on `insertFromPaste` / `insertFromDrop`~~ | ✅ Shipped — text/plain + text/html via `js_data_transfer_new_with_strings()` (CE-3 follow-up) |
