...
- The Tree-sitter **runtime** remains linked for document grammars, but JavaScript, TypeScript, and Lambda grammar archives link only into the isolated `lambda-cst` verifier under D8.1.3v10. Ordinary `lambda.exe` therefore has no JS/TS grammar or query-highlighting path. Upstream `queries/highlights.scm` files remain reference material for bash/js(+jsx)/python/ruby. The formats Lambda cares most about (markdown, HTML, CSS, JSON, YAML, TOML, Lambda itself) have **no vendored grammar**, and each is a multi-MB `parser.c` vendored dep.
...
Only after a separate decision restores a production grammar link for a proven
need may a native Tree-sitter highlighting binding be considered. The entry
criteria remain a concrete correctness gap a stream mode cannot close and a
measured binding cost; the isolated `lambda-cst` reference grammars are not a
host API. Until then, Tree-sitter stays untouched (rule 16 posture).
