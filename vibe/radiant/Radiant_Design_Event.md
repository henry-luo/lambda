...
| Pseudo-state mutation (`:hover`, `:active`, `:focus`) | `update_hover_state` / `update_active_state` | Writes through StateStore/ViewState writer APIs; `DomElement::pseudo_state` is no longer a dynamic cache. |
