...
- **Page scripts see only the v3.2 language surface**: `start(target, args)` tasks ride the page loop (level 1 — ideal for I/O concurrency inside a page); heavy compute goes to a tier-2 worker — `{mode: 'thread'}` for zero-copy flats (bitmaps, typed arrays) or `{mode: 'process'}` for crash containment (S13.1.1v2, K31v2, `Lambda_Design_Concurrency.md` §10.3.2); Stage B (M:N tasks) is a later, optional relief for CPU-bound page work rather than the only road. A CPU-bound stretch on the page thread janks *that page only* — same trade every browser makes.
