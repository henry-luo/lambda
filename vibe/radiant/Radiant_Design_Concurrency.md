...
- **Page scripts see only the v3 language surface**: `start` tasks ride the page loop (level 1 — ideal for I/O concurrency inside a page); heavy compute goes to a tier-2 worker — `start worker(spec, isolation: 'thread')` for zero-copy flats (bitmaps, typed arrays) or `'process'` for crash containment (K31, `Lambda_Design_Concurrency.md` §10.3.2); Stage B (M:N tasks) is a later, optional relief for CPU-bound page work rather than the only road. A CPU-bound stretch on the page thread janks *that page only* — same trade every browser makes.
