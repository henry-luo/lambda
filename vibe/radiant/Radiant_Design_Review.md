...
6. **Duplicated representations.** Grid's legacy layer + Taffy layer bridged by a per-pass converting adapter, with a "pure" §11 driver that skips §11.5 and is effectively dead (`grid_sizing_algorithm.hpp:1516`); canonical `DomSelection` vs legacy `state_store` caret/selection projection with duplicated write paths; paint op/payload definitions triple-mirrored across `PaintOp`/`DisplayOp`/`rdt_*` (~3 edit sites per new primitive); a legacy list-bullet render path alongside the `::marker` path (`render_list.cpp:217`); three overlapping view-teardown walks (`free_view`/`release_view_owned_resources_in_node`/`clear_view_owned_pointers_in_node`).
...
8. **Script-before-cascade ordering.** Document scripts run before CSS cascade ([RAD_21](../../doc/dev/radiant/RAD_21_JS_Scripting_Integration.md)) — a spec divergence that will surface as compatibility bugs (scripts reading computed style / geometry at parse time).
...
10. **`g_layout_table_height` global workaround** for a "mysterious corruption" never root-caused (`layout_block.cpp:990`, save/restore at `:4699/:4767/:7756`) — a standing violation of the project's own fix-root-causes rule and a masking layer over a real memory bug.
11. **Silent fixed caps that drop content.** 256-slot z-order arrays in the render walk silently drop overflow children (`render_walk.cpp:390/418`); grid `MAX_GRID_TRACKS=64`/`MAX_GRID_ITEMS=256`; ThorVG clip stack depth 8 drops clips (`rdt_vector_tvg.cpp:1461`); flex's fixed 1000-entry global measurement cache; several `max_iterations` float-stepping loops truncate silently.
...
- **P1 (S): Compile-time ABI guard for the zero-field invariant.** Add `static_assert(sizeof(ViewBlock) == sizeof(DomElement))` (and for `ViewText`/`ViewTable`/all wrappers) next to the class definitions in `view.hpp`, plus a comment pointing at `lib/tagged.hpp`. Turns Tier-3 issue #9 from silent corruption into a compile error. Zero risk.
- **P2 (M): Root-cause `g_layout_table_height`.** The workaround has known save/restore choreography, which is itself a reproduction recipe: instrument with ASan + a watchpoint on the corrupted field under the layout test suite, find the writer, delete the global. Until then, at minimum document the workaround at all four sites with a shared comment tag so it can't grow new tendrils.
- **P3 (S): Verification mode for incremental relayout.** Debug-build flag: when a child is skipped as clean, *also* lay it out and assert the cached `layout_height_contribution` matches. Run in CI on the layout suite. Catches Tier-3 #12 class bugs permanently.
- **P4 (M): Make silent caps loud, then growable.** One sweep: every fixed-capacity drop site (z-order 256, grid caps, clip depth 8, flex cache, iteration caps) gets a `log_warn` with a distinct prefix; then convert the two content-dropping ones (z-order arrays, clip stack) to arena-allocated growth. The grid caps can stay fixed (Taffy heritage) but must log.
- **P5 (S): Delete the duplicated webview hit-test block** in `event.cpp` and honor z-index in positioned hit-testing (or file it explicitly against the paint-order refactor in P11).
- **P6 (M): Kill the dead code found by the survey.** The dead "pure" grid §11 driver (or make the adapter call it — either way remove the trap), the dead `IntrinsicSizeCache` header struct, `parse_font_face_rule_OLD` (already fully commented out), the stale `table_auto_layout_algorithm`/`table_fixed_layout_algorithm` header decls, `RDT_VIEW_MATH`.
...
- **P14 (M): Finish the selection/caret migration.** Make `DomSelection` the single source of truth and reduce `state_store_legacy_*` to a read-only projection (or delete it), removing the dual write paths. Sequenced with the Stage-4B editor work, which is already forcing clarity here.
...
- **P16 (M): One teardown walk.** Replace the three overlapping view-teardown walks with a single visitor driven by a per-property-group release table; new pooled pointers then register in one place.
- **P17 (S/M): Spec-ordering fix for scripts.** Apply cascade before running document scripts (or at minimum before `AFTER_SCRIPTS`-class tasks), keeping the current behavior behind a flag until the layout suite is re-baselined. Cheap now; expensive after more content depends on the current order.
