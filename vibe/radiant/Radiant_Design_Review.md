...
- ✅ **P11 (L): Paint-order/stacking-context cleanup.** `radiant/stacking_order.{hpp,cpp}` is now the shared positioned/positive-z collection and stable paint-order sorter. Render walk paints those lists forward; hit-testing consumes the same lists in reverse, including positive-z descendants and positioned overlays above iframe/layer-webview self-content, with a regression at `test/ui/hit_test_stacking_order.*`. Full CSS stacking-context layer expansion remains documented as a narrower follow-up in RAD_13.
