...
- ◐ **P2 (M): Root-cause `g_layout_table_height`.** The workaround has known save/restore choreography, which is itself a reproduction recipe: instrument with ASan + a watchpoint on the corrupted field under the layout test suite, find the writer, delete the global. Until then, at minimum document the workaround at all four sites with a shared comment tag so it can't grow new tendrils. The shared `RAD_TABLE_HEIGHT_GUARD` comments are landed; the underlying writer/corruption root cause remains open.
