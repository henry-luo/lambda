...
- All §3 non-goals of the support doc unchanged: no `execCommand`/default contenteditable mutation, no Shadow DOM/customElements, no WebSocket/Worker, IndexedDB KIV.
...
3. **Three new WPT runners, red-baselined** (the baseline pins current truth; passing sets ratchet later):
   - `test_wpt_input_events_gtest` over `ref/wpt/input-events/` (29 files) — acceptance for the §3.1 script-owned-editing contract and prerequisite confidence for CodeMirror 6. Note: `input-events-exec-command.html` and other execCommand-dependent files are non-goal skips, listed as such in the baseline header.
...
2. **`input-events/`** — `beforeinput`/`input` ordering, `inputType` values, `getTargetRanges()` in editable regions, cut/paste input events. execCommand-dependent files stay non-goal skips. This suite is the conformance statement of the §3.1 script-owned-editing contract.
...
The proof that dropping `execCommand` costs nothing for modern editors: contenteditable as a flag, all mutations applied by the library from `beforeinput` + its own model, MutationObserver for reconciliation, Selection/Range throughout. Everything it needs exists after Phases 5; this phase is integration and bug-fixing, not new API surface.
