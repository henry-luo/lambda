...
| 2026-07-30 | 8 | Removed the orphaned WPT-only legacy command shim after confirming the native command APIs are absent and no caller can reach the shim. | `JavaScriptTests/JsFileTest.Run/dom_module_props`, `WptClipboard/WptClipboardTest.Run/clipboard_events_synthetic`, `make test-editable`, cleanup source audits, and no-int-cast lint pass. | Immutable upstream WPT corpus references remain outside active-source audits. |
