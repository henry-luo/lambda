...
TEST_F(FlexLayoutTest, BasicRowLayout) {
    ...
    EXPECT_EQ(item1->x, 0) << "First item should be at x=0";
    EXPECT_EQ(item2->x, 100) << "Second item should be at x=100";
    EXPECT_EQ(item3->x, 200) << "Third item should be at x=200";
    
    // Check that all items are aligned at top
    EXPECT_EQ(item1->y, 0) << "First item should be at y=0";
    EXPECT_EQ(item2->y, 0) << "Second item should be at y=0";
    EXPECT_EQ(item3->y, 0) << "Third item should be at y=0";
...
TEST_F(FlexLayoutTest, FlexGrowDistribution) {
    ...
    // item3 gets 0 extra
    EXPECT_EQ(item1->content_width, 133) << "Item1 should grow to 133px";
    EXPECT_EQ(item2->content_width, 167) << "Item2 should grow to 167px";
    EXPECT_EQ(item3->content_width, 100) << "Item3 should remain 100px";
...
TEST_F(FlexLayoutTest, JustifyContentCenter) {
    ...
    EXPECT_EQ(item1->x, 100) << "First item should be centered at x=100";
    EXPECT_EQ(item2->x, 200) << "Second item should be at x=200";
...
TEST_F(FlexLayoutTest, JustifyContentSpaceBetween) {
    ...
    EXPECT_EQ(item1->x, 0) << "First item should be at x=0";
    EXPECT_EQ(item2->x, 150) << "Second item should be at x=150";
    EXPECT_EQ(item3->x, 300) << "Third item should be at x=300";
    
    cleanup_test_container(container);
}

TEST_F(FlexLayoutTest, ColumnDirection) {
    ...
    EXPECT_EQ(item1->y, 0) << "First item should be at y=0";
    EXPECT_EQ(item2->y, 100) << "Second item should be at y=100";
    EXPECT_EQ(item3->y, 200) << "Third item should be at y=200";
    
    // Check that all items are aligned at left
    EXPECT_EQ(item1->x, 0) << "First item should be at x=0";
    EXPECT_EQ(item2->x, 0) << "Second item should be at x=0";
    EXPECT_EQ(item3->x, 0) << "Third item should be at x=0";
...
TEST_F(FlexLayoutTest, AlignItemsCenter) {
    ...
    EXPECT_EQ(item1->y, 25) << "Item1 should be centered at y=25 (100-50)/2";
    EXPECT_EQ(item2->y, 0) << "Item2 should be at y=0 (tallest item)";
    EXPECT_EQ(item3->y, 12) << "Item3 should be centered at y=12 (100-75)/2";
    
    cleanup_test_container(container);
}

TEST_F(FlexLayoutTest, WrapMultiline) {
    ...
    EXPECT_EQ(item1->x, 0) << "Item1 should be at x=0";
    EXPECT_EQ(item2->x, 100) << "Item2 should be at x=100";
    EXPECT_EQ(item1->y, 0) << "Item1 should be at y=0 (first line)";
    EXPECT_EQ(item2->y, 0) << "Item2 should be at y=0 (first line)";
    
    EXPECT_EQ(item3->x, 0) << "Item3 should be at x=0 (second line)";
    EXPECT_EQ(item4->x, 100) << "Item4 should be at x=100 (second line)";
    EXPECT_EQ(item3->y, 50) << "Item3 should be at y=50 (second line)";
    EXPECT_EQ(item4->y, 50) << "Item4 should be at y=50 (second line)";
...
TEST_F(FlexLayoutTest, GapProperties) {
    ...
    EXPECT_EQ(item1->x, 0) << "Item1 should be at x=0";
    EXPECT_EQ(item2->x, 125) << "Item2 should be at x=125 (100 + 25 gap)";
    EXPECT_EQ(item3->x, 250) << "Item3 should be at x=250 (100 + 25 + 100 + 25)";
...
TEST_F(FlexLayoutTest, OrderProperty) {
    ...
    FlexContainerLayout* flex = container->embed->flex_container;
    EXPECT_EQ(flex->line_count, 1) << "Should have one line";
    
    if (flex->line_count > 0) {
        FlexLineInfo* line = &flex->lines[0];
        EXPECT_EQ(line->item_count, 3) << "Line should have 3 items";
        
        // Items should be sorted by order
        EXPECT_EQ(line->items[0]->order, 0) << "First item should have order=0";
        EXPECT_EQ(line->items[1]->order, 1) << "Second item should have order=1";
        EXPECT_EQ(line->items[2]->order, 2) << "Third item should have order=2";
    }
    
    cleanup_test_container(container);
}

TEST_F(FlexLayoutTest, AlignSelfOverride) {
    ...
    EXPECT_EQ(item1->y, 0) << "Item1 should use align-items: start";
    EXPECT_EQ(item2->y, 0) << "Item2 should use align-self: end (50-50=0 in this case)";
    EXPECT_EQ(item3->y, 0) << "Item3 should use align-items: start";
