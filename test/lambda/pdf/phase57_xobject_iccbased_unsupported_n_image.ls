// Phase 57 - raw Image XObject ICCBased unsupported N renders black.

import interp: lambda.package.pdf.interp

fn ref(n) { { type: "indirect_ref", object_num: n } }
fn name(s) { { kind: "name", value: s } }
fn has(s: string, needle: string) { (index_of(s, needle) >= 0) }

fn image_obj(n, dict, data) {
    { object_num: n, gen_num: 0, content: { dictionary: dict, data: data } }
}

pn main() {
    let img_dict = {
        Type: "XObject", Subtype: "Image",
        Width: 1, Height: 1,
        ColorSpace: ["ICCBased", { N: 5 }],
        BitsPerComponent: 8
    }
    let doc = { objects: [image_obj(157, img_dict, "ABCDE")], pages: [] }
    let page = { dict: { Resources: { XObject: { Im0: ref(157) } } } }
    let ops = [
        { op: "cm", operands: [1.0, 0.0, 0.0, 1.0, 7.0, 8.0] },
        { op: "Do", operands: [name("Im0")] }
    ]
    let r = interp.render_page(doc, page, ops, 100.0)
    let xml = format(r.paths[0], 'xml')
    print({
        count: len(r.paths),
        black_pixel: has(xml, "fill=\"rgb(0,0,0)\""),
        not_rgb: not has(xml, "fill=\"rgb(65,66,67)\""),
        no_img_handle: not has(xml, "href=\"img:157\""),
        kept_ctm: has(xml, "transform=\"matrix(1 0 0 1 7 8)\"")
    })
}
