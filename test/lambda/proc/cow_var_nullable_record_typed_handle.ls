// Companion of cow_var_nullable_record.ls with the ANNOTATED handle
// (`var left: N? = node.left`). T0 does not lower a member store through a
// typed place copy, so every tier runs this file on MIR; the golden still
// pins the semantics.
//
