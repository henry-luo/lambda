import type { MathfieldElement } from '../../../../ref/mathlive/src/public/mathfield-element';
...
test('virtual-keyboard-toggle visibility', async ({ page }) => {
  ...
  expect(
    await page.locator('.lm_virtual-keyboard-toggle').nth(0).isVisible()
  ).toBe(true);

  expect(
    await page.locator('.lm_virtual-keyboard-toggle').nth(1).isVisible()
  ).toBe(false);
});
...
test('virtual keyboard with two math fields', async ({ page }) => {
  ...
  await page.locator('.lm_virtual-keyboard-toggle').nth(0).click();
...
test('math fields in iframe with virtual keyboard', async ({ page }) => {
  ...
  if (frame) {
    // toggle the virtual keyboard to visible and focus first math field
    await frame.locator('.lm_virtual-keyboard-toggle').nth(0).click();
