...
// S11.1.6 / S11.1.1v3: the occurrence family (T? T* T+ T{n,m}) is a run —
...
// image). Annotations never change a value's kind. (phase P5)
...
[xs, 9];
[ys, 9]
fn evens(a: int[]) int* => for (x in a) if (x % 2 == 0) x;
