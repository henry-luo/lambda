...
/**
 * Lambda REPL CLI Interface Tests (Catch2 Version)
 *
 * Converted from Criterion test suite with comprehensive coverage for reliable execution.
...
TEST_CASE("executable_exists", "[lambda_repl]") {
    int result = system("test -x ./lambda.exe");
    REQUIRE(result == 0);
    INFO("Lambda executable should exist and be executable");
}

TEST_CASE("startup_and_quit", "[lambda_repl]") {
    struct test_result result = run_lambda_repl(".quit\\n");
    
    REQUIRE(result.output != NULL);
    INFO("Expected output from REPL");
    
    REQUIRE(result.output_len > 0);
    INFO("REPL should produce output");
    
    REQUIRE(output_contains(result.output, "Lambda"));
    INFO("Output should mention Lambda");
    
    free_test_result(&result);
}

TEST_CASE("basic_arithmetic", "[lambda_repl]") {
    struct test_result result = run_lambda_repl("2 + 3\\n.quit\\n");
    
    REQUIRE(result.output != NULL);
    INFO("Expected output from arithmetic");
    
    REQUIRE(output_contains(result.output, "5"));
    INFO("Should show arithmetic result: 5");
    
    free_test_result(&result);
}

TEST_CASE("help_command", "[lambda_repl]") {
    struct test_result result = run_lambda_repl(".help\\n.quit\\n");
    
    REQUIRE(result.output != NULL);
    INFO("Expected output from .help command");
    
    bool has_quit_help = output_contains(result.output, ".quit") || output_contains(result.output, "quit");
    REQUIRE(has_quit_help);
    INFO("Help should mention quit command");
    
    free_test_result(&result);
}

TEST_CASE("multiple_commands", "[lambda_repl]") {
    struct test_result result = run_lambda_repl("1 + 1\\n2 * 3\\n.quit\\n");
    
    REQUIRE(result.output != NULL);
    INFO("Expected output from multiple commands");
    
    bool has_results = output_contains(result.output, "2") || output_contains(result.output, "6");
    REQUIRE(has_results);
    INFO("Should show results from multiple expressions");
    
    free_test_result(&result);
}

TEST_CASE("quit_variations", "[lambda_repl]") {
    // Test .q short form
    struct test_result result1 = run_lambda_repl(".q\\n");
    REQUIRE(result1.output != NULL);
    INFO("Expected output from .q");
    free_test_result(&result1);
    
    // Test .exit
    struct test_result result2 = run_lambda_repl(".exit\\n");
    REQUIRE(result2.output != NULL);
    INFO("Expected output from .exit");
    free_test_result(&result2);
}

TEST_CASE("complex_arithmetic", "[lambda_repl]") {
    struct test_result result = run_lambda_repl("5 * 7\\n8 / 2\\n.quit\\n");
    
    REQUIRE(result.output != NULL);
    INFO("Expected output from complex arithmetic");
    
    bool has_results = output_contains(result.output, "35") || output_contains(result.output, "4");
    REQUIRE(has_results);
    INFO("Should show complex arithmetic results");
    
    free_test_result(&result);
}

TEST_CASE("error_recovery", "[lambda_repl]") {
    struct test_result result = run_lambda_repl("2 +\\n1 + 1\\n.quit\\n");
    
    REQUIRE(result.output != NULL);
    INFO("Expected output from error recovery test");
    
    // Should continue running despite syntax error
    bool has_recovery = output_contains(result.output, "2") || output_contains(result.output, "Lambda");
    REQUIRE(has_recovery);
    INFO("Should recover from syntax error");
    
    free_test_result(&result);
}

TEST_CASE("version_display", "[lambda_repl]") {
    struct test_result result = run_lambda_repl(".quit\\n");
    
    REQUIRE(result.output != NULL);
    INFO("Expected output from REPL");
    
    bool has_version = output_contains(result.output, "1.0") || output_contains(result.output, "v1");
    REQUIRE(has_version);
    INFO("Should show version information");
    
    free_test_result(&result);
}

TEST_CASE("repl_functionality", "[lambda_repl]") {
    struct test_result result = run_lambda_repl(".quit\\n");
    
    REQUIRE(result.output != NULL);
    INFO("Expected output to check REPL behavior");
    ...
    INFO("Should show REPL startup information");
    
    free_test_result(&result);
}

TEST_CASE("command_sequence_stability", "[lambda_repl]") {
    struct test_result result = run_lambda_repl("1 + 1\\n.help\\n2 * 2\\n.quit\\n");
    
    REQUIRE(result.output != NULL);
    INFO("Expected output from command sequence");
    
    REQUIRE(result.output_len > 50);
    INFO("Should produce substantial output");
...
TEST_CASE("prompt_display", "[lambda_repl_interactive]") {
    struct test_result result = run_lambda_repl_interactive(".quit\n");
    
    REQUIRE(result.output != NULL);
    INFO("Expected output from interactive REPL");
    ...
    INFO("Interactive mode should show Lambda prompt (λ> or L>)");
    
    free_test_result(&result);
}

TEST_CASE("prompt_with_expressions", "[lambda_repl_interactive]") {
    struct test_result result = run_lambda_repl_interactive("2 + 3\n.quit\n");
    
    REQUIRE(result.output != NULL);
    INFO("Expected output from interactive expressions");
    ...
    INFO("Should show either prompts or startup in interactive mode");
    
    free_test_result(&result);
}

TEST_CASE("unicode_prompt_support", "[lambda_repl_interactive]") {
    struct test_result result = run_lambda_repl_interactive(".quit\n");
    
    REQUIRE(result.output != NULL);
    INFO("Expected output to check Unicode support");
    ...
    INFO("Should display appropriate prompt for locale");
    
    free_test_result(&result);
}

TEST_CASE("multiple_prompt_sequence", "[lambda_repl_interactive]") {
    struct test_result result = run_lambda_repl_interactive("1 + 1\n2 * 2\n.quit\n");
    
    REQUIRE(result.output != NULL);
    INFO("Expected output from multiple prompts");
    ...
    REQUIRE(has_content);
    INFO("Should have some output in interactive mode");
