// GC regression (2026-09-07): a reduced splay (40 nodes). Its node literal
// {key: any, left: null, right: null, value} puts the null lanes at unaligned
// offsets; a child linked into such a lane in place stayed invisible to the
// collector (shape walk skips null lanes, the word sweep never lands on
// offset 17) and was freed while linked. The forced-collection stress sweep
// (collection at every allocation, freed objects poisoned) failed here on
// every tier before the collector marked null lanes conservatively.
// JetStream Benchmark: splay (Octane) — UNTYPED baseline
...
let TREE_SIZE = 40
let TREE_MODIFICATIONS = 10

// Node: {key, left, right, value}
pn create_node(key, value) {
    return {key: key, left: null, right: null, value: value}
}
...
pn generate_payload(depth: int, tag) {
    if (depth == 0) {
