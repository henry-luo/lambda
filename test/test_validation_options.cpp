...
TEST_F(ValidationOptionsTest, SetAndGetOptions) {
    ...
    ValidationOptions opts = ast_validator_default_options();
    ...
    // set options
    ast_validator_set_options(validator, &opts);

    // get and verify
    ValidationOptions* retrieved = ast_validator_get_options(validator);
...
TEST_F(ValidationOptionsTest, ConvenienceSetters) {
    ...
    ast_validator_set_max_errors(validator, 5);
    ast_validator_set_timeout(validator, 1000);
    ast_validator_set_show_suggestions(validator, false);
    ast_validator_set_show_context(validator, false);
...
TEST_F(ValidationOptionsTest, MaxDepthEnforcement) {
    ...
    int result = ast_validator_load_schema(validator, schema, "Nested1");
    ASSERT_EQ(result, 0);

    // set very low max depth
    ast_validator_set_max_errors(validator, 1);  // stop after first error
    ValidationOptions* opts = ast_validator_get_options(validator);
    ...
    ValidationResult* val_result = ast_validator_validate(validator, item, "Nested1");
    ...
    EXPECT_EQ(validator->options.max_depth, 2);
}
...
TEST_F(ValidationOptionsTest, MaxErrorsStopsValidation) {
    ...
    int result = ast_validator_load_schema(validator, schema, "Document");
    ASSERT_EQ(result, 0);
    ...
    // validate
    ValidationResult* val_result = ast_validator_validate(validator, item, "Document");
    ...
    EXPECT_EQ(validator->options.max_errors, 2);
...
TEST_F(ValidationOptionsTest, TimeoutPreventsLongValidation) {
    ...
    int result = ast_validator_load_schema(validator, schema, "Simple");
    ASSERT_EQ(result, 0);

    // set very short timeout
    ast_validator_set_timeout(validator, 1);  // 1ms - may timeout immediately
    ...
    ValidationResult* val_result = ast_validator_validate(validator, item, "Simple");

    ASSERT_NE(val_result, nullptr);

    // timeout is hard to test reliably - validation may be too fast
    // just verify the option was set
    EXPECT_EQ(validator->options.timeout_ms, 1);
...
TEST_F(ValidationOptionsTest, ShowSuggestionsOption) {
    ...
    ast_validator_set_show_suggestions(validator, true);
...
TEST_F(ValidationOptionsTest, ShowContextOption) {
    ...
    ast_validator_set_show_context(validator, true);
...
TEST_F(ValidationOptionsTest, StrictModeOption) {
    ...
    ast_validator_set_strict_mode(validator, true);
    EXPECT_TRUE(validator->options.strict_mode);

    ast_validator_set_strict_mode(validator, false);
    EXPECT_FALSE(validator->options.strict_mode);
}
