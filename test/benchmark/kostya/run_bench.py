#!/usr/bin/env python3
"""Kostya Benchmark Runner — Lambda Script vs Python
...
PY_DIR = "test/benchmark/kostya/python"
...
# Language startup overheads (measured separately)
...
print(f"\033[1mKostya Benchmark Suite — Lambda Script vs Python\033[0m")
print("=" * 78)
...
for name, cat, desc in BENCHMARKS:
    ...
    lm_exec[name] = me
    jit_ms = max(0, mw - me)
    print(f"  wall={format_time_ms(mw):>10s}  exec={format_time_ms(me):>10s}  jit={format_time_ms(jit_ms):>8s}")
...
print(f"\033[1m{'RESULTS SUMMARY':^82s}\033[0m")
print(f"\033[1m{SEP}\033[0m")
print()

print(f"\033[36mExecution time comparison (exec = internal __TIMING__, wall = startup-subtracted)\033[0m")
print(f"{'Benchmark':14s} {'Lm(exec)':>10s} {'Py(exec)':>10s} {'Lm(wall)':>10s} {'Py(wall)':>10s} {'Lm/Py(exec)':>12s}")
...
for name, cat, desc in BENCHMARKS:
    ...
    if le > 0 and pe > 0:
        ...
        rs = "N/A"
    print(f"{name:14s} {format_time_ms(le):>10s} {format_time_ms(pe):>10s} {format_time_ms(lw_t):>10s} {format_time_ms(pw_t):>10s} {rs:>12s}")
