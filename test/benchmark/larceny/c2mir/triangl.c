/* Native C2MIR port of larceny/triangl.ls. */
...
int main(void) {
    ...
    while (depth >= 0) {
        ...
        if (pegs == 1) {
            ...
            if (depth < 0) break;
            last = stack[depth];
            board[move_from[last]] = 1; board[move_over[last]] = 1; board[move_to[last]] = 0;
            pegs++; stack[depth] = last + 1;
            continue;
        }
        for (i = stack[depth]; i < 36; i++) {
            if (board[move_from[i]] && board[move_over[i]] && !board[move_to[i]]) {
                board[move_from[i]] = 0; board[move_over[i]] = 0; board[move_to[i]] = 1;
    ...
    printf("triangl: solutions=%d\n%s\n", solutions, solutions == 29760 ? "triangl: PASS" : "triangl: DONE");
    return solutions != 29760;
}
