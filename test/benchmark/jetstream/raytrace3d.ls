...
pn triangle_intersect(tri, orig: float[], dir: float[], near: float, far: float) float {
    var u = (tri.axis + 1) % 3
    var v = (tri.axis + 2) % 3
...
pn scene_add_light(scene, pos: float[], colour: float[]) {
...
pn scene_intersect(scene, origin: float[], dir: float[], near: float, far: float, depth: int) {
    if (depth > 3) {
