// JetStream Benchmark: navier-stokes (Octane) (Typed version)
...
pn add_fields(var x: float[], s: float[], dt: float) any {
    var i: int = 0
    while (i < GRID_SIZE) {
        x[i] = x[i] + dt * s[i]
        i = i + 1
...
pn set_bnd(b: int, var x: float[]) any {
    if (b == 1) {
        var i: int = 1
        while (i <= WIDTH) {
            x[i] = x[i + ROW_SIZE]
    ...
    var max_edge: int = (HEIGHT + 1) * ROW_SIZE
...
pn lin_solve(b: int, var x: float[], x0: float[], a: float, c: float, iterations: int) any {
    if (a == 0.0) {
        if (c == 1.0) {
            var j: int = 1
            while (j <= HEIGHT) {
                var cr: int = j * ROW_SIZE + 1
                var i: int = 0
                while (i < WIDTH) {
                    x[cr] = x0[cr]
                    cr = cr + 1
    ...
    var inv_c: float = 1.0 / c
    var k: int = 0
    while (k < iterations) {
        var j: int = 1
        while (j <= HEIGHT) {
            var last_row: int = (j - 1) * ROW_SIZE
            var current_row: int = j * ROW_SIZE
            var next_row: int = (j + 1) * ROW_SIZE
            var last_x: float = x[current_row]
...
pn diffuse(b: int, var x: float[], x0: float[], dt: float, iterations: int) any {
    var a: float = 0.0
    lin_solve(b, x, x0, a, 1.0 + 4.0 * a, iterations)
}

pn advect(b: int, var d: float[], d0: float[], u: float[], v: float[], dt: float) any {
    var w_dt0: float = dt * float(WIDTH)
    var h_dt0: float = dt * float(HEIGHT)
    var wp5: float = float(WIDTH) + 0.5
    var hp5: float = float(HEIGHT) + 0.5
    var j: int = 1
    while (j <= HEIGHT) {
        var pos: int = j * ROW_SIZE
        var i: int = 1
        while (i <= WIDTH) {
            pos = pos + 1
            var x: float = float(i) - w_dt0 * u[pos]
            var y: float = float(j) - h_dt0 * v[pos]
            if (x < 0.5) {
                x = 0.5
            } else {
                if (x > wp5) {
            ...
            var j0: int = int(floor(y))
            var j1: int = j0 + 1
            var s1: float = x - float(i0)
            var s0: float = 1.0 - s1
            var t1: float = y - float(j0)
            var t0: float = 1.0 - t1
            var row1: int = j0 * ROW_SIZE
            var row2: int = j1 * ROW_SIZE
...
pn project(var u: float[], var v: float[], var p: float[], var dv: float[], iterations: int) any {
    var h: float = -0.5 / math.sqrt(float(WIDTH * HEIGHT))
    var j: int = 1
    while (j <= HEIGHT) {
        var row: int = j * ROW_SIZE
        var i: int = 1
        while (i <= WIDTH) {
            var idx: int = row + i
    ...
    var w_scale: float = 0.5 * float(WIDTH)
    var h_scale: float = 0.5 * float(HEIGHT)
    j = 1
    while (j <= HEIGHT) {
        var row: int = j * ROW_SIZE
        var i: int = 1
...
pn dens_step(var x: float[], var x0: float[], u: float[], v: float[], dt: float, iterations: int) any {
    add_fields(x, x0, dt)
    diffuse(0, x0, x, dt, iterations)
    advect(0, x, x0, u, v, dt)
}

pn vel_step(var u: float[], var v: float[], var u0: float[], var v0: float[], dt: float, iterations: int) any {
    add_fields(u, u0, dt)
    add_fields(v, v0, dt)
    // swap u,u0 and v,v0 by copying
    var i: int = 0
    while (i < GRID_SIZE) {
        var tmp_u: float = u0[i]
        u0[i] = u[i]
        u[i] = tmp_u
        var tmp_v: float = v0[i]
        v0[i] = v[i]
        v[i] = tmp_v
        i = i + 1
    }
...
pn add_points(var dens: float[], var u: float[], var v: float[]) any {
    var n: int = 64
    var i: int = 1
    while (i <= n) {
        var n_float: float = float(n)
        var idx1: int = (i + 1) + (i + 1) * ROW_SIZE
        u[idx1] = n_float
        v[idx1] = n_float
        dens[idx1] = 5.0

        var idx2: int = (i + 1) + (n - i + 1) * ROW_SIZE
        u[idx2] = 0.0 - n_float
        v[idx2] = 0.0 - n_float
        dens[idx2] = 20.0

        var idx3: int = (128 - i + 1) + (n + i + 1) * ROW_SIZE
        u[idx3] = 0.0 - n_float
        v[idx3] = 0.0 - n_float
        dens[idx3] = 30.0
...
pn run_navier_stokes() int {
    ...
    let dt: float = 0.1
    var dens: float[] = fill(GRID_SIZE, 0.0)
    var dens_prev: float[] = fill(GRID_SIZE, 0.0)
    var u: float[] = fill(GRID_SIZE, 0.0)
    var u_prev: float[] = fill(GRID_SIZE, 0.0)
    var v: float[] = fill(GRID_SIZE, 0.0)
    var v_prev: float[] = fill(GRID_SIZE, 0.0)

    var frames_till_add: int = 0
    var frames_between: int = 5
