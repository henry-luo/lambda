...
pn offset_momentum(var bvx: float[], var bvy: float[], var bvz: float[],
        bmass: float[]) any {
    var px: float = 0.0
    var py: float = 0.0
    var pz: float = 0.0
...
pn advance(var bx: float[], var by: float[], var bz: float[],
           var bvx: float[], var bvy: float[], var bvz: float[],
           bmass: float[], dt: float) any {
    var i: int = 0
    while (i < 5) {
        var j: int = i + 1
        while (j < 5) {
...
pn run_nbody_system(n_steps: int) float {
    ...
    let bmass: float[] = [SOLAR_MASS, 9.54791938424326609e-04 * SOLAR_MASS,
