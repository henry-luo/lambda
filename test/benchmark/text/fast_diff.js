...
// Text benchmark: fast-diff's Myers-style text diff and semantic cleanup.
// The workload uses source-like multiline edits and repeats each pair 256 times.
...
for (var fast_diff_round = 0; fast_diff_round < fast_diff_rounds; fast_diff_round++) {
  for (var fast_diff_index = 0; fast_diff_index < fast_diff_pairs.length; fast_diff_index++) {
    var fast_diff_result = fast_diff(
      fast_diff_pairs[fast_diff_index][0],
      fast_diff_pairs[fast_diff_index][1],
      null,
      true
    );
    fast_diff_checksum =
      (fast_diff_checksum + fast_diff_result.length * 17) % 1000000007;
    for (var fast_diff_part = 0; fast_diff_part < fast_diff_result.length; fast_diff_part++) {
      fast_diff_checksum =
        (fast_diff_checksum +
          fast_diff_result[fast_diff_part][0] * 31 +
          fast_diff_result[fast_diff_part][1].length) %
        1000000007;
    }
  }
}
