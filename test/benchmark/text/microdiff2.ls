// Typed text benchmark: microdiff's recursive object/array snapshot diff.
// Snapshot maps remain dynamic because the JavaScript library accepts mixed values.

fn make_remove(path: array, old_value: any) map =>
    {type: "REMOVE", path: path, oldValue: old_value}

fn make_create(path: array, value: any) map =>
    {type: "CREATE", path: path, value: value}

fn make_change(path: array, value: any, old_value: any) map =>
    {type: "CHANGE", path: path, value: value, oldValue: old_value}

pn is_rich_leaf(value: any) bool {
    if (value is map) {
...
pn rich_leaf_equal(old_value: any, fresh_value: any) bool {
...
pn both_nan(left: any, right: any) bool {
    return left is nan and right is nan
}

pn diff_array(old: array, fresh: array, prefix: array) array {
    var changes: array = []
    var index: int = 0
    while (index < len(old)) {
        let path: array = prefix ++ [index]
        if (index >= len(fresh)) {
...
pn diff_map(old: map, fresh: map, prefix: array) array {
    var changes: array = []
    for (key, old_value at old) {
        let path: array = prefix ++ [string(key)]
        if (key at fresh) {
...
pn diff_value(old_value: any, fresh_value: any, path: array) array {
...
fn rich_leaf(kind: string, value: any) map =>
    {__microdiff_rich_type: kind, value: value}

fn make_microdiff_snapshot(revised: bool) map => {
    document: {
...
pn valid_snapshot_diffs(pairs: array) bool {
