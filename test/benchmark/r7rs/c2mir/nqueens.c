...
static int *fill_zero(int n) {
    /* fill(n, 0); a zero-length request still yields a distinct block */
    return (int *) calloc((unsigned long) (n > 0 ? n : 1), sizeof(int));
}

static int ok(int row, int dist, int *placed, int placed_len) {
    if (dist > placed_len) return 1;
    {
        int p = placed[placed_len - dist];
...
static int solve(int *candidates, int cand_len, int *rest, int rest_len,
                 int *placed, int placed_len) {
    int row;
    int count = 0;
    int ci, ri, ni;
    int *new_rest;
    ...
    if (ok(row, 1, placed, placed_len) == 1) {
        int *new_cands = fill_zero(cand_len - 1 + rest_len);
        int *empty = fill_zero(1);
        ni = 0;
...
static int nqueens(int n) {
    int *candidates = fill_zero(n);
    int *placed = fill_zero(n);
    int *empty = fill_zero(1);
    int i;
    int result;
