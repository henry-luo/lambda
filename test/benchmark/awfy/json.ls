...
pn benchmark() {
    var parse_result = parse_json(RAP)
    // Parsing can fail; do not pass its error value into the JSON object helpers.
    var result = match parse_result { case error: null case map: parse_result }
    if (result == null) {
