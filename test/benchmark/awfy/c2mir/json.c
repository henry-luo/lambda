...
/* substring [start, end_excl) of s, malloc'd */
static char *j_substr(const char *s, int start, int end_excl) {
    int n = end_excl - start;
    char *r;
    int i;
    if (n < 0) n = 0;
    r = (char *) malloc((unsigned long) (n + 1));
    for (i = 0; i < n; i++) r[i] = s[start + i];
    r[n] = 0;
    return r;
}

static char *j_concat(const char *a, const char *b) {
    int la = j_strlen(a);
    int lb = j_strlen(b);
    char *r = (char *) malloc((unsigned long) (la + lb + 1));
    int i;
    for (i = 0; i < la; i++) r[i] = a[i];
    for (i = 0; i < lb; i++) r[la + i] = b[i];
    r[la + lb] = 0;
    return r;
}

/* --- Vector (growable, matches vec_new/vec_add/vec_at/vec_size) --- */
typedef struct {
    void **items;
    int size;
    int cap;
...
/* --- Hash Index Table: 32 slots keyed by len(name) % 32 --- */
static void hit_add(int tbl[32], const char *name, int index) {
    int slot = j_strlen(name) % 32;
    if (index < 255) tbl[slot] = (index + 1) % 256;
    else tbl[slot] = 0;
}

static int hit_get(int tbl[32], const char *name) {
    int slot = j_strlen(name) % 32;
    return tbl[slot] - 1;
}
 ...
 * jt=3: number (sv), jt=4: string (sv)
 * jt=5: array (vals), jt=6: object (names, vals, ht) */
typedef struct {
    int jt;
    char *sv;
    Vec *vals;
...
static JsonValue *jv_alloc(int jt) {
    ...
    v->vals = 0;
    v->names = 0;
    for (i = 0; i < 32; i++) v->ht[i] = 0;
    return v;
}

static JsonValue *jv_null(void) { return jv_alloc(0); }
static JsonValue *jv_true(void) { return jv_alloc(1); }
static JsonValue *jv_false(void) { return jv_alloc(2); }

static JsonValue *jv_number(char *s) {
    JsonValue *v = jv_alloc(3);
    v->sv = s;
    return v;
}

static JsonValue *jv_string(char *s) {
    JsonValue *v = jv_alloc(4);
    v->sv = s;
    return v;
}

static JsonValue *jv_array(void) {
    JsonValue *v = jv_alloc(5);
    v->vals = vec_new();
    return v;
}

static JsonValue *jv_object(void) {
    JsonValue *v = jv_alloc(6);
    v->names = vec_new();
    v->vals = vec_new();
    return v;
}

static void jv_arr_add(JsonValue *arr, JsonValue *item) { vec_add(arr->vals, item); }
static int jv_arr_size(JsonValue *arr) { return vec_size(arr->vals); }

static void jv_obj_add(JsonValue *obj, char *name, JsonValue *value) {
    int idx = vec_size(obj->names);
    hit_add(obj->ht, name, idx);
    vec_add(obj->names, name);
    vec_add(obj->vals, value);
}

static JsonValue *jv_obj_get(JsonValue *obj, const char *name) {
    int idx = hit_get(obj->ht, name);
    char *n;
    if (idx == -1) return 0;
    n = (char *) vec_at(obj->names, idx);
    if (j_streq(n, name)) return (JsonValue *) vec_at(obj->vals, idx);
    return 0;
}

static int jv_is_object(JsonValue *v) { return v->jt == 6 ? 1 : 0; }
static int jv_is_array(JsonValue *v) { return v->jt == 5 ? 1 : 0; }

/* --- Parser --- ('~' is the EOF sentinel, as in the .ls) */
typedef struct {
    const char *inp;
    int inplen;
    int idx;
    int ln;
    int col;
    int cs;      /* capture start */
    char *cb;    /* capture buffer */
    int cbf;     /* capture buffer flag */
    char cur;
} Parser;

static void p_init(Parser *p, const char *input) {
    p->inp = input;
    p->inplen = j_strlen(input);
    p->idx = -1;
    p->ln = 1;
    p->col = 0;
    p->cs = -1;
...
static void p_read(Parser *p) {
    ...
    idx = p->idx + 1;
    p->idx = idx;
    if (idx < p->inplen) p->cur = p->inp[idx];
    else p->cur = '~';
}

static int p_read_char(Parser *p, char ch) {
    if (p->cur != ch) return 0;
    p_read(p);
    return 1;
}

static void p_start_capture(Parser *p) { p->cs = p->idx; }

static void p_pause_capture(Parser *p) {
    int end = p->idx - 1;
    char *sub = j_substr(p->inp, p->cs, end + 1);
    if (p->cbf == 0) {
        p->cb = sub;
        p->cbf = 1;
    } else {
        char *nc = j_concat(p->cb, sub);
        free(p->cb);
        free(sub);
        p->cb = nc;
    }
...
static char *p_end_capture(Parser *p) {
    int end = p->idx;
    char *sub;
    char *captured;
    if (p->cur != '~') end = p->idx - 1;
    sub = j_substr(p->inp, p->cs, end + 1);
    captured = sub;
    if (p->cbf != 0) {
        captured = j_concat(p->cb, sub);
        free(p->cb);
        free(sub);
        p->cb = 0;
        p->cbf = 0;
...
static void p_skip_ws(Parser *p) {
    while (p_is_ws(p) == 1) p_read(p);
}

static int p_is_digit(Parser *p) {
    char c = p->cur;
    if (c >= '0' && c <= '9') return 1;
    return 0;
}

static int p_read_digit(Parser *p) {
    if (p_is_digit(p) == 0) return 0;
    p_read(p);
    return 1;
}

static JsonValue *p_read_null(Parser *p) {
    p_read(p); p_read(p); p_read(p); p_read(p);
    return jv_null();
}

static JsonValue *p_read_true(Parser *p) {
    p_read(p); p_read(p); p_read(p); p_read(p);
    return jv_true();
}

static JsonValue *p_read_false(Parser *p) {
    p_read(p); p_read(p); p_read(p); p_read(p); p_read(p);
    return jv_false();
}

/* append one decoded escape char to the capture buffer */
static void p_append_escape(Parser *p, char ch) {
    char one[2];
    one[0] = ch;
    one[1] = 0;
    if (p->cbf == 0) {
        p->cb = j_substr(one, 0, 1);
        p->cbf = 1;
    } else {
        char *nc = j_concat(p->cb, one);
        free(p->cb);
        p->cb = nc;
    }
}

static void p_read_escape(Parser *p) {
    char cur;
    p_read(p);
    cur = p->cur;
    if (cur == '"') p_append_escape(p, '"');
    if (cur == '/') p_append_escape(p, '/');
    if (cur == '\\') p_append_escape(p, '\\');
    if (cur == 'n') p_append_escape(p, '\n');
    if (cur == 'r') p_append_escape(p, '\r');
    if (cur == 't') p_append_escape(p, '\t');
    p_read(p);
}

static char *p_read_string_internal(Parser *p) {
    char *result;
    p_read(p);
    p_start_capture(p);
    while (p->cur != '"') {
    ...
    result = p_end_capture(p);
    p_read(p);
    return result;
}

static JsonValue *p_read_string(Parser *p) {
    return jv_string(p_read_string_internal(p));
}

static int p_read_fraction(Parser *p) {
    int cont;
    if (p_read_char(p, '.') == 0) return 0;
    p_read_digit(p);
    cont = p_read_digit(p);
    while (cont == 1) cont = p_read_digit(p);
    return 1;
}

static int p_read_exponent(Parser *p) {
    int cont;
    if (p_read_char(p, 'e') == 0) {
        if (p_read_char(p, 'E') == 0) return 0;
    }
    if (p_read_char(p, '+') == 0) p_read_char(p, '-');
    p_read_digit(p);
    cont = p_read_digit(p);
    while (cont == 1) cont = p_read_digit(p);
    return 1;
...
static JsonValue *p_read_number(Parser *p) {
    char cur;
    p_start_capture(p);
    p_read_char(p, '-');
    cur = p->cur;
    p_read_digit(p);
    if (cur != '0') {
        int more = p_read_digit(p);
        while (more == 1) more = p_read_digit(p);
    }
    p_read_fraction(p);
    p_read_exponent(p);
    return jv_number(p_end_capture(p));
}

static JsonValue *p_read_value(Parser *p);

static JsonValue *p_read_array(Parser *p) {
    JsonValue *arr;
    int cont;
    p_read(p);
    arr = jv_array();
    p_skip_ws(p);
    if (p_read_char(p, ']') == 1) return arr;
    cont = 1;
    while (cont == 1) {
        JsonValue *val;
        p_skip_ws(p);
...
static JsonValue *p_read_object(Parser *p) {
    JsonValue *obj;
    int cont;
    p_read(p);
    obj = jv_object();
    p_skip_ws(p);
    if (p_read_char(p, '}') == 1) return obj;
    cont = 1;
    while (cont == 1) {
        char *name;
        JsonValue *val;
        p_skip_ws(p);
        name = p_read_string_internal(p);
        p_skip_ws(p);
        p_read_char(p, ':');
        p_skip_ws(p);
        val = p_read_value(p);
...
static JsonValue *p_read_value(Parser *p) {
    char cur = p->cur;
    if (cur == 'n') return p_read_null(p);
    if (cur == 't') return p_read_true(p);
    if (cur == 'f') return p_read_false(p);
    if (cur == '"') return p_read_string(p);
    if (cur == '[') return p_read_array(p);
    if (cur == '{') return p_read_object(p);
    return p_read_number(p);
}

static JsonValue *parse_json(const char *input) {
    Parser p;
    p_init(&p, input);
    p_read(&p);
    p_skip_ws(&p);
...
static int benchmark(void) {
    JsonValue *result = parse_json(RAP);
    JsonValue *head;
    JsonValue *ops;
    int opsz;
    if (result == 0) {
    ...
    if (jv_is_object(result) != 1) {
        printf("FAIL: not object\n");
        return 0;
    }
    head = jv_obj_get(result, "head");
    if (head == 0) {
        printf("FAIL: no head\n");
        return 0;
    }
    if (jv_is_object(head) != 1) {
        printf("FAIL: head not object\n");
        return 0;
    }
    ops = jv_obj_get(result, "operations");
    if (ops == 0) {
        printf("FAIL: no operations\n");
        return 0;
    }
    if (jv_is_array(ops) != 1) {
        printf("FAIL: operations not array\n");
        return 0;
    }
    opsz = jv_arr_size(ops);
    if (opsz == 156) return 1;
    printf("FAIL: operations size=%d\n", opsz);
    return 0;
}

int main(void) {
    int result = benchmark();
    printf(result == 1 ? "Json: PASS\n" : "Json: FAIL\n");
    return result != 1;
}
