...
- Caches that exact release binary in `test/benchmark/exe/` before measurement. The
  cache name includes the Result number and commit and deliberately omits `.exe`,
  so broad test cleanup does not remove it. A pre-existing cache entry must be
  byte-identical; otherwise the run aborts rather than attaching a report to an
...
|---|---|---|
| A new comparable ResultN report | `run_standard_benchmarks.py` | AC/release/profile gates, exact-binary cache, fresh versioned JSON, logs, and report generation |
