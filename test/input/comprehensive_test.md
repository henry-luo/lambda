...
Here's another: `(-b ± sqrt(b^2-4ac))/2a` for quadratic formula.

#### Block Math (Simplified)
Block math expressions are supported but complex LaTeX expressions may cause parsing delays.

Simple block math example:
```
Mathematical content: a² + b² = c²
```
...
- Tables ✓
- Blockquotes ✓
- Mathematical blocks (code block syntax) ✓
- Horizontal rules ✓
