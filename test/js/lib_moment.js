// Moment.js 2.30.1 Library Support Tests
// Tests loading, core API availability, and basic functionality
// under the Lambda JS engine.
...
// === Moment.js 2.30.1 (minified, inlined) ===
...
function test(name, fn) {
  try {
    ...
    if (result === true) {
      ...
      fail++;
      errors.push("FAIL: " + name + " => " + JSON.stringify(result));
    }
  } catch(e) {
    fail++;
    errors.push("ERROR: " + name + " => " + (e.message || e));
  }
}
...
// === 1. Library Loading ===
test("loaded", function() { return typeof moment === "function"; });
test("version", function() { return moment.version === "2.30.1"; });

// === 2. Creation ===
test("moment-now", function() { return moment().isValid(); });
test("moment-string-iso", function() { return moment("2024-03-15").isValid(); });
test("moment-string-datetime", function() { return moment("2024-03-15T10:30:00").isValid(); });
test("moment-string-invalid", function() { return !moment("not-a-date").isValid(); });
test("moment-unix-ms", function() { return moment(1710496200000).isValid(); });
test("moment-array", function() { return moment([2024, 2, 15]).isValid(); });
test("moment-object", function() { return moment({year: 2024, month: 2, day: 15}).isValid(); });
test("moment-utc", function() {
  var m = moment.utc("2024-03-15T12:00:00Z");
  return m.isValid() && m.isUtc();
});
test("moment-unix", function() {
  var m = moment.unix(1710496200);
  return m.isValid();
});

// === 3. Formatting ===
test("format-YYYY-MM-DD", function() {
  return moment.utc(REF).format("YYYY-MM-DD") === "2024-06-15";
});
test("format-HH:mm:ss", function() {
  return moment.utc(REF).format("HH:mm:ss") === "14:30:45";
});
test("format-full", function() {
  return moment.utc(REF).format("YYYY-MM-DDTHH:mm:ss") === "2024-06-15T14:30:45";
});
test("format-year-2digit", function() {
  return moment.utc(REF).format("YY") === "24";
});
test("format-month-short", function() {
  return moment.utc(REF).format("MMM") === "Jun";
});
test("format-month-long", function() {
  return moment.utc(REF).format("MMMM") === "June";
});
test("format-day-short", function() {
  return moment.utc(REF).format("ddd") === "Sat";
});
test("format-day-long", function() {
  return moment.utc(REF).format("dddd") === "Saturday";
});
test("format-12h", function() {
  return moment.utc(REF).format("h:mm A") === "2:30 PM";
});
test("format-ordinal", function() {
  return moment.utc(REF).format("Do") === "15th";
});
test("format-quarter", function() {
  return moment.utc(REF).format("Q") === "2";
});
test("format-X-unix", function() {
  return moment.utc(REF).format("X") === "" + Math.floor(moment.utc(REF).valueOf() / 1000);
});

// === 4. Parsing ===
test("parse-iso-date", function() {
  var m = moment.utc("2024-03-15");
  return m.year() === 2024 && m.month() === 2 && m.date() === 15;
});
test("parse-iso-datetime", function() {
  var m = moment.utc("2024-03-15T10:30:00Z");
  return m.hour() === 10 && m.minute() === 30;
});
test("parse-iso-ms", function() {
  var m = moment.utc("2024-03-15T10:30:00.456Z");
  return m.millisecond() === 456;
});
test("parse-format-string", function() {
  var m = moment.utc("15/03/2024", "DD/MM/YYYY");
  return m.isValid() && m.year() === 2024 && m.month() === 2 && m.date() === 15;
});
test("parse-format-multiformat", function() {
  var m = moment.utc("03-15-2024", ["YYYY-MM-DD", "MM-DD-YYYY"]);
  return m.isValid() && m.month() === 2 && m.date() === 15;
});
test("parse-strict-valid", function() {
  return moment.utc("2024-03-15", "YYYY-MM-DD", true).isValid();
});
test("parse-strict-invalid", function() {
  return !moment.utc("2024-3-15", "YYYY-MM-DD", true).isValid();
});
test("parse-rfc2822", function() {
  var m = moment.utc("Fri, 15 Mar 2024 10:30:00 +0000");
  return m.isValid() && m.year() === 2024 && m.month() === 2 && m.date() === 15;
});

// === 5. Getters/Setters ===
test("getter-year", function() { return moment.utc(REF).year() === 2024; });
test("getter-month", function() { return moment.utc(REF).month() === 5; });
test("getter-date", function() { return moment.utc(REF).date() === 15; });
test("getter-day", function() { return moment.utc(REF).day() === 6; });
test("getter-hour", function() { return moment.utc(REF).hour() === 14; });
test("getter-minute", function() { return moment.utc(REF).minute() === 30; });
test("getter-second", function() { return moment.utc(REF).second() === 45; });
test("getter-millisecond", function() { return moment.utc(REF).millisecond() === 123; });
test("getter-dayOfYear", function() { return moment.utc("2024-02-01").dayOfYear() === 32; });
test("getter-weekday", function() { return typeof moment.utc(REF).weekday() === "number"; });
test("getter-isoWeekday", function() { return moment.utc(REF).isoWeekday() === 6; });
test("getter-isoWeek", function() { return typeof moment.utc(REF).isoWeek() === "number"; });
test("getter-quarter", function() { return moment.utc(REF).quarter() === 2; });
test("getter-weeksInYear", function() { return typeof moment.utc(REF).weeksInYear() === "number"; });

test("setter-year", function() {
  return moment.utc(REF).year(2025).year() === 2025;
});
test("setter-month", function() {
  return moment.utc(REF).month(0).month() === 0;
});
test("setter-date", function() {
  return moment.utc(REF).date(1).date() === 1;
});
test("setter-hour", function() {
  return moment.utc(REF).hour(8).hour() === 8;
});
test("setter-minute", function() {
  return moment.utc(REF).minute(0).minute() === 0;
});
test("setter-second", function() {
  return moment.utc(REF).second(0).second() === 0;
});
test("setter-millisecond", function() {
  return moment.utc(REF).millisecond(500).millisecond() === 500;
});

// === 6. Manipulation ===
test("add-days", function() {
  return moment.utc("2024-03-15").add(7, 'days').format("YYYY-MM-DD") === "2024-03-22";
});
test("add-months", function() {
  return moment.utc("2024-01-31").add(1, 'month').format("YYYY-MM-DD") === "2024-02-29";
});
test("add-years", function() {
  return moment.utc("2024-06-15").add(1, 'year').format("YYYY-MM-DD") === "2025-06-15";
});
test("add-hours", function() {
  return moment.utc("2024-06-15T10:00:00").add(5, 'hours').hour() === 15;
});
test("add-minutes", function() {
  return moment.utc("2024-06-15T10:30:00").add(45, 'minutes').minute() === 15;
});
test("subtract-days", function() {
  return moment.utc("2024-03-15").subtract(10, 'days').format("YYYY-MM-DD") === "2024-03-05";
});
test("subtract-months", function() {
  return moment.utc("2024-03-31").subtract(1, 'month').format("YYYY-MM-DD") === "2024-02-29";
});
test("subtract-years", function() {
  return moment.utc("2024-06-15").subtract(1, 'year').format("YYYY-MM-DD") === "2023-06-15";
});
test("startOf-year", function() {
  return moment.utc(REF).startOf('year').format("YYYY-MM-DD") === "2024-01-01";
});
test("startOf-month", function() {
  return moment.utc(REF).startOf('month').format("YYYY-MM-DD") === "2024-06-01";
});
test("startOf-day", function() {
  return moment.utc(REF).startOf('day').format("HH:mm:ss") === "00:00:00";
});
test("startOf-hour", function() {
  var m = moment.utc(REF).startOf('hour');
  return m.minute() === 0 && m.second() === 0;
});
test("endOf-month", function() {
  return moment.utc("2024-02-15").endOf('month').date() === 29;
});
test("endOf-day", function() {
  var m = moment.utc(REF).endOf('day');
  return m.hour() === 23 && m.minute() === 59 && m.second() === 59;
});
test("endOf-year", function() {
  return moment.utc(REF).endOf('year').format("YYYY-MM-DD") === "2024-12-31";
});

// === 7. Comparison ===
test("isBefore", function() {
  return moment.utc("2024-03-15").isBefore(moment.utc("2024-03-20"));
});
test("isAfter", function() {
  return moment.utc("2024-03-20").isAfter(moment.utc("2024-03-15"));
});
test("isSame-day", function() {
  return moment.utc("2024-03-15T10:00:00").isSame(moment.utc("2024-03-15T18:00:00"), 'day');
});
test("isSame-month", function() {
  return moment.utc("2024-03-15").isSame(moment.utc("2024-03-20"), 'month');
});
test("isBetween", function() {
  return moment.utc("2024-03-15").isBetween(moment.utc("2024-03-10"), moment.utc("2024-03-20"));
});
test("isSameOrBefore", function() {
  return moment.utc("2024-03-15").isSameOrBefore(moment.utc("2024-03-15"));
});
test("isSameOrAfter", function() {
  return moment.utc("2024-03-15").isSameOrAfter(moment.utc("2024-03-15"));
});
test("diff-days", function() {
  return moment.utc("2024-03-20").diff(moment.utc("2024-03-15"), 'days') === 5;
});
test("diff-months", function() {
  return moment.utc("2024-06-15").diff(moment.utc("2024-03-15"), 'months') === 3;
});
test("diff-seconds", function() {
  return moment.utc("2024-03-15T10:00:30").diff(moment.utc("2024-03-15T10:00:00"), 'seconds') === 30;
});

// === 8. Display ===
test("toISOString", function() {
  return moment.utc(REF).toISOString() === "2024-06-15T14:30:45.123Z";
});
test("toJSON", function() {
  return moment.utc(REF).toJSON() === "2024-06-15T14:30:45.123Z";
});
test("valueOf", function() {
  return typeof moment.utc(REF).valueOf() === "number";
});
test("unix", function() {
  return moment.utc(REF).unix() === Math.floor(moment.utc(REF).valueOf() / 1000);
});
test("toDate", function() {
  return moment.utc(REF).toDate() instanceof Date;
});
test("toArray", function() {
  var a = moment.utc(REF).toArray();
  return a[0] === 2024 && a[1] === 5 && a[2] === 15 && a[3] === 14 && a[4] === 30 && a[5] === 45;
});
test("toObject", function() {
  var o = moment.utc(REF).toObject();
  return o.years === 2024 && o.months === 5 && o.date === 15;
});
test("toString", function() {
  return typeof moment.utc(REF).toString() === "string";
});

// === 9. Query ===
test("isValid-true", function() { return moment("2024-03-15").isValid(); });
test("isValid-false", function() { return !moment("invalid").isValid(); });
test("isLeapYear-2024", function() { return moment([2024]).isLeapYear(); });
test("isLeapYear-2023", function() { return !moment([2023]).isLeapYear(); });
test("isMoment", function() { return moment.isMoment(moment()); });
test("isMoment-false", function() { return !moment.isMoment({}); });
test("isDate", function() { return moment.isDate(new Date()); });
test("isDate-false", function() { return !moment.isDate("2024-03-15"); });

// === 10. Duration ===
test("duration-create", function() {
  var d = moment.duration(2, 'hours');
  return moment.isDuration(d);
});
test("duration-hours", function() {
  return moment.duration(2, 'hours').hours() === 2;
});
test("duration-asMinutes", function() {
  return moment.duration(2, 'hours').asMinutes() === 120;
});
test("duration-asSeconds", function() {
  return moment.duration(1, 'minute').asSeconds() === 60;
});
test("duration-asDays", function() {
  return moment.duration(48, 'hours').asDays() === 2;
});
test("duration-complex", function() {
  var d = moment.duration({hours: 2, minutes: 30});
  return d.hours() === 2 && d.minutes() === 30;
});
test("duration-add", function() {
  var d = moment.duration(1, 'hour').add(30, 'minutes');
  return d.asMinutes() === 90;
});
test("duration-subtract", function() {
  var d = moment.duration(2, 'hours').subtract(30, 'minutes');
  return d.asMinutes() === 90;
});
test("duration-humanize", function() {
  var h = moment.duration(2, 'hours').humanize();
  return h === "2 hours";
});
test("duration-humanize-suffix", function() {
  var h = moment.duration(-2, 'hours').humanize(true);
  return h === "2 hours ago";
});
test("duration-iso", function() {
  return moment.duration(90, 'seconds').toISOString() === "PT1M30S";
});
test("duration-from-iso", function() {
  var d = moment.duration("PT2H30M");
  return d.hours() === 2 && d.minutes() === 30;
});
test("duration-days", function() {
  return moment.duration(3, 'days').days() === 3;
});
test("duration-months", function() {
  return moment.duration(2, 'months').months() === 2;
});
test("duration-years", function() {
  return moment.duration(1, 'year').years() === 1;
});
test("duration-clone", function() {
  var d1 = moment.duration(5, 'hours');
  var d2 = d1.clone();
  d2.add(1, 'hour');
  return d1.hours() === 5 && d2.hours() === 6;
});

// === 11. Clone ===
test("clone-independent", function() {
  var orig = moment.utc("2024-03-15");
  var cloned = orig.clone();
  cloned.add(1, 'day');
  return orig.date() === 15 && cloned.date() === 16;
});

// === 12. Chaining ===
test("chain-add-subtract", function() {
  return moment.utc("2024-03-15").add(1, 'month').subtract(2, 'days').format("YYYY-MM-DD") === "2024-04-13";
});
test("chain-startOf-format", function() {
  return moment.utc(REF).startOf('month').format("YYYY-MM-DD") === "2024-06-01";
});

// === 13. Min / Max ===
test("moment-min", function() {
  var a = moment.utc("2024-03-15");
  var b = moment.utc("2024-06-15");
  return moment.min(a, b).format("YYYY-MM-DD") === "2024-03-15";
});
test("moment-max", function() {
  var a = moment.utc("2024-03-15");
  var b = moment.utc("2024-06-15");
  return moment.max(a, b).format("YYYY-MM-DD") === "2024-06-15";
});

// === 14. Locale ===
test("locale-default", function() {
  return moment.locale() === "en";
});
test("locale-months", function() {
  var months = moment.months();
  return months.length === 12 && months[0] === "January" && months[11] === "December";
});
test("locale-weekdays", function() {
  var days = moment.weekdays();
  return days.length === 7 && days[0] === "Sunday" && days[6] === "Saturday";
});
test("locale-monthsShort", function() {
  var ms = moment.monthsShort();
  return ms.length === 12 && ms[0] === "Jan" && ms[11] === "Dec";
});

// === 15. UTC / Offset ===
test("utc-isUtc", function() {
  return moment.utc().isUtc();
});
test("utc-local", function() {
  var m = moment.utc(REF).local();
  return !m.isUtc() && m.isValid();
});
test("utcOffset-get", function() {
  return typeof moment.utc(REF).utcOffset() === "number";
});
test("utcOffset-zero-utc", function() {
  return moment.utc(REF).utcOffset() === 0;
});

// === 16. Edge Cases ===
test("invalid-date-format", function() {
  return moment("invalid").format() === "Invalid date";
});
test("invalid-date-valueOf", function() {
  return isNaN(moment("invalid").valueOf());
});
test("epoch", function() {
  return moment.utc(0).format("YYYY-MM-DD") === "1970-01-01";
});
test("leap-day", function() {
  return moment.utc("2024-02-29").isValid() && moment.utc("2024-02-29").date() === 29;
});
test("non-leap-feb29", function() {
  return !moment.utc("2023-02-29", "YYYY-MM-DD", true).isValid();
});
test("daysInMonth-feb-leap", function() {
  return moment.utc("2024-02-01").daysInMonth() === 29;
});
test("daysInMonth-feb-nonleap", function() {
  return moment.utc("2023-02-01").daysInMonth() === 28;
});
test("daysInMonth-jan", function() {
  return moment.utc("2024-01-01").daysInMonth() === 31;
});
test("daysInMonth-apr", function() {
  return moment.utc("2024-04-01").daysInMonth() === 30;
});
...
console.log("MOMENT_LOAD_OK");
console.log(pass + "/" + (pass + fail) + " tests passed");
console.log("MOMENT_DONE");
