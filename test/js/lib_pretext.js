// Pretext.js test suite for Lambda JS engine
// Library: @chenglou/pretext 0.0.5 — text measurement & layout
// Mocks: Intl.Segmenter (word/grapheme), OffscreenCanvas (text.length*8 width)
...
// Mock OffscreenCanvas — each char = 8px width
...
var pretext = (() => {
  ...
  // temp/pretext_pkg/package/dist/layout.js
  var layout_exports = {};
  ...
  // temp/pretext_pkg/package/dist/generated/bidi-data.js
  var latin1BidiTypes = [
    "BN",
    "BN",
    "BN",
  ...
  // temp/pretext_pkg/package/dist/bidi.js
  ...
  // temp/pretext_pkg/package/dist/analysis.js
  ...
  // temp/pretext_pkg/package/dist/measurement.js
  ...
  // temp/pretext_pkg/package/dist/line-break.js
...
function assert(cond, msg) {
  if (cond) { _passed++; }
  else { _failed++; _errors.push(msg); console.log("FAIL: " + msg); }
}
function assertEq(a, b, msg) { assert(a === b, msg + " (got " + a + ", expected " + b + ")"); }
function assertType(v, t, msg) { assert(typeof v === t, msg + " (got " + typeof v + ")"); }
...
// 1. Module exports
// ============================================================
assertType(pretext, "object", "1.1 pretext is object");
assertType(pretext.prepare, "function", "1.2 prepare is function");
assertType(pretext.prepareWithSegments, "function", "1.3 prepareWithSegments is function");
assertType(pretext.layout, "function", "1.4 layout is function");
assertType(pretext.layoutWithLines, "function", "1.5 layoutWithLines is function");
assertType(pretext.walkLineRanges, "function", "1.6 walkLineRanges is function");
assertType(pretext.measureLineStats, "function", "1.7 measureLineStats is function");
assertType(pretext.measureNaturalWidth, "function", "1.8 measureNaturalWidth is function");
assertType(pretext.clearCache, "function", "1.9 clearCache is function");
assertType(pretext.setLocale, "function", "1.10 setLocale is function");
assertType(pretext.layoutNextLine, "function", "1.11 layoutNextLine is function");
assertType(pretext.layoutNextLineRange, "function", "1.12 layoutNextLineRange is function");
assertType(pretext.materializeLineRange, "function", "1.13 materializeLineRange is function");

// ============================================================
// 2. prepare() basic
// ============================================================
var p1 = pretext.prepare("Hello World", "16px monospace");
assertType(p1, "object", "2.1 prepare returns object");
assert(p1 !== null, "2.2 prepare not null");

// prepared object should have widths array
assert(Array.isArray(p1.widths), "2.3 prepared.widths is array");
assertGt(p1.widths.length, 0, "2.4 widths non-empty");

// prepared should have kinds array
assert(Array.isArray(p1.kinds), "2.5 prepared.kinds is array");
assertEq(p1.widths.length, p1.kinds.length, "2.6 widths/kinds same length");

// ============================================================
// 3. prepare() empty string
// ============================================================
var p_empty = pretext.prepare("", "16px monospace");
assertType(p_empty, "object", "3.1 prepare empty returns object");
assertEq(p_empty.widths.length, 0, "3.2 empty widths");

// ============================================================
// 4. prepareWithSegments()
// ============================================================
var ps1 = pretext.prepareWithSegments("Hello World", "16px monospace");
assertType(ps1, "object", "4.1 prepareWithSegments returns object");
assert(Array.isArray(ps1.segments), "4.2 has segments array");
assertGt(ps1.segments.length, 0, "4.3 segments non-empty");
assert(Array.isArray(ps1.widths), "4.4 has widths");

// ============================================================
// 5. layout() single line
// ============================================================
var lay1 = pretext.layout(p1, 1000, 20);
assertType(lay1, "object", "5.1 layout returns object");
assertEq(lay1.lineCount, 1, "5.2 single line at wide width");
assertEq(lay1.height, 20, "5.3 height = lineCount * lineHeight");

// ============================================================
// 6. layout() word wrap
// ============================================================
var p2 = pretext.prepare("The quick brown fox jumps over the lazy dog", "16px monospace");
var lay2 = pretext.layout(p2, 100, 20);
assertGt(lay2.lineCount, 1, "6.1 wraps at narrow width");
assertEq(lay2.height, lay2.lineCount * 20, "6.2 height consistency");

// ============================================================
// 7. layoutWithLines()
// ============================================================
var ps2 = pretext.prepareWithSegments("Hello World", "16px monospace");
var lwl = pretext.layoutWithLines(ps2, 1000, 20);
assertType(lwl, "object", "7.1 layoutWithLines returns object");
assertEq(lwl.lineCount, 1, "7.2 single line");
assert(Array.isArray(lwl.lines), "7.3 has lines array");
assertEq(lwl.lines.length, 1, "7.4 one line object");
assertType(lwl.lines[0].text, "string", "7.5 line has text");
assertType(lwl.lines[0].width, "number", "7.6 line has width");
assertGt(lwl.lines[0].width, 0, "7.7 line width > 0");

// ============================================================
// 8. layoutWithLines() wrapping
// ============================================================
...
// ============================================================
var ranges = [];
var wlr_count = pretext.walkLineRanges(p2, 100, function(line) {
  ranges.push(line);
});
assertType(wlr_count, "number", "9.1 walkLineRanges returns number");
assertGt(wlr_count, 1, "9.2 multiple ranges");
assertEq(ranges.length, wlr_count, "9.3 callback count matches");
assertType(ranges[0].width, "number", "9.4 range has width");
assertType(ranges[0].start, "object", "9.5 range has start");
assertType(ranges[0].end, "object", "9.6 range has end");

// ============================================================
// 10. measureLineStats()
// ============================================================
var stats = pretext.measureLineStats(p2, 100);
assertType(stats, "object", "10.1 measureLineStats returns object");
assertType(stats.lineCount, "number", "10.2 has lineCount");
assertType(stats.maxLineWidth, "number", "10.3 has maxLineWidth");
assertGt(stats.lineCount, 1, "10.4 multiple lines");
assertGt(stats.maxLineWidth, 0, "10.5 maxLineWidth > 0");

// ============================================================
// 11. measureNaturalWidth()
// ============================================================
var nw = pretext.measureNaturalWidth(p1);
assertType(nw, "number", "11.1 measureNaturalWidth returns number");
assertGt(nw, 0, "11.2 natural width > 0");

// "Hello World" = 11 chars, each char = 8px via mock
// Natural width depends on segmentation, but should be > 0

// ============================================================
// 12. layoutNextLine()
// ============================================================
var ps4 = pretext.prepareWithSegments("Alpha Beta Gamma Delta", "16px monospace");
var cursor = { segmentIndex: 0, graphemeIndex: 0 };
var line1 = pretext.layoutNextLine(ps4, cursor, 80);
assertType(line1, "object", "12.1 layoutNextLine returns object");
assert(line1 !== null, "12.2 first line not null");
assertType(line1.text, "string", "12.3 line has text");
assertType(line1.width, "number", "12.4 line has width");
assertType(line1.end, "object", "12.5 line has end cursor");

// Get next line
var line2 = pretext.layoutNextLine(ps4, line1.end, 80);
assertType(line2, "object", "12.6 second line exists");
assert(line2 !== null, "12.7 second line not null");

// ============================================================
// 13. layoutNextLineRange()
// ============================================================
var p5 = pretext.prepare("Foo Bar Baz Qux", "16px monospace");
var cursor2 = { segmentIndex: 0, graphemeIndex: 0 };
var lr1 = pretext.layoutNextLineRange(p5, cursor2, 60);
assertType(lr1, "object", "13.1 layoutNextLineRange returns object");
assert(lr1 !== null, "13.2 not null");
assertType(lr1.width, "number", "13.3 has width");
assertType(lr1.start, "object", "13.4 has start");
assertType(lr1.end, "object", "13.5 has end");

// ============================================================
// 14. materializeLineRange()
// ============================================================
var ps5 = pretext.prepareWithSegments("Foo Bar Baz Qux", "16px monospace");
var mranges = [];
pretext.walkLineRanges(ps5, 60, function(line) { mranges.push(line); });
if (mranges.length > 0) {
  var mline = pretext.materializeLineRange(ps5, mranges[0]);
  assertType(mline, "object", "14.1 materializeLineRange returns object");
  assertType(mline.text, "string", "14.2 has text");
  assertGt(mline.text.length, 0, "14.3 text non-empty");
} else {
  assert(false, "14.0 no ranges to materialize");
}
...
try {
  ...
  _failed++; _errors.push("15.1 clearCache threw: " + e.message);
  console.log("FAIL: 15.1 clearCache threw: " + e.message);
}

// After clearCache, prepare should still work
var p_after = pretext.prepare("test", "16px monospace");
assertType(p_after, "object", "15.2 prepare works after clearCache");
...
try {
  ...
  _failed++; _errors.push("16.1 setLocale threw: " + e.message);
  console.log("FAIL: 16.1 setLocale threw: " + e.message);
}

// ============================================================
// 17. layout consistency
// ============================================================
var p6 = pretext.prepare("Hello World", "16px monospace");
var r1 = pretext.layout(p6, 1000, 20);
var r2 = pretext.layout(p6, 1000, 20);
assertEq(r1.lineCount, r2.lineCount, "17.1 layout deterministic lineCount");
assertEq(r1.height, r2.height, "17.2 layout deterministic height");

// ============================================================
// 18. Different maxWidth values
// ============================================================
var p7 = pretext.prepare("A B C D E F G H I J K L M", "16px monospace");
var wide = pretext.layout(p7, 10000, 20);
var narrow = pretext.layout(p7, 30, 20);
assertGte(narrow.lineCount, wide.lineCount, "18.1 narrow >= wide lineCount");

// ============================================================
// 19. Long text
// ============================================================
var longText = "";
for (var i = 0; i < 50; i++) longText += "word" + i + " ";
var pLong = pretext.prepare(longText.trim(), "16px monospace");
assertType(pLong, "object", "19.1 long text prepare");
var layLong = pretext.layout(pLong, 200, 20);
assertGt(layLong.lineCount, 1, "19.2 long text wraps");

// ============================================================
// 20. Single character
// ============================================================
var pChar = pretext.prepare("X", "16px monospace");
assertType(pChar, "object", "20.1 single char prepare");
var layChar = pretext.layout(pChar, 1000, 20);
assertEq(layChar.lineCount, 1, "20.2 single char = 1 line");

// ============================================================
// 21. Whitespace handling
// ============================================================
var pSpace = pretext.prepare("   ", "16px monospace");
assertType(pSpace, "object", "21.1 spaces prepare");

var pTab = pretext.prepare("\t", "16px monospace");
assertType(pTab, "object", "21.2 tab prepare");

// ============================================================
// 22. measureNaturalWidth consistency
// ============================================================
var nw1 = pretext.measureNaturalWidth(p1);
var nw2 = pretext.measureNaturalWidth(p1);
assertEq(nw1, nw2, "22.1 naturalWidth deterministic");

// ============================================================
// 23. layoutWithLines text reconstruction
// ============================================================
var ps_recon = pretext.prepareWithSegments("abc def ghi", "16px monospace");
var lwl_recon = pretext.layoutWithLines(ps_recon, 1000, 20);
assertEq(lwl_recon.lines.length, 1, "23.1 single line");
// reconstructed text should contain original chars
var recon_text = lwl_recon.lines[0].text;
assert(recon_text.indexOf("abc") >= 0, "23.2 contains abc");
assert(recon_text.indexOf("def") >= 0, "23.3 contains def");
assert(recon_text.indexOf("ghi") >= 0, "23.4 contains ghi");

// ============================================================
// 24. pre-wrap whiteSpace option
// ============================================================
var p_pre = pretext.prepare("line one\nline two", "16px monospace", { whiteSpace: "pre-wrap" });
assertType(p_pre, "object", "24.1 pre-wrap prepare");

// ============================================================
// 25. wordBreak option
// ============================================================
var p_brk = pretext.prepare("Supercalifragilisticexpialidocious", "16px monospace", { wordBreak: "break-all" });
assertType(p_brk, "object", "25.1 break-all prepare");
var lay_brk = pretext.layout(p_brk, 80, 20);
assertGt(lay_brk.lineCount, 1, "25.2 break-all wraps long word");

// ============================================================
// Summary
// ============================================================
console.log("");
console.log("Pretext tests: " + _passed + " passed, " + _failed + " failed out of " + (_passed + _failed));
if (_errors.length > 0) {
  for (var ei = 0; ei < _errors.length; ei++) console.log("  - " + _errors[ei]);
}
