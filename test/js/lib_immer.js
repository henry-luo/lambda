// immer 10.0.4 Library Support Tests
// Tests Proxy-based draft state, structural sharing, freeze semantics.
// CJS bundle: we shim `module`/`exports` and extract from module.exports.
...
const immer = module.exports;
const { produce, isDraft, isDraftable, freeze, original, current, enableMapSet, setAutoFreeze } = immer;

// === Test 1: immer module exposed ===
console.log(typeof immer);
console.log(typeof produce);
console.log(typeof isDraft);

// === Test 2: produce — simple object mutation ===
const base1 = { count: 0, name: 'a' };
const next1 = produce(base1, draft => { draft.count = 5; });
console.log(next1.count);
console.log(next1.name);
console.log(base1.count);             // unchanged
console.log(next1 !== base1);

// === Test 3: produce — nested mutation, structural sharing ===
const base2 = { user: { name: 'x', age: 10 }, items: [1, 2, 3] };
const next2 = produce(base2, draft => { draft.user.age = 20; });
console.log(next2.user.age);
console.log(base2.user.age);
console.log(next2.items === base2.items);   // unchanged subtree shared

// === Test 4: produce — array push ===
const next3 = produce([1, 2, 3], draft => { draft.push(4); });
console.log(JSON.stringify(next3));

// === Test 5: produce — array splice ===
const next4 = produce([10, 20, 30, 40], draft => { draft.splice(1, 2); });
console.log(JSON.stringify(next4));

// === Test 6: produce — return new state directly ===
const next5 = produce({ a: 1 }, () => ({ b: 2 }));
console.log(JSON.stringify(next5));

// === Test 7: produce — no-change short-circuit returns same reference ===
const base3 = { x: 1 };
const next6 = produce(base3, () => {});
console.log(next6 === base3);

// === Test 8: isDraft on draft vs plain object ===
let inside = false;
produce({ a: 1 }, draft => { inside = isDraft(draft); });
console.log(inside);
console.log(isDraft({ a: 1 }));

// === Test 9: isDraftable on various values ===
console.log(isDraftable({}));
console.log(isDraftable([]));
console.log(isDraftable(null));
console.log(isDraftable(42));

// === Test 10: freeze — top-level ===
const frozen = freeze({ a: 1 });
console.log(Object.isFrozen(frozen));

// === Test 11: produce — curried form (recipe first) ===
const incr = produce(draft => { draft.n++; });
console.log(incr({ n: 5 }).n);

// === Test 12: produce — original() inside recipe ===
const base4 = { v: 7 };
let origVal;
produce(base4, draft => { draft.v = 99; origVal = original(draft).v; });
console.log(origVal);

// === Test 13: produce — current() inside recipe ===
let curVal;
produce({ v: 1 }, draft => { draft.v = 2; curVal = current(draft).v; });
console.log(curVal);

// === Test 14: enableMapSet + producing Maps ===
enableMapSet();
const m = new Map([['k', 1]]);
const m2 = produce(m, d => { d.set('k', 100); d.set('j', 2); });
console.log(m2.get('k'));
console.log(m2.get('j'));
console.log(m.get('k'));            // unchanged

// === Test 15: producing Sets ===
const s = new Set([1, 2, 3]);
const s2 = produce(s, d => { d.add(4); d.delete(1); });
console.log(s2.has(4));
console.log(s2.has(1));
console.log(s.has(1));

console.log("IMMER_DONE");
