...
TEST_CASE("StrBuf creation and initialization", "[strbuf][basic]") {
    SECTION("test_new") {
...
TEST_CASE("StrBuf string operations", "[strbuf][append]") {
    SECTION("test_append_str") {
        StrBuf* sb = strbuf_new();
...
TEST_CASE("StrBuf character operations", "[strbuf][char]") {
    SECTION("test_append_char") {
        StrBuf* sb = strbuf_new();
...
TEST_CASE("StrBuf format operations", "[strbuf][format]") {
    SECTION("test_append_format") {
        StrBuf* sb = strbuf_new();
    ...
    SECTION("test_vappend_and_format_functions") {
        StrBuf* sb = strbuf_new();
...
TEST_CASE("StrBuf copy and duplicate operations", "[strbuf][copy]") {
    SECTION("test_copy_and_dup") {
    ...
    SECTION("test_copy_with_reallocation") {
        ...
        REQUIRE(dst->length == src->length);
        REQUIRE(dst->capacity > initial_capacity);
...
TEST_CASE("StrBuf file operations", "[strbuf][file]") {
    SECTION("test_append_file") {
...
TEST_CASE("StrBuf memory reallocation", "[strbuf][memory]") {
    SECTION("test_memory_reallocation") {
    ...
    SECTION("test_char_append_reallocation") {
    ...
    SECTION("test_char_n_append_reallocation") {
...
TEST_CASE("StrBuf edge cases and boundary conditions", "[strbuf][edge]") {
    ...
    SECTION("test_boundary_conditions") {
...
TEST_CASE("StrBuf capacity management", "[strbuf][capacity]") {
    SECTION("test_capacity_management") {
    ...
    SECTION("test_ensure_cap_edge_cases") {
        StrBuf* sb = strbuf_new();
...
TEST_CASE("StrBuf integer operations", "[strbuf][integer]") {
    SECTION("test_append_integer_functions") {
        StrBuf* sb = strbuf_new();
...
TEST_CASE("StrBuf stress testing", "[strbuf][stress]") {
    SECTION("test_stress_scenarios") {
        StrBuf* sb = strbuf_new_cap(8);
    ...
    SECTION("test_reallocation_pattern_verification") {
        StrBuf* sb = strbuf_new_cap(4);
        size_t prev_capacity = sb->capacity;
    ...
    SECTION("test_memory_reallocation_preservation") {
        StrBuf* sb = strbuf_new_cap(8);
...
TEST_CASE("StrBuf memory management", "[strbuf][memory_free]") {
    SECTION("test_full_reset") {
    ...
    SECTION("test_free_regular_memory") {
    ...
    SECTION("test_free_empty_regular_memory") {
    ...
    SECTION("test_free_after_full_reset") {
