// Shared adapter for the Chromium editing test suite — pure conversion logic
// (no editor / DOM dependency). Used by both tools/harvest-chromium.ts
// (scenario harvest) and tools/divergence-report.ts (conformance gauge).
//
// Parses Chromium `assert_selection(input, tester, expected)` tests, converts
// the input markers + operation sequence into our fixture vocabulary.

import fs from 'node:fs'
import path from 'node:path'

export type Op = { command: string; args?: Record<string, unknown> }
export interface AssertCall { input: string; tester: string; expected: string; name: string }

// ---------------------------------------------------------------------------
// execCommand → our event mapping
// ---------------------------------------------------------------------------

export function mapExec(name: string, arg: string | null): Op | null {
  ...
  switch (n) {
    ...
    case 'inserthtml':        return arg !== null ? { command: 'insertHTML', args: { html: arg } } : null
    default:                  return null  // unsupported → caller skips the whole test
  }
}

// ---------------------------------------------------------------------------
// Tester body → operation list
// ---------------------------------------------------------------------------

const UNSUPPORTED_SIGNAL = /execCommand\(\s*['"](copy|cut|undo|redo|insertimage|removeformat|fontsize|fontname|justify|inserthorizontalrule|stylewithcss|defaultparagraph)/i
const CLIPBOARD_EVENT = /\.clipboardData|addEventListener\(\s*['"]paste|onpaste\s*=/
...
export function parseTester(body: string): Op[] | null {
  ...
  if (UNSUPPORTED_SIGNAL.test(body)) return skip('copy/undo/unsupported-exec')
  if (CLIPBOARD_EVENT.test(body)) return skip('clipboard-event-handler')
  ...
  for (const l of loops) {
    if (l.n > 50) return skip('loop-too-large')
  ...
  // Linear scan: execCommand(...), .modify(...) and setClipboardData(...) in order.
  const callRe = /(?:selection\.)?document\.execCommand\(|selection\.modify\(|setClipboardData\(/g
  const ops: Op[] = []
  let clipboard: string | null = null
  callRe.lastIndex = 0
  guard = 0
  while ((m = callRe.exec(expanded)) !== null && guard++ < 5000) {
    const kind = m[0]!.includes('modify') ? 'modify' : m[0]!.includes('setClipboardData') ? 'clipboard' : 'exec'
    ...
    if (kind === 'clipboard') {
      clipboard = evalStringExpr(splitTopLevel(argsText)[0] ?? '')
      if (clipboard === null) return skip('clipboard-nonstatic')
      continue
    }
    ...
    if (kind === 'modify') {
      ...
      if (name.toLowerCase() === 'paste') {
        if (clipboard === null) return skip('paste-no-clipboard')
        ops.push({ command: 'paste', args: { html: clipboard } })
        continue
      }
...
export function convertInput(html: string): string | null {
...
// Strip tags + selection markers + decode the few entities the corpus uses,
// then collapse whitespace — the "text-content level" of a Chromium expected.
export function expectedPlainText(html: string): string {
  const m = html.match(/<div[^>]*contenteditable[^>]*>([\s\S]*)<\/div>\s*$/i)
  let s = m ? m[1]! : html
  s = s.replace(/[|^]/g, '')                    // selection markers
  s = s.replace(/<br[^>]*>/gi, '\n')            // breaks → newlines
...
export function extractAsserts(src: string): AssertCall[] {
...
// Substring inside the parens starting at `start` (just past '('), to the
// matching ')'. Respects strings and nested brackets.
...
// or a `'a' + 'b'` concatenation of literals.
...
export function walk(dir: string, acc: string[]): void {
