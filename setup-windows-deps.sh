...
if [ "$1" = "clean" ] || [ "$1" = "--clean" ]; then
    ...
    if [ -d "$DEPS_DIR/src" ]; then
        ...
        # Clean up zlog-specific build files
        if [ -d "$DEPS_DIR/src/zlog" ]; then
            cd "$DEPS_DIR/src/zlog"
            rm -rf build-windows CMakeCache.txt CMakeFiles/ cmake_install.cmake 2>/dev/null || true
            find . -name "*.cmake" -type f ! -path "./cmake/*" -delete 2>/dev/null || true
            make clean 2>/dev/null || true
...
# Function to build zlog for Windows
build_zlog_for_windows() {
    echo "Building zlog for Windows..."
    
    # Ensure zlog source is available
    if [ ! -d "$DEPS_DIR/src/zlog" ]; then
        cd "$DEPS_DIR/src"
    ...
    # zlog-specific cross-compilation flags for Windows
    export CFLAGS="-O2 -static -DWIN32 -D_WIN32 -DWINDOWS -D_GNU_SOURCE -fPIC"
    export CXXFLAGS="-O2 -static -DWIN32 -D_WIN32 -DWINDOWS -D_GNU_SOURCE -fPIC"
    export LDFLAGS="-static"
    
    echo "Building zlog with cross-compiler..."
    
    # Try different build approaches in order of preference
    local PREFIX_PATH="$(pwd)/../../../$DEPS_DIR"
    local BUILD_SUCCESS=0
    
    # Method 1: Try CMake if available
...
set(CMAKE_C_FLAGS "-static -O2 -DNDEBUG -D_GNU_SOURCE -DWIN32")
set(CMAKE_CXX_FLAGS "-static -O2 -DNDEBUG -D_GNU_SOURCE -DWIN32")
set(CMAKE_EXE_LINKER_FLAGS "-static")
set(CMAKE_SHARED_LINKER_FLAGS "-static")
set(CMAKE_MODULE_LINKER_FLAGS "-static")
EOF
        ...
        echo "Configuring zlog with CMake..."
        
        # Configure with CMake, disable problematic dependencies
        if cmake .. \
            -DCMAKE_TOOLCHAIN_FILE=../cmake/mingw-w64-x86_64.cmake \
            -DCMAKE_BUILD_TYPE=Release \
            ...
            -DZLOG_ENABLE_EXAMPLES=OFF \
            -DZLOG_ENABLE_TESTS=OFF; then
            ...
            NPROC=$(nproc 2>/dev/null || sysctl -n hw.ncpu 2>/dev/null || echo 4)
            if make -j"$NPROC" VERBOSE=0; then
                echo "zlog CMake build successful, installing..."
                
                # Try installing
                if make install; then
                    echo "zlog installed successfully via CMake"
                    BUILD_SUCCESS=1
                else
                    echo "CMake install failed, copying manually..."
                    ...
                    if [ -d "../src" ]; then
                        mkdir -p "../../../../$DEPS_DIR/include"
                        cp ../src/zlog.h "../../../../$DEPS_DIR/include/" 2>/dev/null && echo "Copied zlog.h"
                        BUILD_SUCCESS=1
                    fi
                fi
            else
                echo "Warning: zlog CMake build failed"
            fi
        else
            echo "Warning: CMake configuration failed for zlog"
        fi
        
        cd .. # Back to zlog root
    fi
    
    # Method 2: Try traditional make if CMake failed and Makefile exists
    if [ "$BUILD_SUCCESS" -eq 0 ] && [ -f "Makefile" ]; then
        echo "Attempting traditional make build..."
        
        # Try different make approaches
        echo "Trying make with PREFIX..."
        if make PREFIX="$PREFIX_PATH" \
               CC="$CC" \
               AR="$AR" \
               CFLAGS="$CFLAGS" \
               static 2>/dev/null; then
            
            echo "zlog static build successful, installing..."
            
            # Install libraries and headers
            if make PREFIX="$PREFIX_PATH" install 2>/dev/null; then
                echo "zlog installed successfully via make"
                BUILD_SUCCESS=1
            else
                echo "Make install failed, copying manually..."
                
                # Manual copy of libraries
                find . -name "*.a" -exec cp {} "../../../$DEPS_DIR/lib/" \; 2>/dev/null && echo "Copied static libraries"
                find . -name "libzlog.a" -exec cp {} "../../../$DEPS_DIR/lib/" \; 2>/dev/null && echo "Copied libzlog.a"
            ...
            echo "Standard make failed, trying simplified make..."
            
            # Try simplified make without static target
            if make CC="$CC" AR="$AR" CFLAGS="$CFLAGS" 2>/dev/null; then
                echo "Simplified make successful, copying outputs..."
                
                # Manual copy of any produced libraries
                find . -name "*.a" -exec cp {} "../../../$DEPS_DIR/lib/" \; 2>/dev/null && echo "Copied libraries"
                
                # Copy headers
                mkdir -p "../../../$DEPS_DIR/include"
                if [ -f "src/zlog.h" ]; then
                    cp src/zlog.h "../../../$DEPS_DIR/include/" && echo "Copied zlog.h"
                    BUILD_SUCCESS=1
                fi
            else
                echo "Warning: All make approaches failed"
            fi
        fi
    fi
    
    # Method 3: Manual compilation if standard methods fail
    if [ "$BUILD_SUCCESS" -eq 0 ] && [ -d "src" ]; then
        echo "Standard builds failed, attempting manual compilation..."
        
        # Find all C source files in src directory
        ZLOG_SOURCES=$(find src -name "*.c" 2>/dev/null | grep -v test | tr '\n' ' ')
        
        if [ -n "$ZLOG_SOURCES" ]; then
            echo "Compiling zlog sources manually: $ZLOG_SOURCES"
            
            # Compile all source files to object files
            ZLOG_OBJECTS=""
            local COMPILE_SUCCESS=1
            
            for src in $ZLOG_SOURCES; do
                obj="${src%.c}.o"
                echo "Compiling $src..."
                if $CC $CFLAGS -I. -c "$src" -o "$obj"; then
                    ZLOG_OBJECTS="$ZLOG_OBJECTS $obj"
                else
            ...
            # Create static library if we have objects
            if [ -n "$ZLOG_OBJECTS" ] && [ "$COMPILE_SUCCESS" -eq 1 ]; then
                echo "Creating libzlog.a from objects: $ZLOG_OBJECTS"
                if $AR rcs libzlog.a $ZLOG_OBJECTS; then
                    # Copy library and headers
                    cp libzlog.a "../../../$DEPS_DIR/lib/" && echo "Copied libzlog.a"
                    mkdir -p "../../../$DEPS_DIR/include"
                    cp src/zlog.h "../../../$DEPS_DIR/include/" 2>/dev/null && echo "Copied zlog.h"
                    
                    echo "zlog manual build completed successfully"
                    BUILD_SUCCESS=1
                else
                    echo "Warning: Failed to create static library"
                fi
            else
                echo "Warning: No object files created or compilation failed"
            fi
        else
    ...
    # Verify that we got the library we need
    if [ "$BUILD_SUCCESS" -eq 1 ] && [ -f "../../../$DEPS_DIR/lib/libzlog.a" ]; then
        echo "✓ Successfully built libzlog.a"
        
        # Check library contents and size
        echo "Library info:"
        ls -lh "../../../$DEPS_DIR/lib/libzlog.a" 2>/dev/null || true
        echo "Library contents (first 10 symbols):"
        $TARGET_TRIPLET-ar -t "../../../$DEPS_DIR/lib/libzlog.a" 2>/dev/null | head -10 || true
        
        # Verify header was copied
        if [ -f "../../../$DEPS_DIR/include/zlog.h" ]; then
            echo "✓ zlog.h header available"
        else
            echo "Warning: zlog.h header not found"
        fi
        
        cd - > /dev/null
        echo "zlog built successfully for Windows"
        return 0
    else
        echo "Warning: zlog build failed - libzlog.a not found after all attempts"
        cd - > /dev/null
        return 1
    fi
}

# Function to create zlog stub as fallback
build_zlog_stub() {
    echo "Creating zlog stub library..."
    ...
    # Create basic zlog headers
    mkdir -p "$DEPS_DIR/include"
    
    # Create comprehensive zlog.h stub
    cat > "$DEPS_DIR/include/zlog.h" << 'EOF'
...
EOF
    ...
    cat > "$DEPS_DIR/src/zlog_stub.c" << 'EOF'
...
EOF
    ...
    # Compile stub library
    cd "$DEPS_DIR/src"
    echo "Compiling zlog stub..."
    if $TARGET_TRIPLET-gcc -I../include -c zlog_stub.c -o zlog_stub.o; then
        echo "Creating libzlog.a stub..."
        if $TARGET_TRIPLET-ar rcs ../lib/libzlog.a zlog_stub.o; then
            echo "✓ zlog stub library created successfully"
            
            # Verify the library
            echo "Stub library info:"
            ls -lh ../lib/libzlog.a 2>/dev/null || true
            echo "Stub library contents:"
            $TARGET_TRIPLET-ar -t ../lib/libzlog.a 2>/dev/null || true
        else
            echo "Warning: Failed to create zlog stub library"
            cd - > /dev/null
            return 1
        fi
    else
        echo "Warning: Failed to compile zlog stub"
        cd - > /dev/null
        return 1
    fi
    
    cd - > /dev/null
    echo "zlog stub library created (for compilation testing only)"
    return 0
}
...
if [ ! -f "$DEPS_DIR/lib/libzlog.a" ]; then
    ...
    if ! build_zlog_for_windows; then
        echo "Warning: zlog build failed, falling back to stub implementation"
        cd "$SCRIPT_DIR"  # Return to script directory
        build_zlog_stub
    else
    ...
    echo "zlog already built for Windows"
fi
