...
def transform_latexml_to_hybrid(html: str) -> str:
    ...
    # Remove LaTeXML footer completely (contains "Generated by LaTeXML", "Mascot Sammy", etc.)
    result = re.sub(r'<footer[^>]*>.*?</footer>', '', result, flags=re.DOTALL)
    ...
    # Remove LaTeXML ERROR spans (unsupported commands shown as raw text)
    result = re.sub(r'<span class="ltx_ERROR[^"]*">[^<]*</span>\s*', '', result)
    ...
    # Remove orphan </div> tags at end of paragraphs, list items, etc. (LaTeXML artifacts)
    result = re.sub(r'</p>\s*</div>', '</p>', result)
    result = re.sub(r'</li>\s*</div>', '</li>', result)
    result = re.sub(r'</dd>\s*</div>', '</dd>', result)
    result = re.sub(r'</article>\s*</div>', '</article>', result)
    result = re.sub(r'</section>\s*</div>', '</section>', result)
    
    # Remove empty divs
    result = re.sub(r'<div>\s*</div>', '', result)
    result = re.sub(r'<div\s+>\s*</div>', '', result)
    
    # Remove trailing orphan </div> tags
    while result.rstrip().endswith('</div>'):
        result = result.rstrip()[:-6]
...
def main():
    ...
    if args.latexjs_only:
        # Only process files in the latexjs/ subdirectory
        latexjs_files = [f for f in latexjs_files if '/latexjs/' in str(f)]
