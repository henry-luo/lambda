...
static MIR_reg_t emit_call_6(MirTranspiler* mt, const char* fn_name,
    ...
    return emit_call_with_args(mt, fn_name, ret_type, 6, types, ops);
}
...
static MIR_reg_t transpile_ident(MirTranspiler* mt, AstIdentNode* ident) {
    ...
    if (ident->entry && ident->entry->import) {
        ...
        if (entry_node && (entry_node->node_type == AST_NODE_ASSIGN ||
            ...
            MIR_reg_t val_reg;
            if (var_tid == LMD_TYPE_INT || var_tid == LMD_TYPE_FLOAT || var_tid == LMD_TYPE_BOOL ||
                var_tid == LMD_TYPE_STRING || var_tid == LMD_TYPE_INT64) {
