...
static MIR_reg_t jm_initialize_resumable_env(JsMirTranspiler* mt,
    ...
    if (args_slot >= 0 && fc->uses_arguments) {
...
static void jm_hoist_function_body_bindings(JsMirTranspiler* mt,
    ...
    jm_collect_body_locals(fn->body, body_locals, true);
...
static void jm_hoist_inner_function_declarations(JsMirTranspiler* mt,
    ...
    for (JsAstNode* statement = block->statements; statement; statement = statement->next) {
        ...
        const char* var_name = jm_format_name("_js_%.*s",
...
static void jm_collect_lexical_decl_names(JsAstNode* node, struct hashmap* names) {
    ...
    case JS_AST_NODE_CLASS_DECLARATION: {
        ...
        if (cls->name) {
            const char* name = jm_format_name("_js_%.*s", (int)cls->name->len, cls->name->chars);
...
static bool jm_capture_is_nfe_binding(JsMirTranspiler* mt, JsFuncCollected* fc, const char* name) {
    ...
    while (idx >= 0 && idx < mt->func_count) {
        ...
        if (fn && fn->node_type == JS_AST_NODE_FUNCTION_EXPRESSION && fn->name) {
            const char* self_name = jm_format_name("_js_%.*s", (int)fn->name->len, fn->name->chars);
...
static void jm_emit_resumable_captures(JsMirTranspiler* mt,
    ...
    const char* self_capture_name = fn->name
        ? jm_format_name("_js_%.*s", (int)fn->name->len, fn->name->chars) : NULL;
    for (int ci = 0; ci < fc->capture_count; ci++) {
        ...
        bool is_self_capture = (self_capture_name && self_capture_name[0] &&
            strcmp(fc->captures[ci].name, self_capture_name) == 0);
        if (!has_scope_slot && !is_self_capture && mt->module_consts) {
            ...
            if (mc && mc->const_type == MCONST_MODVAR &&
                    !fc->captures[ci].force_env_capture) {
                continue;
            }
        }
        ...
        jm_emit_load_i64(mt, cap_reg, (cap_offset + ci) * (int)sizeof(uint64_t),
            mt->gen_env_reg);
        JsVarScopeEntry entry;
        jm_init_resumable_env_entry(mt, &entry, fc->captures[ci].name,
...
static JsMirResumableLayout jm_resumable_layout(JsFuncCollected* fc,
    ...
    JsMirResumableLayout layout = {0, fc->capture_count,
        fc->capture_count + param_count, 0, -1, -1, 0, 0, -1};
...
void jm_define_function(JsMirTranspiler* mt, JsFuncCollected* fc) {
    ...
    if (fn->is_generator) {
        ...
        JsMirResumableLayout layout = jm_resumable_layout(fc, param_count,
            local_count, fn->is_arrow, fc->uses_arguments);
        ...
        const char* self_capture_name = fn->name
            ...
            for (int i = 0; i < fc->capture_count; i++) {
                ...
                if (!has_scope_slot && !is_self_capture) {
                    ...
                    if (mt->module_consts) {
                        JsModuleConstEntry mclookup;
                        mclookup.name = jm_persist_name(fc->captures[i].name);
