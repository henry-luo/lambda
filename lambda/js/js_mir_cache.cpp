...
static uint64_t js_mir_lease_mix(uint64_t hash, uint64_t value) {
...
static uint64_t js_mir_lease_preamble_abi_hash(const JsPreambleState* preamble) {
    ...
    uint64_t hash = js_mir_lease_mix(0xcbf29ce484222325ULL,
                                     jube_interface_registry_digest());
    if (!preamble) return hash;

    hash = js_mir_lease_mix(hash, (uint64_t)preamble->module_var_count);
    hash = js_mir_lease_mix(hash, 0x4e494431u);
    hash = js_mir_lease_mix(hash, (uint64_t)preamble->module_property_count);
    hash = js_mir_lease_mix(hash, (uint64_t)preamble->module_property_bytes_size);
    if (preamble->module_property_specs && preamble->module_property_bytes_size > 0) {
        hash = js_mir_lease_mix(hash, hashmap_xxhash3(
    ...
    for (int i = 0; preamble->entries && i < preamble->entry_count; i++) {
        ...
        hash = js_mir_lease_mix(hash, hashmap_xxhash3(entry->name, name_len, 0, 0));
        hash = js_mir_lease_mix(hash, (uint64_t)entry->const_type);
        hash = js_mir_lease_mix(hash, (uint64_t)entry->int_val);
        hash = js_mir_lease_mix(hash, (uint64_t)entry->var_kind);
        hash = js_mir_lease_mix(hash, (uint64_t)entry->modvar_type);
        hash = js_mir_lease_mix(hash, entry->is_nested_func_hoist ? 1 : 0);
        hash = js_mir_lease_mix(hash, entry->is_iife_func_decl ? 1 : 0);
        hash = js_mir_lease_mix(hash, entry->is_live_default_binding ? 1 : 0);
    }
    return hash;
}
