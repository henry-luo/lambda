...
static bool net_is_normalized_args(Item value) {
    ...
    return js_is_truthy(js_get_key_default(value, net_normalized_args_key()));
}
...
enum JsSocketValueSlot {
    JS_SOCKET_VALUE_OBJECT,
    JS_SOCKET_VALUE_TIMEOUT_TIMER,
    JS_SOCKET_VALUE_ABORT_SIGNAL,
    JS_SOCKET_VALUE_ABORT_HANDLER,
    JS_SOCKET_VALUE_ONREAD_BUFFER_SOURCE,
    JS_SOCKET_VALUE_ONREAD_BUFFER,
    JS_SOCKET_VALUE_ONREAD_CALLBACK,
    JS_SOCKET_VALUE_TLS_SOCKET,
...
// Native write records carry only a stable slot; one vector owns every JS
// callback until its queued, submitted, or shutdown completion consumes it.
static JsSocket* socket_alloc(void) {
    ...
    runtime_callback_slots_init(&sock->write_callbacks, (Context*)context,
        "net.Socket write callbacks");
    sock->end_callback_slot = -1;
    return sock;
}

static void socket_free(JsSocket* sock) {
    if (!sock) return;
    ...
    runtime_callback_slots_destroy(&sock->write_callbacks);
    mem_free(sock);
}
...
static bool net_object_has_key(Item obj, const char* key) {
    ...
    for (int64_t i = 0; i < len; i++) {
        Item item = js_elements_get_int(keys, i);
...
static void socket_shutdown_writes(JsSocket* sock, int64_t callback_slot);
...
static void socket_expose_handle(JsSocket* sock) {
    if (!sock || !SOCKET_VALUE(sock, OBJECT).item || sock->handle_exposed) return;
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "_handle", make_socket_handle_object(sock));
    sock->handle_exposed = true;
}

static void socket_hide_handle(JsSocket* sock) {
    if (!sock || !SOCKET_VALUE(sock, OBJECT).item) return;
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "_handle", ItemNull);
    sock->handle_exposed = false;
}

static void socket_clear_timeout(JsSocket* sock) {
    if (!sock || !sock->timeout_timer_active) return;
    js_clearTimeout(SOCKET_VALUE(sock, TIMEOUT_TIMER));
    SOCKET_SET_VALUE(sock, TIMEOUT_TIMER, make_undefined_item());
    sock->timeout_timer_active = false;
    if (SOCKET_VALUE(sock, OBJECT).item) {
        js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "__timeout_timer__", make_undefined_item());
    }
}
...
static void socket_update_io_counters(JsSocket* sock) {
    ...
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "bytesRead", js_make_number((double)sock->bytes_read));
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "bytesWritten", js_make_number((double)sock->bytes_written));
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "bufferSize", js_make_number((double)sock->buffer_size));
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "writableLength", js_make_number((double)sock->buffer_size));
}
...
static void socket_update_state_properties(JsSocket* sock) {
    ...
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "connecting", (Item){.item = b2it(connecting)});
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "_connecting", (Item){.item = b2it(connecting)});
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "pending", (Item){.item = b2it(pending)});
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "readyState", make_string_item(ready_state));
}
...
static void socket_update_address_properties(JsSocket* sock) {
    ...
    if (uv_tcp_getsockname(&sock->tcp, (struct sockaddr*)&addr, &addrlen) == 0) {
        net_set_endpoint_properties(SOCKET_VALUE(sock, OBJECT), "local", &addr, true);
    }

    addrlen = sizeof(addr);
    if (uv_tcp_getpeername(&sock->tcp, (struct sockaddr*)&addr, &addrlen) == 0) {
        net_set_endpoint_properties(SOCKET_VALUE(sock, OBJECT), "remote", &addr, true);
    }
}
...
static void socket_pipe_data(Item obj, Item data) {
    ...
    Item write_fn = js_get_key_cstr(dest, "write");
...
static void socket_pipe_end(Item obj) {
    Item dest = js_get_key_cstr(obj, "__pipe_dest__");
    if (dest.item == 0 || dest.item == ITEM_NULL || is_undefined_item(dest)) return;
    Item end_fn = js_get_key_cstr(dest, "end");
...
static void socket_emit_finish_once(JsSocket* sock) {
    ...
    sock->finished = true;
    socket_emit(SOCKET_VALUE(sock, OBJECT), "finish", NULL, 0);
}

static bool socket_allow_half_open(Item obj) {
    Item value = js_get_key_cstr(obj, "allowHalfOpen");
    return get_type_id(value) == LMD_TYPE_BOOL && it2b(value);
}

static void socket_finish_on_remote_end(JsSocket* sock) {
    if (!sock || socket_allow_half_open(SOCKET_VALUE(sock, OBJECT))) return;
    ...
    socket_shutdown_writes(sock, -1);
}


static bool socket_uses_utf8_encoding(JsSocket* sock) {
    if (!sock || !SOCKET_VALUE(sock, OBJECT).item) return false;
    Item encoding = js_get_key_cstr(SOCKET_VALUE(sock, OBJECT), "__encoding__");
...
static void socket_emit_read_data(JsSocket* sock, const char* data, int len) {
    ...
    Item pipe_dest = js_get_key_cstr(SOCKET_VALUE(sock, OBJECT), "__pipe_dest__");
    bool has_pipe = pipe_dest.item != 0 && pipe_dest.item != ITEM_NULL && !is_undefined_item(pipe_dest);
    if (!socket_has_listener(SOCKET_VALUE(sock, OBJECT), "data") && !has_pipe) return;
    Item chunk = ItemNull;
    if (!socket_make_read_data(sock, data, len, &chunk)) return;
    socket_emit(SOCKET_VALUE(sock, OBJECT), "data", &chunk, 1);
    socket_pipe_data(SOCKET_VALUE(sock, OBJECT), chunk);
}
...
static bool socket_prepare_onread_buffer(JsSocket* sock, char** out_data, size_t* out_len) {
    ...
    Item buffer = SOCKET_VALUE(sock, ONREAD_BUFFER);
    if (sock->onread_buffer_factory) {
        Item produced = js_call_function(SOCKET_VALUE(sock, ONREAD_BUFFER_SOURCE),
            SOCKET_VALUE(sock, OBJECT), NULL, 0);
        js_microtask_flush();
        if (js_is_typed_array(produced)) {
            SOCKET_SET_VALUE(sock, ONREAD_BUFFER, produced);
            buffer = produced;
        }
    }
...
static void socket_emit_onread(JsSocket* sock, int nread) {
    if (!sock || !sock->onread_enabled || !is_callable(SOCKET_VALUE(sock, ONREAD_CALLBACK))) return;
    Item args[2] = {
        (Item){.item = i2it(nread)},
        SOCKET_VALUE(sock, ONREAD_BUFFER)
    };
    Item result = js_call_function(SOCKET_VALUE(sock, ONREAD_CALLBACK),
        SOCKET_VALUE(sock, OBJECT), args, 2);
    js_microtask_flush();
...
static void socket_report_write_error(JsSocket* sock, Item callback, Item err) {
    ...
    Rooted<Item> error_root(roots, err);
    if (is_callable(callback_root.get())) {
        js_call_function(callback_root.get(), make_undefined_item(),
            (Item*)error_root.home(), 1);
        js_microtask_flush();
    } else if (sock && SOCKET_VALUE(sock, OBJECT).item) {
        socket_emit(SOCKET_VALUE(sock, OBJECT), "error", (Item*)error_root.home(), 1);
    }
}
...
static bool socket_delegate_close_to_tls(JsSocket* sock, Item error_item) {
    ...
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "destroyed", (Item){.item = ITEM_TRUE});
    socket_update_state_properties(sock);
    if (!is_undefined_item(error_item) && error_item.item != ITEM_NULL) {
        socket_emit(SOCKET_VALUE(sock, OBJECT), "error", &error_item, 1);
    }
    Item destroy_fn = js_get_key_cstr(SOCKET_VALUE(sock, TLS_SOCKET), "destroy");
    if (is_callable(destroy_fn)) {
        ...
        js_call_function(destroy_fn, SOCKET_VALUE(sock, TLS_SOCKET), NULL, 0);
        js_microtask_flush();
    }
    return true;
}
...
static void socket_update_writable(JsSocket* sock, bool writable) {
    if (!sock || !SOCKET_VALUE(sock, OBJECT).item) return;
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "writable", (Item){.item = b2it(writable)});
}

static void socket_update_readable(JsSocket* sock, bool readable) {
    if (!sock || !SOCKET_VALUE(sock, OBJECT).item) return;
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "readable", (Item){.item = b2it(readable)});
}
...
static void socket_maybe_emit_drain(JsSocket* sock) {
    ...
    socket_emit(SOCKET_VALUE(sock, OBJECT), "drain", NULL, 0);
}

static void socket_handle_remote_eof(JsSocket* sock) {
    if (!sock || sock->destroyed) return;
    sock->remote_ended = true;
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "__remote_ended__", (Item){.item = ITEM_TRUE});
    sock->reading = false;
    socket_update_readable(sock, false);
    ...
    if (sock->utf8_pending_len > 0) {
        ...
        sock->utf8_pending_len = 0;
        socket_emit(SOCKET_VALUE(sock, OBJECT), "data", &data, 1);
        socket_pipe_data(SOCKET_VALUE(sock, OBJECT), data);
        if (sock->destroyed) return;
    }

    socket_emit(SOCKET_VALUE(sock, OBJECT), "end", NULL, 0);
    socket_pipe_end(SOCKET_VALUE(sock, OBJECT));
    if (sock->destroyed) return;

    if (socket_allow_half_open(SOCKET_VALUE(sock, OBJECT))) {
        return;
    }
...
static void socket_write_settled(void* ud, int status, const char* data, size_t len) {
    ...
    RootFrame roots(2);
    Rooted<Item> callback_root(roots, sock
        ? runtime_callback_slots_take(&sock->write_callbacks, wreq->callback_slot)
        : ItemNull);
    ...
    if (is_callable(callback_root.get())) {
        if (status == 0) {
            js_call_function(callback_root.get(), make_undefined_item(), NULL, 0);
        } else {
            error_root.set(make_uv_error(status, "write", NULL, -1));
            js_call_function(callback_root.get(), make_undefined_item(),
                (Item*)error_root.home(), 1);
        }
        js_microtask_flush();
...
static bool socket_capture_write_callback(JsSocket* sock, Item callback,
                                          int64_t* callback_slot) {
    if (!callback_slot) return false;
    *callback_slot = -1;
    if (!is_callable(callback)) return true;
    return runtime_callback_slots_add(&sock->write_callbacks, callback, callback_slot);
}

static Item socket_submit_write(JsSocket* sock, char* copy, size_t data_len,
                                int64_t callback_slot, bool counted_before) {
    SocketWriteReq* wreq = (SocketWriteReq*)mem_calloc(1, sizeof(SocketWriteReq), MEM_CAT_JS_RUNTIME);
    if (!wreq) {
        (void)runtime_callback_slots_take(&sock->write_callbacks, callback_slot);
        mem_free(copy);
        return (Item){.item = b2it(false)};
    }
    wreq->len = data_len;
    wreq->sock = sock;
    wreq->callback_slot = callback_slot;
...
static bool socket_queue_write(JsSocket* sock, char* copy, size_t data_len,
        int64_t callback_slot) {
    ...
    pending->callback_slot = callback_slot;
...
static void socket_fail_pending_writes(JsSocket* sock, Item err) {
    if (!sock) return;
    RootFrame error_roots(1);
    Rooted<Item> error_root(error_roots, err);
    PendingSocketWrite* pending = sock->pending_writes_head;
    ...
    while (pending) {
        ...
        Rooted<Item> callback_root(roots,
            runtime_callback_slots_take(&sock->write_callbacks, pending->callback_slot));
        if (is_callable(callback_root.get())) {
            js_call_function(callback_root.get(), make_undefined_item(),
                (Item*)error_root.home(), 1);
            js_microtask_flush();
...
static Item socket_abort_reason(JsSocket* sock) {
    if (sock && SOCKET_VALUE(sock, ABORT_SIGNAL).item) {
        Item reason = js_get_key_cstr(SOCKET_VALUE(sock, ABORT_SIGNAL), "reason");
...
static void socket_remove_abort_listener(JsSocket* sock) {
    ...
    Item remove_fn = js_get_key_cstr(SOCKET_VALUE(sock, ABORT_SIGNAL), "removeEventListener");
    if (is_callable(remove_fn)) {
        Item args[2] = { make_string_item("abort"), SOCKET_VALUE(sock, ABORT_HANDLER) };
        js_call_function(remove_fn, SOCKET_VALUE(sock, ABORT_SIGNAL), args, 2);
        js_microtask_flush();
    }
    sock->abort_handler_set = false;
    SOCKET_SET_VALUE(sock, ABORT_SIGNAL, make_undefined_item());
    SOCKET_SET_VALUE(sock, ABORT_HANDLER, make_undefined_item());
}
...
static bool socket_configure_abort_signal(JsSocket* sock, Item signal) {
    ...
    SOCKET_SET_VALUE(sock, ABORT_SIGNAL, signal);
    ...
    js_microtask_flush();
    SOCKET_SET_VALUE(sock, ABORT_HANDLER, handler);
...
static void socket_configure_onread(JsSocket* sock, Item onread) {
    ...
    SOCKET_SET_VALUE(sock, ONREAD_BUFFER_SOURCE, buffer);
    SOCKET_SET_VALUE(sock, ONREAD_BUFFER, buffer);
    SOCKET_SET_VALUE(sock, ONREAD_CALLBACK, callback);
}
...
static void socket_flush_pending_writes(JsSocket* sock) {
    ...
    while (pending) {
        PendingSocketWrite* next = pending->next;
        socket_submit_write(sock, pending->data, pending->len,
            pending->callback_slot, true);
        mem_free(pending);
        pending = next;
    }
}
...
static Item socket_write_data(Item self, JsSocket* sock, Item data_item, Item callback) {
    ...
    Item remote_ended_marker = js_get_key_cstr(self, "__remote_ended__");
    ...
    Item handle = js_get_key_cstr(self, "_handle");
    ...
    int64_t callback_slot = -1;
    if (!socket_capture_write_callback(sock, callback, &callback_slot)) {
        mem_free(copy);
        return (Item){.item = b2it(false)};
    }
    if (sock->connect_pending && !sock->connected) {
        if (!socket_queue_write(sock, copy, data_len, callback_slot)) {
            return (Item){.item = b2it(false)};
        }
        return (Item){.item = b2it(sock->buffer_size <= sock->high_water_mark)};
    }

    return socket_submit_write(sock, copy, data_len, callback_slot, false);
}
...
extern "C" Item js_socket_write(Item rest_args) {
    ...
    Item data_item = argc > 0 ? js_elements_get_int(rest_args, 0) : make_undefined_item();
...
static void socket_shutdown_writes(JsSocket* sock, int64_t callback_slot) {
    ...
    sreq->sock = sock;
    sreq->callback_slot = callback_slot;
    sreq->req.data = sreq;
    int r = uv_shutdown(&sreq->req, (uv_stream_t*)&sock->tcp,
        [](uv_shutdown_t* req, int status) {
            ...
            RootFrame roots(1);
            Rooted<Item> callback_root(roots, sock && sreq
                ? runtime_callback_slots_take(&sock->write_callbacks,
                    sreq->callback_slot) : ItemNull);
            if (sock && !sock->destroyed) {
    ...
    if (r != 0) {
        RootFrame roots(1);
        Rooted<Item> callback_root(roots,
            runtime_callback_slots_take(&sock->write_callbacks, callback_slot));
        socket_emit_finish_once(sock);
        if (is_callable(callback_root.get())) {
            js_call_function(callback_root.get(), make_undefined_item(), NULL, 0);
            js_microtask_flush();
...
extern "C" Item js_socket_end(Item rest_args) {
    ...
    int64_t argc64 = js_array_length(rest_args);
    int argc = argc64 > 16 ? 16 : (int)argc64;
    if (argc > 0) data_item = js_elements_get_int(rest_args, 0);
    ...
    if (sock->connect_pending && !sock->connected) {
        ...
        (void)runtime_callback_slots_take(&sock->write_callbacks,
            sock->end_callback_slot);
        if (!socket_capture_write_callback(sock, callback, &sock->end_callback_slot)) {
            return self;
        }
        return self;
    }
    ...
    int64_t callback_slot = -1;
    if (!socket_capture_write_callback(sock, callback, &callback_slot)) return self;
    socket_shutdown_writes(sock, callback_slot);
...
extern "C" Item js_socket_destroy(Item error_item) {
    ...
    js_set_key_cstr(self, "destroyed", (Item){.item = ITEM_TRUE});
    socket_update_state_properties(sock);
    if (!is_undefined_item(error_item) && error_item.item != ITEM_NULL) {
...
static void socket_close_handle_cb(uv_handle_t* handle) {
    ...
    js_set_key_cstr(SOCKET_VALUE(s, OBJECT), "__handle__", ItemNull);
    if (notify_ipc_parent) js_process_ipc_notify_socket_closed();
    socket_emit_close(SOCKET_VALUE(s, OBJECT), false);
...
extern "C" uv_tcp_t* js_net_socket_adopt_for_tls(Item socket_obj, Item tls_obj) {
    ...
    SOCKET_SET_VALUE(sock, TLS_SOCKET, tls_obj);
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "__tls_socket__", tls_obj);
...
extern "C" void js_net_socket_tls_closed(Item socket_obj, bool had_error) {
    ...
    socket_update_readable(sock, false);
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "destroyed", (Item){.item = ITEM_TRUE});
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "__handle__", ItemNull);
    js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "__tls_socket__", make_undefined_item());
    socket_update_state_properties(sock);
    net_active_remove(&sock->active_resource_id);
    socket_emit_close(SOCKET_VALUE(sock, OBJECT), had_error);
...
static Item js_socket_setTimeout(Item msecs, Item callback) {
    ...
    js_set_key_cstr(self, "timeout", msecs);
    if (is_callable(callback)) {
        Item args[] = { make_string_item("timeout"), callback };
        Item on_fn = js_get_key_cstr(self, "on");
    ...
    if (delay > 0) {
        ...
        if (sock) {
            SOCKET_SET_VALUE(sock, TIMEOUT_TIMER, timer);
            sock->timeout_timer_active = true;
            js_set_key_cstr(self, "__timeout_timer__", timer);
...
static Item js_socket_setKeepAlive(Item rest_args) {
    ...
    int64_t argc64 = js_array_length(rest_args);
    int argc = argc64 > 16 ? 16 : (int)argc64;
    Item first = argc > 0 ? js_elements_get_int(rest_args, 0) : make_undefined_item();
    Item delay = argc > 1 ? js_elements_get_int(rest_args, 1) : make_undefined_item();
    ...
    if (options_object) {
        ...
        delay = js_get_key_cstr(first, "initialDelay");
        interval = js_get_key_cstr(first, "interval");
        count = js_get_key_cstr(first, "count");
    } else {
    ...
    if (handle.item != 0 && handle.item != ITEM_NULL && !is_undefined_item(handle)) {
        Item fn = js_get_key_cstr(handle, "setKeepAlive");
        if (is_callable(fn)) {
...
static Item js_socket_setNoDelay(Item noDelay) {
    ...
    Item handle = js_get_key_cstr(self, "_handle");
    if (handle.item != 0 && handle.item != ITEM_NULL && !is_undefined_item(handle)) {
        Item fn = js_get_key_cstr(handle, "setNoDelay");
        if (is_callable(fn)) {
...
static Item js_socket_ref_or_unref(bool do_ref) {
    ...
    if (sock && !uv_is_closing((uv_handle_t*)&sock->tcp)) {
        ...
        if (sock->timeout_timer_active) {
            if (do_ref) js_timeout_ref(SOCKET_VALUE(sock, TIMEOUT_TIMER));
            else js_timeout_unref(SOCKET_VALUE(sock, TIMEOUT_TIMER));
        }
    }
    return self;
}
...
static Item js_socket_setEncoding(Item encoding) {
    Item self = js_get_this();
    js_set_key_cstr(self, "__encoding__", encoding);
    return self;
}
...
static bool socket_has_js_read_handle(JsSocket* sock, Item* out_handle) {
    if (!sock || !SOCKET_VALUE(sock, OBJECT).item) return false;
    Item handle = js_get_key_cstr(SOCKET_VALUE(sock, OBJECT), "_handle");
...
static Item js_socket_js_handle_onread(void) {
    ...
    Item self = js_get_key_cstr(handle, "__socket_object__");
    JsSocket* sock = socket_from_object(self);
...
static bool socket_start_read(JsSocket* sock) {
    ...
    if (socket_has_js_read_handle(sock, &js_handle)) {
        Item read_start = js_get_key_cstr(js_handle, "readStart");
        sock->reading = true;
...
static Item socket_make_address_result(const struct sockaddr_storage* addr) {
    ...
    if (addr->ss_family == AF_INET) {
        ...
        js_set_key_cstr(result, "port", (Item){.item = i2it(ntohs(a4->sin_port))});
    } else if (addr->ss_family == AF_INET6) {
        const struct sockaddr_in6* a6 = (const struct sockaddr_in6*)addr;
        char ip[128];
        uv_ip6_name(a6, ip, sizeof(ip));
        js_set_key_cstr(result, "address", make_string_item(ip));
        js_set_key_cstr(result, "family", make_string_item("IPv6"));
        js_set_key_cstr(result, "port", (Item){.item = i2it(ntohs(a6->sin6_port))});
    }
    return result;
}
...
typedef struct JsNetRuntimeState {
    ...
    ArrayList* block_list_instances;
    JsServer* closed_servers;
} JsNetRuntimeState;

struct NetRealmItems {
    Item* namespace_object = NULL;
    Item* socket_prototype = NULL;
    Item* server_prototype = NULL;
    Item* socket_connect_function = NULL;
    Item* stream_socket_constructor = NULL;
};
...
#define net_socket_prototype (*net_realm_slot_existing(JS_REALM_SLOT_NET_SOCKET_PROTOTYPE))
#define net_server_prototype (*net_realm_slot_existing(JS_REALM_SLOT_NET_SERVER_PROTOTYPE))
#define net_socket_connect_fn (*net_realm_slot_existing(JS_REALM_SLOT_NET_SOCKET_CONNECT_FUNCTION))
#define internal_js_stream_socket_ctor (*net_realm_slot_existing(JS_REALM_SLOT_NET_STREAM_SOCKET_CONSTRUCTOR))
#define net_namespace (*net_realm_slot_existing(JS_REALM_SLOT_NET_NAMESPACE))
...
static bool net_ensure_roots(void) {
    ...
    NetRealmItems items = {};
    return net_realm_items(&items, true);
}

#define net_runtime_state (*(JsNetRuntimeState*)js_runtime_state.net_native.native_state)
...
static int net_block_list_entry_count(const NetBlockList* list) {
    return list && list->entries ? list->entries->length : 0;
}

static NetBlockListEntry* net_block_list_entry_at(const NetBlockList* list, int index) {
    return list && list->entries && index >= 0 && index < list->entries->length
        ? (NetBlockListEntry*)arraylist_get(list->entries, index) : NULL;
}

static void net_block_list_destroy(NetBlockList* list) {
    if (!list) return;
    if (list->entries) {
        for (int i = 0; i < list->entries->length; i++) {
            mem_free(arraylist_get(list->entries, i));
        }
        arraylist_free(list->entries);
    }
    mem_free(list);
}

static void net_block_list_instances_clear(JsNetRuntimeState* state) {
    if (!state || !state->block_list_instances) return;
    for (int i = 0; i < state->block_list_instances->length; i++) {
        net_block_list_destroy((NetBlockList*)arraylist_get(state->block_list_instances, i));
    }
    arraylist_free(state->block_list_instances);
    state->block_list_instances = NULL;
}
...
static NetBlockList* net_block_list_alloc(void) {
    ...
    if (!list) return NULL;
    if (!net_runtime_state.block_list_instances) {
        net_runtime_state.block_list_instances = arraylist_new(8);
    }
    list->entries = arraylist_new(8);
    if (!list->entries || !net_runtime_state.block_list_instances ||
            !arraylist_append(net_runtime_state.block_list_instances, list)) {
        net_block_list_destroy(list);
        return NULL;
    }
    return list;
}
...
static Item make_socket_handle_object(JsSocket* sock) {
    ...
    js_set_key_cstr(handle_root.get(), "__socket_handle__", (Item){.item = i2it((int64_t)(uintptr_t)sock)});
#define JS_NET_INSTALL_HANDLE_METHOD(name, target) net_set_native(handle_root.get(), name, target);
    JS_NET_INSTALL_HANDLE_METHOD("setKeepAlive", js_socket_handle_setKeepAlive)
    JS_NET_INSTALL_HANDLE_METHOD("setNoDelay", js_socket_handle_setNoDelay)
    JS_NET_INSTALL_HANDLE_METHOD("close", js_socket_handle_close)
#undef JS_NET_INSTALL_HANDLE_METHOD
...
static Item make_socket_object(JsSocket* sock, bool expose_handle) {
    ...
    js_set_key_cstr(obj_root.get(), "__handle__", (Item){.item = i2it((int64_t)(uintptr_t)sock)});
#define JS_NET_SOCKET_EVENT_METHODS(M) \
    M("on", js_socket_on) M("once", js_socket_once) \
    M("listeners", js_socket_listeners) M("listenerCount", js_socket_listenerCount) \
    M("removeListener", js_socket_removeListener) M("off", js_socket_removeListener) \
    M("removeAllListeners", js_socket_removeAllListeners)
#define JS_NET_INSTALL_SOCKET_METHOD(name, target) net_set_native(obj_root.get(), name, target);
    JS_NET_SOCKET_EVENT_METHODS(JS_NET_INSTALL_SOCKET_METHOD)
#undef JS_NET_INSTALL_SOCKET_METHOD
#undef JS_NET_SOCKET_EVENT_METHODS
    js_set_key_cstr(obj_root.get(), "write", js_new_native_rest_function(js_socket_write));
    js_set_key_cstr(obj_root.get(), "end", js_new_native_rest_function(js_socket_end));
    ...
    js_set_key_cstr(obj_root.get(), "readable", (Item){.item = ITEM_TRUE});
    js_set_key_cstr(obj_root.get(), "writable", (Item){.item = ITEM_TRUE});
    js_set_key_cstr(obj_root.get(), "destroyed", (Item){.item = ITEM_FALSE});
    js_set_key_cstr(obj_root.get(), "bytesRead", js_make_number(0.0));
    js_set_key_cstr(obj_root.get(), "bytesWritten", js_make_number(0.0));
    js_set_key_cstr(obj_root.get(), "bufferSize", js_make_number(0.0));
    js_set_key_cstr(obj_root.get(), "writableLength", js_make_number(0.0));
    hwm_root.set(js_make_number((double)sock->high_water_mark));
    readable_state_root.set(js_new_object());
    js_set_key_cstr(readable_state_root.get(), "highWaterMark", hwm_root.get());
    writable_state_root.set(js_new_object());
    js_set_key_cstr(writable_state_root.get(), "highWaterMark", hwm_root.get());
    js_set_key_cstr(obj_root.get(), "_readableState", readable_state_root.get());
    js_set_key_cstr(obj_root.get(), "_writableState", writable_state_root.get());
    js_set_key_cstr(obj_root.get(), "readableHighWaterMark", hwm_root.get());
    js_set_key_cstr(obj_root.get(), "writableHighWaterMark", hwm_root.get());
    js_set_key_cstr(obj_root.get(), "connecting", (Item){.item = ITEM_FALSE});
    js_set_key_cstr(obj_root.get(), "_connecting", (Item){.item = ITEM_FALSE});
    js_set_key_cstr(obj_root.get(), "pending", (Item){.item = ITEM_TRUE});
    js_set_key_cstr(obj_root.get(), "readyState", make_string_item("closed"));
    if (expose_handle) {
        js_set_key_cstr(obj_root.get(), "_handle", make_socket_handle_object(sock));
        sock->handle_exposed = true;
    } else {
        js_set_key_cstr(obj_root.get(), "_handle", ItemNull);
        sock->handle_exposed = false;
    }
    js_set_key_cstr(obj_root.get(), "allowHalfOpen", (Item){.item = ITEM_FALSE});
    // Additional Socket methods
    net_set_native(obj_root.get(), "setTimeout", js_socket_setTimeout);
    js_set_key_cstr(obj_root.get(), "connect", js_new_native_rest_function(js_socket_connect));
    js_set_key_cstr(obj_root.get(), "setKeepAlive", js_new_native_rest_function(js_socket_setKeepAlive));
#define JS_NET_SOCKET_EXTRA_METHODS(M) \
    M("setNoDelay", js_socket_setNoDelay) M("setEncoding", js_socket_setEncoding) \
    M("setTypeOfService", js_socket_setTypeOfService) M("getTypeOfService", js_socket_getTypeOfService) \
    M("pipe", js_socket_pipe) M("ref", js_socket_ref) M("unref", js_socket_unref) \
    M("cork", js_socket_cork) M("uncork", js_socket_uncork) M("resume", js_socket_resume) \
    M("pause", js_socket_pause) M("address", js_socket_address)
#define JS_NET_INSTALL_SOCKET_EXTRA_METHOD(name, target) net_set_native(obj_root.get(), name, target);
    JS_NET_SOCKET_EXTRA_METHODS(JS_NET_INSTALL_SOCKET_EXTRA_METHOD)
#undef JS_NET_INSTALL_SOCKET_EXTRA_METHOD
#undef JS_NET_SOCKET_EXTRA_METHODS
    SOCKET_SET_VALUE(sock, OBJECT, obj_root.get());
...
static Item make_invalid_ip_address_error(Item address) {
    ...
    js_set_key_cstr(err, "code", make_string_item("ERR_INVALID_IP_ADDRESS"));
    return err;
}
...
static Item make_invalid_address_family_error(int family, const char* host, int port) {
    ...
    Item err = js_new_error(make_string_item(msg));
    js_set_key_cstr(err, "code", make_string_item("ERR_INVALID_ADDRESS_FAMILY"));
    if (host) js_set_key_cstr(err, "host", make_string_item(host));
    js_set_key_cstr(err, "port", (Item){.item = i2it(port)});
    return err;
}
...
static void bound_socket_close_handle(JsBoundSocket* bound, Item owner,
                                      bool clear_js_handle,
                                      bool free_after_close) {
    if (!bound || bound->closed) return;
    bound->closed = true;
    if (clear_js_handle && owner.item) {
        js_set_key_cstr(owner, "__bound_socket_handle__", make_undefined_item());
    }
...
static int bound_socket_dup_fd(JsBoundSocket* bound) {
    ...
    if (dup_fd >= 0) {
        bound->adopted = true;
        bound_socket_close_handle(bound, ItemNull, false, false);
    }
    return dup_fd;
#endif
}
...
static int bound_socket_item_dup_fd(Item self) {
    ...
    js_set_key_cstr(self, "__jube_bound_socket_resource_id__", make_undefined_item());
    js_set_key_cstr(self, "__jube_bound_socket_adopted__", (Item){.item = ITEM_TRUE});
    return descriptor;
}
...
static Item js_bound_socket_close(void) {
    ...
    if (bound->adopted) return bound_socket_throw_adopted();
    JS_ROOTS(roots, self_root, self);
    bound_socket_close_handle(bound, self_root.get(), true, true);
    return make_undefined_item();
}
...
extern "C" Item js_net_BoundSocket(Item options) {
    ...
    if (js_node_is_object_like(options)) {
        ...
        Item opt_ipv6_only = js_get_key_cstr(options, "ipv6Only");
        ipv6_only = get_type_id(opt_ipv6_only) == LMD_TYPE_BOOL && it2b(opt_ipv6_only);
        Item opt_reuse_port = js_get_key_cstr(options, "reusePort");
    ...
    if (r != 0) {
        ...
        bound_socket_close_handle(bound, ItemNull, false, true);
        return js_throw_value(err);
    }

    JS_ROOTS(roots, object_root, js_new_object());
    js_set_key_cstr(object_root.get(), "__bound_socket_handle__",
        (Item){.item = i2it((int64_t)(uintptr_t)bound)});
    net_set_native(object_root.get(), "address", js_bound_socket_address);
    net_set_native(object_root.get(), "fd", js_bound_socket_fd);
    net_set_native(object_root.get(), "close", js_bound_socket_close);
...
static bool net_block_list_check_parsed(NetBlockList* list, const NetBlockListEntry* address) {
    ...
    for (int i = 0; i < net_block_list_entry_count(list); i++) {
        NetBlockListEntry* rule = net_block_list_entry_at(list, i);
        if (!rule) continue;
...
static Item make_ip_blocked_error(const char* address) {
    ...
    Item err = js_new_error(make_string_item(msg));
    js_set_key_cstr(err, "code", make_string_item("ERR_IP_BLOCKED"));
    if (address && address[0]) js_set_key_cstr(err, "address", make_string_item(address));
    return err;
}
...
static void socket_emit_ip_blocked_timer_cb(uv_timer_t* timer) {
    ...
    if (sock && !sock->destroyed) {
        ...
        socket_fail_pending_writes(sock, err);
        socket_emit(SOCKET_VALUE(sock, OBJECT), "error", &err, 1);
        socket_close_now(sock);
    }
    uv_timer_stop(timer);
...
static Item normalize_options_object(Item options, NetConnectOptions* out) {
    ...
    Item hints = js_get_key_cstr(options, "hints");
    ...
    Item port = js_get_key_cstr(options, "port");
    Item path = js_get_key_cstr(options, "path");
    ...
    Item family = js_get_key_cstr(options, "family");
    ...
    Item keep_alive = js_get_key_cstr(options, "keepAlive");
    out->keep_alive = js_is_truthy(keep_alive);
    out->keep_alive_delay_secs =
        net_keep_alive_delay_secs(js_get_key_cstr(options, "keepAliveInitialDelay"));

    Item no_delay = js_get_key_cstr(options, "noDelay");
    out->no_delay = js_is_truthy(no_delay);

    Item local_address = js_get_key_cstr(options, "localAddress");
    ...
    Item local_port = js_get_key_cstr(options, "localPort");
    ...
    Item onread = js_get_key_cstr(options, "onread");
    if (!is_undefined_item(onread) && onread.item != ITEM_NULL) {
        out->onread = onread;
        out->has_onread = true;
    }

    Item lookup = js_get_key_cstr(options, "lookup");
    ...
    Item block_list = js_get_key_cstr(options, "blockList");
    ...
    Item auto_select = js_get_key_cstr(options, "autoSelectFamily");
    ...
    Item auto_select_timeout = js_get_key_cstr(options, "autoSelectFamilyAttemptTimeout");
...
static Item normalize_connect_args(Item rest_args, NetConnectOptions* out) {
    ...
    if (argc == 1 && get_type_id(first) == LMD_TYPE_ARRAY) {
        ...
        first = js_elements_get_int(rest_args, 0);
    }
    ...
    if (first_type == LMD_TYPE_MAP ||
        ...
        if (argc > 1) {
            Item cb = js_elements_get_int(rest_args, 1);
...
static void socket_record_auto_attempt(JsSocket* sock, const char* host, int port) {
    if (!sock || !SOCKET_VALUE(sock, OBJECT).item || !host || !host[0]) return;
    Item arr = js_get_key_cstr(SOCKET_VALUE(sock, OBJECT), "autoSelectFamilyAttemptedAddresses");
    if (get_type_id(arr) != LMD_TYPE_ARRAY) {
        arr = js_array_new(0);
        js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "autoSelectFamilyAttemptedAddresses", arr);
    }
    char endpoint[320];
...
static Item socket_auto_errors_array(JsSocket* sock, bool create) {
    if (!sock || !SOCKET_VALUE(sock, OBJECT).item) return make_undefined_item();
    Item key = make_string_item("__auto_select_errors__");
    Item arr = js_get_key_default(SOCKET_VALUE(sock, OBJECT), key);
    if (get_type_id(arr) != LMD_TYPE_ARRAY && create) {
        arr = js_array_new(0);
        js_set_key_default(SOCKET_VALUE(sock, OBJECT), key, arr);
    }
    return arr;
}
...
static bool socket_connect_auto_next(JsSocket* sock) {
    ...
    Item block_list = SOCKET_VALUE(sock, OBJECT).item ?
        js_get_key_cstr(SOCKET_VALUE(sock, OBJECT), "__block_list__") : make_undefined_item();
...
static bool net_copy_lookup_address(Item value, char* out, int out_size, int* out_family) {
    ...
    if (get_type_id(value) == LMD_TYPE_MAP) {
        address = js_get_key_cstr(value, "address");
        family = js_get_key_cstr(value, "family");
    }
...
static Item net_lookup_complete(Item env_item, Item rest_args) {
    ...
    Item err = argc > 0 ? js_elements_get_int(rest_args, 0) : make_undefined_item();
    ...
    Item value = argc > 1 ? js_elements_get_int(rest_args, 1) : make_undefined_item();
    Item family_item = argc > 2 ? js_elements_get_int(rest_args, 2) : make_undefined_item();
    ...
    if (nr->auto_select_family && get_type_id(value) == LMD_TYPE_ARRAY) {
        ...
        js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "__auto_select_errors__", js_array_new(0));
        for (int i = 0; i < len; i++) {
            Item record = js_elements_get_int(value, i);
            int family = 0;
        ...
        socket_emit(SOCKET_VALUE(sock, OBJECT), "lookup", lookup_args, 4);
...
static int socket_start_connect(JsSocket* sock, const NetConnectOptions* options) {
    ...
    if (options->has_lookup) {
        ...
        if (options->family == 4 || options->family == 6) {
            js_set_key_cstr(lookup_options, "family", (Item){.item = i2it(options->family)});
        }
...
static void socket_store_connect_options(JsSocket* sock, const NetConnectOptions* options) {
    ...
    if (SOCKET_VALUE(sock, OBJECT).item && options->has_block_list) {
        js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "__block_list__", options->block_list);
    }
}
...
static void client_connect_cb(uv_connect_t* req, int status) {
    ...
    sock->connected = true;
    if (sock->auto_select_family && SOCKET_VALUE(sock, OBJECT).item) {
        js_set_key_cstr(SOCKET_VALUE(sock, OBJECT), "__auto_select_errors__", make_undefined_item());
    }
    socket_update_state_properties(sock);
    socket_update_address_properties(sock);
    ...
    socket_emit(SOCKET_VALUE(sock, OBJECT), "connect", NULL, 0);
    socket_emit(SOCKET_VALUE(sock, OBJECT), "ready", NULL, 0);
    if (sock->end_after_connect) {
        int64_t callback_slot = sock->end_callback_slot;
        sock->end_callback_slot = -1;
        socket_shutdown_writes(sock, callback_slot);
    }
...
static Item create_socket_for_connect(const NetConnectOptions* options) {
    ...
    if (options->allow_half_open) {
        js_set_key_cstr(obj, "allowHalfOpen", (Item){.item = ITEM_TRUE});
    }
...
static JsSocket* socket_reattach_for_connect(Item self) {
    ...
    if (!sock) return NULL;
    sock->high_water_mark = NET_SOCKET_DEFAULT_HIGH_WATER_MARK;
    SOCKET_SET_VALUE(sock, OBJECT, self);
    uv_tcp_init(loop, &sock->tcp);
    sock->tcp.data = sock;

    js_set_key_cstr(self, "__handle__", (Item){.item = i2it((int64_t)(uintptr_t)sock)});
    js_set_key_cstr(self, "_handle", make_socket_handle_object(sock));
    sock->handle_exposed = true;
    // reconnect reuses the JS Socket object; clear the prior peer-FIN marker so new writes are not EPIPE.
    js_set_key_cstr(self, "__remote_ended__", make_undefined_item());
    js_set_key_cstr(self, "destroyed", (Item){.item = ITEM_FALSE});
    js_set_key_cstr(self, "readable", (Item){.item = ITEM_TRUE});
    js_set_key_cstr(self, "writable", (Item){.item = ITEM_TRUE});
    socket_update_io_counters(sock);
    socket_update_state_properties(sock);
...
static Item js_socket_connect_args(Item self, Item rest_args) {
    ...
    if (options.allow_half_open) {
        js_set_key_cstr(self, "allowHalfOpen", (Item){.item = ITEM_TRUE});
    }
    socket_sync_no_half_open_listener(self);
    socket_store_connect_options(sock, &options);
    if (options.has_onread) socket_configure_onread(sock, options.onread);
...
extern "C" Item js_net_createConnection(Item rest_args) {
    ...
    if (get_type_id(net_socket_prototype) == LMD_TYPE_MAP) {
        connect_fn = js_get_key_cstr(net_socket_prototype, "connect");
    }
    ...
    int64_t argc64 = js_array_length(rest_args);
    if (argc64 > INT_MAX) {
        return js_throw_range_error("Too many arguments for net.createConnection");
    }
    RootFrame roots(3);
    Rooted<Item> connect_root(roots, connect_fn);
    Rooted<Item> socket_root(roots, socket);
    Rooted<Item> rest_root(roots, rest_args);
    RootSpan args((size_t)argc64);
    if (!roots.valid() || (argc64 > 0 && !args.valid())) {
        return js_throw_range_error("Cannot retain net.createConnection arguments");
    }
    for (int i = 0; i < (int)argc64; i++) {
        args.items()[i] = js_elements_get_int(rest_root.get(), i);
    }
    js_call_function(connect_root.get(), socket_root.get(), args.items(), (int)argc64);
    js_microtask_flush();
    return socket_root.get();
}
...
struct JsServer {
    ...
    JsServer* closed_next;
};

enum JsServerValueSlot {
    JS_SERVER_VALUE_OBJECT,
    JS_SERVER_VALUE_CONNECTION_HANDLER,
    JS_SERVER_VALUE_BLOCK_LIST,
    JS_SERVER_VALUE_PENDING_LISTEN_PORT,
    JS_SERVER_VALUE_PENDING_LISTEN_HOST,
    JS_SERVER_VALUE_PENDING_LISTEN_CALLBACK,
...
static void net_server_register_closed(JsServer* srv) {
    if (!srv) return;
    srv->closed_next = net_runtime_state.closed_servers;
    net_runtime_state.closed_servers = srv;
}

static void net_server_release_closed(JsNetRuntimeState* state) {
    if (!state) return;
    JsServer* srv = state->closed_servers;
    state->closed_servers = NULL;
    while (srv) {
        JsServer* next = srv->closed_next;
        if (srv->handle_closed) {
            runtime_value_slots_destroy(&srv->values);
            mem_free(srv);
        } else {
            // Context destruction cannot reclaim an embedded live uv handle.
            log_error("net: retaining live server during runtime teardown");
        }
        srv = next;
    }
}
...
static void server_maybe_finish_close(JsServer* srv) {
    ...
    bool restart_pending = srv->listen_after_close;
    ...
    if (!srv->close_event_emitted) {
        ...
        js_set_key_cstr(NET_SERVER_VALUE(srv, OBJECT), "listening", (Item){.item = ITEM_FALSE});
        server_emit(NET_SERVER_VALUE(srv, OBJECT), "close", NULL, 0);
        Item close_callback = js_get_key_cstr(NET_SERVER_VALUE(srv, OBJECT), "__close_callback__");
        if (is_callable(close_callback)) {
            js_call_function(close_callback, NET_SERVER_VALUE(srv, OBJECT), NULL, 0);
            js_set_key_cstr(NET_SERVER_VALUE(srv, OBJECT), "__close_callback__", make_undefined_item());
        }
    }
    if (srv->listen_after_close) {
        srv->listen_after_close = false;
        Item args[3] = {
            NET_SERVER_VALUE(srv, PENDING_LISTEN_PORT),
            NET_SERVER_VALUE(srv, PENDING_LISTEN_HOST),
            NET_SERVER_VALUE(srv, PENDING_LISTEN_CALLBACK)
        };
        NET_SERVER_SET_VALUE(srv, PENDING_LISTEN_PORT, make_undefined_item());
        NET_SERVER_SET_VALUE(srv, PENDING_LISTEN_HOST, make_undefined_item());
        NET_SERVER_SET_VALUE(srv, PENDING_LISTEN_CALLBACK, make_undefined_item());
        Item listen_fn = js_get_key_cstr(NET_SERVER_VALUE(srv, OBJECT), "listen");
        if (is_callable(listen_fn)) {
            js_call_function(listen_fn, NET_SERVER_VALUE(srv, OBJECT), args, 3);
            js_microtask_flush();
        }
    }
    if (!restart_pending && !srv->release_registered) {
        srv->release_registered = true;
        net_server_register_closed(srv);
    }
}
...
static int server_max_connections(JsServer* srv) {
    if (!srv || !NET_SERVER_VALUE(srv, OBJECT).item) return -1;
    Item max_item = js_get_key_cstr(NET_SERVER_VALUE(srv, OBJECT), "maxConnections");
...
static void server_emit_drop(JsServer* srv, JsSocket* client) {
    if (!srv || !NET_SERVER_VALUE(srv, OBJECT).item) return;
    Item data = server_make_drop_data(client);
    server_emit(NET_SERVER_VALUE(srv, OBJECT), "drop", &data, 1);
}
...
static bool net_capture_rejections_enabled(void) {
    ...
    Item value = js_get_key_cstr(events, "captureRejections");
    return get_type_id(value) == LMD_TYPE_BOOL && it2b(value);
}
...
static void server_apply_accepted_handle_options(JsServer* srv, JsSocket* client, Item client_handle) {
    ...
    if (srv->keep_alive) {
        ...
        client->keep_alive_delay_secs = srv->keep_alive_initial_delay_ms > 0
            ...
            : 0;
        Item fn = js_get_key_cstr(client_handle, "setKeepAlive");
        if (is_callable(fn)) {
            Item args[2] = {
                (Item){.item = ITEM_TRUE},
    ...
    if (srv->no_delay) {
        ...
        Item fn = js_get_key_cstr(client_handle, "setNoDelay");
        if (is_callable(fn)) {
            Item arg = (Item){.item = ITEM_TRUE};
...
static Item server_accept_client(JsServer* srv, JsSocket* client, Item client_handle) {
    ...
    if (SOCKET_VALUE(client, OBJECT).item) return make_undefined_item();
    ...
    js_set_key_cstr(client_obj, "allowHalfOpen", (Item){.item = b2it(srv->allow_half_open)});
    socket_sync_no_half_open_listener(client_obj);
    srv->connection_count++;
    socket_update_state_properties(client);
    socket_update_address_properties(client);

    if (js_is_callable(NET_SERVER_VALUE(srv, CONNECTION_HANDLER))) {
        Item result = js_call_function(NET_SERVER_VALUE(srv, CONNECTION_HANDLER),
            NET_SERVER_VALUE(srv, OBJECT), &client_obj, 1);
        server_capture_connection_rejection(result, client_obj);
        js_microtask_flush();
    }

    server_emit(NET_SERVER_VALUE(srv, OBJECT), "connection", &client_obj, 1);
...
static Item make_server_handle_object(JsServer* srv) {
    ...
    js_set_key_cstr(handle, "__server_handle__", (Item){.item = i2it((int64_t)(uintptr_t)srv)});
...
extern "C" void js_net_close_all_active_servers(void) {
    ...
    for (int64_t i = 0; i < count; i++) {
        JsServer* srv = server_from_object(js_elements_get_int(servers, i));
        if (!srv) continue;
...
static void server_connection_cb(uv_stream_t* server, int status) {
    ...
    if (uv_accept(server, (uv_stream_t*)&client->tcp) == 0) {
        ...
        if (srv->has_block_list) {
            ...
            if (peer_ip[0] && net_block_list_blocks_item(
                    NET_SERVER_VALUE(srv, BLOCK_LIST), peer_ip, peer_family)) {
                uv_close((uv_handle_t*)&client->tcp, [](uv_handle_t* h) {
                    socket_free((JsSocket*)h->data);
                });
                return;
        ...
        Item server_handle = js_get_key_cstr(NET_SERVER_VALUE(srv, OBJECT), "_handle");
        Item onconnection = js_get_key_cstr(server_handle, "onconnection");
...
static void server_update_connection_key(Item self, JsServer* srv, int requested_port) {
    ...
    js_set_key_cstr(self, "_connectionKey", make_string_item(key));
}
...
static bool server_listen_on_fd(Item self, JsServer* srv, Item callback, uv_os_sock_t fd,
    ...
    js_set_key_cstr(self, "listening", (Item){.item = ITEM_TRUE});
...
extern "C" Item js_server_listen(Item port_item, Item host_item, Item callback) {
    ...
    if (srv->closed && srv->close_requested && !srv->handle_closed) {
        ...
        NET_SERVER_SET_VALUE(srv, PENDING_LISTEN_PORT, port_item);
        NET_SERVER_SET_VALUE(srv, PENDING_LISTEN_HOST, host_item);
        NET_SERVER_SET_VALUE(srv, PENDING_LISTEN_CALLBACK, callback);
        return self;
    }
    ...
    if (port_type == LMD_TYPE_MAP || port_type == LMD_TYPE_VMAP) {
        ...
        Item opt_port = js_get_key_cstr(port_item, "port");
        if (has_port) {
            ...
            Item opt_path = js_get_key_cstr(port_item, "path");
        ...
        Item opt_host = js_get_key_cstr(port_item, "host");
        ...
        Item opt_ipv6_only = js_get_key_cstr(port_item, "ipv6Only");
        ipv6_only = get_type_id(opt_ipv6_only) == LMD_TYPE_BOOL && it2b(opt_ipv6_only);
        Item opt_reuse_port = js_get_key_cstr(port_item, "reusePort");
        reuse_port = get_type_id(opt_reuse_port) == LMD_TYPE_BOOL && it2b(opt_reuse_port);
        Item opt_signal = js_get_key_cstr(port_item, "signal");
...
extern "C" Item js_server_close(Item callback) {
    ...
    srv->listen_pending = false;
    if (is_callable(callback)) {
        js_set_key_cstr(self, "__close_callback__", callback);
    }
...
extern "C" Item js_net_createServer(Item rest_args) {
    ...
    uv_tcp_init(loop, &srv->tcp);
    srv->tcp.data = srv;
    NET_SERVER_SET_VALUE(srv, CONNECTION_HANDLER, handler);
    if (get_type_id(options) == LMD_TYPE_MAP ||
        ...
        Item keep_alive_delay = js_get_key_cstr(options, "keepAliveInitialDelay");
        ...
        Item pause_on_connect = js_get_key_cstr(options, "pauseOnConnect");
        ...
        if (!is_undefined_item(block_list) && block_list.item != ITEM_NULL &&
            ...
            NET_SERVER_SET_VALUE(srv, BLOCK_LIST, block_list);
    ...
    js_set_key_cstr(obj_root.get(), "__server__", (Item){.item = i2it((int64_t)(uintptr_t)srv)});
    js_set_key_cstr(obj_root.get(), "_handle", make_server_handle_object(srv));
#define JS_NET_SERVER_METHODS(M) \
    M("listen", js_server_listen) M("close", js_server_close) \
    M("on", js_server_on) M("once", js_server_once) M("listeners", js_server_listeners) \
    M("removeListener", js_server_removeListener) M("off", js_server_removeListener) \
    M("address", js_server_address) M("ref", js_server_ref) M("unref", js_server_unref) \
    M("getConnections", js_server_getConnections)
#define JS_NET_INSTALL_SERVER_METHOD(name, target) net_set_native(obj_root.get(), name, target);
    JS_NET_SERVER_METHODS(JS_NET_INSTALL_SERVER_METHOD)
#undef JS_NET_INSTALL_SERVER_METHOD
#undef JS_NET_SERVER_METHODS
    js_set_key_cstr(obj_root.get(), "allowHalfOpen", (Item){.item = b2it(srv->allow_half_open)});
    js_set_key_cstr(obj_root.get(), "keepAlive", (Item){.item = b2it(srv->keep_alive)});
    js_set_key_cstr(obj_root.get(), "keepAliveInitialDelay", (Item){.item = i2it(srv->keep_alive_initial_delay_ms)});
    js_set_key_cstr(obj_root.get(), "noDelay", (Item){.item = b2it(srv->no_delay)});
    js_set_key_cstr(obj_root.get(), "pauseOnConnect", (Item){.item = b2it(srv->pause_on_connect)});
    js_set_key_cstr(obj_root.get(), "listening", (Item){.item = ITEM_FALSE});
    if (srv->has_block_list) {
        js_set_key_cstr(obj_root.get(), "__block_list__", NET_SERVER_VALUE(srv, BLOCK_LIST));
    }
    ...
    NET_SERVER_SET_VALUE(srv, OBJECT, obj_root.get());
...
static Item make_server_object_from_fd(uv_loop_t* loop, int fd) {
    ...
    js_set_key_cstr(obj, "listening", (Item){.item = ITEM_TRUE});
    return obj;
}
    ...
    if (get_type_id(options) == LMD_TYPE_MAP ||
        ...
        if (sock->adopted_bound_socket) {
            ...
            if (get_type_id(native_handle) != LMD_TYPE_INT) {
                js_set_key_cstr(handle, "__socket_object__", obj);
                net_set_native(handle, "onread", js_socket_js_handle_onread);
            }
            sock->handle_exposed = true;
        }
        Item readable = js_get_key_cstr(options, "readable");
        if (get_type_id(readable) == LMD_TYPE_BOOL) {
            js_set_key_cstr(obj, "readable", readable);
        }
        Item writable = js_get_key_cstr(options, "writable");
...
static Item js_block_list_add(Item address, Item prefix, Item type, bool subnet) {
    ...
    NetBlockListEntry* stored = (NetBlockListEntry*)mem_calloc(1,
        sizeof(NetBlockListEntry), MEM_CAT_JS_RUNTIME);
    if (!stored) return js_throw_range_error("Cannot grow BlockList");
    *stored = entry;
    if (!arraylist_append(list->entries, stored)) {
        mem_free(stored);
        return js_throw_range_error("Cannot grow BlockList");
    }
    return self;
}
...
extern "C" Item js_net_BlockList(Item options) {
    ...
    if (list) {
        js_set_key_cstr(obj, "__net_block_list__", (Item){.item = i2it((int64_t)(uintptr_t)list)});
    }
#define JS_NET_INSTALL_BLOCK_LIST_METHOD(name, target) net_set_native(obj, name, target);
    JS_NET_INSTALL_BLOCK_LIST_METHOD("addAddress", js_block_list_addAddress)
    JS_NET_INSTALL_BLOCK_LIST_METHOD("addSubnet", js_block_list_addSubnet)
    JS_NET_INSTALL_BLOCK_LIST_METHOD("check", js_block_list_check)
#undef JS_NET_INSTALL_BLOCK_LIST_METHOD
    return obj;
}
...
static Item js_stream_wrap_emit_terminal(Item env_item, bool close_event) {
    ...
    js_set_key_cstr(wrap, "readable", (Item){.item = ITEM_FALSE});
    if (close_event) {
        js_set_key_cstr(wrap, "writable", (Item){.item = ITEM_FALSE});
        js_set_key_cstr(wrap, "destroyed", (Item){.item = ITEM_TRUE});
    }
...
static Item js_stream_wrap_destroy(void) {
    Item self = js_get_this();
    if (js_get_key_cstr(self, "__stream_wrap_closed__").item == ITEM_TRUE) {
        return self;
    }
    Item stream = js_get_key_cstr(self, "stream");
    Item destroy = js_get_key_cstr(stream, "destroy");
...
extern "C" Item js_internal_js_stream_socket_constructor(Item stream) {
    ...
    js_set_key_cstr(wrap, "stream", stream);
    js_set_key_cstr(wrap, "readable", js_get_key_cstr(stream, "readable"));
    js_set_key_cstr(wrap, "writable", js_get_key_cstr(stream, "writable"));
    js_set_key_cstr(wrap, "destroyed", (Item){.item = ITEM_FALSE});
    js_set_key_cstr(wrap, "__stream_wrap_ended__", (Item){.item = ITEM_FALSE});
    js_set_key_cstr(wrap, "__stream_wrap_closed__", (Item){.item = ITEM_FALSE});
...
extern "C" Item js_get_internal_js_stream_socket_constructor(void) {
    ...
    js_set_key_cstr(internal_js_stream_socket_ctor, "StreamWrap", internal_js_stream_socket_ctor);
    js_set_key_cstr(internal_js_stream_socket_ctor, "default", internal_js_stream_socket_ctor);
...
extern "C" Item js_get_net_namespace(void) {
    ...
    js_set_key_cstr(bound_socket_root.get(), "connect", socket_root.get());
    js_set_key_cstr(stream_root.get(), "prototype", bound_socket_root.get());
    ...
    js_set_key_cstr(create_server_root.get(), "prototype", block_list_root.get());
...
extern "C" void js_net_reset(void) {
    ...
    NetRealmItems items = {};
    if (net_realm_items(&items, false)) {
        *items.namespace_object = (Item){0};
        *items.stream_socket_constructor = (Item){0};
        *items.socket_prototype = (Item){0};
        *items.server_prototype = (Item){0};
        *items.socket_connect_function = (Item){0};
    }
    JsNetRuntimeState* state = (JsNetRuntimeState*)js_runtime_state.net_native.native_state;
    if (!state) return;
    net_server_release_closed(state);
    net_block_list_instances_clear(state);
...
extern "C" void js_net_destroy_context(JsRuntimeState* runtime_state) {
    ...
    JsNetRuntimeState* state = (JsNetRuntimeState*)runtime_state->net_native.native_state;
    net_server_release_closed(state);
    net_block_list_instances_clear(state);
    mem_free(state);
    runtime_state->net_native.native_state = NULL;
}
