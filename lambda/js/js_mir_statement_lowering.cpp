...
static void jm_transpile_loop_body(JsMirTranspiler* mt, JsAstNode* body) {
    ...
    if (body->node_type == JS_AST_NODE_BLOCK_STATEMENT) {
        ...
        for (JsAstNode* statement = block->statements; statement;
            ...
            jm_transpile_statement(mt, statement);
        }
        jm_pop_scope(mt);
    } else {
        jm_transpile_statement(mt, body);
    }
}

static void jm_install_for_generator_var(JsMirTranspiler* mt, MIR_reg_t value,
        char name_prefix, int env_slot) {
...
static void jm_bind_module_import_value(JsMirTranspiler* mt, const char* vname,
        MIR_reg_t value, NameEntry* binding) {
    MIR_reg_t var_reg = jm_new_reg(mt, vname, MIR_T_I64);
    jm_emit_mov(mt, var_reg, value);
    jm_set_var(mt, vname, var_reg, MIR_T_I64, LMD_TYPE_ANY, binding);
    JsModuleConstEntry* mce = jm_find_module_const_by_binding(mt, binding);
    if (mce && mce->const_type == MCONST_MODVAR) {
        jm_store_module_var(mt, (uint32_t)mce->int_val, value);
    }
}
...
static void jm_bind_catch_destructure(JsMirTranspiler* mt,
    ...
    while (hashmap_iter(catch_names, &name_iter, &name_item)) {
        ...
        jm_emit_mov(mt, preg, undef_val);
        jm_set_var(mt, ne->name, preg, MIR_T_I64, LMD_TYPE_ANY, ne->entry);
        JsMirVarEntry* ve = jm_find_var_by_binding(mt, ne->entry);
        if (ve) {
            ve->is_let_const = true;
            ve->from_catch_param = true;
        }
        jm_scope_env_mark_and_writeback_entry(mt, ne->name, ne->entry, preg);
    }
...
static JsMirVarEntry* jm_find_nearest_catch_param_var(JsMirTranspiler* mt, const char* name) {
    ...
    for (int depth = start_depth; depth >= 0; depth--) {
        JsMirVarEntry* var = jm_find_var_at(mt, name, depth);
...
void jm_write_last_closure_capture_if_matching(JsMirTranspiler* mt,
    ...
    for (int i = 0; i < mt->tdz_closure_capture_count; i++) {
        ...
        if (tracked->binding_scope_depth != mt->scope_depth ||
            tracked->binding != binding) continue;
...
static void jm_write_env_backing_if_needed(JsMirTranspiler* mt, JsMirVarEntry* var,
    ...
    jm_emit_store_i64(mt, var->env_slot * (int)sizeof(uint64_t), var->env_reg, val);
}

static void jm_emit_for_loop_var_writeback(JsMirTranspiler* mt,
        const char* var_name, int var_len, NameEntry* binding,
        bool is_let_const_loop, MIR_reg_t loop_var) {
    if (!var_name || is_let_const_loop) return;
    const char* wb_vname = jm_var_name(var_name, var_len);
    if (mt->module_consts) {
        JsModuleConstEntry* mc = jm_find_module_const_by_binding(mt, binding);
        JsMirVarEntry* local_var = jm_find_var_by_binding(mt, binding);
        bool is_function_local = local_var && jm_has_current_source_function(mt);
        if (!is_function_local && mc && mc->const_type == MCONST_MODVAR) {
            jm_store_module_var(mt, (uint32_t)mc->int_val, loop_var);
    ...
    if (binding) {
        jm_scope_env_mark_and_writeback_entry(mt, wb_vname, binding, loop_var);
    }
}
...
static bool jm_assignment_targets_binding(JsAstNode* left, NameEntry* binding) {
    ...
    return js_ast_any_binding_pattern_child(left, jm_assignment_target_child,
        &find);
}

static void jm_scope_env_mark_pattern_binding_child(JsAstNode* child,
        void* opaque) {
    jm_writeback_scope_env_pattern_bindings((JsMirTranspiler*)opaque, child);
}
...
void jm_writeback_scope_env_pattern_bindings(JsMirTranspiler* mt, JsAstNode* pat) {
    ...
    case JS_AST_NODE_IDENTIFIER: {
        ...
        JsMirVarEntry* ve = jm_find_var_by_binding(mt, id->entry);
        if (ve) {
    ...
    default:
        js_ast_visit_binding_pattern_children(pat,
            jm_scope_env_mark_pattern_binding_child, mt);
        return;
    }
}

struct JmNativeMutationScan {
    JsMirTranspiler* mt;
    NameEntry* binding;
    TypeId native_type;
    AstNodeId root_id;
...
static bool jm_native_mutation_is_relevant(const AstIndex* index,
        AstNodeId node_id, const JmNativeMutationScan* scan) {
    for (AstNodeId current = node_id; current != scan->root_id;
...
static bool jm_mutable_native_var_needs_boxing(JsMirTranspiler* mt,
    ...
    AstNodeId root_id = ast_index_find(index,
        (AstNode*)mt->current_fc->node->body);
    if (root_id == AST_NODE_ID_INVALID || !id->entry) return false;
    JmNativeMutationScan scan = {mt, id->entry, native_type, root_id,
        index->owner_functions[root_id], false};
    if (!ast_index_visit_subtree(index, root_id, jm_scan_native_mutation, &scan) &&
            !scan.needs_boxing) {
        log_error("js-mir: unable to scan native mutation bindings");
    }
    return scan.needs_boxing;
}
...
static void jm_define_global_var_property_for_main_var(JsMirTranspiler* mt,
    ...
    jm_callr_2(mt, "js_set_global_var_property_fast", MIR_T_I64, key_reg, value);
    jm_emit_error_lane_propagate_check(mt);
}
...
static void jm_declare_evalscript_global_lexical_if_needed(JsMirTranspiler* mt,
    ...
    jm_emit_branch(mt, MIR_BF, skip_global_lex, evalscript_active);
    MIR_reg_t key_reg = jm_box_property_name_literal(mt,
        id->name->chars, id->name->len);
...
// A lexical declaration leaves its TDZ only after its initializer is stored.
static void jm_mark_lexical_binding_initialized(JsMirTranspiler* mt,
        JsVariableDeclarationNode* decl, NameEntry* binding) {
    if (!mt || !decl || !binding ||
            (decl->kind != JS_VAR_LET && decl->kind != JS_VAR_CONST)) return;
    JsMirVarEntry* variable = jm_find_var_by_binding(mt, binding);
    if (!variable) return;
    variable->is_let_const = true;
    variable->is_const = decl->kind == JS_VAR_CONST;
    variable->tdz_active = false;
}

static void jm_initialize_native_declarator(JsMirTranspiler* mt,
        JsVariableDeclarationNode* declaration, JsVariableDeclaratorNode* declarator,
        JsIdentifierNode* identifier, const char* name, TypeId type_id) {
    MIR_type_t mir_type = type_id == LMD_TYPE_FLOAT ? MIR_T_D : MIR_T_I64;
    MIR_reg_t reg = jm_new_reg(mt, name, mir_type);
    MIR_reg_t value = jm_transpile_as_native(mt, declarator->init, type_id);
    if (type_id == LMD_TYPE_FLOAT) {
        jm_emit_dmov(mt, reg, value);
    } else {
        jm_emit_mov(mt, reg, value);
    }
    jm_set_var(mt, name, reg, mir_type, type_id, identifier->entry);
    JsMirVarEntry* variable = jm_find_var_by_binding(mt, identifier->entry);
    jm_mark_lexical_binding_initialized(mt, declaration, identifier->entry);
    jm_write_env_backing_if_needed(mt, variable, reg, type_id);
    jm_scope_env_mark_and_writeback_binding(mt, name, declarator->id, reg, type_id);
    jm_write_last_closure_capture_if_matching(mt, identifier->entry, reg, type_id);

    bool direct_eval_lexical = (declaration->kind == JS_VAR_LET ||
        declaration->kind == JS_VAR_CONST) && mt->is_eval_direct;
    bool main_var = declaration->kind == JS_VAR_VAR && mt->in_main && !mt->is_module;
    if (!direct_eval_lexical && !main_var) return;
    MIR_reg_t boxed = jm_box_native(mt, reg, type_id);
    if (direct_eval_lexical) {
        jm_declare_evalscript_global_lexical_if_needed(mt, declaration, identifier, boxed);
    }
    if (main_var) {
        jm_define_global_var_property_for_main_var(mt, declaration, identifier, boxed);
    }
}
...
static void jm_writeback_pattern_bindings(JsMirTranspiler* mt,
    ...
    if (pattern_at_top_for_writeback &&
        (mt->in_main || jm_current_function_is_iife_body(mt)) && mt->module_consts) {
        ...
        while (hashmap_iter(pat_names, &piter, &pitem)) {
            ...
            JsModuleConstEntry* mc = jm_find_module_const_by_binding(mt, ne->entry);
            ...
            if (in_modvar_scope && mc && mc->const_type == MCONST_MODVAR) {
                JsMirVarEntry* ve = jm_find_var_by_binding(mt, ne->entry);
                if (ve) {
                    jm_store_module_var(mt, (uint32_t)mc->int_val, ve->reg);
                }
            }
        }
            ...
            if (d->id && d->id->node_type == JS_AST_NODE_IDENTIFIER) {
                ...
                if (var->kind == JS_VAR_VAR && d->init && mt->scope_depth >= 0) {
                    ...
                    if (catch_param_var && catch_param_var->from_catch_param && catch_param_var->reg) {
                        ...
                        jm_emit_mov(mt, catch_param_var->reg, boxed_val);
                        jm_scope_env_mark_and_writeback_entry(mt, vname,
                            catch_param_var->binding, catch_param_var->reg);
                        decl = decl->next;
                        continue;
                    }
                }
                ...
                if (mt->module_consts && var->kind != JS_VAR_CONST) {
                    ...
                    if (mc && mc->const_type == MCONST_MODVAR && at_top && !local_var_hoist &&
                        (mt->in_main || (mc->is_iife_var && jm_current_function_is_iife_body(mt)))) {
                        is_modvar = true;
                        modvar_index = (int)mc->int_val;
                    }
                    if (is_modvar && mc->is_iife_var) {
                        JsMirVarEntry* direct_binding = jm_find_var_by_binding(mt,
                            id->entry);
                        if (direct_binding) {
                ...
                if (var->kind == JS_VAR_VAR && d->init && mt->with_depth > 0) {
                    ...
                    if (is_modvar) {
                        ...
                        if (existing_modvar_local && existing_modvar_local->reg &&
                            ...
                            jm_emit_mov(mt, existing_modvar_local->reg, boxed_val);
                        }
                        if (mt->in_main && !mt->is_eval_direct) {
                            ...
                            jm_call_void_3(mt, "js_define_global_property_v",
                                ...
                                MIR_T_I64, MIR_new_reg_op(mt->ctx, key_reg),
                                MIR_T_I64, MIR_new_reg_op(mt->ctx, boxed_val));
                            jm_callr_2(mt, "js_set_global_var_property_fast", MIR_T_I64, key_reg, boxed_val);
                            jm_emit_error_lane_propagate_check(mt);
                        }
                        jm_scope_env_mark_and_writeback_binding(mt, vname,
                            d->id, boxed_val);
                    } else {
                        JsMirVarEntry* existing_var = jm_find_var_by_binding(mt,
                            id->entry);
                        if (existing_var && existing_var->reg && existing_var->from_hoist) {
                            // The hoist prepass owns storage, while this
                            // declarator owns its resolved source binding.
                            existing_var->binding = id->entry;
                            jm_emit_mov(mt, existing_var->reg, boxed_val);
                            jm_write_env_backing_if_needed(mt, existing_var, boxed_val, LMD_TYPE_ANY);
                            jm_scope_env_mark_and_writeback_binding(mt, vname,
                                d->id, existing_var->reg);
                            jm_define_global_var_property_for_main_var(mt, var, id, boxed_val);
                        } else {
                            MIR_reg_t reg = jm_new_reg(mt, vname, MIR_T_I64);
                            jm_emit_mov(mt, reg, boxed_val);
                            jm_set_var(mt, vname, reg, MIR_T_I64, LMD_TYPE_ANY,
                                id->entry);
                            jm_write_env_backing_if_needed(mt,
                                jm_find_var_by_binding(mt, id->entry), reg,
                                LMD_TYPE_ANY);
                            jm_scope_env_mark_and_writeback_binding(mt, vname,
                                d->id, reg);
                            jm_define_global_var_property_for_main_var(mt, var, id, boxed_val);
                ...
                if (!with_var_init_handled && is_modvar) {
                    ...
                    if (d->init) {
                        ...
                        if (mt->is_eval_direct && (var->kind == JS_VAR_LET || var->kind == JS_VAR_CONST)) {
                            ...
                            MIR_label_t skip_global_lex = jm_new_label(mt);
                            jm_emit_branch(mt, MIR_BF, skip_global_lex, eval_env_active);
                            MIR_reg_t key_reg = jm_box_property_name_literal(mt,
                                id->name->chars, id->name->len);
                        ...
                        jm_store_module_var(mt, (uint32_t)modvar_index, undef_reg);
                    ...
                    if (jm_is_native_type(init_type) && !mt->in_generator) {
                        ...
                        if (fc && fc->has_scope_env) {
                            for (int s = 0; s < fc->scope_env_count; s++) {
                                if (jm_scope_env_binding_at(fc, s) == id->entry) {
                    ...
                    if (init_type == LMD_TYPE_INT) {
                        jm_initialize_native_declarator(mt, var, d, id, vname, init_type);
                        ...
                        JsMirVarEntry* jube_var = jm_find_var_by_binding(mt,
                            id->entry);
                        if (jube_var) jube_var->jube_type =
                            jm_infer_jube_type(mt, d->init);
                        jm_mark_lexical_binding_initialized(mt, var, id->entry);
                        jm_write_env_backing_if_needed(mt,
                            jm_find_var_by_binding(mt, id->entry), reg, init_type);
                        jm_scope_env_mark_and_writeback_binding(mt, vname, d->id, reg, init_type);
                        jm_write_last_closure_capture_if_matching(mt, id->entry, reg,
                            init_type);
                        ...
                        if (d->declared_type &&
                            ...
                            if (resolved->type_id == LMD_TYPE_MAP) {
                                JsMirVarEntry* var_entry = jm_find_var_by_binding(mt,
                                    id->entry);
                                if (var_entry) {
                    ...
                    bool skip_init = (var->kind == JS_VAR_VAR) &&
                        jm_find_var_by_binding(mt, id->entry);
                    if (!skip_init) {
                        MIR_reg_t reg = jm_new_reg(mt, vname, MIR_T_I64);
                        jm_emit_reg_op(mt, MIR_MOV, reg, MIR_new_int_op(mt->ctx, (int64_t)ITEM_JS_UNDEFINED));
                        jm_set_var(mt, vname, reg, MIR_T_I64, LMD_TYPE_ANY,
                            id->entry);
                        jm_mark_lexical_binding_initialized(mt, var, id->entry);
                        jm_write_env_backing_if_needed(mt,
                ...
                if (!with_var_init_handled && !is_modvar && at_top_for_writeback && mt->module_consts) {
                    ...
                    if (in_modvar_scope && mc && mc->const_type == MCONST_MODVAR) {
                        ...
                        if (ve) {
                            ...
                            jm_store_module_var(mt, (uint32_t)mc->int_val, boxed_val);
                        }
                    }
...
bool jm_push_typeof_narrow(JsMirTranspiler* mt, JsIdentifierNode* id, TypeId narrowed_type) {
    if (!id) return false;
    const char* vname = jm_var_name(id->name);
    JsMirVarEntry* orig = jm_find_var_by_binding(mt, id->entry);
    ...
    if (narrowed_type == LMD_TYPE_FLOAT) {
        ...
        jm_emit_dmov(mt, narrow_reg, unboxed);
        jm_push_scope(mt);
...
static void jm_init_if_clause_function_binding(JsMirTranspiler* mt, JsAstNode* stmt) {
    ...
    jm_set_var(mt, vname, fn_reg, MIR_T_I64, LMD_TYPE_ANY, fn->entry);
    JsMirVarEntry* ve = jm_find_var_by_binding(mt, fn->entry);
...
static void jm_emit_annexb_global_export(JsMirTranspiler* mt,
    ...
    if (mt->is_eval_direct && !mt->is_global_strict && !mt->is_module) {
        ...
        jm_emit_branch(mt, MIR_BF, global_set, eval_env_active);
        MIR_reg_t bridged_binding = jm_callr_1(mt, "js_eval_env_has_binding", MIR_T_I64, key_reg);
        jm_emit_branch(mt, MIR_BF, local_set, bridged_binding);
        ...
        jm_call_3(mt, "js_set_global_property", MIR_T_I64,
            ...
            MIR_T_I64, MIR_new_int_op(mt->ctx, 0));
        jm_emit_error_lane_propagate_check(mt);
        jm_emit_jmp(mt, set_done);
        jm_emit_label(mt, local_set);
        jm_callr_void_2(mt, "js_eval_local_export_var", key_reg, value_reg);
...
static void jm_reload_typed_scope_value(JsMirTranspiler* mt,
    ...
    if (entry->var.type_id == LMD_TYPE_INT) {
        ...
        jm_emit_mov(mt, entry->var.reg, unboxed);
    } else if (entry->var.type_id == LMD_TYPE_FLOAT) {
        MIR_reg_t unboxed = jm_emit_unbox_float(mt, boxed);
        jm_emit_dmov(mt, entry->var.reg, unboxed);
    } else {
        jm_emit_mov(mt, entry->var.reg, boxed);
    }
}

void jm_scope_env_reload_vars(JsMirTranspiler* mt) {
    bool reload_iife_modvars = mt->module_consts && jm_current_function_is_iife_body(mt);
        ...
        while (hashmap_iter(scope, &iter, &entry_ptr)) {
            ...
            if (e->var.in_scope_env) {
                ...
                if (env_reg != 0) {
                    ...
                    jm_emit_load_i64(mt, boxed, slot * (int)sizeof(uint64_t), env_reg);
            ...
            if (reload_iife_modvars && e->var.reg != 0) {
                ...
                if (mc && mc->const_type == MCONST_MODVAR && mc->is_iife_var &&
                    ...
                    MIR_reg_t boxed = jm_load_module_var(mt, (uint32_t)mc->int_val);
                    jm_reload_typed_scope_value(mt, e, boxed);
                }
            }
        }
...
void jm_env_reload_shared_captures(JsMirTranspiler* mt) {
    for (int sd = 0; sd <= mt->scope_depth; sd++) {
        ...
        while (hashmap_iter(scope, &iter, &entry_ptr)) {
            ...
            jm_emit_load_i64(mt, boxed, slot * (int)sizeof(uint64_t), e->var.env_reg);
            // Captured vars are always boxed Items (no unboxing needed)
            jm_emit_mov(mt, e->var.reg, boxed);
        }
    }
}
...
void jm_transpile_for(JsMirTranspiler* mt, JsForNode* for_node) {
    ...
    if (for_node->init && !init_is_lexical_decl) {
        if (for_node->init->node_type == JS_AST_NODE_VARIABLE_DECLARATION) {
            ...
            (void)jm_transpile_expression_value(mt, for_node->init,
                MIR_VALUE_DISCARD);
    ...
    if (for_node->update) {
        ...
        if (init_is_lexical_decl && for_lexical_init_name &&
            ...
            if (loop_var && loop_var->reg) {
                jm_write_last_closure_capture_if_matching(mt, loop_var->binding,
    ...
    if (init_is_var && for_var_init_name && for_var_init_name[0]) {
        ...
        if (init_var && init_var->type_id == LMD_TYPE_INT && !init_var->from_env) {
            ...
            jm_emit_mov(mt, init_var->reg, boxed_counter);
...
static MIR_reg_t jm_emit_class_static_field_value(JsMirTranspiler* mt,
    ...
    if (previous_private_home) {
        val = jm_callr_2(mt, "js_private_home_class_leave_result", MIR_T_I64, previous_private_home, val);
    }
    jm_emit_error_lane_propagate_check(mt);
    return val;
}
...
void jm_emit_class_static_field(JsMirTranspiler* mt, MIR_reg_t cls_obj, JsClassEntry* ce, JsStaticFieldEntry* sf) {
    ...
    if (sf->computed && sf->key_expr) {
        ...
        if (sf->key_module_var_index >= 0) {
            key = jm_load_module_var(mt, (uint32_t)sf->key_module_var_index);
        } else {
        ...
        jm_callr_void_2(mt, "js_set_function_name_if_anonymous", val, key);
    ...
    if (sf->name) {
        ...
        jm_emit_error_lane_propagate_check(mt);
        jm_callr_void_2(mt, "js_set_function_name_if_anonymous", val, key);
        jm_callr_3(mt, "js_create_data_property", MIR_T_I64, cls_obj, key, val);
    }
    (void)ce;
}
...
void jm_emit_class_static_block(JsMirTranspiler* mt, MIR_reg_t cls_obj,
    ...
    jm_init_block_tdz(mt, block);
    {
        ...
        while (hashmap_iter(static_vars, &iter, &item)) {
            ...
            if (!entry || jm_find_var_at(mt, entry->name, mt->scope_depth)) continue;
            MIR_reg_t undef = jm_emit_undefined(mt);
            jm_set_var(mt, entry->name, undef, MIR_T_I64, LMD_TYPE_ANY);
            JsMirVarEntry* local = jm_find_var_at(mt, entry->name, mt->scope_depth);
...
void jm_emit_class_static_initializers(JsMirTranspiler* mt, MIR_reg_t cls_obj, JsClassEntry* ce,
    ...
    jm_callr_void_1(mt, "js_set_this", cls_obj);
    MIR_reg_t static_new_target = jm_emit_undefined(mt);
    jm_callr_void_1(mt, "js_set_direct_new_target", static_new_target);
...
MIR_reg_t jm_emit_current_class_prototype(JsMirTranspiler* mt, MIR_reg_t cls_obj,
    ...
    MIR_reg_t current = jm_callr_2(mt, "js_get_key_default", MIR_T_I64, cls_obj, proto_key);
...
void jm_emit_class_length_property(JsMirTranspiler* mt, MIR_reg_t cls_obj,
    ...
    jm_emit_reg_op(mt, MIR_MOV, len_val, MIR_new_int_op(mt->ctx, (int64_t)i2it(ctor_len)));
    jm_callr_3(mt, "js_set_key_default", MIR_T_I64, cls_obj, len_key, len_val);
    jm_callr_void_2(mt, "js_mark_non_writable", cls_obj, len_key);
...
void jm_emit_class_prototype_properties(JsMirTranspiler* mt, MIR_reg_t cls_obj,
        MIR_reg_t proto_obj) {
    jm_callr_void_2(mt, "js_set_class_instance_prototype", cls_obj, proto_obj);
    MIR_reg_t prototype_key = jm_box_property_name_literal(mt, "prototype", 9);
    jm_callr_3(mt, "js_set_key_default", MIR_T_I64, cls_obj, prototype_key, proto_obj);
    jm_callr_void_2(mt, "js_mark_non_writable", cls_obj, prototype_key);
    jm_callr_void_2(mt, "js_mark_non_enumerable", cls_obj, prototype_key);
    jm_callr_void_2(mt, "js_mark_non_configurable", cls_obj, prototype_key);
}
...
void jm_emit_class_setup(JsMirTranspiler* mt, MIR_reg_t cls_obj, JsClassEntry* ce,
    ...
    jm_emit_class_prototype_properties(mt, cls_obj, setup->class_proto_obj);
...
// Declaration paths differ only in binding publication; member construction is shared.
void jm_emit_class_members(JsMirTranspiler* mt, MIR_reg_t cls_obj, JsClassEntry* ce,
        JsAstNode* class_node, bool computed_key_before_function) {
    JsMirClassSetup setup;
    jm_emit_class_setup(mt, cls_obj, ce, class_node, computed_key_before_function, &setup);
    MIR_reg_t proto_obj = setup.class_proto_obj;
    jm_callr_void_2(mt, "js_set_default_constructor_property", proto_obj, cls_obj);
    bool heritage_is_null = false;
    MIR_reg_t ctor_super_val = jm_emit_class_prototype_chain(mt, ce, cls_obj,
        setup.heritage, setup.static_superclass, proto_obj, 0, &heritage_is_null);
...
// Construction owns its Item carrier so callers never recover it from a raw
// register after the construct operation (D2.4.1–D2.4.3, D8.2.6).
MirValue jm_emit_new_value(JsMirTranspiler* mt, JsCallNode* call) {
    MIR_reg_t result = call->callee
        ? jm_emit_dynamic_new_expr(mt, call, ast_linked_node_count(call->arguments))
        : jm_emit_null(mt);
    ...
    return em_value_for_rep(result, LMD_TYPE_ANY, VALUE_REP_ITEM,
        ((JsAstNode*)call)->type, (JsAstNode*)call);
}
// switch statement
...
static void jm_precreate_loop_binding(JsMirTranspiler* mt, const char* vname,
        NameEntry* binding, bool is_let_const_loop, bool mark_tdz) {
    JsMirVarEntry* existing = is_let_const_loop ? NULL :
        jm_find_var_by_binding(mt, binding);
    if (existing && existing->reg) return;
    MIR_reg_t preg = jm_new_reg(mt, vname, MIR_T_I64);
    jm_set_var(mt, vname, preg, MIR_T_I64, LMD_TYPE_ANY, binding);
    if (is_let_const_loop) {
        JsMirVarEntry* ve = jm_find_var_by_binding(mt, binding);
        if (ve) {
            ve->is_let_const = true;
            if (mark_tdz) ve->tdz_active = true;
        }
    }
    jm_emit_reg_op(mt, MIR_MOV, preg, MIR_new_int_op(mt->ctx, is_let_const_loop ? (int64_t)ITEM_JS_TDZ : ITEM_NULL_VAL));
}
...
void jm_transpile_for_of(JsMirTranspiler* mt, JsForOfNode* fo) {
    ...
    JsIdentifierNode* loop_identifier = NULL;
    ...
    if (var_name) {
        ...
        JsMirVarEntry* existing = is_let_const_loop ? NULL :
            jm_find_var_by_binding(mt, loop_identifier ? loop_identifier->entry : NULL);
        if (existing && existing->reg) {
            loop_var = existing->reg;
        } else {
            loop_var = jm_new_reg(mt, vname, MIR_T_I64);
            jm_set_var(mt, vname, loop_var, MIR_T_I64, LMD_TYPE_ANY,
                loop_identifier ? loop_identifier->entry : NULL);
        }
    ...
    if (is_for_in && var_name && !is_let_const_loop) {
        ...
        if (init_expr) {
            ...
            jm_scope_env_mark_and_writeback_binding(mt, init_vname,
                (JsAstNode*)loop_identifier, loop_var);
        }
    }
    ...
    if (is_for_in) {
        ...
        MIR_reg_t collection = jm_callr_1(mt, "js_for_in_keys", MIR_T_I64, iterable);

        MIR_reg_t len = jm_callr_1(mt, "js_array_length", MIR_T_I64, collection);

        MIR_reg_t idx = jm_new_reg(mt, "foridx", MIR_T_I64);
        jm_emit_reg_op(mt, MIR_MOV, idx, MIR_new_int_op(mt->ctx, 0));
        ...
        jm_emit_reg_binary(mt, MIR_LTS, cmp, idx, len);
        jm_emit_branch(mt, MIR_BF, l_end, cmp);

        MIR_reg_t idx_item = jm_box_int_reg(mt, idx);
        MIR_reg_t elem = jm_callr_2(mt, "js_get_reference", MIR_T_I64, collection, idx_item);
        MIR_reg_t live_key = jm_callr_2(mt, "js_for_in_key_is_live", MIR_T_I64, iterable, elem);
        jm_emit_branch(mt, MIR_BF, l_update, live_key);
        if (lhs_call_target) {
    ...
    MIR_reg_t iterator = is_for_await
        ? jm_callr_1(mt, "js_get_async_iterator", MIR_T_I64, iterable)
    ...
    if (is_for_await) {
        step_iter_result = jm_callr_1(mt, "js_async_iterator_step_result", MIR_T_I64, iterator);
    ...
    jm_emit_branch(mt, MIR_BT, l_end, is_done);
    if (is_for_await) {
        step_result = jm_callr_1(mt, "js_iterator_result_value", MIR_T_I64, step_iter_result);
    ...
    if (lhs_call_target) {
        ...
        jm_emit_mov(mt, loop_var, step_result);
    }
    if (loop_var_env_slot >= 0) {
        ...
        jm_emit_store_i64(mt, loop_var_env_slot * (int)sizeof(uint64_t), mt->gen_env_reg, loop_var);
    }
    ...
    if (pushed_try) {
        ...
        jm_emit_branch(mt, MIR_BF, l_no_delayed_ret, forit_has_return);
    ...
    if (mt->in_native_func && mt->current_fc) {
        ...
        if (ret->argument) {
            ...
            if (jm_is_native_type(expr_type)) {
                ...
                val = jm_transpile_as_native(mt, ret->argument, ret_type);
            } else {
            ...
            if (ret_type == LMD_TYPE_FLOAT) {
                jm_emit_reg_op(mt, MIR_DMOV, val, MIR_new_double_op(mt->ctx, 0.0));
            } else {
                jm_emit_reg_op(mt, MIR_MOV, val, MIR_new_int_op(mt->ctx, 0));
            }
        }
...
static void jm_transpile_using_tail(JsMirTranspiler* mt, JsAstNode* tail,
    ...
    jm_emit_reg_op(mt, MIR_MOV, return_val_reg, MIR_new_int_op(mt->ctx, 0));
    jm_emit_reg_op(mt, MIR_MOV, has_return_reg, MIR_new_int_op(mt->ctx, 0));

    if (JsTryContext* tc = jm_try_context_push(mt)) {
        jm_try_context_setup(tc, 0, finally_label, end_label,
            return_val_reg, has_return_reg, false, true, NULL, 0);
    }
    ...
    MIR_reg_t saved_error_lane_val = jm_callr_1(mt, "js_error_lane_payload", MIR_T_I64, saved_error_lane_source);
    ...
    jm_emit_branch(mt, MIR_BT, skip_restore, new_exc);
    jm_emit_branch(mt, MIR_BF, skip_restore, saved_error_lane_flag);
    jm_callr_1(mt, "js_throw_value", MIR_T_I64, saved_error_lane_val);
    jm_emit_label(mt, skip_restore);

    jm_emit_branch(mt, MIR_BT, end_label, has_return_reg);
...
void jm_transpile_statement(JsMirTranspiler* mt, JsAstNode* stmt) {
    ...
    case JS_AST_NODE_FUNCTION_DECLARATION: {
        ...
        if (fn_decl->name) {
            ...
            bool current_body_direct = enclosing_fn &&
                jm_direct_body_function_binding(enclosing_fn, fn_vname) != NULL;
            ...
            // Annex B publishes through the resolver-linked var binding. The
            // block declaration keeps its own lexical cell for closures made
            // before this source position, so update both identities below.
            NameEntry* lexical_binding = fn_decl->entry;
            NameEntry* publish_binding = lexical_binding &&
                lexical_binding->annex_b_outer_binding
                ? lexical_binding->annex_b_outer_binding : lexical_binding;
            JsMirVarEntry* lexical_existing = jm_find_var_by_binding(mt,
                lexical_binding);
            JsMirVarEntry* existing = jm_find_var_by_binding(mt,
                publish_binding);
            ...
            if (lexical_existing && lexical_existing->from_block_func_decl) {
                p19_block_func_existing = lexical_existing;
                if (existing == lexical_existing) existing = NULL;
            }
            ...
            if (!existing && mt->module_consts) {
                JsModuleConstEntry* mvc = jm_find_module_const_by_binding(mt,
                    publish_binding);
            ...
            if (fc_decl && fc_decl->func_item) {
                ...
                bool is_direct = jm_function_decl_is_direct_binding(fn_decl, true);
                ...
                if (existing && is_direct && !async_state_machine_body) {
                    ...
                    jm_scope_env_mark_and_writeback_entry(mt, fn_vname,
                        existing->binding, existing->reg);
                    break;
                }
                ...
                if (p19_block_func_existing) {
                    jm_emit_mov(mt, p19_block_func_existing->reg, fn_reg);
                    jm_scope_env_mark_and_writeback_entry(mt, fn_vname,
                        p19_block_func_existing->binding,
                        p19_block_func_existing->reg);
                }
                if (existing) {
                    // Update existing var-scoped binding with function value
                    jm_emit_mov(mt, existing->reg, fn_reg);
                    ...
                    jm_store_module_var(mt, (uint32_t)annexb_modvar->int_val, fn_reg);
                    ...
                    jm_emit_mov(mt, var_reg, fn_reg);
                    jm_set_var(mt, fn_vname, var_reg, MIR_T_I64, LMD_TYPE_ANY,
                        publish_binding);
                    jm_scope_env_mark_and_writeback_entry(mt, fn_vname,
                        publish_binding, var_reg);
                }
                ...
                if (mt->module_consts) {
                    JsModuleConstEntry* mvc = jm_find_module_const_by_binding(mt,
                        ...
                        if (mvc && mvc->const_type == MCONST_MODVAR && mvc->var_kind == 0 &&
                            mvc != annexb_modvar) {
                        jm_store_module_var(mt, (uint32_t)mvc->int_val, fn_reg);
        ...
        if (mt->module_consts) {
            ...
            if (cls_node->name) {
                ...
                if (ce) {
                    ...
                    JsMirVarEntry* local_class_binding = jm_find_var_by_binding(mt,
                    ...
                    jm_emit_class_members(mt, cls_obj, ce, (JsAstNode*)cls_node, true);
                }
            }
        }
        break;
    ...
    case JS_AST_NODE_EXPRESSION_STATEMENT: {
        ...
        if (es->expression) {
            ...
            uint32_t demand = mt->eval_completion_reg
                ? MIR_VALUE_ANY : MIR_VALUE_DISCARD;
            MirValue value = jm_transpile_expression_value(mt, es->expression,
                demand);
            mt->discarded_expression = saved_discarded;
    ...
    case JS_AST_NODE_TRY_STATEMENT: {
        ...
        if (try_context) {
            jm_try_context_setup(try_context, catch_label, finally_label, end_label,
                return_val_reg, has_return_reg, has_catch, has_finally,
                has_finally ? try_node->finalizer : NULL, incoming_error_lane_val_reg);
        }
        ...
        if (has_catch) {
            ...
            MIR_reg_t thrown_val = jm_callr_1(mt, "js_error_lane_payload", MIR_T_I64, routed_exc);
            ...
            if (has_finally) {
                ...
                if (tc) {
                jm_try_context_setup(tc, 0, finally_label, end_label,
                    return_val_reg, has_return_reg, false, true, try_node->finalizer,
                    catch_outgoing_exc_reg ? catch_outgoing_exc_reg : incoming_error_lane_val_reg);
                ...
                JsTryContext* tc = jm_try_context_push(mt);
                if (tc) {
                jm_try_context_setup(tc, 0, 0, 0, return_val_reg, has_return_reg,
                    false, false, NULL, incoming_error_lane_val_reg);
            ...
            if (catch_node->param && catch_node->param->node_type == JS_AST_NODE_IDENTIFIER) {
                ...
                jm_set_var(mt, vname, thrown_val, MIR_T_I64, LMD_TYPE_ANY,
                    param_id->entry);
                JsMirVarEntry* catch_entry = jm_find_var_by_binding(mt,
                    param_id->entry);
        ...
        if (has_finally) {
            ...
            if (mt->in_generator) {
                ...
                if (tc) {
                jm_try_context_setup(tc, 0, 0, 0, return_val_reg, has_return_reg,
                    false, true, NULL, incoming_error_lane_val_reg);
                tc->yield_state_only = true;
            ...
            if (mt->eval_completion_reg) {
                ...
                jm_emit_mov(mt, saved_cptn, mt->eval_completion_reg);
            ...
            if (incoming_error_lane_val_reg || catch_outgoing_exc_reg) {
                ...
                jm_emit_reg_binary_op(mt, MIR_URSH, saved_error_lane_tag, saved_error_lane_val, MIR_new_int_op(mt->ctx, 56));
                saved_error_lane_flag = jm_new_reg(mt, "fin_exc_set", MIR_T_I64);
                jm_emit_reg_binary_op(mt, MIR_EQ, saved_error_lane_flag, saved_error_lane_tag, MIR_new_int_op(mt->ctx, LMD_TYPE_ERROR));
            } else {
            ...
            // new returned lane from finally takes precedence (per spec).
            {
                ...
                jm_emit_branch(mt, MIR_BF, skip_restore, saved_error_lane_flag);
                MIR_reg_t restored_exception = jm_callr_1(mt, "js_throw_value", MIR_T_I64, saved_error_lane_val);
    ...
    case JS_AST_NODE_IMPORT_DECLARATION: {
        ...
        MIR_reg_t ns = jm_callr_1(mt, "js_module_get", MIR_T_I64, spec);
        ...
        if (imp->default_name) {
            ...
            if (is_self_import) {
                ...
                jm_set_var(mt, vname, var_reg, MIR_T_I64, LMD_TYPE_ANY,
                    imp->default_entry);
                JsMirVarEntry* lv = jm_find_var_by_binding(mt,
                    imp->default_entry);
                if (lv) {
                ...
                jm_bind_module_import_value(mt, vname, val, imp->default_entry);
            }
        }
        ...
        if (imp->namespace_name) {
            ...
            jm_bind_module_import_value(mt, vname, ns, imp->namespace_entry);
        }
