    ...
    for (int fi = 0; fi < ce->instance_field_count; fi++) {
        ...
        if (inf->computed && inf->key_expr) {
            ...
            if (private_field_name) {
                ...
                key = jm_box_property_name_literal(mt, field_name->chars, field_name->len);
            }
...
static void jm_emit_instance_fields_for_class(JsMirTranspiler* mt, MIR_reg_t obj,
        JsClassEntry* field_class, MIR_reg_t class_value,
        bool defer_private_fields, bool defer_all_fields) {
    if (!mt || !obj || !field_class) return;
    JsClassEntry* saved_current_class = mt->current_class;
    mt->current_class = field_class;
    if (!defer_private_fields && !defer_all_fields) {
        jm_emit_private_instance_method_brands(mt, obj, class_value, field_class);
    }
    for (int fi = 0; fi < field_class->instance_field_count; fi++) {
        JsInstanceFieldEntry* inf = &field_class->instance_fields[fi];
        if (defer_all_fields) continue;
        MIR_reg_t key;
        String* private_field_name = NULL;
        if (inf->computed && inf->key_expr) {
            int obj_spill = -1;
            if (inf->key_module_var_index < 0 && mt->in_generator &&
                    jm_has_yield(inf->key_expr)) {
                obj_spill = jm_gen_spill_save(mt, obj);
            }
            if (inf->key_module_var_index >= 0) {
                key = jm_call_1(mt, "js_get_module_var", MIR_T_I64,
                    MIR_T_I64, MIR_new_int_op(mt->ctx,
                        (int64_t)inf->key_module_var_index));
            } else {
                key = jm_transpile_box_item(mt, inf->key_expr);
            }
            if (obj_spill >= 0) {
        ...
        if (defer_private_fields && private_field_name) continue;
...
MIR_reg_t jm_emit_class_prototype_chain(JsMirTranspiler* mt, JsClassEntry* ce,
    ...
    if (static_superclass) {
        ctor_super_val = jm_link_static_super_prototype(mt, proto_obj, static_superclass);
    }
...
static JsClassEntry* jm_find_class_entry_by_node(JsMirTranspiler* mt, JsClassNode* cls) {
    if (!mt || !cls) return NULL;
    for (int ci = 0; ci < mt->class_count; ci++) {
        if (mt->class_entries[ci].node == cls) return &mt->class_entries[ci];
    }
    return NULL;
}
...
static JsClassEntry* jm_find_enclosing_class_for_new(JsMirTranspiler* mt, JsAstNode* node,
        JsAstNode* target, const char* name, int len);

static JsClassEntry* jm_find_enclosing_class_in_list(JsMirTranspiler* mt,
        JsAstNode* statements, JsAstNode* target, const char* name, int len) {
    JsClassEntry* candidate = NULL;
    for (JsAstNode* s = statements; s; s = s->next) {
        ...
        if (jm_node_contains_node(s, target)) {
            JsClassEntry* inner = jm_find_enclosing_class_for_new(mt, s, target, name, len);
...
static JsClassEntry* jm_find_enclosing_class_for_new(JsMirTranspiler* mt, JsAstNode* node,
        JsAstNode* target, const char* name, int len) {
    if (!mt || !node || !target || !name || len <= 0) return NULL;
    if (!jm_node_contains_node(node, target)) {
        return jm_find_enclosing_class_for_new(mt, node->next, target, name, len);
    }
    switch (node->node_type) {
    case JS_AST_NODE_PROGRAM: {
        JsProgramNode* program = (JsProgramNode*)node;
        return jm_find_enclosing_class_in_list(mt, program->body, target, name, len);
    }
    case JS_AST_NODE_BLOCK_STATEMENT: {
        JsBlockNode* block = (JsBlockNode*)node;
        return jm_find_enclosing_class_in_list(mt, block->statements, target, name, len);
    }
    case JS_AST_NODE_VARIABLE_DECLARATION: {
        JsVariableDeclarationNode* var = (JsVariableDeclarationNode*)node;
        for (JsAstNode* d = var->declarations; d; d = d->next) {
            JsClassEntry* inner = jm_find_enclosing_class_for_new(mt, d, target, name, len);
    ...
    case JS_AST_NODE_VARIABLE_DECLARATOR: {
        ...
        return jm_find_enclosing_class_for_new(mt, decl->init, target, name, len);
    }
    case JS_AST_NODE_EXPRESSION_STATEMENT: {
        JsExpressionStatementNode* expr = (JsExpressionStatementNode*)node;
        return jm_find_enclosing_class_for_new(mt, expr->expression, target, name, len);
    }
    default:
        return NULL;
    }
...
static JsClassEntry* jm_find_preceding_class_for_new(JsMirTranspiler* mt, JsAstNode* target,
        const char* name, int len) {
    if (!mt || !target || ts_node_is_null(target->node) || !name || len <= 0) return NULL;
    uint32_t target_start = ts_node_start_byte(target->node);
    ...
    for (int ci = 0; ci < mt->class_count; ci++) {
        ...
        if (!best || class_start >= best_start) {
            best = ce;
            best_start = class_start;
        }
    }
    return best;
}

static JsClassEntry* jm_find_class_for_identifier_binding(JsMirTranspiler* mt, JsIdentifierNode* id) {
    if (!mt || !id || !id->entry || !id->entry->node) return NULL;
    JsAstNode* binding_node = (JsAstNode*)id->entry->node;
    JsClassNode* class_node = NULL;
    if (binding_node->node_type == JS_AST_NODE_CLASS_DECLARATION ||
        binding_node->node_type == JS_AST_NODE_CLASS_EXPRESSION) {
    ...
    if (!class_node && binding_node->node_type == JS_AST_NODE_IDENTIFIER &&
        !ts_node_is_null(binding_node->node)) {
        uint32_t bind_start = ts_node_start_byte(binding_node->node);
    ...
    return jm_find_class_entry_by_node(mt, class_node);
}
...
MIR_reg_t jm_transpile_new_expr(JsMirTranspiler* mt, JsCallNode* call) {
    ...
    bool exact_class_binding = false;
    bool duplicate_class_name = !jm_class_name_is_unique(mt, ctor_name, ctor_len);
    if (!duplicate_class_name && call->callee->node_type == JS_AST_NODE_IDENTIFIER) {
        JsIdentifierNode* ctor_id = (JsIdentifierNode*)call->callee;
        ce = jm_find_class_for_identifier_binding(mt, ctor_id);
        exact_class_binding = ce != NULL;
        if (!ce && ctor_id->entry && ctor_id->entry->node) {
            // A resolved non-class binding can hold any runtime constructor;
            // matching its spelling to an unrelated class violates lexical binding.
            return jm_emit_dynamic_new_expr(mt, call, arg_count);
        }
    }
    if (!ce) {
        ce = jm_find_enclosing_class_for_new(mt, mt->root_node, (JsAstNode*)call, ctor_name, ctor_len);
        exact_class_binding = ce != NULL;
    }
    if (!ce) {
        ce = jm_find_preceding_class_for_new(mt, (JsAstNode*)call, ctor_name, ctor_len);
        exact_class_binding = ce != NULL;
    }
    if (!ce) ce = jm_find_class(mt, ctor_name, ctor_len);
    if (ce && !exact_class_binding && !jm_class_name_is_unique(mt, ctor_name, ctor_len)) {
        return jm_emit_dynamic_new_expr(mt, call, arg_count);
    }
    if (ce) {
        ...
        // Walk inheritance chain base-first, then own fields
        {
            ...
            for (int ci = field_chain_len - 1; ci >= 0; ci--) {
                ...
                jm_emit_instance_fields_for_class(mt, obj, fc_ce, fc_cls_val,
                    defer_private_instance_init, false);
            }
            ...
            jm_emit_instance_fields_for_class(mt, obj, ce, own_cls_val,
                defer_private_instance_init, defer_own_instance_fields);
            jm_call_void_0(mt, "js_private_field_init_end");
        ...
        if (active_ctor) {
            ...
            if (active_ctor->fc->capture_count > 0) {
                MIR_reg_t ctor_key = jm_box_property_name_literal(mt, "__ctor__", 8);
        ...
        if (!active_ctor && !ce->superclass && ce->node && ce->node->superclass &&
            ...
            MIR_reg_t proto_key2 = jm_box_property_name_literal(mt, "prototype", 9);
    ...
    // The compile-time class table cannot prove an identifier that receives a
    // class expression or another runtime constructor. The former fallback
    // called that value as a plain function, so a class map reached
    // js_call_function and failed before [[Construct]] could inspect __ctor__.
    // Keep one ordinary construct boundary for both unresolved functions and
    // runtime-valued class objects; it preserves the actual prototype and
    // new.target through js_new_from_class_object.
    return jm_emit_dynamic_new_expr(mt, call, arg_count);
}
