...
static bool jm_shared_scope_env_captures_valid(JsMirTranspiler* mt,
    ...
    for (int ci = 0; ci < fc->capture_count; ci++) {
        ...
        if (reject_nfe_binding && fc->captures[ci].is_nfe_binding) return false;
        if (fc->captures[ci].scope_env_slot < 0) {
            if (jm_capture_uses_live_module_var(mt, &fc->captures[ci])) continue;
            return false;
        }
        if (!reject_nfe_binding && fc->captures[ci].is_nfe_binding) continue;
...
static void jm_prepare_closure_captures(JsMirTranspiler* mt, JsFuncCollected* fc,
    ...
    for (int ci = 0; ci < fc->capture_count; ci++) {
        FnCapture* capture = &fc->captures[ci];
...
static MIR_reg_t jm_emit_with_local_writeback(JsMirTranspiler* mt,
        JsMirVarEntry* var, const char* vname, MIR_reg_t with_key,
    ...
    MIR_reg_t local_result = jm_new_reg(mt, result_name, MIR_T_I64);
...
static MIR_reg_t jm_emit_module_with_writeback(JsMirTranspiler* mt,
        MIR_reg_t with_key, MIR_reg_t rhs, bool strict_put,
        JsModuleConstEntry* mc, String* name, const char* vname) {
    MIR_reg_t wrote_with_item = jm_call_3(mt, "js_set_last_with_binding_if_valid", MIR_T_I64,
        MIR_T_I64, MIR_new_reg_op(mt->ctx, with_key),
        MIR_T_I64, MIR_new_reg_op(mt->ctx, rhs),
        MIR_T_I64, MIR_new_int_op(mt->ctx, strict_put ? 1 : 0));
    ...
    jm_call_void_2(mt, "js_set_module_var",
        ...
        MIR_T_I64, MIR_new_reg_op(mt->ctx, rhs));
    jm_emit_global_var_property_sync(mt, mc, name, rhs);
...
static MIR_reg_t jm_emit_logical_assignment(JsMirTranspiler* mt,
    ...
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_MOV,
        MIR_new_reg_op(mt->ctx, result), MIR_new_reg_op(mt->ctx, old_val)));
    ...
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_MOV,
        MIR_new_reg_op(mt->ctx, result), MIR_new_reg_op(mt->ctx, rhs)));
...
static MIR_reg_t jm_emit_private_key_for_access(JsMirTranspiler* mt,
    ...
    if (owner && owner != lexical_home) {
        ...
        if (owner_class) {
            ...
            return jm_call_2(mt, "js_private_key_for_class", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, owner_class),
                MIR_T_I64, MIR_new_reg_op(mt->ctx, source_key));
        }
    }
    return jm_call_1(mt, "js_private_key_for_current_class", MIR_T_I64,
        MIR_T_I64, MIR_new_reg_op(mt->ctx, source_key));
}
...
static void jm_emit_class_computed_field_module_key(JsMirTranspiler* mt,
    ...
    int cls_key_spill = -1;
    if (mt->in_generator && jm_has_yield(key_expr)) {
    ...
    MIR_reg_t previous_private_home = jm_call_1(mt, "js_private_home_class_enter", MIR_T_I64,
        MIR_T_I64, MIR_new_reg_op(mt->ctx, cls_obj));
    ...
    key = jm_call_1(mt, "js_to_property_key", MIR_T_I64,
        MIR_T_I64, MIR_new_reg_op(mt->ctx, key));
    key = jm_call_2(mt, "js_private_home_class_leave_result", MIR_T_I64,
    ...
    jm_call_void_2(mt, "js_set_module_var",
        MIR_T_I64, MIR_new_int_op(mt->ctx, (int64_t)module_var_index),
        MIR_T_I64, MIR_new_reg_op(mt->ctx, key));
}
...
static MIR_reg_t jm_emit_current_this(JsMirTranspiler* mt) {
    if (mt && mt->current_fc && mt->current_fc->node &&
        ...
        if (var) {
            ...
            MIR_reg_t resolved = jm_call_1(mt, "js_resolve_lexical_this", MIR_T_I64,
...
static void jm_emit_public_instance_fields_for_super(JsMirTranspiler* mt, MIR_reg_t obj, JsClassEntry* ce) {
    ...
    mt->current_class = ce;
    MIR_reg_t private_home_class = 0;
    if (jm_class_or_ancestor_has_private_members(ce)) {
        private_home_class = jm_emit_class_object_for_entry(mt, ce);
        if (private_home_class) jm_create_gc_root_slot(mt, private_home_class);
    }
    if (private_home_class) {
        // Private instance methods are branded before any field initializer;
        // otherwise a public field after `super()` cannot call its own method.
        jm_emit_private_instance_method_brands(mt, obj, private_home_class, ce);
    }
    for (int fi = 0; fi < ce->instance_field_count; fi++) {
        JsInstanceFieldEntry* inf = &ce->instance_fields[fi];
        if (!inf) continue;
        ...
        if (inf->initializer && private_home_class) {
            previous_private_home = jm_call_1(mt, "js_private_home_class_enter", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, private_home_class));
            jm_create_gc_root_slot(mt, previous_private_home);
        }
        MIR_reg_t val = inf->initializer ? jm_transpile_box_item(mt, inf->initializer) : jm_emit_undefined(mt);
        if (previous_private_home) {
            // `super()` initializes public fields before the derived method resumes,
            // so this path must supply the lexical private home explicitly.
...
MIR_reg_t jm_emit_get_value(JsMirTranspiler* mt, const JsMirReference* ref) {
    ...
    case JS_MIR_REF_SUPER_PROPERTY: {
        ...
        MIR_reg_t result = jm_call_2(mt, "js_super_property_get", MIR_T_I64,
            MIR_T_I64, MIR_new_reg_op(mt->ctx, ref->base_reg),
            MIR_T_I64, MIR_new_reg_op(mt->ctx, ref->key_reg));
        jm_emit_error_lane_propagate_check(mt);
        return result;
...
static MIR_reg_t jm_emit_compound_assign(JsMirTranspiler* mt, JsOperator op,
    ...
    return jm_call_2(mt, jm_compound_assign_fn(op), MIR_T_I64,
...
static MIR_reg_t jm_emit_int32_bitwise_binary(JsMirTranspiler* mt,
    ...
    MIR_reg_t li = (left_type == LMD_TYPE_INT)
        ? jm_transpile_as_native(mt, left, left_type, LMD_TYPE_INT)
        : jm_call_1(mt, "js_double_to_int32", MIR_T_I64,
            MIR_T_D, MIR_new_reg_op(mt->ctx,
                jm_transpile_as_native(mt, left, left_type, LMD_TYPE_FLOAT)));
    MIR_reg_t ri = (right_type == LMD_TYPE_INT)
        ? jm_transpile_as_native(mt, right, right_type, LMD_TYPE_INT)
        : jm_call_1(mt, "js_double_to_int32", MIR_T_I64,
            MIR_T_D, MIR_new_reg_op(mt->ctx,
                jm_transpile_as_native(mt, right, right_type, LMD_TYPE_FLOAT)));
...
static MIR_reg_t jm_apply_with_identifier_fallback(JsMirTranspiler* mt, JsIdentifierNode* id, MIR_reg_t fallback) {
    ...
    MIR_reg_t result = jm_call_2(mt, jm_with_binding_get_name(mt), MIR_T_I64,
        MIR_T_I64, MIR_new_reg_op(mt->ctx, key),
...
MIR_reg_t jm_transpile_identifier(JsMirTranspiler* mt, JsIdentifierNode* id) {
    ...
    if (var) {
        ...
        if (mt->with_depth > 0 || read_with_outer_binding) {
            ...
            var_read_reg = jm_call_2(mt, jm_with_binding_get_name(mt), MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, lookup_key),
                MIR_T_I64, MIR_new_reg_op(mt->ctx, var_read_reg));
            jm_emit_error_lane_propagate_check(mt);
        }
        ...
        if (mc) {
            ...
            case MCONST_FUNC: {
                ...
                if (fi >= 0 && fi < mt->func_count && mt->func_entries[fi].func_item) {
                    ...
                    MIR_reg_t fn_reg = jm_call_2(mt, "js_new_function_mir", MIR_T_I64,
    ...
    const JsBuiltinGlobalSpec* global_spec = js_builtin_global_find(
        id->name->chars, (int)id->name->len);
    if (global_spec && (global_spec->flags & JS_BUILTIN_GLOBAL_MIR_DIRECT)) {
        if (global_spec->kind == JS_BUILTIN_GLOBAL_NAMESPACE) {
            const char* getter = NULL;
            switch (global_spec->id) {
            case JS_BUILTIN_GLOBAL_NS_MATH: getter = "js_get_math_object_value"; break;
            case JS_BUILTIN_GLOBAL_NS_JSON: getter = "js_get_json_object_value"; break;
            case JS_BUILTIN_GLOBAL_NS_REFLECT: getter = "js_get_reflect_object_value"; break;
            case JS_BUILTIN_GLOBAL_NS_ATOMICS: getter = "js_get_atomics_object_value"; break;
            case JS_BUILTIN_GLOBAL_NS_CONSOLE: getter = "js_get_console_object_value"; break;
            case JS_BUILTIN_GLOBAL_NS_DOCUMENT: getter = "js_get_document_object_value"; break;
            case JS_BUILTIN_GLOBAL_NS_PROCESS: getter = "js_get_process_object_value"; break;
            case JS_BUILTIN_GLOBAL_NS_HOST_262: getter = "js_get_262_object_value"; break;
            case JS_BUILTIN_GLOBAL_NS_CSS: getter = "js_get_css_object_value"; break;
            default: break;
            }
            if (getter) {
                MIR_reg_t value = jm_call_0(mt, getter, MIR_T_I64);
                return jm_apply_with_identifier_fallback(mt, id, value);
            }
        } else if (global_spec->kind == JS_BUILTIN_GLOBAL_FUNCTION) {
            char prefixed[128];
            snprintf(prefixed, sizeof(prefixed), "_js_%s", global_spec->name);
            if (!jm_find_var(mt, prefixed)) {
                MIR_reg_t global_id = jm_box_int_const(mt, (int64_t)global_spec->id);
                MIR_reg_t value = jm_call_1(mt, "js_get_global_builtin_fn_by_id", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, global_id));
                return jm_apply_with_identifier_fallback(mt, id, value);
            }
        } else if (global_spec->kind == JS_BUILTIN_GLOBAL_CONSTRUCTOR) {
            MIR_reg_t name_reg = jm_box_string_literal(mt, global_spec->name, global_spec->len);
...
static void jm_emit_eval_private_bind_name(JsMirTranspiler* mt, JsClassEntry* ce, String* name) {
    ...
    jm_call_void_2(mt, "js_eval_private_bind",
        MIR_T_I64, MIR_new_reg_op(mt->ctx, source_item),
        MIR_T_I64, MIR_new_reg_op(mt->ctx, private_key));
}
...
static bool jm_emit_native_update(JsMirTranspiler* mt, JsMirVarEntry* var,
                                  bool decrement, bool prefix, MIR_reg_t* out) {
    ...
    if (type == LMD_TYPE_INT) {
        ...
        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_DMOV,
            MIR_new_reg_op(mt->ctx, one), MIR_new_double_op(mt->ctx, 1.0)));
        jm_emit(mt, MIR_new_insn(mt->ctx, decrement ? MIR_DSUB : MIR_DADD,
            MIR_new_reg_op(mt->ctx, var->reg), MIR_new_reg_op(mt->ctx, var->reg),
            MIR_new_reg_op(mt->ctx, one)));
    }
    // Scope environments are the shared closure storage; write the native
    // update back there so closures observe the same value as the local fast path.
    if (var->in_scope_env && var->scope_env_reg != 0) {
        MIR_reg_t boxed = jm_box_native(mt, var->reg, type);
        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_MOV,
            MIR_new_mem_op(mt->ctx, MIR_T_I64,
                var->scope_env_slot * (int)sizeof(uint64_t), var->scope_env_reg, 0, 1),
    ...
    if (operand && operand->node_type == JS_AST_NODE_IDENTIFIER && mt->with_depth > 0) {
        ...
        if (var) {
            *with_key = jm_box_string_literal(mt, id->name->chars, (int)id->name->len);
            ...
            MIR_reg_t value = jm_call_2(mt, jm_with_binding_get_name(mt), MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, *with_key),
                MIR_T_I64, MIR_new_reg_op(mt->ctx, fallback));
            jm_emit_error_lane_propagate_check(mt);
            return value;
...
static void jm_emit_unary_local_writeback(JsMirTranspiler* mt, JsMirVarEntry* var,
    ...
    if ((var->type_id == LMD_TYPE_INT || var->type_id == LMD_TYPE_FLOAT) && !var->from_env) {
        ...
        if (var->type_id == LMD_TYPE_INT) {
            ...
            jm_emit(mt, MIR_new_insn(mt->ctx, MIR_MOV,
                MIR_new_reg_op(mt->ctx, var->reg), MIR_new_reg_op(mt->ctx, native_i)));
        } else {
            MIR_reg_t native_d = jm_call_1(mt, "js_get_number", MIR_T_D,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, num_result));
            jm_emit(mt, MIR_new_insn(mt->ctx, MIR_DMOV,
                MIR_new_reg_op(mt->ctx, var->reg), MIR_new_reg_op(mt->ctx, native_d)));
        }
    } else {
        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_MOV,
            MIR_new_reg_op(mt->ctx, var->reg), MIR_new_reg_op(mt->ctx, result)));
    }
    if (var->from_env) {
        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_MOV,
            MIR_new_mem_op(mt->ctx, MIR_T_I64, var->env_slot * (int)sizeof(uint64_t),
                var->env_reg, 0, 1), MIR_new_reg_op(mt->ctx, var->reg)));
    }
    if (var->in_scope_env) {
        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_MOV,
            MIR_new_mem_op(mt->ctx, MIR_T_I64, var->scope_env_slot * (int)sizeof(uint64_t),
                var->scope_env_reg, 0, 1), MIR_new_reg_op(mt->ctx, var->reg)));
    }
}
...
static bool jm_emit_unary_identifier_writeback(JsMirTranspiler* mt, JsIdentifierNode* id,
    ...
    JsMirVarEntry* var = jm_find_var(mt, vname);
    if (mt->with_depth > 0) {
        if (!*with_key) *with_key = jm_box_string_literal(mt, id->name->chars, (int)id->name->len);
        ...
        *with_done_label = jm_new_label(mt);
        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_BF,
            MIR_new_label_op(mt->ctx, local_write_label), MIR_new_reg_op(mt->ctx, wrote_with)));
        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_JMP,
...
static MIR_reg_t jm_transpile_update_unary(JsMirTranspiler* mt, JsUnaryNode* un, bool decrement) {
    ...
    const char* static_old_name = decrement ? "dec_old_static" : "inc_old_static";
    ...
    if (un->operand && un->operand->node_type == JS_AST_NODE_IDENTIFIER) {
        ...
        if (var) {
            ...
            if (jm_emit_native_update(mt, var, decrement, un->prefix, &native_result)) {
    ...
    if (un->operand && un->operand->node_type == JS_AST_NODE_MEMBER_EXPRESSION) {
        ...
        if (sf) {
            ...
            if (!un->prefix) {
                old_value = jm_new_reg(mt, static_old_name, MIR_T_I64);
                jm_emit(mt, MIR_new_insn(mt->ctx, MIR_MOV,
                    MIR_new_reg_op(mt->ctx, old_value), MIR_new_reg_op(mt->ctx, num_operand)));
            }
            MIR_reg_t result = jm_call_1(mt, update_function, MIR_T_I64,
            ...
            jm_call_3(mt, "js_property_set", MIR_T_I64,
                ...
                MIR_T_I64, MIR_new_reg_op(mt->ctx, result));
            log_debug("static-field-update: %.*s.%.*s%s -> module_var[%d]",
                (int)obj_id->name->len, obj_id->name->chars,
                (int)prop_id->name->len, prop_id->name->chars,
                decrement ? "--" : "++", sf->module_var_index);
    ...
    if (!un->prefix) {
        ...
        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_MOV,
            MIR_new_reg_op(mt->ctx, saved), MIR_new_reg_op(mt->ctx, num_operand)));
        num_operand = saved;
    }
    MIR_reg_t result = jm_call_1(mt, update_function, MIR_T_I64,
        MIR_T_I64, MIR_new_reg_op(mt->ctx, num_operand));
    ...
    case JS_OP_TYPEOF: {
        ...
        if (un->operand && un->operand->node_type == JS_AST_NODE_IDENTIFIER) {
            ...
            if (id->name) {
                ...
                const JsBuiltinGlobalSpec* global_spec = js_builtin_global_find(n, nl);
                if (global_spec) {
                    char prefixed[128];
                    snprintf(prefixed, sizeof(prefixed), "_js_%.*s", nl, n);
                    if (!jm_find_var(mt, prefixed)) {
                        const char* type_name = global_spec->kind == JS_BUILTIN_GLOBAL_NAMESPACE ?
                            "object" : "function";
                        int type_name_len = global_spec->kind == JS_BUILTIN_GLOBAL_NAMESPACE ? 6 : 8;
                        MIR_reg_t r = jm_new_reg(mt, "typeof_r", MIR_T_I64);
                        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_MOV,
                            MIR_new_reg_op(mt->ctx, r),
                            MIR_new_int_op(mt->ctx, (int64_t)s2it(
                                name_pool_create_len(
                                    jm_compiled_name_pool(mt), type_name, type_name_len)))));
                        return r;
                    }
                }
            }
...
static void jm_emit_iterator_abrupt_marks_done(JsMirTranspiler* mt,
    ...
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_BF,
        MIR_new_label_op(mt->ctx, normal),
...
static void jm_emit_destructure_pre_binding_probe(JsMirTranspiler* mt, JsAstNode* target) {
    ...
    MIR_reg_t probe_result = jm_call_1(mt, "js_probe_with_binding", MIR_T_I64,
        MIR_T_I64, MIR_new_reg_op(mt->ctx, key));
    jm_emit_error_lane_propagate_check(mt);
    (void)probe_result;
}
...
void jm_emit_destructure_target(JsMirTranspiler* mt, JsAstNode* target, MIR_reg_t val) {
    ...
    if (target->node_type == JS_AST_NODE_IDENTIFIER) {
        ...
        if (member->computed) {
            ...
            prop_key = jm_box_string_literal(mt, key_name->chars, key_name->len);
...
void jm_emit_array_destructure(JsMirTranspiler* mt, JsAstNode* pattern_node, MIR_reg_t src) {
    ...
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_JMP,
        MIR_new_label_op(mt->ctx, arr_destr_done)));
...
static MIR_reg_t jm_emit_destructure_assignment(JsMirTranspiler* mt,
    ...
    int src_spill = -1;
    if (mt->in_generator && jm_has_yield(pattern)) {
...
MIR_reg_t jm_transpile_assignment(JsMirTranspiler* mt, JsAssignmentNode* asgn) {
    ...
    if (asgn->left->node_type == JS_AST_NODE_IDENTIFIER) {
        ...
        if (!var) {
            ...
            if (mt->module_consts) {
                ...
                if (mc && (mc->const_type == MCONST_MODVAR || mc->const_type == MCONST_CLASS)) {
                    ...
                    if (asgn->op != JS_OP_ASSIGN) {
                        ...
                        if (mt->with_depth > 0) {
                            with_key = jm_box_string_literal(mt, id->name->chars, (int)id->name->len);
                            old_val = jm_call_2(mt, jm_with_binding_get_name(mt), MIR_T_I64,
                                MIR_T_I64, MIR_new_reg_op(mt->ctx, with_key),
                                MIR_T_I64, MIR_new_reg_op(mt->ctx, old_val));
                            jm_emit_error_lane_propagate_check(mt);
                        }
                        ...
                        if (mt->with_depth > 0) {
                            return jm_emit_module_with_writeback(mt, with_key, rhs,
                                strict_put, mc, id->name, vname);
                        }
                    } else {
                        ...
                        if (mt->with_depth > 0) {
                            return jm_emit_module_with_writeback(mt, simple_with_key, rhs,
                                strict_put, mc, id->name, vname);
                        }
                    }
            ...
            // Implicit global assignment: write to global object via js_set_global_property
            {
                ...
                if (strict_lhs_exists) {
                    jm_call_3(mt, "js_set_global_property_strict_prechecked", MIR_T_I64,
        ...
        if (mt->with_depth > 0 && asgn->op != JS_OP_ASSIGN && asgn->op != JS_OP_AND_ASSIGN &&
            ...
            bool strict_put = mt->is_global_strict || mt->is_module ||
                (mt->current_fc && mt->current_fc->is_strict);
            return jm_emit_with_local_writeback(mt, var, vname,
                with_key, rhs, "lwa_res", strict_put);
        }
    ...
    if (asgn->left->node_type == JS_AST_NODE_MEMBER_EXPRESSION) {
        ...
        // P2: Guarded simple assignment for inferred shaped class instances.
        if (asgn->op == JS_OP_ASSIGN && !member->computed && !member->optional &&
            member->object && member->object->node_type == JS_AST_NODE_IDENTIFIER &&
            member->property && member->property->node_type == JS_AST_NODE_IDENTIFIER) {
            ...
            if (p2_ce && p2_ce->constructor && p2_ce->constructor->fc) {
                ...
                if (p2_slot >= 0) {
                    MIR_reg_t obj_reg = jm_transpile_box_item(mt, member->object);
                    MIR_reg_t new_val = jm_transpile_box_item(mt, asgn->right);
                    // inferred receiver classes are speculative (especially in
                    // minified for-of loops), so an unguarded slot write can
                    // overwrite an unrelated object's prototype storage.
                    jm_call_5(mt, "js_set_shaped_slot_guarded", MIR_T_I64,
                        MIR_T_I64, MIR_new_reg_op(mt->ctx, obj_reg),
                        MIR_T_I64, MIR_new_int_op(mt->ctx, (int64_t)p2_slot),
                        MIR_T_P, MIR_new_int_op(mt->ctx,
                            (int64_t)(uintptr_t)p2_prop->name->chars),
                        MIR_T_I64, MIR_new_int_op(mt->ctx,
                            (int64_t)p2_prop->name->len),
                        MIR_T_I64, MIR_new_reg_op(mt->ctx, new_val));
                    log_debug("P2: guarded shaped assign %.*s.%.*s → slot %d",
...
MIR_reg_t jm_transpile_math_call(JsMirTranspiler* mt, JsCallNode* call, String* method) {
    ...
    int math_builtin_id = js_builtin_catalog_lookup_id(
        JS_BUILTIN_OWNER_MATH_METHOD, method->chars, (int)method->len);
    ...
    for (JsAstNode* chk = call->arguments; chk; chk = chk->next) {
        if (chk->node_type == JS_AST_NODE_SPREAD_ELEMENT) {
            log_debug("phase5: Math.%.*s called with spread arg, using js_math_apply",
                (int)method->len, method->chars);
    ...
    if (argc >= 1) {
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_ABS) {
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_FLOOR) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
            MIR_reg_t num = jm_call_1(mt, "js_to_number", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, a));
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_CEIL) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
            MIR_reg_t num = jm_call_1(mt, "js_to_number", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, a));
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_ROUND) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
            MIR_reg_t num = jm_call_1(mt, "js_to_number", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, a));
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_SQRT) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
            MIR_reg_t num = jm_call_1(mt, "js_to_number", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, a));
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_LOG) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
            MIR_reg_t num = jm_call_1(mt, "js_to_number", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, a));
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_LOG10) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
            MIR_reg_t num = jm_call_1(mt, "js_to_number", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, a));
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_EXP) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
            MIR_reg_t num = jm_call_1(mt, "js_to_number", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, a));
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_SIN) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
            MIR_reg_t num = jm_call_1(mt, "js_to_number", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, a));
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_COS) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
            MIR_reg_t num = jm_call_1(mt, "js_to_number", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, a));
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_TAN) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
            MIR_reg_t num = jm_call_1(mt, "js_to_number", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, a));
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_SIGN) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
            MIR_reg_t num = jm_call_1(mt, "js_to_number", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, a));
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_TRUNC) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
            MIR_reg_t num = jm_call_1(mt, "js_to_number", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, a));
    ...
    if (argc >= 2 && arg1) {
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_POW) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_MIN) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_MAX) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
            MIR_reg_t na = jm_call_1(mt, "js_to_number", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, a));
            MIR_reg_t result = na;
        ...
        if (math_builtin_id == JS_BUILTIN_MATH_ATAN2) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
            MIR_reg_t b = jm_transpile_box_item(mt, arg1);
            MIR_reg_t na = jm_call_1(mt, "js_to_number", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, a));
    ...
    if (argc == 1) {
        if ((math_builtin_id == JS_BUILTIN_MATH_MIN) || (math_builtin_id == JS_BUILTIN_MATH_MAX)) {
            MIR_reg_t a = jm_transpile_box_item(mt, arg0);
    ...
    log_debug("phase5: unresolved Math.%.*s, using runtime dispatch",
        (int)method->len, method->chars);
    MIR_reg_t method_str = jm_box_string_literal(mt, method->chars, method->len);
...
TypeId jm_math_return_type(String* method, JsMirTranspiler* mt, JsAstNode* arg0) {
    ...
    if ((math_builtin_id == JS_BUILTIN_MATH_SQRT) || (math_builtin_id == JS_BUILTIN_MATH_SIN) || (math_builtin_id == JS_BUILTIN_MATH_COS) ||
        (math_builtin_id == JS_BUILTIN_MATH_TAN) || (math_builtin_id == JS_BUILTIN_MATH_LOG) || (math_builtin_id == JS_BUILTIN_MATH_LOG10) ||
        (math_builtin_id == JS_BUILTIN_MATH_EXP) || (math_builtin_id == JS_BUILTIN_MATH_POW) || (math_builtin_id == JS_BUILTIN_MATH_RANDOM) ||
        (math_builtin_id == JS_BUILTIN_MATH_ATAN2))
    ...
    if ((math_builtin_id == JS_BUILTIN_MATH_TRUNC) || (math_builtin_id == JS_BUILTIN_MATH_SIGN))
        return LMD_TYPE_FLOAT;
    ...
    if ((math_builtin_id == JS_BUILTIN_MATH_FLOOR) || (math_builtin_id == JS_BUILTIN_MATH_CEIL) || (math_builtin_id == JS_BUILTIN_MATH_ROUND))
        return LMD_TYPE_FLOAT;
...
MIR_reg_t jm_transpile_math_native(JsMirTranspiler* mt, JsCallNode* call,
    ...
    if (arg_numeric) {
        ...
        if ((math_builtin_id == JS_BUILTIN_MATH_POW) && argc >= 2) {
        ...
        if ((math_builtin_id == JS_BUILTIN_MATH_MIN) && argc >= 2) {
            JsAstNode* arg1_n = arg0->next;
            TypeId a1t = jm_get_effective_type(mt, arg1_n);
            if (a1t == LMD_TYPE_INT || a1t == LMD_TYPE_FLOAT) {
                MIR_reg_t da = jm_transpile_as_native(mt, arg0, arg_type, LMD_TYPE_FLOAT);
        ...
        if ((math_builtin_id == JS_BUILTIN_MATH_MAX) && argc >= 2) {
            JsAstNode* arg1_n = arg0->next;
            TypeId a1t = jm_get_effective_type(mt, arg1_n);
            if (a1t == LMD_TYPE_INT || a1t == LMD_TYPE_FLOAT) {
                MIR_reg_t da = jm_transpile_as_native(mt, arg0, arg_type, LMD_TYPE_FLOAT);
...
static MIR_reg_t jm_emit_optional_method_call(JsMirTranspiler* mt, MIR_reg_t recv,
                                               ...
                                               int arg_count, const char* result_prefix,
    ...
    MIR_reg_t call_result = jm_call_function_into(mt,
        MIR_new_reg_op(mt->ctx, fn), MIR_new_reg_op(mt->ctx, recv), args_op,
...
static bool jm_pack_extra_call_args(JsMirTranspiler* mt, JsAstNode* first_extra,
        MIR_reg_t* out_array) {
    if (!first_extra || !out_array) return false;
    int count = 0;
    MIR_reg_t values[4];
    for (JsAstNode* arg = first_extra; arg && count < 4; arg = arg->next) {
        values[count++] = jm_transpile_box_item(mt, arg);
    }
    if (count == 1) *out_array = jm_call_1(mt, "js_pack_args_1", MIR_T_I64,
        MIR_T_I64, MIR_new_reg_op(mt->ctx, values[0]));
    else if (count == 2) *out_array = jm_call_2(mt, "js_pack_args_2", MIR_T_I64,
        MIR_T_I64, MIR_new_reg_op(mt->ctx, values[0]),
        MIR_T_I64, MIR_new_reg_op(mt->ctx, values[1]));
    else if (count == 3) *out_array = jm_call_3(mt, "js_pack_args_3", MIR_T_I64,
        MIR_T_I64, MIR_new_reg_op(mt->ctx, values[0]),
        MIR_T_I64, MIR_new_reg_op(mt->ctx, values[1]),
        MIR_T_I64, MIR_new_reg_op(mt->ctx, values[2]));
    else *out_array = jm_call_4(mt, "js_pack_args_4", MIR_T_I64,
        MIR_T_I64, MIR_new_reg_op(mt->ctx, values[0]),
        MIR_T_I64, MIR_new_reg_op(mt->ctx, values[1]),
        MIR_T_I64, MIR_new_reg_op(mt->ctx, values[2]),
        MIR_T_I64, MIR_new_reg_op(mt->ctx, values[3]));
    return true;
}
...
MIR_reg_t jm_transpile_call(JsMirTranspiler* mt, JsCallNode* call) {
        ...
        if (id->name && id->name->len == 5 && strncmp(id->name->chars, "super", 5) == 0) {
            ...
            if (static_superclass) {
                ...
                if (parent->constructor && parent->constructor->fc && parent->constructor->fc->func_item) {
                    ...
                    if (super_has_spread) {
                        MIR_reg_t super_result = parent_ctor_captured
                            ? jm_super_apply_class_into(mt,
                                MIR_new_reg_op(mt->ctx, ctor_fn), MIR_new_reg_op(mt->ctx, this_val),
                                MIR_new_reg_op(mt->ctx, args_ptr))
                    ...
                    while (ancestor) {
                        if (ancestor->constructor && ancestor->constructor->fc && ancestor->constructor->fc->func_item) {
                            ...
                            if (anc_has_spread) {
                                MIR_reg_t super_result = ancestor_ctor_captured
                                    ? jm_super_apply_class_into(mt,
                                        MIR_new_reg_op(mt->ctx, ctor_fn), MIR_new_reg_op(mt->ctx, this_val),
                                        MIR_new_reg_op(mt->ctx, args_ptr))
                                    : jm_apply_function_into(mt,
    ...
    if (jm_test262_fast_paths_enabled(mt) &&
        ...
        if (id->name) {
            ...
            if (id->name->len == 14 && strncmp(id->name->chars, "verifyProperty", 14) == 0 && arg_count >= 3) {
                ...
                jm_call_4(mt, "js_verify_property", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, obj_reg),
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, name_reg),
            ...
            if (id->name->len == 6 && strncmp(id->name->chars, "assert", 6) == 0 && arg_count >= 1 && arg_count <= 2) {
                ...
                jm_call_2(mt, "js_assert_base", MIR_T_I64,
        ...
        if (!m->computed && m->object && m->object->node_type == JS_AST_NODE_IDENTIFIER &&
            ...
            if (ce) {
                ...
                if (found && found->fc && found->fc->func_item &&
                    ...
                    if (has_spread) {
                        MIR_reg_t r = jm_apply_function_into(mt,
                            MIR_new_reg_op(mt->ctx, fn_item),
                            MIR_new_reg_op(mt->ctx, class_this),
                            MIR_new_reg_op(mt->ctx, args_ptr));
                        jm_emit_error_lane_propagate_check(mt);
                        return r;
                    }
    ...
    if (call->callee && call->callee->node_type == JS_AST_NODE_MEMBER_EXPRESSION) {
        ...
        if (!m->computed && m->object && m->object->node_type == JS_AST_NODE_IDENTIFIER &&
            ...
            JsIdentifierNode* obj = (JsIdentifierNode*)m->object;
            JsIdentifierNode* prop = (JsIdentifierNode*)m->property;
            int catalog_builtin_id = js_builtin_catalog_lookup_member_id(
                obj->name->chars, (int)obj->name->len,
                prop->name->chars, (int)prop->name->len);
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_KEYS) {
            ...
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_GET_OWN_PROPERTY_NAMES) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                return jm_call_1(mt, "js_object_get_own_property_names", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
            }
            // Object.getOwnPropertySymbols(obj)
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_GET_OWN_PROPERTY_SYMBOLS) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                return jm_call_1(mt, "js_object_get_own_property_symbols", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
            }
            // Object.getOwnPropertyDescriptor(obj, name)
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_GET_OWN_PROPERTY_DESCRIPTOR) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
            ...
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_GET_OWN_PROPERTY_DESCRIPTORS) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
            ...
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_CREATE) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
            ...
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_DEFINE_PROPERTY) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
                JsAstNode* a3 = a2 ? a2->next : NULL;
                MIR_reg_t obj_arg = a1 ? jm_transpile_box_item(mt, a1) : jm_emit_null(mt);
            ...
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_DEFINE_PROPERTIES) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
                MIR_reg_t obj_arg = a1 ? jm_transpile_box_item(mt, a1) : jm_emit_null(mt);
            ...
            // Date.now()
            if (catalog_builtin_id == JS_BUILTIN_DATE_NOW) {
                return jm_call_0(mt, "js_date_now", MIR_T_I64);
            }
            // Date.UTC(year, month, day, hour, min, sec, ms)
            if (catalog_builtin_id == JS_BUILTIN_DATE_UTC) {
            ...
            if (catalog_builtin_id == JS_BUILTIN_DATE_PARSE) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                return jm_call_1(mt, "js_date_parse", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
            }
            // Array.isArray(x)
            if (catalog_builtin_id == JS_BUILTIN_ARRAY_IS_ARRAY) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                return jm_call_1(mt, "js_array_is_array", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
            }
            // ArrayBuffer.isView(x)
            if (catalog_builtin_id == JS_BUILTIN_ARRAYBUFFER_ISVIEW) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                return jm_call_1(mt, "js_arraybuffer_is_view", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
            }
            // Object.values(obj)
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_VALUES) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                return jm_call_1(mt, "js_object_values", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
            }
            // Object.entries(obj)
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_ENTRIES) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                return jm_call_1(mt, "js_object_entries", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
            }
            // Object.fromEntries(iterable)
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_FROM_ENTRIES) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                return jm_call_1(mt, "js_object_from_entries", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
            }
            // Object.is(value1, value2)
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_IS) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
            ...
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_ASSIGN) {
                JsAstNode* a1 = call->arguments;
            ...
            // Object.freeze(obj)
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_FREEZE) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                return jm_call_1(mt, "js_object_freeze", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
            }
            // Object.isFrozen(obj)
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_IS_FROZEN) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                return jm_call_1(mt, "js_object_is_frozen", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
            }
            // Object.hasOwn(obj, key)
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_HAS_OWN) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
                MIR_reg_t obj_arg = a1 ? jm_transpile_box_item(mt, a1) : jm_emit_null(mt);
            ...
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_GROUP_BY) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
            ...
            if (catalog_builtin_id == JS_BUILTIN_MAP_GROUP_BY) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
                MIR_reg_t items_arg = a1 ? jm_transpile_box_item(mt, a1) : jm_emit_null(mt);
                MIR_reg_t cb_arg = a2 ? jm_transpile_box_item(mt, a2) : jm_emit_null(mt);
            ...
            if (catalog_builtin_id == JS_BUILTIN_PROXY_REVOCABLE) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
            ...
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_GET_PROTOTYPE_OF) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                jm_emit_error_lane_propagate_check(mt);
            ...
            if (catalog_builtin_id == JS_BUILTIN_REFLECT_CONSTRUCT) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
                JsAstNode* a3 = a2 ? a2->next : NULL;
                MIR_reg_t target_arg = a1 ? jm_transpile_box_item(mt, a1) : jm_emit_null(mt);
            ...
            // Reflect.ownKeys(obj)
            if (catalog_builtin_id == JS_BUILTIN_REFLECT_OWN_KEYS) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                return jm_call_1(mt, "js_reflect_own_keys", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
            }
            // Reflect.has(obj, key)
            if (catalog_builtin_id == JS_BUILTIN_REFLECT_HAS) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
                MIR_reg_t obj_arg = a1 ? jm_transpile_box_item(mt, a1) : jm_emit_null(mt);
                MIR_reg_t key_arg = a2 ? jm_transpile_box_item(mt, a2) : jm_emit_null(mt);
            ...
            // Reflect.get(obj, key)
            if (catalog_builtin_id == JS_BUILTIN_REFLECT_GET) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
                JsAstNode* a3 = a2 ? a2->next : NULL;
                MIR_reg_t obj_arg = a1 ? jm_transpile_box_item(mt, a1) : jm_emit_null(mt);
            ...
            if (catalog_builtin_id == JS_BUILTIN_REFLECT_SET) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
                JsAstNode* a3 = a2 ? a2->next : NULL;
                JsAstNode* a4 = a3 ? a3->next : NULL;
            ...
            if (catalog_builtin_id == JS_BUILTIN_REFLECT_DEFINE_PROPERTY) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
                JsAstNode* a3 = a2 ? a2->next : NULL;
                MIR_reg_t obj_arg = a1 ? jm_transpile_box_item(mt, a1) : jm_emit_null(mt);
            ...
            if (catalog_builtin_id == JS_BUILTIN_REFLECT_DELETE_PROPERTY) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
                MIR_reg_t obj_arg = a1 ? jm_transpile_box_item(mt, a1) : jm_emit_null(mt);
                MIR_reg_t key_arg = a2 ? jm_transpile_box_item(mt, a2) : jm_emit_null(mt);
            ...
            if (catalog_builtin_id == JS_BUILTIN_REFLECT_GET_OWN_PROPERTY_DESCRIPTOR) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
                MIR_reg_t obj_arg = a1 ? jm_transpile_box_item(mt, a1) : jm_emit_null(mt);
                MIR_reg_t name_arg = a2 ? jm_transpile_box_item(mt, a2) : jm_emit_null(mt);
            ...
            if (catalog_builtin_id == JS_BUILTIN_REFLECT_GET_PROTOTYPE_OF) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                return jm_call_1(mt, "js_reflect_get_prototype_of", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
            }
            // Reflect.setPrototypeOf(obj, proto)
            if (catalog_builtin_id == JS_BUILTIN_REFLECT_SET_PROTOTYPE_OF) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
                MIR_reg_t obj_arg = a1 ? jm_transpile_box_item(mt, a1) : jm_emit_null(mt);
            ...
            if (catalog_builtin_id == JS_BUILTIN_REFLECT_IS_EXTENSIBLE) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                return jm_call_1(mt, "js_reflect_is_extensible", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
            }
            // Reflect.preventExtensions(obj)
            if (catalog_builtin_id == JS_BUILTIN_REFLECT_PREVENT_EXTENSIONS) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                return jm_call_1(mt, "js_reflect_prevent_extensions", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
            }
            // Reflect.apply(target, thisArg, argsList)
            if (catalog_builtin_id == JS_BUILTIN_REFLECT_APPLY) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
                JsAstNode* a3 = a2 ? a2->next : NULL;
                MIR_reg_t target_arg = a1 ? jm_transpile_box_item(mt, a1) : jm_emit_null(mt);
            ...
            if (catalog_builtin_id == JS_BUILTIN_OBJECT_SET_PROTOTYPE_OF) {
                JsAstNode* a1 = call->arguments;
                JsAstNode* a2 = a1 ? a1->next : NULL;
            ...
            if (catalog_builtin_id == JS_BUILTIN_JSON_PARSE) {
            ...
            if (catalog_builtin_id == JS_BUILTIN_JSON_STRINGIFY) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
            ...
            if (catalog_builtin_id == JS_BUILTIN_ARRAY_FROM) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
            ...
            if (catalog_builtin_id == JS_BUILTIN_ARRAY_OF) {
            ...
            if (catalog_builtin_id == JS_BUILTIN_BIGINT_AS_INT_N) {
            ...
            if (catalog_builtin_id == JS_BUILTIN_BIGINT_AS_UINT_N) {
                MIR_reg_t bits_arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                JsAstNode* arg2 = call->arguments ? call->arguments->next : NULL;
                MIR_reg_t val_arg = arg2 ? jm_transpile_box_item(mt, arg2) : jm_emit_null(mt);
    ...
    if (call->callee && call->callee->node_type == JS_AST_NODE_MEMBER_EXPRESSION) {
        ...
        if (!m->computed && m->object && m->object->node_type == JS_AST_NODE_IDENTIFIER &&
            ...
            int catalog_builtin_id = js_builtin_catalog_lookup_member_id(
                obj->name->chars, (int)obj->name->len,
                prop->name->chars, (int)prop->name->len);
            if (catalog_builtin_id == JS_BUILTIN_STRING_FROM_CHAR_CODE) {
                // Count arguments
                int argc = 0;
            ...
            if (catalog_builtin_id == JS_BUILTIN_STRING_FROM_CODE_POINT) {
                int argc = 0;
                for (JsAstNode* a = call->arguments; a; a = a->next) argc++;
                if (argc == 0) {
                    return jm_box_string_literal(mt, "", 0);
    ...
    if (call->callee && call->callee->node_type == JS_AST_NODE_MEMBER_EXPRESSION) {
        ...
        if (m->property && m->property->node_type == JS_AST_NODE_IDENTIFIER) {
            ...
            // Function.prototype call/apply/bind use ordinary property dispatch.
            // A method name alone does not prove the receiver is callable; class
            // APIs such as ChangeSet.apply() are ordinary instance methods.
            ...
            if (jm_is_private_name(method_key_name)) {
                // Method-call lowering bypasses the generic member path, so it
                // must recover the class-owned key instead of dispatching on
                // the source spelling shared by unrelated private declarations.
                method_name = jm_emit_private_key_for_access(mt, (JsAstNode*)m->property,
                    method_key_name);
            }
            ...
            if (!m->computed && prop && prop->name && !jm_is_private_name(method_key_name)) {
                ...
                if (p3_ce) {
                    ...
                    // Direct dispatch bypasses JsFunction invocation, which is where
                    // a method's lexical private-name home is installed.
                    if (p3_method && !jm_class_or_ancestor_has_private_members(p3_ce) &&
                        p3_method->fc->func_item && p3_method->fc->capture_count == 0 && !p3_overridden) {
                        ...
                        MIR_reg_t p3_saved_wd = p3_method->fc->uses_with
                        ...
                        jm_readback_closure_env(mt);
                        return jm_publish_call_result(mt, p3_result);
                    }
                }
            }
            ...
            if (recv_type == LMD_TYPE_STRING) {
                MIR_reg_t r = jm_call_4(mt, "js_string_method", MIR_T_I64,
                    ...
                    MIR_T_I64, MIR_new_int_op(mt->ctx, arg_count));
                jm_readback_closure_env(mt);
                // native string methods perform user-controlled coercions;
                // route their ERROR Item before normal expression flow consumes it.
                jm_emit_error_lane_propagate_check(mt);
                return jm_publish_call_result(mt, r);
            }
            if (recv_type == LMD_TYPE_ARRAY) {
                MIR_reg_t r = jm_array_method_direct_into(mt,
                    MIR_new_reg_op(mt->ctx, recv), MIR_new_reg_op(mt->ctx, method_name),
                    args_op, MIR_new_int_op(mt->ctx, arg_count));
                jm_readback_closure_env(mt);
                jm_emit_error_lane_propagate_check(mt);
                return jm_publish_call_result(mt, r);
            }
            ...
            if (recv_type == LMD_TYPE_MAP) {
                ...
                jm_readback_closure_env(mt);
                jm_publish_call_result(mt, map_r);
                jm_emit_error_lane_propagate_check(mt);
                return map_r;
            }
            ...
            jm_emit_label(mt, l_fallback);
            {
                ...
                MIR_reg_t r = jm_call_function_into(mt,
                    MIR_new_reg_op(mt->ctx, fn),
                    MIR_new_reg_op(mt->ctx, recv), args_op,
                    MIR_new_int_op(mt->ctx, arg_count));
                jm_emit_clear_assert_pending_call_source(mt, emitted_call_source);
                jm_emit(mt, MIR_new_insn(mt->ctx, MIR_MOV,
                    MIR_new_reg_op(mt->ctx, result), MIR_new_reg_op(mt->ctx, r)));
    ...
    if (call->callee && call->callee->node_type == JS_AST_NODE_IDENTIFIER) {
        ...
        if (id->name) {
            ...
            if (nl == 8 && strncmp(n, "parseInt", 8) == 0 && !jm_find_var(mt, "_js_parseInt")) {
                ...
                MIR_reg_t result = jm_call_2(mt, "js_parseInt", MIR_T_I64,
            ...
            if (nl == 10 && strncmp(n, "parseFloat", 10) == 0 && !jm_find_var(mt, "_js_parseFloat")) {
                MIR_reg_t str = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                MIR_reg_t result = jm_call_1(mt, "js_parseFloat", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, str));
                jm_emit_error_lane_propagate_check(mt);
                return result;
            }
            ...
            if (nl == 18 && strncmp(n, "encodeURIComponent", 18) == 0 && !jm_find_var(mt, "_js_encodeURIComponent")) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                MIR_reg_t result = jm_call_1(mt, "js_encodeURIComponent", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
                jm_emit_error_lane_propagate_check(mt);
                return result;
            }
            ...
            if (nl == 9 && strncmp(n, "encodeURI", 9) == 0 && !jm_find_var(mt, "_js_encodeURI")) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_null(mt);
                MIR_reg_t result = jm_call_1(mt, "js_encodeURI", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
                jm_emit_error_lane_propagate_check(mt);
                return result;
            }
            ...
            if (nl == 28 && strncmp(n, "validateNativeFunctionSource", 28) == 0 &&
                ...
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_undefined(mt);
                jm_call_1(mt, "js_validate_native_function_source", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
                jm_emit_error_lane_propagate_check(mt);
                return jm_emit_undefined(mt);
            }
            ...
            if (nl == 8 && strncmp(n, "unescape", 8) == 0 && !jm_find_var(mt, "_js_unescape")) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_undefined(mt);
                MIR_reg_t result = jm_call_1(mt, "js_unescape", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
                jm_emit_error_lane_propagate_check(mt);
                return result;
            }
            ...
            if (nl == 6 && strncmp(n, "escape", 6) == 0 && !jm_find_var(mt, "_js_escape")) {
                MIR_reg_t arg = call->arguments ? jm_transpile_box_item(mt, call->arguments) : jm_emit_undefined(mt);
                MIR_reg_t result = jm_call_1(mt, "js_escape", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, arg));
                jm_emit_error_lane_propagate_check(mt);
                return result;
            }
            ...
            if (nl == 4 && strncmp(n, "eval", 4) == 0 && !jm_find_var(mt, "_js_eval")) {
                ...
                // eval compiles a nested program; its SyntaxError must be
                // routed through the caller's active completion handler before
                // bridge cleanup can otherwise consume the abrupt result.
                jm_emit_error_lane_propagate_check(mt);
            ...
            if (nl == 8 && strncmp(n, "Function", 8) == 0) {
                MIR_reg_t args_ptr = jm_build_args_array(mt, call->arguments, arg_count);
                MIR_reg_t result = jm_call_2(mt, "js_new_function_from_string", MIR_T_I64,
                    MIR_T_I64, args_ptr ? MIR_new_reg_op(mt->ctx, args_ptr) : MIR_new_int_op(mt->ctx, 0),
                    MIR_T_I64, MIR_new_int_op(mt->ctx, arg_count));
                jm_emit_error_lane_propagate_check(mt);
                return result;
            ...
            if (nl == 10 && strncmp(n, "setTimeout", 10) == 0 &&
                ...
                MIR_reg_t arr;
                if (jm_pack_extra_call_args(mt, extra, &arr)) {
        ...
        if (resolved_fn) {
            ...
            if (fc && (fc->func_item || fc->native_func_item) && fc->capture_count == 0) {
                ...
                MIR_reg_t prev_this = fc->observes_this
                    ? jm_call_0(mt, "js_get_lexical_this_binding", MIR_T_I64) : 0;
                MIR_reg_t prev_nt_dc = fc->observes_new_target
                ...
                if (fc->observes_this || fc->observes_new_target) {
                ...
                if (fc->observes_new_target) jm_call_void_1(mt,
                    "js_set_direct_new_target", MIR_T_I64,
                    MIR_new_reg_op(mt->ctx, undef_this));

                // save with-scope depth before direct call (function may return from inside 'with')
                MIR_reg_t saved_wd = fc->uses_with
                    ? jm_call_0(mt, "js_with_save_depth", MIR_T_I64) : 0;
                ...
                if (saved_wd) jm_call_void_1(mt, "js_with_restore_depth",
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, saved_wd));
                if (prev_this) jm_call_void_1(mt, "js_set_this",
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, prev_this));
                if (prev_nt_dc) jm_call_void_1(mt, "js_set_direct_new_target",
    ...
    if (!call->optional && jm_has_optional_chain(call->callee)) {
        return jm_emit_optional_function_call(mt, callee, call, arg_count, site_id,
...
static void jm_prepare_typed_array_data(JsMirTranspiler* mt,
        MIR_reg_t arr_reg, int ta_type, MIR_reg_t h_data, MIR_reg_t h_len,
        bool write_access, bool need_length, MIR_reg_t* out_data,
        MIR_reg_t* out_length) {
    if (h_data && (!need_length || h_len)) {
        *out_data = h_data;
        if (need_length) *out_length = h_len;
        return;
    }
    // property upgrades move the native descriptor out of Map.data; querying
    // through the runtime keeps indexed access valid after user properties exist.
    (void)jm_call_1(mt, "js_get_typed_array_ptr", MIR_T_I64,
        MIR_T_I64, MIR_new_reg_op(mt->ctx, arr_reg));
    if (need_length) {
        *out_length = jm_call_1(mt, "js_typed_array_length", MIR_T_I64,
            MIR_T_I64, MIR_new_reg_op(mt->ctx, arr_reg));
    }
    *out_data = jm_call_1(mt,
        write_access ? "js_typed_array_prepare_write_ptr" :
                       "js_typed_array_current_data_ptr",
        MIR_T_I64, MIR_T_I64, MIR_new_reg_op(mt->ctx, arr_reg));
    (void)ta_type;
}
...
MIR_reg_t jm_transpile_typed_array_get(JsMirTranspiler* mt, MIR_reg_t arr_reg,
    ...
    jm_prepare_typed_array_data(mt, arr_reg, ta_type, h_data, h_len,
        false, true, &data_ptr, &ta_len);
...
static MIR_reg_t jm_emit_typed_array_element_address(JsMirTranspiler* mt,
        MIR_reg_t data_ptr, MIR_reg_t idx_native, int ta_type) {
    int elem_size = jm_typed_array_elem_size(ta_type);
    MIR_reg_t byte_offset = jm_new_reg(mt, "ta_off", MIR_T_I64);
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_MUL, MIR_new_reg_op(mt->ctx, byte_offset),
        MIR_new_reg_op(mt->ctx, idx_native), MIR_new_int_op(mt->ctx, elem_size)));
...
MIR_reg_t jm_transpile_typed_array_get_native(JsMirTranspiler* mt, MIR_reg_t arr_reg,
    ...
    MIR_reg_t data_ptr;
    jm_prepare_typed_array_data(mt, arr_reg, ta_type, h_data, 0,
        false, false, &data_ptr, NULL);

    // Compute element address: data_ptr + idx * elem_size.
    MIR_reg_t elem_addr = jm_emit_typed_array_element_address(mt, data_ptr, idx_native, ta_type);
...
static MIR_reg_t jm_emit_guarded_shaped_slot_get(JsMirTranspiler* mt,
                                                  MIR_reg_t obj_reg,
                                                  String* key_name,
                                                  int slot) {
    ...
    int64_t byte_offset = (int64_t)slot * (int64_t)sizeof(void*);

    // field-name inference is speculative; verify the runtime shape before a
    // slot read so a same-named minified class cannot expose another field.
    MIR_reg_t match = jm_call_4(mt, "js_shape_slot_guard", MIR_T_I64,
        ...
        MIR_T_I64, MIR_new_int_op(mt->ctx, byte_offset));
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_BT,
        MIR_new_label_op(mt->ctx, l_fast), MIR_new_reg_op(mt->ctx, match)));
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_JMP, MIR_new_label_op(mt->ctx, l_slow)));
...
static MIR_reg_t jm_emit_optional_member_access(JsMirTranspiler* mt,
    ...
    if (mem->computed) {
        ...
        if (jm_is_private_name(key_name)) {
            key = jm_emit_private_key_for_access(mt, (JsAstNode*)mem->property, key_name);
        }
    } else {
        key = jm_transpile_box_item(mt, mem->property);
    }
...
MIR_reg_t jm_transpile_member(JsMirTranspiler* mt, JsMemberNode* mem) {
    ...
    if (!mem->computed && !mem->optional && mem->property &&
        ...
        if (prop->name && prop->name->len == 6 && strncmp(prop->name->chars, "length", 6) == 0) {
            ...
            MIR_reg_t length = jm_call_1(mt, "js_get_length_item", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, obj));
            // accessors on prototype-backed length properties can complete abruptly;
            // the expression boundary must route that ERROR lane to the active handler.
            jm_emit_error_lane_propagate_check(mt);
            return length;
        }
    }
            ...
            if (p4_slot >= 0) {
                ...
                if (field_type == LMD_TYPE_INT || field_type == LMD_TYPE_FLOAT) {
                    ...
                    if (p1_ce->shape_cache_ptr) {
                        ...
                        // Constructor field inference is speculative: later methods
                        // can store an object in a numeric-initialized slot, so boxed
                        // reads must preserve its runtime value and identity.
                        jm_emit_label(mt, l_fast);
                        if (profile_shape_guard) {
                        ...
                        MIR_reg_t boxed = jm_call_2(mt, "js_get_shaped_slot", MIR_T_I64,
                            MIR_T_I64, MIR_new_reg_op(mt->ctx, obj_reg),
                            MIR_T_I64, MIR_new_int_op(mt->ctx, p4_slot));
                        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_MOV,
                            MIR_new_reg_op(mt->ctx, result),
                            MIR_new_reg_op(mt->ctx, boxed)));
                        ...
                        // Fast boxed reads preserve the slot's runtime type; a
                        // constructor's initial numeric type is not permanent.
                        jm_emit_label(mt, l_fast);
                        MIR_reg_t boxed = jm_call_2(mt, "js_get_shaped_slot", MIR_T_I64,
                            MIR_T_I64, MIR_new_reg_op(mt->ctx, obj_reg),
                            MIR_T_I64, MIR_new_int_op(mt->ctx, p4_slot));
                ...
                // P4: Untyped fallback — guard speculative class inference.
                log_debug("P4: guarded shaped load %.*s.%.*s → slot %d",
                          (int)p4_obj->name->len, p4_obj->name->chars,
                          (int)p4_prop->name->len, p4_prop->name->chars, p4_slot);
                return jm_emit_guarded_shaped_slot_get(mt, obj_reg,
                    p4_prop->name, p4_slot);
            }
        }
    }
...
static bool jm_object_static_shape(JsMirTranspiler* mt, JsObjectNode* obj,
        const char*** names_out, int** lens_out, int* count_out) {
    if (!mt || !mt->tp || !mt->tp->ast_pool || !obj || !names_out ||
            !lens_out || !count_out) return false;
    int count = 0;
    for (JsAstNode* node = obj->properties; node; node = node->next) {
        if (node->node_type != JS_AST_NODE_PROPERTY) return false;
        JsPropertyNode* prop = (JsPropertyNode*)node;
        if (prop->computed || prop->method || prop->is_getter ||
                prop->is_setter || !prop->key ||
                prop->key->node_type != JS_AST_NODE_IDENTIFIER ||
                jm_is_proto_literal_key(prop->key)) return false;
        count++;
    }
    if (count <= 0) return false;
    const char** names = (const char**)pool_calloc(mt->tp->ast_pool,
        (size_t)count * sizeof(char*));
    int* lens = (int*)pool_calloc(mt->tp->ast_pool,
        (size_t)count * sizeof(int));
    if (!names || !lens) return false;
    int index = 0;
    for (JsAstNode* node = obj->properties; node; node = node->next) {
        JsIdentifierNode* key = (JsIdentifierNode*)((JsPropertyNode*)node)->key;
        for (int prior = 0; prior < index; prior++) {
            if (lens[prior] == (int)key->name->len &&
                    strncmp(names[prior], key->name->chars, key->name->len) == 0) {
                return false;
            }
        }
        names[index] = key->name->chars;
        lens[index] = (int)key->name->len;
        index++;
    }
    *names_out = names;
    *lens_out = lens;
...
MIR_reg_t jm_transpile_object(JsMirTranspiler* mt, JsObjectNode* obj) {
    ...
    MIR_reg_t object = static_shape
        ? jm_call_3(mt, "js_new_object_with_shape", MIR_T_I64,
            ...
            MIR_T_P, MIR_new_int_op(mt->ctx, (int64_t)(uintptr_t)shape_lens),
            MIR_T_I64, MIR_new_int_op(mt->ctx, shape_count))
            ...
            if (p->is_getter || p->is_setter) {
                ...
                if (is_proto_literal) {
                    jm_call_void_2(mt, "js_object_proto_setter",
                        ...
                        MIR_T_I64, MIR_new_reg_op(mt->ctx, val));
                } else if (static_shape) {
                    jm_call_void_3(mt, "js_set_shaped_slot",
                        MIR_T_I64, MIR_new_reg_op(mt->ctx, object),
                        MIR_T_I64, MIR_new_int_op(mt->ctx, shape_slot),
                        MIR_T_I64, MIR_new_reg_op(mt->ctx, val));
                } else {
                    jm_call_3(mt, "js_create_data_property", MIR_T_I64,
                        MIR_T_I64, MIR_new_reg_op(mt->ctx, object),
...
MIR_reg_t jm_create_func_or_closure(JsMirTranspiler* mt, JsFuncCollected* fc) {
    ...
    if (fc->capture_count > 0) {
        ...
        if (use_scope_env) use_scope_env = jm_shared_scope_env_captures_valid(mt, fc, true);
        if (use_scope_env &&
...
MIR_reg_t jm_emit_module_const_value(JsMirTranspiler* mt,
    ...
    case MCONST_NULL:
        return jm_emit_null(mt);
    case MCONST_UNDEFINED: {
        MIR_reg_t undef = jm_new_reg(mt, "mundef", MIR_T_I64);
        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_MOV,
...
MIR_reg_t jm_transpile_func_expr(JsMirTranspiler* mt, JsFunctionNode* fn) {
    ...
    if (fc->capture_count > 0) {
        ...
        bool use_scope_env = (!force_copied_field_env && !fc->closure_env_has_parent_link &&
            mt->scope_env_reg != 0 && fc->captures[0].scope_env_slot >= 0);
        if (use_scope_env) use_scope_env = jm_shared_scope_env_captures_valid(mt, fc, false);
        if (use_scope_env &&
                (!mt->allow_loop_let_scope_env_for_immediate_call ||
                 fc->closure_env_has_parent_link)) {
            for (int ci = 0; ci < fc->capture_count; ci++) {
...
static MIR_reg_t jm_emit_class_static_field_value(JsMirTranspiler* mt,
                                                   JsStaticFieldEntry* sf) {
    if (sf->initializer) return jm_transpile_box_item(mt, sf->initializer);
    MIR_reg_t value = jm_new_reg(mt, "sf_undef", MIR_T_I64);
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_MOV,
        MIR_new_reg_op(mt->ctx, value),
        MIR_new_int_op(mt->ctx, (int64_t)ITEM_JS_UNDEFINED)));
    return value;
...
static void jm_emit_class_static_property(JsMirTranspiler* mt, MIR_reg_t cls_obj,
...
MIR_reg_t jm_transpile_expression(JsMirTranspiler* mt, JsAstNode* expr) {
    ...
    case JS_AST_NODE_CALL_EXPRESSION: {
        ...
        // A mark exists only when lowering pushed a transient argument buffer.
        // Restore before observing exceptions on both normal and throw paths.
        ...
        jm_scope_env_reload_vars(mt);
        // test each callee's returned ERROR lane — if it threw,
    ...
    case JS_AST_NODE_NEW_EXPRESSION: {
        ...
        jm_end_arg_stack_scope(mt, &arg_scope);
        jm_scope_env_reload_vars(mt);
        // test the constructor's returned ERROR lane
        jm_emit_error_lane_propagate_check(mt);
        jm_env_reload_shared_captures(mt);
        return r;
    }
    ...
    case JS_AST_NODE_YIELD_EXPRESSION: {
        ...
        if (mt->in_generator) {
            ...
            {
                MIR_reg_t is_throw_signal = jm_call_1(mt, "js_gen_is_throw_signal", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, mt->gen_input_reg));
                MIR_label_t no_throw_signal = jm_new_label(mt);
                jm_emit(mt, MIR_new_insn(mt->ctx, MIR_BF,
                    MIR_new_label_op(mt->ctx, no_throw_signal),
                    MIR_new_reg_op(mt->ctx, is_throw_signal)));
                MIR_reg_t throw_value = jm_call_1(mt, "js_gen_throw_signal_value", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, mt->gen_input_reg));
        ...
        if (cls_expr->superclass &&
            ...
            if (ce) {
                ...
                for (int class_index = static_chain_length - 1; class_index >= 0; class_index--) {
                    ...
                    for (int method_index = 0; method_index < parent->method_count; method_index++) {
                        JsMirClassMethodInstallPolicy policy = {
                            cls_obj, parent_class, 0, parent, method_index,
                ...
                for (int method_index = 0; method_index < ce->method_count; method_index++) {
                    JsMirClassMethodInstallPolicy policy = {
                        cls_obj, cls_obj, 0, ce, method_index,
                        JS_MIR_CLASS_METHOD_OWN_STATIC, JS_MIR_COMPUTED_KEY_AFTER_FUNCTION
                    };
                    jm_emit_class_method_install(mt, &policy);
                }
            }
            ...
            if (ce) {
                ...
                jm_call_void_2(mt, "js_set_default_constructor_property",
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, proto_obj),
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, cls_obj));
                jm_call_void_2(mt, "js_refresh_prototype_method_homes",
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, proto_obj),
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, cls_obj));
                ...
                for (int method_index = 0; method_index < ce->method_count; method_index++) {
                    JsMirClassMethodInstallPolicy policy = {
                        ...
                        JS_MIR_CLASS_METHOD_OWN_INSTANCE, JS_MIR_COMPUTED_KEY_AFTER_FUNCTION
                    };
                    jm_emit_class_method_install(mt, &policy);
                }
            ...
            if (!emitted_ordered_static_elements) for (int fi = 0; ce && fi < ce->static_field_count; fi++) {
                ...
                if (sf->computed && sf->key_expr) {
                    ...
                    MIR_reg_t val = jm_emit_class_static_field_value(mt, sf);
                    if (cls_val_spill >= 0) {
                    ...
                    jm_emit_class_static_property(mt, cls_obj, key, val,
                        sf->name && jm_is_private_name(sf->name));
                    ...
                    jm_call_void_2(mt, "js_set_function_name_if_anonymous",
                        MIR_T_I64, MIR_new_reg_op(mt->ctx, val),
                        MIR_T_I64, MIR_new_reg_op(mt->ctx, key));
                    jm_emit_class_static_property(mt, cls_obj, key, val, false);
                } else if (sf->name) {
                    MIR_reg_t val = jm_emit_class_static_field_value(mt, sf);
                    MIR_reg_t fn_name = jm_box_string_literal(mt, sf->name->chars, (int)sf->name->len);
                    jm_call_void_2(mt, "js_set_function_name_if_anonymous",
