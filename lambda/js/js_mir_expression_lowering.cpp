...
static MIR_reg_t jm_emit_fixed_typed_array_read(JsMirTranspiler* mt,
        JsMemberNode* member, MIR_reg_t receiver, MIR_reg_t key);
static MIR_reg_t jm_emit_fixed_typed_array_read_number(JsMirTranspiler* mt,
        JsMemberNode* member, MIR_reg_t receiver, MIR_reg_t key);
static MIR_reg_t jm_emit_fixed_typed_array_store(JsMirTranspiler* mt,
        const JsMirReference* ref, MIR_reg_t value);
static MIR_reg_t jm_emit_fixed_typed_array_number_store(JsMirTranspiler* mt,
        const JsMirReference* ref, MIR_reg_t value, MIR_reg_t number);
static MIR_reg_t jm_emit_existing_array_number_store(JsMirTranspiler* mt,
        const JsMirReference* ref, MIR_reg_t value, MIR_reg_t number);
static MIR_reg_t jm_emit_existing_array_store(JsMirTranspiler* mt,
        const JsMirReference* ref, MIR_reg_t value);
static MIR_reg_t jm_emit_indexed_assignment_fallback(JsMirTranspiler* mt,
        const JsMirReference* ref, MIR_reg_t value);
...
static bool jm_shared_scope_env_captures_valid(JsMirTranspiler* mt,
    ...
    for (int ci = 0; ci < JM_CAPTURE_COUNT(fc); ci++) {
        JsMirVarEntry* cv = jm_find_var_by_binding(mt,
...
static void jm_prepare_closure_captures(JsMirTranspiler* mt, JsFuncCollected* fc,
    ...
    for (int ci = 0; ci < JM_CAPTURE_COUNT(fc); ci++) {
        ...
        if (!capture->is_nfe_binding && capture->scope_env_slot < 0 &&
            ...
            capture->scope_env_slot = jm_scope_env_slot_for_capture(
                mt->current_fc, capture);
        }
        JsMirVarEntry* cv = jm_find_var_by_binding(mt, capture->entry);
...
static MIR_reg_t jm_emit_logical_assignment(JsMirTranspiler* mt,
    ...
    MIR_reg_t cond;
    if (asgn->op == OPERATOR_JS_NULLISH_ASSIGN) {
        ...
        jm_emit(mt, MIR_new_insn(mt->ctx,
            asgn->op == OPERATOR_JS_AND_ASSIGN ? MIR_BT : MIR_BF,
    ...
    if (module_store) {
        ...
        jm_scope_env_mark_and_writeback_binding(mt, vname, (JsAstNode*)id, rhs);
    } else {
...
static bool jm_name_is(const String* name, const char* lit, int len) {
    return name && len >= 0 && name->len == (uint32_t)len &&
        strncmp(name->chars, lit, (size_t)len) == 0;
}
...
static bool jm_test262_assert_is_local(JsMirTranspiler* mt, JsIdentifierNode* id) {
    return id && id->entry && id->entry->node;
}
...
static bool jm_js_string_literal_eq(JsAstNode* node, const char* name, uint32_t len) {
    ...
    String* value = lit->literal_type == AST_LITERAL_STRING ?
...
static bool jm_call_needs_pending_call_source(JsMirTranspiler* mt, JsCallNode* call) {
    ...
    if (call->callee->node_type != AST_NODE_MEMBER_EXPR) return false;
...
static JsFuncCollected* jm_find_direct_function_decl_for_identifier(
    ...
    if (!definition || definition->node_type != AST_NODE_FUNC ||
...
static JsClassEntry* jm_current_inner_class_binding(JsMirTranspiler* mt,
        NameEntry* binding, JsAstNode* ref_node) {
    ...
    if (mt->current_class && mt->current_class->node &&
        mt->current_class->node->entry == binding &&
    ...
    if (mt->current_fc && mt->current_fc->node) {
        ...
        if (ce && ce->node && ce->node->entry == binding &&
...
static MIR_reg_t jm_emit_member_key(JsMirTranspiler* mt, JsMemberNode* mem) {
    ...
    if (mem->property && mem->property->node_type == AST_NODE_IDENT) {
...
void jm_emit_class_instance_field_metadata(JsMirTranspiler* mt, MIR_reg_t cls_obj, JsClassEntry* ce) {
    ...
    for (int member_index = 0; member_index < ce->member_count; member_index++) {
        ...
        if (inf->initializer && inf->initializer->node_type == AST_NODE_LITERAL) {
...
static void jm_emit_named_evaluation_for_identifier(JsMirTranspiler* mt, JsAstNode* rhs_node, MIR_reg_t rhs, String* name) {
    ...
    if (rhs_node->node_type == AST_NODE_FUNC_EXPR ||
        rhs_node->node_type == AST_NODE_ARROW_FUNC) {
        JsFunctionNode* fn_node = (JsFunctionNode*)rhs_node;
        if (!fn_node->name) {
            jm_emit_set_function_name(mt, rhs, name->chars);
        }
    } else if (rhs_node->node_type == AST_NODE_CLASS_EXPR ||
               rhs_node->node_type == AST_NODE_CLASS) {
...
JsMirReference jm_emit_reference(JsMirTranspiler* mt, JsAstNode* node) {
    ...
    if (node->node_type == AST_NODE_MEMBER_EXPR) {
        ...
        bool is_super = false;
        if (mem->object && mem->object->node_type == AST_NODE_IDENT) {
...
typedef struct JmWriteReferenceRhsScan {
    bool has_call;
    bool has_direct_eval;
} JmWriteReferenceRhsScan;

static void jm_scan_write_reference_rhs(JsAstNode* node,
        JmWriteReferenceRhsScan* scan);

static void jm_scan_write_reference_rhs_child(JsAstNode* child, void* opaque) {
    jm_scan_write_reference_rhs(child, (JmWriteReferenceRhsScan*)opaque);
}

// Function bodies are source text at this point; their writes become live only
// through a call, which the enclosing scan records separately.
static void jm_scan_write_reference_rhs(JsAstNode* node,
        JmWriteReferenceRhsScan* scan) {
    if (!node || !scan || scan->has_direct_eval) return;
    ...
    case AST_NODE_CALL_EXPR: {
        ...
        if (!call->optional && call->callee &&
            ...
            if (callee->name && callee->name->len == 4 &&
                    memcmp(callee->name->chars, "eval", 4) == 0) {
    ...
    js_ast_visit_children(node, jm_scan_write_reference_rhs_child, scan);
}

// A direct local identifier is the only ordinary expression producer that can
// return a mutable compiler home.  Reuse the indexed write census, then retain
// the snapshot around calls only for bindings that reload from a shared env.
static bool jm_write_reference_source_needs_capture(JsMirTranspiler* mt,
        JsAstNode* source, JsAstNode* rhs) {
    if (!mt || !source || !rhs || mt->with_depth > 0) return true;
    source = (JsAstNode*)ast_unwrap_primary((AstNode*)source);
    if (!source || source->node_type != AST_NODE_IDENT) return true;
    JsIdentifierNode* identifier = (JsIdentifierNode*)source;
    if (!identifier->entry) return true;

    struct hashmap* assigned = hashmap_new(sizeof(JsNameSetEntry), 8, 0, 0,
        jm_name_hash, jm_binding_cmp, NULL, NULL);
    if (!assigned) return true;
    jm_collect_indexed_func_assignments(mt, rhs, assigned);
    bool capture = jm_binding_set_has(assigned, identifier->entry);
    hashmap_free(assigned);
    if (capture) return true;

    JmWriteReferenceRhsScan scan = {};
    jm_scan_write_reference_rhs(rhs, &scan);
    if (scan.has_direct_eval) return true;
    if (!scan.has_call) return false;

    JsMirVarEntry* variable = jm_find_var_by_binding(mt, identifier->entry);
    return !variable || variable->in_scope_env || variable->from_env ||
        variable->from_shared_env;
}

// A write Reference is fixed before its RHS runs. Native local registers are
// mutable compiler homes, so snapshot only a source binding that the RHS can
// overwrite; member updates retain both parts through their coercion steps.
static void jm_capture_write_reference(JsMirTranspiler* mt,
        JsMirReference* ref, JsAstNode* rhs = NULL) {
    if (!mt || !ref) return;
    JsMemberNode* member = ref->member;
    bool capture_base = !rhs || !member ||
        jm_write_reference_source_needs_capture(mt, member->object, rhs);
    bool capture_key = !rhs || !member || !member->computed ||
        jm_write_reference_source_needs_capture(mt, member->property, rhs);
    if (capture_base && ref->base_reg != 0) {
        MIR_reg_t base = jm_new_reg(mt, "write_reference_base", MIR_T_I64);
        jm_emit_mov(mt, base, ref->base_reg);
        ref->base_reg = base;
        jm_create_gc_root_slot(mt, base);
    }
    if (capture_key && ref->native_key_reg != 0) {
        MIR_type_t key_type = ref->native_key_type == LMD_TYPE_FLOAT
            ? MIR_T_D : MIR_T_I64;
        MIR_reg_t key = jm_new_reg(mt, "write_reference_key", key_type);
        if (key_type == MIR_T_D) jm_emit_dmov(mt, key, ref->native_key_reg);
        else jm_emit_mov(mt, key, ref->native_key_reg);
        ref->native_key_reg = key;
    } else if (capture_key && ref->key_reg != 0) {
        MIR_reg_t key = jm_new_reg(mt, "write_reference_key", MIR_T_I64);
        jm_emit_mov(mt, key, ref->key_reg);
        ref->key_reg = key;
...
static const JubeTypeDef* jm_jube_seed_type(JsMirTranspiler* mt,
    ...
    if (jm_find_var_by_binding(mt, id->entry)) return NULL;
    if (mt->module_consts) {
        if (jm_find_module_const_by_binding(mt, id->entry)) return NULL;
    }
...
const JubeTypeDef* jm_infer_jube_type(JsMirTranspiler* mt, JsAstNode* node) {
    ...
    if (node->node_type == AST_NODE_IDENT) {
        JsIdentifierNode* id = (JsIdentifierNode*)node;
        JsMirVarEntry* var = id->entry
            ? jm_find_var_by_binding(mt, id->entry) : NULL;
    ...
    if (node->node_type == AST_NODE_MEMBER_EXPR) {
        ...
        if (member->computed || !member->property ||
                member->property->node_type != AST_NODE_IDENT) return NULL;
    ...
    case JS_MIR_REF_PROPERTY:
        ...
        if (ref->is_private) {
            ...
            if (ref->named_key_index != UINT32_MAX) {
                ...
                result = jm_emit_fixed_typed_array_store(mt, ref, value);
                if (!result) result = jm_emit_existing_array_store(mt, ref, value);
                if (!result) {
                    MIR_reg_t key = jm_emit_reference_key(mt, ref);
...
static const char* jm_compound_assign_fn(JsOperator op) {
    switch (op) {
    case OPERATOR_JS_ADD_ASSIGN: return "js_add";
    case OPERATOR_JS_SUB_ASSIGN: return "js_subtract";
    case OPERATOR_JS_MUL_ASSIGN: return "js_multiply";
    case OPERATOR_JS_DIV_ASSIGN: return "js_divide";
    case OPERATOR_JS_MOD_ASSIGN: return "js_modulo";
    case OPERATOR_JS_EXP_ASSIGN: return "js_power";
    case OPERATOR_JS_BIT_AND_ASSIGN: return "js_bitwise_and";
    case OPERATOR_JS_BIT_OR_ASSIGN: return "js_bitwise_or";
    case OPERATOR_JS_BIT_XOR_ASSIGN: return "js_bitwise_xor";
    case OPERATOR_JS_LSHIFT_ASSIGN: return "js_left_shift";
    case OPERATOR_JS_RSHIFT_ASSIGN: return "js_right_shift";
    case OPERATOR_JS_URSHIFT_ASSIGN: return "js_unsigned_right_shift";
    default: return "js_add";
    }
}

static bool jm_compound_assign_number_op(JsOperator op, Operator* out) {
    if (!out) return false;
    switch (op) {
    case OPERATOR_JS_ADD_ASSIGN: *out = OPERATOR_ADD; return true;
    case OPERATOR_JS_SUB_ASSIGN: *out = OPERATOR_SUB; return true;
    case OPERATOR_JS_MUL_ASSIGN: *out = OPERATOR_MUL; return true;
    case OPERATOR_JS_DIV_ASSIGN: *out = OPERATOR_DIV; return true;
    case OPERATOR_JS_MOD_ASSIGN: *out = OPERATOR_MOD; return true;
...
// Literal lowering keeps immediate Items unless a numeric consumer requests
// the raw double; that demand avoids a boxed constant's scratch-memory decode.
static MirValue jm_transpile_literal_value(JsMirTranspiler* mt,
        JsLiteralNode* lit, ValueRep required = VALUE_REP_NONE) {
    MIR_reg_t result = 0;
    ...
    case AST_LITERAL_NUMBER: {
        ...
        if (lit->is_bigint) {
            ...
            result = jm_box_bigint_literal(mt, lit->bigint_str);
            break;
        }
        double val = lit->value.number_value;
        if (required == VALUE_REP_F64) {
            result = jm_new_reg(mt, "literal_number", MIR_T_D);
            jm_emit_reg_op(mt, MIR_DMOV, result, MIR_new_double_op(mt->ctx, val));
            return jm_expression_value(mt, (JsAstNode*)lit, result,
                LMD_TYPE_FLOAT, VALUE_REP_F64);
        }
        result = jm_box_float_const(mt, val);
        break;
    }
    case AST_LITERAL_STRING: {
        String* sv = lit->value.string_value;
        result = jm_box_string_literal(mt, sv->chars, sv->len);
        break;
    }
    case AST_LITERAL_BOOLEAN: {
        uint64_t bval = lit->value.boolean_value ? ITEM_TRUE_VAL : ITEM_FALSE_VAL;
        result = jm_boxed_immediate_const(mt, bval, "bool");
        break;
    }
    ...
    return jm_expression_value(mt, (JsAstNode*)lit, result,
        jm_get_effective_type(mt, (JsAstNode*)lit), VALUE_REP_ITEM);
}
...
// Source binding lookup may emit a number of runtime checks, but it remains a
// local Item-producing primitive. The public expression producer below owns
// the descriptor and is the only identifier entry point for consumers.
static MIR_reg_t jm_emit_identifier_read(JsMirTranspiler* mt,
        JsIdentifierNode* id) {
    if (!id || !id->name) {
    ...
    JsClassEntry* inner_class_binding = jm_current_inner_class_binding(mt,
        id->entry, (JsAstNode*)id);
    ...
    if (mt->module_consts) {
        JsModuleConstEntry* mc = id->entry
        ...
        if (mc) {
            ...
            case MCONST_CLASS: {
                ...
                JsClassEntry* inner_ce = jm_current_inner_class_binding(mt,
                    id->entry, (JsAstNode*)id);
...
static MirValue jm_transpile_identifier_value(JsMirTranspiler* mt,
        JsIdentifierNode* id) {
    JsMirVarEntry* var = id ? jm_find_var_by_binding(mt, id->entry) : NULL;
    if (var && jm_is_native_type(var->type_id)) {
        return jm_expression_value(mt, (JsAstNode*)id, var->reg, var->type_id,
            var->type_id == LMD_TYPE_FLOAT ? VALUE_REP_F64 : VALUE_REP_I64);
    }
    return jm_expression_value(mt, (JsAstNode*)id,
        jm_emit_identifier_read(mt, id),
        jm_get_effective_type(mt, (JsAstNode*)id), VALUE_REP_ITEM);
}
...
static bool jm_uri_compare_arg_is_simple(JsAstNode* node) {
    ...
    return node->node_type == AST_NODE_IDENT ||
...
static int jm_typed_array_constructor_kind(JsAstNode* expression) {
    expression = expression ? (JsAstNode*)ast_unwrap_primary((AstNode*)expression) : NULL;
    if (expression && expression->node_type == AST_NODE_IDENT) {
        JsIdentifierNode* identifier = (JsIdentifierNode*)expression;
        NameEntry* binding = identifier->entry;
        JsAstNode* definition = binding ? (JsAstNode*)binding->node : NULL;
        if (definition && definition->node_type == AST_NODE_VARIABLE_DECLARATOR) {
            expression = ((JsVariableDeclaratorNode*)definition)->init;
        }
    }
    if (!expression || expression->node_type != AST_NODE_NEW_EXPR) return -1;
    JsCallNode* construct = (JsCallNode*)expression;
    ...
    if (name->len == 10 && memcmp(name->chars, "Int32Array", 10) == 0) {
        return JS_TYPED_INT32;
    }
...
enum {
    JM_TYPED_ARRAY_KIND_UNKNOWN = -1,
    JM_TYPED_ARRAY_KIND_CYCLE = -2,
    JM_TYPED_ARRAY_INFERENCE_DEPTH_MAX = 8,
};

static int jm_typed_array_parameter_index(const JsFunctionNode* function,
        const NameEntry* binding) {
    if (!function || !binding) return -1;
    ...
    for (JsAstNode* parameter = function->params; parameter;
        ...
        if (parameter_id && parameter_id->entry == binding) return parameter_index;
    }
    return -1;
}

static JsFuncCollected* jm_typed_array_call_owner(JsMirTranspiler* mt,
        JsCallNode* call) {
    if (!mt || !mt->tp || !call) return NULL;
    AstIndex* index = &mt->tp->ast_index;
    AstNodeId node_id = ast_index_find(index, (AstNode*)call);
    if (node_id == AST_NODE_ID_INVALID || node_id >= index->count) return NULL;
    return jm_collected_func_by_id(mt, index->owner_functions[node_id]);
}

static int jm_typed_array_parameter_kind_impl(JsMirTranspiler* mt,
        JsFuncCollected* function, NameEntry* binding,
        NameEntry* const* active_bindings, int active_count);

static int jm_typed_array_argument_kind(JsMirTranspiler* mt, JsCallNode* call,
        JsAstNode* argument, NameEntry* const* active_bindings, int active_count) {
    int kind = jm_typed_array_constructor_kind(argument);
    if (kind >= 0) return kind;
    argument = argument ? (JsAstNode*)ast_unwrap_primary((AstNode*)argument) : NULL;
    if (!argument || argument->node_type != AST_NODE_IDENT) {
        return JM_TYPED_ARRAY_KIND_UNKNOWN;
    }
    JsIdentifierNode* identifier = (JsIdentifierNode*)argument;
    if (!identifier->entry || !identifier->entry->is_parameter) {
        return JM_TYPED_ARRAY_KIND_UNKNOWN;
    }
    JsFuncCollected* caller = jm_typed_array_call_owner(mt, call);
    return jm_typed_array_parameter_kind_impl(mt, caller, identifier->entry,
        active_bindings, active_count);
}

// Follow only stable direct-call parameter forwarding. The runtime kind guard
// still owns dynamic calls, rebinding, and every unsupported argument source.
static int jm_typed_array_parameter_kind_impl(JsMirTranspiler* mt,
        JsFuncCollected* function, NameEntry* binding,
        NameEntry* const* active_bindings, int active_count) {
    if (!mt || !mt->tp || !function || !function->node || !binding ||
            active_count >= JM_TYPED_ARRAY_INFERENCE_DEPTH_MAX) {
        return JM_TYPED_ARRAY_KIND_UNKNOWN;
    }
    int parameter_index = jm_typed_array_parameter_index(function->node, binding);
    if (parameter_index < 0) return JM_TYPED_ARRAY_KIND_UNKNOWN;
    for (int i = 0; i < active_count; i++) {
        if (active_bindings[i] == binding) return JM_TYPED_ARRAY_KIND_CYCLE;
    }

    NameEntry* next_active[JM_TYPED_ARRAY_INFERENCE_DEPTH_MAX] = {};
    for (int i = 0; i < active_count; i++) next_active[i] = active_bindings[i];
    next_active[active_count++] = binding;
    ...
    for (uint32_t call_index = 0; call_index < indexed_call_count; call_index++) {
        ...
        if (candidate < 0 || (kind >= 0 && kind != candidate)) {
            return JM_TYPED_ARRAY_KIND_UNKNOWN;
        }
        kind = candidate;
        call_count++;
    }
    return call_count > 0 ? kind : JM_TYPED_ARRAY_KIND_UNKNOWN;
}

static int jm_typed_array_parameter_kind(JsMirTranspiler* mt,
        JsIdentifierNode* identifier) {
    if (!mt || !mt->tp || !mt->current_fc || !mt->current_fc->node ||
            !identifier || !identifier->entry) return -1;
    return jm_typed_array_parameter_kind_impl(mt, mt->current_fc,
        identifier->entry, NULL, 0);
}

static int jm_fixed_typed_array_receiver_kind(JsMirTranspiler* mt,
        JsMemberNode* member) {
    if (!member || !member->object ||
            member->object->node_type != AST_NODE_IDENT) return -1;
    JsIdentifierNode* identifier = (JsIdentifierNode*)member->object;
    int parameter_kind = jm_typed_array_parameter_kind(mt, identifier);
    if (parameter_kind >= 0) return parameter_kind;
    if (!identifier->entry || !identifier->entry->node ||
            identifier->entry->node->node_type != AST_NODE_VARIABLE_DECLARATOR) {
        return -1;
    }
    JsVariableDeclaratorNode* declaration =
        (JsVariableDeclaratorNode*)identifier->entry->node;
    return jm_typed_array_constructor_kind(declaration->init);
}

bool jm_fixed_typed_array_member_number_candidate(JsMirTranspiler* mt,
        JsAstNode* node) {
    if (!mt || !node || node->node_type != AST_NODE_MEMBER_EXPR) return false;
    JsMemberNode* member = (JsMemberNode*)node;
    TypeId key_type = member->computed
        ? jm_get_effective_type(mt, member->property) : LMD_TYPE_ANY;
    return (key_type == LMD_TYPE_INT || key_type == LMD_TYPE_FLOAT) &&
        jm_fixed_typed_array_receiver_kind(mt, member) >= 0;
}

// Typed element helpers share one numeric key spelling so their guarded
// leaves receive the same Number index without materializing an Item key.
static MIR_reg_t jm_emit_native_key_as_number(JsMirTranspiler* mt,
        MIR_reg_t key, TypeId key_type, const char* reg_name) {
    if (!mt || !key || key_type != LMD_TYPE_INT) return key;
    MIR_reg_t numeric_key = jm_new_reg(mt, reg_name, MIR_T_D);
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_I2D,
        MIR_new_reg_op(mt->ctx, numeric_key), MIR_new_reg_op(mt->ctx, key)));
    return numeric_key;
}

static MIR_reg_t jm_emit_indexed_assignment_fallback(JsMirTranspiler* mt,
        const JsMirReference* ref, MIR_reg_t value) {
    if (!mt || !ref || !value) return value;
    // Reuse the evaluated Reference for every optimized indexed-store miss.
    // This is the sole complete Set continuation, preserving key conversion,
    // strict failure, descriptors, proxies, and assignment result semantics.
    MIR_reg_t key = jm_emit_reference_key(mt, ref);
    MIR_reg_t lane = jm_callr_1(mt, "js_property_lane_for_canonical_key",
        MIR_T_I64, key);
    MIR_reg_t set_result = jm_callr_5(mt, "js_set", MIR_T_I64,
        ref->base_reg, lane, key, value, ref->base_reg);
    return jm_call_5(mt, "js_assignment_set_result", MIR_T_I64,
        MIR_T_I64, MIR_new_reg_op(mt->ctx, value),
        MIR_T_I64, MIR_new_reg_op(mt->ctx, key),
...
static MIR_reg_t jm_emit_fixed_typed_array_store(JsMirTranspiler* mt,
    ...
    if (!mt || !ref || !ref->member || !ref->computed_key ||
            ...
            (ref->native_key_type != LMD_TYPE_INT &&
             ref->native_key_type != LMD_TYPE_FLOAT) ||
            jm_fixed_typed_array_receiver_kind(mt, ref->member) < 0) {
        return 0;
    }

    // The declaration admits the typed-array setter only as a runtime-checked
    // leaf. It preserves typed-index corner cases (including -0), coercion,
    // detach, and each exotic fallback in the existing runtime operation.
    MIR_reg_t numeric_key = jm_emit_native_key_as_number(mt,
        ref->native_key_reg, ref->native_key_type, "typed_store_key");

    MIR_reg_t result = jm_new_reg(mt, "typed_store", MIR_T_I64);
    MIR_label_t miss = jm_new_label(mt);
    MIR_label_t done = jm_new_label(mt);
    MIR_reg_t typed_result = jm_call_3(mt, "js_typed_array_set_numeric_key",
        MIR_T_I64, MIR_T_I64, MIR_new_reg_op(mt->ctx, ref->base_reg),
        MIR_T_D, MIR_new_reg_op(mt->ctx, numeric_key),
        MIR_T_I64, MIR_new_reg_op(mt->ctx, value));
    jm_emit_error_lane_propagate_check(mt);
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_BEQ,
        MIR_new_label_op(mt->ctx, miss), MIR_new_reg_op(mt->ctx, typed_result),
        MIR_new_int_op(mt->ctx, (int64_t)ITEM_NULL_VAL)));
    jm_emit_mov(mt, result, typed_result);
    jm_emit_jmp(mt, done);

    // A non-typed receiver can arise after a source binding is reassigned.
    // Reuse the original evaluated key for the one complete Set continuation.
    jm_emit_label(mt, miss);
    jm_emit_mov(mt, result, jm_emit_indexed_assignment_fallback(mt, ref,
        value));
    jm_emit_value_join(mt, done, result, JS_ERROR_LANE_UNKNOWN);
...
static MIR_reg_t jm_emit_fixed_typed_array_number_store(JsMirTranspiler* mt,
    ...
    int element_type = jm_fixed_typed_array_receiver_kind(mt, ref->member);
    if (element_type < 0) return 0;

    MIR_reg_t numeric_key = jm_emit_native_key_as_number(mt,
        ref->native_key_reg, ref->native_key_type, "typed_number_store_key");

    MIR_reg_t result = jm_new_reg(mt, "typed_number_store", MIR_T_I64);
    MIR_label_t miss = jm_new_label(mt);
    MIR_label_t done = jm_new_label(mt);
    MIR_reg_t completed = jm_call_4(mt, "js_typed_array_set_number_if_kind",
        MIR_T_I64, MIR_T_I64, MIR_new_reg_op(mt->ctx, ref->base_reg),
        MIR_T_D, MIR_new_reg_op(mt->ctx, numeric_key),
        MIR_T_I64, MIR_new_int_op(mt->ctx, element_type),
        MIR_T_D, MIR_new_reg_op(mt->ctx, number));
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_BEQ,
        MIR_new_label_op(mt->ctx, miss), MIR_new_reg_op(mt->ctx, completed),
        MIR_new_int_op(mt->ctx, 0)));
    jm_emit_mov(mt, result, value);
    jm_emit_jmp(mt, done);

    // A rebinding or a wrong typed-array kind must retain the one complete
    // Set continuation. `value` preserves the RHS's original Number form.
    jm_emit_label(mt, miss);
    jm_emit_mov(mt, result, jm_emit_indexed_assignment_fallback(mt, ref,
        value));
    jm_emit_value_join(mt, done, result, JS_ERROR_LANE_UNKNOWN);
...
static MIR_reg_t jm_emit_existing_array_number_store(JsMirTranspiler* mt,
        const JsMirReference* ref, MIR_reg_t value, MIR_reg_t number) {
    if (!mt || !ref || !ref->member || !ref->computed_key || !value ||
            !number || ref->native_key_reg == 0 ||
            (ref->native_key_type != LMD_TYPE_INT &&
             ref->native_key_type != LMD_TYPE_FLOAT) ||
            !jm_is_array_literal_candidate(mt, ref->member)) {
        return 0;
    ...
    MIR_reg_t numeric_key = ref->native_key_reg;
    if (ref->native_key_type == LMD_TYPE_INT) {
        numeric_key = jm_new_reg(mt, "packed_number_store_key", MIR_T_D);
        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_I2D,
            MIR_new_reg_op(mt->ctx, numeric_key),
            MIR_new_reg_op(mt->ctx, ref->native_key_reg)));
    }

    // The leaf proves a present own numeric slot before writing. Its false arm
    // owns every observable Set case, using the Reference already evaluated
    // before the RHS (S1.11, D8.2.3).
    MIR_reg_t result = jm_new_reg(mt, "packed_number_store", MIR_T_I64);
    MIR_label_t miss = jm_new_label(mt);
    MIR_label_t done = jm_new_label(mt);
    MIR_reg_t stored = jm_call_3(mt, "js_array_set_existing_number_no_gc",
        MIR_T_I64, MIR_T_I64, MIR_new_reg_op(mt->ctx, ref->base_reg),
        MIR_T_D, MIR_new_reg_op(mt->ctx, numeric_key),
        MIR_T_D, MIR_new_reg_op(mt->ctx, number));
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_BEQ,
        MIR_new_label_op(mt->ctx, miss), MIR_new_reg_op(mt->ctx, stored),
        MIR_new_int_op(mt->ctx, 0)));
    jm_emit_mov(mt, result, value);
    jm_emit_jmp(mt, done);
...
static bool jm_same_pure_array_member(JsMemberNode* left,
        JsMemberNode* right) {
    if (!left || !right || !left->computed || !right->computed ||
            left->optional || right->optional || !left->object ||
            !right->object || !left->property || !right->property ||
            left->object->node_type != AST_NODE_IDENT ||
            right->object->node_type != AST_NODE_IDENT ||
            left->property->node_type != AST_NODE_IDENT ||
            right->property->node_type != AST_NODE_IDENT) {
        return false;
    }
    JsIdentifierNode* left_object = (JsIdentifierNode*)left->object;
    JsIdentifierNode* right_object = (JsIdentifierNode*)right->object;
    JsIdentifierNode* left_key = (JsIdentifierNode*)left->property;
    JsIdentifierNode* right_key = (JsIdentifierNode*)right->property;
    // Identifiers have no observable evaluation step. The same resolved
    // bindings therefore let the fast arm reuse the already-built LHS
    // Reference; dynamic names and computed keys retain ordinary evaluation.
    return left_object->entry && left_object->entry == right_object->entry &&
        left_key->entry && left_key->entry == right_key->entry;
}

static bool jm_try_emit_packed_number_self_update(JsMirTranspiler* mt,
        JsAssignmentNode* assignment, const JsMirReference* ref,
        MIR_reg_t* out_result) {
    if (!mt || !assignment || !ref || !out_result || mt->with_depth > 0 ||
            assignment->op != OPERATOR_ASSIGN || !ref->member ||
    ...
    JsAstNode* rhs = assignment->right ? (JsAstNode*)ast_unwrap_primary(
        (AstNode*)assignment->right) : NULL;
    if (!rhs || rhs->node_type != AST_NODE_BINARY) return false;
    JsBinaryNode* binary = (JsBinaryNode*)rhs;
    JsAstNode* load_node = binary->left ? (JsAstNode*)ast_unwrap_primary(
        (AstNode*)binary->left) : NULL;
    if (!load_node || load_node->node_type != AST_NODE_MEMBER_EXPR ||
            !binary->right || binary->right->node_type != AST_NODE_LITERAL) {
        return false;
    }
    JsLiteralNode* constant = (JsLiteralNode*)binary->right;
    JsMemberNode* load_member = (JsMemberNode*)load_node;
    MirNumericOpPlan plan;
    if (constant->literal_type != AST_LITERAL_NUMBER || constant->is_bigint ||
            !em_numeric_op_plan(binary->op, &plan) || plan.is_comparison ||
            !jm_same_pure_array_member(ref->member, load_member)) {
        return false;
    }

    MIR_reg_t numeric_key = ref->native_key_reg;
    if (ref->native_key_type == LMD_TYPE_INT) {
        numeric_key = jm_new_reg(mt, "packed_update_key", MIR_T_D);
        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_I2D,
            MIR_new_reg_op(mt->ctx, numeric_key),
            MIR_new_reg_op(mt->ctx, ref->native_key_reg)));
    }
    MIR_reg_t result = jm_new_reg(mt, "packed_number_update", MIR_T_I64);
    MIR_label_t generic = jm_new_label(mt);
    MIR_label_t done = jm_new_label(mt);

    // The LHS Reference has already evaluated the two pure bindings. On the
    // direct arm, load, arithmetic, and existing-slot write stay F64. A miss
    // starts the normal RHS evaluation before applying that saved Reference.
    MIR_reg_t loaded = jm_emit_packed_array_load_number(mt, ref->member,
        ref->base_reg, numeric_key, generic);
    MIR_reg_t operand = jm_transpile_as_native(mt, binary->right,
        LMD_TYPE_FLOAT);
    MIR_reg_t updated;
    if (plan.helper_name) {
        updated = jm_call_2(mt, plan.helper_name, MIR_T_D,
            MIR_T_D, MIR_new_reg_op(mt->ctx, loaded),
            MIR_T_D, MIR_new_reg_op(mt->ctx, operand));
    } else {
        updated = jm_new_reg(mt, plan.reg_name, MIR_T_D);
        jm_emit_reg_binary(mt, plan.f64_opcode, updated, loaded, operand);
    }
    MIR_reg_t updated_item = jm_box_native(mt, updated, LMD_TYPE_FLOAT);
    jm_emit_mov(mt, result, jm_emit_existing_array_number_store(mt, ref,
        updated_item, updated));
    jm_emit_jmp(mt, done);

    jm_emit_label(mt, generic);
...
static int jm_literal_shape_count_static_field_uses(JsMirTranspiler* mt,
        JsObjectNode* object, const AstNodeId* member_ids,
    ...
    for (uint32_t i = 0; i < member_count; i++) {
        ...
        for (JsAstNode* property_node = object->properties; property_node;
            ...
            if (property_node->node_type != AST_NODE_PROPERTY) return -1;
...
static bool jm_recursive_literal_member_targets_function(JsMirTranspiler* mt,
    ...
    AstIndex* index = &mt->tp->ast_index;
    AstBindingId consumer_binding = ast_index_find_binding_entry(index,
        consumer->entry);
...
static int jm_recursive_literal_static_field_uses(JsMirTranspiler* mt,
    ...
    AstIndex* index = &mt->tp->ast_index;
    int uses = 0;
    // A returned node can reach a field through a consumer parameter, so its
    // source call does not directly own the eventual member expression.
...
void jm_plan_literal_field_shapes(JsMirTranspiler* mt) {
    ...
    for (int function_index = 0; function_index < mt->func_count; function_index++) {
        ...
        int static_field_uses = jm_recursive_literal_static_field_uses(mt,
            function, fields, field_count);
...
static bool jm_current_function_is_instance_constructor(JsMirTranspiler* mt) {
    if (!mt || !mt->current_class || !mt->current_fc) return false;
    for (int member_index = 0; member_index < mt->current_class->member_count;
            member_index++) {
        JsClassMember* method = jm_class_member_method(mt->current_class,
...
static bool jm_class_shape_field_name_matches(const JsMirStaticShapeField* fields,
        int field_count, String* name) {
    if (!fields || !name) return false;
    for (int field_index = 1; field_index < field_count; field_index++) {
        String* field_name = fields[field_index].name;
        if (field_name && name->len == field_name->len &&
                memcmp(name->chars, field_name->chars, name->len) == 0) {
            return true;
        }
    }
    return false;
...
static int jm_class_shape_static_field_use_count(JsMirTranspiler* mt,
        JsClassEntry* entry, const JsMirStaticShapeField* fields,
        int field_count) {
    ...
    for (int member_index = 0; member_index < entry->member_count;
        ...
        for (uint32_t node_id = function_id; node_id < function_end; node_id++) {
            ...
            String* name = ((JsIdentifierNode*)member->property)->name;
            if (jm_class_shape_field_name_matches(fields, field_count, name))
                static_field_uses++;
        }
    }
    return static_field_uses;
...
typedef struct JmConstructorShapeRhsScan {
    bool observes_receiver;
    bool has_direct_eval;
} JmConstructorShapeRhsScan;

static void jm_scan_constructor_shape_rhs(JsAstNode* node,
        JmConstructorShapeRhsScan* scan);

static void jm_scan_constructor_shape_rhs_child(JsAstNode* child,
        void* opaque) {
    jm_scan_constructor_shape_rhs(child,
        (JmConstructorShapeRhsScan*)opaque);
}
...
static void jm_scan_constructor_shape_rhs(JsAstNode* node,
    ...
    js_ast_visit_children(node, jm_scan_constructor_shape_rhs_child, scan);
}

static bool jm_constructor_shape_field_type(JsMirTranspiler* mt,
        JsAstNode* value, TypeId* out_type) {
    if (!mt || !value || !out_type) return false;
    JmConstructorShapeRhsScan scan = {};
    jm_scan_constructor_shape_rhs(value, &scan);
...
static bool jm_constructor_shape_is_directive_statement(JsAstNode* statement) {
    if (!statement || statement->node_type != AST_NODE_EXPR_STMT) return false;
    JsAstNode* expression = ((JsExpressionStatementNode*)statement)->expression;
    return expression && expression->node_type == AST_NODE_LITERAL &&
        ((JsLiteralNode*)expression)->literal_type == AST_LITERAL_STRING;
}

static bool jm_append_constructor_shape_field_assignment(
        JsMirTranspiler* mt, JsAstNode* expression,
        JsMirStaticShapeField* fields, int* field_count) {
    if (!mt || !expression || !fields || !field_count ||
            expression->node_type != AST_NODE_ASSIGN) return false;
    JsAssignmentNode* assignment = (JsAssignmentNode*)expression;
    JsMemberNode* member = assignment->left &&
            assignment->left->node_type == AST_NODE_MEMBER_EXPR
    ...
    if (!jm_constructor_shape_field_type(mt, assignment->right, &type_id) ||
            !name || jm_is_private_name(name) ||
...
static bool jm_append_straight_line_constructor_shape_fields(
        JsMirTranspiler* mt, JsFunctionNode* constructor,
        JsMirStaticShapeField* fields, int* field_count) {
    if (!mt || !constructor || !fields || !field_count || !constructor->body ||
            constructor->body->node_type != AST_NODE_BLOCK) return false;
    JsBlockNode* body = (JsBlockNode*)constructor->body;
    for (JsAstNode* statement = body->statements; statement;
            statement = statement->next) {
        if (statement->node_type != AST_NODE_EXPR_STMT) return false;
        JsAstNode* expression = ((JsExpressionStatementNode*)statement)->expression;
        if (jm_constructor_shape_is_directive_statement(statement)) continue;
        if (!jm_append_constructor_shape_field_assignment(mt, expression, fields,
                field_count)) return false;
    }
    return true;
}

static bool jm_append_plain_constructor_shape_prefix(JsMirTranspiler* mt,
        JsFunctionNode* constructor, JsMirStaticShapeField* fields,
        int* field_count) {
    if (!mt || !constructor || !fields || !field_count || !constructor->body ||
            constructor->body->node_type != AST_NODE_BLOCK) return false;
    ...
    for (JsAstNode* statement = body->statements; statement;
            statement = statement->next) {
        JsAstNode* expression = statement->node_type == AST_NODE_EXPR_STMT
            ? ((JsExpressionStatementNode*)statement)->expression : NULL;
        if (jm_constructor_shape_is_directive_statement(statement) &&
                !prefix_started) continue;
        if (jm_append_constructor_shape_field_assignment(mt, expression, fields,
                field_count)) {
            prefix_started = true;
            continue;
        }
        if (prefix_started) return true;
        JmConstructorShapeRhsScan scan = {};
        jm_scan_constructor_shape_rhs(statement, &scan);
        // The prelude may validate arguments, but it cannot observe this or
        // direct eval before the reserved source-order prefix (D8.4.1v2).
        if (scan.observes_receiver || scan.has_direct_eval) return false;
    }
    return prefix_started;
}
...
static TypeMap* jm_class_instance_shape_for_class(JsMirTranspiler* mt,
    ...
    JsFunctionNode* constructor = entry->constructor && entry->constructor->fc
        ? entry->constructor->fc->node : NULL;
    if (constructor && !jm_append_straight_line_constructor_shape_fields(mt,
            constructor, fields, &field_count)) {
        return NULL;
    }
    if (field_count == 1) return NULL;
    if (jm_class_shape_static_field_use_count(mt, entry, fields,
            field_count) < 2) {
        return NULL;
    }
...
static JsFunctionNode* jm_plain_constructor_function_for_binding(
        NameEntry* binding) {
    JsAstNode* definition = binding ? (JsAstNode*)binding->node : NULL;
    if (!definition) return NULL;
    if (definition->node_type == AST_NODE_FUNC) {
        return (JsFunctionNode*)definition;
    }
    ...
    return initializer && initializer->node_type == AST_NODE_FUNC_EXPR
        ? (JsFunctionNode*)initializer : NULL;
}

static bool jm_plain_constructor_source(JsFunctionNode* function) {
    return function && !function->is_arrow && !function->is_async &&
        !function->is_generator && (function->node_type == AST_NODE_FUNC ||
...
static JsFunctionNode* jm_plain_constructor_for_prototype_target(
    ...
    JsIdentifierNode* property_name = (JsIdentifierNode*)target->property;
    if (!property_name->name || property_name->name->len != 9 ||
            memcmp(property_name->name->chars, "prototype", 9) != 0) {
        return NULL;
    }
    return jm_plain_constructor_function_for_binding(
...
static int jm_plain_constructor_shape_static_field_use_count(
        JsMirTranspiler* mt, JsFunctionNode* constructor,
        const JsMirStaticShapeField* fields, int field_count) {
    if (!mt || !mt->tp || !constructor) return 0;
    AstIndex* index = &mt->tp->ast_index;
    int static_field_uses = 0;
    for (uint32_t node_id = 0; node_id < index->count; node_id++) {
        ...
        String* name = ((JsIdentifierNode*)member->property)->name;
        if (!jm_class_shape_field_name_matches(fields, field_count, name)) continue;
        if (jm_plain_constructor_new_receiver_matches(mt, member->object,
                constructor)) {
            static_field_uses++;
            continue;
        }
        if (!jm_member_uses_this(member)) continue;
        AstFunctionId owner = index->owner_functions[node_id];
        if (owner == AST_FUNCTION_ID_INVALID || owner >= index->function_count) {
            continue;
        }
        JsFunctionNode* owner_function = (JsFunctionNode*)index->functions[owner].node;
        if (jm_plain_constructor_for_prototype_method(mt, owner_function) ==
                constructor) {
            static_field_uses++;
        }
    }
...
TypeMap* jm_plain_function_instance_shape_for_function(JsMirTranspiler* mt,
        JsFunctionNode* function) {
    JsFuncCollected* fc = jm_find_collected_func(mt, function);
    if (!fc) return NULL;
    if (fc->instance_shape_planned) return fc->instance_shape;
    fc->instance_shape_planned = true;
    if (!jm_plain_constructor_source(function)) return NULL;

    JsMirStaticShapeField fields[16] = {};
    // Reserve the proto position while the source proof is still speculative.
    // Materializing its NamePool string for every rejected method changes the
    // generated module's constant table even though no recipe is published.
    int field_count = 1;
    // The field run is exact and source ordered. An argument-only prelude and
    // work after the first non-field statement retain the generic path.
    if (!jm_append_plain_constructor_shape_prefix(mt, function, fields,
            &field_count) || field_count == 1 ||
            jm_plain_constructor_shape_static_field_use_count(mt, function,
    ...
    if (!internal_proto) return NULL;
    fields[0] = {internal_proto, LMD_TYPE_MAP};
    fc->instance_shape = jm_build_static_shape(mt, fields, field_count);
    return fc->instance_shape;
}

static TypeMap* jm_current_plain_prototype_instance_shape(
        JsMirTranspiler* mt) {
    if (!mt || !mt->current_fc || !mt->current_fc->node) return NULL;
    JsFunctionNode* constructor = jm_plain_constructor_for_prototype_method(mt,
        mt->current_fc->node);
    return constructor ? jm_plain_function_instance_shape_for_function(mt,
        constructor) : NULL;
}
...
static MIR_reg_t jm_emit_predicted_literal_field_data_guard(JsMirTranspiler* mt,
    ...
    // Exact shape identity proves the ordinary layout, but a prototype setter
    // can intercept a source assignment and leave this per-instance slot
    // absent. Keep that publication bit as a guard, not as a recipe promise
    // (D8.4.1v2).
...
static MIR_reg_t jm_emit_predicted_literal_field_store(JsMirTranspiler* mt,
    ...
    if (!ref || !ref->member || ref->computed_key || ref->is_private ||
            ref->named_key_index == UINT32_MAX ||
            (jm_current_function_is_instance_constructor(mt) &&
             jm_member_uses_this(ref->member))) return 0;
    String* name = NULL;
    MirFieldAccessPlan plan = {};
...
static bool jm_fixed_typed_array_number_layout(int element_type, int* width,
        MIR_type_t* storage_type, bool* integer) {
    if (!width || !storage_type || !integer) return false;
    switch (element_type) {
    case JS_TYPED_UINT8:
        *width = 1;
        *storage_type = MIR_T_U8;
        *integer = true;
        return true;
    case JS_TYPED_INT32:
        *width = 4;
        *storage_type = MIR_T_I32;
        *integer = true;
        return true;
    case JS_TYPED_FLOAT64:
        *width = (int)sizeof(double);
        *storage_type = MIR_T_D;
        *integer = false;
        return true;
    default:
        return false;
...
// Validate the exact typed-index snapshot once. Its raw data borrow must be
// dereferenced before another runtime call (D8.4.1v2).
static MIR_reg_t jm_emit_fixed_typed_array_data_if_kind(JsMirTranspiler* mt,
        JsMemberNode* member, MIR_reg_t receiver, MIR_reg_t key,
        MIR_label_t miss, MIR_reg_t* out_index) {
    int element_type = jm_fixed_typed_array_receiver_kind(mt, member);
    if (!mt || element_type < 0 || !member || !member->computed || !out_index) {
        return 0;
    }
    ...
    MIR_reg_t data = jm_call_3(mt, "js_typed_array_data_at_if_kind",
        MIR_T_I64, MIR_T_I64, MIR_new_reg_op(mt->ctx, receiver),
        MIR_T_I64, MIR_new_int_op(mt->ctx, element_type),
...
static MIR_reg_t jm_emit_fixed_typed_array_load_number(JsMirTranspiler* mt,
        JsMemberNode* member, MIR_reg_t receiver, MIR_reg_t key,
        MIR_label_t miss) {
    int element_type = jm_fixed_typed_array_receiver_kind(mt, member);
    int width = 0;
    MIR_type_t storage_type = MIR_T_UNDEF;
    bool integer = false;
    if (!mt || element_type < 0 || !member || !member->computed ||
            !jm_fixed_typed_array_number_layout(element_type, &width,
                &storage_type, &integer)) return 0;

    // The declaration selects only the element-kind arm. The raw borrow is
    // consumed before any later call (D8.4.1v2).
    MirEmitter* emitter = &mt->func_em->em;
    MIR_reg_t index = 0;
    MIR_reg_t data = jm_emit_fixed_typed_array_data_if_kind(mt, member,
        receiver, key, miss, &index);
    if (!data) return 0;
    MIR_reg_t address = em_element_address(emitter, data, index, width);
    MIR_reg_t loaded = em_load_at(emitter, address, 0, storage_type,
        integer ? "typed_integer" : "typed_float64");
    if (!integer) return loaded;
    MIR_reg_t numeric = jm_new_reg(mt, "typed_integer_f64", MIR_T_D);
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_I2D,
        MIR_new_reg_op(mt->ctx, numeric), MIR_new_reg_op(mt->ctx, loaded)));
    return numeric;
}

static MIR_reg_t jm_emit_fixed_typed_array_read_impl(JsMirTranspiler* mt,
        JsMemberNode* member, MIR_reg_t receiver, MIR_reg_t key,
        bool native_number) {
    int element_type = jm_fixed_typed_array_receiver_kind(mt, member);
    if (!mt || element_type < 0 || !member || !member->computed) return 0;
    MIR_reg_t result = jm_new_reg(mt, native_number ? "typed_read_f64" :
        "typed_read", native_number ? MIR_T_D : MIR_T_I64);
    MIR_label_t miss = jm_new_label(mt);
    MIR_label_t done = jm_new_label(mt);
    MIR_reg_t numeric = jm_emit_fixed_typed_array_load_number(mt, member,
        receiver, key, miss);
    if (!numeric) return 0;
    if (native_number) {
        jm_emit_dmov(mt, result, numeric);
    } else if (element_type == JS_TYPED_UINT8 || element_type == JS_TYPED_INT32) {
        jm_emit_mov(mt, result, jm_box_native(mt,
            jm_emit_double_to_int(mt, numeric), LMD_TYPE_INT));
    } else {
        jm_emit_mov(mt, result, jm_box_native(mt, numeric, LMD_TYPE_FLOAT));
    }
    jm_emit_jmp(mt, done);
...
static MIR_reg_t jm_emit_fixed_typed_array_read(JsMirTranspiler* mt,
        JsMemberNode* member, MIR_reg_t receiver, MIR_reg_t key) {
    return jm_emit_fixed_typed_array_read_impl(mt, member, receiver, key,
        false);
}
...
static MIR_reg_t jm_emit_packed_array_read_impl(JsMirTranspiler* mt,
        JsMemberNode* member, MIR_reg_t receiver, MIR_reg_t key,
        bool native_number, MIR_label_t external_miss) {
    if (!mt || !member || !member->computed ||
            !jm_is_array_literal_candidate(mt, member)) {
        return 0;
    }

    // The direct plan inlines a plain packed tagged array. A companion-bearing
    // array can still prove one present own dense element through the existing
    // no-GC runtime leaf; every hole or descriptor state keeps the JS fallback
    // (D1.3, D8.2.3).
    MIR_reg_t result = jm_new_reg(mt, native_number ? "dense_read_f64" :
        "dense_read", native_number ? MIR_T_D : MIR_T_I64);
    MIR_label_t miss = external_miss ? external_miss : jm_new_label(mt);
    MIR_label_t done = jm_new_label(mt);
    ...
    jm_emit_label(mt, tagged);
    MIR_label_t tagged_with_props = jm_new_label(mt);
    MIR_label_t tagged_value = jm_new_label(mt);
    // MIR has no phi register at this control-flow join. Both physical arms
    // therefore copy their value into this common carrier before decoding it.
    MIR_reg_t loaded = jm_new_reg(mt, "dense_loaded", MIR_T_I64);
    ...
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_BNE,
        MIR_new_label_op(mt->ctx, tagged_with_props),
        MIR_new_reg_op(mt->ctx, attr_type),
        MIR_new_int_op(mt->ctx, 0)));
    ...
    MIR_reg_t plain_loaded = em_load_at(emitter, address, 0, MIR_T_I64,
        "dense_plain_loaded");
    jm_emit_mov(mt, loaded, plain_loaded);
    jm_emit_jmp(mt, tagged_value);

    jm_emit_label(mt, tagged_with_props);
    MIR_reg_t companion_loaded = jm_callr_2(mt,
        "js_array_get_existing_own_dense_with_props_or_missing", MIR_T_I64,
        receiver, index);
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_BEQ,
        MIR_new_label_op(mt->ctx, miss), MIR_new_reg_op(mt->ctx, companion_loaded),
        MIR_new_int_op(mt->ctx, (int64_t)JS_DELETED_SENTINEL_VAL)));
    jm_emit_mov(mt, loaded, companion_loaded);

    jm_emit_label(mt, tagged_value);
    if (!native_number) {
        jm_emit_mov(mt, result, loaded);
    } else {
        // A packed tagged slot can bypass ToNumber only when it already holds
        // a JS Number. Other primitives retain the exact generic Get/ToNumber
        // continuation below (S1.11).
        MIR_reg_t inline_bits = jm_new_reg(mt, "dense_number_bits", MIR_T_I64);
        jm_emit_reg_binary_op(mt, MIR_AND, inline_bits, loaded,
            MIR_new_int_op(mt->ctx, (int64_t)ITEM_DBL_MASK));
        MIR_label_t tagged_float = jm_new_label(mt);
        MIR_label_t tagged_int = jm_new_label(mt);
        jm_emit_branch(mt, MIR_BT, tagged_float, inline_bits);
        MIR_reg_t tag = jm_new_reg(mt, "dense_number_tag", MIR_T_I64);
        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_URSH,
            MIR_new_reg_op(mt->ctx, tag), MIR_new_reg_op(mt->ctx, loaded),
            MIR_new_int_op(mt->ctx, 56)));
        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_BEQ,
            MIR_new_label_op(mt->ctx, tagged_int), MIR_new_reg_op(mt->ctx, tag),
            MIR_new_int_op(mt->ctx, LMD_TYPE_INT)));
        jm_emit_jmp(mt, miss);

        jm_emit_label(mt, tagged_float);
        jm_emit_dmov(mt, result, em_emit_bits_double(emitter, loaded));
        jm_emit_jmp(mt, done);

        jm_emit_label(mt, tagged_int);
        MIR_reg_t integer = jm_emit_unbox_int(mt, loaded);
        MIR_reg_t number = jm_new_reg(mt, "dense_int_f64", MIR_T_D);
        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_I2D,
            MIR_new_reg_op(mt->ctx, number), MIR_new_reg_op(mt->ctx, integer)));
        jm_emit_dmov(mt, result, number);
    }
    jm_emit_jmp(mt, done);
    ...
    if (!native_number) {
        ...
        MIR_reg_t number = jm_new_reg(mt, "numeric_i64_f64", MIR_T_D);
        jm_emit(mt, MIR_new_insn(mt->ctx, MIR_I2D,
            MIR_new_reg_op(mt->ctx, number), MIR_new_reg_op(mt->ctx, numeric_i64)));
        jm_emit_dmov(mt, result, number);
    }
    jm_emit_jmp(mt, done);
    ...
    if (!native_number) {
        ...
        jm_emit_dmov(mt, result, numeric_f64);
    }
    jm_emit_jmp(mt, done);
...
static const char* jm_numeric_binary_fallback_fn(JsOperator op) {
    switch (op) {
    case OPERATOR_ADD: return "js_add";
    case OPERATOR_SUB: return "js_subtract";
    case OPERATOR_MUL: return "js_multiply";
    case OPERATOR_DIV: return "js_divide";
    case OPERATOR_MOD: return "js_modulo";
    case OPERATOR_JS_EXP: return "js_power";
    default: return NULL;
    }
}
...
static TypeId jm_numeric_binary_operand_type(JsMirTranspiler* mt,
    ...
    if (node && node->node_type == AST_NODE_IDENT) {
        JsIdentifierNode* identifier = (JsIdentifierNode*)node;
        type = jm_numeric_binding_type(mt, identifier->entry);
        if (type == LMD_TYPE_INT || type == LMD_TYPE_FLOAT) return type;
    }
    return LMD_TYPE_ANY;
}
...
static MIR_reg_t jm_emit_typed_array_number_binary_slow(
        JsMirTranspiler* mt, const char* fallback_fn, bool member_is_left,
        const JsMirReference* ref, JsAstNode* peer, MIR_reg_t peer_native) {
...
static bool jm_try_emit_typed_array_number_binary(JsMirTranspiler* mt,
    ...
    if (!mt || !bin || !out_result) return false;
    bool member_is_left = bin->left &&
        bin->left->node_type == AST_NODE_MEMBER_EXPR;
    JsAstNode* member_node = member_is_left ? bin->left : bin->right;
    JsAstNode* peer = member_is_left ? bin->right : bin->left;
    if (!member_node || member_node->node_type != AST_NODE_MEMBER_EXPR ||
            !peer) return false;
    const char* fallback_fn = jm_numeric_binary_fallback_fn(bin->op);
    MirNumericOpPlan plan;
    if (!fallback_fn || !em_numeric_op_plan(bin->op, &plan) ||
            plan.is_comparison) return false;
    JsMemberNode* member = (JsMemberNode*)member_node;
    TypeId peer_type = jm_numeric_binary_operand_type(mt, peer);
    // Keep this proof aligned with jm_emit_reference().  A binding fact can
    // prove an operand Number, but only the reference's effective type admits
    // a native key register before either operand has been evaluated.
    TypeId key_type = member->computed
        ? jm_get_effective_type(mt, member->property) : LMD_TYPE_ANY;
    if ((peer_type != LMD_TYPE_INT && peer_type != LMD_TYPE_FLOAT) ||
            (key_type != LMD_TYPE_INT && key_type != LMD_TYPE_FLOAT) ||
            jm_fixed_typed_array_receiver_kind(mt, member) < 0) {
        return false;
    }

    // Preserve JavaScript's left-to-right Get order. The native peer is pure
    // with respect to ToNumeric; the guarded member arm alone supplies the
    // fast Number witness and its generic miss resumes from that Reference.
    MIR_reg_t peer_native = 0;
    if (!member_is_left) {
        peer_native = jm_transpile_as_native(mt, peer, LMD_TYPE_FLOAT);
    }
    JsMirReference ref = jm_emit_reference(mt, member_node);
    jm_emit_error_lane_propagate_check(mt);
    if (ref.native_key_reg == 0) {
        *out_result = jm_emit_typed_array_number_binary_slow(mt, fallback_fn,
            member_is_left, &ref, peer, peer_native);
        jm_emit_compile_profile(mt, JS_OPT_MIR_NUMBER_ADMITTED,
            JS_OPT_OUTCOME_TAKEN);
        return true;
    }
    MIR_reg_t numeric_key = jm_emit_native_key_as_number(mt,
        ref.native_key_reg, ref.native_key_type, "typed_binary_index");

    MIR_reg_t result = jm_new_reg(mt, "typed_binary", MIR_T_I64);
    MIR_label_t miss = jm_new_label(mt);
    MIR_label_t done = jm_new_label(mt);
    MIR_reg_t member_number = jm_emit_fixed_typed_array_load_number(mt, member,
        ref.base_reg, numeric_key, miss);
    if (!member_number) {
        *out_result = jm_emit_typed_array_number_binary_slow(mt, fallback_fn,
            member_is_left, &ref, peer, peer_native);
        jm_emit_compile_profile(mt, JS_OPT_MIR_NUMBER_ADMITTED,
    ...
    MIR_reg_t left = member_is_left ? member_number : peer_native;
    MIR_reg_t right = member_is_left ? peer_native : member_number;
    MIR_reg_t numeric_result;
    if (plan.helper_name) {
        numeric_result = jm_call_2(mt, plan.helper_name, MIR_T_D,
            MIR_T_D, MIR_new_reg_op(mt->ctx, left),
            MIR_T_D, MIR_new_reg_op(mt->ctx, right));
    } else {
        numeric_result = jm_new_reg(mt, plan.reg_name, MIR_T_D);
        jm_emit_reg_binary(mt, plan.f64_opcode, numeric_result, left, right);
    }
    jm_emit_mov(mt, result, jm_box_native(mt, numeric_result, LMD_TYPE_FLOAT));
    jm_emit_jmp(mt, done);
...
static bool jm_try_emit_packed_array_strict_equal(JsMirTranspiler* mt,
        JsBinaryNode* bin, MIR_reg_t* out_result) {
    if (!mt || !bin || !out_result ||
            (bin->op != OPERATOR_JS_STRICT_EQ &&
             bin->op != OPERATOR_JS_STRICT_NE)) {
        return false;
    }
    JsAstNode* left_node = (JsAstNode*)ast_unwrap_primary((AstNode*)bin->left);
    JsAstNode* right_node = (JsAstNode*)ast_unwrap_primary((AstNode*)bin->right);
    if (!left_node || !right_node || left_node->node_type != AST_NODE_MEMBER_EXPR ||
    ...
    MIR_reg_t result = jm_new_reg(mt, "packed_strict_equal", MIR_T_I64);
    MIR_label_t left_miss = jm_new_label(mt);
    MIR_label_t right_miss = jm_new_label(mt);
    MIR_label_t done = jm_new_label(mt);

    // Evaluate the left MemberExpression fully before beginning the right one.
    // Direct guards do not run guest code; either miss resumes the ordinary Get
    ...
    jm_emit(mt, MIR_new_insn(mt->ctx, MIR_DEQ,
        MIR_new_reg_op(mt->ctx, equal), MIR_new_reg_op(mt->ctx, left_number),
        MIR_new_reg_op(mt->ctx, right_number)));
    if (bin->op == OPERATOR_JS_STRICT_NE) {
        jm_emit_reg_binary_op(mt, MIR_XOR, equal, equal,
            MIR_new_int_op(mt->ctx, 1));
    }
    jm_emit_reg_op_binary(mt, MIR_OR, result,
        MIR_new_int_op(mt->ctx, (int64_t)ITEM_FALSE_VAL), equal);
    jm_emit_jmp(mt, done);
    ...
    if (bin->op == OPERATOR_JS_STRICT_NE) {
        jm_emit_reg_binary_op(mt, MIR_XOR, left_generic, left_generic,
            MIR_new_int_op(mt->ctx, 1));
    }
    jm_emit_mov(mt, result, left_generic);
    jm_emit_jmp(mt, done);
    ...
    if (bin->op == OPERATOR_JS_STRICT_NE) {
        jm_emit_reg_binary_op(mt, MIR_XOR, right_generic, right_generic,
            MIR_new_int_op(mt->ctx, 1));
    }
...
static MirValue jm_emit_binary_expression(JsMirTranspiler* mt,
    ...
    if (bin->op == OPERATOR_ADD) {
        ...
        if (jm_test262_fast_paths_enabled(mt) &&
            ...
            return publish(jm_callr_2(mt, "js_test262_concat_percent_hex", MIR_T_I64,
                left_reg, n_reg), VALUE_REP_ITEM);
        }
#endif
    }
    ...
    if (bin->op == OPERATOR_JS_STRICT_EQ || bin->op == OPERATOR_JS_STRICT_NE) {
        MIR_reg_t result;
        if (jm_try_emit_uri_compare_fast_path(mt, bin->left, bin->right, &result)) {
            if (bin->op == OPERATOR_JS_STRICT_NE) {
                return publish(jm_callr_1(mt, "js_logical_not", MIR_T_I64, result),
                    VALUE_REP_ITEM);
            }
            return publish(result, VALUE_REP_ITEM);
        }
        if (jm_try_emit_packed_array_strict_equal(mt, bin, &result)) {
            return publish(result, VALUE_REP_ITEM);
        }
    }
    ...
    if (jm_try_emit_typed_array_number_binary(mt, bin, &typed_binary)) {
        return publish(typed_binary, VALUE_REP_ITEM);
    }
    ...
    if (bin->op == OPERATOR_OR || bin->op == OPERATOR_AND || bin->op == OPERATOR_JS_NULLISH_COALESCE) {
        ...
        MIR_reg_t cond;
        if (bin->op == OPERATOR_JS_NULLISH_COALESCE) {
        ...
        } else if (bin->op == OPERATOR_JS_NULLISH_COALESCE) {
    ...
    {
        bool is_typeof_eq = (bin->op == OPERATOR_JS_STRICT_EQ || bin->op == OPERATOR_EQ);
        bool is_typeof_ne = (bin->op == OPERATOR_JS_STRICT_NE || bin->op == OPERATOR_NE);
        if (is_typeof_eq || is_typeof_ne) {
            JsAstNode* typeof_side = NULL;
            JsAstNode* literal_side = NULL;
            if (bin->left && bin->left->node_type == AST_NODE_UNARY &&
                ((JsUnaryNode*)bin->left)->op == OPERATOR_JS_TYPEOF) {
                typeof_side = bin->left; literal_side = bin->right;
            } else if (bin->right && bin->right->node_type == AST_NODE_UNARY &&
                       ((JsUnaryNode*)bin->right)->op == OPERATOR_JS_TYPEOF) {
                typeof_side = bin->right; literal_side = bin->left;
            }
            if (typeof_side && literal_side &&
                literal_side->node_type == AST_NODE_LITERAL) {
                ...
                if (lit->literal_type == AST_LITERAL_STRING && lit->value.string_value) {
                    ...
                    if (operand && operand->node_type == AST_NODE_IDENT) {
    ...
    switch (bin->op) {
    case OPERATOR_ADD:        fn_name = "js_add"; break;
    case OPERATOR_SUB:        fn_name = "js_subtract"; break;
    case OPERATOR_MUL:        fn_name = "js_multiply"; break;
    case OPERATOR_DIV:        fn_name = "js_divide"; break;
    case OPERATOR_MOD:        fn_name = "js_modulo"; break;
    case OPERATOR_JS_EXP:        fn_name = "js_power"; break;
    case OPERATOR_EQ:         fn_name = "js_equal"; break;
    case OPERATOR_NE:         fn_name = "js_equal"; invert_box = true; break;
    case OPERATOR_JS_STRICT_EQ:  fn_name = "js_strict_equal"; break;
    case OPERATOR_JS_STRICT_NE:  fn_name = "js_strict_equal"; invert_box = true; break;
    ...
    case OPERATOR_LT:         fn_name = "js_compare"; compare_op = 0; break;
    case OPERATOR_LE:         fn_name = "js_compare"; compare_op = 2; break;
    case OPERATOR_GT:         fn_name = "js_compare"; compare_op = 1; break;
    case OPERATOR_GE:         fn_name = "js_compare"; compare_op = 3; break;
    case OPERATOR_AND:        fn_name = "js_logical_and"; break;
    case OPERATOR_OR:         fn_name = "js_logical_or"; break;
    case OPERATOR_JS_BIT_AND:    fn_name = "js_bitwise_and"; break;
    case OPERATOR_JS_BIT_OR:     fn_name = "js_bitwise_or"; break;
    case OPERATOR_JS_BIT_XOR:    fn_name = "js_bitwise_xor"; break;
    case OPERATOR_JS_LSHIFT: fn_name = "js_left_shift"; break;
    case OPERATOR_JS_RSHIFT: fn_name = "js_right_shift"; break;
    case OPERATOR_JS_URSHIFT: fn_name = "js_unsigned_right_shift"; break;
    case OPERATOR_JS_INSTANCEOF: fn_name = "js_instanceof"; break;
    case OPERATOR_IN:         fn_name = "js_in"; break;
    case OPERATOR_JS_NULLISH_COALESCE: fn_name = "js_nullish_coalesce"; break;
    default:
        log_error("js-mir: unknown binary op %d", bin->op);
        return publish(jm_emit_null(mt), VALUE_REP_ITEM);
    }

    // `#field in obj` resolves the current class's lexical private binding.
    MIR_reg_t left;
    if (bin->op == OPERATOR_IN && bin->left &&
        bin->left->node_type == AST_NODE_IDENT) {
    ...
    if (bin->op == OPERATOR_JS_INSTANCEOF && bin->right &&
        bin->right->node_type == AST_NODE_IDENT) {
        ...
        if (rid->name) {
            ...
            if (is_builtin_class) {
                ...
                return publish(jm_callr_2(mt, "js_instanceof_classname", MIR_T_I64,
                    left, classname), VALUE_REP_ITEM);
            }
        }
    }
...
static MIR_reg_t jm_transpile_unary_numeric_operand(JsMirTranspiler* mt,
                                                     ...
                                                     MIR_reg_t* with_key) {
    *with_key = 0;
    if (operand && operand->node_type == AST_NODE_IDENT && mt->with_depth > 0) {
        JsIdentifierNode* id = (JsIdentifierNode*)operand;
        JsMirVarEntry* var = jm_find_var_by_binding(mt, id->entry);
        if (var) {
...
static bool jm_emit_unary_identifier_writeback(JsMirTranspiler* mt, JsIdentifierNode* id,
    ...
    if (var) {
        ...
        if (mc && mc->const_type == MCONST_MODVAR) {
            ...
            jm_scope_env_mark_and_writeback_binding(mt, vname, (JsAstNode*)id,
                result);
        } else if (!mc) {
            MIR_reg_t name_reg = jm_box_property_name_literal(mt,
                id->name->chars, id->name->len);
...
static MirValue jm_transpile_update_unary(JsMirTranspiler* mt, JsUnaryNode* un,
    ...
    const char* numeric_update_function = decrement ? "js_decrement_numeric" :
        "js_increment_numeric";
    ...
    bool result_discarded = mt->discarded_expression == (JsAstNode*)un;
    TypeId result_type = jm_get_effective_type(mt, (JsAstNode*)un);
    auto publish = [mt, un, result_type](MIR_reg_t reg, ValueRep rep) {
        return jm_expression_value(mt, (JsAstNode*)un, reg, result_type, rep);
    };

    if (un->operand && un->operand->node_type == AST_NODE_CALL_EXPR) {
    ...
    if (un->operand && un->operand->node_type == AST_NODE_IDENT) {
    ...
    if (un->operand && un->operand->node_type == AST_NODE_MEMBER_EXPR) {
        JsMirReference ref = jm_emit_reference(mt, un->operand);
        jm_capture_write_reference(mt, &ref);
        jm_emit_canonicalize_computed_key_for_get_put(mt, &ref);
        MIR_reg_t operand = jm_emit_get_value(mt, &ref);
        MIR_reg_t old_value = 0;
        MIR_reg_t result = 0;
        if (result_discarded) {
            // A discarded update never observes postfix's prior numeric value.
            result = jm_callr_1(mt, update_function, MIR_T_I64, operand);
        } else {
            MIR_reg_t num_operand = jm_callr_1(mt, "js_to_numeric", MIR_T_I64,
                operand);
            old_value = num_operand;
            if (!un->prefix) {
                old_value = jm_new_reg(mt, reference_old_name, MIR_T_I64);
                jm_emit_mov(mt, old_value, num_operand);
            }
            result = jm_callr_1(mt, numeric_update_function, MIR_T_I64,
                num_operand);
        }
        jm_emit_put_value(mt, &ref, result);
        return publish(un->prefix || result_discarded ? result : old_value,
            VALUE_REP_ITEM);
    }
    ...
    MIR_reg_t num_operand = 0;
    MIR_reg_t result = 0;
    if (result_discarded) {
        // The full entry owns ToNumeric when no postfix completion is needed.
        result = jm_callr_1(mt, update_function, MIR_T_I64, operand);
    } else {
        num_operand = jm_callr_1(mt, "js_to_numeric", MIR_T_I64, operand);
        if (!un->prefix) {
            MIR_reg_t saved = jm_new_reg(mt, boxed_old_name, MIR_T_I64);
            jm_emit_mov(mt, saved, num_operand);
            num_operand = saved;
        }
        result = jm_callr_1(mt, numeric_update_function, MIR_T_I64, num_operand);
    }
    if (un->operand && un->operand->node_type == AST_NODE_IDENT) {
        JsIdentifierNode* id = (JsIdentifierNode*)un->operand;
    ...
    return publish(un->prefix || result_discarded ? result : num_operand,
        VALUE_REP_ITEM);
}

static MirValue jm_emit_unary_expression(JsMirTranspiler* mt,
        JsUnaryNode* un) {
    TypeId result_type = jm_get_effective_type(mt, (JsAstNode*)un);
    auto publish = [mt, un, result_type](MIR_reg_t reg, ValueRep rep) {
        return jm_expression_value(mt, (JsAstNode*)un, reg, result_type, rep);
    ...
    case OPERATOR_JS_BIT_NOT:
        return publish(jm_callr_1(mt, "js_bitwise_not", MIR_T_I64,
            jm_transpile_box_item(mt, un->operand)), VALUE_REP_ITEM);
    case OPERATOR_JS_TYPEOF: {
        ...
        bool direct_identifier = un->operand && un->operand->node_type == AST_NODE_IDENT;
        mt->in_typeof = direct_identifier;
        MIR_reg_t operand_val = jm_transpile_box_item(mt, un->operand);
        mt->in_typeof = false;
        return publish(jm_callr_1(mt, "js_typeof", MIR_T_I64, operand_val),
            VALUE_REP_ITEM);
    }
    case OPERATOR_POS:
    case OPERATOR_ADD:
    ...
    case OPERATOR_JS_VOID: {
        ...
        return publish(undef_reg, VALUE_REP_ITEM);
    }
    case OPERATOR_JS_DELETE: {
        ...
        if (un->operand && un->operand->node_type == AST_NODE_MEMBER_EXPR) {
            ...
            if (m->object && m->object->node_type == AST_NODE_IDENT) {
            ...
            MIR_T_I64, MIR_new_reg_op(mt->ctx, msg));
                    ...
                    MIR_reg_t r = jm_new_reg(mt, "dfalse", MIR_T_I64);
                    jm_emit_reg_op(mt, MIR_MOV, r, MIR_new_int_op(mt->ctx, (int64_t)ITEM_FALSE_VAL));
                    return publish(r, VALUE_REP_ITEM);
                }
            }
            JsMirReference ref = jm_emit_reference(mt, un->operand);
            return publish(jm_emit_delete_reference(mt, &ref), VALUE_REP_ITEM);
        }
        if (un->operand && un->operand->node_type == AST_NODE_IDENT) {
            ...
            if (del_name) {
                ...
                MIR_reg_t key = jm_box_property_name_literal(mt, del_name,
                    del_id->name->len);
                return publish(jm_call_2(mt, "js_delete_identifier_with_binding", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, key),
...
static bool jm_emit_destructure_pre_reference(JsMirTranspiler* mt, JsAstNode* target, JsMirReference* ref) {
    ...
    if (target->node_type == AST_NODE_MEMBER_EXPR) {
        *ref = jm_emit_reference(mt, target);
        return ref->kind != JS_MIR_REF_INVALID;
    }
    if (target->node_type == AST_NODE_ASSIGN_PATTERN) {
        JsAssignmentPatternNode* ap = (JsAssignmentPatternNode*)target;
        if (ap->left && ap->left->node_type == AST_NODE_MEMBER_EXPR) {
...
static JsIdentifierNode* jm_destructure_binding_identifier_target(JsAstNode* target) {
    ...
    if (!target || target->node_type != AST_NODE_IDENT) return NULL;
...
// Bind a value to one resolved binding. Handles closure and scope-env writes.
void jm_bind_destructure_var(JsMirTranspiler* mt, JsIdentifierNode* id,
        MIR_reg_t val) {
    if (!mt || !id || !id->name) return;
    const char* vname = jm_var_name(id->name);
    JsMirVarEntry* var = jm_find_var_by_binding(mt, id->entry);
    ...
    if (mt->module_consts) {
        module_var = jm_find_module_const_by_binding(mt, id->entry);
    ...
    bool current_param_binding = id->entry && id->entry->is_parameter;
    ...
    bool writes_module_binding = module_var != NULL;
    if (writes_module_binding) {
        // A var-pattern loop can retain a local MIR home for a binding that
        // also has a module slot. Update both homes before returning so the
        // immediately following loop body observes the destructured value.
        if (var) {
            ...
            if (!mt->destructure_assignment_mode) var->tdz_active = false;
        }
    ...
    jm_set_var(mt, vname, reg, MIR_T_I64, LMD_TYPE_ANY, id->entry);
    if (existing_from_env && jm_has_current_source_function(mt) && module_var) {
        if (module_var->const_type == MCONST_MODVAR) {
            if (module_var->var_kind == 2) {
                const char* js_name = (strncmp(vname, "_js_", 4) == 0) ? vname + 4 : vname;
                jm_emit_const_assign_error(mt, js_name, (int)strlen(js_name));
                return;
            }
            jm_store_module_var(mt, (uint32_t)module_var->int_val, reg);
        }
    }
    ...
    if (!is_local_let_const && module_var) {
        JsModuleConstEntry* mc = module_var;
...
void jm_emit_destructure_target(JsMirTranspiler* mt, JsAstNode* target, MIR_reg_t val) {
    ...
    if (target->node_type == AST_NODE_IDENT) {
        ...
        jm_bind_destructure_var(mt, id, val);
    } else if (target->node_type == AST_NODE_ARRAY_PATTERN ||
               target->node_type == AST_NODE_ARRAY) {
        jm_emit_array_destructure(mt, target, val);
    } else if (target->node_type == AST_NODE_MAP_PATTERN ||
               target->node_type == AST_NODE_MAP) {
        jm_emit_object_destructure(mt, target, val);
    } else if (target->node_type == AST_NODE_ASSIGN_PATTERN) {
        JsAssignmentPatternNode* ap = (JsAssignmentPatternNode*)target;
        MIR_reg_t resolved = jm_emit_destructure_default(mt, val, ap->right);
        // function name inference for destructuring defaults
        if (ap->left && ap->left->node_type == AST_NODE_IDENT && ap->right) {
            JsIdentifierNode* id = (JsIdentifierNode*)ap->left;
            bool is_anon_func = false;
            bool is_anon_class = false;
            if (ap->right->node_type == AST_NODE_ARROW_FUNC) {
                is_anon_func = true;
            } else if (ap->right->node_type == AST_NODE_FUNC_EXPR) {
                JsFunctionNode* fn = (JsFunctionNode*)ap->right;
                is_anon_func = (fn->name == NULL);
            } else if (ap->right->node_type == AST_NODE_CLASS_EXPR ||
                       ap->right->node_type == AST_NODE_CLASS) {
...
void jm_emit_array_destructure(JsMirTranspiler* mt, JsAstNode* pattern_node, MIR_reg_t src) {
    ...
    // Validate: rest element must be last, and must not have an initializer
    {
        ...
        while (chk) {
            if (chk->node_type == AST_NODE_REST_ELEMENT ||
                chk->node_type == AST_NODE_SPREAD) {
                JsSpreadElementNode* sp = (JsSpreadElementNode*)chk;
                if (sp->argument && sp->argument->node_type == AST_NODE_ASSIGN_PATTERN) {
    ...
    while (elem) {
        if (elem->node_type == AST_NODE_SPREAD ||
            elem->node_type == AST_NODE_REST_ELEMENT) {
    ...
    // carried by last_call_result; an untyped async join would clear that
...
void jm_emit_object_destructure(JsMirTranspiler* mt, JsAstNode* pattern_node, MIR_reg_t src) {
    ...
    if (mt->destructure_assignment_mode) {
        ...
        while (p) {
            ...
            } else if (p->node_type == AST_NODE_REST_ELEMENT ||
                       p->node_type == AST_NODE_REST_PROPERTY ||
                       p->node_type == AST_NODE_SPREAD) {
                JsSpreadElementNode* sp = (JsSpreadElementNode*)p;
                target = sp->argument;
            }
            // Unwrap assignment pattern: const { x = default } = obj
            if (target && target->node_type == AST_NODE_ASSIGN_PATTERN) {
                JsAssignmentPatternNode* ap = (JsAssignmentPatternNode*)target;
                target = ap->left;
            }
            if (target && target->node_type == AST_NODE_IDENT) {
                JsIdentifierNode* id = (JsIdentifierNode*)target;
                jm_bind_destructure_var(mt, id, pre_undef);
            }
            p = p->next;
        }
    }
    ...
    while (prop) {
        if (prop->node_type == AST_NODE_PROPERTY) {
            ...
            if (sp->argument) {
                ...
                while (pp && pp != prop) {
                    if (pp->node_type == AST_NODE_PROPERTY) exclude_count++;
                    pp = pp->next;
                }
                ...
                while (pp && pp != prop) {
                    if (pp->node_type == AST_NODE_PROPERTY) {
                        JsPropertyNode* ep = (JsPropertyNode*)pp;
                        MIR_reg_t ki_item = 0;
                        if (!ep->computed && ep->key && ep->key->node_type == AST_NODE_IDENT) {
...
static MirValue jm_emit_assignment_value(JsMirTranspiler* mt,
        JsAssignmentNode* asgn) {
    TypeId result_type = jm_get_effective_type(mt, (JsAstNode*)asgn);
    auto publish = [mt, asgn](MIR_reg_t reg, ValueRep rep,
            TypeId type_id) -> MirValue {
        return jm_expression_value(mt, (JsAstNode*)asgn, reg, type_id, rep);
    };
    auto publish_item = [&publish, result_type](MIR_reg_t reg) -> MirValue {
        return publish(reg, VALUE_REP_ITEM, result_type);
    };
    if (!asgn->left || !asgn->right) return publish_item(jm_emit_null(mt));

    if (asgn->left->node_type == AST_NODE_CALL_EXPR) {
    ...
    if (asgn->left->node_type == AST_NODE_IDENT) {
        JsIdentifierNode* id = (JsIdentifierNode*)asgn->left;
        const char* vname = jm_var_name(id->name);
        JsMirVarEntry* var = jm_find_var_by_binding(mt, id->entry);
        JsClassEntry* inner_ce = jm_current_inner_class_binding(mt, id->entry,
            (JsAstNode*)id);
        ...
        if (!var) {
            ...
            if (mt->module_consts) {
                ...
                if (mc && (mc->const_type == MCONST_MODVAR || mc->const_type == MCONST_CLASS)) {
                    ...
                    if (mt->with_depth <= 0 && mc->modvar_type == LMD_TYPE_INT && asgn->op != OPERATOR_ASSIGN) {
                        MIR_insn_code_t p5_mir_op = MIR_ADD;
                        bool p5_handled = true;
                        switch (asgn->op) {
                        case OPERATOR_JS_ADD_ASSIGN:    p5_mir_op = MIR_ADD;  break;
                        case OPERATOR_JS_SUB_ASSIGN:    p5_mir_op = MIR_SUB;  break;
                        case OPERATOR_JS_MUL_ASSIGN:    p5_mir_op = MIR_MUL;  break;
                        case OPERATOR_JS_BIT_AND_ASSIGN: p5_mir_op = MIR_AND; break;
                        case OPERATOR_JS_BIT_OR_ASSIGN:  p5_mir_op = MIR_OR;  break;
                        case OPERATOR_JS_BIT_XOR_ASSIGN: p5_mir_op = MIR_XOR; break;
                        case OPERATOR_JS_LSHIFT_ASSIGN:  p5_mir_op = MIR_LSH; break;
                        case OPERATOR_JS_RSHIFT_ASSIGN:  p5_mir_op = MIR_RSH; break;
                        case OPERATOR_JS_URSHIFT_ASSIGN: p5_mir_op = MIR_URSH; break;
                        ...
                        if (p5_handled) {
                            ...
                            jm_scope_env_mark_and_writeback_binding(mt, vname,
                                (JsAstNode*)id, boxed_new);
                            return publish_item(boxed_new);
                        }
                    }
                    if (asgn->op == OPERATOR_JS_AND_ASSIGN || asgn->op == OPERATOR_JS_OR_ASSIGN ||
                        asgn->op == OPERATOR_JS_NULLISH_ASSIGN) {
                        // Logical assignment with short-circuit for module vars
                        MIR_reg_t old_val = jm_load_module_var(mt, (uint32_t)mc->int_val);
                        return publish_item(jm_emit_logical_assignment(mt, asgn, id,
                            old_val, true, mc, vname, false, 0));
                    }
                    MIR_reg_t rhs;
                    if (asgn->op != OPERATOR_ASSIGN) {
                        ...
                        if (mt->with_depth > 0) {
                            return publish_item(jm_emit_with_writeback(mt, with_key,
                        ...
                        if (!asgn->lhs_is_parenthesized && asgn->right &&
                            (asgn->right->node_type == AST_NODE_FUNC_EXPR ||
                             asgn->right->node_type == AST_NODE_ARROW_FUNC)) {
                        ...
                        if (mt->with_depth > 0) {
                            return publish_item(jm_emit_with_writeback(mt, simple_with_key,
                                rhs, "mwa_res", strict_put, NULL, mc, id->name, vname));
                        }
                    }
            ...
            // Implicit global assignment: write to global object via js_set_global_property
            {
                ...
                if (asgn->op == OPERATOR_JS_AND_ASSIGN || asgn->op == OPERATOR_JS_OR_ASSIGN ||
                    ...
                    jm_emit_error_lane_propagate_check(mt);
                    return publish_item(jm_emit_logical_assignment(mt, asgn, id,
                        old_val, false, NULL, NULL, strict_put, set_global_strict_flag));
                }
                MIR_reg_t simple_with_key = 0;
                if (mt->with_depth > 0 && asgn->op == OPERATOR_ASSIGN) {
                simple_with_key = jm_box_property_name_literal(mt,
                    id->name->chars, id->name->len);
                    jm_callr_1(mt, "js_capture_with_binding", MIR_T_I64, simple_with_key);
                    jm_emit_error_lane_propagate_check(mt);
                }
                if (strict_put && asgn->op == OPERATOR_ASSIGN) {
        ...
        if (var->is_nfe_binding && asgn->op == OPERATOR_ASSIGN) {
            MIR_reg_t rhs = jm_transpile_box_item(mt, asgn->right);
            jm_emit_set_class_assignment_name(mt, asgn, rhs, id->name);
        ...
        if (rhs_type != LMD_TYPE_INT) {
            ...
            if (asgn->op == OPERATOR_ASSIGN) {
                ...
                switch (asgn->op) {
                case OPERATOR_JS_ADD_ASSIGN: op = MIR_ADD; break;
                case OPERATOR_JS_SUB_ASSIGN: op = MIR_SUB; break;
                case OPERATOR_JS_MUL_ASSIGN: op = MIR_MUL; break;
                case OPERATOR_JS_DIV_ASSIGN: op = MIR_DIV; break;
                case OPERATOR_JS_MOD_ASSIGN: op = MIR_MOD; break;
                case OPERATOR_JS_BIT_AND_ASSIGN: op = MIR_AND; break;
                case OPERATOR_JS_BIT_OR_ASSIGN: op = MIR_OR; break;
                case OPERATOR_JS_BIT_XOR_ASSIGN: op = MIR_XOR; break;
                case OPERATOR_JS_LSHIFT_ASSIGN: op = MIR_LSH; break;
                case OPERATOR_JS_RSHIFT_ASSIGN: op = MIR_RSH; break;
                case OPERATOR_JS_URSHIFT_ASSIGN: op = MIR_URSH; break;
                default: break;
                }
    ...
    if (mt->with_depth <= 0 && var->type_id == LMD_TYPE_FLOAT && !var->from_env) {
        if (asgn->op == OPERATOR_ASSIGN) {
            ...
            Operator numeric_op;
            MirNumericOpPlan numeric_plan;
            bool binding_is_proven_number = jm_numeric_binding_type(mt,
                id->entry) == LMD_TYPE_FLOAT;
            if (binding_is_proven_number &&
                    jm_compound_assign_number_op(asgn->op, &numeric_op) &&
                    em_numeric_op_plan(numeric_op, &numeric_plan)) {
                // The source-body fact has joined every write to this
                // binding as Number. Reuse the common scalar plan instead
                // of boxing the accumulator just to call a JS helper.
                MIR_reg_t rval = jm_transpile_as_native(mt, asgn->right,
                    LMD_TYPE_FLOAT);
                if (numeric_plan.helper_name) {
                    MIR_reg_t result = jm_call_2(mt, numeric_plan.helper_name,
                        MIR_T_D, MIR_T_D, MIR_new_reg_op(mt->ctx, var->reg),
                        MIR_T_D, MIR_new_reg_op(mt->ctx, rval));
                    jm_emit_dmov(mt, var->reg, result);
                } else {
                    jm_emit_reg_binary(mt, numeric_plan.f64_opcode, var->reg,
                        var->reg, rval);
                }
                jm_emit_compile_profile(mt, JS_OPT_MIR_NUMBER_ADMITTED,
                    JS_OPT_OUTCOME_TAKEN);
    ...
    if (asgn->op == OPERATOR_ASSIGN) {
        ...
        if (asgn->right && (asgn->right->node_type == AST_NODE_FUNC_EXPR ||
                            asgn->right->node_type == AST_NODE_ARROW_FUNC)) {
            JsFunctionNode* fn_node = (JsFunctionNode*)asgn->right;
            if (!fn_node->name && id->name) {
                jm_emit_set_function_name(mt, rhs, id->name->chars);
        ...
        if (mt->with_depth > 0) {
            return publish_item(jm_emit_with_writeback(mt, simple_with_key,
                rhs, "lsa_res", strict_put, var, NULL, NULL, vname));
        }
    } else if (asgn->op == OPERATOR_JS_AND_ASSIGN || asgn->op == OPERATOR_JS_OR_ASSIGN ||
               asgn->op == OPERATOR_JS_NULLISH_ASSIGN) {
        ...
        if (asgn->op == OPERATOR_JS_NULLISH_ASSIGN) {
            ...
            if (asgn->op == OPERATOR_JS_AND_ASSIGN) {
    ...
    if (mt->with_depth > 0 && asgn->op != OPERATOR_ASSIGN && asgn->op != OPERATOR_JS_AND_ASSIGN &&
        asgn->op != OPERATOR_JS_OR_ASSIGN && asgn->op != OPERATOR_JS_NULLISH_ASSIGN) {
        MIR_reg_t with_key = jm_box_property_name_literal(mt,
            id->name->chars, id->name->len);
        bool strict_put = jm_strict_put(mt);
        return publish_item(jm_emit_with_writeback(mt, with_key, rhs,
            "lwa_res", strict_put, var, NULL, NULL, vname));
    }

    jm_emit_mov(mt, var->reg, rhs);
    var->jube_type = asgn->op == OPERATOR_ASSIGN
...
if (asgn->left->node_type == AST_NODE_MEMBER_EXPR) {
    ...
    if (asgn->op == OPERATOR_ASSIGN && member_is_super) {
            JsMirReference ref = jm_emit_reference(mt, asgn->left);
            jm_capture_write_reference(mt, &ref, asgn->right);
            jm_emit_error_lane_propagate_check(mt);
            if (ref.uninitialized_this) {
    ...
    JsMirReference ref = jm_emit_reference(mt, asgn->left);
    jm_capture_write_reference(mt, &ref,
        asgn->op == OPERATOR_ASSIGN ? asgn->right : NULL);
    jm_emit_error_lane_propagate_check(mt);
    MIR_reg_t packed_number_update = 0;
    if (jm_try_emit_packed_number_self_update(mt, asgn, &ref,
            &packed_number_update)) {
        jm_sync_arguments_param_after_write(mt, member);
        jm_readback_closure_env(mt);
        jm_scope_env_reload_vars(mt);
        return publish_item(packed_number_update);
    }
    MIR_reg_t result = 0;
        TypeId rhs_type = jm_get_effective_type(mt, asgn->right);
        ...
        bool fixed_number_store = native_number_store &&
            jm_fixed_typed_array_receiver_kind(mt, ref.member) >= 0;
        bool packed_number_store = native_number_store && !fixed_number_store &&
            jm_is_array_literal_candidate(mt, ref.member);
        if (fixed_number_store || packed_number_store) {
            // Preserve the RHS's Number representation for the assignment
            // result and miss continuation, but pass a raw F64 to the
            // noncoercing packed-store leaf on the guarded hit.
            MIR_reg_t rhs_native = jm_transpile_as_native(mt, asgn->right,
                rhs_type);
            MIR_reg_t rhs_number = rhs_native;
            if (rhs_type == LMD_TYPE_INT) {
                rhs_number = jm_new_reg(mt, "native_store_number", MIR_T_D);
                jm_emit(mt, MIR_new_insn(mt->ctx, MIR_I2D,
                    MIR_new_reg_op(mt->ctx, rhs_number),
                    MIR_new_reg_op(mt->ctx, rhs_native)));
            }
            MIR_reg_t new_val = jm_box_native(mt, rhs_native, rhs_type);
            result = fixed_number_store
                ? jm_emit_fixed_typed_array_number_store(mt, &ref, new_val,
                    rhs_number)
            ...
            if (asgn->op == OPERATOR_ASSIGN) {
                ...
                MIR_reg_t logical_result = jm_new_reg(mt, "la_res", MIR_T_I64);
                jm_emit_canonicalize_computed_key_for_get_put(mt, &ref);
                ...
                jm_emit_mov(mt, logical_result, cur_val);
                jm_emit_jmp(mt, l_end);
                ...
                jm_emit_label(mt, l_end);
                return publish_item(logical_result);
            } else {
        ...
        jm_readback_closure_env(mt);
        jm_scope_env_reload_vars(mt);
        return publish_item(result);
    }

    if (asgn->op == OPERATOR_ASSIGN &&
        (asgn->left->node_type == AST_NODE_ARRAY_PATTERN ||
         asgn->left->node_type == AST_NODE_ARRAY ||
         asgn->left->node_type == AST_NODE_MAP_PATTERN ||
         asgn->left->node_type == AST_NODE_MAP)) {
        bool is_array = asgn->left->node_type == AST_NODE_ARRAY_PATTERN ||
            asgn->left->node_type == AST_NODE_ARRAY;
        return publish_item(jm_emit_destructure_assignment(mt, asgn->left,
            asgn->right, is_array));
    }

    log_error("js-mir: unsupported assignment target %d", asgn->left->node_type);
    return publish_item(jm_emit_null(mt));
}
...
static void jm_eval_push_one_binding(JsMirTranspiler* mt,
        struct hashmap* bridged, bool global_lexical, bool* pushed,
        const char* name, MIR_reg_t value, bool immutable, NameEntry* binding) {
    if (!mt || !bridged || !pushed || !name || !value) return;
    JsNameSetEntry seen;
    memset(&seen, 0, sizeof(seen));
    seen.name = jm_persist_name(name);
    if (hashmap_get(bridged, &seen)) return;
    ...
    const char* js_name = name + 4;
    MIR_reg_t key_reg = jm_box_property_name_literal(mt, js_name, strlen(js_name));
    if (global_lexical) {
        MIR_reg_t immutable_reg = jm_new_reg(mt, "eval_immutable", MIR_T_I64);
        jm_emit_reg_op(mt, MIR_MOV, immutable_reg,
            MIR_new_int_op(mt->ctx, immutable ? 1 : 0));
        jm_callr_void_3(mt, "js_eval_global_lexical_bind", key_reg, value,
            immutable_reg);
    } else {
        jm_callr_void_2(mt, "js_eval_env_bind", key_reg, value);
    }
}
...
static struct hashmap* jm_eval_push_bindings(JsMirTranspiler* mt,
    ...
    for (int depth = mt->scope_depth; depth >= 0; depth--) {
        ...
        while (hashmap_iter(scope, &iter, &item)) {
            ...
            jm_eval_push_one_binding(mt, bridged, global_lexical, &pushed,
                entry->name, value_reg, entry->var.is_const,
                entry->var.binding);
        }
    }
    // Top-level lexical declarations are module-slot backed, so they have no
    // ordinary MIR local to appear in the loop above. Direct eval must bridge
    // the same resolved binding and restore its module slot afterward.
    if (global_lexical && mt->in_main && !mt->is_module && mt->module_consts) {
        size_t iter = 0;
        void* item = NULL;
        while (hashmap_iter(mt->module_consts, &iter, &item)) {
            JsModuleConstEntry* module_var = (JsModuleConstEntry*)item;
            if (module_var->const_type != MCONST_MODVAR ||
                    (module_var->var_kind != JS_VAR_LET &&
                     module_var->var_kind != JS_VAR_CONST) ||
                    module_var->is_iife_var) continue;
            MIR_reg_t value_reg = jm_load_module_var(mt,
                (uint32_t)module_var->int_val);
            jm_eval_push_one_binding(mt, bridged, true, &pushed,
                module_var->name, value_reg,
                module_var->var_kind == JS_VAR_CONST, module_var->binding);
        }
    }
...
static void jm_eval_global_lexical_writeback_bindings(JsMirTranspiler* mt,
        struct hashmap* bridged) {
    if (!mt || !bridged || !mt->module_consts) return;
    size_t iter = 0;
    void* item = NULL;
    while (hashmap_iter(bridged, &iter, &item)) {
        JsNameSetEntry* seen = (JsNameSetEntry*)item;
        JsModuleConstEntry* module_var = seen->entry
            ? jm_find_module_const_by_binding(mt, seen->entry) : NULL;
        if (!module_var || module_var->const_type != MCONST_MODVAR ||
                (module_var->var_kind != JS_VAR_LET &&
                 module_var->var_kind != JS_VAR_CONST)) continue;
        const char* js_name = module_var->name + 4;
        MIR_reg_t key_reg = jm_box_property_name_literal(mt, js_name,
            strlen(js_name));
        MIR_reg_t value_reg = jm_callr_1(mt, "js_get_global_property",
            MIR_T_I64, key_reg);
        jm_store_module_var(mt, (uint32_t)module_var->int_val, value_reg);
    }
}
...
static void jm_eval_env_writeback_bindings(JsMirTranspiler* mt, struct hashmap* bridged) {
    ...
    while (hashmap_iter(bridged, &iter, &item)) {
        ...
        var->mir_type = MIR_T_I64;
        if (var->binding) {
            jm_scope_env_mark_and_writeback_entry(mt, seen->name,
                var->binding, value_reg);
        } else {
            jm_scope_env_mark_and_writeback_generated(mt, seen->name, value_reg);
        }
...
static MIR_reg_t jm_emit_intrinsic_direct_eval(JsMirTranspiler* mt,
    ...
    if (eval_bridged) {
        if (mt->eval_local_frame_reg != 0) {
            ...
            jm_eval_global_lexical_writeback_bindings(mt, eval_bridged);
...
static MirValue jm_emit_call_item_value(JsMirTranspiler* mt,
        JsCallNode* call, MIR_reg_t result) {
    return jm_expression_value(mt, (JsAstNode*)call, result,
        jm_get_effective_type(mt, (JsAstNode*)call), VALUE_REP_ITEM);
}

static MirValue jm_emit_call_native_value(JsMirTranspiler* mt,
        JsCallNode* call, JsFuncCollected* fc, MIR_reg_t result) {
    if (JM_JS_FACT(fc, native_return_kind) == NATIVE_RETURN_ITEM) {
        return jm_expression_value(mt, (JsAstNode*)call, result, LMD_TYPE_ANY,
            VALUE_REP_ITEM);
    }
    TypeId result_type = JM_JS_FACT(fc, return_type);
    return jm_expression_value(mt, (JsAstNode*)call, result, result_type,
        result_type == LMD_TYPE_FLOAT ? VALUE_REP_F64 : VALUE_REP_I64);
}

static MirValue jm_emit_call_expression(JsMirTranspiler* mt,
        JsCallNode* call) {
        ...
        if (type_id->name && type_id->name->len == 4 &&
            ...
            if (!type_is_shadowed) {
                ...
                return jm_emit_call_item_value(mt, call,
                    jm_callr_1(mt, "ts_type_info", MIR_T_I64, value));
            }
        }
    }
    ...
    if (call->callee && call->callee->node_type == AST_NODE_IDENT) {
        ...
        if (callee_id->name && callee_id->name->len == 7 &&
            ...
            bool require_is_local = callee_id->entry != NULL;
            if (!require_is_local) {
                JsAstNode* arg = call->arguments;
    ...
    if (call->callee && call->callee->node_type == AST_NODE_IDENT) {
        ...
        if (callee_id->name && callee_id->name->len == 6 &&
            ...
            bool import_is_local = callee_id->entry != NULL;
            if (!import_is_local) {
                JsAstNode* arg = call->arguments;
                if (arg && arg->node_type == AST_NODE_LITERAL) {
                    JsLiteralNode* lit = (JsLiteralNode*)arg;
    ...
    if (call->callee && call->callee->node_type == AST_NODE_IDENT) {
        ...
        if (id->name && id->name->len == 5 && strncmp(id->name->chars, "super", 5) == 0) {
            ...
            if (mt->current_class && mt->current_class->node && mt->current_class->node->superclass &&
                (mt->current_class->node->superclass->node_type == AST_NODE_NULL ||
                 (mt->current_class->node->superclass->node_type == AST_NODE_LITERAL &&
                  ((JsLiteralNode*)mt->current_class->node->superclass)->literal_type == AST_LITERAL_NULL))) {
                ...
                return jm_emit_call_item_value(mt, call,
                    jm_emit_super_bind_this_with_public_fields(mt, this_val, super_result));
            }
            ...
            if (static_superclass) {
                ...
                MIR_reg_t super_result = super_has_spread
                    ...
                    : jm_super_call_class_into(mt,
                        ...
                        MIR_new_int_op(mt->ctx, arg_count));
                return jm_emit_call_item_value(mt, call,
                    jm_emit_super_bind_this_with_public_fields(
                ...
                if (mt->current_class && mt->current_class->node &&
                    ...
                    mt->current_class->node->superclass->node_type == AST_NODE_IDENT) {
                    ...
                    if (super_id->name) {
                        ...
                        if (is_error_class) {
                            ...
                            return jm_emit_call_item_value(mt, call,
                                jm_emit_super_bind_this_with_public_fields(mt, this_val, this_val));
                        }
                        ...
                        // this merge the derived `this` would lack the base fields like `type`.
                        {
                            ...
                            if (super_has_spread) {
                                ...
                                return jm_emit_call_item_value(mt, call, bound_this);
                            } else {
                ...
                return jm_emit_call_item_value(mt, call, jm_emit_null(mt));
            }
        }
    }
    ...
    if (call->callee && call->callee->node_type == AST_NODE_MEMBER_EXPR) {
        JsMemberNode* m = (JsMemberNode*)call->callee;
        if (!m->computed && m->object && m->object->node_type == AST_NODE_IDENT &&
            m->property && m->property->node_type == AST_NODE_IDENT) {
            ...
            if (obj->name && obj->name->len == 5 && strncmp(obj->name->chars, "super", 5) == 0) {
                ...
                if (mt->current_class && mt->current_class->superclass) {
                    ...
                    if (found_method && found_method->fc && found_method->fc->func_item) {
                        ...
                        return jm_emit_call_item_value(mt, call,
                            jm_emit_super_call(mt, call, this_val, fn_item, arg_count));
                    } else {
        ...
        if (m->computed && m->object && m->object->node_type == AST_NODE_IDENT) {
            ...
            if (obj_id->name && obj_id->name->len == 5 && strncmp(obj_id->name->chars, "super", 5) == 0) {
                ...
                if (mt->current_class && mt->current_class->superclass && m->property) {
                    ...
                    } else if (m->property->node_type == AST_NODE_MEMBER_EXPR) {
                        // e.g. Symbol.iterator — use the property name
                        JsMemberNode* kmem = (JsMemberNode*)m->property;
                        if (kmem->property && kmem->property->node_type == AST_NODE_IDENT) {
                    ...
                    if (key_id_name) {
                        ...
                        while (parent && !found_method) {
                            for (int i = 0; i < parent->member_count; i++) {
                                ...
                                if (me->key_expr->node_type == AST_NODE_IDENT) {
                                    me_key_name = ((JsIdentifierNode*)me->key_expr)->name;
                                } else if (me->key_expr->node_type == AST_NODE_MEMBER_EXPR) {
    ...
    if (jm_test262_fast_paths_enabled(mt)) {
        ...
        } else if (call->callee && call->callee->node_type == AST_NODE_IDENT) {
            JsIdentifierNode* id = (JsIdentifierNode*)call->callee;
        ...
        if (spec) return jm_emit_call_item_value(mt, call,
            jm_emit_test262_intercept(mt, call, spec));
    }
#endif
        ...
        if (m->property && m->property->node_type == AST_NODE_IDENT) {
            ...
            if (can_reuse_reference && !named_ref.is_private) {
                if (named_ref.jube_slot >= 0 &&
                    ...
                    jm_readback_closure_env(mt);
                    return jm_emit_call_item_value(mt, call,
                        jm_publish_call_result(mt, direct_result));
                }
            }
    ...
    if (call->callee && call->callee->node_type == AST_NODE_IDENT) {
        ...
        if (!call->optional && id->name && id->name->len == 4 &&
            ...
            return jm_emit_call_item_value(mt, call,
                jm_emit_eval_identifier_call(mt, call));
        }
        ...
        if (resolved_fn) {
            if (mt->module_consts) {
                JsModuleConstEntry* direct_mc = jm_find_module_const_by_binding(mt,
                    id->entry);
...
MIR_reg_t jm_transpile_member_as_number(JsMirTranspiler* mt, JsMemberNode* member) {
    // A numeric consumer can retain an admitted typed or ordinary packed
    // element in F64. Each miss continues from the already-evaluated Reference
    // through the existing Get/ToNumber contract (S1.11, D8.2.3).
    if (mt && member && member->computed) {
        JsMirReference ref = jm_emit_reference(mt, (JsAstNode*)member);
        if (ref.native_key_reg != 0 &&
                (ref.native_key_type == LMD_TYPE_INT ||
                 ref.native_key_type == LMD_TYPE_FLOAT)) {
            MIR_reg_t numeric_key = ref.native_key_reg;
            if (ref.native_key_type == LMD_TYPE_INT) {
                numeric_key = jm_new_reg(mt, "native_read_index_double", MIR_T_D);
                jm_emit(mt, MIR_new_insn(mt->ctx, MIR_I2D,
                    MIR_new_reg_op(mt->ctx, numeric_key),
                    MIR_new_reg_op(mt->ctx, ref.native_key_reg)));
            }
            if (jm_fixed_typed_array_receiver_kind(mt, member) >= 0) {
                return jm_emit_fixed_typed_array_read_number(mt, member,
                    ref.base_reg, numeric_key);
            }
            MIR_reg_t dense = jm_emit_packed_array_read_number(mt, member,
                ref.base_reg, numeric_key);
            if (dense) return dense;
        }
        MIR_reg_t fallback = jm_emit_get_value(mt, &ref);
        jm_emit_error_lane_propagate_check(mt);
        return jm_emit_unbox_float(mt, fallback);
    }
...
static Item* jm_static_inline_number_array_items(JsMirTranspiler* mt,
    ...
    for (JsAstNode* element = array->elements; element; element = element->next) {
        if (element->node_type != AST_NODE_LITERAL || count >= array->length) {
            return NULL;
        }
        JsLiteralNode* literal = (JsLiteralNode*)element;
    ...
    // This pointer is embedded in MIR, so its values use the realm-owned pool
    // already retained for generated code, as static literal shapes do (D8.5.1v3).
    Item* items = (Item*)pool_calloc(js_input->pool,
        (size_t)count * sizeof(Item));
    if (!items) return NULL;
    int index = 0;
    for (JsAstNode* element = array->elements; element; element = element->next) {
        double value = ((JsLiteralNode*)element)->value.number_value;
        uint64_t bits;
        __builtin_memcpy(&bits, &value, sizeof(bits));
        items[index++].item = value == 0.0
            ? ITEM_FLOAT_P0 | (bits >> 63) : bits;
    }
    return items;
}

static bool jm_static_primitive_object_value(JsLiteralNode* literal,
        JsStaticObjectProperty* property) {
    if (!literal || !property) return false;
    property->string_chars = NULL;
    property->string_len = 0;
    property->immediate = ITEM_NULL;
    ...
    case AST_LITERAL_NUMBER: {
        ...
        uint64_t bits;
        __builtin_memcpy(&bits, &value, sizeof(bits));
        property->immediate = value == 0.0
            ? ITEM_FLOAT_P0 | (bits >> 63) : bits;
        property->value_kind = JS_STATIC_OBJECT_VALUE_IMMEDIATE;
        return true;
    }
    case AST_LITERAL_STRING:
        if (!literal->value.string_value) return false;
        property->string_chars = literal->value.string_value->chars;
        property->string_len = (int)literal->value.string_value->len;
        property->value_kind = JS_STATIC_OBJECT_VALUE_STRING;
        return true;
    case AST_LITERAL_BOOLEAN:
        property->immediate = literal->value.boolean_value ? ITEM_TRUE : ITEM_FALSE;
        property->value_kind = JS_STATIC_OBJECT_VALUE_IMMEDIATE;
        return true;
    case AST_LITERAL_NULL:
        property->immediate = ITEM_NULL;
        ...
        return true;
    case AST_LITERAL_UNDEFINED:
        property->immediate = ITEM_JS_UNDEFINED;
        property->value_kind = JS_STATIC_OBJECT_VALUE_IMMEDIATE;
        return true;
    default:
        return false;
...
static char* jm_static_literal_copy(JsMirTranspiler* mt, String* source) {
    if (!mt || !source || !js_input || !js_input->pool) return NULL;
    char* copy = (char*)pool_calloc(js_input->pool, (size_t)source->len + 1);
    if (!copy) return NULL;
    memcpy(copy, source->chars, source->len);
    return copy;
}
...
static JsStaticObjectProperty* jm_static_primitive_object_properties(
    ...
    int count = 0;
    for (JsAstNode* node = object->properties; node; node = node->next) {
        if (node->node_type != AST_NODE_PROPERTY) return NULL;
        JsPropertyNode* property = (JsPropertyNode*)node;
        String* name = jm_literal_property_name(property);
        JsStaticObjectProperty candidate = {};
        if (!name || (name->len == 9 && memcmp(name->chars, "__proto__", 9) == 0) ||
                !property->value || property->value->node_type != AST_NODE_LITERAL ||
                !jm_static_primitive_object_value((JsLiteralNode*)property->value,
                    &candidate)) {
            return NULL;
        }
        count++;
    ...
    // MIR retains these addresses after the parser's Script and its AST die.
    // Copy all spelling bytes into the generated-code realm (D8.5.1v3).
    JsStaticObjectProperty* properties = (JsStaticObjectProperty*)pool_calloc(
        js_input->pool, (size_t)count * sizeof(JsStaticObjectProperty));
    if (!properties) return NULL;
    int index = 0;
    for (JsAstNode* node = object->properties; node; node = node->next) {
        JsPropertyNode* property = (JsPropertyNode*)node;
        JsStaticObjectProperty* target = &properties[index++];
        String* name = jm_literal_property_name(property);
        JsLiteralNode* literal = (JsLiteralNode*)property->value;
        target->key_chars = jm_static_literal_copy(mt, name);
        target->key_len = (int)name->len;
        if (!target->key_chars || !jm_static_primitive_object_value(literal, target)) {
            return NULL;
        }
        if (target->value_kind == JS_STATIC_OBJECT_VALUE_STRING) {
            target->string_chars = jm_static_literal_copy(mt,
                literal->value.string_value);
            if (!target->string_chars) return NULL;
        }
    }
...
// Array expression
static MirValue jm_emit_array_value(JsMirTranspiler* mt,
        JsArrayNode* arr) {
    // Check if any element is a spread element
    bool has_spread = false;
    JsAstNode* check = arr->elements;
    while (check) {
        if (check->node_type == AST_NODE_SPREAD) { has_spread = true; break; }
        check = check->next;
    }
    MIR_reg_t array;
    ...
    if (static_items) {
        ...
        while (elem) {
            if (elem->node_type == AST_NODE_SPREAD) {
                ...
                MIR_reg_t val;
                if (elem->node_type == AST_NODE_NULL) {
    ...
    return jm_expression_value(mt, (JsAstNode*)arr, array, LMD_TYPE_ARRAY,
        VALUE_REP_ITEM);
}
...
static MirValue jm_emit_object_value(JsMirTranspiler* mt,
    ...
    int static_property_count = 0;
    JsStaticObjectProperty* static_properties = literal_shape ? NULL :
        jm_static_primitive_object_properties(mt, obj, &static_property_count);
    if (static_properties) {
        MIR_reg_t object = jm_call_2(mt, "js_object_new_from_static_properties",
            MIR_T_I64,
            MIR_T_I64, MIR_new_int_op(mt->ctx,
    ...
    // An exact field plan must construct the matching layout; a generic
    // composite recipe would force its guarded accesses onto fallback slots.
    JsStaticLiteralRecipe* static_recipe = literal_shape ? NULL :
        jm_static_composite_literal_recipe(mt, (JsAstNode*)obj);
    if (static_recipe) {
    ...
    while (prop) {
        if (prop->node_type == AST_NODE_PROPERTY) {
            ...
            } else if (p->key->node_type == AST_NODE_IDENT) {
            ...
            if (!is_proto_literal && p->value &&
                (p->value->node_type == AST_NODE_FUNC_EXPR ||
                 p->value->node_type == AST_NODE_ARROW_FUNC ||
                 p->value->node_type == AST_NODE_FUNC ||
                 p->value->node_type == AST_NODE_CLASS ||
                 p->value->node_type == AST_NODE_CLASS_EXPR)) {
        ...
        } else if (prop->node_type == AST_NODE_SPREAD) {
...
static MirValue jm_emit_conditional_value(JsMirTranspiler* mt,
...
// Template literal
static MirValue jm_emit_template_literal_value(JsMirTranspiler* mt,
        JsTemplateLiteralNode* tmpl) {
    ...
    // Box as string
    return jm_expression_value(mt, (JsAstNode*)tmpl,
        jm_box_string(mt, result_str), LMD_TYPE_STRING, VALUE_REP_ITEM);
}

// Tagged template literal: tag`str0 ${expr0} str1 ${expr1} str2`
static MirValue jm_emit_tagged_template_value(JsMirTranspiler* mt,
        JsTaggedTemplateNode* tt) {
    MIR_reg_t tag_fn;
    MIR_reg_t this_val = 0;
    if (tt->tag && tt->tag->node_type == AST_NODE_MEMBER_EXPR) {
        ...
        } else if (mem->property && mem->property->node_type == AST_NODE_IDENT) {
            JsIdentifierNode* prop = (JsIdentifierNode*)mem->property;
    ...
    if (!tmpl) return jm_expression_value(mt, (JsAstNode*)tt,
        jm_emit_undefined(mt), LMD_TYPE_ANY, VALUE_REP_ITEM);
    ...
    return jm_expression_value(mt, (JsAstNode*)tt, jm_call_function_into(mt,
        ...
        jm_get_effective_type(mt, (JsAstNode*)tt), VALUE_REP_ITEM);
}
...
static void jm_track_tdz_closure_captures(JsMirTranspiler* mt, MIR_reg_t env,
    ...
    for (int ci = 0; ci < JM_CAPTURE_COUNT(fc); ci++) {
        ...
        tracked->binding = JM_CAPTURE_ARRAY(fc)[ci].entry;
...
static void jm_sync_arguments_param_after_write(JsMirTranspiler* mt,
    ...
    if (!mt || !member || mt->arguments_reg == 0 ||
        ...
        !member->property || member->property->node_type != AST_NODE_LITERAL) {
        return;
    }
    ...
    if (idx_lit->literal_type != AST_LITERAL_NUMBER || idx_lit->has_decimal) return;
...
MIR_reg_t jm_create_func_or_closure(JsMirTranspiler* mt, JsFuncCollected* fc) {
    ...
    if (JM_CAPTURE_COUNT(fc) > 0) {
        ...
        if (use_scope_env) {
            fn_reg = jm_call_4(mt, "js_new_closure_mir_pending", MIR_T_I64,
                MIR_T_I64, MIR_new_ref_op(mt->ctx, fc->func_item),
        ...
        fn_reg = jm_call_2(mt, "js_new_function_mir_pending", MIR_T_I64,
            MIR_T_I64, MIR_new_ref_op(mt->ctx, fc->func_item),
            MIR_T_I64, MIR_new_int_op(mt->ctx, pc));
    }
...
static MirValue jm_emit_function_value(JsMirTranspiler* mt,
        JsFunctionNode* fn) {
    JsFuncCollected* fc = jm_find_collected_func(mt, fn);
    if (!fc || !fc->func_item) {
        log_error("js-mir: function expression not found in collected functions");
        return jm_expression_value(mt, (JsAstNode*)fn, jm_emit_null(mt),
            LMD_TYPE_FUNC, VALUE_REP_ITEM);
    }
    ...
    if (JM_CAPTURE_COUNT(fc) > 0) {
        ...
        fn_reg = jm_call_2(mt, "js_new_distinct_function_mir_pending", MIR_T_I64,
            MIR_T_I64, MIR_new_ref_op(mt->ctx, fc->func_item),
            MIR_T_I64, MIR_new_int_op(mt->ctx, param_count));
    }
    ...
    return jm_expression_value(mt, (JsAstNode*)fn, fn_reg, LMD_TYPE_FUNC,
        VALUE_REP_ITEM);
}
...
static MirValue jm_transpile_expression_direct(JsMirTranspiler* mt,
        JsAstNode* expr);

static MirValue jm_expression_value(JsMirTranspiler* mt, JsAstNode* item,
        MIR_reg_t reg, TypeId type_id, ValueRep rep) {
    return em_value_for_rep(reg, type_id, rep, item ? item->type : NULL, item);
}
...
MirValue jm_transpile_expression_value(JsMirTranspiler* mt, JsAstNode* item,
    ...
    MirValue value = item && item->node_type == AST_NODE_LITERAL
        ? jm_transpile_literal_value(mt, (JsLiteralNode*)item, required)
        : jm_transpile_expression_direct(mt, item);
    return em_apply_value_demand(&mt->func_em->em,
        value, demand, required);
}

MIR_reg_t jm_transpile_box_item(JsMirTranspiler* mt, JsAstNode* item) {
    return jm_transpile_expression_value(mt, item, MIR_VALUE_REQUIRED_REP,
        VALUE_REP_ITEM).reg;
}
...
MIR_reg_t jm_transpile_condition(JsMirTranspiler* mt, JsAstNode* expr) {
    ...
    if (expr->node_type == AST_NODE_BINARY) {
        ...
        if (is_comparison) {
            ...
            if (bin->op == OPERATOR_JS_STRICT_EQ || bin->op == OPERATOR_JS_STRICT_NE) {
                ...
                if (jm_try_emit_packed_array_strict_equal(mt, bin, &boxed)) {
                    return jm_emit_is_truthy(mt, jm_item_value(boxed,
                        LMD_TYPE_BOOL));
                }
            }
    ...
    if (expr->node_type == AST_NODE_UNARY) {
        JsUnaryNode* un = (JsUnaryNode*)expr;
        if (un->op == OPERATOR_NOT && un->operand) {
...
static MirValue jm_transpile_invocation_value(JsMirTranspiler* mt,
        JsCallNode* call, bool construct) {
    JsMirArgStackScope arg_scope;
    jm_begin_arg_stack_scope(mt, &arg_scope);
    MirValue result = construct
        ? jm_emit_new_value(mt, call)
        : jm_emit_call_expression(mt, call);
...
static MirValue jm_transpile_call_value(JsMirTranspiler* mt,
        JsCallNode* call, bool construct) {
    return jm_transpile_invocation_value(mt, call, construct);
}
...
// The principal expression dispatcher publishes its producer-owned carrier.
// Sub-lowerers remain language-specific, but no caller re-classifies their
// result from a bare register (D2.4.1–D2.4.3, D8.2.6).
static MirValue jm_transpile_expression_direct(JsMirTranspiler* mt,
        JsAstNode* expr) {
    if (!expr) {
    ...
    case AST_NODE_IDENT: {
        ...
        return jm_transpile_identifier_value(mt, id);
    }
    case AST_NODE_LITERAL:
        return jm_transpile_literal_value(mt, (JsLiteralNode*)expr);
    case AST_NODE_BINARY:
        return jm_emit_binary_expression(mt, (JsBinaryNode*)expr);
    case AST_NODE_UNARY:
        return jm_emit_unary_expression(mt, (JsUnaryNode*)expr);
    case AST_NODE_CALL_EXPR:
        return jm_transpile_call_value(mt, (JsCallNode*)expr, false);
    case AST_NODE_MEMBER_EXPR:
        return jm_emit_member_value(mt, (JsMemberNode*)expr);
    case AST_NODE_ARRAY:
        return jm_emit_array_value(mt, (JsArrayNode*)expr);
    case AST_NODE_MAP:
        return jm_emit_object_value(mt, (JsObjectNode*)expr);
    case AST_NODE_FUNC_EXPR:
    case AST_NODE_ARROW_FUNC:
    case AST_NODE_FUNC: // class method functions built as FUNCTION_DECLARATION
        return jm_emit_function_value(mt, (JsFunctionNode*)expr);
    case AST_NODE_SPREAD: {
        ...
        MIR_reg_t value = sp->argument ? jm_transpile_box_item(mt, sp->argument)
            : jm_emit_null(mt);
        return jm_expression_value(mt, expr, value,
            jm_get_effective_type(mt, expr), VALUE_REP_ITEM);
    }
    case AST_NODE_CONDITIONAL_EXPR:
        return jm_emit_conditional_value(mt, (JsConditionalNode*)expr);
    case JS_AST_NODE_TEMPLATE_LITERAL:
        return jm_emit_template_literal_value(mt, (JsTemplateLiteralNode*)expr);
    case JS_AST_NODE_TAGGED_TEMPLATE:
        return jm_emit_tagged_template_value(mt, (JsTaggedTemplateNode*)expr);
    case AST_NODE_ASSIGN:
        return jm_emit_assignment_value(mt, (JsAssignmentNode*)expr);
    case AST_NODE_NEW_EXPR:
        return jm_transpile_call_value(mt, (JsCallNode*)expr, true);
    case AST_NODE_SEQ: {
        ...
        // The comma expression adopts its final operand's physical carrier;
        // only its source contract/provenance changes at this boundary.
        sequence.result.semantic_contract = expr->type;
        sequence.result.semantic_type = jm_get_effective_type(mt, expr);
        sequence.result.provenance_node = expr;
    ...
    case JS_AST_NODE_REGEX: {
        ...
        return jm_expression_value(mt, expr,
            jm_callr_2(mt, "js_create_regex_literal_items", MIR_T_I64, pattern, flags),
    ...
    case AST_NODE_YIELD: {
        ...
        if (!mt->in_generator && !yield_node->argument && !yield_node->delegate) {
            ...
            yield_id.node_type = AST_NODE_IDENT;
            yield_id.name = name_pool_create_len(mt->tp->name_pool, "yield", 5);
            MirValue value = jm_transpile_identifier_value(mt, &yield_id);
            value.semantic_contract = expr->type;
            value.semantic_type = LMD_TYPE_ANY;
            value.provenance_node = expr;
            return value;
        }
        MIR_reg_t val;
        ...
        if (mt->in_generator) {
            ...
            return jm_expression_value(mt, expr, mt->gen_input_reg,
                LMD_TYPE_ANY, VALUE_REP_ITEM);
        }
    ...
    case AST_NODE_AWAIT: {
        ...
        if (mt->in_generator && mt->in_async) {
            return jm_expression_value(mt, expr,
                jm_emit_await_value_reg(mt, promise_val, JS_MIR_SUSPEND_AWAIT),
                LMD_TYPE_ANY, VALUE_REP_ITEM);
        }
    ...
    case AST_NODE_CLASS_EXPR: {
        ...
        if (cls_expr->superclass &&
            cls_expr->superclass->node_type != AST_NODE_IDENT &&
            cls_expr->superclass->node_type != AST_NODE_NULL &&
            !(cls_expr->superclass->node_type == AST_NODE_LITERAL &&
              ((JsLiteralNode*)cls_expr->superclass)->literal_type == AST_LITERAL_NULL)) {
        ...
        return jm_expression_value(mt, expr, cls_obj, LMD_TYPE_FUNC,
            VALUE_REP_ITEM);
    }
    default:
        log_error("js-mir: unsupported expression type %d", expr->node_type);
        return jm_expression_value(mt, expr, jm_emit_null(mt), LMD_TYPE_NULL,
            VALUE_REP_ITEM);
    }
}
