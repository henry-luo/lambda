...
struct HttpRealmItems {
    Item* namespace_object = NULL;
    Item* server_prototype = NULL;
    Item* incoming_message_prototype = NULL;
    Item* server_response_prototype = NULL;
    Item* outgoing_message_prototype = NULL;
};
...
static bool js_http_object_has_key(Item obj, const char* key) {
    ...
    for (int64_t idx = 0; idx < len; idx++) {
        Item item = js_elements_get_int(keys, idx);
...
static bool js_http_has_marker(Item obj, const char* name) {
    Item marker = js_get_key_default(obj, make_string_item(name));
...
static void http_response_mark_headers_sent(Item response) {
    js_set_key_cstr(response, "__write_head_called__", (Item){.item = b2it(true)});
    js_set_key_cstr(response, "headersSent", (Item){.item = b2it(true)});
}

static void http_response_mark_sent(Item response, bool finished) {
    js_set_key_cstr(response, "__sent__", (Item){.item = b2it(true)});
    http_response_mark_headers_sent(response);
    if (finished) {
        js_set_key_cstr(response, "writableFinished", (Item){.item = b2it(true)});
    }
}
...
static const char* http_response_status_message(Item response, int status) {
    Item status_message = js_get_key_cstr(response, "statusMessage");
    if (get_type_id(status_message) == LMD_TYPE_STRING) {
        String* sm = it2s(status_message);
        Item default_status_message = js_get_key_cstr(response, "__default_status_message__");
...
static Item http_throw_invalid_status_code(Item status_item) {
    ...
    js_set_key_cstr(err, "code", make_string_item("ERR_HTTP_INVALID_STATUS_CODE"));
...
typedef struct JsHttpServer {
    uv_tcp_t  tcp;
    uv_pipe_t pipe;
    // Exact roots for the server's durable script values.
...
enum JsHttpServerValueSlot {
    JS_HTTP_SERVER_VALUE_OBJECT,
    JS_HTTP_SERVER_VALUE_REQUEST_HANDLER,
    JS_HTTP_SERVER_VALUE_INCOMING_MESSAGE_CTOR,
    JS_HTTP_SERVER_VALUE_SERVER_RESPONSE_CTOR,
    JS_HTTP_SERVER_VALUE_TIMEOUT_CALLBACK,
...
enum JsHttpConnValueSlot {
    JS_HTTP_CONN_VALUE_ASYNC_RESOURCE,
    JS_HTTP_CONN_VALUE_SOCKET_OBJECT,
    JS_HTTP_CONN_VALUE_CURRENT_REQUEST,
    JS_HTTP_CONN_VALUE_CURRENT_RESPONSE,
    JS_HTTP_CONN_VALUE_TIMEOUT_RESPONSE,
    JS_HTTP_CONN_VALUE_REQUEST_TIMEOUT_CALLBACK,
    JS_HTTP_CONN_VALUE_RESPONSE_TIMEOUT_CALLBACK,
    JS_HTTP_CONN_VALUE_TIMEOUT_TIMER,
...
static void http_conn_free_if_done(JsHttpConn* conn) {
    ...
    runtime_value_slots_destroy(&conn->values);
    mem_free(conn);
}
...
static void http_conn_update_socket_counters(JsHttpConn* conn) {
    if (!conn || HTTP_CONN_VALUE(conn, SOCKET_OBJECT).item == 0) return;
    // Node socket counters are public JS Number APIs; keep them explicit.
    js_set_key_cstr(HTTP_CONN_VALUE(conn, SOCKET_OBJECT), "bytesRead", js_make_number((double)conn->bytes_read));
    js_set_key_cstr(HTTP_CONN_VALUE(conn, SOCKET_OBJECT), "bytesWritten", js_make_number((double)conn->bytes_written));
    js_set_key_cstr(HTTP_CONN_VALUE(conn, SOCKET_OBJECT), "bufferSize", js_make_number((double)conn->pending_write_bytes));
}
...
// listeners
//
// http listeners live in the shared Node emitter. C0.2 first funnelled every
// registration and read through these helpers when the storage was still a
// per-module "__on_<event>__" slot; they now forward to js_ee_*, so the array
// promotion C0.2 added is the emitter's own storage.
// -----------------------------------------------------------------------

// snapshot of the listeners registered for `event`, in registration order
static Item http_listener_list(Item obj, const char* event) {
    if (obj.item == 0 || !event) return js_array_new(0);
    return js_ee_listeners(obj, make_string_item(event));
}

// js_ee_listenerCount returns a JS number (double-backed), so ask for the
// listener array and measure it rather than type-testing the count.
static bool http_has_listener_event(Item obj, const char* event) {
    if (obj.item == 0 || !event) return false;
    return js_array_length(js_ee_listeners(obj, make_string_item(event))) > 0;
}

// call every listener for `event`; returns true if at least one ran
static bool http_emit_event(Item obj, const char* event, Item* args, int argc) {
    if (obj.item == 0 || !event) return false;
    if (!http_has_listener_event(obj, event)) return false;
    RootFrame roots(2);
...
static void http_listener_add(Item obj, Item event_item, Item callback) {
    js_ee_on(obj, event_item, callback);
}

static void http_response_emit(Item self, const char* event, Item* args, int argc, bool flush_tasks) {
    if (http_emit_event(self, event, args, argc) && flush_tasks) {
        js_microtask_flush();
    }
}
...
static void http_response_append_body(Item self, Item chunk_item) {
    ...
    Item chunks = js_get_key_cstr(self, "__chunks__");
    ...
    js_set_key_cstr(self, "__body_len__", (Item){.item = i2it(total_len)});
}
...
static int http_response_body_len(Item self) {
    ...
    Item body = js_get_key_cstr(self, "__body__");
...
static Item http_response_materialize_body(Item self) {
    ...
    if (body_len <= 0) {
        ...
        js_set_key_cstr(self, "__body__", body);
        js_set_key_cstr(self, "__body_len__", (Item){.item = i2it(0)});
        return body;
    }
    ...
    mem_free(buf);
    js_set_key_cstr(self, "__body__", body);
    js_set_key_cstr(self, "__body_len__", (Item){.item = i2it(copied)});
    return body;
}
...
static bool http_response_single_body_bytes(Item self, const char** data, int* len) {
    ...
    *len = 0;
    ...
    Item chunk = js_elements_get_int(chunks, 0);
...
static Item http_response_raw_headers(Item self) {
    Item raw = js_get_key_cstr(self, "__raw_headers__");
    if (get_type_id(raw) != LMD_TYPE_ARRAY) {
        raw = js_array_new(0);
        js_set_key_cstr(self, "__raw_headers__", raw);
    }
    return raw;
}
...
static void http_raw_headers_remove_name(Item self, Item name) {
    ...
    for (int64_t i = 0; i + 1 < len; i += 2) {
        Item raw_name = js_elements_get_int(raw, i);
        Item raw_value = js_elements_get_int(raw, i + 1);
        if (http_header_name_case_equals(raw_name, name)) continue;
        js_array_push(next, raw_name);
        js_array_push(next, raw_value);
    }
    js_set_key_cstr(self, "__raw_headers__", next);
}
...
static bool http_response_header_has_token(Item self, const char* name, const char* token) {
    ...
    Item value = js_get_key_default(headers, make_string_item(name));
...
static int http_append_header_value(char* resp_buf, int pos, int cap, Item name, Item value) {
    ...
    if (http_header_value_is_array(value)) {
        ...
        if (http_header_name_equals(ks, "set-cookie", 10)) {
            for (int64_t i = 0; i < len; i++) {
                Item part = http_item_to_header_string(js_elements_get_int(value, i));
...
static void http_response_set_header_pair(Item self, Item name_item, Item value_item) {
    ...
    if (get_type_id(headers) != LMD_TYPE_MAP) {
        ...
        js_set_key_cstr(self, "__headers__", headers);
    }
    js_set_key_default(headers, http_lowercase_header_name(name), value);
...
static bool http_response_headers_committed(Item self) {
    Item sent = js_get_key_cstr(self, "__sent__");
    if (get_type_id(sent) == LMD_TYPE_BOOL && it2b(sent)) return true;
    Item headers_sent = js_get_key_cstr(self, "__headers_sent__");
    if (get_type_id(headers_sent) == LMD_TYPE_BOOL && it2b(headers_sent)) return true;
    Item write_head = js_get_key_cstr(self, "__write_head_called__");
...
static int http_response_append_headers(char* resp_buf, int pos, int cap, Item self,
    ...
    if (get_type_id(raw) == LMD_TYPE_ARRAY && js_array_length(raw) > 0) {
        int64_t len = js_array_length(raw);
        for (int64_t i = 0; i + 1 < len; i += 2) {
            Item k = js_elements_get_int(raw, i);
            Item v = js_elements_get_int(raw, i + 1);
...
static Item http_response_writeHead(Item self, Item status_item, Item reason_or_headers, Item headers_item) {
    ...
    js_set_key_cstr(self, "statusCode", (Item){.item = i2it(status)});
    ...
    if (get_type_id(reason_or_headers) == LMD_TYPE_STRING) {
        js_set_key_cstr(self, "statusMessage", reason_or_headers);
        status_message_item = reason_or_headers;
        headers_arg = headers_item;
    } else {
        js_set_key_cstr(self, "statusMessage", status_message_item);
    ...
    if (get_type_id(headers_arg) == LMD_TYPE_ARRAY) {
        ...
        if (len > 0 && get_type_id(js_elements_get_int(headers_arg, 0)) == LMD_TYPE_ARRAY) {
            pair_array = true;
        }
        if (pair_array) {
            for (int64_t i = 0; i < len; i++) {
                Item pair = js_elements_get_int(headers_arg, i);
                ...
                http_raw_headers_remove_name(self, js_elements_get_int(pair, 0));
            }
            for (int64_t i = 0; i < len; i++) {
                Item pair = js_elements_get_int(headers_arg, i);
                http_response_append_raw_header_pair(self,
                    js_elements_get_int(pair, 0),
                    js_elements_get_int(pair, 1));
            }
        } else {
            if ((len & 1) != 0) {
                return js_throw_type_error_code("ERR_INVALID_ARG_VALUE",
            ...
            for (int64_t i = 0; i + 1 < len; i += 2) {
                http_raw_headers_remove_name(self, js_elements_get_int(headers_arg, i));
            }
            for (int64_t i = 0; i + 1 < len; i += 2) {
                http_response_append_raw_header_pair(self,
                    js_elements_get_int(headers_arg, i),
                    js_elements_get_int(headers_arg, i + 1));
            }
        }
    ...
    js_set_key_cstr(self, "__write_head_called__", (Item){.item = b2it(true)});
    js_set_key_cstr(self, "__explicit_write_head_called__", (Item){.item = b2it(true)});
    js_set_key_cstr(self, "__write_head_status__", (Item){.item = i2it(status)});
    js_set_key_cstr(self, "__write_head_status_message__", status_message_item);
    js_set_key_cstr(self, "headersSent", (Item){.item = b2it(true)});
    return self;
}
...
extern "C" Item js_http_res_getHeader(Item self, Item name_item) {
    ...
    return js_get_key_default(headers, http_lowercase_header_name(name_item));
}
...
extern "C" Item js_http_res_getHeaders(Item self) {
    ...
    for (int64_t i = 0; i < len; i++) {
        ...
        if (type != LMD_TYPE_UNDEFINED && type != LMD_TYPE_NULL) {
            js_set_key_default(copy, key, value);
        }
    }
    return copy;
}
...
extern "C" Item js_http_res_removeHeader(Item self, Item name_item) {
    ...
    if (get_type_id(name_item) == LMD_TYPE_STRING) {
        js_set_key_default(headers, http_lowercase_header_name(name_item), make_js_undefined());
...
static Item http_res_write_ex(Item self, Item chunk_item, Item encoding_item, Item callback_item) {
    if (js_is_callable(encoding_item)) {
    ...
    Item method = js_get_key_cstr(self, "__request_method__");
    ...
    js_set_key_cstr(self, "__has_body_write__", (Item){.item = b2it(true)});
    js_set_key_cstr(self, "__headers_sent__", (Item){.item = b2it(true)});
    http_response_mark_headers_sent(self);
    bool below_hwm = true;
...
extern "C" Item js_http_res_send_internal(Item self, Item chunk_item) {
    ...
    js_set_key_cstr(self, "__used_send__", (Item){.item = b2it(true)});
    return self;
}
...
static void http_response_flush(Item self) {
    ...
    if (conn->destroyed) {
        ...
        if (HTTP_CONN_VALUE(conn, TIMEOUT_RESPONSE).item == self.item) {
            HTTP_CONN_SET_VALUE(conn, TIMEOUT_RESPONSE, make_js_undefined());
            HTTP_CONN_SET_VALUE(conn, RESPONSE_TIMEOUT_CALLBACK, make_js_undefined());
        }
        if (HTTP_CONN_VALUE(conn, CURRENT_RESPONSE).item == self.item) {
            HTTP_CONN_SET_VALUE(conn, CURRENT_RESPONSE, make_js_undefined());
        }
    ...
    if (http_response_bool_prop(self, "__partial_sent__")) {
        ...
        bool partial_chunked = http_response_bool_prop(self, "__partial_chunked__");
        bool partial_close_after = http_response_bool_prop(self, "__partial_close_after__");
        if (partial_chunked) {
            int frame_cap = remaining_len + 40;
            ...
            if (remaining_len > 0) {
                ...
                fpos += snprintf(frame + fpos, frame_cap - fpos, "%X\r\n", (unsigned int)copied);
                memcpy(frame + fpos, remaining, (size_t)copied);
                fpos += copied;
                memcpy(frame + fpos, "\r\n", 2);
                fpos += 2;
                mem_free(remaining);
            ...
            memcpy(frame + fpos, "0\r\n\r\n", 5);
            fpos += 5;
            http_conn_write_bytes(conn, make_string_item(frame, fpos), partial_close_after);
            mem_free(frame);
            js_set_key_cstr(self, "__partial_body_len__", (Item){.item = i2it(body_len)});
        } else if (remaining_len > 0) {
            char* remaining = (char*)mem_alloc(remaining_len, MEM_CAT_JS_RUNTIME);
            int copied = http_response_copy_body_range(self, remaining, sent_len, remaining_len);
            http_conn_write_bytes(conn, make_string_item(remaining, copied), partial_close_after);
            mem_free(remaining);
            js_set_key_cstr(self, "__partial_body_len__", (Item){.item = i2it(body_len)});
        } else {
            http_conn_write_bytes(conn, make_string_item("", 0), partial_close_after);
        }
        http_response_mark_sent(self, true);
        http_response_emit(self, "finish", NULL, 0, false);
        http_response_close_request(self);
    ...
    // get status code
    Item status_item = js_get_key_cstr(self, "statusCode");
    Item committed_status = js_get_key_cstr(self, "__write_head_status__");
    ...
    Item committed_status_message = js_get_key_cstr(self, "__write_head_status_message__");
    ...
    Item req_version = js_get_key_cstr(self, "__request_http_version__");
    ...
    bool send_date = true;
    Item send_date_item = js_get_key_cstr(self, "sendDate");
    ...
    Item method_item = js_get_key_cstr(self, "__request_method__");
    ...
    http_response_mark_sent(self, false);
}
...
static bool http_response_flush_partial(Item self) {
    ...
    if (first_partial) {
        ...
        bool explicit_chunked_body = has_transfer_encoding &&
            http_response_header_has_token(self, "transfer-encoding", "chunked");
        bool chunked_body = !has_content_length && !request_http_10 &&
                            (explicit_chunked_body || !close_after);
        if (!has_content_type) {
            head_len += snprintf(head + head_len, sizeof(head) - head_len,
                                 "Content-Type: text/plain\r\n");
        }
        ...
        // Partial HTTP/1.1 responses need chunk framing to keep the socket
        // reusable; without it close is the only unambiguous body delimiter.
        js_set_key_cstr(self, "__partial_chunked__", (Item){.item = b2it(chunked_body)});
        js_set_key_cstr(self, "__partial_close_after__", (Item){.item = b2it(close_after || (!chunked_body && !has_content_length))});
        head_len += snprintf(head + head_len, sizeof(head) - head_len, "\r\n");
    }

    int emit_len = body_len - sent_len;
    bool partial_chunked = http_response_bool_prop(self, "__partial_chunked__");
    int total = head_len + emit_len + (partial_chunked && emit_len > 0 ? 32 : 0);
    char* full = (char*)mem_alloc(total, MEM_CAT_JS_RUNTIME);
    if (head_len > 0) memcpy(full, head, (size_t)head_len);
    int output_len = head_len;
    if (emit_len > 0) {
        if (partial_chunked) {
            char* chunk = (char*)mem_alloc(emit_len, MEM_CAT_JS_RUNTIME);
            int copied = http_response_copy_body_range(self, chunk, sent_len, emit_len);
            output_len += snprintf(full + output_len, total - output_len, "%X\r\n", (unsigned int)copied);
            memcpy(full + output_len, chunk, (size_t)copied);
            output_len += copied;
            memcpy(full + output_len, "\r\n", 2);
            output_len += 2;
            mem_free(chunk);
        } else {
            int copied = http_response_copy_body_range(self, full + output_len, sent_len, emit_len);
            output_len += copied;
        }
    }
    ...
    bool below_hwm = http_conn_write_bytes(conn, make_string_item(full, output_len), false);
    if (!below_hwm) {
        // write(false) must be paired with a later drain event or handlers
        // that wait to call end() leave the response and connection open.
        js_set_key_cstr(self, "__need_drain__", (Item){.item = b2it(true)});
    }
    mem_free(full);
    js_set_key_cstr(self, "__partial_sent__", (Item){.item = b2it(true)});
    js_set_key_cstr(self, "__partial_body_len__", (Item){.item = i2it(body_len)});
    js_set_key_cstr(self, "__headers_sent__", (Item){.item = b2it(true)});
    http_response_mark_headers_sent(self);
    return below_hwm;
...
static Item http_res_end_ex(Item self, Item data_item, Item encoding_item, Item callback_item) {
    ...
    if (js_is_callable(data_item)) {
        callback = data_item;
        data_item = make_js_undefined();
        encoding_item = make_js_undefined();
    } else if (js_is_callable(encoding_item)) {
    ...
    if (js_is_callable(callback)) {
        js_set_key_cstr(self, "__end_callback__", callback);
    } else {
        js_set_key_cstr(self, "__end_callback__", make_js_undefined());
    }
    js_set_key_cstr(self, "__ending__", (Item){.item = b2it(true)});
    ...
    js_set_key_cstr(self, "__ending__", (Item){.item = b2it(false)});
    js_set_key_cstr(self, "finished", (Item){.item = b2it(true)});
    js_set_key_cstr(self, "writableEnded", (Item){.item = b2it(true)});
...
static Item js_http_res_inst_on(Item maybe_self, Item event_item, Item callback) {
    ...
    http_listener_add(self, event_item, callback);
...
static Item make_response_object(JsHttpConn* conn) {
    ...
    HttpRealmItems items = {};
    if (http_realm_items(&items, false) &&
            get_type_id(*items.server_response_prototype) == LMD_TYPE_MAP) {
        js_set_prototype(res, *items.server_response_prototype);
    }
    js_set_key_cstr(res, "__conn__", (Item){.item = i2it((int64_t)(uintptr_t)conn)});
#define JS_HTTP_RESPONSE_FIELDS(M) \
    M("statusCode", (Item){.item = i2it(200)}) \
    M("__headers__", http_new_header_object()) M("__body__", make_string_item("", 0)) \
    M("__body_len__", (Item){.item = i2it(0)}) M("__sent__", (Item){.item = b2it(false)}) \
    M("__headers_sent__", (Item){.item = b2it(false)}) \
    M("__write_head_called__", (Item){.item = b2it(false)}) \
    M("__used_send__", (Item){.item = b2it(false)}) \
    M("__has_body_write__", (Item){.item = b2it(false)}) \
    M("__reject_nonstandard_body_writes__", (Item){.item = b2it(conn && conn->server && conn->server->reject_nonstandard_body_writes)}) \
    M("headersSent", (Item){.item = b2it(false)}) M("__raw_headers__", js_array_new(0)) \
    M("__chunks__", js_array_new(0)) M("__ending__", (Item){.item = b2it(false)}) \
    M("statusMessage", make_string_item(http_status_text(200))) \
    M("__default_status_message__", make_string_item(http_status_text(200))) \
    M("finished", (Item){.item = b2it(false)}) M("writableEnded", (Item){.item = b2it(false)}) \
    M("writableFinished", (Item){.item = b2it(false)}) M("writable", (Item){.item = b2it(true)}) \
    M("writableCorked", (Item){.item = i2it(0)})
#define JS_HTTP_SET_RESPONSE_FIELD(name, value) js_set_key_cstr(res, name, value);
    JS_HTTP_RESPONSE_FIELDS(JS_HTTP_SET_RESPONSE_FIELD)
#undef JS_HTTP_SET_RESPONSE_FIELD
#undef JS_HTTP_RESPONSE_FIELDS
    if (conn) {
        Item socket = http_conn_socket_object(conn);
        js_set_key_cstr(res, "socket", socket);
        js_set_key_cstr(res, "connection", socket);
    }

#define JS_HTTP_RESPONSE_METHODS(M) \
    M("writeHead", js_http_res_inst_writeHead) M("setHeader", js_http_res_inst_setHeader) \
    M("getHeader", js_http_res_inst_getHeader) M("getHeaders", js_http_res_inst_getHeaders) \
    M("getHeaderNames", js_http_res_inst_getHeaderNames) M("getRawHeaderNames", js_http_res_inst_getRawHeaderNames) \
    M("hasHeader", js_http_res_inst_hasHeader) M("removeHeader", js_http_res_inst_removeHeader) \
    M("write", js_http_res_inst_write) M("_send", js_http_res_inst_send_internal) \
    M("end", js_http_res_inst_end) M("flushHeaders", js_http_res_inst_flushHeaders) \
    M("writeContinue", js_http_res_inst_writeContinue) M("setTimeout", js_http_res_inst_setTimeout) \
    M("on", js_http_res_inst_on)
#define JS_HTTP_INSTALL_RESPONSE_METHOD(name, target) js_http_set_direct_method(res, name, target);
    JS_HTTP_RESPONSE_METHODS(JS_HTTP_INSTALL_RESPONSE_METHOD)
#undef JS_HTTP_INSTALL_RESPONSE_METHOD
#undef JS_HTTP_RESPONSE_METHODS

    if (conn && conn->server) {
        Item ctor = HTTP_SERVER_VALUE(conn->server, SERVER_RESPONSE_CTOR);
...
static void http_request_headers_append(Item headers, const char* name, int name_len,
    ...
    if (get_type_id(existing) == LMD_TYPE_STRING) {
        ...
        js_set_key_default(headers, key, incoming);
    }
}
...
static Item make_request_object(JsHttpConn* conn, HttpRequestHead* req) {
    ...
    HttpRealmItems items = {};
    if (http_realm_items(&items, false) &&
            get_type_id(*items.incoming_message_prototype) == LMD_TYPE_MAP) {
        js_set_prototype(msg, *items.incoming_message_prototype);
    }
    ...
    js_set_key_cstr(msg, "httpVersion", make_string_item(version_string));
    js_set_key_cstr(msg, "httpVersionMajor", (Item){.item = i2it(http_major)});
    js_set_key_cstr(msg, "httpVersionMinor", (Item){.item = i2it(http_minor)});
    if (conn) {
        Item socket = http_conn_socket_object(conn);
        js_set_key_cstr(msg, "socket", socket);
        js_set_key_cstr(msg, "connection", socket);
        js_set_key_cstr(msg, "__server_req_conn__", (Item){.item = i2it((int64_t)(uintptr_t)conn)});
#define JS_HTTP_INSTALL_REQUEST_METHOD(name, target) js_http_set_direct_method(msg, name, target);
        JS_HTTP_INSTALL_REQUEST_METHOD("destroy", js_http_server_req_destroy)
        JS_HTTP_INSTALL_REQUEST_METHOD("setTimeout", js_http_server_req_setTimeout)
#undef JS_HTTP_INSTALL_REQUEST_METHOD
    }
    Item async_resource = js_async_hooks_create_resource("HTTPINCOMINGMESSAGE", 19);
    js_set_key_cstr(msg, "__async_resource__", async_resource);
    ...
    if (req->body_complete) {
        ...
        js_set_key_cstr(msg, "__server_req_complete__", (Item){.item = b2it(true)});
    } else {
        js_set_key_cstr(msg, "__server_req_complete__", (Item){.item = b2it(false)});
    }

    if (conn && conn->server) {
        Item ctor = HTTP_SERVER_VALUE(conn->server, INCOMING_MESSAGE_CTOR);
        if (get_type_id(ctor) == LMD_TYPE_FUNC || get_type_id(ctor) == LMD_TYPE_MAP) {
            Item proto = js_get_key_cstr(ctor, "prototype");
...
static bool http_conn_is_idle(JsHttpConn* conn) {
    ...
    if (HTTP_CONN_VALUE(conn, CURRENT_REQUEST).item != 0 &&
        get_type_id(HTTP_CONN_VALUE(conn, CURRENT_REQUEST)) != LMD_TYPE_UNDEFINED) return false;
...
static void http_server_maybe_finish_close(JsHttpServer* srv) {
    ...
    if (!srv->close_event_emitted) {
        ...
        http_emit_event(HTTP_SERVER_VALUE(srv, OBJECT), "close", NULL, 0);
    }
...
static Item http_server_error_tick(Item env_item) {
    ...
    if (http_emit_event(self, "error", &err, 1)) js_microtask_flush();
    if (failed_srv) {
...
static void http_server_close_failed_listen(JsHttpServer* srv, Item self) {
    if (!srv) return;
    js_set_key_cstr(self, "listening", (Item){.item = b2it(false)});
    js_set_key_cstr(self, "__server__", ItemNull);
    ...
    if (handle && !uv_is_closing(handle)) {
        ...
        uv_close(handle, [](uv_handle_t* h) {
            ...
            if (s) {
                runtime_value_slots_destroy(&s->values);
                mem_free(s);
            }
        });
        return;
...
static void http_request_emit_close_now(Item req) {
    ...
    js_set_key_default(req, close_emitted_key, (Item){.item = b2it(true)});
    js_set_key_cstr(req, "closed", (Item){.item = b2it(true)});
    Item handle_item = js_get_key_cstr(req, "__server_req_conn__");
    if (get_type_id(handle_item) == LMD_TYPE_INT) {
        JsHttpConn* conn = (JsHttpConn*)(uintptr_t)it2i(handle_item);
        if (conn) HTTP_CONN_SET_VALUE(conn, REQUEST_TIMEOUT_CALLBACK, make_js_undefined());
    }
    Item async_resource = js_get_key_cstr(req, "__async_resource__");
    ...
    Item emit_fn = js_get_key_cstr(req, "emit");
...
static Item js_http_server_req_destroy(Item maybe_self, Item err_item) {
    ...
    if (actual_err.item != 0 && err_type != LMD_TYPE_UNDEFINED && err_type != LMD_TYPE_NULL) {
        js_set_key_cstr(self, "errored", actual_err);
    }
    // Server-side IncomingMessage.destroy(err) aborts the socket, not an
    // uncaught stream error on the request object.
    js_set_key_cstr(self, "destroyed", (Item){.item = b2it(true)});
    js_set_key_cstr(self, "readableAborted", (Item){.item = b2it(true)});
    http_request_emit_close_now(self);
    Item handle_item = js_get_key_cstr(self, "__server_req_conn__");
    if (get_type_id(handle_item) == LMD_TYPE_INT) {
        JsHttpConn* conn = (JsHttpConn*)(uintptr_t)it2i(handle_item);
        http_conn_close_now(conn);
    }
...
static void http_conn_close_now(JsHttpConn* conn) {
    ...
    conn->destroyed = true;
    if (HTTP_CONN_VALUE(conn, SOCKET_OBJECT).item != 0) {
        js_set_key_cstr(HTTP_CONN_VALUE(conn, SOCKET_OBJECT), "destroyed", (Item){.item = b2it(true)});
    }
...
static JsHttpConn* http_conn_from_socket_object(Item self) {
    Item handle_item = js_get_key_cstr(self, "__http_conn__");
...
static void http_conn_socket_emit(JsHttpConn* conn, const char* event) {
    if (!conn || HTTP_CONN_VALUE(conn, SOCKET_OBJECT).item == 0 || !event) return;
    Item socket = HTTP_CONN_VALUE(conn, SOCKET_OBJECT);
    if (http_emit_event(HTTP_CONN_VALUE(conn, SOCKET_OBJECT), event, &socket, 1)) {
        js_microtask_flush();
    }
}
...
static void http_conn_clear_timeout(JsHttpConn* conn) {
    ...
    js_clearTimeout(HTTP_CONN_VALUE(conn, TIMEOUT_TIMER));
    HTTP_CONN_SET_VALUE(conn, TIMEOUT_TIMER, make_js_undefined());
...
static Item js_http_conn_socket_timeout_fire(Item env_item) {
    ...
    HTTP_CONN_SET_VALUE(conn, TIMEOUT_TIMER, make_js_undefined());
    Item socket = http_conn_socket_object(conn);
    if (js_is_callable(HTTP_CONN_VALUE(conn, REQUEST_TIMEOUT_CALLBACK))) {
        js_call_function(HTTP_CONN_VALUE(conn, REQUEST_TIMEOUT_CALLBACK), socket, &socket, 1);
        js_microtask_flush();
    }
    Item timeout_response = HTTP_CONN_VALUE(conn, TIMEOUT_RESPONSE);
    Item response_timeout_callback = HTTP_CONN_VALUE(conn, RESPONSE_TIMEOUT_CALLBACK);
    ...
    if (js_is_callable(response_timeout_callback)) {
    ...
    if (response_event_target.item == 0 ||
        ...
        response_event_target = HTTP_CONN_VALUE(conn, CURRENT_RESPONSE);
    }
    if (response_event_target.item != 0 &&
        get_type_id(response_event_target) != LMD_TYPE_UNDEFINED) {
        if (http_emit_event(response_event_target, "timeout", &socket, 1)) {
            js_microtask_flush();
        }
    }
    http_conn_socket_emit(conn, "timeout");
    if (conn->server) {
        Item server_cb = HTTP_SERVER_VALUE(conn->server, TIMEOUT_CALLBACK);
        if (js_is_callable(server_cb)) {
            js_call_function(server_cb, HTTP_SERVER_VALUE(conn->server, OBJECT), &socket, 1);
            js_microtask_flush();
        } else if (http_emit_event(HTTP_SERVER_VALUE(conn->server, OBJECT), "timeout", &socket, 1)) {
            js_microtask_flush();
        }
    }
    return make_js_undefined();
...
static void http_conn_start_timeout(JsHttpConn* conn, int64_t delay) {
    ...
    HTTP_CONN_SET_VALUE(conn, TIMEOUT_TIMER, timer);
...
static Item js_http_conn_socket_on(Item maybe_self, Item event_item, Item callback_item) {
    ...
    http_listener_add(self, actual_event, actual_callback);
    return self;
}

enum JsHttpTimeoutLane {
    JS_HTTP_TIMEOUT_SOCKET,
    JS_HTTP_TIMEOUT_REQUEST,
    JS_HTTP_TIMEOUT_RESPONSE,
};

static Item js_http_set_timeout(Item maybe_self, Item msecs_item,
        Item callback_item, JsHttpTimeoutLane lane) {
    const char* marker = lane == JS_HTTP_TIMEOUT_SOCKET ? "__http_conn__"
        : lane == JS_HTTP_TIMEOUT_REQUEST ? "__server_req_conn__" : "__conn__";
    Item self = js_http_receiver(maybe_self, marker);
    ...
    JsHttpConn* conn = lane == JS_HTTP_TIMEOUT_SOCKET
        ? http_conn_from_socket_object(self) : NULL;
    if (!conn) {
        Item handle_item = js_get_key_cstr(self, marker);
        if (get_type_id(handle_item) != LMD_TYPE_INT) return self;
        conn = (JsHttpConn*)(uintptr_t)it2i(handle_item);
    }
    if (!conn || conn->destroyed) return self;
    if (lane == JS_HTTP_TIMEOUT_SOCKET) {
        if (js_is_callable(actual_callback)) {
            js_http_conn_socket_on(self, make_string_item("timeout"), actual_callback);
        }
    } else if (lane == JS_HTTP_TIMEOUT_REQUEST) {
        HTTP_CONN_SET_VALUE(conn, REQUEST_TIMEOUT_CALLBACK,
            !http_response_bool_prop(self, "__server_req_complete__") &&
            js_is_callable(actual_callback) ? actual_callback : make_js_undefined());
    } else {
        if (HTTP_CONN_VALUE(conn, TIMEOUT_RESPONSE).item != 0 &&
            get_type_id(HTTP_CONN_VALUE(conn, TIMEOUT_RESPONSE)) != LMD_TYPE_UNDEFINED &&
            HTTP_CONN_VALUE(conn, TIMEOUT_RESPONSE).item != self.item) return self;
        HTTP_CONN_SET_VALUE(conn, TIMEOUT_RESPONSE, self);
        HTTP_CONN_SET_VALUE(conn, RESPONSE_TIMEOUT_CALLBACK,
            js_is_callable(actual_callback) ? actual_callback : make_js_undefined());
    }
    int64_t delay = 0;
    http_item_to_integral_int64(actual_msecs, &delay);
    if (lane == JS_HTTP_TIMEOUT_SOCKET && delay < 0) delay = 0;
    http_conn_start_timeout(conn, delay);
    return self;
}

#define JS_HTTP_TIMEOUT_WRAPPER(name, lane) \
    static Item name(Item maybe_self, Item msecs_item, Item callback_item) { \
        return js_http_set_timeout(maybe_self, msecs_item, callback_item, lane); \
    }
JS_HTTP_TIMEOUT_WRAPPER(js_http_conn_socket_setTimeout, JS_HTTP_TIMEOUT_SOCKET)
JS_HTTP_TIMEOUT_WRAPPER(js_http_server_req_setTimeout, JS_HTTP_TIMEOUT_REQUEST)
JS_HTTP_TIMEOUT_WRAPPER(js_http_res_inst_setTimeout, JS_HTTP_TIMEOUT_RESPONSE)
#undef JS_HTTP_TIMEOUT_WRAPPER

static Item http_conn_socket_object(JsHttpConn* conn) {
    if (!conn) return make_js_undefined();
    if (HTTP_CONN_VALUE(conn, SOCKET_OBJECT).item != 0) {
        return HTTP_CONN_VALUE(conn, SOCKET_OBJECT);
    }

    Item obj = js_new_object_with_class(JS_CLASS_SOCKET);
    HTTP_CONN_SET_VALUE(conn, SOCKET_OBJECT, obj);
    ...
    js_set_key_cstr(obj, "__http_conn__", (Item){.item = i2it((int64_t)(uintptr_t)conn)});
#define JS_HTTP_SOCKET_METHODS(M) \
    M("on", js_http_conn_socket_on) M("once", js_http_conn_socket_on) \
    M("write", js_http_conn_socket_write) M("end", js_http_conn_socket_end) \
    M("destroy", js_http_conn_socket_destroy) M("setTimeout", js_http_conn_socket_setTimeout)
#define JS_HTTP_INSTALL_SOCKET_METHOD(name, target) js_http_set_direct_method(obj, name, target);
    JS_HTTP_SOCKET_METHODS(JS_HTTP_INSTALL_SOCKET_METHOD)
#undef JS_HTTP_INSTALL_SOCKET_METHOD
#undef JS_HTTP_SOCKET_METHODS
    js_set_key_cstr(obj, "destroyed", (Item){.item = b2it(false)});
    js_set_key_cstr(obj, "bytesRead", js_make_number((double)conn->bytes_read));
    js_set_key_cstr(obj, "bytesWritten", js_make_number((double)conn->bytes_written));
    js_set_key_cstr(obj, "bufferSize", js_make_number(0.0));
    Item hwm = js_make_number((double)(16 * 1024));
    Item readable_state = js_new_object();
    js_set_key_cstr(readable_state, "highWaterMark", hwm);
    Item writable_state = js_new_object();
    js_set_key_cstr(writable_state, "highWaterMark", hwm);
    js_set_key_cstr(obj, "_readableState", readable_state);
    js_set_key_cstr(obj, "_writableState", writable_state);
    js_set_key_cstr(obj, "readableHighWaterMark", hwm);
...
static int http_server_max_requests(JsHttpServer* srv) {
    if (!srv) return 0;
    Item value = js_get_key_cstr(HTTP_SERVER_VALUE(srv, OBJECT), "maxRequestsPerSocket");
    if (get_type_id(value) == LMD_TYPE_INT) return (int)it2i(value);
    return 0;
}
...
static void http_conn_feed_request_body(JsHttpConn* conn) {
    if (!conn || HTTP_CONN_VALUE(conn, CURRENT_REQUEST).item == 0) return;
    if (conn->request_body_chunked) {
        ...
        if (decoded_len > 0) {
            js_readable_push(HTTP_CONN_VALUE(conn, CURRENT_REQUEST), make_string_item(conn->recv_buf, decoded_len));
        }
        js_readable_push(HTTP_CONN_VALUE(conn, CURRENT_REQUEST), ItemNull);
        js_set_key_cstr(HTTP_CONN_VALUE(conn, CURRENT_REQUEST), "__server_req_complete__", (Item){.item = b2it(true)});
        HTTP_CONN_SET_VALUE(conn, REQUEST_TIMEOUT_CALLBACK, make_js_undefined());
    ...
    while (conn->recv_len > 0 && conn->request_body_remaining > 0) {
        ...
        if (take > 0) {
            js_readable_push(HTTP_CONN_VALUE(conn, CURRENT_REQUEST), make_string_item(conn->recv_buf, take));
...
static bool http_server_emit_client_error(JsHttpConn* conn, const char* code,
    ...
    if (!http_has_listener_event(HTTP_SERVER_VALUE(conn->server, OBJECT), "clientError")) return false;

    Item err = js_new_error(make_string_item(message));
    js_set_key_cstr(err, "code", make_string_item(code));
    js_set_key_cstr(err, "bytesParsed", (Item){.item = i2it(bytes_parsed)});
    js_set_key_cstr(err, "rawPacket", js_buffer_from_bytes(conn->recv_buf, conn->recv_len));
    Item socket = http_conn_socket_object(conn);
    Item args[2] = { err, socket };
    http_emit_event(HTTP_SERVER_VALUE(conn->server, OBJECT), "clientError", args, 2);
    js_microtask_flush();
    return true;
}
...
static Item http_response_for_request(JsHttpConn* conn, HttpRequestHead* req, bool force_close,
    ...
    if (conn) {
        ...
        HTTP_CONN_SET_VALUE(conn, CURRENT_RESPONSE, res_obj);
    }
    ...
    js_set_key_cstr(res_obj, "__close_after_response__", (Item){.item = b2it(close_after)});
    js_set_key_cstr(res_obj, "__keep_alive_max__", (Item){.item = i2it(max_requests)});
    js_set_key_cstr(res_obj, "__keep_alive_max_null__", (Item){.item = b2it(http_server_max_requests_is_null(conn ? conn->server : NULL))});
    js_set_key_cstr(res_obj, "__request_http_version__", req ? make_string_item(req->http_version) : make_string_item("HTTP/1.1"));
    js_set_key_cstr(res_obj, "__request_method__", req ? make_string_item(req->method) : make_string_item("GET"));
    js_set_key_cstr(res_obj, "__request_accepts_chunked_response__", (Item){.item = b2it(req && http_request_accepts_chunked_response(req))});
    return res_obj;
}
...
static void http_server_send_service_unavailable(JsHttpConn* conn) {
    ...
    js_set_key_cstr(res_obj, "statusCode", (Item){.item = i2it(503)});
    js_set_key_cstr(res_obj, "statusMessage", make_string_item("Service Unavailable"));
    js_set_key_cstr(res_obj, "__body__", make_string_item("", 0));
...
static void http_server_send_expectation_failed(JsHttpConn* conn, HttpRequestHead* req,
    ...
    js_set_key_cstr(res_obj, "statusCode", (Item){.item = i2it(417)});
    js_set_key_cstr(res_obj, "statusMessage", make_string_item("Expectation Failed"));
    js_set_key_cstr(res_obj, "__body__", make_string_item("", 0));
    http_response_flush(res_obj);
}

// run one request-lane listener inside the connection's async resource
static void http_dispatch_one(JsHttpConn* conn, Item listener, Item* args) {
    if (!js_is_callable(listener)) return;
    Item previous_resource = js_async_hooks_enter_resource(
        HTTP_CONN_VALUE(conn, ASYNC_RESOURCE));
    js_call_function(listener, HTTP_SERVER_VALUE(conn->server, OBJECT), args, 2);
...
static void http_server_dispatch_request(JsHttpConn* conn, HttpRequestHead* req,
        ...
        bool has_buffered_request) {
    JsHttpServer* srv = conn->server;
    if (HTTP_CONN_VALUE(conn, ASYNC_RESOURCE).item == 0) {
        HTTP_CONN_SET_VALUE(conn, ASYNC_RESOURCE, js_new_object());
    }
    ...
    js_set_key_cstr(res_obj, "__request__", req_obj);
    if (!req->body_complete) {
        HTTP_CONN_SET_VALUE(conn, CURRENT_REQUEST, req_obj);
    ...
    if (has_handler) {
        http_dispatch_one(conn, HTTP_SERVER_VALUE(srv, REQUEST_HANDLER), args);
    }
    if (has_request_event) {
        ...
        http_dispatch_listeners(conn, on_req,
            has_handler ? HTTP_SERVER_VALUE(srv, REQUEST_HANDLER) : ItemNull, args);
    }
}
...
static void http_server_process_parsed_request(JsHttpConn* conn,
    ...
    Item on_req = http_listener_list(HTTP_SERVER_VALUE(srv, OBJECT), "request");
    Item on_expect = expect_continue
        ? http_listener_list(HTTP_SERVER_VALUE(srv, OBJECT), "checkContinue")
        : (expect_unknown ? http_listener_list(HTTP_SERVER_VALUE(srv, OBJECT), "checkExpectation")
                          : js_array_new(0));
    bool has_handler = js_is_callable(HTTP_SERVER_VALUE(srv, REQUEST_HANDLER));
    bool has_request_event = js_array_length(on_req) > 0;
    bool has_expect_handler = js_array_length(on_expect) > 0;
    ...
    if (!has_expect_handler && !has_handler && !has_request_event) return;
    if (response_at_eof && HTTP_CONN_VALUE(conn, ASYNC_RESOURCE).item == 0) {
        HTTP_CONN_SET_VALUE(conn, ASYNC_RESOURCE, js_new_object());
    }
...
static void http_server_read_cb(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf) {
    ...
    if (nread > 0) {
        ...
        while (conn->recv_len > 0 && !conn->destroyed) {
            ...
            http_server_process_parsed_request(conn, &req, expect_continue,
                expect_unknown, false, has_buffered_request);
            int remaining = conn->recv_len - consumed;
            if (remaining > 0) memmove(conn->recv_buf, conn->recv_buf + consumed, (size_t)remaining);
            conn->recv_len = remaining;
        }
    ...
    if (nread < 0 && conn && !conn->destroyed) {
        ...
        if (conn->recv_len > 0 && HTTP_CONN_VALUE(conn, CURRENT_REQUEST).item == 0) {
            HttpRequestHead req;
            http_request_head_init(&req, conn->recv_buf);
            int consumed = 0;
            if (parse_http_request(conn->recv_buf, conn->recv_len, &req, &consumed) == 0 &&
        ...
        if (conn->request_count == 0 ||
            (conn->recv_len == 0 && (HTTP_CONN_VALUE(conn, CURRENT_REQUEST).item == 0 ||
             get_type_id(HTTP_CONN_VALUE(conn, CURRENT_REQUEST)) == LMD_TYPE_UNDEFINED))) {
...
static void http_server_connection_cb(uv_stream_t* server, int status) {
    ...
    if (uv_accept(server, http_conn_stream(conn)) == 0) {
        ...
        uv_close((uv_handle_t*)http_conn_stream(conn), [](uv_handle_t* h) {
            JsHttpConn* c = (JsHttpConn*)h->data;
            if (c) {
                runtime_value_slots_destroy(&c->values);
                mem_free(c);
            }
        });
    }
...
static Item js_http_server_listening_tick(Item env_item) {
    ...
    Item self = env[0];
    Item callback = env[1];
    http_emit_event(self, "listening", NULL, 0);
...
extern "C" Item js_http_server_address(Item self) {
    ...
    if (saddr.ss_family == AF_INET) {
        ...
        js_set_key_cstr(result, "address", make_string_item(ip));
        js_set_key_cstr(result, "port", (Item){.item = i2it(ntohs(in->sin_port))});
        js_set_key_cstr(result, "family", make_string_item("IPv4"));
    }
    return result;
}
...
static Item js_http_server_inst_setTimeout(Item maybe_self, Item msecs_item, Item callback_item) {
    ...
    if (js_is_callable(actual_callback)) {
        HTTP_SERVER_SET_VALUE(srv, TIMEOUT_CALLBACK, actual_callback);
    }
    return self;
}
...
extern "C" Item js_http_createServer(Item options_or_handler, Item maybe_handler) {
    ...
    if ((options_type == LMD_TYPE_MAP) &&
        ...
        response_ctor = js_get_key_cstr(options_or_handler, "ServerResponse");
    }
    ...
    HTTP_SERVER_SET_VALUE(srv, REQUEST_HANDLER, handler);
    HTTP_SERVER_SET_VALUE(srv, INCOMING_MESSAGE_CTOR, incoming_ctor);
    HTTP_SERVER_SET_VALUE(srv, SERVER_RESPONSE_CTOR, response_ctor);
    Item reject_body = js_get_key_cstr(options_or_handler, "rejectNonStandardBodyWrites");
    ...
    HTTP_SERVER_SET_VALUE(srv, OBJECT, obj);
    HttpRealmItems items = {};
    if (http_realm_items(&items, false) &&
            get_type_id(*items.server_prototype) == LMD_TYPE_MAP) {
        js_set_prototype(obj, *items.server_prototype);
    }
    js_set_key_cstr(obj, "__server__", (Item){.item = i2it((int64_t)(uintptr_t)srv)});
#define JS_HTTP_SERVER_METHODS(M) \
    M("listen", js_http_server_inst_listen) M("close", js_http_server_inst_close) \
    M("getConnections", js_http_server_inst_getConnections) M("on", js_http_server_inst_on) \
    M("setTimeout", js_http_server_inst_setTimeout) M("address", js_http_server_inst_address) \
    M("ref", js_http_server_inst_ref) M("unref", js_http_server_inst_unref)
#define JS_HTTP_INSTALL_SERVER_METHOD(name, target) js_http_set_direct_method(obj, name, target);
    JS_HTTP_SERVER_METHODS(JS_HTTP_INSTALL_SERVER_METHOD)
#undef JS_HTTP_INSTALL_SERVER_METHOD
#undef JS_HTTP_SERVER_METHODS
    js_set_key_cstr(obj, "listening", (Item){.item = b2it(false)});
...
typedef struct JsHttpClientReq {
    uv_tcp_t   tcp;
    uv_pipe_t  pipe;
    // Exact roots for the request's durable script values.
    RuntimeValueSlots values;
    ...
    int        response_decoded_delivered;
    char       method[16];
    ...
    // keep-alive reuse state is part of the post-ab4 HTTP recovery path.
    bool       keep_alive_agent;
    bool       reusable_idle;
} JsHttpClientReq;

enum JsHttpClientValueSlot {
    JS_HTTP_CLIENT_VALUE_OBJECT,
    JS_HTTP_CLIENT_VALUE_CALLBACK,
    JS_HTTP_CLIENT_VALUE_ALS_CONTEXT,
    JS_HTTP_CLIENT_VALUE_ASYNC_RESOURCE,
    JS_HTTP_CLIENT_VALUE_SOCKET_OBJECT,
    JS_HTTP_CLIENT_VALUE_AGENT,
    JS_HTTP_CLIENT_VALUE_RESPONSE,
    JS_HTTP_CLIENT_VALUE_ABORT_SIGNAL,
    JS_HTTP_CLIENT_VALUE_ABORT_HANDLER,
...
static void http_client_mark_socket_idle(JsHttpClientReq* creq);
...
static void http_client_sync_header_property(JsHttpClientReq* creq, Item req_obj) {
    ...
    js_set_key_cstr(req_obj, "_header", make_string_item(creq->send_buf, creq->send_head_len));
}

static void http_client_metadata_ensure(Item req_obj, Item* out_headers, Item* out_raw) {
    Item headers = js_get_key_cstr(req_obj, "__headers__");
    if (get_type_id(headers) != LMD_TYPE_MAP) {
        headers = http_new_header_object();
        js_set_key_cstr(req_obj, "__headers__", headers);
    }
    Item raw = js_get_key_cstr(req_obj, "__raw_headers__");
    if (get_type_id(raw) != LMD_TYPE_ARRAY) {
        raw = js_array_new(0);
        js_set_key_cstr(req_obj, "__raw_headers__", raw);
    }
...
static void http_client_metadata_set(Item req_obj, Item raw_name, Item value) {
    ...
    js_set_key_default(headers, lower, value);
    ...
    if (get_type_id(headers_item) == LMD_TYPE_MAP) {
        ...
        for (int64_t i = 0; i < len; i++) {
            ...
            if (get_type_id(entry) == LMD_TYPE_ARRAY && js_array_length(entry) >= 2) {
                http_client_metadata_set(req_obj, js_elements_get_int(entry, 0),
                                         js_elements_get_int(entry, 1));
            } else if (i + 1 < len) {
                http_client_metadata_set(req_obj, entry, js_elements_get_int(headers_item, i + 1));
                i++;
            }
        }
    }
...
static void http_client_emit(Item req_obj, const char* event) {
    if (http_emit_event(req_obj, event, NULL, 0)) {
        js_microtask_flush();
    }
}

static Item http_client_finish_tick(Item req_obj) {
    js_set_key_cstr(req_obj, "writableFinished", (Item){.item = b2it(true)});
    http_client_emit(req_obj, "finish");
    return make_js_undefined();
}

static void http_client_schedule_finish(Item req_obj) {
    Item scheduled = js_get_key_cstr(req_obj, "__finish_scheduled__");
    if (get_type_id(scheduled) == LMD_TYPE_BOOL && it2b(scheduled)) return;
    js_set_key_cstr(req_obj, "__finish_scheduled__", (Item){.item = b2it(true)});
...
static int64_t http_client_resource_int_prop(Item resource, const char* name) {
    ...
    return get_type_id(value) == LMD_TYPE_INT ? it2i(value) : -1;
}

static void http_client_stamp_socket_async_ids(Item socket, int64_t async_id, int64_t trigger_id) {
    if (socket.item == 0) return;
    js_set_key_default(socket, http_client_async_symbol_key("nodejs.async_id_symbol", 22),
                    (Item){.item = i2it(async_id)});
    js_set_key_default(socket, http_client_async_symbol_key("nodejs.trigger_async_id_symbol", 30),
                    (Item){.item = i2it(trigger_id)});
}

static void http_client_stamp_socket_from_resource(JsHttpClientReq* creq) {
    Item socket = HTTP_CLIENT_VALUE(creq, SOCKET_OBJECT);
    Item async_resource = HTTP_CLIENT_VALUE(creq, ASYNC_RESOURCE);
    if (!creq || socket.item == 0) return;
    int64_t async_id = http_client_resource_int_prop(async_resource, "__lambda_async_id__");
    int64_t trigger_id = http_client_resource_int_prop(async_resource, "__lambda_trigger_async_id__");
    http_client_stamp_socket_async_ids(socket, async_id, trigger_id);
}

static Item http_client_socket_object(JsHttpClientReq* creq) {
    if (!creq) return make_js_undefined();
    Item socket = HTTP_CLIENT_VALUE(creq, SOCKET_OBJECT);
    if (socket.item != 0) {
        if (!creq->reusable_idle) {
            // Reused keep-alive sockets keep their JS identity but need the
            // fresh request async id after the idle marker parked them at -1.
            http_client_stamp_socket_from_resource(creq);
        }
        return socket;
    }

    Item obj = js_new_object_with_class(JS_CLASS_SOCKET);
    HTTP_CLIENT_SET_VALUE(creq, SOCKET_OBJECT, obj);
    Item net_proto = js_net_get_socket_prototype();
    if (get_type_id(net_proto) == LMD_TYPE_MAP) {
        js_set_prototype(obj, net_proto);
    }
    js_set_key_cstr(obj, "__client__", (Item){.item = i2it((int64_t)(uintptr_t)creq)});
    js_set_key_cstr(obj, "destroyed", (Item){.item = b2it(false)});
    js_set_key_cstr(obj, "bytesRead", js_make_number(0.0));
    js_set_key_cstr(obj, "bytesWritten", js_make_number(0.0));
    js_set_key_cstr(obj, "bufferSize", js_make_number(0.0));
    Item hwm = js_make_number((double)(16 * 1024));
    Item readable_state = js_new_object();
    js_set_key_cstr(readable_state, "highWaterMark", hwm);
    ...
    js_set_key_cstr(obj, "writableHighWaterMark", hwm);
    http_client_stamp_socket_from_resource(creq);
    Item request = HTTP_CLIENT_VALUE(creq, OBJECT);
    if (request.item != 0) js_set_key_cstr(request, "socket", obj);
    return obj;
}
...
static void http_client_mark_socket_idle(JsHttpClientReq* creq) {
    ...
    // Keep-alive reuse parks the public socket between requests; Node marks
    // the old handle id dead until a fresh async resource is assigned.
    js_async_hooks_emit_destroy_resource(HTTP_CLIENT_VALUE(creq, ASYNC_RESOURCE));
    ...
    *raw_headers = js_array_new(0);
    ...
    while (p < hdr_end - 2) {
        ...
        if (nlen == 10 && memcmp(lc_name, "set-cookie", 10) == 0) {
            Item existing = js_get_key_default(*headers_obj, key);
            if (get_type_id(existing) == LMD_TYPE_ARRAY) {
                ...
                js_set_key_default(*headers_obj, key, cookies);
            }
        } else {
            js_set_key_default(*headers_obj, key, value);
        }
...
extern "C" Item js_http_client_res_on(Item self2, Item ev2, Item cb2) {
    if (get_type_id(ev2) == LMD_TYPE_STRING && js_is_callable(cb2)) {
        String* ev = it2s(ev2);
        if (ev->len == 5 && memcmp(ev->chars, "close", 5) == 0 &&
            ...
            js_set_key_cstr(self2, "__close_emitted__", (Item){.item = b2it(true)});
            js_set_key_cstr(self2, "closed", (Item){.item = b2it(true)});
...
static Item js_http_client_res_destroy(Item maybe_self, Item err_item) {
    ...
    Item handle_item = js_get_key_cstr(self, "__client__");
    if (get_type_id(handle_item) == LMD_TYPE_INT) {
        JsHttpClientReq* creq = (JsHttpClientReq*)(uintptr_t)it2i(handle_item);
        if (creq && !creq->destroyed) {
            creq->destroyed = true;
            js_set_key_cstr(HTTP_CLIENT_VALUE(creq, OBJECT), "destroyed",
                (Item){.item = b2it(true)});
...
static Item js_http_econnreset_error(Item err) {
    ...
    js_set_key_cstr(result, "code", make_string_item("ECONNRESET"));
    return result;
}
...
static Item js_http_abort_reason(JsHttpClientReq* creq) {
    Item signal = HTTP_CLIENT_VALUE(creq, ABORT_SIGNAL);
    if (creq && signal.item) {
...
static void js_http_client_remove_abort_listener(JsHttpClientReq* creq) {
    ...
    Item signal = HTTP_CLIENT_VALUE(creq, ABORT_SIGNAL);
    Item handler = HTTP_CLIENT_VALUE(creq, ABORT_HANDLER);
    ...
    HTTP_CLIENT_SET_VALUE(creq, ABORT_SIGNAL, make_js_undefined());
    HTTP_CLIENT_SET_VALUE(creq, ABORT_HANDLER, make_js_undefined());
}
...
static Item js_http_client_abort_scheduled(Item env_item) {
    ...
    if (http_emit_event(req_obj, "error", &err, 1)) js_microtask_flush();
    js_http_close_client_req(creq);
    return make_js_undefined();
}
...
static bool js_http_client_configure_abort_signal(JsHttpClientReq* creq, Item signal) {
    if (!creq || !HTTP_CLIENT_VALUE(creq, OBJECT).item) return false;
    TypeId type = get_type_id(signal);
    if (type != LMD_TYPE_MAP && type != LMD_TYPE_VMAP) return false;

    HTTP_CLIENT_SET_VALUE(creq, ABORT_SIGNAL, signal);
    ...
    js_microtask_flush();
    HTTP_CLIENT_SET_VALUE(creq, ABORT_HANDLER, handler);
    creq->abort_handler_set = true;
    return false;
}

static void js_http_agent_clear_idle_client(JsHttpClientReq* creq) {
    Item agent = HTTP_CLIENT_VALUE(creq, AGENT);
    if (!creq || agent.item == 0) return;
    Item key = make_string_item("__lambda_idle_client__");
    Item idle_item = js_get_key_default(agent, key);
    if (get_type_id(idle_item) == LMD_TYPE_INT &&
        (JsHttpClientReq*)(uintptr_t)it2i(idle_item) == creq) {
        js_set_key_default(agent, key, ItemNull);
    }
}

static void js_http_agent_store_idle_client(JsHttpClientReq* creq) {
    Item agent = HTTP_CLIENT_VALUE(creq, AGENT);
    if (!creq || !creq->keep_alive_agent || agent.item == 0 ||
        creq->destroyed || !creq->connected) return;
    // keepAlive agents own one idle native connection here; closing it at
    // response end prevents official reuse tests from ever seeing a hot socket.
    http_client_mark_socket_idle(creq);
    creq->reusable_idle = true;
    js_set_key_cstr(HTTP_CLIENT_VALUE(creq, OBJECT), "__client__", ItemNull);
    js_set_key_cstr(agent, "__lambda_idle_client__",
...
static void js_http_free_client_req(JsHttpClientReq* creq) {
    ...
    js_async_hooks_emit_destroy_resource(HTTP_CLIENT_VALUE(creq, ASYNC_RESOURCE));
    runtime_value_slots_destroy(&creq->values);
...
static void js_http_close_client_req(JsHttpClientReq* creq) {
    ...
    Item request = HTTP_CLIENT_VALUE(creq, OBJECT);
    if (request.item != 0) {
        js_set_key_cstr(request, "__client__", ItemNull);
    }
...
extern "C" Item js_http_agent_socket_error_tick(Item req_obj, Item err) {
    ...
    if (http_emit_event(req_obj, "error", &err, 1)) js_microtask_flush();
    if (http_emit_event(req_obj, "close", NULL, 0)) js_microtask_flush();
...
extern "C" Item js_http_agent_socket_cb(Item req_obj, Item err, Item socket) {
    ...
    js_set_key_cstr(req_obj, "socket", socket);
    return make_js_undefined();
}
...
static void js_http_emit_client_response(JsHttpClientReq* creq, Item res) {
    ...
    Item request = HTTP_CLIENT_VALUE(creq, OBJECT);
    Item callback = HTTP_CLIENT_VALUE(creq, CALLBACK);
    Item als_context = HTTP_CLIENT_VALUE(creq, ALS_CONTEXT);
    Item on_response = http_listener_list(request, "response");
    if (js_is_callable(callback)) {
...
static int js_http_response_content_length(Item headers) {
    ...
    Item value = js_get_key_cstr(headers, "content-length");
...
static void js_http_client_finish_response_if_complete(JsHttpClientReq* creq, Item res,
    ...
    Item status_item = js_get_key_cstr(res, "statusCode");
...
static Item js_http_make_client_response(JsHttpClientReq* creq, int status_code,
        Item status_message, Item headers, Item raw_headers, bool attach_socket) {
    Item response = js_readable_new_with_class(ItemNull, JS_CLASS_INCOMING_MESSAGE);
    HTTP_CLIENT_SET_VALUE(creq, RESPONSE, response);
    js_set_key_cstr(response, "__client__", (Item){.item = i2it((int64_t)(uintptr_t)creq)});
    js_set_key_cstr(response, "statusCode", (Item){.item = i2it(status_code)});
    js_set_key_cstr(response, "statusMessage", status_message);
    js_set_key_cstr(response, "headers", headers);
    js_set_key_cstr(response, "rawHeaders", raw_headers);
    if (attach_socket) {
        Item socket = http_client_socket_object(creq);
        js_set_key_cstr(response, "socket", socket);
        js_set_key_cstr(HTTP_CLIENT_VALUE(creq, OBJECT), "socket", socket);
    }
#define JS_HTTP_INSTALL_CLIENT_RESPONSE_METHOD(name, target) js_http_set_direct_method(response, name, target);
    JS_HTTP_INSTALL_CLIENT_RESPONSE_METHOD("destroy", js_http_client_res_inst_destroy)
    JS_HTTP_INSTALL_CLIENT_RESPONSE_METHOD("on", js_http_client_res_inst_on)
    JS_HTTP_INSTALL_CLIENT_RESPONSE_METHOD("once", js_http_client_res_inst_on)
#undef JS_HTTP_INSTALL_CLIENT_RESPONSE_METHOD
    return response;
}

static bool js_http_client_try_emit_response(JsHttpClientReq* creq) {
    if (!creq || HTTP_CLIENT_VALUE(creq, RESPONSE).item != 0) {
        return creq && HTTP_CLIENT_VALUE(creq, RESPONSE).item != 0;
    }
    ...
    if (status_code == 100) {
        ...
        http_client_emit(HTTP_CLIENT_VALUE(creq, OBJECT), "continue");
        return js_http_client_try_emit_response(creq);
    }

    Item res = js_http_make_client_response(creq, status_code, status_message,
    ...
    if (emitted_body_len > 0) {
        ...
        js_set_key_cstr(res, "__chunks__", body_chunks);
        js_set_key_cstr(res, "body", body);
        js_readable_push(res, body);
    }
    if (decoded_body) mem_free(decoded_body);

    HTTP_CLIENT_SET_VALUE(creq, RESPONSE, res);
    creq->response_hdr_size = hdr_size;
    creq->response_delivered = creq->recv_len;
    creq->response_chunked = chunked;
    creq->response_decoded_delivered = chunked ? decoded_body_len : 0;
...
static void http_client_read_cb(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf) {
    ...
    if (nread > 0) {
        ...
        if (HTTP_CLIENT_VALUE(creq, RESPONSE).item != 0) {
            int body_start = creq->response_hdr_size;
            int prev = creq->response_delivered;
            if (prev < body_start) prev = body_start;
            if (creq->response_chunked) {
                int raw_body_len = creq->recv_len - creq->response_hdr_size;
                if (raw_body_len > 0) {
                    char* decoded_body = (char*)mem_alloc(raw_body_len + 1, MEM_CAT_JS_RUNTIME);
                    int chunked_consumed = 0;
                    bool chunked_complete = false;
                    int decoded_len = http_decode_chunked_body(
                        creq->recv_buf + creq->response_hdr_size, raw_body_len,
                        decoded_body, &chunked_consumed, &chunked_complete);
                    if (decoded_len > creq->response_decoded_delivered) {
                        // Transfer-Encoding frames are transport delimiters;
                        // response streams must expose only newly decoded data.
                js_readable_push(HTTP_CLIENT_VALUE(creq, RESPONSE),
                                         make_string_item(decoded_body + creq->response_decoded_delivered,
                                                          decoded_len - creq->response_decoded_delivered));
                        creq->response_decoded_delivered = decoded_len;
                    }
                    if (chunked_complete && !creq->response_ended) {
                        js_readable_push(HTTP_CLIENT_VALUE(creq, RESPONSE), ItemNull);
                        creq->response_ended = true;
                        if (creq->keep_alive_agent) {
                            js_http_agent_store_idle_client(creq);
                        } else {
                            creq->destroyed = true;
                            js_set_key_cstr(HTTP_CLIENT_VALUE(creq, OBJECT), "destroyed",
                                (Item){.item = b2it(true)});
                    ...
                    mem_free(decoded_body);
                }
                creq->response_delivered = creq->recv_len;
            } else if (creq->recv_len > prev) {
                js_readable_push(HTTP_CLIENT_VALUE(creq, RESPONSE),
                    make_string_item(creq->recv_buf + prev, creq->recv_len - prev));
                creq->response_delivered = creq->recv_len;
                Item response = HTTP_CLIENT_VALUE(creq, RESPONSE);
                Item headers = js_get_key_cstr(response, "headers");
                js_http_client_finish_response_if_complete(creq, response, headers,
        ...
        if (HTTP_CLIENT_VALUE(creq, RESPONSE).item != 0) {
            ...
            if (parse_http_response_head(creq->recv_buf, creq->recv_len,
                ...
                if (body_len > 0) {
                    ...
                    js_set_key_cstr(res, "body", make_string_item(creq->recv_buf + hdr_size, body_len));
            ...
            if (http_emit_event(HTTP_CLIENT_VALUE(creq, OBJECT), "error", &err, 1)) {
                js_microtask_flush();
            }
        }
...
static void http_client_update_pending_body(JsHttpClientReq* creq, Item req_obj) {
    ...
    Item body = js_get_key_cstr(req_obj, "__req_body__");
...
static void http_client_connect_cb(uv_connect_t* req, int status) {
    ...
    if (status != 0) {
        if (creq) {
            ...
            http_emit_event(HTTP_CLIENT_VALUE(creq, OBJECT), "error", &err, 1);
            creq->destroyed = true;
            js_set_key_cstr(HTTP_CLIENT_VALUE(creq, OBJECT), "destroyed",
                (Item){.item = b2it(true)});
            js_http_close_client_req(creq);
...
static Item http_client_write_ex(Item self, Item data_item, Item encoding_item, Item callback_item) {
    ...
    Item body = js_get_key_cstr(self, "__req_body__");
...
static Item http_client_end_ex(Item self, Item data_item, Item encoding_item, Item callback_item) {
    ...
    Item ended = js_get_key_cstr(self, "writableEnded");
...
extern "C" Item js_http_client_destroy(Item self, Item err_item) {
    ...
    http_emit_event(self, "error", &err, 1);
    if (HTTP_CLIENT_VALUE(creq, RESPONSE).item != 0) {
        js_stream_destroy(HTTP_CLIENT_VALUE(creq, RESPONSE), err);
    }
    uv_handle_t* handle = http_client_handle(creq);
    if (handle && !uv_is_closing(handle)) {
        uv_close(handle, [](uv_handle_t* h) {
...
static void http_client_insert_pending_header(JsHttpClientReq* creq, const char* line, int line_len) {
    ...
    http_client_sync_header_property(creq, HTTP_CLIENT_VALUE(creq, OBJECT));
}
...
static Item http_client_make_request_object(JsHttpClientReq* creq,
    ...
    HTTP_CLIENT_SET_VALUE(creq, OBJECT, obj);
    js_set_key_cstr(obj, "__client_request__", (Item){.item = b2it(true)});
    if (creq) {
        js_set_key_cstr(obj, "__client__", (Item){.item = i2it((int64_t)(uintptr_t)creq)});
    }
    js_set_key_cstr(obj, "path", make_string_item((path && path[0]) ? path : "/"));
    js_set_key_cstr(obj, "method", make_string_item((method && method[0]) ? method : "GET"));
#define JS_HTTP_CLIENT_METHODS(M) \
    M("write", js_http_client_inst_write) M("end", js_http_client_inst_end) \
    M("on", js_http_client_inst_on) M("once", js_http_client_inst_on) \
    M("setHeader", js_http_client_inst_setHeader) M("getHeaderNames", js_http_client_inst_getHeaderNames) \
    M("getRawHeaderNames", js_http_client_inst_getRawHeaderNames) M("destroy", js_http_client_inst_destroy)
#define JS_HTTP_INSTALL_CLIENT_METHOD(name, target) js_http_set_direct_method(obj, name, target);
    JS_HTTP_CLIENT_METHODS(JS_HTTP_INSTALL_CLIENT_METHOD)
#undef JS_HTTP_INSTALL_CLIENT_METHOD
#undef JS_HTTP_CLIENT_METHODS
    js_set_key_cstr(obj, "destroyed", (Item){.item = b2it(false)});
    js_set_key_cstr(obj, "writable", (Item){.item = b2it(true)});
    js_set_key_cstr(obj, "writableEnded", (Item){.item = b2it(false)});
    js_set_key_cstr(obj, "writableFinished", (Item){.item = b2it(false)});
    js_set_key_cstr(obj, "__finish_scheduled__", (Item){.item = b2it(false)});
    return obj;
}
        ...
        for (int64_t i = 0; i < len; i++) {
            ...
            if (get_type_id(entry) == LMD_TYPE_ARRAY && js_array_length(entry) >= 2) {
                JS_RETURN_IF_ERROR(http_client_validate_header_pair(
                    js_elements_get_int(entry, 0), js_elements_get_int(entry, 1)));
            } else if (i + 1 < len) {
                JS_RETURN_IF_ERROR(http_client_validate_header_pair(
                    entry, js_elements_get_int(headers_item, i + 1)));
                i++;
            }
        }
    }
...
static int http_client_append_header_line(char* req_str, int rlen, int cap,
    ...
    if (get_type_id(value_item) == LMD_TYPE_ARRAY) {
        ...
        if (http_header_name_equals(name, "cookie", 6)) {
            ...
            int pos = 0;
            for (int64_t i = 0; i < len; i++) {
                Item part = http_item_to_header_string(js_elements_get_int(value_item, i));
                if (get_type_id(part) != LMD_TYPE_STRING) continue;
                String* ps = it2s(part);
        ...
        for (int64_t i = 0; i < len; i++) {
            rlen = http_client_append_header_line(req_str, rlen, cap,
                                                  name_item, js_elements_get_int(value_item, i),
                                                  has_content_length);
        }
        return rlen;
    }
...
static int http_client_append_headers(char* req_str, int rlen, int cap,
    ...
    if (get_type_id(headers_item) == LMD_TYPE_MAP) {
        ...
        for (int64_t i = 0; i < len; i++) {
            ...
            if (get_type_id(entry) == LMD_TYPE_ARRAY && js_array_length(entry) >= 2) {
                rlen = http_client_append_header_line(req_str, rlen, cap,
                                                      js_elements_get_int(entry, 0),
                                                      js_elements_get_int(entry, 1),
                                                      has_content_length);
            } else if (i + 1 < len) {
                rlen = http_client_append_header_line(req_str, rlen, cap,
                                                      entry,
                                                      js_elements_get_int(headers_item, i + 1),
                                                      has_content_length);
                i++;
            }
        }
...
extern "C" Item js_http_request(Item options_item, Item callback) {
    ...
    if (get_type_id(options_item) == LMD_TYPE_MAP) {
        Item spath = js_get_key_cstr(options_item, "socketPath");
        ...
        Item h = js_get_key_cstr(options_item, "hostname");
        if (get_type_id(h) != LMD_TYPE_STRING)
            h = js_get_key_cstr(options_item, "host");
        if (get_type_id(h) == LMD_TYPE_STRING) {
            String* hs = it2s(h);
        ...
        Item pa = js_get_key_cstr(options_item, "path");
    ...
    if (get_type_id(options_item) == LMD_TYPE_MAP) {
        ...
        Item sdh = js_get_key_cstr(options_item, "setDefaultHeaders");
        if (get_type_id(sdh) == LMD_TYPE_BOOL) set_default_headers = it2b(sdh);
        Item sh = js_get_key_cstr(options_item, "setHost");
        if (get_type_id(sh) == LMD_TYPE_BOOL) {
            set_host = it2b(sh);
            explicit_set_host = true;
        }
        if (!set_default_headers && !explicit_set_host) set_host = false;
        custom_headers = js_get_key_cstr(options_item, "headers");
        auth_item = js_get_key_cstr(options_item, "auth");
        signal_item = js_get_key_cstr(options_item, "signal");
        ...
        if (auth_type == LMD_TYPE_UNDEFINED || auth_type == LMD_TYPE_NULL) {
            auth_item = http_client_auth_from_url_parts(
                js_get_key_cstr(options_item, "username"),
                js_get_key_cstr(options_item, "password"));
        }
    }
    ...
    bool method_allows_body = strcmp(method_buf, "GET") != 0 && strcmp(method_buf, "HEAD") != 0;
    bool use_chunked_request_body = !explicit_content_length &&
        (has_expect_header || has_transfer_encoding || method_allows_body);
    ...
    if (use_chunked_request_body && !has_transfer_encoding) {
        // Reused sockets can write the body after headers are already sent;
        // unknown-length request bodies need chunk framing in that state.
        rlen += snprintf(req_str + rlen, sizeof(req_str) - rlen, "Transfer-Encoding: chunked\r\n");
    }

    rlen += snprintf(req_str + rlen, sizeof(req_str) - rlen, "\r\n");

    Item request_agent = make_js_undefined();
    bool request_keep_alive = false;
    JsHttpClientReq* idle_creq = NULL;
    if (get_type_id(options_item) == LMD_TYPE_MAP && !use_pipe) {
        request_agent = js_get_key_cstr(options_item, "agent");
        if (get_type_id(request_agent) == LMD_TYPE_MAP) {
            Item keep_alive = js_get_key_cstr(request_agent, "keepAlive");
            request_keep_alive = get_type_id(keep_alive) == LMD_TYPE_BOOL && it2b(keep_alive);
            Item idle_item = js_get_key_cstr(request_agent, "__lambda_idle_client__");
            if (request_keep_alive && get_type_id(idle_item) == LMD_TYPE_INT) {
                idle_creq = (JsHttpClientReq*)(uintptr_t)it2i(idle_item);
                if (!idle_creq || idle_creq->destroyed || !idle_creq->reusable_idle) {
                    idle_creq = NULL;
                    js_set_key_cstr(request_agent, "__lambda_idle_client__", ItemNull);
                }
            }
        }
    }
    ...
    JsHttpClientReq* creq = idle_creq;
    bool reused_idle_client = creq != NULL;
    if (reused_idle_client) {
        js_set_key_cstr(request_agent, "__lambda_idle_client__", ItemNull);
        // Reused keep-alive sockets keep the uv handle but must forget the
        // previous response parser state before the next request is written.
        creq->reusable_idle = false;
        HTTP_CLIENT_SET_VALUE(creq, OBJECT, ItemNull);
        HTTP_CLIENT_SET_VALUE(creq, CALLBACK, ItemNull);
        HTTP_CLIENT_SET_VALUE(creq, ALS_CONTEXT, ItemNull);
        HTTP_CLIENT_SET_VALUE(creq, ASYNC_RESOURCE, ItemNull);
        HTTP_CLIENT_SET_VALUE(creq, AGENT, ItemNull);
        HTTP_CLIENT_SET_VALUE(creq, RESPONSE, ItemNull);
        HTTP_CLIENT_SET_VALUE(creq, ABORT_SIGNAL, ItemNull);
        HTTP_CLIENT_SET_VALUE(creq, ABORT_HANDLER, ItemNull);
        creq->recv_len = 0;
        creq->response_hdr_size = 0;
        creq->response_delivered = 0;
        creq->response_decoded_delivered = 0;
        creq->response_emitted = false;
        creq->response_ended = false;
        creq->response_chunked = false;
        creq->sent = false;
        creq->end_called = false;
        creq->close_after_send = false;
        creq->abort_handler_set = false;
        creq->abort_scheduled = false;
    ...
    HTTP_CLIENT_SET_VALUE(creq, CALLBACK, callback);
    HTTP_CLIENT_SET_VALUE(creq, ALS_CONTEXT, js_als_capture_context());
    HTTP_CLIENT_SET_VALUE(creq, ASYNC_RESOURCE,
        js_async_hooks_create_resource("HTTPCLIENTREQUEST", 17));
    HTTP_CLIENT_SET_VALUE(creq, AGENT, request_agent);
    creq->keep_alive_agent = request_keep_alive;
    if (!creq->recv_buf) {
        creq->recv_cap = 16384;
        creq->recv_buf = (char*)mem_alloc(creq->recv_cap, MEM_CAT_JS_RUNTIME);
    }
    ...
    if (reused_idle_client) http_client_socket_object(creq);
    ...
    if (get_type_id(options_item) == LMD_TYPE_MAP) {
        Item agent = js_get_key_cstr(options_item, "agent");
        if (get_type_id(agent) == LMD_TYPE_MAP) {
            Item create_socket = js_get_key_cstr(agent, "createSocket");
    ...
    if (r != 0) {
        ...
        mem_free(conn);
        creq->destroyed = true;
        js_set_key_cstr(obj, "destroyed", (Item){.item = b2it(true)});
        js_http_close_client_req(creq);
        return ItemNull;
    }
...
extern "C" Item js_http_get(Item options_item, Item callback) {
    ...
    if (get_type_id(options_item) == LMD_TYPE_MAP) {
        js_set_key_cstr(options_item, "method", make_string_item("GET"));
    }
...
extern "C" Item js_http_agent_getName(Item options) {
    ...
    if (get_type_id(options) == LMD_TYPE_MAP) {
        Item h = js_get_key_cstr(options, "host");
        if (get_type_id(h) == LMD_TYPE_STRING) {
            String* s = it2s(h);
            int len = (int)s->len < 255 ? (int)s->len : 255;
        ...
        Item sp = js_get_key_cstr(options, "socketPath");
...
extern "C" Item js_http_agent_destroy(void) {
    ...
    Item self = js_get_this();
    js_set_key_cstr(self, "sockets", js_new_object());
    js_set_key_cstr(self, "freeSockets", js_new_object());
    js_set_key_cstr(self, "requests", js_new_object());
    return ItemNull;
}
...
static void js_http_agent_assign_socket(Item request, Item socket) {
    Item on_socket = js_get_key_cstr(request, "onSocket");
...
extern "C" Item js_http_agent_addRequest(Item request, Item options,
    ...
    if (get_type_id(options_root.get()) == LMD_TYPE_STRING) {
        ...
        js_set_key_cstr(normalized_root.get(), "port", port_root.get());
        js_set_key_cstr(normalized_root.get(), "localAddress", local_address_root.get());
    } else {
        agent_options_root.set(js_get_key_cstr(agent_root.get(), "options"));
    ...
    Item get_name = js_get_key_cstr(agent_root.get(), "getName");
    ...
    Item sockets = js_get_key_cstr(agent_root.get(), "sockets");
    if (get_type_id(sockets) != LMD_TYPE_MAP) {
        sockets = js_new_object();
        js_set_key_cstr(agent_root.get(), "sockets", sockets);
    }
    sockets_root.set(sockets);
    Item active = js_get_key_default(sockets_root.get(), name_root.get());
    if (get_type_id(active) != LMD_TYPE_ARRAY) {
        active = js_array_new(0);
        js_set_key_default(sockets_root.get(), name_root.get(), active);
    }
    ...
    if (get_type_id(socket_root.get()) == LMD_TYPE_UNDEFINED) {
        Item max_sockets = js_get_key_cstr(agent_root.get(), "maxSockets");
        ...
        Item create_connection = js_get_key_cstr(agent_root.get(), "createConnection");
...
#define JS_HTTP_AGENT_METHODS(M) \
    M("getName", js_http_agent_getName) M("destroy", js_http_agent_destroy) \
    M("createConnection", js_http_agent_createConnection) M("addRequest", js_http_agent_addRequest)
#define JS_HTTP_AGENT_INSTALL(name, target) \
    js_http_set_direct_method(agent, name, target);
    JS_HTTP_AGENT_METHODS(JS_HTTP_AGENT_INSTALL)
#undef JS_HTTP_AGENT_INSTALL
#undef JS_HTTP_AGENT_METHODS
}
...
extern "C" Item js_http_Agent(Item options) {
    ...
    if (get_type_id(options) == LMD_TYPE_MAP) {
        Item ms = js_get_key_cstr(options, "maxSockets");
        int64_t ms_value = 0;
        if (http_item_to_integral_int64(ms, &ms_value)) max_sockets = (int)ms_value;
        Item ka = js_get_key_cstr(options, "keepAlive");
        if (get_type_id(ka) == LMD_TYPE_BOOL) keep_alive = it2b(ka);
        Item kam = js_get_key_cstr(options, "keepAliveMsecs");
        int64_t kam_value = 0;
        if (http_item_to_integral_int64(kam, &kam_value)) keep_alive_msecs = (int)kam_value;
    }

    js_set_key_cstr(agent, "maxSockets", (Item){.item = i2it(max_sockets)});
    js_set_key_cstr(agent, "keepAlive", (Item){.item = b2it(keep_alive)});
    js_set_key_cstr(agent, "keepAliveMsecs", (Item){.item = i2it(keep_alive_msecs)});
    ...
    js_set_key_cstr(agent, "maxFreeSockets", (Item){.item = i2it(256)});
    js_set_key_cstr(agent, "options", options);
...
extern "C" Item js_get_http_namespace(void) {
    HttpRealmItems items = {};
    if (!http_realm_items(&items, true)) return ItemError;
    Item& http_namespace = *items.namespace_object;
    Item& http_server_prototype = *items.server_prototype;
    Item& http_incoming_message_prototype = *items.incoming_message_prototype;
    Item& http_server_response_prototype = *items.server_response_prototype;
    Item& http_outgoing_message_prototype = *items.outgoing_message_prototype;
    ...
    JS_ROOTS(server_roots, server_fn_root,
        js_new_native_constructor(js_http_createServer));
    js_set_key_cstr(http_namespace, "Server", server_fn_root.get());
    http_server_prototype = http_constructor_prototype(server_fn_root.get(),
        JS_CLASS_SERVER);
    ...
    JS_ROOTS(constructors_roots,
        incoming_fn_root, js_new_distinct_native_constructor(js_http_stub_ctor),
        outgoing_fn_root, js_new_distinct_native_constructor(js_http_stub_ctor),
        response_fn_root, js_new_distinct_native_constructor(js_http_stub_ctor));
    js_set_key_cstr(http_namespace, "IncomingMessage", incoming_fn_root.get());
    http_incoming_message_prototype =
        http_constructor_prototype(incoming_fn_root.get(), JS_CLASS_INCOMING_MESSAGE);
    js_set_key_cstr(http_namespace, "OutgoingMessage", outgoing_fn_root.get());
    http_outgoing_message_prototype =
        http_constructor_prototype(outgoing_fn_root.get(), JS_CLASS_OBJECT);
    js_set_key_cstr(http_namespace, "ServerResponse", response_fn_root.get());
    http_server_response_prototype =
        http_constructor_prototype(response_fn_root.get(), JS_CLASS_SERVER_RESPONSE);
    ...
    js_set_key_default(http_namespace, default_key, http_namespace);
...
extern "C" void js_http_reset(void) {
    ...
    HttpRealmItems items = {};
    if (!http_realm_items(&items, false)) return;
    *items.namespace_object = (Item){0};
    *items.server_prototype = (Item){0};
    *items.incoming_message_prototype = (Item){0};
    *items.server_response_prototype = (Item){0};
    *items.outgoing_message_prototype = (Item){0};
}
