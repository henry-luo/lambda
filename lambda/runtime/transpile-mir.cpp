...
struct MirTranspiler {
    ...
    String* compact_loop_sub_lhs;
    String* compact_loop_sub_rhs;
    String* compact_loop_add_lhs;
...
static TypeId resolve_declared_param_type(AstNamedNode* param, TypeParam* type_param) {
    ...
    // The global two-pass builder may retain a compact TypeFunc parameter
    // chain from its declaration pass.  The parameter node is the authority
    // for optional/default spelling, so do not accidentally native-unbox a
    // missing `a?: int` into zero in the raw body.
    TypeParam* declared_param = param->type ? (TypeParam*)param->type : type_param;
    bool may_be_null = declared_param && declared_param->is_optional &&
        !declared_param->default_value;
...
// G0: `int` has one native lane, the IEEE double, but transpile_expr hands back
...
static MIR_reg_t emit_text_pointer_lane(MirTranspiler* mt, MIR_reg_t value,
        TypeId type_id) {
...
static bool mir_argument_may_return_item_error(AstNode* argument) {
    return argument && argument->type && lambda_type_accepts_error(argument->type);
}
...
static bool mir_is_type_definition_binding(AstNode* node) {
    return node && node->node_type == AST_NODE_ASSIGN &&
        ((AstNamedNode*)node)->is_type_definition;
}
...
static MIR_reg_t transpile_ident(MirTranspiler* mt, AstIdentNode* ident) {
    ...
    if (var) {
        if (var->string_buffer_owned) {
            // A normal identifier read can create an alias or cross an
            // unknown call boundary. Freeze before publishing the local buffer.
            MIR_reg_t frozen = emit_call_1(mt, "fn_string_freeze", MIR_T_P,
                MIR_T_P, MIR_new_reg_op(mt->ctx, var->reg));
            emit_insn(mt, MIR_new_insn(mt->ctx, MIR_MOV,
                MIR_new_reg_op(mt->ctx, var->reg), MIR_new_reg_op(mt->ctx, frozen)));
            var->string_buffer_owned = false;
            update_gc_root_slot(mt, var);
        }
    ...
    if (ident->entry && ident->entry->import) {
        ...
        if (mir_is_type_definition_binding(entry_node) && entry_node->type) {
            ...
            return mir_emit_declared_type_value(mt, entry_node->type);
        }
...
static inline MIR_reg_t mir_emit_as_double(MirTranspiler* mt, MIR_reg_t reg, TypeId tid) {
    ...
    // uint64, bool, sized) still need a conversion.
    if (type_to_mir(tid) == MIR_T_D) return reg;
...
static TypeId get_effective_type(MirTranspiler* mt, AstNode* node) {
    ...
    if (node->node_type == AST_NODE_CALL_EXPR) {
        ...
        TypeFunc* direct_signature = direct_callee && direct_callee->type &&
            ...
            ? (TypeFunc*)direct_callee->type : NULL;
        if (direct_signature && direct_signature->is_proc) {
            // pn calls keep the boxed Item ABI. Their inferred result supports
            // static boundaries, but using it as a raw MIR carrier changes
            // arithmetic before the caller has crossed an explicit boundary.
            return LMD_TYPE_ANY;
        }
        ...
        if (tid == LMD_TYPE_TYPE && node->type &&
                (node->type->kind == TYPE_KIND_BINARY ||
                 node->type == &TYPE_INTEGER || node->type == &TYPE_NUMBER)) {
            // Semantic unions and abstract numeric contracts are Item-valued
            // call results, not runtime `type` objects. Preserve their ledger
            // information on the AST while lowering through the boxed lane.
            return LMD_TYPE_ANY;
        }
        if (call->function && call->function->node_type == AST_NODE_SYS_FUNC) {
            AstSysFuncNode* sys = (AstSysFuncNode*)call->function;
            SysFuncInfo* info = sys->fn_info;
            // The registry records these math builtins as `any` because their
            // general runtime form is element-wise over containers. When every
            // argument is already a native scalar, their scalar branch is
            ...
            if (info && info->native_c_name && info->native_func_ptr &&
                ...
                AstNode* first = call->argument;
                AstNode* second = first ? first->next : NULL;
                bool arity_matches = info->native_arg_count == 1
                    ? first && !second
                    : first && second && !second->next;
                if (arity_matches && mir_native_math_arg_type(get_effective_type(mt, first)) &&
                        (info->native_arg_count == 1 ||
                         mir_native_math_arg_type(get_effective_type(mt, second)))) {
        ...
        if (!idx_is_type && !idx_is_mask) {
            ...
            if (obj_unwrapped && obj_unwrapped->node_type == AST_NODE_IDENT) {
                ...
                if (ov && ov->elem_type != LMD_TYPE_ANY &&
                    ...
                    // G0: an int element load is now exactly the case the note
                    // above warns about. The read joins in ItemNull when the
                    // index is out of range, and int's double lane cannot hold
                    // it, so the load is boxed -- promising INT here is what
                    // made the consumer box an already-boxed Item.
                    if (ov->elem_type == LMD_TYPE_INT) return LMD_TYPE_ANY;
                    return ov->elem_type;
                }
            }
        }
    ...
    if (node->node_type == AST_NODE_BINARY) {
        ...
        // C14c routes `div`/`%` through helpers whose result can be float or
        // decimal. Keep their self-describing Item at expression boundaries;
        // a typed declaration, parameter, or explicit conversion owns the
        // corresponding checked unbox.
        ...
        // Type-set narrowing for `or` is semantic evidence for later checked
        // boundaries. Its runtime implementation still selects boxed Items, so
        // only the all-bool fast path may advertise a native register result.
        if (bi->op == OPERATOR_AND || bi->op == OPERATOR_OR) {
            return (lt == LMD_TYPE_BOOL && rt == LMD_TYPE_BOOL)
                ? LMD_TYPE_BOOL : LMD_TYPE_ANY;
        }
    ...
    // An INDEXED or MEMBER assignment does NOT return its RHS: its emitters
    // hand back the assignment result -- a COW replacement container, a
    // checked-setter Item, a null. Inheriting the RHS type from the AST made
    // callers read those Items as native values of the RHS's type.
    if (node->node_type == AST_NODE_INDEX_ASSIGN_STAM ||
            node->node_type == AST_NODE_MEMBER_ASSIGN_STAM) {
        return LMD_TYPE_ANY;
    }
    // A `return` transfers control; the register left behind is unreachable
    // filler, not the returned expression. Inheriting the expression's type
    // promised a native lane for a value no one can legitimately read.
    if (node->node_type == AST_NODE_RETURN_STAM) return LMD_TYPE_ANY;
    ...
    if (node->node_type == AST_NODE_INDEX_EXPR) {
        ...
        if (obj_eff == LMD_TYPE_ARRAY_NUM && is_integer_type_id(idx_eff)) {
            ...
            if (obj_unwrapped && obj_unwrapped->node_type == AST_NODE_IDENT) {
                ...
                MirVarEntry* ov = find_var(mt, oname);
                if (ov && ov->elem_type != LMD_TYPE_ANY) {
                    // Same join as the N-D case above: an out-of-range read
                    // yields ItemNull, so the subscript's type is `elem | null`
                    // rather than `elem`. The retired i64 lane could carry the
                    // null's bits inside a "native int" register and hid this;
                    // int's double lane cannot, so the type has to say so or
                    // callers re-box an Item that is already boxed. FLOAT keeps
                    // its long-standing native read (OOB yields 0.0 there).
                    if (ov->elem_type == LMD_TYPE_INT) return LMD_TYPE_ANY;
                    return ov->elem_type;
                }
            }
        ...
        //
        // That was stated here but never returned, so the node kept its AST
        // element type and the tail below handed back INT. Under the retired
        // i64 lane both the boxed and the "native int" answer lived in an i64
        // register and nothing noticed; with int in the double lane the caller
        // boxed an already-boxed Item. An out-of-range read also joins in
        // ItemNull, which the double lane cannot represent -- the same reason
        // the N-D case above is ANY.
...
static MIR_reg_t mir_emit_u32_binary(MirTranspiler* mt, AstBinaryNode* binary) {
    MIR_reg_t left_item = transpile_expr(mt, binary->left);
    MIR_reg_t right_item = transpile_expr(mt, binary->right);
...
static MIR_reg_t mir_unbox_exact_native_numeric_result(
    ...
    if (binary->op == OPERATOR_IDIV || binary->op == OPERATOR_MOD) {
        // C14c's number-domain helpers return a tagged Item. AST inference can
        // still describe a nested operand as int even when its MIR value was
        // promoted, so unboxing here can pass a double through the Item ABI.
        return boxed;
    }
    LambdaNumericDecision decision;
...
static MIR_reg_t mir_emit_string_concat_item(MirTranspiler* mt, MIR_reg_t left_ptr,
                                              AstNode* right_expr) {
    MIR_reg_t left_item = emit_box_string(mt, left_ptr);
    int left_root = create_gc_root_slot(mt, left_item);
    MIR_reg_t right_item = transpile_box_item(mt, right_expr);
    int right_root = create_gc_root_slot(mt, right_item);
    left_item = load_gc_root_slot(mt, left_root, "strcat_item_l");
    right_item = load_gc_root_slot(mt, right_root, "strcat_item_r");
    left_ptr = emit_unbox(mt, left_item, LMD_TYPE_STRING);
    MIR_reg_t right_ptr = emit_call_1(mt, "fn_string", MIR_T_P,
        MIR_T_I64, MIR_new_reg_op(mt->ctx, right_item));
    MIR_reg_t right_string_item = emit_box_string(mt, right_ptr);
    int right_string_root = create_gc_root_slot(mt, right_string_item);
    right_string_item = load_gc_root_slot(mt, right_string_root, "strcat_item_rs");
    right_ptr = emit_unbox(mt, right_string_item, LMD_TYPE_STRING);
    ...
    MIR_reg_t result_item = emit_box_string(mt, result_ptr);
    int result_root = create_gc_root_slot(mt, result_item);
    result_item = load_gc_root_slot(mt, result_root, "strcat_item_rv");
    return emit_unbox(mt, result_item, LMD_TYPE_STRING);
}
...
static bool mir_matches_compact_loop_sub(MirTranspiler* mt, AstBinaryNode* binary) {
    ...
    return left_ident->name->len == mt->compact_loop_sub_lhs->len &&
        right_ident->name->len == mt->compact_loop_sub_rhs->len &&
        memcmp(left_ident->name->chars, mt->compact_loop_sub_lhs->chars,
            left_ident->name->len) == 0 &&
        memcmp(right_ident->name->chars, mt->compact_loop_sub_rhs->chars,
            right_ident->name->len) == 0;
}
...
static bool mir_matches_compact_loop_add(MirTranspiler* mt, AstBinaryNode* binary) {
    ...
    return left_ident->name->len == mt->compact_loop_add_lhs->len &&
        memcmp(left_ident->name->chars, mt->compact_loop_add_lhs->chars,
...
static MIR_reg_t emit_machine_count(MirTranspiler* mt, AstNode* node) {
    ...
    if (!is_integer_type_id(tid)) {
        val = emit_unbox(mt, emit_box(mt, val, tid), LMD_TYPE_INT);
        tid = LMD_TYPE_INT;
    }
    return emit_machine_index(mt, val, tid);
}
    ...
    // ======================================================================
    {
        ...
        if ((both_string || both_symbol) &&
            ...
            MIR_reg_t left = emit_text_pointer_lane(mt,
                transpile_expr(mt, bi->left), text_type);
            MIR_reg_t right = emit_text_pointer_lane(mt,
                transpile_expr(mt, bi->right), text_type);
...
static MIR_reg_t transpile_if(MirTranspiler* mt, AstIfNode* if_node) {
    ...
    if (if_node->then) {
        ...
        if (mt->block_returned) {
            ...
            MIR_reg_t src = emit_scalar_native_lane(mt, then_val, then_tid);
            if (MIR_reg_type(mt->ctx, src, mt->em.func) != MIR_T_D) {
                src = emit_unbox(mt, src, if_tid == LMD_TYPE_INT ? LMD_TYPE_INT : LMD_TYPE_FLOAT);
            }
            emit_insn(mt, MIR_new_insn(mt->ctx, MIR_DMOV, MIR_new_reg_op(mt->ctx, result),
                MIR_new_reg_op(mt->ctx, src)));
        } else {
            MIR_reg_t src = then_val;
            if (MIR_reg_type(mt->ctx, src, mt->em.func) == MIR_T_D) {
                src = emit_box(mt, src, then_tid);
            }
            emit_insn(mt, MIR_new_insn(mt->ctx, MIR_MOV, MIR_new_reg_op(mt->ctx, result),
                MIR_new_reg_op(mt->ctx, src)));
        }
    ...
    if (if_node->otherwise) {
        ...
        if (mt->block_returned) {
            ...
            // i64 placeholder a value-less statement block yields. Coerce, or
            // MIR rejects the branch move.
            MIR_reg_t src = emit_scalar_native_lane(mt, else_val, else_tid);
            if (MIR_reg_type(mt->ctx, src, mt->em.func) != MIR_T_D) {
                src = emit_unbox(mt, src, if_tid == LMD_TYPE_INT ? LMD_TYPE_INT : LMD_TYPE_FLOAT);
            }
            emit_insn(mt, MIR_new_insn(mt->ctx, MIR_DMOV, MIR_new_reg_op(mt->ctx, result),
            ...
            MIR_reg_t src = else_val;
            if (MIR_reg_type(mt->ctx, src, mt->em.func) == MIR_T_D) {
                src = emit_box(mt, src, else_tid);
            }
            emit_insn(mt, MIR_new_insn(mt->ctx, MIR_MOV, MIR_new_reg_op(mt->ctx, result),
                MIR_new_reg_op(mt->ctx, src)));
        }
...
static MIR_reg_t transpile_for(MirTranspiler* mt, AstForNode* for_node) {
    ...
    MIR_type_t val_mtype = type_to_mir(LMD_TYPE_INT);
    MIR_reg_t val_reg = 0;
    if (counted_range) {
...
static bool mir_string_names_equal(const String* left, const String* right) {
    return left && right && left->len == right->len &&
        memcmp(left->chars, right->chars, left->len) == 0;
}
...
static bool mir_positive_sub_loop_candidate(MirTranspiler* mt,
    ...
    String* rhs_name = ((AstIdentNode*)right)->name;
    if (!lhs_name || !rhs_name || mir_string_names_equal(lhs_name, rhs_name)) return false;

    char lhs_buf[128];
    char rhs_buf[128];
    snprintf(lhs_buf, sizeof(lhs_buf), "%.*s", (int)lhs_name->len, lhs_name->chars);
    snprintf(rhs_buf, sizeof(rhs_buf), "%.*s", (int)rhs_name->len, rhs_name->chars);
    MirVarEntry* lhs_var = find_var(mt, lhs_buf);
    MirVarEntry* rhs_var = find_var(mt, rhs_buf);
    ...
    for (AstNode* item = body->item; item; item = item->next) {
        ...
        if (item->node_type == AST_NODE_IF_EXPR || item->node_type == AST_NODE_WHILE_STAM ||
                item->node_type == AST_NODE_FOR_STAM || item->node_type == AST_NODE_RETURN_STAM ||
        ...
        if (mir_string_names_equal(assign->target, lhs_name)) {
            ...
            if (subtract->op != OPERATOR_SUB ||
                    !mir_node_is_ident_named(subtract->left, lhs_name) ||
                    !mir_node_is_ident_named(subtract->right, rhs_name)) {
                return false;
            }
            lhs_subtract_count++;
            continue;
        ...
        if (value && value->node_type == AST_NODE_BINARY) {
            ...
            if (add->op == OPERATOR_ADD &&
                    mir_node_is_ident_named(add->left, assign->target) &&
                    mir_is_one_int_literal(mt, add->right)) {
                if (counter_name && !mir_string_names_equal(counter_name, assign->target)) {
                    return false;
                }
                counter_name = assign->target;
            }
        }
    }
    ...
    if (counter_name) {
        ...
        snprintf(counter_buf, sizeof(counter_buf), "%.*s", (int)counter_name->len,
            counter_name->chars);
        MirVarEntry* counter_var = find_var(mt, counter_buf);
        if (!counter_var || counter_var->type_id != LMD_TYPE_INT ||
            ...
            for (AstNode* item = body->item; item; item = item->next) {
                ...
                AstBinaryNode* add = (AstBinaryNode*)value;
                if (add->op != OPERATOR_ADD ||
                        !mir_node_is_ident_named(add->left, counter_name) ||
                        !mir_is_one_int_literal(mt, add->right)) {
                    counter_name = NULL;
                    break;
                }
            ...
            if (counter_writes != 1) counter_name = NULL;
        }
    }
    *out_lhs = lhs_name;
    *out_rhs = rhs_name;
...
static MIR_reg_t transpile_while_core(MirTranspiler* mt, AstWhileNode* while_node,
        String* compact_lhs, String* compact_rhs, String* compact_counter) {
    String* saved_compact_lhs = mt->compact_loop_sub_lhs;
    String* saved_compact_rhs = mt->compact_loop_sub_rhs;
    String* saved_compact_counter = mt->compact_loop_add_lhs;
...
static MIR_reg_t transpile_while(MirTranspiler* mt, AstWhileNode* while_node) {
    String* lhs_name = NULL;
    String* rhs_name = NULL;
    String* counter_name = NULL;
    if (!mir_positive_sub_loop_candidate(mt, while_node, &lhs_name, &rhs_name,
            &counter_name)) {
        return transpile_while_core(mt, while_node, NULL, NULL, NULL);
    }

    // A positive decrement makes lhs - rhs safe only on that dynamic branch.
    // The y <= 0 lane keeps the canonical flexible-int lowering unchanged.
    char rhs_buf[128];
    snprintf(rhs_buf, sizeof(rhs_buf), "%.*s", (int)rhs_name->len, rhs_name->chars);
    MirVarEntry* rhs_var = find_var(mt, rhs_buf);
    ...
    emit_insn(mt, MIR_new_insn(mt->ctx, MIR_BT, MIR_new_label_op(mt->ctx, l_fast),
        MIR_new_reg_op(mt->ctx, positive)));
    emit_insn(mt, MIR_new_insn(mt->ctx, MIR_JMP, MIR_new_label_op(mt->ctx, l_generic)));

    emit_label(mt, l_fast);
    transpile_while_core(mt, while_node, lhs_name, rhs_name, counter_name);
    emit_insn(mt, MIR_new_insn(mt->ctx, MIR_JMP, MIR_new_label_op(mt->ctx, l_end)));
...
static MIR_reg_t transpile_local_fault_expression(MirTranspiler* mt,
        AstNode* expression) {
...

    {
        ...
        MIR_reg_t value = transpile_expr(mt, expression);
        mt->preserve_proc_if_result = saved_preserve_proc_if_result;
        MIR_reg_t boxed_value = emit_box(mt, value,
            get_effective_type(mt, expression));
        emit_insn(mt, MIR_new_insn(mt->ctx, MIR_MOV,
...
static void transpile_error_destructure(MirTranspiler* mt, AstNamedNode* asn) {
    ...
    MIR_reg_t boxed = local_fault_boundary
        ? transpile_local_fault_expression(mt, asn->as)
        : transpile_box_item(mt, asn->as);
    MIR_reg_t type_id = emit_item_tag(mt, boxed);
        ...
        if (declare->node_type == AST_NODE_ASSIGN) {
            ...
            if (asn->as) {
                ...
                if (typed_array_element_type) {
                    ...
                    // ensure_typed_array returns the raw ArrayNum pointer used by
                    // typed parameters too. Keep that carrier after the checked
                    // declaration boundary; treating it as any makes a declared
                    // float[] read return a boxed Item to a native double call.
                    var_tid = LMD_TYPE_ARRAY_NUM;
                }
                ...
                if (declared_value_type && declared_value_type->type_id == LMD_TYPE_NUM_SIZED) {
                    ...
                    // G0/E4: int and float share the native lane, so widening an
                    // int to float is the identity here — no instruction at all.
                ...
                // an `int` initializer for a `float` binding (they share the
                // lane). Coerce before the move instead of assuming they agree.
                if (mtype == MIR_T_D) {
                    MIR_reg_t src = emit_scalar_native_lane(mt, val, expr_tid);
                    if (MIR_reg_type(mt->ctx, src, mt->em.func) != MIR_T_D) {
                        src = emit_unbox(mt, src, var_tid);
                    }
                    emit_insn(mt, MIR_new_insn(mt->ctx, MIR_DMOV,
                ...
                if (declared_var) {
                    ...
                    // The compact TypeId selects a local carrier; full_type is
                    // the binding contract used by later map/array writes.
                    declared_var->full_type = declared_value_type;
                }
                if (declared_var && typed_array_element_type &&
                        !(mt->func_body && has_elem_type_invalidation(name_buf,
                            mt->func_body, typed_array_element_type->type_id))) {
                    // A declared array is checked at its binding boundary, but
                    // aliases can still change its physical representation later.
                    // Keep the element witness guarded so indexed native loads
                    // verify the ArrayNum layout before reading raw lanes.
...
static MIR_reg_t transpile_array(MirTranspiler* mt, AstArrayNode* arr_node) {
    ...
    for (AstNode* item = arr_node->item; item; item = item->next) {
        TypeId item_type = get_effective_type(mt, item);
    ...
    bool is_int_array = !has_dynamic_item && arr_type && arr_type->nested &&
        arr_type->nested->type_id == LMD_TYPE_INT;
    bool is_float_array = !has_dynamic_item && arr_type && arr_type->nested &&
        arr_type->nested->type_id == LMD_TYPE_FLOAT;
    ...
    if (is_float_array && !any_spread && !has_let && arr_node->item) {
        ...
        while (item) {
            ...
            if (val_tid == LMD_TYPE_ANY) {
                ...
                val = mir_emit_as_double(mt, val, val_tid);
            }
...
static MIR_reg_t transpile_map(MirTranspiler* mt, AstMapNode* map_node) {
            ...
            while (item && field) {
                ...
                if (is_null_literal || field_type == LMD_TYPE_NULL) {
                    ...
                    if (val_tid == LMD_TYPE_FLOAT) {
                        ...
                        val = mir_emit_as_double(mt, transpile_expr(mt, value_node), val_tid);
                    } else {
    ...
    if (mt->region_capability_reg) {
        m = emit_call_3(mt, "map_with_region_tl", MIR_T_P,
            MIR_T_P, MIR_new_reg_op(mt->ctx, mt->region_capability_reg),
            MIR_T_I64, MIR_new_int_op(mt->ctx, type_index),
            MIR_T_P, MIR_new_reg_op(mt->ctx, emit_load_module_type_list(mt)));
    } else {
...
static MIR_reg_t emit_mir_direct_field_read(MirTranspiler* mt, MIR_reg_t obj_boxed,
    ...
    // C16/G0: `int` and `float` share one native lane and one field storage
...
static MIR_reg_t emit_checked_index_load(MirTranspiler* mt, MIR_reg_t arr_ptr,
    ...
    // G0: an out-of-bounds read has to yield ItemNull, and a double register
    // cannot carry it. The retired i64 lane could smuggle the null's bits
    // through a "native int" register; the double lane cannot, so a load whose
    // OOB answer is null delivers a boxed result instead of a native one.
    if (policy.oob_kind == MIR_INDEX_OOB_ITEM_NULL &&
            policy.result_kind == MIR_INDEX_RESULT_NATIVE_INT_FROM_DOUBLE) {
        policy.result_kind = MIR_INDEX_RESULT_BOXED_INT_FROM_DOUBLE;
    }
    // G0: a "native int" result is a double now, so the result register class
    // follows the lane, not the name.
    ...
    emit_label(mt, l_oob);
    // G0: a native-lane result cannot carry ItemNull — it is a double now, as
    // the float lane always was. So an out-of-bounds read on a native lane
    // yields 0.0 there, exactly as it already did for float; only a boxed
    // result can carry the null through.
    if (policy.oob_kind == MIR_INDEX_OOB_FLOAT_ZERO || result_type == MIR_T_D) {
        emit_insn(mt, MIR_new_insn(mt->ctx, MIR_DMOV, MIR_new_reg_op(mt->ctx, result),
            MIR_new_double_op(mt->ctx, 0.0)));
    } else {
...
static MIR_reg_t emit_index_value(MirTranspiler* mt, AstNode* field, bool use_native) {
    (void)use_native;  // every index is now evaluated in the value's own lane
    ...
    // then transpile_expr correctly hands back a boxed Item. Converting here
    ...
    TypeId tid = get_effective_type(mt, field);
    MIR_reg_t value = transpile_expr(mt, field);
    if (tid != LMD_TYPE_INT && tid != LMD_TYPE_FLOAT) {
        return emit_unbox(mt, value, LMD_TYPE_INT);
    }
    return value;
}
    ...
    if (field_node->field && field_node->field->next) {
        ...
        for (AstNode* it = field_node->field; it; it = it->next) {
            ...
            if (!is_integer_type_id(vt)) {
                ...
                val = emit_machine_index(mt, val, vt);
            }
            emit_insn(mt, MIR_new_insn(mt->ctx, MIR_MOV,
                MIR_new_mem_op(mt->ctx, MIR_T_I64,
    ...
    if (obj_tid == LMD_TYPE_ARRAY && is_integer_type_id(idx_tid)) {
        ...
        if (nested_int) {
            ...
            return emit_call_2(mt, "item_at", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, boxed_obj),
                MIR_T_I64, MIR_new_reg_op(mt->ctx, emit_machine_index(mt, idx_native, LMD_TYPE_INT)));
        }
    }
    ...
    if (idx_tid == LMD_TYPE_INT && obj_tid == LMD_TYPE_ANY) {
        ...
        if (mir_is_native_scalar_value_type(result_tid) || result_tid == LMD_TYPE_STRING) {
            ...
            MIR_reg_t boxed_result = emit_call_2(mt, "fn_index", MIR_T_I64,
                MIR_T_I64, MIR_new_reg_op(mt->ctx, boxed_obj),
                MIR_T_I64, MIR_new_reg_op(mt->ctx, boxed_idx));
            return emit_unbox(mt, boxed_result, result_tid);
        }
...
static MIR_reg_t transpile_call_raw(MirTranspiler* mt, AstCallNode* call_node) {
    ...
    if (sys_fn_expr && sys_fn_expr->node_type == AST_NODE_SYS_FUNC) {
        ...
        if (arg_count == 1) {
            arg = call_node->argument;
            MIR_reg_t boxed_a1 = transpile_box_item(mt, arg);
            if (sysfunc_params_reject_error(info)) emit_return_if_item_error(mt, boxed_a1);
            async_emit_invoke_resume_point(mt, call_node);
    ...
    if (fn_expr->node_type == AST_NODE_IDENT) {
        ...
        if (fn_mangled && (local_func || entry_node)) {
            ...
            if (mt->tco_func && mt->in_tail_position && is_recursive_call(call_node, mt->tco_func)) {
                ...
                while (arg && param && arg_idx < 16) {
                    ...
                    MIR_reg_t val = short_circuit_error
                        ? transpile_box_item(mt, arg) : transpile_expr(mt, arg);
                    TypeId val_tid = short_circuit_error ? LMD_TYPE_ANY : get_effective_type(mt, arg);
                ...
                if (native_call && i < call_nfi->param_count &&
                    ...
                    if (resolved_args[i]) {
                        ...
                        if (parameter_contract && parameter_contract != &TYPE_ANY &&
                            ...
                            snprintf(boundary, sizeof(boundary), "argument %d of %s",
                                i + 1, fn_mangled);
                            val = emit_parameter_boundary(mt, val, LMD_TYPE_ANY,
                                parameter_contract, boundary);
                        }
                        arg_root_slots[i] = create_gc_root_slot(mt, val);
                        arg_ops[i] = MIR_new_reg_op(mt->ctx, val);
            ...
            if (has_parameter_error_guard) {
                MIR_label_t parameter_error_done = new_label(mt);
                MIR_reg_t joined_result = new_reg(mt, "call_or_param_error", MIR_T_I64);
                if (region_callsite) {
                    emit_call_void_1(mt, "lambda_region_end",
                        MIR_T_P, MIR_new_reg_op(mt->ctx, region_callsite_reg));
                ...
                emit_insn(mt, MIR_new_insn(mt->ctx, MIR_MOV,
                    MIR_new_reg_op(mt->ctx, joined_result),
                    MIR_new_reg_op(mt->ctx, boxed_normal)));
                emit_insn(mt, MIR_new_insn(mt->ctx, MIR_JMP,
                    MIR_new_label_op(mt->ctx, parameter_error_done)));

                emit_label(mt, parameter_error_label);
                if (region_callsite) {
                    emit_call_void_1(mt, "lambda_region_end",
                        MIR_T_P, MIR_new_reg_op(mt->ctx, region_callsite_reg));
                }
                parameter_error_result = root_gc_result_if_needed(mt,
                    parameter_error_result, MIR_T_I64, LMD_TYPE_ANY, "param_error");
                emit_insn(mt, MIR_new_insn(mt->ctx, MIR_MOV,
                    MIR_new_reg_op(mt->ctx, joined_result),
...
static MIR_reg_t transpile_call(MirTranspiler* mt, AstCallNode* call_node) {
    ...
    if (!call_node->propagate &&
            (mir_is_native_scalar_value_type(call_tid) || is_text_type_id(call_tid))) {
        result = emit_unbox(mt, result, call_tid);
    }
    return result;
}
...
static MIR_reg_t transpile_pipe(MirTranspiler* mt, AstPipeNode* pipe_node) {
        ...
        if (right_node && right_node->node_type == AST_NODE_CALL_EXPR) {
            ...
            if (fn_expr && fn_expr->node_type == AST_NODE_SYS_FUNC) {
                ...
                if (info) {
                    ...
                    // double now, so this emitter has to read the C return
                    // convention instead of assuming every result is an Item.
                    // The value stays in that lane on the way out: callers box
                    // by the node's effective type, and boxing here too would
                    // rotate an already-encoded Item a second time.
                    TypeId c_ret_tid = sysfunc_c_ret_type_id(info);
    ...
    {
        MIR_reg_t arr_idx = emit_box_machine_int(mt, idx);
        emit_insn(mt, MIR_new_insn(mt->ctx, MIR_MOV, MIR_new_reg_op(mt->ctx, pipe_index),
    ...
    emit_insn(mt, MIR_new_insn(mt->ctx, MIR_MOV, MIR_new_reg_op(mt->ctx, pipe_item),
        MIR_new_reg_op(mt->ctx, boxed_left)));
    {
        MIR_reg_t scalar_idx = emit_box_machine_int(mt, idx);
        emit_insn(mt, MIR_new_insn(mt->ctx, MIR_MOV, MIR_new_reg_op(mt->ctx, pipe_index),
...
static MIR_reg_t transpile_return(MirTranspiler* mt, AstReturnNode* ret_node) {
    ...
    // Unreachable filler after the return. A `return` statement yields no
    // usable value, so it is typed ANY (see get_effective_type) and consumers
    // read this register as an Item -- the filler's class follows that, not the
    // function's return type. MIR validates operand modes even on code it can
    // never reach, so the two must agree.
    MIR_type_t dummy_type = MIR_T_I64;
...
static MIR_reg_t transpile_assign_stam(MirTranspiler* mt, AstAssignStamNode* assign) {
    ...
    if (var && var->type_id == LMD_TYPE_STRING && var->env_offset < 0 &&
        ...
        if (concat->op == OPERATOR_JOIN && concat_left &&
            ...
            if (left_ident->name->len == assign->target->len &&
                    memcmp(left_ident->name->chars, assign->target->chars,
                        assign->target->len) == 0) {
                ...
                MIR_reg_t val = mir_emit_string_concat_item(mt, var->reg,
                    concat->right);
                emit_insn(mt, MIR_new_insn(mt->ctx, MIR_MOV,
    ...
    if (assign_value && assign_value->node_type == AST_NODE_BINARY &&
        ...
        // ordinary flex-int AST type is conservatively boxed/ANY.
    ...
    if (var) {
        ...
        if (var_tid == LMD_TYPE_NUM_SIZED && var->num_type == NUM_UINT32 &&
            ...
            if (var->mir_type == MIR_T_D) {
                MIR_reg_t src = emit_scalar_native_lane(mt, val, val_tid);
            ...
            emit_insn(mt, MIR_new_insn(mt->ctx, MIR_DMOV, MIR_new_reg_op(mt->ctx, var->reg),
                MIR_new_reg_op(mt->ctx, mir_emit_as_double(mt, val, val_tid))));
            ...
            if (var->full_type) {
                emit_insn(mt, MIR_new_insn(mt->ctx, MIR_DMOV, MIR_new_reg_op(mt->ctx, var->reg),
                    MIR_new_reg_op(mt->ctx, emit_scalar_native_lane(mt, val, val_tid))));
            } else {
                MIR_reg_t boxed = emit_box(mt, val, val_tid);
                MIR_reg_t any_reg = new_reg(mt, "anyv", MIR_T_I64);
                emit_insn(mt, MIR_new_insn(mt->ctx, MIR_MOV, MIR_new_reg_op(mt->ctx, any_reg),
...
static MIR_reg_t transpile_box_item(MirTranspiler* mt, AstNode* node) {
    ...
    if (node->node_type == AST_NODE_CALL_EXPR) {
        ...
        if (direct_callee && direct_callee->node_type == AST_NODE_SYS_FUNC &&
            ...
            // int64 | error. The union lowers as ANY, so box at this native
            // ABI boundary before the generic Item-carrier early return.
    ...
    if (node->node_type == AST_NODE_BINARY) {
        ...
        // overflow can promote safely. C14c `div`/`%` stay boxed until a typed
        // boundary consumes their semantic number-domain result.
        // POW falls through to boxed runtime (AST type is ANY).
        if (both_int) {
            switch (op) {
            case OPERATOR_ADD: case OPERATOR_SUB: case OPERATOR_MUL:
                // G1: int's native lane is the IEEE double, so when
                // transpile_binary took the native path this `val` is a
                // MIR_T_D holding the arithmetic result, not an Item. The two
                // decisions must be read from the same predicate or a raw
                // double reaches a helper expecting a boxed operand.
                if (mir_is_native_int_arith(mt, node)) return emit_box_int_double(mt, val);
                return val;
            case OPERATOR_DIV:
        ...
        if (has_int64_b) {
            // int64 against a binary float now joins in the FLOAT lane rather
            // than decimal, and mir_unbox_exact_native_numeric_result has
            // already recovered that result as a native double. Passing the raw
            // double through here handed a MIR_T_D register to a site expecting
            // an Item, which MIR rejects ("unexpected operand mode ... Got
            // 'double', expected 'int'"). Box it in its own lane instead.
            if (numeric_decision.valid &&
                    numeric_decision.result == LAMBDA_NUM_FLOAT) {
                return emit_box_float(mt, val);
            }
            return val;
        }

        // 4. both_float: C14c `div`/`%` still use a boxed semantic result.
        if (both_float) {
            switch (op) {
            case OPERATOR_ADD: case OPERATOR_SUB: case OPERATOR_MUL:
            case OPERATOR_DIV:
            ...
            case OPERATOR_IDIV: case OPERATOR_MOD:
                return val;
            default:
                return val; // POW → boxed fallback
            }
        }

        // 5. int_float: C14c `div`/`%` still use a boxed semantic result.
        if (int_float) {
            switch (op) {
            case OPERATOR_ADD: case OPERATOR_SUB: case OPERATOR_MUL:
            case OPERATOR_DIV:
    ...
    // re-boxing it as a native 0 value.
    // Native-lane results are excluded: they come back in a double register,
    // which cannot hold ItemNull and cannot be compared against it with BNE.
    // That covered `float` before G0 and covers `int` now that it shares the
    // lane — such a read yields 0.0 rather than null.
    if (node->node_type == AST_NODE_INDEX_EXPR &&
        tid != LMD_TYPE_FLOAT && tid != LMD_TYPE_INT) {
...
static MIR_reg_t transpile_expr(MirTranspiler* mt, AstNode* node) {
    ...
    case AST_NODE_INDEX_ASSIGN_STAM: {
        ...
        if (ca->key && !ca->key->next) {
            ...
            Type* typed_contract = typed_root && typed_root->full_type
                ? mir_unwrap_decl_type(typed_root->full_type) : NULL;
            if (typed_root && typed_contract &&
                ...
                MIR_reg_t owner = emit_box(mt, typed_root->reg, typed_root->type_id);
                MIR_reg_t replacement = emit_call_5(mt, "lambda_map_path_set_checked", MIR_T_I64,
                    MIR_T_I64, MIR_new_reg_op(mt->ctx, owner),
            ...
            if (typed_root && typed_path.count == 0 && typed_contract &&
                    typed_contract->type_id == LMD_TYPE_TYPE &&
                    typed_contract->kind == TYPE_KIND_UNARY &&
                    ((TypeUnary*)typed_contract)->op == OPERATOR_REPEAT) {
                TypeId index_type = get_effective_type(mt, ca->key);
                Type* element_contract = mir_array_occurrence_element(typed_contract);
                TypeId value_type = get_effective_type(mt, ca->value);
                bool writes_through_caller = typed_root->is_var_param || typed_root->is_proc_param;
                bool native_write_witness = writes_through_caller &&
                    typed_root->type_id == LMD_TYPE_ARRAY_NUM && element_contract &&
                    typed_root->elem_type == element_contract->type_id &&
                    is_integer_type_id(index_type) && value_type == typed_root->elem_type;
                if (!native_write_witness) {
                    ...
                    MIR_reg_t replacement = emit_call_5(mt, checked_set, MIR_T_I64,
                        ...
                        MIR_T_P, MIR_new_int_op(mt->ctx, (int64_t)(uintptr_t)typed_root->full_type),
                        MIR_T_P, MIR_new_reg_op(mt->ctx,
                            emit_load_string_literal(mt, "typed array element assignment")));
                    emit_return_if_item_error(mt, replacement);
                    MIR_reg_t replacement_array = emit_unbox(mt, replacement,
                        LMD_TYPE_ARRAY_NUM);
                    emit_insn(mt, MIR_new_insn(mt->ctx, MIR_MOV,
                        MIR_new_reg_op(mt->ctx, typed_root->reg),
                        MIR_new_reg_op(mt->ctx, replacement_array)));
                    // The successful checked replacement satisfies T[] and is
                    // still ArrayNum; retain its raw carrier so the next typed
                    // read does not send a boxed Item through a native lane.
    ...
    case AST_NODE_CURRENT_INDEX: {
        ...
        // all. So this arm hands the Item straight back -- boxing it again
        // (I2D over tagged bits, then the int encoder) produced the encoding
        // itself as a value. Consumers that need the native lane coerce, the
        // same as for any other Item-producing expression.
    ...
    case AST_NODE_LAST_INDEX: {
        ...
        if (!mt->last_index_object) {
            ...
            MIR_reg_t zero_d = new_reg(mt, "last_idx_d", MIR_T_D);
            emit_insn(mt, MIR_new_insn(mt->ctx, MIR_DMOV, MIR_new_reg_op(mt->ctx, zero_d),
                MIR_new_double_op(mt->ctx, 0.0)));
            return zero_d;
        }
        ...
        emit_insn(mt, MIR_new_insn(mt->ctx, MIR_SUB, MIR_new_reg_op(mt->ctx, r),
            MIR_new_reg_op(mt->ctx, len), MIR_new_int_op(mt->ctx, 1)));
        // G0: the length is machine bookkeeping, but `last` is a Lambda `int`
        // the user can do arithmetic on. transpile_expr returns NATIVE lanes,
...
// A checked PARAMETER boundary can fail, and emit_parameter_boundary returns
// that error straight out of the function -- so such a function may return an
// error even when its body and tail expression prove otherwise.
static bool function_params_may_check_boundary(AstFuncNode* fn_node) {
    for (AstNamedNode* param = fn_node ? fn_node->param : NULL; param;
            param = (AstNamedNode*)param->next) {
        if (!param->declared_type) continue;
        Type* declared = mir_unwrap_decl_type(param->declared_type);
        if (!declared || declared->type_id == LMD_TYPE_ANY) continue;
        if (lambda_type_accepts_error(declared)) continue;
        return true;
    }
    return false;
}
...
static bool function_return_may_defer(AstFuncNode* fn_node) {
    ...
    // Same for a parameter boundary. Without this a function like
    // `fn f(v: int) int` kept the native double return lane, and the error its
    // parameter check produced was converted into that lane -- becoming NaN and
    // losing the failure entirely.
    if (function_params_may_check_boundary(fn_node)) return true;
    ...
    return !body->type || mir_decl_type_id(body->type) == LMD_TYPE_ANY;
}
...
static MirScalarReturnMode infer_boxed_return_mode(MirTranspiler* mt,
    ...
    if (is_proc) return MIR_SCALAR_RETURN_DYNAMIC;
...
static FnVariantAnalysis* analyze_lambda_mir_variants(MirTranspiler* mt,
    ...
    bool may_return_boundary_error = function_return_may_defer(fn);
    ...
    AstFuncNode* fn_node, NativeFuncInfo* nfi, bool is_method)
{
    ...
    MirCallOptions options = {{MIR_FRAME_REF_NONE, 0},
        (uint8_t)(FN_RETURN_HOME_NORMAL | FN_RETURN_HOME_ERROR), false, true};
...
static void transpile_func_def(MirTranspiler* mt, AstFuncNode* fn_node) {
    ...
    // Direct calls forward the caller register; no callee reloads `_lambda_rt`.
    params[param_count] = {MIR_T_P, raw_strdup("runtime"), 0};
    param_count++;
...
#pragma clang diagnostic pop
    ...
    while (param && pi < user_param_count) {
        ...
        if (generate_native && mir_is_native_param_type(tid)) {
            ...
            Type* param_decl_type = param->declared_type
                ? mir_unwrap_decl_type(param->declared_type)
                : mir_unwrap_decl_type((Type*)param->type);
    ...
    mt->current_return_type = fn_type
        ? (fn_type->return_contract ? fn_type->return_contract : fn_type->returned) : NULL;
    mt->native_return_tid = LMD_TYPE_ANY;
    if (native_return) {
    ...
    if (native_return) {
        ...
        if (!mt->block_returned) {
            ...
            if (nfi_body->return_mir == MIR_T_D && body_mir != MIR_T_D) {
                // A statically float call can still return a checked boxed Item
                // (for example through its public boundary wrapper). The prior
                // type-only test emitted DMOV from that Item register, which
                // MIR rejects and would abort the rest of a batch.
                body_result = emit_unbox(mt, body_result, nfi_body->return_type);
            } else if (nfi_body->return_mir != MIR_T_D && body_mir == MIR_T_D) {
                MIR_reg_t boxed = emit_box(mt, body_result, LMD_TYPE_FLOAT);
                body_result = emit_unbox(mt, boxed, nfi_body->return_type);
            } else if (body_tid != nfi_body->return_type) {
            ...
            if (proc_node->name && strcmp(proc_node->name->chars, "main") == 0) {
                ...
                if (main_func) {
                    LocalFuncEntry* entry = find_local_func_entry(&mt,
                        main_name->str);
                    MirCallOptions main_options = {{MIR_FRAME_REF_NONE, 0},
                        (uint8_t)(FN_RETURN_HOME_NORMAL | FN_RETURN_HOME_ERROR),
                        false, true};
