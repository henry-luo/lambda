...
// A dotted member name is a static key, not an identifier read: lowering
// interns it as a runtime-pool name Item (the property-key path), and reading
// it as a binding would resolve to null. Member access and the Tier-3 CRUD
...
static Item interp_eval_member_key(InterpFrame* f, AstNode* field) {
    if (field && field->node_type == AST_NODE_IDENT) {
        AstIdentNode* name = (AstIdentNode*)field;
        return (Item){.item = s2it(heap_create_name(name->name->chars,
            name->name->len))};
    }
