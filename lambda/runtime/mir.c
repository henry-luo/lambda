...
void* jit_gen_func(MIR_context_t ctx, const char *func_name) {
    ...
    // LAMBDA_MIR_LOG_CODE_ADDR=1: publish every generated function's code
    // address (eager interface: all are generated by the link above), so a
    // sampling profiler's anonymous JIT frames can be attributed to Lambda
    // functions by nearest lower start address. log_notice survives release.
    const char* log_addrs = getenv("LAMBDA_MIR_LOG_CODE_ADDR");
    if (log_addrs && log_addrs[0] && strcmp(log_addrs, "0") != 0) {
        for (MIR_module_t module = DLIST_HEAD(MIR_module_t, *MIR_get_module_list(ctx));
            ...
            for (MIR_item_t item = DLIST_HEAD(MIR_item_t, module->items); item != NULL;
                ...
                if (item->item_type != MIR_func_item || !item->u.func->machine_code) continue;
                log_notice("mir-code-addr: %s %p", item->u.func->name,
                    item->u.func->machine_code);
            }
        }
    }
    return func_ptr;
