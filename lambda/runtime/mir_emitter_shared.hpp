...
struct MirCallOptions {
    ...
    // options prevents direct calls from accidentally passing the scalar-home
    // lane as `_env_ptr`.
    bool has_hidden_env;
...
static inline MirCallResult em_call_direct(MirEmitter* em,
    ...
    if (env_arg_count) {
        physical_types[context_arg_count] = MIR_T_P;
        physical_ops[context_arg_count] = MIR_new_reg_op(em->ctx,
            options->hidden_env);
    }
    ...
    for (int i = 0; i < nargs; i++) {
        ...
        ValueRep param_rep = !runtime && !env && variant && variant->params &&
            (i - hidden_arg_count) < variant->param_count
            ? variant->params[i - hidden_arg_count].canonical_rep : VALUE_REP_NONE;
