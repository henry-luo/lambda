...
fn is_iframe(node) {
    node != null and radiant.closest(node, "iframe") == node
}

fn find_named_frame(node, target_name) {
    if (node == null) { null }
    else if (is_iframe(node)) {
        if (radiant.attr(node, "name") == target_name) { node }
        else {
            ...
            else { find_named_frame(radiant.next_element_sibling(node), target_name) }
        }
    }
    else {
        let inside = find_named_frame(radiant.first_element_child(node), target_name);
        if (inside != null) { inside }
        else { find_named_frame(radiant.next_element_sibling(node), target_name) }
    }
}
...
fn find_id(node, fragment) {
    if (node == null) { null }
    else if (radiant.attr(node, "id") == fragment) { node }
    else {
        let inside = find_id(radiant.first_element_child(node), fragment);
        if (inside != null) { inside }
        else { find_id(radiant.next_element_sibling(node), fragment) }
    }
}

fn is_named_anchor(node) {
    node != null and radiant.closest(node, "a") == node
}

fn find_named_anchor(node, fragment) {
    if (node == null) { null }
    else if (is_named_anchor(node) and radiant.attr(node, "name") == fragment) { node }
    else {
        let inside = find_named_anchor(radiant.first_element_child(node), fragment);
        if (inside != null) { inside }
        else { find_named_anchor(radiant.next_element_sibling(node), fragment) }
    }
}
...
fn parent_root(root) {
    ...
    if (iframe == null) { root } else { radiant.document_root(iframe) }
}

fn top_root(root) {
    let iframe = radiant.embedding_element(root);
    if (iframe == null) { root } else { top_root(radiant.document_root(iframe)) }
}

fn resolve_target(source, raw_target) {
    let source_root = radiant.document_root(source);
...
pub pn activate(anchor) {
    let href = radiant.attr(anchor, "href");
    if (href == null) { 'pass' }
    else {
        let resolved = resolve_target(anchor, radiant.attr(anchor, "target"));
        let destination = if (resolved.target_root == null) null
            else radiant.navigation_destination(anchor, href, resolved.target_root);
