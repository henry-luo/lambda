...
#define dom_expando_has_property radiant_host_api->realm->expando_has_property
...
static void radiant_dom_commit_geometry_layout(DomDocument* doc) {
    ...
    // Parser-blocking scripts retain the host's pre-commit snapshot. Once the
    // document is interactive, geometry reads must flush current layout.
    if (doc->js.host_driven_loop && doc->js.ready_state &&
        strcmp(doc->js.ready_state, "loading") == 0) return;
