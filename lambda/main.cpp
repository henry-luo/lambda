...
#include "../lib/str.h"     // For str_to_int64_or, str_to_double_or
...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 2 && strcmp(argv[1], "render") == 0) {
        ...
        for (int i = 2; i < argc; i++) {
            if (strcmp(argv[i], "-o") == 0 || strcmp(argv[i], "--output") == 0) {
                ...
                if (i + 1 < argc) {
                    i++;
                    viewport_width = (int)str_to_int64_or(argv[i], strlen(argv[i]), 0);
                    if (viewport_width <= 0) {
                ...
                if (i + 1 < argc) {
                    i++;
                    viewport_height = (int)str_to_int64_or(argv[i], strlen(argv[i]), 0);
                ...
                if (i + 1 < argc) {
                    i++;
                    render_scale = (float)str_to_double_or(argv[i], strlen(argv[i]), 0.0);
                ...
                if (i + 1 < argc) {
                    i++;
                    pixel_ratio = (float)str_to_double_or(argv[i], strlen(argv[i]), 0.0);
                    if (pixel_ratio <= 0.0f) {
    ...
    if (argc >= 2 && strcmp(argv[1], "fetch") == 0) {
        ...
        for (int i = 2; i < argc; i++) {
            if ((strcmp(argv[i], "-o") == 0 || strcmp(argv[i], "--output") == 0) && i + 1 < argc) {
                ...
                i++;
                timeout_ms = (int)str_to_int64_or(argv[i], strlen(argv[i]), 0);
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--mir") == 0) {
            ...
            if (i + 1 < argc) {
                i++;
                max_errors = (int)str_to_int64_or(argv[i], strlen(argv[i]), 0);
            ...
            optimize_level = (int)str_to_int64_or(argv[i] + 11, strlen(argv[i] + 11), 0);
