...
static MIR_reg_t bm_new_temp(BashMirTranspiler* mt) {
    ...
    return MIR_new_func_reg(mt->ctx, MIR_item_func(mt->current_func), name, MIR_T_I64);
}
...
static void bm_emit_set_var(BashMirTranspiler* mt, const char* name, MIR_op_t value) {
    // create string Item for variable name
    String* name_str = heap_create_name(name);
    MIR_reg_t name_reg = bm_new_temp(mt);
    MIR_append_insn(mt->ctx, mt->current_func,
        MIR_new_insn(mt->ctx, MIR_MOV, MIR_new_reg_op(mt->ctx, name_reg),
                     MIR_new_uint_op(mt->ctx, s2it(name_str))));

    // call bash_set_var(name, value)
    MIR_append_insn(mt->ctx, mt->current_func,
        MIR_new_call_insn(mt->ctx, 3,
            MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_set_var")),
            MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_set_var", 0, NULL, 2,
                MIR_T_I64, "name", MIR_T_I64, "value")),
...
static MIR_op_t bm_emit_get_var(BashMirTranspiler* mt, const char* name) {
    ...
    MIR_append_insn(mt->ctx, mt->current_func,
        MIR_new_call_insn(mt->ctx, 4,
            MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_get_var")),
            MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_get_var", 1, &res_type, 1,
                MIR_T_I64, "name")),
...
static void bm_transpile_statement(BashMirTranspiler* mt, BashAstNode* node) {
    ...
    case BASH_AST_NODE_LIST: {
        ...
        if (list->op == BASH_LIST_AND) {
            ...
            // check exit code
            MIR_reg_t ec = bm_new_temp(mt);
            MIR_type_t res_type = MIR_T_I64;
            MIR_append_insn(mt->ctx, mt->current_func,
                MIR_new_call_insn(mt->ctx, 3,
                    MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_get_exit_code")),
                    MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_gec", 1, &res_type, 0)),
            ...
            MIR_append_insn(mt->ctx, mt->current_func, skip);
            ...
            MIR_append_insn(mt->ctx, mt->current_func,
                MIR_new_call_insn(mt->ctx, 3,
                    MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_get_exit_code")),
                    MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_gec2", 1, &res_type, 0)),
                    MIR_new_reg_op(mt->ctx, ec)));
            MIR_append_insn(mt->ctx, mt->current_func,
    ...
    case BASH_AST_NODE_SUBSHELL: {
        ...
        MIR_append_insn(mt->ctx, mt->current_func,
            MIR_new_call_insn(mt->ctx, 2,
                MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_scope_push_subshell")),
                MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_spush", 0, NULL, 0))));
        bm_transpile_statement(mt, subshell->body);
        // pop subshell scope
        MIR_append_insn(mt->ctx, mt->current_func,
            MIR_new_call_insn(mt->ctx, 2,
                MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_scope_pop_subshell")),
                MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_spop", 0, NULL, 0))));
        break;
    }
    ...
    case BASH_AST_NODE_SPECIAL_VARIABLE: {
        ...
        if (sv->special_id == BASH_SPECIAL_QUESTION) {
            ...
            MIR_append_insn(mt->ctx, mt->current_func,
                MIR_new_call_insn(mt->ctx, 3,
                    MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_get_exit_code")),
                    MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_gec3", 1, &res_type, 0)),
                    MIR_new_reg_op(mt->ctx, result)));
            return MIR_new_reg_op(mt->ctx, result);
        }
        if (sv->special_id == BASH_SPECIAL_HASH) {
            ...
            MIR_append_insn(mt->ctx, mt->current_func,
                MIR_new_call_insn(mt->ctx, 3,
                    MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_get_arg_count")),
                    MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_gac", 1, &res_type, 0)),
                    MIR_new_reg_op(mt->ctx, result)));
            return MIR_new_reg_op(mt->ctx, result);
        }
        if (sv->special_id >= BASH_SPECIAL_POS_1 && sv->special_id <= BASH_SPECIAL_POS_9) {
            ...
            MIR_append_insn(mt->ctx, mt->current_func,
                MIR_new_call_insn(mt->ctx, 4,
                    MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_get_positional")),
                    MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_gpos", 1, &res_type, 1,
                        MIR_T_I64, "index")),
                    MIR_new_reg_op(mt->ctx, result),
...
static MIR_op_t bm_transpile_string(BashMirTranspiler* mt, BashStringNode* node) {
    ...
    while (part) {
        ...
        MIR_reg_t concat_result = bm_new_temp(mt);
        MIR_type_t res_type = MIR_T_I64;
        MIR_append_insn(mt->ctx, mt->current_func,
            MIR_new_call_insn(mt->ctx, 5,
                MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_string_concat")),
                MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_concat", 1, &res_type, 2,
                    MIR_T_I64, "left", MIR_T_I64, "right")),
                MIR_new_reg_op(mt->ctx, concat_result),
                result,
                next_val));
...
static MIR_op_t bm_transpile_concat(BashMirTranspiler* mt, BashConcatNode* node) {
    ...
    while (part) {
        ...
        MIR_append_insn(mt->ctx, mt->current_func,
            MIR_new_call_insn(mt->ctx, 5,
                MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_string_concat")),
                MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_concat2", 1, &res_type, 2,
                    MIR_T_I64, "left", MIR_T_I64, "right")),
                MIR_new_reg_op(mt->ctx, concat_result),
                result,
                next_val));
...
static MIR_op_t bm_transpile_command(BashMirTranspiler* mt, BashCommandNode* cmd) {
    ...
    if (cmd_len == 4 && memcmp(cmd_name, "echo", 4) == 0) {
        // collect arguments into array on stack
        int argc = cmd->arg_count;
        if (argc == 0) {
            // echo with no args: just print newline
            MIR_reg_t result = bm_new_temp(mt);
            MIR_type_t res_type = MIR_T_I64;
            MIR_append_insn(mt->ctx, mt->current_func,
                MIR_new_call_insn(mt->ctx, 5,
                    MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_builtin_echo")),
                    MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_echo", 1, &res_type, 2,
                        MIR_T_I64, "args", MIR_T_I64, "argc")),
                    MIR_new_reg_op(mt->ctx, result),
        ...
        MIR_append_insn(mt->ctx, mt->current_func,
            MIR_new_call_insn(mt->ctx, 5,
                MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_builtin_echo")),
                MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_echo2", 1, &res_type, 2,
                    MIR_T_I64, "args", MIR_T_I64, "argc")),
                MIR_new_reg_op(mt->ctx, result),
    ...
    if (cmd_len == 4 && memcmp(cmd_name, "true", 4) == 0) {
        ...
        MIR_append_insn(mt->ctx, mt->current_func,
            MIR_new_call_insn(mt->ctx, 3,
                MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_builtin_true")),
                MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_true", 1, &res_type, 0)),
                MIR_new_reg_op(mt->ctx, result)));
        return MIR_new_reg_op(mt->ctx, result);
    }
    ...
    if (cmd_len == 5 && memcmp(cmd_name, "false", 5) == 0) {
        ...
        MIR_append_insn(mt->ctx, mt->current_func,
            MIR_new_call_insn(mt->ctx, 3,
                MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_builtin_false")),
                MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_false", 1, &res_type, 0)),
                MIR_new_reg_op(mt->ctx, result)));
        return MIR_new_reg_op(mt->ctx, result);
    }
...
static void bm_transpile_if(BashMirTranspiler* mt, BashIfNode* node) {
    ...
    MIR_append_insn(mt->ctx, mt->current_func,
        MIR_new_call_insn(mt->ctx, 3,
            MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_get_exit_code")),
            MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_if_ec", 1, &res_type, 0)),
            MIR_new_reg_op(mt->ctx, ec)));
    ...
    if (node->elif_clauses) {
        MIR_append_insn(mt->ctx, mt->current_func, first_elif_or_else);
        ...
        while (elif) {
            ...
            MIR_reg_t elif_ec = bm_new_temp(mt);
            MIR_append_insn(mt->ctx, mt->current_func,
                MIR_new_call_insn(mt->ctx, 3,
                    MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_get_exit_code")),
                    MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_elif_ec", 1, &res_type, 0)),
            ...
            if (elif->next) {
                MIR_append_insn(mt->ctx, mt->current_func, next_elif);
            }
            elif = elif->next;
        }
    }
    ...
    // else body
    if (node->else_body) {
        MIR_append_insn(mt->ctx, mt->current_func, else_label);
        bm_transpile_statement(mt, node->else_body);
    }

    MIR_append_insn(mt->ctx, mt->current_func, end_label);
}
...
static void bm_transpile_for(BashMirTranspiler* mt, BashForNode* node) {
    ...
    while (word) {
        ...
        MIR_append_insn(mt->ctx, mt->current_func, loop_continue);

        word = word->next;
    }

    MIR_append_insn(mt->ctx, mt->current_func, loop_end);
...
static void bm_transpile_for_arith(BashMirTranspiler* mt, BashForArithNode* node) {
    ...
    // loop start
    MIR_append_insn(mt->ctx, mt->current_func, loop_start);
...
static void bm_transpile_while(BashMirTranspiler* mt, BashWhileNode* node) {
    ...
    MIR_append_insn(mt->ctx, mt->current_func,
        MIR_new_call_insn(mt->ctx, 3,
            MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_get_exit_code")),
            MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_while_ec", 1, &res_type, 0)),
            MIR_new_reg_op(mt->ctx, ec)));
...
static void bm_transpile_case(BashMirTranspiler* mt, BashCaseNode* node) {
    ...
    while (item) {
        ...
        while (pattern) {
            ...
            MIR_reg_t cmp_result = bm_new_temp(mt);
            MIR_type_t res_type = MIR_T_I64;
            MIR_append_insn(mt->ctx, mt->current_func,
                MIR_new_call_insn(mt->ctx, 5,
                    MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_test_str_eq")),
                    MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_case_eq", 1, &res_type, 2,
                        MIR_T_I64, "left", MIR_T_I64, "right")),
        ...
        // match body
        MIR_append_insn(mt->ctx, mt->current_func, match_label);
        bm_transpile_statement(mt, case_item->body);
        MIR_append_insn(mt->ctx, mt->current_func,
            MIR_new_insn(mt->ctx, MIR_JMP, MIR_new_label_op(mt->ctx, end_label)));

        MIR_append_insn(mt->ctx, mt->current_func, next_item);
        item = item->next;
    }
...
static MIR_op_t bm_transpile_arith(BashMirTranspiler* mt, BashAstNode* node) {
    ...
    case BASH_AST_NODE_ARITH_BINARY: {
        ...
        MIR_append_insn(mt->ctx, mt->current_func,
            MIR_new_call_insn(mt->ctx, 5,
                MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, func_name)),
                MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_arith_bin", 1, &res_type, 2,
                    MIR_T_I64, "left", MIR_T_I64, "right")),
                MIR_new_reg_op(mt->ctx, result),
                left,
                right));
    ...
    case BASH_AST_NODE_ARITH_UNARY: {
        ...
        if (unary->op == BASH_OP_NEGATE || unary->op == BASH_OP_SUB) {
            ...
            MIR_append_insn(mt->ctx, mt->current_func,
                MIR_new_call_insn(mt->ctx, 4,
                    MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_negate")),
                    MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_neg", 1, &res_type, 1,
                        MIR_T_I64, "operand")),
                    MIR_new_reg_op(mt->ctx, result),
                    operand));
            return MIR_new_reg_op(mt->ctx, result);
...
Item transpile_bash_to_mir(Runtime* runtime, const char* bash_source, const char* filename) {
    ...
    mt->current_func = MIR_new_func(ctx, "bash_main", 1, &ret_type, 0);
    ...
    // return exit code
    MIR_reg_t final_ec = bm_new_temp(mt);
    MIR_type_t ec_type = MIR_T_I64;
    MIR_append_insn(mt->ctx, mt->current_func,
        MIR_new_call_insn(mt->ctx, 3,
            MIR_new_ref_op(mt->ctx, MIR_new_import(mt->ctx, "bash_get_exit_code")),
            MIR_new_ref_op(mt->ctx, MIR_new_proto(mt->ctx, "p_main_ec", 1, &ec_type, 0)),
            MIR_new_reg_op(mt->ctx, final_ec)));
