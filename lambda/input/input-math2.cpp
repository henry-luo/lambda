...
struct MathParseContext {
    ...
    char* node_text_dup(TSNode node) {
        ...
        char* result = (char*)mem_alloc(len + 1, MEM_CAT_INPUT_MATH);
...
static Item build_math(MathParseContext& ctx, TSNode node) {
    ...
    Item* items = (Item*)mem_alloc(child_count * sizeof(Item), MEM_CAT_INPUT_MATH);
    int count = 0;
...
static Item build_fraction(MathParseContext& ctx, TSNode node) {
    ...
    char* cmd = ts_node_is_null(cmd_node) ? mem_strdup("\\frac", MEM_CAT_INPUT_MATH) : ctx.node_text_dup(cmd_node);
...
static Item build_binomial(MathParseContext& ctx, TSNode node) {
    TSNode cmd_node = ts_node_child_by_field_name(node, "cmd", 3);
    char* cmd = ts_node_is_null(cmd_node) ? mem_strdup("\\binom", MEM_CAT_INPUT_MATH) : ctx.node_text_dup(cmd_node);
...
static Item build_delimiter_group(MathParseContext& ctx, TSNode node) {
    ...
    char* left = ts_node_is_null(left_node) ? mem_strdup("(", MEM_CAT_INPUT_MATH) : ctx.node_text_dup(left_node);
    char* right = ts_node_is_null(right_node) ? mem_strdup(")", MEM_CAT_INPUT_MATH) : ctx.node_text_dup(right_node);
...
static Item build_accent(MathParseContext& ctx, TSNode node) {
    ...
    char* cmd = ts_node_is_null(cmd_node) ? mem_strdup("\\hat", MEM_CAT_INPUT_MATH) : ctx.node_text_dup(cmd_node);
...
static Item build_big_operator(MathParseContext& ctx, TSNode node) {
    ...
    char* op = ts_node_is_null(op_node) ? mem_strdup("\\sum", MEM_CAT_INPUT_MATH) : ctx.node_text_dup(op_node);
...
static Item build_text_command(MathParseContext& ctx, TSNode node) {
    ...
    char* cmd = ts_node_is_null(cmd_node) ? mem_strdup("\\text", MEM_CAT_INPUT_MATH) : ctx.node_text_dup(cmd_node);
...
static Item build_environment(MathParseContext& ctx, TSNode node) {
    ...
    TSNode name_node = ts_node_child_by_field_name(node, "name", 4);
    char* env_name = ts_node_is_null(name_node) ? mem_strdup("matrix", MEM_CAT_INPUT_MATH) : ctx.node_text_dup(name_node);
