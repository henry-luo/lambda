...
import mermaid_config: lambda.graph.mermaid.config
import polygon: lambda.graph.polygon
import graphviz_colors: lambda.graph.graphviz.colors
...
fn html_node(node, index, group, assigned_classes, style_declarations, interaction, palette) {
  ...
  <node class: node_class(node, assigned_classes),
      ...
      'data-order-group': source_attr(node, "group", null),
      'data-ordering': source_attr(node, "ordering", null),
...
fn html_edge(edge, index, group, graph_directed, assigned_classes,
  ...
  <edge class: edge_class(assigned_classes), 'data-graph-role': "edge", 'data-edge-id': id,
      ...
      'data-route-mode': source_attr(edge, "route-mode", null),
      'data-style-declarations': style_declarations,
...
pub fn to_html(graph, opts = null) {
  ...
  <'graph' class: "lambda-graph lambda-graph-theme-" ++ theme,
      ...
      'data-ordering': source_attr(graph, "ordering", null),
      'data-new-rank': source_attr(graph, "new-rank", null),
