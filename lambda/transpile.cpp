...
void pre_define_closure_envs(Transpiler* tp, AstNode* node) {
    ...
    case AST_NODE_PARENT_EXPR:
        ...
        break;
    case AST_NODE_VIEW: {
        AstViewNode* view_node = (AstViewNode*)node;
        pre_define_closure_envs(tp, view_node->body);
        // check handlers for closures
        AstEventHandler* handler = view_node->handler;
        while (handler) {
            pre_define_closure_envs(tp, handler->body);
...
// transpile_call_argument, find_param_by_name, get_param_at_index,
// transpile_tail_call, is_tco_tail_call, and transpile_call_expr
// are now in transpile-call.cpp
...
void transpile_expr(Transpiler* tp, AstNode *expr_node) {
    ...
    case AST_NODE_VIEW:  case AST_NODE_STATE_ENTRY:  case AST_NODE_EVENT_HANDLER:
...
void transpile_ast_root(Transpiler* tp, AstScript *script) {
    ...
    if (!tp->is_main) {
        ...
        // Override const pool pointer — all const_* macros redirect automatically
        strbuf_append_str(tp->code_buf, "#undef _const_pool\n");
        strbuf_append_str(tp->code_buf, "#define _const_pool _mod_consts\n");
