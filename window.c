...
void render_text_to_screen(GLFWwindow *window, const char *text, const char *font_path, int font_size) {
    ...
    unsigned char *big_bitmap = (unsigned char *)calloc(total_width * max_height * 3, sizeof(unsigned char));
    if (!big_bitmap) {
    ...
    for (const char *p = text; *p; p++) {
        ...
        for (int y = 0; y < bitmap.rows; y++) {
            for (int x = 0; x < bitmap.width; x++) {
                ...
                int index = (big_y * total_width + big_x) * 3;
    ...
    float x_ratio = total_width / (float)WINDOW_WIDTH / 2;
    float y_ratio = max_height / (float)WINDOW_HEIGHT / 2;
