...
static bool multicol_has_visible_inline_content(View* first,
                                                bool stop_at_spanner = false);
static bool multicol_should_break_line_for_widows_orphans(
    int line_slot, float visual_bottom, float target_height,
    int line_index, int line_count, int widows, int orphans,
    float next_bottom);
static float multicol_normal_line_offset(float line_advance, float visual_height);
static void multicol_snapshot_inline_cb_edges(View* view);
...
static int multicol_fragment_count(float extent, float capacity) {
    if (extent <= 0.0f || capacity <= 0.0f) return 1;
    int count = (int)ceilf(extent / capacity); // INT_CAST_OK: fragment count from positive flow extent
    return count > 0 ? count : 1;
}

static int multicol_fragment_row_count(int fragment_count, int column_count) {
    if (fragment_count < 1) fragment_count = 1;
    if (column_count < 1) column_count = 1;
    return (fragment_count + column_count - 1) / column_count;
}

struct MulticolFragmentGrid {
    int layout_column_count;
    int used_columns;
    int row_count;
};

static MulticolFragmentGrid multicol_fragment_grid(
    int fragment_count, int column_count, bool overflow_columns) {
    if (fragment_count < 1) fragment_count = 1;
    if (column_count < 1) column_count = 1;
    MulticolFragmentGrid grid = {};
    grid.layout_column_count = overflow_columns ? fragment_count : column_count;
    grid.used_columns = overflow_columns ? fragment_count :
        min(column_count, fragment_count);
    grid.row_count = multicol_fragment_row_count(
        fragment_count, grid.layout_column_count);
    return grid;
}

static void multicol_fragment_grid_dimensions(
    int fragment_count, int column_count, bool overflow_columns,
    int* layout_column_count, int* used_columns, int* row_count) {
    MulticolFragmentGrid grid = multicol_fragment_grid(
        fragment_count, column_count, overflow_columns);
    *layout_column_count = grid.layout_column_count;
    *used_columns = grid.used_columns;
    *row_count = grid.row_count;
}

static float multicol_last_fragment_piece_height(float total_height,
                                                 float fragment_capacity) {
    if (fragment_capacity <= 0.0f) return 0.0f;
    float piece = fmodf(total_height, fragment_capacity);
    return piece > 0.0f ? piece : fragment_capacity;
}
...
static void multicol_set_inline_position(TextRect* rect, View* view,
                                         float x, float y) {
    if (rect) {
        rect->x = x;
        rect->y = y;
...
static void multicol_init_inline_fragment_item(
    InlineFragmentItem* item, View* view, TextRect* rect, bool forced_break) {
    if (!item || !view) return;
    item->view = view;
    item->rect = rect;
    item->original_x = rect ? rect->x : view->x;
    // TextRect coordinates are parent-block local; keep the source origin so
    // projection does not subtract the newly placed block offset twice.
    item->original_y = rect ? rect->y : view->y;
    item->line_y = item->original_y;
    item->height = rect ? rect->height : view->height;
    item->fragment_y_offset = 0.0f;
    item->line_index = 0;
    item->is_text = rect != nullptr;
    item->is_forced_break = forced_break;
    item->reproject_rtl = false;
}
...
static bool multicol_reflow_mixed_direct_flow(
    ...
    float target_height_override = -1.0f,
    int initial_fragment_index = 0,
    float initial_fragment_offset = 0.0f,
    bool normalize_inline_origin = false,
    int* out_last_fragment = nullptr,
    float* out_end_offset = nullptr
);
...
static void multicol_record_used_columns(
    ColumnGroup* group, int used_columns, int* out_used_columns);
...
static LayoutFragmentBox* multicol_append_layout_fragment(
    Pool* pool, DomElement* elem, LayoutFragmentBox** first,
    LayoutFragmentBox** previous, int fragment_index, int column_index,
    int row_index, float x, float y, float width, float height);
...
static bool multicol_last_fragment_flow_offset(
    ViewBlock* child,
    float* out_offset,
...
static float multicol_content_box_limit(ViewBlock* block, LayoutAxis axis,
                                        bool allow_column_height) {
    if (!block) return -1.0f;

    const MultiColumnProp* multicol = block->multicol_prop();
    bool one_column_height_only = multicol_is_single_column_height_only(block);
    float limit = -1.0f;
    if (allow_column_height && multicol && multicol->column_height_is_specified &&
        ...
        limit = layout_axis_given_size(block->block(), axis);
        if (limit < 0.0f) {
            limit = layout_axis_given_max_size(block->block(), axis);
        }
    }

    if (limit < 0.0f) return -1.0f;
    if (block->bound && layout_uses_border_box(block)) {
        limit -= layout_axis_box_metrics(block, axis).pad_border;
...
static float multicol_content_box_height_limit(ViewBlock* block) {
    LayoutAxis axis = multicol_has_vertical_inline_axis(block)
        ? LAYOUT_AXIS_X : LAYOUT_AXIS_Y;
    return multicol_content_box_limit(block, axis, true);
}
...
static float multicol_content_box_inline_limit(ViewBlock* block) {
    ...
    return multicol_content_box_limit(block, axis, false);
}
...
static float multicol_resolve_line_advance(
    LayoutContext* lycon, ViewBlock* block,
    float current = 0.0f, float fallback = 0.0f) {
    if (current > 0.0f) return current;
    float line_advance = 0.0f;
    if (lycon && block && block->blk && block->block()->line_height) {
    ...
    if (line_advance <= 0.0f && block && block->fontp() &&
        block->fontp()->font_handle) {
        line_advance = calc_normal_line_height(block->fontp()->font_handle);
    }
    return line_advance > 0.0f ? line_advance : fallback;
}

static ViewBlock* multicol_in_flow_block(View* view) {
    ViewBlock* block = lam::view_as_block(view);
    return block && !layout_block_is_out_of_flow_positioned(block) &&
        block->view_type != RDT_VIEW_MARKER ? block : nullptr;
}

static ViewBlock* multicol_find_parent_container(ViewElement* start) {
    for (ViewElement* ancestor = start; ancestor;
         ancestor = ancestor->parent_view()) {
        if (!ancestor->is_block()) continue;
        ViewBlock* block = lam::view_require_block(ancestor);
        if (is_multicol_container(block)) return block;
    }
    return nullptr;
}

static float multicol_static_flow_offset(
    ViewBlock* containing_block, ViewBlock* child, LayoutAxis axis) {
    if (!containing_block || !child) return 0.0f;
    float offset = 0.0f;
    ...
    for (View* sibling = containing_block->first_placed_child();
        ...
        ViewBlock* sibling_block = multicol_in_flow_block(sibling);
        if (!sibling_block || !layout_view_is_block_flow_box(sibling_block)) {
            continue;
        }
        float sibling_extent = horizontal
            ? sibling_block->width : max(sibling_block->height, 0.0f);
        if (sibling_block->blk &&
            layout_axis_has_given_size(sibling_block, horizontal)) {
            float given = layout_axis_given_size(sibling_block->block(), axis);
            float used = layout_used_border_box_size(
                sibling_block, given, horizontal);
            sibling_extent = horizontal ? used : max(sibling_extent, used);
        }
        ...
        offset += sibling_extent + margin_before + margin_after;
    }
    return offset;
}
...
static void multicol_align_spanner_content(ViewBlock* spanner, float available_width) {
    ...
    CssEnum text_align = layout_resolve_text_align(
        spanner->block()->text_align, spanner->block()->direction);
...
static bool multicol_has_forced_break_descendant(View* view) {
    return view && layout_view_tree_any(view, [](View* candidate) {
        ViewBlock* block = lam::view_as_block(candidate);
        return block && block->blk &&
            (multicol_forces_column_break(block->block()->break_before) ||
...
static bool multicol_has_out_of_flow_descendant(View* view) {
    return view && view->is_element() && layout_view_tree_any(
        lam::view_require_element(view)->first_placed_child(),
        [](View* candidate) { return layout_view_is_out_of_flow_positioned(candidate); });
}
...
static bool multicol_has_direct_spanner_child(ViewBlock* child) {
    return child && multicol_spanner_can_escape_child(child) &&
        multicol_find_escaping_spanner(child->first_placed_child()) != nullptr;
}
...
static void multicol_reanchor_br_only_block(
    ...
    float line_advance = multicol_resolve_line_advance(lycon, block);
    for (View* child = block->first_placed_child(); child; child = child->next()) {
...
static bool multicol_has_visible_inline_content(View* first,
    ...
    for (View* current = first; current; current = current->next()) {
        ...
        if (stop_at_spanner) {
            if (ViewBlock* current_block = lam::view_as_block(current)) {
                ...
                if (multicol_has_direct_spanner_child(current_block)) {
                    if (multicol_has_visible_inline_content(
                            current_block->first_placed_child(), true)) {
                        return true;
                    }
                    continue;
                }
        ...
        if (current->view_type == RDT_VIEW_INLINE) {
            ...
            if (multicol_has_visible_inline_content(
                    span->first_child, stop_at_spanner)) return true;
            continue;
        }
...
static float multicol_reanchor_trailing_br_after_flow(
    ...
    float line_advance = multicol_resolve_line_advance(
        lycon, block, 0.0f, first_br->height);
...
static bool multicol_requires_separate_spanner_group(ViewBlock* child) {
    ...
    return !multicol_has_visible_inline_content(
        child->first_placed_child(), true);
}
...
static float multicol_spanner_prefix_flow_extent(ViewBlock* child) {
    ...
    for (View* descendant = child->first_placed_child(); descendant;
        ...
        multicol_flow_margins(child, descendant_block,
                              &margin_before, &margin_after);
        float extent = layout_axis_size(descendant_block, axis);
...
static void multicol_mirror_inline_text_rects(
    View* view, float inline_origin_x, float inline_width) {
    if (!view || inline_width <= 0.0f) return;
    if (view->view_type == RDT_VIEW_TEXT) {
        ViewText* text = lam::view_require<RDT_VIEW_TEXT>(view);
        for (TextRect* rect = text->rect; rect; rect = rect->next) {
            rect->x = 2.0f * inline_origin_x + inline_width -
                (rect->x + rect->width);
        }
        return;
    }
    ...
    for (View* child = element->first_placed_child(); child;
         child = child->next()) {
        multicol_mirror_inline_text_rects(
            child, inline_origin_x, inline_width);
    }
}
...
static void multicol_mirror_rtl_horizontal_text_descendants(ViewBlock* block) {
    if (!multicol_uses_reverse_horizontal_columns(block)) return;
    for (View* child = block->first_placed_child(); child; child = child->next()) {
        if (child->view_type != RDT_VIEW_INLINE) continue;
        // css writing modes: apply the RTL glyph reflection after inline
        // bounds are finalized, so the DOMRect union remains stable.
        multicol_mirror_inline_text_rects(child, child->x, child->width);
    }
}
...
static float multicol_trailing_inline_flow_extent(ViewBlock* block, float flow_start_offset) {
    ...
    multicol_for_each_inline_leaf(block->first_placed_child(), [&](View* leaf) {
        ...
        for (TextRect* rect = text->rect; rect; rect = rect->next) {
            if (!layout_text_rect_is_visible(text, rect)) continue;
...
static bool multicol_has_fragmentable_block_children(ViewBlock* child) {
    ...
    bool all_blocks_avoid_break = true;
    for (View* descendant = child->first_placed_child(); descendant;
         descendant = descendant->next()) {
        ViewBlock* descendant_block = multicol_in_flow_block(descendant);
        if (!descendant_block) continue;
        ...
        if (!descendant_block->blk ||
            descendant_block->block()->break_inside != CSS_VALUE_AVOID) {
    ...
    return is_list_item || (in_flow_block_count > 1 &&
        (all_blocks_size_contained || all_blocks_avoid_break));
}

static bool multicol_has_unbreakable_flow_child(ViewBlock* parent) {
    if (!parent) return false;
...
static float multicol_decorated_child_min_fragmentainer(
    ...
    for (View* child = item->first_placed_child(); child; child = child->next()) {
        ...
        float child_extent = layout_axis_size(child_block, axis);
...
static float multicol_block_children_balance_floor(
    ...
    if (!parent || column_count <= 1 ||
        (!multicol_has_fragmentable_block_children(parent) &&
         ...
         !multicol_has_unbreakable_flow_child(parent))) {
        return 0.0f;
    }

    LayoutAxis axis = multicol_has_vertical_inline_axis(parent)
        ? LAYOUT_AXIS_X : LAYOUT_AXIS_Y;
    int child_count = 0;
    float lower = 0.0f;
    float upper = 0.0f;
    for (View* child = parent->first_placed_child(); child;
        ...
        multicol_flow_margins(parent, child_block,
                              &margin_before, &margin_after);
        float extent = max(layout_axis_size(child_block, axis), 0.0f);
...
static void multicol_init_flow_item(MulticolFlowItem* item,
    ...
    if (fixed_size_overflow && !multicol_has_vertical_inline_axis(container) &&
    ...
    bool root_body_flow = container &&
        ...
        is_multicol_container(container) &&
        multicol_has_unbreakable_flow_child(child);
    item->can_fragment = multicol_has_fragmentable_line_boxes(child) ||
        multicol_has_fragmentable_block_children(child) ||
        has_descendant_fragmentable_lines || root_body_flow;
...
static bool multicol_has_mixed_direct_flow(ViewBlock* block) {
    if (!block) return false;
    bool has_inline_flow = false;
    bool has_block_flow = false;
    for (View* child = block->first_placed_child(); child; child = child->next()) {
        ViewBlock* child_block = lam::view_as_block(child);
        if (child_block) {
            if (!layout_block_is_out_of_flow_positioned(child_block) &&
                child_block->view_type == RDT_VIEW_BLOCK &&
                layout_view_is_block_flow_box(child_block)) {
                has_block_flow = true;
            }
            continue;
    ...
    return has_inline_flow && has_block_flow;
}

static bool multicol_should_reflow_mixed_direct_flow(ViewBlock* block) {
    if (!multicol_has_mixed_direct_flow(block)) return false;
    int block_count = 0;
    for (View* child = block->first_placed_child(); child; child = child->next()) {
        ViewBlock* child_block = lam::view_as_block(child);
        if (!child_block || layout_block_is_out_of_flow_positioned(child_block) ||
        ...
        block_count++;
        if (multicol_has_mixed_direct_flow(child_block)) return true;
    }
    // css fragmentation: a single simple block plus an empty inline break is
    // handled by ordinary block fragmentation; mixed reflow starts at a
    // genuine block sequence or a nested mixed-flow boundary.
...
static void multicol_for_each_nested_horizontal_slot(
    const MulticolNestedHorizontalSequence* sequence,
    float stop_at, SlotFn visit) {
    if (!sequence) return;
    float consumed = 0.0f;
    for (int parent_column = 0;
         parent_column < MAX_MULTICOL_BLOCKS &&
         (stop_at < 0.0f || consumed < stop_at - 0.001f);
         parent_column++) {
        for (int nested_column = 0;
            ...
            if (!visit(slot_start, slot_end, capacity,
                       parent_column, nested_column)) return;
            consumed = slot_end;
        }
    }
}
...
static bool multicol_for_each_nested_horizontal_fragment(
    ...
    multicol_for_each_nested_horizontal_slot(
        sequence, end,
        [&](float slot_start, float slot_end, float, int parent_column,
            int nested_column) {
            float overlap_start = max(start, slot_start);
            float overlap_end = min(end, slot_end);
            if (overlap_end <= overlap_start) return true;
            ...
            float slot_x = visual_parent_column *
                    (sequence->parent_column_width + sequence->parent_column_gap) +
                visual_nested_column *
                    (sequence->nested_column_width + sequence->nested_column_gap) +
                visual_parent_column * sequence->nested_column_gap;
            ...
            visit(slot_x, slot_y + overlap_start - slot_start,
                overlap_end - overlap_start, visual_parent_column, row_index);
            fragment_count++;
            return true;
        });
...
static float multicol_nested_horizontal_unbreakable_start(
    ...
    multicol_for_each_nested_horizontal_slot(
        sequence, -1.0f,
        [&](float slot_start, float slot_end, float capacity, int, int) {
            if (!found_start && start < slot_end - 0.001f) {
                float remaining = slot_end - start;
                if (flow_extent <= remaining + 0.5f) {
                    result = start;
                    return false;
                }
...
static bool multicol_store_nested_horizontal_fragments(
    ...
    multicol_for_each_nested_horizontal_fragment(
        ...
        [&](float x, float y, float piece_extent, int column_index, int row_index) {
            LayoutFragmentBox* fragment = multicol_append_layout_fragment(
                pool, elem, &first, &previous, fragment_index,
                column_index, row_index, x - min_x, y - min_y,
                fragment_width, piece_extent);
            if (!fragment) return;
            fragment_index++;
        }, nullptr);
...
static bool multicol_project_fragmented_inline_descendants(
    ...
    multicol_for_each_inline_leaf(child->first_placed_child(), [&](View* descendant) {
        ...
        if (descendant->view_type == RDT_VIEW_TEXT) {
            ...
            while (rect && item_count < MAX_INLINE_FRAGMENT_ITEMS) {
                ...
                multicol_init_inline_fragment_item(
                    &items[item_count], descendant, rect, false);
                item_count++;
                rect = rect->next;
            }
        } else if (descendant->view_type == RDT_VIEW_BR) {
            ...
            multicol_init_inline_fragment_item(
                &items[item_count], descendant, nullptr, true);
            item_count++;
        }
    });
    ...
    for (int i = 0; i < item_count; i++) {
        ...
        items[i].fragment_y_offset = parent_column_index * parent_pitch +
        ...
        if (vertical_writing && inner_column_count == 1) {
            ...
            new_y = items[i].original_y +
                parent_column_index * parent_pitch +
                parent_row_index * (fragment_height + row_gap);
            items[i].reproject_rtl = items[i].is_text && fragment_direction_rtl &&
                inner_fragment_index > 0;
            ...
            float normalized_x = multicol_normalize_inline_x(
                items[i].original_x, 0.0f, inner_pitch);
            float line_local_delta_y = items[i].is_text
                ? items[i].original_y - items[i].line_y : 0;
        ...
        multicol_set_inline_position(
            items[i].rect, items[i].is_text ? nullptr : items[i].view,
            new_x, new_y);
    }

    if (vertical_writing && child->block()->direction == CSS_VALUE_RTL) {
        // css position fragmentation: snapshot the original fragment edge
        // before continuation text is reordered into source order.
        multicol_snapshot_inline_cb_edges(static_cast<View*>(child));
        for (int i = 0; i < item_count; i++) {
            if (!items[i].reproject_rtl) continue;
            float line_origin = items[i].original_y;
            float source_order_offset = 0.0f;
            int last_break = -1;
            for (int previous = i - 1; previous >= 0; previous--) {
                if (items[previous].is_forced_break) {
                    last_break = previous;
                    break;
                }
            }
            for (int candidate = last_break + 1; candidate < item_count; candidate++) {
                if (fabsf(items[candidate].original_x - items[i].original_x) > 1.0f ||
                    !items[candidate].is_text || items[candidate].height <= 0.0f) {
                    continue;
                }
                line_origin = min(line_origin, items[candidate].original_y);
                if (candidate < i) source_order_offset += items[candidate].height;
            }
            // css position fragmentation: the source-order continuation offset
            // already includes the nested inline's line progression.
            items[i].rect->y = line_origin + source_order_offset +
                items[i].fragment_y_offset;
        }
    }
...
static int multicol_project_fragmented_descendants(
    ...
    bool root_body_projection = layout_block_is_viewport_body(child) &&
        parent_block && is_multicol_container(parent_block);
    bool project_block_flow = has_block_flow_child || root_body_projection;
    ...
    bool mixed_direct_flow = !child_is_multicol && !vertical_writing &&
        !(child->blk && child->block()->text_box_trim) &&
        multicol_has_mixed_direct_flow(child);
    ...
    if (mixed_direct_flow) {
        int start_fragment = 0;
        float start_offset = max(initial_fragment_offset, 0.0f);
        if (fragment_height > 0.0f && start_offset >= fragment_height) {
            start_fragment = (int)floorf(start_offset / fragment_height); // INT_CAST_OK: non-negative fragment index
            start_offset -= start_fragment * fragment_height;
        }
        float mixed_height = 0.0f;
        int mixed_columns = 0;
        int mixed_last_fragment = start_fragment;
        float mixed_end_offset = start_offset;
        float mixed_origin_x = layout_axis_decoration_start(
            child->bound ? child->boundary() : nullptr, LAYOUT_AXIS_X);
        if (multicol_reflow_mixed_direct_flow(
                lycon, child, column_count, column_width, column_gap,
                mixed_origin_x, 0.0f, &mixed_height, &mixed_columns,
                fragment_height, start_fragment, start_offset, false,
                &mixed_last_fragment, &mixed_end_offset)) {
            // css fragmentation: a mixed direct flow consumes its line and
            // block children in source order before descendant projection.
            return mixed_last_fragment + 1;
        }
    }
    ...
    while (descendant) {
        ...
        if (project_block_flow && !child_is_multicol && !vertical_writing) {
            ...
            if (root_body_projection) {
                // css fragmentation: the root body's initial margin belongs to
                // the first fragment only; do not count it twice in flow space.
                descendant_origin_offset = 0.0f;
                descendant_flow_offset = source_offset;
            } else {
        ...
        if (root_body_projection && placement.fragment_index > 0) {
            descendant_origin_offset = initial_fragment_offset;
        }
        ...
        if (horizontal_table && descendant_block &&
            ...
            int caption_fragment_count = multicol_fragment_count(
                initial_fragment_offset + caption_flow_extent, fragment_height);
        ...
        if (!handled_nested_horizontal && child_is_multicol) {
            ...
            if (descendant->is_block()) {
                ...
                float descendant_flow_extent = layout_axis_size(
                    descendant_block,
                    vertical_writing ? LAYOUT_AXIS_X : LAYOUT_AXIS_Y);
                ...
                if (nested_multicol_spanner_projection &&
                    ...
                    int descendant_fragment_count = multicol_fragment_count(
                        descendant_flow_extent, block_split_height);
        ...
        if (!descendant_fragmented && descendant->is_block() &&
            ...
            ViewBlock* descendant_block = lam::view_require_block(descendant);
            float descendant_block_extent = layout_axis_size(
                descendant_block,
                vertical_writing ? LAYOUT_AXIS_X : LAYOUT_AXIS_Y);
            if (nested_descendants_local && !vertical_writing && descendant_block->blk &&
                layout_axis_has_given_size(descendant_block, false)) {
            ...
            if (descendant_block_extent > block_split_height &&
                !multicol_is_contained_monolithic(descendant_block)) {
                int descendant_fragment_count = multicol_fragment_count(
                    descendant_block_extent, block_split_height);
                MulticolFragmentGrid descendant_grid = multicol_fragment_grid(
                    descendant_fragment_count, column_count, false);
                int used_columns = descendant_grid.used_columns;
                int row_count = descendant_grid.row_count;
        ...
        if (preserve_escaping_spanner_descendants ||
            (descendant->is_block() && !descendant_fragmented)) {
...
static LayoutFragmentBox* multicol_append_layout_fragment(
    ...
    LayoutFragmentBox** previous, int fragment_index, int column_index,
    int row_index, float x, float y, float width, float height) {
    if (!pool || !elem || !first || !previous) return nullptr;
...
static LayoutFragmentBox* multicol_fragment_at(LayoutFragmentBox* first, int index) {
    for (int current = 0; first && current < index; current++) {
        first = first->next;
    }
    return first;
}
...
static void multicol_store_layout_fragments(
    ...
    for (int fi = 0; fi < fragment_count; fi++) {
        ...
        float fragment_position_y = 0.0f;
        if (vertical_writing) {
            // Fragment offsets follow the logical block/inline axes rather
            // than retaining the horizontal serializer's x/y convention.
            fragment_x = fi == 0 ? initial_fragment_offset : 0.0f;
            fragment_position_y = fragment_inline_origin +
                column_index * (column_width + column_gap) +
                row_index * (column_width + column_gap);
        } else {
            fragment_x = column_index * (column_width + column_gap);
            fragment_position_y = fragment_y +
                (fi == 0 ? initial_fragment_offset : 0.0f);
        }
        LayoutFragmentBox* fragment = multicol_append_layout_fragment(
            pool, elem, &first, &prev, fi, column_index, row_index,
            fragment_x, fragment_position_y, fragment_visual_width,
            fragment_piece_extent);
        if (!fragment) break;
...
static bool multicol_last_fragment_flow_offset(
    ...
    if (out_fragment_index) *out_fragment_index = 0;
    if (!child || !child->is_element() ||
        child->as_element()->layout_fragments_count() <= 1) {
        return false;
    }

    LayoutFragmentBox* last = child->as_element()->layout_fragment_list();
    while (last && last->next) last = last->next;
    if (!last) return false;

    float fragment_left = child->x + last->x;
    float fragment_right = fragment_left + last->width;
    float fragment_top = child->y + last->y;
    float last_flow_end = fragment_top;
    bool found_child = false;
    for (View* view = child->first_placed_child(); view; view = view->next()) {
        ViewBlock* child_block = lam::view_as_block(view);
        if (!child_block || layout_block_is_out_of_flow_positioned(child_block) ||
            child_block->view_type == RDT_VIEW_MARKER) {
            continue;
        }
        float child_left = child_block->x;
        float child_right = child_left + child_block->width;
        if (child_right <= fragment_left + 0.5f ||
            child_left >= fragment_right - 0.5f) {
            continue;
        }
        float child_end = child_block->y + child_block->height;
        if (!found_child || child_end > last_flow_end) {
            last_flow_end = child_end;
            found_child = true;
        }
    }
    if (!found_child) return false;

    last_flow_end += layout_axis_decoration_end(
        child->bound, LAYOUT_AXIS_Y);
    if (out_offset) {
        *out_offset = max(last_flow_end - fragment_top, 0.0f);
    }
    if (out_fragment_index) *out_fragment_index = last->fragment_index;
    return true;
}
...
static bool multicol_collect_grid_rows(
    ...
    for (View* view = grid->first_placed_child(); view; view = view->next()) {
        ViewBlock* item = multicol_in_flow_block(view);
        if (!item) continue;
...
static float multicol_fragment_grid_union_height(
    bool wraps_rows, int row_count, float fragment_height, float row_gap) {
    return wraps_rows
        ? row_count * fragment_height + (row_count - 1) * row_gap
        : fragment_height;
}
...
static bool multicol_fragment_grid_rows(
    ...
    int used_columns = 0;
    int row_count = 0;
    multicol_fragment_grid_dimensions(
        fragment_count, column_count, false, &layout_column_count,
        &used_columns, &row_count);
    float row_gap = multicol_row_gap(container);
    if (row_gap < 0.0f) row_gap = 0.0f;
    float union_height = multicol_fragment_grid_union_height(
        multicol_group_wraps_rows(container), row_count, used_fragment_height, row_gap);
    ...
    for (int row = 0; row < rows.row_count; row++) {
        if (row > 0 && rows.row_bottom[row] - fragment_start_top >
...
static bool multicol_fragment_abspos_in_context(
    ...
    ViewBlock* multicol = multicol_find_parent_container(containing_block);
    ...
    if (vertical_writing) {
        ...
        ViewBlock* parent_multicol = multicol_find_parent_container(
            multicol->parent_view());
        ...
        float static_block_offset = multicol_static_flow_offset(
            containing_block, child, LAYOUT_AXIS_X);
        ...
        int fragment_count = multicol_fragment_count(
            initial_block_offset + continuous_extent, fragment_capacity);
        if (fragment_count <= 1) return false;
        ...
        for (int fi = 0; fi < fragment_count && remaining_extent > 0.0f; fi++) {
            ...
            LayoutFragmentBox* fragment = multicol_append_layout_fragment(
                pool, child_elem, &first, &previous, fi,
                visual_parent_column, parent_column / parent_column_count,
                fragment_x, fragment_y, piece_extent, fragment_inline_size);
            if (!fragment) break;
        ...
        layout_set_view_geometry(child, min_x, min_y,
    ...
    if (parent_multicol && parent_multicol->multicol_prop()) {
        ...
        if (fragment_capacity > 0.0f &&
            ...
            float static_block_offset = multicol_static_flow_offset(
                containing_block, child, LAYOUT_AXIS_Y);
            ...
            if (multicol_store_nested_horizontal_fragments(
                    ...
                    &fragment_count)) {
                layout_set_view_geometry(child,
                    containing_block->x + min_x, containing_block->y + min_y,
                    max_x - min_x, max_y - min_y);
    ...
    int fragment_count = multicol_fragment_count(continuous_height, fragment_height);
    if (fragment_count <= 1) return false;
    ...
    for (int fi = 0; fi < fragment_count; fi++) {
        ...
        if (context_is_parent) {
            ...
            LayoutFragmentBox* parent_fragment = multicol_fragment_at(
                context_fragments, parent_index);
            ...
            LayoutFragmentBox* context_fragment = multicol_fragment_at(
                context_fragments, slot_index);
        ...
        LayoutFragmentBox* fragment = multicol_append_layout_fragment(
            pool, child_elem, &first, &previous, fi, column_index,
            row_index, local_x, local_y, fragment_visual_width, piece_height);
        if (!fragment) break;
        min_x = min(min_x, local_x);
...
struct MulticolInlineCbAbsChildFlags {
    bool has_direct_child;
    bool has_direct_inline_child;
};

static MulticolInlineCbAbsChildFlags multicol_inline_cb_abs_child_flags(
    ViewSpan* span) {
    MulticolInlineCbAbsChildFlags flags = {false, false};
    if (!span || !span->position) return flags;
    for (ViewBlock* positioned_child = span->positionp()->first_abs_child;
        ...
        flags.has_direct_child = true;
        if (layout_element_was_inline(
                static_cast<DomElement*>(positioned_child), false)) {
            flags.has_direct_inline_child = true;
        }
    }
    return flags;
}
...
static void multicol_snapshot_inline_cb_edges(View* view) {
    ...
    if (view->view_type == RDT_VIEW_INLINE) {
        ViewSpan* span = lam::view_require<RDT_VIEW_INLINE>(view);
        if (span->position && span->has_fragment_union(FRAGMENT_UNION_INLINE_CB)) {
            MulticolInlineCbAbsChildFlags flags =
                multicol_inline_cb_abs_child_flags(span);
            if (!flags.has_direct_inline_child) {
                float first_edge_y = 0.0f;
                if (multicol_inline_fragment_edge_y(view, false, &first_edge_y)) {
                    span->ensure_fragment_union(
                        FRAGMENT_UNION_INLINE_CB)->min_y = first_edge_y;
                }
            }
            float edge_y = 0.0f;
            if (multicol_inline_fragment_edge_y(view, true, &edge_y)) {
                span->ensure_fragment_union(FRAGMENT_UNION_INLINE_CB)->max_y = edge_y;
                span->set_has_inline_cb_edge_snapshot(true);
            }
        }
    }
    ...
    for (View* child = element->first_child; child; child = child->next_sibling) {
        multicol_snapshot_inline_cb_edges(child);
    }
}
...
static void multicol_normalize_vertical_inline_fragment_bounds(View* view) {
    ...
    if (view->view_type == RDT_VIEW_INLINE) {
        ...
        if (ancestor && layout_block_inline_axis_is_vertical(ancestor) &&
            ...
            MulticolInlineCbAbsChildFlags abs_child_flags =
                multicol_inline_cb_abs_child_flags(span);
            bool has_originally_inline_abs_child =
                abs_child_flags.has_direct_inline_child;
            bool has_direct_abs_child = abs_child_flags.has_direct_child;
            bool first_fragment_is_cb_start = !had_inline_cb ||
                (span->has_collapsed_line_fragment_union() &&
                 !has_originally_inline_abs_child);
            bool preserve_inline_cb_end = span->has_inline_cb_edge_snapshot();
            ...
            if (preserve_inline_cb_end) {
                // css position fragmentation: the containing-block end is
                // established before continuation text is reprojected.
            } else if (use_vertical_inline_edges && found_first_fragment && found_last_fragment) {
            ...
            if (!use_vertical_inline_edges && !preserve_inline_cb_end) {
...
static float multicol_fragmented_child_union(
    ...
    int fragment_count = multicol_fragment_count(
        initial_fragment_offset + item_height, fragment_capacity);
    bool overflow_columns = multicol_allows_overflow_columns(container);
    int layout_column_count = 0;
    int used_columns = 0;
    int row_count = 0;
    multicol_fragment_grid_dimensions(
        fragment_count, column_count, overflow_columns, &layout_column_count,
        &used_columns, &row_count);
    ...
    if (contained_monolithic_child_count > 0 && fragment_height > 0.0f) {
        ...
        if (last_fragment >= 0) {
            ...
            multicol_fragment_grid_dimensions(
                fragment_count, column_count, overflow_columns,
                &layout_column_count, &used_columns, &row_count);
        }
    }
    ...
    if (overflow_columns && multicol_inline_line_metrics(
        ...
        float line_box_height = multicol_resolve_line_advance(lycon, child);
    ...
    if (single_line_overflow) {
        ...
        if (fragment_count < 1) fragment_count = 1;
        multicol_fragment_grid_dimensions(
            fragment_count, column_count, true, &layout_column_count,
            &used_columns, &row_count);
    }
    ...
    if (vertical_writing) {
        ...
        union_height = multicol_fragment_grid_union_height(
            multicol_group_wraps_rows(container), row_count, fragment_height, row_gap);
        ...
        if (multicol_uses_fixed_wrapped_rows(container) &&
            container->multicol_prop()->fill == COLUMN_FILL_AUTO &&
            row_count > 1) {
            float last_piece_height = multicol_last_fragment_piece_height(
                initial_fragment_offset + item_height, fragment_capacity);
...
static void multicol_distribute_flow_group(
    ...
    for (int index = group_start; index < group_end; index++) {
        ...
        if (!content_handled && target_height > 0.0f &&
            ...
            int nested_fragment_count = multicol_fragment_count(
                cursor->block_offset + descendant_flow_extent, target_height);
            ...
            if (nested_fragment_count > 1) {
                int nested_row_count = multicol_fragment_row_count(
                    required_fragment_count, group->column_count);
        ...
        if (parallel_flow) {
            ...
            multicol_record_used_columns(
                group, parallel_used_columns, used_column_count);
        ...
        if (zero_height_fragmentainer && !zero_height_monolithic) {
            ...
            multicol_record_used_columns(
                group, zero_used_columns, used_column_count);
        ...
        if (!content_handled && should_fragment &&
            ...
            multicol_record_used_columns(
                group, used_columns, used_column_count);
...
static float multicol_split_child_around_spanners(
    ...
    if (has_only_nested_spanner_child && nested_spanner_child) {
        ...
        layout_set_view_geometry(child, nested_spanner_child->x,
            nested_wrapper ? 0.0f : original_child_y + original_child_height,
            nested_spanner_child->width, nested_spanner_child->height);
    ...
    if (non_spanner_count == 0 && spanner_extent <= 0.0f && !has_inline_text &&
        ...
        layout_set_view_geometry(child,
            original_child_x + continuation_column * (column_width + column_gap),
            child_fragment_origin_y + continuation_row *
                (fragmentainer_height + row_gap), original_child_width,
            min(original_child_height, fragmentainer_height));
    ...
    if (child->blk && child->block()->given_height >= 0.0f &&
        ...
        if (has_inline_wrapper) {
            ...
            layout_set_view_geometry(child, original_child_x, inline_origin_y,
                                     original_child_width, new_height);
...
static void multicol_init_mixed_inline_record(
    MulticolMixedInlineRecord* record, View* view, TextRect* rect) {
    if (!record || !view) return;
    record->text = rect ? lam::view_require<RDT_VIEW_TEXT>(view) : nullptr;
    record->rect = rect;
    record->br = rect ? nullptr : view;
    record->original_x = rect ? rect->x : view->x;
    record->original_y = rect ? rect->y : view->y;
    record->width = rect ? rect->width : 0.0f;
    record->height = rect ? rect->height : view->height;
    record->line_index = -1;
}
...
static void multicol_realign_inline_for_fragmented_float(
    ViewBlock* block,
    float float_x,
    float float_y,
    float float_height,
    float fragment_width,
    float fragment_pitch
) {
    if (!block || fragment_width <= 0.0f || fragment_pitch <= 0.0f ||
        !block->blk) return;

    constexpr int MAX_FLOAT_INLINE_RECORDS = 512;
    MulticolMixedInlineRecord records[MAX_FLOAT_INLINE_RECORDS] = {};
    int record_count = 0;
    multicol_collect_mixed_inline_records(
        block->first_placed_child(), records, &record_count, false);
    if (record_count == 0) return;

    float local_float_left = float_x - block->x;
    float local_float_top = float_y - block->y;
    float local_float_bottom = local_float_top + float_height;
    int float_fragment = (int)floorf(
        local_float_left / fragment_pitch); // INT_CAST_OK: non-negative fragment index
    if (float_fragment < 0) float_fragment = 0;
    CssEnum text_align = layout_resolve_text_align(
        block->block()->text_align, block->block()->direction);
    if (text_align != CSS_VALUE_CENTER && text_align != CSS_VALUE_RIGHT) return;

    for (int i = 0; i < record_count; i++) {
        MulticolMixedInlineRecord& line = records[i];
        float line_min_x = line.original_x;
        float line_max_x = line.original_x + line.width;
        float line_top = line.original_y;
        float line_bottom = line.original_y + line.height;
        int line_end = i + 1;
        while (line_end < record_count &&
               fabsf(records[line_end].original_y - line.original_y) <= 1.0f) {
            line_min_x = min(line_min_x, records[line_end].original_x);
            line_max_x = max(line_max_x,
                records[line_end].original_x + records[line_end].width);
            line_bottom = max(line_bottom,
                records[line_end].original_y + records[line_end].height);
            line_end++;
        }
        int line_fragment = (int)floorf(
            line_min_x / fragment_pitch); // INT_CAST_OK: non-negative fragment index
        if (line_fragment != float_fragment ||
            line_bottom <= local_float_top + 0.5f ||
            line_top >= local_float_bottom - 0.5f) {
            i = line_end - 1;
            continue;
        }
        float fragment_left = float_fragment * fragment_pitch;
        float available_right = min(
            fragment_left + fragment_width, local_float_left);
        float line_width = line_max_x - line_min_x;
        if (available_right <= fragment_left + line_width + 0.5f) {
            i = line_end - 1;
            continue;
        }
        float target_x = text_align == CSS_VALUE_CENTER
            ? fragment_left + (available_right - fragment_left - line_width) * 0.5f
            : available_right - line_width;
        float shift = target_x - line_min_x;
        for (int ri = i; ri < line_end; ri++) {
            if (records[ri].rect) {
                records[ri].rect->x += shift;
            } else if (records[ri].br) {
                records[ri].br->x += shift;
            }
...
static bool multicol_collect_mixed_inline_records(
    ...
    for (View* current = first; current; current = current->next()) {
        if (!multicol_collect_mixed_inline_view(
                current, records, record_count, reject_blocks)) {
            return false;
        }
    }
    return true;
...
static bool multicol_collect_mixed_inline_view(
    ...
    if (current->view_type == RDT_VIEW_TEXT) {
        ...
        for (TextRect* rect = text->rect; rect; rect = rect->next) {
            ...
            multicol_init_mixed_inline_record(&record, current, rect);
            (*record_count)++;
        }
        return true;
    }
    if (current->view_type == RDT_VIEW_BR) {
        MulticolMixedInlineRecord& record = records[*record_count];
        multicol_init_mixed_inline_record(&record, current, nullptr);
        (*record_count)++;
        return true;
    }
    if (current->view_type == RDT_VIEW_INLINE) {
        ...
        if (!multicol_collect_mixed_inline_records(
                span->first_child, records, record_count, reject_blocks)) {
            return false;
        }
        return true;
    }
    ...
    return reject_blocks ? !lam::view_as_block(current) : true;
}
...
static bool multicol_reflow_mixed_direct_flow(
    ...
    float* out_height,
    int* out_used_columns,
    float target_height_override,
    int initial_fragment_index,
    float initial_fragment_offset,
    bool normalize_inline_origin,
    int* out_last_fragment,
    float* out_end_offset
) {
    if (out_height) *out_height = 0.0f;
    if (out_used_columns) *out_used_columns = 0;
    if (out_last_fragment) *out_last_fragment = initial_fragment_index;
    if (out_end_offset) *out_end_offset = initial_fragment_offset;
    ...
    auto append_inline_item = [&](int record_start) -> bool {
        ...
        line_advance = multicol_resolve_line_advance(
            lycon, block, line_advance, lines[line_start].line_height);
    ...
    for (DomNode* child = block->first_child; child; child = child->next_sibling) {
        ...
        if (child_block) {
            ...
            item.flow.can_fragment = multicol_has_fragmentable_line_boxes(child_block) ||
                multicol_has_fragmentable_block_children(child_block) ||
                multicol_has_mixed_direct_flow(child_block);
            if (child_block->blk &&
                child_block->block()->break_inside == CSS_VALUE_AVOID) {
            ...
            item.flow.parallel_flow = layout_position_is_floated(child_block->position) &&
                child_block->parent == static_cast<DomNode*>(block);
    ...
    float target_height = target_height_override > 0.0f
        ? target_height_override
        : multicol_group_target_height(
            block, ceilf(total_height / column_count), total_height);
    if (target_height_override <= 0.0f) {
        target_height = multicol_balanced_target_search(
    ...
    int fragment_index = initial_fragment_index;
    float fragment_used = max(initial_fragment_offset, 0.0f);
    float max_height = 0.0f;
    int used_columns = 0;
    int previous_parallel_fragment = -1;
    int active_parallel_fragment = -1;
    float previous_parallel_x = 0.0f;
    float previous_parallel_y = 0.0f;
    float previous_parallel_height = 0.0f;
    bool previous_item_inline_flow = false;
    float previous_inline_advance = 0.0f;
        ...
        if (item.inline_flow) {
            for (int li = 0; li < item.line_count; li++) {
                ...
                float line_y = content_start_y + row * (target_height + row_gap) +
                    fragment_used;
                bool realign_after_parallel_float =
                    previous_parallel_fragment >= 0 &&
                    fragment_index != previous_parallel_fragment;
                float realign_delta = 0.0f;
                if (realign_after_parallel_float && block->blk) {
                    float line_min_x = FLT_MAX;
                    float line_max_x = -FLT_MAX;
                    for (int ri = 0; ri < record_count; ri++) {
                        MulticolMixedInlineRecord& record = records[ri];
                        if (record.line_index != item.line_start + li) continue;
                        float record_width = record.rect ? record.rect->width : 0.0f;
                        line_min_x = min(line_min_x, record.original_x);
                        line_max_x = max(line_max_x,
                            record.original_x + record_width);
                    }
                    if (line_min_x < line_max_x) {
                        ...
                        if (text_align == CSS_VALUE_CENTER ||
                            ...
                            float decoration_extent =
                                layout_axis_decoration_start(
                                    block->bound ? block->boundary() : nullptr,
                                    LAYOUT_AXIS_X) +
                                layout_axis_decoration_end(
                                    block->bound ? block->boundary() : nullptr,
                                    LAYOUT_AXIS_X);
                            float fragment_inline_width = max(
                                column_width - decoration_extent, 0.0f);
                            float line_width = line_max_x - line_min_x;
                            float target_x = normalize_inline_origin
                                ? 0.0f : column_group_origin_x;
                            target_x += text_align == CSS_VALUE_CENTER
                                ? (fragment_inline_width - line_width) * 0.5f
                                : fragment_inline_width - line_width;
                            realign_delta = target_x - line_min_x;
                        }
                    }
                }
                for (int ri = 0; ri < record_count; ri++) {
                    ...
                    float record_x = record.original_x + realign_delta;
                    int relative_column = normalize_inline_origin
                        ? column - initial_fragment_index : column;
                    if (normalize_inline_origin) {
                        record_x += relative_column * pitch;
                    }
                    float projected_x = normalize_inline_origin
                        ? record_x : record_x + column * pitch;
                    multicol_set_inline_position(
                        record.rect, record.br, projected_x, line_y + line_delta);
                }
                fragment_used += item.line_advance;
                if (realign_after_parallel_float) previous_parallel_fragment = -1;
            }
            previous_item_inline_flow = true;
            previous_inline_advance = item.line_advance;
        } else {
            float item_height = item.flow.height;
            ViewBlock* child_block = item.flow.block;
            if (item.flow.parallel_flow) {
                float original_float_x = child_block->x;
                int float_fragment = fragment_index;
                float float_offset = fragment_used + item.flow.margin_before -
                    (previous_item_inline_flow ? previous_inline_advance : 0.0f);
                if (float_offset < 0.0f) float_offset = 0.0f;
                if (float_offset > 0.0f &&
                    float_offset + item.flow.content_height > target_height + 0.5f) {
                    float_fragment++;
                    float_offset = 0.0f;
                }
                int float_column = float_fragment % column_count;
                int float_row = float_fragment / column_count;
                int relative_column = normalize_inline_origin
                    ? float_column - initial_fragment_index : float_column;
                float decoration_extent =
                    layout_axis_decoration_start(
                        block->bound ? block->boundary() : nullptr,
                        LAYOUT_AXIS_X) +
                    layout_axis_decoration_end(
                        block->bound ? block->boundary() : nullptr,
                        LAYOUT_AXIS_X);
                float float_inline_offset = column_width - decoration_extent -
                    child_block->width;
                if (normalize_inline_origin) {
                    float_inline_offset = original_float_x;
                }
                if (float_inline_offset < 0.0f) float_inline_offset = 0.0f;
                child_block->x = (normalize_inline_origin ? 0.0f : column_group_origin_x) +
                    relative_column * pitch + float_inline_offset;
                child_block->y = content_start_y +
                    float_row * (target_height + row_gap) + float_offset;
                multicol_clear_layout_fragments(child_block);
                if (float_column + 1 > used_columns) {
                    used_columns = float_column + 1;
                }
                previous_parallel_fragment = float_fragment;
                active_parallel_fragment = float_fragment;
                previous_parallel_x = child_block->x;
                previous_parallel_y = child_block->y;
                previous_parallel_height = child_block->height;
                previous_item_inline_flow = false;
            ...
            previous_item_inline_flow = false;
            bool fragment_child_flow = child_block && item.flow.can_fragment &&
                multicol_has_mixed_direct_flow(child_block) &&
                item.flow.break_before_column == false &&
                item.flow.break_after_column == false;
            bool fragment_block_children = child_block && item.flow.can_fragment &&
                multicol_has_fragmentable_block_children(child_block) &&
                item.flow.break_before_column == false &&
                item.flow.break_after_column == false;
            if (fragment_child_flow) {
                int child_start_fragment = fragment_index;
                float child_start_offset = fragment_used + item.flow.margin_before;
                int child_last_fragment = child_start_fragment;
                float child_end_offset = child_start_offset;
                float child_height = 0.0f;
                int child_used_columns = 0;
                float original_child_width = child_block->width;
                bool child_reflowed = multicol_reflow_mixed_direct_flow(
                    lycon, child_block, column_count, column_width, column_gap,
                    0.0f, 0.0f, &child_height,
                    &child_used_columns, target_height, child_start_fragment,
                    child_start_offset, true, &child_last_fragment,
                    &child_end_offset);
                if (child_reflowed) {
                    int start_row = child_start_fragment / column_count;
                    int end_row = child_last_fragment / column_count;
                    int start_column = child_start_fragment % column_count;
                    int end_column = child_last_fragment % column_count;
                    int span_columns = end_row == start_row
                        ? end_column - start_column + 1 : column_count;
                    if (span_columns < 1) span_columns = 1;
                    float child_fragment_width = original_child_width > 0.0f
                        ? original_child_width : column_width;
                    for (View* child_view = child_block->first_placed_child();
                         child_view; child_view = child_view->next()) {
                        ViewBlock* nested_block = lam::view_as_block(child_view);
                        if (nested_block && !layout_block_is_out_of_flow_positioned(
                                nested_block)) {
                            child_fragment_width = max(child_fragment_width,
                                nested_block->width);
                        }
                    }
                    bool definite_height = child_block->blk &&
                        layout_axis_has_given_size(child_block, false);
                    float given_height = definite_height
                        ? layout_axis_given_size(child_block->block(), LAYOUT_AXIS_Y)
                        : -1.0f;
                    bool principal_box_overflow = definite_height &&
                        given_height < child_height;
                    if (principal_box_overflow) {
                        // css fragmentation: overflowing content advances through
                        // fragments while a definite principal box keeps its
                        // authored size at the final fragmentainer.
                        int principal_row = child_last_fragment / column_count;
                        int principal_column = child_last_fragment % column_count;
                        float given_width = layout_axis_has_given_size(
                            child_block, true)
                            ? layout_axis_given_size(child_block->block(), LAYOUT_AXIS_X)
                            : (original_child_width > 0.0f
                                ? original_child_width : column_width);
                        float original_child_x = column_group_origin_x +
                            start_column * pitch;
                        float original_child_y = content_start_y +
                            start_row * (target_height + row_gap);
                        child_block->x = column_group_origin_x +
                            principal_column * pitch;
                        child_block->y = content_start_y +
                            principal_row * (target_height + row_gap);
                        child_block->width = layout_used_border_box_size(
                            child_block, given_width, true);
                        child_block->height = layout_used_border_box_size(
                            child_block, given_height, false);
                        float principal_shift_x = child_block->x - original_child_x;
                        float principal_shift_y = child_block->y - original_child_y;
                        for (View* descendant = child_block->first_placed_child();
                             descendant; descendant = descendant->next()) {
                            // keep overflowing descendants at their fragment
                            // coordinates when the principal box moves.
                            descendant->x -= principal_shift_x;
                            descendant->y -= principal_shift_y;
                            if (descendant->view_type == RDT_VIEW_TEXT) {
                                layout_shift_text_rects(
                                    lam::view_require<RDT_VIEW_TEXT>(descendant),
                                    -principal_shift_x, -principal_shift_y);
                            }
                        }
                    } else {
                        child_block->x = column_group_origin_x +
                        ...
                        child_block->y = content_start_y +
                            start_row * (target_height + row_gap);
                        child_block->width = child_fragment_width +
                            (span_columns - 1) * pitch;
                        child_block->height = max(child_block->height,
                            target_height);
                    }
                    if (active_parallel_fragment == child_start_fragment) {
                        float decoration_extent =
                            layout_axis_decoration_start(
                                block->bound ? block->boundary() : nullptr,
                                LAYOUT_AXIS_X) +
                            layout_axis_decoration_end(
                                block->bound ? block->boundary() : nullptr,
                                LAYOUT_AXIS_X);
                        multicol_realign_inline_for_fragmented_float(
                            child_block, previous_parallel_x, previous_parallel_y,
                            previous_parallel_height,
                            max(column_width - decoration_extent, 0.0f), pitch);
                    }
                    child_block->content_height = child_block->height;
                    fragment_index = child_last_fragment;
                    fragment_used = child_end_offset + item.flow.margin_after;
                    if (child_used_columns > used_columns) {
                        used_columns = child_used_columns;
                    }
                    continue;
                }
            }
            if (fragment_block_children) {
                if (fragment_used > 0.0f &&
                    fragment_used + item.flow.margin_before > target_height + 0.5f) {
                    advance_fragment();
                }
                int column = fragment_column();
                int row = fragment_row();
                int relative_column = normalize_inline_origin
                    ? column - initial_fragment_index : column;
                child_block->x = (normalize_inline_origin ? 0.0f : column_group_origin_x) +
                    relative_column * pitch;
                child_block->y = content_start_y + row * (target_height + row_gap) +
                    fragment_used + item.flow.margin_before;
                int child_used_columns = 1;
                // css fragmentation: a block container breaks between its
                // in-flow children while retaining the wrapper's fragment union.
                multicol_fragmented_child_union(
                    lycon, block, child_block, item.flow.content_height,
                    target_height, column_count, column_width, column_gap,
                    fragment_used + item.flow.margin_before,
                    &child_used_columns, nullptr);
                if (child_used_columns > used_columns) {
                    used_columns = child_used_columns;
                }
                float child_end_offset = 0.0f;
                int child_last_fragment = 0;
                if (multicol_last_fragment_flow_offset(
                        child_block, &child_end_offset, &child_last_fragment)) {
                    fragment_index += child_last_fragment;
                    fragment_used = child_end_offset + item.flow.margin_after;
                } else {
                    fragment_used += item.flow.margin_before +
                        item.flow.content_height;
                }
                continue;
            }
            if (fragment_used > 0.0f &&
            ...
            child_block->y = content_start_y + row * (target_height + row_gap) +
                fragment_used + item.flow.margin_before;
            if (active_parallel_fragment == fragment_index &&
                !child_block->first_placed_child() &&
                previous_parallel_y < child_block->y + child_block->height &&
                previous_parallel_y + previous_parallel_height > child_block->y) {
                float margin_start = layout_axis_margin_start(
                    child_block->bound, LAYOUT_AXIS_X);
                float available_right = previous_parallel_x;
                float adjusted_x = child_block->x + margin_start;
                if (available_right > adjusted_x + 0.5f &&
                    child_block->width > available_right - adjusted_x) {
                    // css 2.1 §9.5.1: an empty block's auto width is reduced
                    // to the line space remaining beside an overlapping float.
                    child_block->x = adjusted_x;
                    child_block->width = available_right - adjusted_x;
                }
            }
...
static float multicol_project_mixed_direct_inline_content(
    ...
    for (int li = 0; li < line_count; li++) {
        ...
        bool should_break = multicol_should_break_line_for_widows_orphans(
            line_slot, visual_bottom, target_height, li, line_count,
            widows, orphans, fragment_start_offset +
            (line_slot + 1) * line_advance + line_offset + visual_height);
...
static bool multicol_text_has_visible_rect(DomText* text) {
    ...
    for (TextRect* rect = text->rect; rect; rect = rect->next) {
        if (layout_text_rect_is_visible(
                lam::view_require_text(static_cast<View*>(text)), rect)) return true;
    }
    return false;
}
...
static bool multicol_should_break_line_for_widows_orphans(
    ...
    if (!should_break && widows > 1 && line_index + 1 < line_count) {
        int remaining_after_this = line_count - (line_index + 1);
        int remaining_with_this = line_count - line_index;
...
static void multicol_record_used_columns(
    ...
    if (out_used_columns && used_columns > *out_used_columns) {
        *out_used_columns = used_columns;
    }
}
...
void layout_multicol_content(LayoutContext* lycon, ViewBlock* block) {
    ...
    while (i < block_count) {
        ...
        if (multicol_uses_fixed_balanced_rows(block)) {
            ...
            if (row_height > 0.0f && group_extent > 0.0f) {
                ...
                int required_slot_count = multicol_fragment_count(
                    group_total_height, row_height);
                int row_count = multicol_fragment_row_count(
                    required_slot_count, group.column_count);
    ...
    bool handled_mixed_direct_flow = !has_text_box_trim &&
        multicol_should_reflow_mixed_direct_flow(block) &&
        multicol_reflow_mixed_direct_flow(
    ...
    multicol_mirror_rtl_horizontal_text_descendants(block);
