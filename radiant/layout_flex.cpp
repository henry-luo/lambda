...
void layout_flex_container(LayoutContext* lycon, ViewBlock* container) {
    ...
    if (is_main_axis_horizontal(flex_layout)) {
        ...
        for (int i = 0; i < line_count; i++) {
            // For column flex, sum up item heights (main sizes) INCLUDING margins
            ...
            for (int j = 0; j < line->item_count; j++) {
                ...
                if (item) {
                    ...
                    if (item->bound) {
                        total_line_main += (int)(item->bound->margin.top + item->bound->margin.bottom);
                    }
                }
            }
            // Add gaps between items
        ...
        // Phase 7 (Column): Update container cross-axis (width) to max outer item width.
        // Only applies to shrink-to-fit containers (abs-pos / fixed with no explicit width).
        // Per CSS §10.3.7 shrink-to-fit: the cross-size of the flex container equals the
        // largest outer cross size of its flex items (item width + horizontal margins).
        // Normal-flow column flex items get their width from the containing block — items
        // must NOT expand the container in that case.
        {
            ...
            for (int i = 0; i < line_count; i++) {
                ...
                for (int j = 0; j < line->item_count; j++) {
                    ...
                    if (item) {
                        ...
                        if (item_width > max_outer_cross) max_outer_cross = item_width;
                    }
                }
            }
            if (max_outer_cross > 0.0f && max_outer_cross > flex_layout->cross_axis_size) {
                log_debug("Phase 7: (Column) Updating cross_axis_size from %.1f to %.1f (max outer item width with margins)",
                         flex_layout->cross_axis_size, max_outer_cross);
                flex_layout->cross_axis_size = max_outer_cross;
                float padding_border_width = 0.0f;
                if (container->bound) {
                    padding_border_width += container->bound->padding.left + container->bound->padding.right;
                ...
                container->width = (int)(max_outer_cross + padding_border_width);
            }
...
float calculate_hypothetical_main_size(ViewElement* item, FlexContainerLayout* flex_layout) {
    ...
    if (is_horizontal) {
        min_main = (float)item->fi->resolved_min_width;
        if (item->fi->resolved_max_width > 0 && item->fi->resolved_max_width < INT_MAX) {
            max_main = (float)item->fi->resolved_max_width;
        }
    } else {
        min_main = (float)item->fi->resolved_min_height;
        if (item->fi->resolved_max_height > 0 && item->fi->resolved_max_height < INT_MAX) {
            max_main = (float)item->fi->resolved_max_height;
        }
    }
    ...
    if (min_width_is_auto) {
        if (is_horizontal) {
            ...
            if (overflow_not_visible && item->scroller->overflow_x != CSS_VALUE_VISIBLE) {
                ...
                int content_suggestion = item->fi->intrinsic_width.min_content;
    ...
    if (min_height_is_auto) {
        if (!is_horizontal) {
            ...
            if (overflow_not_visible && item->scroller->overflow_y != CSS_VALUE_VISIBLE) {
                ...
                int content_suggestion = item->fi->intrinsic_height.min_content;
    ...
    if (item->fi && item->fi->aspect_ratio > 0) {
        ...
        if (is_stretch && flex_layout && flex_layout->cross_axis_size > 0) {
            ...
            if (is_horizontal && min_width_is_auto && !(has_css_width && item->blk->given_width > 0)) {
                ...
                if (max_height > 0 && max_height < INT_MAX && effective_cross > max_height) {
                    effective_cross = (float)max_height;
                }
                if (min_height > 0 && effective_cross < min_height) {
                    effective_cross = (float)min_height;
                }
                // min_width >= effective_cross * ratio
                int transferred_min = (int)(effective_cross * r);
                // Clamp by main-axis max (max-width) to avoid exceeding it
                if (max_width > 0 && max_width < INT_MAX && transferred_min > max_width) {
                ...
                if (max_width > 0 && max_width < INT_MAX && effective_cross > max_width) {
                    effective_cross = (float)max_width;
                }
                if (min_width > 0 && effective_cross < min_width) {
                    effective_cross = (float)min_width;
                }
                // min_height >= effective_cross / ratio
                int transferred_min = (int)(effective_cross / r);
                // Clamp by main-axis max (max-height) to avoid exceeding it
                if (max_height > 0 && max_height < INT_MAX && transferred_min > max_height) {
...
void reposition_baseline_items(LayoutContext* lycon, ViewBlock* flex_container) {
    ...
    for (int line_idx = 0; line_idx < flex_layout->line_count; line_idx++) {
        ...
        if (has_baseline) {
            ...
            int new_cross = (int)(baseline_extent > max_non_baseline ? baseline_extent : max_non_baseline);
            if (new_cross > line->cross_size) {
                log_debug("Baseline line %d cross size %d -> %d (pre=%.0f, post=%.0f)",
...
void resolve_flexible_lengths(FlexContainerLayout* flex_layout, FlexLineInfo* line) {
    ...
    while (iteration < MAX_ITERATIONS) {
        ...
        //
        // For ROW FLEX with indefinite main axis (shrink-to-fit), shrinking IS allowed:
        // The shrink-to-fit width is computed from items' max-content contributions (§9.9.1),
        // giving a REAL container size (possibly 0 for contentless items). Items must shrink
        // to fit within this computed width.
        ...
        if (top_auto || bottom_auto) {
            ...
            switch (effective_align) {
                ...
                case CSS_VALUE_END:  // Handle 'end' same as 'flex-end'
                    cross_pos = available_cross_size - item_cross_size - (int)margin_cross_end;
                    break;
                case ALIGN_CENTER:
                    cross_pos = (int)margin_cross_start +
                        (available_cross_size - item_cross_size - (int)(margin_cross_start + margin_cross_end)) / 2;
                    break;
                case ALIGN_STRETCH:
...
void calculate_line_cross_sizes(FlexContainerLayout* flex_layout) {
    ...
    for (int i = 0; i < flex_layout->line_count; i++) {
        ...
        line->cross_size = (int)line_cross;
        log_debug("LINE_CROSS_SIZE: line %d = %d (baseline_extent=%.1f, max_non_baseline=%.1f, has_baseline=%d)",
                  i, (int)line_cross, baseline_extent, max_non_baseline_cross, has_baseline_items);
    }
}
                ...
                if (item->blk && item->blk->given_height >= 0) {
                    ...
                    if (measured_height > 0) {
                        ...
                        if (text_height_at_width > 0) {
                            ...
                            item->height = (int)hypothetical_cross;
                            item->content_height = (int)text_height_at_width;
