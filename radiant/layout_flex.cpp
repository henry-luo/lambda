    ...
    *lines = NULL;
    ...
    for (int i = 0; i < layout_count; i++) {
        ...
        if (container->wrap == WRAP_NOWRAP || remaining_space >= space_needed) {
            current_line.items = (FlexItem**)realloc(current_line.items, (current_line.item_count + 1) * sizeof(FlexItem*));
            ...
            if (current_line.item_count > 0) {
                *lines = (FlexLine*)realloc(*lines, (*line_count + 1) * sizeof(FlexLine));
                (*lines)[(*line_count)++] = current_line;
                current_line = (FlexLine){ .items = NULL, .item_count = 0, .total_base_size = 0, .height = 0 };
            }
            current_line.items = (FlexItem**)realloc(current_line.items, sizeof(FlexItem*));
...
void layout_flex_container(FlexContainer* container) {
    ...
    FlexItemWithIndex* items_with_indices = (FlexItemWithIndex*)malloc(container->item_count * sizeof(FlexItemWithIndex));
    FlexItem* layout_items = (FlexItem*)malloc(container->item_count * sizeof(FlexItem));
    int layout_count = 0;
