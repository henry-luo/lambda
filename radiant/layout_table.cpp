...
static float table_inter_row_spacing(ViewTable* table);
...
struct TableCellVerticalBounds {
    float min_y;
    float max_y;
    bool has_content;
};

static TableCellVerticalBounds table_cell_vertical_bounds(ViewTableCell* tcell,
                                                          bool include_margins = false) {
    TableCellVerticalBounds bounds = {1e9f, 0.0f, false};
...
static void apply_cell_vertical_align(LayoutContext* lycon, ViewTableCell* tcell, float cell_height, float content_height) {
    ...
    TableCellVerticalBounds bounds = table_cell_vertical_bounds(tcell, true);
...
static void table_position_row_group_box(ViewTable* table, TableMetadata* meta,
    ...
    if (table->tb->border_collapse) {
        ...
        float column_spacing = table_inter_column_spacing(table);
...
static void table_apply_fixed_height_distribution(LayoutContext* lycon, ViewTable* table, int rows) {
    ...
    float row_spacing = table_inter_row_spacing(table);
...
static float table_inter_row_spacing(ViewTable* table) {
    if (!table || !table->tb || table->tb->border_collapse) return 0.0f;
    return table->tb->border_spacing_v > 0.0f ? table->tb->border_spacing_v : 0.0f;
}
...
static void distribute_rowspan_heights(ViewTable* table, TableMetadata* meta) {
    ...
    for (int i = 0; i < rowspan_cells->length; i++) {
        ...
        for (int r = rsc->start_row; r < rsc->end_row; r++) {
            current_total += meta->row_heights[r];
            if (row_spacing > 0.0f && r < rsc->end_row - 1) {
                current_total += row_spacing;
            }
        }
...
// Append any DOM node; table repair moves text and elements through the same
// sibling links, so keeping one append primitive avoids a second relinking path.
...
static void reapply_rowspan_vertical_alignment(ViewTableCell* tcell) {
    ...
    TableCellVerticalBounds bounds = table_cell_vertical_bounds(tcell);
    if (!bounds.has_content) return;
...
static void layout_table_cell_content(LayoutContext* lycon, ViewBlock* cell, ViewBlock* table) {
    ...
    if (lycon->block.given_height < 0 && table && table->blk && table->block_mut()->given_height > 0) {
        ...
        float row_spacing = table_inter_row_spacing(parent_table);
        if (row_spacing > 0.0f) {
