...
static float table_direct_right_float_intrusion(ViewTable* table, float y, float table_width) {
    ...
    for (View* child = table->first_child; child; child = child->next_sibling) {
        if (!child->is_block()) continue;
        ViewBlock* floating = lam::view_require_block(child);
        if (!floating->position || floating->position->float_prop != CSS_VALUE_RIGHT) continue;
        if (!table_direct_float_overlaps_y(floating, table, y)) continue;
...
float find_first_baseline_recursive(LayoutContext* lycon, View* parent, float cumulative_y, bool use_normal_lh) {
    for (View* child = lam::view_require_element(parent)->first_child; child; child = child->next_sibling) {
        ...
        if (is_table_structure) {
            ...
            if (child->view_type == RDT_VIEW_TABLE_ROW) {
                float row_baseline = find_table_row_baseline(lycon, lam::view_require<RDT_VIEW_TABLE_ROW>(child));
                if (row_baseline >= 0) return cumulative_y + child->y + row_baseline;
            }
...
static void apply_fixed_row_height(LayoutContext* lycon, ViewTableRow* trow, float fixed_height) {
    ...
    for (ViewTableCell* cell = trow->first_cell(); cell; cell = trow->next_cell(cell)) {
        if (cell->height < fixed_height) {
            ...
            apply_cell_vertical_align(lycon, cell, fixed_height, content_height);
        }
    }
}
...
static void update_rowspan_cell_heights(ViewTable* table, TableMetadata* meta) {
    ...
    for (ViewTableRow* row = table->first_row(); row; row = table->next_row(row)) {
        for (ViewTableCell* tcell = row->first_cell(); tcell; tcell = row->next_cell(tcell)) {
            if (!tcell->td || tcell->td->row_span <= 1) continue;
            ...
            if (spanned_height <= 0.0f) continue;
...
static void update_single_row_cells_after_height_change(LayoutContext* lycon, ViewTableRow* trow,
    ...
    for (ViewTableCell* tcell = trow->first_cell(); tcell; tcell = trow->next_cell(tcell)) {
        ...
        apply_cell_vertical_align(lycon, tcell, tcell->height, content_h);
    }
}
    ...
    for (int i = 0; i < ordered_elements->length; i++) {
        ...
        if (child->view_type == RDT_VIEW_TABLE_ROW_GROUP) {
            ...
            for (View* row_view = child->first_child; row_view; row_view = row_view->next_sibling) {
                ...
                if (!row || row->view_type != RDT_VIEW_TABLE_ROW) continue;
                ...
                update_single_row_cells_after_height_change(
                    lycon, lam::view_require<RDT_VIEW_TABLE_ROW>(row), row_height);
...
static float table_cell_width_constraint_border_box(ViewTableCell* cell, float css_width,
    ...
    if (cell->blk->box_sizing == CSS_VALUE_BORDER_BOX) return css_width;
...
static TableMetadata* analyze_table_structure(LayoutContext* lycon, ViewTable* table) {
    ...
    for (ViewTableRow* row = table->first_row(); row; row = table->next_row(row)) {
        rows++;
        int row_cells = 0;
        for (View* child = static_cast<View*>(row->first_child); child; child = static_cast<View*>(child->next_sibling)) {
            if (child->view_type == RDT_VIEW_TABLE_CELL) {
                ViewTableCell* cell = lam::view_require<RDT_VIEW_TABLE_CELL>(child);
                row_cells += cell->td->col_span;
            } else if (is_out_of_flow_table_cell_slot(child)) {
                row_cells++;
            }
        }
            ...
            for (ViewTableRow* row = table->first_row(); row; row = table->next_row(row)) {
                ...
                for (View* child = static_cast<View*>(row->first_child); child; child = static_cast<View*>(child->next_sibling)) {
                    if (is_out_of_flow_table_cell_slot(child)) {
                        ...
                        if (col < est_cols) occupied[cur_row * est_cols + col] = true;
                        int right = col + 1;
                        if (right > max_col_used) max_col_used = right;
                        col++;
                        continue;
                    }
                    if (child->view_type != RDT_VIEW_TABLE_CELL) continue;
    ...
    for (ViewTableRow* row = table->first_row(); row; row = table->next_row(row)) {
        ...
        for (View* child = static_cast<View*>(row->first_child); child; child = static_cast<View*>(child->next_sibling)) {
            if (is_out_of_flow_table_cell_slot(child)) {
                ...
                if (col < columns) {
                    meta->grid(current_row, col) = true;
                }
                col++;
                continue;
            }
...
void table_auto_layout(LayoutContext* lycon, ViewTable* table) {
    ...
    if (explicit_table_width > 0) {
        ...
        if (table_width_is_border_box && table->bound && table->bound->border) {
            table_content_width -= (table->bound->border->width.left + table->bound->border->width.right);
    ...
    for (ViewTableRow* row = table->first_row(); row; row = table->next_row(row)) {
        for (ViewTableCell* tcell = row->first_cell(); tcell; tcell = row->next_cell(tcell)) {
            ...
            if (cell_width_percent > 0.0f && col >= 0 && col < meta->column_count) {
                ...
                for (int c = col; c < col + span && c < meta->column_count; c++) {
    ...
    for (int c = 0; c < columns; c++) {
        ...
        if (!col_elem) {
            ...
            const char* span_str = col_elem->get_attribute("span");
        ...
        if (col_css_width > 0 && width_divisor > 1.0f) {
            col_css_width /= width_divisor;
        }
        if (col_css_width > 0) {
        ...
        if (col_min_w > 0 && width_divisor > 1.0f) {
            col_min_w /= width_divisor;
        }
        if (col_min_w > 0) {
        ...
        if (col_max_w >= 0 && width_divisor > 1.0f) {
            col_max_w /= width_divisor;
        }
        if (col_max_w >= 0) {
            ...
            if (dom_elem->specified_style) {
                ...
                if (width_decl && width_decl->value) {
                    ...
                    // Handle length value
                        ...
                        float resolved_width = resolve_length_value(lycon, CSS_PROPERTY_WIDTH, width_decl->value);
                        fixed_explicit_width = resolved_width;
                        log_debug("FIXED LAYOUT - read table CSS width: %.1fpx", fixed_explicit_width);
                    }
                }
            }
        }
      ...
      if (fixed_explicit_width > 0) {
        ...
        if (table->node_type == DOM_NODE_ELEMENT) {
            ...
            if (dom_elem->specified_style) {
                ...
                if (height_decl && height_decl->value) {
                    ...
                    if (resolved_height > 0) {
                        explicit_table_height = resolved_height;
                        log_debug("FIXED LAYOUT - read table CSS height: %.1fpx (resolved)", explicit_table_height);
                    }
                }
            }
            ...
            if (explicit_table_height <= 0 && table->blk && table->blk->given_height > 0) {
                explicit_table_height = table->blk->given_height;
                log_debug("FIXED LAYOUT - read table HTML height attribute: %.1fpx", explicit_table_height);
            }
        }
        ...
        if (explicit_table_height > 0) {
            ...
            if (height_is_border_box && table->bound && table->bound->border) {
                content_height -= (table->bound->border->width.top + table->bound->border->width.bottom);
            }
            ...
            if (!table->tb->border_collapse && table->tb->border_spacing_v > 0 && total_rows > 0) {
                content_height -= (total_rows + 1) * table->tb->border_spacing_v;
            ...
            float height_per_row = total_rows > 0 ? content_height / total_rows : 0;
    ...
    float total_percent_col_width = 0.0f;
    for (int i = 0; i < columns; i++) {
        total_percent_col_width += meta->col_percent_widths[i];
    }
    ...
    if (caption) {
        if (caption->blk && caption->blk->given_width > 0) {
            ...
            caption_width_contribution = adjust_min_max_width(caption, caption->blk->given_width);
            log_debug("Caption has explicit CSS width: %.1fpx (after min/max clamp)", caption_width_contribution);
        } else {
        ...
        if (caption->bound) {
            ...
            float mr = (caption->bound->margin.right_type != CSS_VALUE_AUTO && caption->bound->margin.right > 0)
                           ? caption->bound->margin.right : 0;
            if (ml + mr > 0) {
                caption_width_contribution += ml + mr;
    ...
    if (!has_explicit_table_width) {
        ...
        // Note: lycon->block.content_width is the table content-box width computed
        // by layout_block.cpp from parent_content - margins - border - padding.
        ...
        if (!table_box_already_subtracted) {
            if (!table->tb->border_collapse && table->bound) {
                ...
                if (table->bound->border) {
                    table_horizontal_overhead += table->bound->border->width.left +
    ...
    bool used_percent_distribution = false;
    if (total_percent_col_width > 0.0f &&
        ...
        if (assigned_total > available_content_width) {
            ...
            if (auto_grow_count > 0) {
                ...
                for (int i = 0; i < columns; i++) {
                    if (meta->col_percent_widths[i] > 0.0f) {
                        percent_total += meta->col_percent_widths[i];
                    }
                }
                if (percent_total > 0.0f) {
                    for (int i = 0; i < columns; i++) {
                        if (meta->col_percent_widths[i] <= 0.0f) continue;
                        col_widths[i] += extra * meta->col_percent_widths[i] / percent_total;
                    }
                }
            }
        }
        used_percent_distribution = true;
    ...
    if (table->tb->border_collapse) {
        ...
        // col_widths already include per-cell border halves (added during measurement).
        // No additional border contribution needed for table width.
    ...
    // CSS 2.1 §17.6.2: Padding on table elements is ignored in border-collapse mode
    float table_padding_horizontal = 0;
    ...
    if (captions->length > 0 && table->tb->caption_side == TableProp::CAPTION_SIDE_TOP) {
        ...
        for (int ci = 0; ci < captions->length; ci++) {
            ...
            adjust_table_caption_width(cap, wrapper_content_width);
    ...
    for (int _i = 0; _i < ordered_elements->length; _i++) {
        ...
        if (child->view_type == RDT_VIEW_TABLE_ROW_GROUP) {
            ...
            if (table->tb->border_collapse) {
                ...
                log_debug("%s Border-collapse tbody width: col_positions[%d]=%.1f - col_positions[0]=%.1f = %.1f", table->source_loc(),
            ...
            if (has_direct_float && table->width > 0.0f && tbody_content_width > 0.0f) {
                for (int guard = 0; guard < 32; guard++) {
                    ...
                    float right_intrusion = table_direct_right_float_intrusion(table, current_y, table->width);
                    if (left_intrusion + tbody_content_width + right_intrusion <= table->width + 0.01f) {
                        break;
                    }
                    float next_y = table_direct_float_next_clear_y(table, current_y);
                    if (next_y <= current_y + 0.01f) break;
                    current_y = next_y;
                }
            }
	            ...
	            for (View* row_view = child->first_child; row_view; row_view = row_view->next_sibling) {
                    ...
                    if (is_collapsed) {
                        ...
                        if (global_row_index < meta->row_count) {
                            ...
                            log_debug("%s Collapsed row %d: y=%.1f, height=0", table->source_loc(), global_row_index, current_y);
                        }
                        global_row_index++;
                    ...
                    if (table->tb->fixed_row_height > 0) {
                        ...
                        for (ViewTableCell* tcell = trow->first_cell(); tcell; tcell = trow->next_cell(tcell)) {
                            if (tcell->height < row->height) {
                                ...
                                // Re-apply vertical alignment with correct cell height
                                float content_height = measure_cell_content_height(lycon, tcell);
                                apply_cell_vertical_align(lycon, tcell, tcell->height, content_height);
                            }
                        }
                    }
                    ...
                    if (global_row_index < meta->row_count) {
                        ...
                        log_debug("%s Tracking row %d: y=%.1f, height=%.1f", table->source_loc(), global_row_index, current_y, row->height);
                    }
                    global_row_index++;
            ...
            if (child->height > content_group_height) {
                ...
                current_y += extra;
                log_debug("%s Advanced current_y by %.1f for expanded row group", table->source_loc(), extra);
                        ...
                        if (r->view_type == RDT_VIEW_TABLE_ROW && r->height > 0) {
                            ...
                            for (ViewTableCell* tcell = trow->first_cell(); tcell; tcell = trow->next_cell(tcell)) {
                                if (tcell->height < r->height) {
                                    ...
                                    apply_cell_vertical_align(lycon, tcell, tcell->height, cell_content_h);
                                }
                            }
            ...
            if (is_collapsed) {
                ...
                if (global_row_index < meta->row_count) {
                    meta->row_y_positions[global_row_index] = current_y;
                    meta->row_heights[global_row_index] = 0;
                    log_debug("%s Collapsed direct row %d: y=%.1f, height=0", table->source_loc(), global_row_index, current_y);
                }
                global_row_index++;
            ...
            if (has_direct_float && table->width > 0.0f && table_width > 0.0f) {
                for (int guard = 0; guard < 32; guard++) {
                    float left_intrusion = table_direct_left_float_intrusion(table, current_y, table->width);
                    float right_intrusion = table_direct_right_float_intrusion(table, current_y, table->width);
                    if (left_intrusion + table_width + right_intrusion <= table->width + 0.01f) {
                        break;
                    }
                    float next_y = table_direct_float_next_clear_y(table, current_y);
                    if (next_y <= current_y + 0.01f) break;
            ...
            if (global_row_index < meta->row_count) {
                ...
                log_debug("%s Tracking direct row %d: y=%.1f, height=%.1f", table->source_loc(), global_row_index, current_y, trow->height);
            }
            global_row_index++;
    ...
    if (table->blk && table->blk->given_min_height >= 0) {
        float min_h = table->blk->given_min_height;
        if (min_h > explicit_css_height) {
            explicit_css_height = min_h;
            log_debug("%s Table min-height applied: explicit_css_height raised to %.1fpx", table->source_loc(), explicit_css_height);
        }
    }
    ...
    if (table->blk && table->blk->given_max_height >= 0) {
        float max_h = table->blk->given_max_height;
        // min-height overrides max-height per CSS 2.1 §10.7
        if (table->blk->given_min_height >= 0 && max_h < table->blk->given_min_height) {
            max_h = table->blk->given_min_height;
        }
        if (explicit_css_height > max_h) {
            explicit_css_height = max_h;
            log_debug("%s Table max-height applied: explicit_css_height capped at %.1fpx", table->source_loc(), explicit_css_height);
        }
    }
    ...
    if (table->bound && table->bound->border) {
        if (table->tb->border_collapse) {
            ...
            table_border_vert = table->bound->border->width.top + table->bound->border->width.bottom;
        }
    }
    if (!table->tb->border_collapse && table->tb->border_spacing_v > 0) {
    ...
    if (explicit_css_height > 0 && meta->row_count > 0) {
        ...
        if (extra_height > 0) {
            ...
            float non_body_grid_height = 0;
            float body_natural_height = 0;
            int body_row_count = 0;
            int section_count = 0;  // Count row-group sections for grid spacing
            ...
            for (View* child_view = table->first_child; child_view; child_view = child_view->next_sibling) {
                ...
                if (is_caption) {
                    log_debug("%s   Caption height outside grid: %.1f", table->source_loc(), child->height);
                    ...
                    for (ViewTableRow* row = group->first_row(); row; row = group->next_row(row)) {
                        ...
                        for (ViewTableCell* tcell = row->first_cell(); tcell; tcell = row->next_cell(tcell)) {
                            ...
                            if (row_idx >= 0 && row_idx < meta->row_count) {
                                ...
                                row_count_in_group++;
                                log_debug("%s     Row %d natural height: %.1f", table->source_loc(), row_idx, row_height);
                    ...
                    if (is_body_group) {
                        ...
                        section_count++;
                        log_debug("%s   Body group natural height: %.1f (rows only), rows: %d", table->source_loc(), group_height, row_count_in_group);
                    } else {
                        non_body_grid_height += group_height;
                        section_count++;
                        log_debug("%s   Header/Footer group natural height: %.1f (rows only)", table->source_loc(), group_height);
                    }
                }
            }
            ...
            if (extra_for_body > 0 && body_row_count > 0) {
                                    ...
                                    for (ViewTableCell* tcell = trow->first_cell(); tcell; tcell = trow->next_cell(tcell)) {
                                        ...
                                        if (row_idx >= 0 && row_idx < meta->row_count) {
                                            ...
                                            float row_extra = eligible_height_total > 0.0f
                                                ? extra_for_body * natural_height / eligible_height_total
                                            ...
                                            log_debug("%s     Body row %d: natural=%.1f + extra=%.1f = %.1f", table->source_loc(),
                                                     row_idx, natural_height, row_extra, meta->row_heights[row_idx]);
                                            break;  // Found row index
                                        }
                                    }
                                }
                ...
                if (eligible_row_count > 0) {
                    ...
                    for (int r = 0; r < meta->row_count; r++) {
                        float natural_height = meta->row_heights[r];
                        float row_extra = eligible_height_total > 0.0f
                            ? extra_for_body * natural_height / eligible_height_total
                            : extra_for_body / eligible_row_count;
                        meta->row_heights[r] += row_extra;
                        log_debug("%s   Non-body row %d: natural=%.1f + extra=%.1f = %.1f", table->source_loc(),
                                  r, natural_height, row_extra, meta->row_heights[r]);
                    }
                                ...
                                if (row_idx >= 0 && row_idx < meta->row_count) {
                                    ...
                                    for (ViewTableCell* cell = trow->first_cell(); cell; cell = trow->next_cell(cell)) {
                                        if (cell->td->row_span == 1) {
                                            ...
                                            apply_cell_vertical_align(lycon, cell, cell->height, content_h);
                                        }
                                    }
