    ...
    for (ViewElement* child = (ViewElement*)table->first_child; child; child = (ViewElement*)child->next_sibling) {
        if (child->view_type == RDT_VIEW_TABLE_COLUMN_GROUP) {
            ...
            if (col_count == 0) {
                ...
                col_count = (span_str && *span_str) ? (int)str_to_int64_or(span_str, strlen(span_str), 0) : 1;
...
static void resolve_table_properties(LayoutContext* lycon, DomNode* element, ViewTable* table) {
    ...
    if (element->node_type == DOM_NODE_ELEMENT) {
        ...
        if (dom_elem->tag() == HTM_TAG_TABLE) {
            ...
            if (cellspacing_attr) {
                float spacing = (float)str_to_double_or(cellspacing_attr, strlen(cellspacing_attr), 0.0);
...
static void parse_cell_attributes(LayoutContext* lycon, DomNode* cellNode, ViewTableCell* cell) {
    ...
    if (cellNode->node_type == DOM_NODE_ELEMENT) {
        ...
        if (colspan_str && colspan_str[0] != '\0') {
            int span = (int)str_to_int64_or(colspan_str, strlen(colspan_str), 0);
            if (span > 0 && span <= 1000) {
        ...
        if (rowspan_str && rowspan_str[0] != '\0') {
            int span = (int)str_to_int64_or(rowspan_str, strlen(rowspan_str), 0);
