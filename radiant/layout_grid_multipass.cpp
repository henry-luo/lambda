...
int resolve_grid_item_styles(LayoutContext* lycon, ViewBlock* grid_container) {
    ...
    GridNodeScratch nodes(lycon, grid_container, true);
    int item_count = 0;
    for (int node_index = 0; node_index < nodes.count; node_index++) {
        DomNode* child = nodes.data[node_index];
        if (child->is_element()) {
...
void measure_grid_items(LayoutContext* lycon, GridContainerLayout* grid_layout) {
    ...
    GridNodeScratch nodes(lycon, container, false);
    for (int node_index = 0; node_index < nodes.count; node_index++) {
        DomNode* child = nodes.data[node_index];
        if (child->is_element()) {
...
static void layout_grid_item_final_content_multipass(LayoutContext* lycon, ViewBlock* grid_item) {
    ...
    if (grid_item->display.inner == CSS_VALUE_GRID) {
        ...
        layout_flow_children(lycon, grid_item->first_child, true);
    }
    ...
    log_info("Grid item content layout complete: %s, content=%.0fx%.0f",
