...
void render_text_view_svg(SvgRenderContext* ctx, ViewText* text) {
    ...
    int font_size = ctx->font.face ? (ctx->font.face->size->metrics.y_ppem >> 6) : 16;
    float baseline_y = y + (ctx->font.face ? (ctx->font.face->size->metrics.ascender >> 6) : font_size * 0.8f);

    strbuf_append_format(ctx->svg_content,
        "<text x=\"%.2f\" y=\"%.2f\" font-family=\"%s\" font-size=\"%d\" fill=\"%s\"",
        x, baseline_y,
        ctx->font.face ? ctx->font.face->family_name : "Arial",
        font_size,
        color_str);
...
char* render_view_tree_to_svg(UiContext* uicon, View* root_view, int width, int height) {
    ...
    ctx.font.face = NULL; // Will be set if needed
