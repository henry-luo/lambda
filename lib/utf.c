...
// Unicode 17.0 emoji-data.txt Emoji_Presentation ranges, coalesced for lookup.
static const UtfCodepointRange utf_emoji_presentation_ranges[] = {
...
bool utf_is_emoji_presentation_default(uint32_t cp) {
    size_t lower = 0;
    size_t upper = sizeof(utf_emoji_presentation_ranges) /
        sizeof(utf_emoji_presentation_ranges[0]);
    while (lower < upper) {
        size_t middle = lower + (upper - lower) / 2;
        UtfCodepointRange range = utf_emoji_presentation_ranges[middle];
        if (cp < range.first) upper = middle;
        else if (cp > range.last) lower = middle + 1;
        else return true;
    }
    return false;
}
