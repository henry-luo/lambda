...
// struct definition moved to hashmap.h
...
void hashmap_clear(struct hashmap *map, bool update_cap) {
    ...
    if (update_cap) {
        ...
        void *new_buckets = map->_malloc(map->bucketsz*map->cap);
        if (new_buckets) {
            map->_free(map->buckets);
...
static bool resize0(struct hashmap *map, size_t new_cap) {
    struct hashmap *map2 = hashmap_new_with_allocator(map->_malloc, map->_realloc, 
        map->_free, map->elsize, new_cap, map->seed0, map->seed1, map->hash, 
