...
The editing subsystem has two separate action domains. Standard contenteditable hosts use the action registry described in [RAD_18](RAD_18_Editing_Selection_Ranges.md): a `dom-compat` action can make narrow text changes, a live template owner can reconcile its model, and editor-owned structural commands return `PASS`. Form controls are the deliberate exception. An `<input>` or `<textarea>` value is not (yet) a tree of DOM text nodes a script editor can splice — it is a flat UTF-8 buffer (`FormControlProp::current_value`, `view.hpp`) with UTF-16 selection offsets. So the value mutation, caret placement, undo snapshot, and validation are all applied in C++ on the text-control action path.

> **Amendment (2026-08-25)** — the *policy* half of this rationale is superseded by **ES9** in [`vibe/Lambda_Design_DOM_State.md`](../../../vibe/Lambda_Design_DOM_State.md) (DOM-state migration, phases F5/F6): the edit decisions — input-type→edit mapping, word/line boundaries, paste sanitization, `maxlength`, undo/redo, change-on-blur, IME *commit content* — move to a Lambda behavior template acting as the `beforeinput` applier, driving a native `replace_range` splice primitive. The representation argument above stands: the flat UTF-8 buffer, the splice itself, UTF-8↔UTF-16 conversions, caret/selection geometry, and IME preedit sessions stay native. §§4–7 below describe the implementation as it exists today and remain accurate until F5 lands, at which point this document needs a v2 revision.
...
Every form element is a "replaced element" — its box has intrinsic dimensions from control type, not content flow. All of its live state hangs off a single POD, `struct FormControlProp` (`view.hpp`), reached through `DomElement::form` when `item_prop_type == ITEM_PROP_FORM`. Following the C+ convention it has no constructor/destructor; lifecycle is `form_control_prop_init` (assign non-zero defaults, `text_control.cpp:70`) and `form_control_prop_release` (free owned heap pointers, `text_control.cpp:89`).
