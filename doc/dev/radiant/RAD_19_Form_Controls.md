...
The editing subsystem has two separate action domains. Standard contenteditable hosts use the action registry described in [RAD_18](RAD_18_Editing_Selection_Ranges.md): a `dom-compat` action can make narrow text changes, a live template owner can reconcile its model, and editor-owned structural commands return `PASS`. An `<input>` or `<textarea>` value is a flat UTF-8 buffer with UTF-16 selection offsets, not a tree of DOM text nodes a script editor can splice. Its storage, splice, caret placement, undo snapshot, and validation mechanism stay native; the durable value allocation belongs to `ViewState.data.form.current_value`, while `FormControlProp::current_value` is only its borrowed cache.

> **Amendment (2026-08-31)** — **ES9** in [`vibe/Lambda_Design_DOM_State.md`](../../../vibe/Lambda_Design_DOM_State.md) moves edit policy to the Lambda behavior template acting as the `beforeinput` applier, driving native `replace_range`. **ESO22** now makes `ViewState.data.form.current_value` the single owner: `form_control_store_text_value` replaces its allocation and only then rebinds `FormControlProp`'s cache. The flat buffer, splice, UTF-8↔UTF-16 conversions, caret/selection geometry, and IME preedit sessions remain native.
...
Every form element is a "replaced element" — its box has intrinsic dimensions from control type, not content flow. Its view-local mechanics hang off `struct FormControlProp` (`view.hpp`), reached through `DomElement::form` when `item_prop_type == ITEM_PROP_FORM`; durable control state lives in its `ViewState`. Following the C+ convention the prop has no constructor/destructor; lifecycle is `form_control_prop_init` (assign non-zero defaults) and `form_control_prop_release` (release owned mechanics and clear borrowed caches).

`FormControlType` (`view.hpp`) is the coarse category — `TEXT` (covers text/password/email/url/search/tel/number), `CHECKBOX`, `RADIO`, `BUTTON`, `SELECT`, `TEXTAREA`, `RANGE`, `IMAGE`, `HIDDEN` — resolved from the `type` attribute by the inline `get_input_control_type` (`view.hpp`). Date/time/color/file types currently fall through to `TEXT` (`view.hpp`).

The struct groups its fields by concern:

- **Back-pointers**: `state_ref` (fast-read `DocState*`; writers must go through StateStore APIs), `form_state_ref`, `scroll_state_ref` (`view.hpp`).
...
- **State bitfield**: `disabled`, `readonly`, `checked`, `required`, `autofocus`, `multiple`, `dropdown_open`, `appearance_none` (`view.hpp`). Note these bits are a *projection*; the canonical pseudo-state (`:disabled`, `:checked`, …) lives in StateStore ([RAD_17](RAD_17_Interaction_State.md)) and is read via `form_control_is_disabled`/`_readonly`/`_required`.
...
- **Flex/grid item props**: because a form control uses `FormControlProp` in the view's item-property union slot, its flex fields (`flex_grow/shrink/basis`) are duplicated here, and the original `GridItemProp*` is stashed in `grid_item` (`view.hpp`) after the union is switched to `FORM`.
- **Text-control selection state** ([§4](#4-the-text-control-value-and-selection-idl)): `current_value` is a borrowed cache of `ViewState.data.form.current_value`, with cached byte/UTF-16 lengths; `selection_start`/`selection_end` (UTF-16), `selection_direction`, `tc_initialized`, plus the `selectionchange` coalescing link `tc_sc_next_pending`.
- **Editing state**: `custom_validity_msg`, the change-on-blur snapshot `value_at_focus`, the opaque undo ring `history` (`EditHistory*`), auto-scroll (`scroll_x/y`), caret blink (`caret_blink_t`/`caret_on`), password reveal, and the IME preedit trio (`preedit_utf8`/`preedit_len`/`preedit_caret`, `view.hpp`).
...
- `calc_text_input_size` (`layout_form.cpp:18`) starts from `FormDefaults::TEXT_*` (a Chrome-matched 153×21 border-box at 1× — `view.hpp`) but special-cases date/time/datetime-local/color inputs to fixed Chrome intrinsic widths (`layout_form.cpp:24-`); width scales by `size` characters.
- `calc_textarea_size` (`layout_form.cpp:168`) uses `cols`×`rows` against the font advance (defaults 20×2, `view.hpp`).
...
`text_control.cpp`, through declarations in `event.hpp`, is the bridge between the WHATWG IDL surface (UTF-16 `.value`, `.selectionStart`/`.selectionEnd`, `setSelectionRange`) that JS sees and the UTF-8 buffer the engine stores. `tc_is_text_control` (`text_control.cpp:49`) is the identity gate, mirroring HTML §4.10.5.1, and works even before layout has populated `elem->form` (JS may run on a parsed-but-unlaid DOM), lazily creating the prop via `tc_get_or_create_form` (`text_control.cpp:98`).

`tc_ensure_init` seeds `ViewState.data.form.current_value` from the HTML default (input `value` attribute / textarea children), then binds the prop cache and collapses selection at end-of-text. Conversions `tc_utf8_to_utf16_length`/`tc_utf16_to_utf8_offset`/`tc_utf8_to_utf16_offset` do the code-unit ↔ byte math (surrogate pairs count as 2 UTF-16 units). `tc_set_value` calls `form_control_store_text_value`, the single persistent writer: it replaces the ViewState buffer, rebinds the cached `current_value` and legacy renderer `form->value` aliases, collapses selection to the new end, and refreshes placeholder/validation. The writer accepts a live detached DOM element, so `document.createElement()` uses the same owner before layout; the state entry retains only its lifecycle-validated weak identity. State restore only rebinds the cache; it does not copy the value. `tc_set_selection_range` clamps and writes the `(start, end, dir)` triple.

`tc_sync_selection_to_form` (`text_control.cpp:404`) reconciles the text-control mirrors: when `state->sel.kind == EDIT_SEL_TEXT_CONTROL` and targets this element, StateStore's `EditingSelection` is treated as the source of truth (`text_control.cpp:422`); otherwise it falls back to the caret/anchor projection cache or a `DomSelection` whose boundaries land on the control element. The canonical `EditingSelection` plus HTML-facing `form->selection_*` mirror is the same projection-cache surface flagged across [RAD_18](RAD_18_Editing_Selection_Ranges.md) and is called out in [§8](#8-known-issues--future-improvements).
...
3. **Projection-cache consumers remain.** Canonical `EditingSelection` (StateStore), the HTML-facing `form->selection_start/end` mirror, and caret/anchor projection cache are reconciled by hand in `tc_sync_selection_to_form` (`text_control.cpp:404`). The remaining cleanup is to port projection consumers directly to `EditingSelection` / DomRange geometry.
...
|---|---|
| `radiant/view.hpp` | `FormControlType`, `FormDefaults` intrinsic-size constants, the `FormControlProp` POD (all live control + text-control state), `get_input_control_type`/`is_text_input_type`, prop init/release decls. |
...
| `radiant/event.hpp` / `text_control.cpp` | UTF-8↔UTF-16 conversions, value/selection IDL, projection sync, `selectionchange` coalescing, and active text-control tracking. |
| `radiant/event.hpp` / `text_edit.cpp` | Byte-range edits, word/line scanners, undo/redo, change-on-blur, IME, paste sanitization, validation, and ARIA reflection. |
| `radiant/event.hpp` | `editing_compute_target_ranges` — the synthetic StaticRange for text-control InputEvents. |
...
- [RAD_18 — Editing, Selection & DOM Ranges](RAD_18_Editing_Selection_Ranges.md) — the registered `contenteditable` action gate and shared Selection/Range substrate.
- [`vibe/Lambda_Design_DOM_State.md`](../../../vibe/Lambda_Design_DOM_State.md) — the DOM-state migration proposal whose ES9 moves the text-control *edit policy* described here to Lambda behavior templates (see the §1 amendment).
