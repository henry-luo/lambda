...
Radiant loads a document into a DOM tree, resolves CSS onto it, lays it out with browser-compatible algorithms, and paints it to a window surface, a PNG/JPEG, a PDF, or an SVG. On top of that static pipeline it layers a full interactive stack — events, editing, animation, and an application/browsing shell — so the same engine drives `lambda layout`, `lambda render`, and `lambda view`. Four design decisions recur across the set:
...
| [RAD_09 — Grid Layout](RAD_09_Grid_Layout.md) | The canonical Taffy-derived computed-track model, placement/auto-placement, the §11 track-sizing algorithm, and the multipass driver. |
...
| [RAD_14 — SVG, Vector Graphics & Diagram Layout](RAD_14_SVG_Vector_Graph.md) | The `RdtVector` abstraction and active ThorVG backend, retained excluded CoreGraphics exploration source, inline-SVG rendering, view→SVG-text serializer, and Dagre-inspired graph layout. |
...
- **Duplicated representations and code paths.** Grid sizing now stores one canonical enhanced track model and selection derives logical boundaries from `EditingSelection`/`DomSelection`, leaving only presentation geometry cached ([RAD_09](RAD_09_Grid_Layout.md), [RAD_17](RAD_17_Interaction_State.md), [RAD_18](RAD_18_Editing_Selection_Ranges.md)). Remaining duplication includes the render layer's legacy list-bullet path alongside the `::marker` path and the paint op/payload definitions mirrored across PaintIR, the display list, and the vector layer ([RAD_12](RAD_12_Paint_IR_Display_List.md)).
- **Hard-coded caps and magic-number fallbacks remain, but the worst silent drops are hardened.** Render z-order stacks, ThorVG/CoreGraphics clip stacks, and flex measurement scratch storage now grow; grid/clip-shape/float-placement fallbacks emit `RAD_CAP_*` diagnostics before falling back. Remaining fixed caps and constants (for example a 366px flex width, a 100px float width, 300×150 replaced defaults) should follow the same rule: centralize, compute, or log before falling back.
- **Incomplete or unwired features.** CSS transitions are declared but not wired (only `@keyframes` create animation instances, [RAD_16](RAD_16_Animation_Frame_Scheduling.md)); `content:` counters/attr/url, `clip: rect()`, and named colors are unimplemented in resolution ([RAD_02](RAD_02_CSS_Style_Resolution.md)); the OS clipboard has only in-memory + GLFW plain-text backends ([RAD_18](RAD_18_Editing_Selection_Ranges.md)); webview↔runtime IPC is TODO on all platforms and there is no Linux/Windows video ([RAD_22](RAD_22_Media_Webview.md)).
- **Fragile invariants worth keeping guarded.** The zero-field view invariant and the `unsafe_*` casts it licenses now have compile-time ABI checks ([RAD_01](RAD_01_View_and_DOM_Model.md)); the old table-height global workaround has been removed ([RAD_03](RAD_03_Layout_Driver_Block_BFC.md)); intrinsic height is still an admitted estimation recomputed every call ([RAD_05](RAD_05_Intrinsic_Sizing.md)); and incremental relayout now has an opt-in verification mode for `layout_height_contribution`.
...
- **`RdtVector`** — the immediate-mode vector-graphics abstraction currently implemented by ThorVG; an excluded CoreGraphics source remains for future exploration.
