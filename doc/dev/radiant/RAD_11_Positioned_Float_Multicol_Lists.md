...
`layout_sticky_positioned` (`:341`) is a constraint solver, not an offset: it finds the nearest scroll-container ancestor (`overflow != visible`), converts the element's position into that scroller's content coordinates by accumulating ancestor border-box offsets plus padding/border (`:385`), and then clamps the element so its edges honor the inset constraints (`top` means "top edge ≥ scrollport_top + T", etc.). A second clamp keeps the element inside its containing block (`:457`–`:491`). At scroll offset 0 this typically yields no shift.

Solving is idempotent, so a scroll can re-solve without relayout. Layout's placement hook (the deferred call from block, flex, grid and inline layout) marks the box as freshly placed; each solve records in `PositionProp` the offset it applied (`sticky_offset_x/y`) and the position it left the box at (`sticky_placed_x/y`). The next solve starts from the normal-flow position — the current position minus the recorded offset, unless the box is no longer where the last solve left it, meaning layout placed it again — so a stuck box unsticks when the scroll moves back, and only the change is applied (inline spans shift their children by it). A scroll only repaints, so `scroll_state_set_position_for_view` marks `DocState::sticky_scroll_stale`, and `layout_resolve_scrolled_sticky` re-solves the document's sticky boxes (outer before inner, skipped when the last pass found none — `ViewTree::sticky_box_count`) before paint (`render_output_render_view_tree_to_target`, and `render_embed_doc` for frames) and before hit-testing (`handle_event`). The end-of-pass solve clears the flag.
