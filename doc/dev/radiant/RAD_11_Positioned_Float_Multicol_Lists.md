...
Rendering is in `render_list.cpp` (detailed in [RAD_13 — Render Walk & Painters](RAD_13_Render_Walk_Painters.md)). `render_marker_view` (`:14`) draws the marker: a list-style-image (rasterizing SVG on demand), or a vector `disc`/`circle`/`square`/`disclosure-*` shape, or right-aligned glyph runs for numeric/alphabetic markers (`:154`+). Note `render_list_bullet`/`render_litem_view`/`render_list_view` (`:217`/`:235`/`:241`) are an older bullet path keyed off `rdcon->list.list_style_type` that still hard-codes a `disc` at a fixed `−15px`/`+7px` offset — legacy alongside the `::marker` path.
...
5. **Two divergent list-marker render paths.** `render_list.cpp` has both the `::marker` path (`render_marker_view`, `:14`) and a legacy `render_list_bullet` (`:217`) that hard-codes a disc at `−15px`/`+7px`. The legacy path is dead-ish but still compiled and reachable via `render_list_view`. *Improvement:* delete the legacy bullet path once the `::marker` path fully covers it.
6. **Image-load failure uses a placeholder box.** Failed abspos `<img>` loads fall back to a hard-coded 40×30 placeholder (`layout_positioned.cpp:1464`); marker image-load failure silently falls through to the numeric/bullet render (`render_list.cpp:70`).
7. **Very few explicit TODO/FIXME markers.** The structural debt in this area shows up as `simplified`/`placeholder`/"Phase N" narration rather than tagged TODOs, so grep-based debt tracking under-counts it (notably the mega-functions in §7.4 and multicol §7.1).
...
| `radiant/render_list.cpp` | Marker rendering (`render_marker_view`) + legacy bullet path. |
