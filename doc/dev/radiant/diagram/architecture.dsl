workspace "Radiant" "HTML/CSS layout, rendering and interaction engine inside Lambda" {
  ...
  model {
    dev = person "Developer" "Runs layout/render/view, or embeds Radiant."
    hostos = softwareSystem "Host OS / GPU / Windowing" "GLFW window, ThorVG raster, platform WebView, OS fonts and input." "External"

    lambda = softwareSystem "Lambda" "General-purpose data and document runtime with a MIR JIT." {
      cli = container "CLI" "Dispatches the layout, render and view subcommands." "lambda/main.cpp"
      ...
      radiant = container "Radiant engine" "Turns HTML plus CSS into laid-out, painted, interactive documents." "radiant" {
        ...
        vector = component "SVG and vector graphics" "RdtVector with active ThorVG backend, inline SVG and diagram layout." "rdt_vector_tvg.cpp, render_svg_inline.cpp, graph_*"
        ...
        shell = component "Shell and browsing" "UiContext window loop, browsing session, media and webview." "window.cpp, browsing_session.cpp, webview_*"
      }
    ...
    dev -> cli "runs layout / render / view"
    ...
    render -> hostos "surface and ThorVG raster"
