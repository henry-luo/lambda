...
>
> **Primary sources:** `radiant/render.hpp` (PaintIR, DisplayList, retained rendering, tile-pool, paint-gateway, and backend declarations), `radiant/paint_ir.cpp` and `radiant/display_list.cpp` (IR lowering and DisplayList lifecycle), the `display_list_record_*.cpp` and `display_list_replay*.cpp` families, `radiant/display_list_storage.cpp` (deep-copy into the arena), `radiant/retained_display_list.cpp` (fragment cache), `radiant/tile_pool.cpp` (parallel replay), `radiant/view.hpp` (clip and transform data), `radiant/render_painter.cpp` (`rc_*`), and `radiant/render_output.cpp` (orchestration).
...
**PaintIR** (`render.hpp`) is the upper layer: *target-neutral, semantic* paint commands. The intent recorded at the top of the header (`render.hpp`) is that PaintIR is the single place the per-element paint algorithm lives, so every backend — raster, SVG, PDF — consumes the same commands instead of each re-deriving "how to paint a background with a border and a shadow." A `PaintList` (`render.hpp`) is a growable array of `PaintCmd` tagged unions backed by an `Arena`.

**DisplayList** (`render.hpp`) is the lower layer: *one* raster lowering of PaintIR. Its ops are pixel-domain — rasterised glyph bitmaps, premultiplied compositing, direct-surface fills — and it is explicitly "Phase 1 of the multi-threaded rendering proposal" (`render.hpp`). A `DisplayList` (`render.hpp`) is a growable array of `DisplayItem` plus a `ScratchArena` that owns *all* variable-length data (paths, gradient stops, dash arrays, clip-shape polygons) so the list is self-contained.

The two layers relate by a single function: `paint_ir_lower_raster` (`render.hpp`, body `paint_ir.cpp:944`) turns a `PaintList` into a `DisplayList`. The vector-primitive PaintIR ops lower **1:1** onto the matching `dl_*` command, so a lowered DisplayList is byte-for-byte identical to recording those `dl_*` calls directly (`render.hpp`). This "thin layer above DisplayList" property is what made PaintIR safe to insert under the existing raster path without changing a pixel. Vector backends (SVG/PDF) consume PaintIR through a *different* lowering, `paint_ir_lower_svg` (`render.hpp`), and never touch the DisplayList except for raster-only fallbacks like page backdrops.

### 1.1 The three PaintOp tiers

`enum PaintOp` (`render.hpp`) is deliberately layered into three bands, and the band an op sits in tells you which backends handle it natively.

| Tier | Ops | Contract |
|---|---|---|
| Vector primitives (`render.hpp`) | `PAINT_FILL_RECT`, `PAINT_FILL_ROUNDED_RECT`, `PAINT_FILL_PATH`, `PAINT_STROKE_PATH`, gradients, `PAINT_DRAW_IMAGE`/`_RESOURCE`, `PAINT_DRAW_GLYPH`, `PAINT_DRAW_PICTURE`, video/webview placeholders, `PAINT_PUSH/POP_CLIP`, `PAINT_PUSH/POP_TRANSFORM` | lower 1:1 to `dl_*`; byte-identical raster; exact SVG representation where possible |
| Raster-lowering tier (`render.hpp`) | `PAINT_SAVE_BACKDROP`, `PAINT_COMPOSITE_OPACITY`, `PAINT_APPLY_BLEND_MODE`, `PAINT_APPLY_FILTER`, `PAINT_BOX_BLUR_REGION`/`_INSET`, shadow-clip ops, `PAINT_OUTER_SHADOW`, `PAINT_FILL_SURFACE_RECT`, `PAINT_BLIT_SURFACE_SCALED` | pixel-domain ops mirroring DisplayList commands; **not** the semantic contract — they are the raster expansion of the effect-group op below |
| Higher-level semantic ops (`render.hpp`) | `PAINT_GLYPH_RUN`, `PAINT_BEGIN/END_EFFECT_GROUP`, `PAINT_SVG_SUBSCENE` | the canonical cross-backend contract; lowered incrementally per target |

The header states the rationale for the middle tier plainly (`render.hpp`): routing pixel-domain effect ops through the builder during migration gives the raster path a *single* emission gateway, while the higher-level `PAINT_*_EFFECT_GROUP` op it expands from is fleshed out. A vector backend is meant to consume the effect group, not these pixel ops.

### 1.2 The semantic ops

`PaintEffectGroup` (`render.hpp`) is a descriptor for a CSS stacking effect: visual `bounds`, optional clip/transform, `opacity`, `blend_mode`, `filter`, `backdrop_filter`, `shadow`, `isolation`. It exists so each backend can decide native-vs-fallback for a whole grouped subtree in one place. `PaintGlyphRun` (`render.hpp`) is a run of glyphs sharing a font/colour carrying positions and an optional UTF-8 text payload — *not* rasterised bitmaps — so SVG can emit native `<text>`. `PaintSvgSubscene` (`render.hpp`) carries a nested inline/external SVG root plus the inherited paint (`currentColor`, `fill`, `stroke`) and viewBox geometry that must survive lowering, plus a `resource_generation` for retain safety (Phase F). Because glyph-run and SVG-subscene lowering pull in heavyweight font/SVG dependencies, they are injected as **registered function pointers** (`render.hpp`, `385-387`; e.g. `render_svg.cpp:124` registers `render_glyph_run_raster_lower`) rather than linked into the PaintIR core.

`paint_ir_validate` (`paint_ir.cpp:115`) walks a `PaintList` and checks clip/backdrop/shadow-clip/effect-group depth balance, returning a `PaintIrValidationResult` (`render.hpp`); `paint_ir_validate_or_log` (`paint_ir.cpp:449`) is the log-on-failure wrapper the lowerings call before emitting.
...
The `rc_*` functions (`render.hpp+`, bodies in `render_painter.cpp`) are the drawing wrappers every feature painter in [RAD_13](RAD_13_Render_Walk_Painters.md) calls — `rc_fill_rect`, `rc_draw_glyph`, `rc_push_clip`, `rc_outer_shadow`, and so on. Each routes through the **`PaintRecordTarget` gateway** (`render.hpp`), which holds a `PaintList*`, a `DisplayList*`, and a log prefix. Every `paint_record_*` inline (e.g. `paint_record_fill_rect` at `render.hpp`) does exactly two things: append one `PaintCmd` to the `PaintList` via the matching `paint_*` builder, then immediately call `paint_record_lower_pending` (`render.hpp`), which runs `paint_ir_lower_raster_fragment` and `paint_list_clear`.
...
Around each retainable view subtree, the walk brackets the emitted range with element markers: `dl_begin_element`/`dl_end_element` (`render.hpp`) emit `DL_BEGIN_ELEMENT`/`DL_END_ELEMENT` items carrying a `view_id` and a `matching_index` that links begin↔end (`DlElementMarker`, `render.hpp`). These markers are the granularity unit for both dirty-clip skipping ([§5](#5-single-threaded-replay-and-dirty-clip-skipping)) and retained capture ([§6](#6-retained-incremental-replay)).
...
`render_output_replay_display_list` (`render_output.cpp:291`) chooses the replay strategy. The tiled path engages only when **all** of these hold (`render_output.cpp:309`): the frame is not a dirty/selective replay, the configured thread count is not 1, the list is non-empty, and `dl_contains_glyphs` (`render.hpp`) is false. When it engages it builds a `TileGrid` of 256-CSS-px tiles (512 physical at 2×) with `tile_grid_init` (`render.hpp`, `TILE_SIZE_CSS` = 256 at `render.hpp`), clears them to the canvas background, builds a `TileJob[]` (`render.hpp`) sharing the read-only DisplayList, dispatches to the `RenderPool` worker threads with `render_pool_dispatch` (`render.hpp`), and composites the tiles back with `tile_grid_composite` (`render.hpp`). Otherwise it falls back to serial `dl_replay` (`render_output.cpp:338`).
...
`RenderBackend` (`render.hpp`) is a vtable of function pointers — `render_block`, `render_inline`, `render_bound`, `render_text`, `render_image`, `render_marker`, `begin/end_block_children`, `begin/end_effect_group`, `begin/end_transform`, `on_font_change`, and more — that the shared tree walker `render_walk_block`/`render_walk_inline` (`render.hpp`) dispatches through, carrying accumulated position/font/colour in `RenderWalkState` (`render.hpp`). All coordinates at this seam are CSS logical pixels.

The two seams coexist deliberately. The **raster** backend uses full-node overrides (`render_block`/`render_inline`) that funnel into the `rc_*`/PaintIR/DisplayList pipeline of this doc, because raster needs HiDPI scaling, pixel post-processing, scrollbars, and display-list markers that the generic callbacks do not model (`render.hpp`). The **SVG/PDF** backends implement the vtable and consume PaintIR *directly*: they register the glyph-run lowerer and call `paint_ir_lower_raster` only for raster-only page backdrops (`render_svg.cpp:125`, `render_pdf.cpp:208`) and `paint_ir_lower_svg_stream` for native vector output (`render_svg.cpp:139`). Capability negotiation between them lives in `render_backend_caps.*` (`RenderExportTargetCaps`, referenced from `PaintSvgLoweringOptions`, `render.hpp`).

**Clip and transform** state threads through both IR levels as payload fields, not as ambient global state. `ClipShape` (`view.hpp`) is a tagged union over polygon/circle/ellipse/inset/rounded-rect, capped at `RDT_MAX_CLIP_SHAPES` = 8 (`view.hpp`); the header is header-only and carries the point-in-shape, rect-inside, and scanline-bounds helpers (`view.hpp`) that replay and tiling use to mask direct-pixel ops, plus `clip_shape_to_params`/`clip_shape_from_params` (`view.hpp`/`307`) for the serialised `float[8]` form stored in `DlClipShapeStack` (`render.hpp`). Transforms are combined once by `radiant::compute_transform_matrix` (`view.hpp`), which folds a CSS `TransformFunction` chain — 2D affine plus a perspective/`matrix3d` quad-mapping branch (`view.hpp`) — into a single `RdtMatrix` that paint ops carry alongside a `has_transform` flag.

---

## 9. Known Issues & Future Improvements

1. **Eager per-command lowering yields no batching benefit.** The gateway records one `PaintCmd`, lowers it, then clears the `PaintList` on every op (`render.hpp`, and every `paint_record_*` inline). PaintIR is a "thin layer above DisplayList" (`render.hpp`) that today adds a copy-and-switch per op with no cross-op optimisation. The overhead is unmeasured. *Improvement:* record whole subtrees into the `PaintList` and lower in bulk, enabling coalescing/culling before raster.
2. **Triple-mirrored op and payload definitions.** `PaintOp` ↔ `DisplayOp` ↔ the `rdt_*` calls, and `Paint*` ↔ `Dl*` payload structs, mirror each other by hand (`render.hpp` vs `render.hpp`). Adding one primitive means edits in ~3 struct sites plus the lowering switch (`paint_ir.cpp:944`) and the replay switch (`display_list_replay.cpp:85`) — roughly 3 edit sites per primitive before it renders. *Improvement:* generate the mirrored trio from one op table.
...
4. **Semantic ops are not fully lowered.** Raster lowering ignores `PAINT_GLYPH_RUN` semantic runs and richer effects (`render.hpp`); SVG handles only text runs and opacity-only groups; `PAINT_SVG_SUBSCENE` is a Phase-F stub (`render.hpp`, `330-331`). Most effect groups round-trip through raster fallback rather than native vector constructs, so the "one paint algorithm for all backends" promise is only partly realised.
...
|---|---|
| `radiant/render.hpp` / `paint_ir.cpp` | `PaintOp` tiers, `PaintCmd`/`PaintList`, effect-group/glyph-run/SVG-subscene ops, `paint_ir_lower_raster` switch, SVG lowering, validation, registered lowerers. |
| `radiant/render.hpp` / `display_list.cpp` | `DisplayOp`/`DisplayItem`/`DisplayList`, per-op payload structs, element markers, lifecycle. |
| `radiant/display_list_record_*.cpp` | Vector / raster / effect recording APIs (`dl_*` builders) that lowering targets. |
| `radiant/render.hpp` / `display_list_replay*.cpp` | `dl_replay` control flow + the backdrop/shadow/effects/glyph/raster/state replay families and dirty-clip skip. |
| `radiant/render.hpp` / `display_list_storage.cpp` | Deep-copy of stops/dashes/paths/clip-shapes into the DisplayList `ScratchArena`. |
| `radiant/render.hpp` / `retained_display_list.cpp` | `RetainedDisplayListCache`/`Fragment`, capture, resource-generation-gated reuse, stats. |
| `radiant/render.hpp` / `tile_pool.cpp` | `TileGrid`/`RenderPool`/`TileJob`, `dl_replay_tile`, page-absolute → tile-local translation. |
| `radiant/render.hpp` | `PaintRecordTarget` + `paint_record_*` inlines; the eager record-then-lower gateway. |
| `radiant/render.hpp` / `render_painter.cpp` | `rc_*` drawing wrappers the feature painters call. |
| `radiant/render.hpp` | `RenderBackend` vtable + shared tree-walker API. |
| `radiant/render_output.cpp` | Frame orchestration: record, capture, choose serial/tiled/dirty replay. |
| `radiant/view.hpp` / `radiant/view.hpp` | Clip-shape union + hit/scanline helpers; `compute_transform_matrix`. |
