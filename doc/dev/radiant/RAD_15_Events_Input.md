...
3. **Positioned content, in reverse paint order** — positive-z descendants (`target_positive_z_descendants`), then absolute/fixed children (`target_positioned_children`). Both consume the stable, `z-index`-sorted lists the painter builds in `stacking_order.cpp` ([RAD_13 §2.4](RAD_13_Render_Walk_Painters.md)) from the end, so a higher `z-index` — or, at equal `z-index`, the later box — takes the hit. They come before the block's embedded content so an iframe or webview cannot steal a click on positioned content painted over it. A custom layout (`layout(fn)`) instead walks its own signed-z paint list in reverse (`target_custom_layout_children`).
4. **Layer-mode webviews** — a `WEBVIEW_MODE_LAYER` embed targets the block and injects the translated mouse coordinate into the offscreen web view via `webview_layer_platform_inject_mouse`.
5. **Embedded iframe documents** — a block with `embed->doc` recurses into the iframe's `view_tree` via a nested `target_html_doc`, and on a hit records `iframe_container` so the event propagates across the boundary.
6. **In-flow children** — `target_children` mirrors the painter's `render_walk_children`. In-flow positioned children (`relative`/`sticky` with `z-index` auto or 0 — CSS 2.1 Appendix E step 8, `radiant_stack_is_in_flow_positioned_step8`) paint after their non-positioned siblings, so they are tried first, the later sibling first. The non-positioned children follow in tree order, reversed when floats are present (a later float paints over an earlier one). Each child dispatches to `target_inline_view`/`target_text_view` or a recursive `target_block_view`. Inside rich-editable subtrees, `find_editable_margin_text_hit` snaps clicks in whitespace/margins to the nearest text run so the caret has somewhere to land.
7. **The block itself** — finally, the block's own border box is tested. Replaced elements (img/video/canvas/iframe/embed/object/hr) inside a rich editable are explicitly made hit-testable here so an image can be clicked and selected rather than snapping to adjacent text.
...
1. *(Resolved.)* The duplicated layer-mode webview block is gone; `target_block_view` has one `WEBVIEW_MODE_LAYER` inject block.
2. **Stacking is resolved among siblings, not per stacking context.** Hit-testing follows the painter ([RAD_13 §2.4](RAD_13_Render_Walk_Painters.md)) — positive z, then absolute/fixed by `z-index`, then CSS 2.1 Appendix E step-8 in-flow positioned boxes, then the rest — but both stop short of full Appendix E: a positioned box nested in a non-positioned block is not lifted above that block's later siblings; negative `z-index` boxes stay in tree order instead of below in-flow content; absolute/fixed children do not interleave with step-8 boxes by tree order; and overlapping *non-positioned* siblings are tried in tree order, while CSS paints later block backgrounds below earlier inline content. DOM `elementFromPoint` (`lambda/dom/dom.cpp`, `dom_element_from_point_walk`) is a separate walker (last child first, no stacking) and can disagree with the event target where boxes overlap.
