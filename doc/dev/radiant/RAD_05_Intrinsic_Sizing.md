...
Two result wrappers sit on top. `TextIntrinsicWidths {min_content, max_content}` (`intrinsic_sizing.hpp:96`) is the text-measurement return. `IntrinsicSizesBidirectional {min/max_content_width, min/max_content_height}` (`intrinsic_sizing.hpp:243`) is the four-value result of the unified entry, with the axis extractors `intrinsic_sizes_width` / `intrinsic_sizes_height` / `intrinsic_sizes_for_axis` (`:275-295`). `CellIntrinsicWidths {min_width, max_width}` (`:308`) is the table-cell MCW/PCW pair.

### 2.2 The real cache lives on `DomElement` — width only

There is a header struct `IntrinsicSizeCache` (`intrinsic_sizing.hpp:25-86`) with a `min/max_content_width` pair *and* a four-entry per-width height cache (`HeightCacheEntry height_cache[4]`). **It is dead** — a grep for `IntrinsicSizeCache` finds zero references anywhere outside its own declaration. The actual cache is four fields on `DomElement` (`dom_element.hpp:434-437`): `cached_min_content_width`, `cached_max_content_width`, `has_cached_intrinsic_widths`, and the re-entrancy flag `measuring_intrinsic_width`. This cache is **width-only**: height (the width-dependent, expensive part) is never cached and is recomputed on every call.
...
1. **Dead `IntrinsicSizeCache` struct.** The header struct (`intrinsic_sizing.hpp:25-86`), including its four-slot per-width `HeightCacheEntry` height cache, has zero references anywhere. The real cache is the four `DomElement` fields (`dom_element.hpp:434-437`) and caches **width only**. *Improvement:* delete the struct, or wire up its height cache to fix issue 2.
...
|---|---|
| `radiant/intrinsic_sizing.hpp` | Result structs (`TextIntrinsicWidths`, `IntrinsicSizesBidirectional`, `CellIntrinsicWidths`), public API declarations, the dead `IntrinsicSizeCache`. |
...
| `radiant/layout_mode.hpp` | `RunMode::ComputeSize` — the measurement-only run mode. |
| `radiant/layout_pass.{hpp,cpp}` | `LayoutRunModeScope`; snapshot/restore of the `DomElement` width cache + re-entrancy flag across speculative passes. |
