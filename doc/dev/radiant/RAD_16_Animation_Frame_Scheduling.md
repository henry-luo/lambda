...
>
> **Primary sources:** `radiant/frame_clock.{h,cpp}` (`RadiantFrameClock`, native vsync thread, timeout policy), `radiant/animation.{h,cpp}` (`AnimationScheduler`, `AnimationInstance`, `TimingFunction`), `radiant/css_animation.{h,cpp}` (`KeyframeRegistry`, `CssKeyframes`, keyframe sampling, style-resolution wiring), `radiant/video_frame_wake.{h,cpp}` (video wake), `radiant/window.cpp` (`view_frame_clock` main loop, `view_wake_glfw`, `view_next_wait_timeout`).
...
`css_animation_create` (`css_animation.cpp:882`) allocates a `CssAnimState { keyframes, element }` (`css_animation.h:112`), fills an `AnimationInstance` with `tick = css_animation_tick` / `on_finish = css_animation_finish`, seeds `bounds` from the element's absolute layout position (walking the parent chain), and adds it to the scheduler.
...
|---|---|
| `radiant/frame_clock.h` | `RadiantFrameClock` struct, mode enum, public API. |
...
| `radiant/animation.h` | `AnimationInstance`, `AnimationScheduler`, `TimingFunction`, animation-type/direction/fill/play enums. |
...
| `radiant/css_animation.h` | `CssKeyframes`/`CssKeyframeStop`/`CssAnimatedProp`, `KeyframeRegistry`, `CssAnimProp`/`CssTransitionProp`, `CssAnimState`. |
...
| `radiant/video_frame_wake.{h,cpp}` | Global video wake callback; `radiant_video_notify_frame_ready` sets `video_frame_pending` + wakes. |
