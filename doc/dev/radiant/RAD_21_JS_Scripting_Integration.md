...
> **Last verified against tree:** 2026-09-22
...
The preamble aliases `window = globalThis`, wires `window.document`/`navigator`/`console`/`performance`/`screen`, forwards the timer and rAF globals, and leaves optional module-loader globals absent. In particular it does **not** create `define.amd`: UMD libraries must select their browser-global branch unless the document itself loaded an AMD loader. This preserves ordinary browser script semantics and ensures a following classic script can observe a UMD export (D1.3v3). `scrollTo`/`scroll`/`scrollBy` are pure JS that record scroll offsets onto `window`/`documentElement`/`body` since Radiant has no live scroll during load. A representative slice of what the preamble supplies:
...
Page scripts are untrusted, and large minified bundles can hang the JIT or crash inside compiled code. `execute_document_scripts` therefore wraps the transpile-and-run in a **process-level signal watchdog** (`#ifndef _WIN32`). It installs `SA_SIGINFO` handlers for SIGSEGV and SIGBUS and uses `SIGPROF`/`ITIMER_PROF` for CPU time. Trusted browser-preamble preparation completes before the timer is armed: because the signal is process-scoped, timing that setup could interrupt a parallel helper instead of page-owned source work. The page-code budget is 10 seconds plus 10 seconds per 16 KiB after 8 KiB, capped at 120 seconds (env-overridable up to 600 seconds). Recovery keeps the batch unsafe through document teardown, which must not re-enter a possibly interrupted global reset (D4.2.2v2, D4.2.3, D4.5.1v3).
...
The host↔JS wiring around this is set up earlier in the shell: `dom_set_ui_context` and `dom_set_host_driven_loop` (`window.cpp`, [RAD_20](RAD_20_Application_Shell_Browsing.md)) tell LambdaJS about Radiant's geometry APIs and who owns the event loop. When the loop is host-driven (interactive `view`), `execute_document_scripts` flushes only microtasks and leaves timers queued for the window loop to pump post-commit; draining `setTimeout(0)` before the first layout would make geometry APIs read zero-sized boxes. `view --headless` is instead a first-render snapshot: after load it services ready work in bounded turns, then closes remaining background handles rather than waiting for arbitrary third-party timer or I/O chains. `layout` and `render` retain their explicit full-drain/`--auto-close` policy. The event-loop state remains document-context-owned, as required by D5.4.4.
