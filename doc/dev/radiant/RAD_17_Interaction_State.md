...
## 2. The generic state core: `state_set`
...
`state_set` interns the name, upserts the entry via `hashmap_set` (replacing in place — the map stores entries by value), invokes an already-attached entry callback if present, sets `state->is_dirty`, bumps the monotonic `version`, and finally calls `state_assert_after_mutation`. The unused public callback-registration API was retired; the callback fields remain part of the internal entry representation. The `version` counter is the coarse change clock the whole store shares — `last_modified` stamps and the selection sequence counters all read from it.
...
| `SelectionPresentation`, `FocusState` | `state_store_internal.hpp` | Selection geometry/blink/dirty-rectangle cache with no logical boundaries, plus focus navigation state. |
...
`ViewState` (`event.hpp`) deserves emphasis: it is a bit-packed union, so a given view is *either* a scroll surface *or* a form control, never carrying both payloads — the `ViewStateKind` discriminant (`VIEW_STATE_SCROLL`/`VIEW_STATE_FORM_CONTROL`, `event.hpp`) selects the arm. This is what lets pseudo-class reads (`view_state_get_hovered`, `event.hpp`) and form-value reads stay allocation-free on the common path. `SelectionPresentation` stores only `float` caret/range geometry, blink state, iframe offsets, and the previous paint rectangle; anchor/focus nodes and offsets are always derived from `DomSelection` + `EditingSelection` ([RAD_18 — Editing, Selection & DOM Ranges](RAD_18_Editing_Selection_Ranges.md)).
...
The state machine holds **no** state. Its job is to (a) map a caller's intent to a schema `SmEvent`, (b) mutate `DocState` through the existing writers, and (c) validate. There are per-family transition entry points — `focus_transition`, `caret_transition`, `selection_transition`, `hover_transition`, `active_transition`, and `drag_transition`. Each follows the same shape: map the transition kind to an `SmEvent`, open a `SmTransitionGuard`, enter the transition, perform the mutation through the ordinary writer, leave, refresh presentation when needed, commit the guard, then validate.
...
`radiant_state_validate_interaction` delegates to `radiant_state_validate_interaction_schema`, which walks `RADIANT_INVARIANTS`, skips bindings that do not match the derived current state, and runs the matching primitive validator. Roughly twenty `validate_*` functions back these primitives — focus/hover/active/drag graph walks, canonical caret/selection and presentation checks, editing-surface / false-island / target-range checks for rich editing, the view-state registry, dirty-tracking, and input-event ordering. `radiant_state_assert_valid` is a debug-only assert that logs the failing invariant's message and `assert()`s; under `NDEBUG` it is a no-op.
...
State snapshots reuse Lambda's own data model so they are diffable text usable by layout-regression and event-simulation tooling. `radiant_state_dump_mark` spins up a scratch pool and a Lambda `Input`, builds a Mark element tree using `MarkBuilder`, and serializes it with `print_root_item`. The dump collects the `state_map` grouped by node, emits document-level attributes, and recurses the DOM root. Diffing helpers (`state_dump_*_changed`/`differs`) suppress default-valued state so a snapshot shows only what deviates.

`radiant_state_dump_emit_cascade` appends a per-cascade `<entry cascade:… seq:…>` record to an open `StateDumpLog` (path `./temp/state/state_${pid}_${doc}.mark`, `event.hpp`). Because settle calls this on every cascade, a session produces a chronological, diffable trail of interaction-state evolution.
...
On the output side, rendering derives logical selection boundaries from `EditingSelection`/`DomSelection` and reads only geometry and animation from `SelectionPresentation`; focus, dropdown, context-menu, and video placement remain presentation caches. The `DirtyTracker`/`ReflowScheduler` and the `needs_repaint`/`needs_reflow` flags gate incremental relayout and redraw ([RAD_13 — Render Walk & Painters](RAD_13_Render_Walk_Painters.md)); `state_sync_dirty_flags_before_assert` keeps those flags aligned with the authoritative dirty tracker before each settle so the render loop and the invariant checks observe one source. The `EventStateLog` and `StateDumpLog` feed diagnostics and regression tests.
...
1. **`state_store.cpp` remains a large monolith** spanning interned names, dump, lifecycle, generic state, constraints, dropdowns, immutable mode, batching, dirty/reflow, caret/selection presentation, focus, and clipboard. It is the prime split candidate in this subsystem — a natural decomposition follows the function-prefix families (`doc_state_*`, `form_*`, `view_state_*`, `selection_*`, `caret_*`, `state_dump_*`).
...
3. **`s_in_batch` is a file-static global**, not a `DocState` field. It gates whether mutation-time asserts fire, so it is not per-document and is unsafe if multiple documents or threads mutate state concurrently. *Improvement:* move it into `DocState` alongside `active_cascade_depth`/`transition_depth`.
4. **Selectionchange coalescing spans document and text-control drains.** `selection_mutation_seq`/`selection_event_seq` plus the per-text-control `tc_selectionchange_head` linked-list drain form a subtle ordering surface with no single owner.
5. **Cached `View*`/`DomElement*` pointers require prune-after-reflow.** Numerous `DocState` fields (`hover_target`, `active_target`, `cursor->view`, drag/drop views, editing surfaces) hold raw pointers that go stale on relayout and depend on `state_store_prune_after_reflow` being called at the right time.
...
| `radiant/state_store.cpp` | `state_set`/`state_set_bool`, batching (`s_in_batch`), `state_assert_after_mutation`, state dump emission, canonical selection accessors, `state_store_prune_after_reflow`, immutable-mode stub, and typed state writers. |
| `radiant/state_store_internal.hpp` | Presentation-only `SelectionPresentation` and focus navigation state. |
...
- [RAD_13 — Render Walk & Painters](RAD_13_Render_Walk_Painters.md) — consumes the `DirtyTracker`/`ReflowScheduler`, canonical selection snapshots, and presentation geometry this store maintains.
