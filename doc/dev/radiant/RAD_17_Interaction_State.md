...
>
> **Primary sources:** `radiant/state_store.hpp` / `state_store.cpp` (the `DocState` struct, `state_set`/`state_set_bool`/`state_on_change`, batching, `radiant_state_dump_mark`, prune-after-reflow), `radiant/state_store_internal.hpp` (legacy `CaretState`/`SelectionState`/`FocusState` projections), `radiant/state_machine.hpp` / `state_machine.cpp` (transition APIs, cascade boundary, `radiant_state_settle`, invariant checking), `radiant/state_schema.hpp` / `state_schema.cpp` (`SmFamily`/`SmEvent`/`SmInvariantId` enums, `RADIANT_STATE_RULES`, `RADIANT_INVARIANTS`, `sm_derive_*`, `sm_classify_view`, `SmTransitionGuard`).
...
`DocState` (`state_store.hpp:318`) is a single, per-document, mutable C struct. Every interactive fact about a rendered document lives directly in its fields: who has focus, what is hovered or pressed, the current text selection and caret, the IME composition, an in-flight drag-and-drop, the open `<select>` dropdown and native context-menu overlays, live form-control values, scroll offsets, and the incremental dirty/reflow queues. It is owned one-per-document by a thin `StateStore` wrapper (`state_store.hpp:186`) that carries the document pointer plus the pool and arena; `state_store_create` (`state_store.cpp:983`) allocates the store and its `DocState`, and `state_store_doc_state` (`state_store.cpp:1027`) returns the canonical projection.
...
The one genuinely reactive primitive is a `HashMap` keyed by `StateKey` and holding `StateEntry` values. A `StateKey` (`state_store.hpp:52`) is `{ void* node, const char* name }` where `name` is an interned string such as `:hover` or `caret-offset`; a `StateEntry` (`state_store.hpp:66`) carries the `Item value`, a `last_modified` version stamp, and an optional `on_change` callback plus user data. This is the mechanism behind generic per-node boolean/`Item` flags.
...
|---|---|---|
| `state_map` | `state_store.hpp:325` | `StateKey → StateEntry` — the generic reactive per-node flag map ([§2](#2-the-reactive-core-state_set--state_on_change)). |
| `view_state_map` | `state_store.hpp:326` | `(view_id, kind) → ViewStateEntry` — lazily-created per-view interaction state. |
| `ViewState` | `state_store.hpp:133` | Packed per-view struct: hover/active/focus flag bits plus a union of `scroll{x,y,max,drag}` and `form{disabled/readonly/checked, selected_index, range_value, selection_start/end, current_value}`. |
| `template_state_map`, `render_map` | `state_store.hpp:331,335` | Lambda reactive-template reconciliation maps (see `template_state.h` / `render_map.h`) — the observer keys that drive template re-render and the DOM↔source bridge. |
| `dom_selection`, `EditingSelection sel` | `state_store.hpp:373,380` | **Canonical** DOM-spec selection + the StateStore selection authority for the active editing surface. |
...
| `EditingInteractionState editing` | `state_store.hpp:275` | Shared editing-controller projection: active surface, pointer-selection, IME composition, autoscroll, and the rich-edit transaction FSM fields. |
| `DirtyTracker`, `ReflowScheduler` | `state_store.hpp:86,120` | Incremental repaint/relayout queues (a linked list of `DirtyRect`, and a queue of `ReflowRequest` with a `ReflowScope`). |
| dropdown / context-menu overlay fields | `state_store.hpp:410–426` | `open_dropdown` + geometry, and `context_menu_target` + geometry + hover index, drawn as document-level overlay layers. |

`ViewState` (`state_store.hpp:133`) deserves emphasis: it is a bit-packed union, so a given view is *either* a scroll surface *or* a form control, never carrying both payloads — the `ViewStateKind` discriminant (`VIEW_STATE_SCROLL`/`VIEW_STATE_FORM_CONTROL`, `state_store.hpp:126`) selects the arm. This is what lets pseudo-class reads (`view_state_get_hovered`, `state_store.hpp:571`) and form-value reads stay allocation-free on the common path. The legacy projection structs in `state_store_internal.hpp` are plain structs of `float` geometry and `int` offsets; they exist purely so rendering and validation paths that predate the DOM-spec selection can keep reading caret/selection geometry while the canonical model is `DomSelection` + `EditingSelection` ([RAD_18 — Editing, Selection & DOM Ranges](RAD_18_Editing_Selection_Ranges.md)).

The interned pseudo-class and form state names, plus the `CHANGE_*` reflow-reason bitmask, are defined at the bottom of `state_store.hpp:1356–1410`.
...
FSM states are *derived*, not stored. `sm_derive_state` (`state_schema.cpp:767`) dispatches by `SmFamily` to a per-family deriver — `sm_derive_focus_state`, `sm_derive_selection_state`, `sm_derive_rich_edit_state`, and so on (`state_schema.cpp:642–801`) — each of which reads `DocState` fields and returns the current FSM-state enum value for that family (`SelectionFsmState`, `FocusFsmState`, … `RichEditFsmState`, defined `state_schema.hpp:45–132`). `sm_classify_view` (`state_schema.cpp:804`) maps a `View*` to an `SmViewClass` (text control, checkbox, radio, select, range, …) by inspecting its `FormControlProp`. Because these derive on demand, a transition never has to write "the new FSM state" anywhere — it writes the underlying fields and the deriver reports whatever those fields imply.
...
The schema is two static tables. `RADIANT_STATE_RULES[]` (`state_schema.cpp:236`, **80 rules**) enumerates, per `(family, view_class, from_state, event)`, the allowed `to_states`, the side-effect `actions` bitmask (`SmActionFlag`, `state_schema.hpp:235` — e.g. `SM_ACT_DISPATCH_BEFOREINPUT`, `SM_ACT_MUTATE_DOM`), and a human-readable name. `RADIANT_INVARIANTS[]` (`state_schema.cpp:517`, 26 bindings over 21 distinct `SmInvariantId` values, `state_schema.hpp:209`) binds each invariant to the family/state where it must hold. The families themselves are `SmFamily` — 15 of them (`state_schema.hpp:12`: document, focus, selection, IME, hover, active, drag-drop, scroll, the four form families, dropdown, context-menu, rich-edit) — and events are `SmEvent`, ~67 of them (`state_schema.hpp:134`).

`SmTransitionGuard` (`state_schema.hpp:302`) is an RAII debug recorder. On construction (`sm_transition_scope_begin`, `state_schema.cpp:861`) it snapshots the family, event, view class, target, and the *current* `from_state` (via `sm_derive_state`) into the `DocState`'s `sm_active_transition` scope. Writers call `sm_observe_action` (`state_schema.cpp:883`) to record which side effects actually fired (e.g. `SM_ACT_WRITE_CHECKED` from `form_control_set_checked_for_event`, `state_store.cpp:4125`); `guard.commit()` marks the transition legitimate. Under `NDEBUG` the whole guard compiles to empty inline stubs (`state_schema.hpp:317`), so it costs nothing in release builds. This is the mechanism by which the schema's declared `actions` can, in debug, be checked against what the code really did.
...
3. **Dual selection/caret model with duplicated write paths.** Canonical `DomSelection`/`EditingSelection` coexist with legacy `CaretState`/`SelectionState` projections (`state_store_internal.hpp`), and the `state_store_legacy_selection_*` writers (`state_store.hpp:799–829`) mirror into the canonical model. Several invariants — notably `SM_INV_LEGACY_SELECTION_PROJECTION` (`state_schema.hpp:230`) — exist only to police this duplication. Migrating call sites off the legacy API would let both the projections and those invariants be deleted.
...
|---|---|
| `radiant/state_store.hpp` | `DocState` struct, `StateKey`/`StateEntry`, `ViewState`, `DirtyTracker`/`ReflowScheduler`, the full state/selection/form/focus API surface, interned-name and `CHANGE_*` macros. |
...
| `radiant/state_store_internal.hpp` | Legacy `CaretState`/`SelectionState`/`FocusState` render/validation projection structs. |
| `radiant/state_machine.hpp` / `.cpp` | Transition APIs (`focus_transition` … `drag_transition`), `transition_enter`/`leave`, the cascade boundary (`state_begin_event_cascade`/`radiant_state_settle`/`state_end_event_cascade`), `radiant_state_validate_interaction`, `radiant_state_assert_valid`, the `validate_*` invariant primitives. |
| `radiant/state_schema.hpp` / `.cpp` | `SmFamily`/`SmEvent`/`SmInvariantId`/`SmActionFlag` enums, per-family FSM-state enums, `RADIANT_STATE_RULES`/`RADIANT_INVARIANTS` tables, `sm_derive_*`, `sm_classify_view`, `SmTransitionGuard`/`SmTransitionScope`. |
| `radiant/source_pos_bridge.{hpp,cpp}` | `(DomNode*,offset) ↔ (source_path,offset)` mapping over `render_map` (detailed in RAD_01). |
