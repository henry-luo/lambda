...
4. **Source-position bridge maturity.** The header warns that functions may be no-op `return false` pending `render_map` path recording (`source_pos_bridge.hpp:24-27`); editor round-trip fragility is concentrated here and depends on a weak/incremental `render_map` dependency.
