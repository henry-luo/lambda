...
`struct ViewTree` (`view.hpp:1451`) owns two allocators plus the root: a bump `Arena` (O(1) allocation, bulk free) for node-scale allocation, and a `Pool` for property groups. `view_pool_init` creates the pool (`MEM_ROLE_VIEW`) and arena.

Property groups are always pool-allocated through `alloc_prop`, which is a `pool_calloc` from `view_tree->pool` with a corruption guard, plus the typed wrappers `alloc_inline_prop`/`alloc_block_prop`/`alloc_font_prop`/`alloc_position_prop`/`alloc_flex_prop`/`alloc_grid_prop`/…. Because the groups all come from one pool, a relayout can free them wholesale once the teardown visitor has released external handles and nulled retained DOM pointers.

Teardown is a single table-driven visitor. `view_teardown_visit_node` walks normal children plus generated pseudo nodes, applies `VIEW_PROP_TEARDOWN` for each property group, and is parameterized by flags for external-resource release, pointer clearing, pool frees, and node frees. `free_view`, `view_pool_release_detached_subtree`, `view_pool_reset_retained`, and `view_pool_destroy` all use that same visitor, so a new pool-owned pointer registers its release/clear/free hooks in one table entry instead of being threaded through several walks. Replaced-element child walking remains explicit so fallback font handles on select/textarea/button DOM children are still released.
...
`view_pool_reset_retained` is the mutation-safe reset: it releases and clears view-owned pointers on the existing tree, destroys the arena+pool, and re-inits them — but **keeps the DOM/view nodes themselves**. This is what lets `DocState`/`ViewState` weak references, which bind by the node's monotonic `id` (`dom_node.hpp:76`) rather than by pointer, survive a relayout. `view_pool_release_detached_subtree` handles a subtree that a mutation detached, so the final `view_pool_destroy` walk does not lose reachability to it.
...
2. **`native_element` is a transitional copy, not an alias.** `DomElement::native_element` is marked `TODO(Phase 4): Remove; use dom_element_to_element()` (`dom_element.hpp:307`), and the embedded `Element` is currently *copied* from the source element in `dom_element_init` rather than aliased. Until removed, there are two notions of "the element" to keep in sync.
3. **Incremental relayout correctness is implicit.** The skip-clean optimization depends on `layout_height_contribution` being exactly recomputed on every non-skipped child (`layout_block.cpp`). There is no assertion tying the cached contribution to the actual advance, so a bug there manifests as silently mis-placed siblings rather than a crash.
4. **Source-position bridge maturity.** The header warns that functions may be no-op `return false` pending `render_map` path recording (`event.hpp`); editor round-trip fragility is concentrated here and depends on a weak/incremental `render_map` dependency.
5. **Class-name handling is still a single-token stub.** The deprecated `RDT_VIEW_MATH` enum span has been removed, but the class-name path in `view_pool.cpp` still assumes a single class ("TODO: split on whitespace for multiple classes").
...
| `radiant/view_pool.cpp` | `set_view` (in-place tagging), `alloc_prop` families, `VIEW_PROP_TEARDOWN` / `view_teardown_visit_node` teardown, `view_pool_init/reset_retained/destroy`. |
| `lib/tagged.hpp` | `namespace lam` cast helpers: `view_require_*`, `dom_view`/`view_dom_node`, `dom_as<T>`, the `unsafe_*` family. |
| `radiant/render.hpp` | Ownership-tracked retention (`PersistentFieldRef`/`PoolPtr`/`SessionPtr`) for heap buffers that outlive a pool swap (font family, bg image, marker text, image source). |
| `radiant/event.hpp` / `source_pos_bridge.cpp` | `(DomNode*,offset) ↔ (source_path,offset)` mapping over `render_map` for the editor. |
