...
> **Last verified against tree:** 2026-09-22 *(unified UA/model action snapshot, revisioned selection completion, and Selection API single-range correction)*
...
### 2.3 `DomSelection` — single range, Selection API-compatible

`struct DomSelection` (`event.hpp`) holds `ranges[DOM_SELECTION_MAX_RANGES]` where **`DOM_SELECTION_MAX_RANGES == 1`** (`event.hpp`). The [Selection API](https://w3c.github.io/selection-api/) defines one range per document selection: `rangeCount` is 0 or 1, and `addRange()` aborts when a range is already present. Radiant therefore supports `range_count ∈ {0,1}` rigorously and **silently ignores any `addRange()` beyond the first**, matching the specification and Chromium (`event.hpp`). A collapsed selection *is* the canonical caret. The `direction` field (`DOM_SEL_DIR_FORWARD`/`BACKWARD`, `event.hpp`) records anchor-vs-focus ordering. `associated_doc_root` (`event.hpp`) captures the document root when the range was set; the Range mutators use it to implement the "selection range moved into a different root" drop rule (`dom_range.cpp:349-356`, `705`, `720`) — when the active range's new root differs from the captured one, the range is dropped and `range_count` falls to 0.
...
`dom_selection_modify` with `dom_boundary_move` implements the browser-convention `Selection.modify` surface: `move` collapses at the moved focus and `extend` retains the anchor; `forward`/`backward` are logical directions, while `left`/`right` use visual, bidi-aware character and word paths. It accepts `character`, `word`, `line`, `lineboundary`, `sentence`, `sentenceboundary`, `paragraph`, `paragraphboundary`, and `documentboundary`; an empty selection is a no-op and invalid arguments report `SyntaxError`. Character movement follows visible text and collapsed whitespace; line and line-boundary movement use the editing-host layout stops when available; paragraph movement recognizes block and `<br>` boundaries; and document-boundary movement reaches the DOM-tree edges. It is a checked native DOM/Selection mechanism rather than a separate contenteditable default action, consistent with **D7.2.5**.

Movement remains within an editing host and `text_is_selectable_for_modify` skips non-selectable subtrees. The fidelity limit is narrow: word breaks use a simple alphanumeric classifier, and sentence plus some line fallbacks are structural approximations rather than locale-aware Unicode segmentation. These are `Selection.modify` conformance limits, not a general contenteditable-support gap. `SelectionModify*` cases in `test/test_dom_range_gtest.cpp` cover the base move/extend, collapsed-whitespace, empty-selection, and invalid-argument contracts; `test/ui/dom_pkg_edge_text.json` covers paragraph-boundary traversal through an empty inline.

Browser-style boundary discovery lives alongside: `dom_selection_compute_select_all_boundaries` (trimming whitespace-only edges and treating `<br>`/`<table>` as edge stops), `user_select_all` handling, and `dom_selection_triple_click_range_for_node` (table-cell-aware). Stringification is `dom_range_to_string` and `dom_range_to_string_ex` with two modes — `DOM_STRINGIFY_RAW` matching `Range.toString()` and `DOM_STRINGIFY_RENDERED` matching `Selection.toString()`, which skips text hidden by `user-select: none`/`content-visibility` or by the not-rendered-as-text tag list (`event.hpp`).
...
5. **`Selection.modify` fidelity.** All accepted argument families are implemented, including bidi-aware visual left/right character and word movement. Word segmentation remains a simple alphanumeric classifier, while sentence and some layout-free line fallbacks are structural approximations; locale-sensitive and complex-script results can diverge from a browser. This is a bounded Selection-API conformance limit, not a general contenteditable-support gap; see [§6](#6-dom-range-mutation-selection-modify-stringification).
6. **Single-range selection.** `DOM_SELECTION_MAX_RANGES == 1`; extra `addRange()` calls are silently ignored, as required by the Selection API. This is not a conformance gap.
