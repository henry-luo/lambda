...
2. **Projection-cache consumers remain.** The canonical `DomSelection`/`EditingSelection` model still feeds `CaretState`/`SelectionState` projection caches for render/debug paths (`dom_range.hpp:8-11`, `dom_range_resolver.hpp:95-106`). *Improvement:* migrate renderers to the DomRange layout cache directly and delete the projection structs plus their invariants.
