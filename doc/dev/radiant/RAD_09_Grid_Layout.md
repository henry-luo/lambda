...
>
> **Primary sources:** `radiant/layout_grid_multipass.cpp`/`.hpp` (the multipass driver `layout_grid_content`), `radiant/layout_grid.cpp` (`layout_grid_container` — the grid algorithm proper), `radiant/grid_positioning.cpp` (track offsets + alignment), `radiant/grid_sizing.cpp` (track allocation + enhanced-sizing wrapper), `radiant/grid_utils.cpp` (template-area parsing + intrinsic API); headers `radiant/grid.hpp` (legacy state), `radiant/grid_types.hpp` (dual coordinates), `radiant/grid_track.hpp` (enhanced tracks), `radiant/grid_occupancy.hpp` (`CellOccupancyMatrix`), `radiant/grid_placement.hpp` (placement), `radiant/grid_baseline.hpp` (baseline), `radiant/grid_sizing_algorithm.hpp` (§11 algorithm, ~1848 lines), `radiant/grid_enhanced_adapter.hpp` (the legacy↔enhanced seam, ~1305 lines).
...
**Legacy layer** (`grid.hpp`, C linkage): `GridContainerLayout : GridProp` (`grid.hpp:102`) is the central mutable state — `computed_rows`/`computed_columns` arrays of `GridTrack` (`grid.hpp:75`), the `grid_items` pointer array, explicit/implicit/negative-implicit counts, auto cursors, container/content dimensions, the `auto_fit_columns[64]`/`auto_fit_rows[64]` masks, ownership flags, and a `lycon` back-reference. `GridTrackSize` (`grid.hpp:47`) is the *parsed* CSS spec (type enum `GridTrackSizeType` at `grid.hpp:34`: `LENGTH`/`PERCENTAGE`/`FR`/`MIN_CONTENT`/`MAX_CONTENT`/`AUTO`/`FIT_CONTENT`/`MINMAX`/`REPEAT`), carrying `minmax` children and `repeat()` metadata (`is_auto_fill`/`is_auto_fit`). Per-item placement inputs and computed outputs live on `GridItemProp`, reached through `grid_item_prop(item)` (`grid.hpp:20`), which also routes form-control items via `item->form->grid_item`.

**Enhanced (Taffy) layer** (`namespace radiant::grid`): the working structs the algorithm actually operates on. The Taffy heritage is stated in code, not just folklore — explicit "Based on Taffy" attributions appear at `grid_types.hpp:7`, `grid_track.hpp:7`, `grid_occupancy.hpp:7`, `grid_placement.hpp:15`, `grid_baseline.hpp:8`, and `grid_sizing_algorithm.hpp:16` ("Based on Taffy's implementation with adaptations for Radiant's architecture"), with the ItemBatcher pattern called out at `grid_sizing_algorithm.hpp:526`/`543`. §2 covers these structs.
...
### 2.1 Dual coordinate system (`grid_types.hpp`)

Grid lines are represented in two coordinate systems, following Taffy's design (documented at `grid_types.hpp:7-24`). `GridLine` (`grid_types.hpp:127`) is the CSS-spec 1-based coordinate: line 1 is the start of the explicit grid, line -1 is its end, 0 is invalid. `OriginZeroLine` (`grid_types.hpp:71`) is the normalized 0-based coordinate the algorithm computes in: 0 is the explicit-grid start, negatives extend up/left into the negative implicit grid. `OriginZeroLine` carries the helpers `implied_negative_implicit_tracks()` (`grid_types.hpp:106`) and `implied_positive_implicit_tracks(explicit_count)` (`grid_types.hpp:113`) that tell placement how many implicit tracks a given line forces into existence. `LineSpan` (`grid_types.hpp:160`), `TrackCounts` (`grid_types.hpp:186`, three fields: `negative_implicit`, `explicit_count`, `positive_implicit`, with conversion helpers between the two coordinate systems), `AbsoluteAxis` (`grid_types.hpp:280`), and the `CellOccupancyState` enum (`grid_types.hpp:58`: `Unoccupied`/`DefinitelyPlaced`/`AutoPlaced`) round out the module.
...
|---|---|
| `radiant/layout_grid_multipass.cpp` / `.hpp` | The multipass driver `layout_grid_content`; passes 0–3, depth guard, pass cache, Pass-3 row-height reconciliation, per-item content layout (flex reuse). |
...
| `radiant/grid.hpp` | Legacy C-layer state: `GridContainerLayout`, `GridTrack`, `GridTrackSize`, `grid_item_prop`. |
| `radiant/grid_types.hpp` | Dual `GridLine`/`OriginZeroLine` coordinates, `TrackCounts`, `LineSpan`, `CellOccupancyState`. |
...
| `radiant/grid_baseline.hpp` | Row baseline grouping, first-baseline computation, baseline shims. |
