...
>
> **Primary sources:** `radiant/view.hpp` (`struct UiContext` at `view.hpp:1491`), `radiant/radiant.hpp` + `radiant/ui_context.cpp` (`ui_context_init`, `free_document`, `ui_context_cleanup`), `radiant/radiant.hpp` + `radiant/window.cpp` (`view_doc_in_window_with_events_internal`, the main loop, `render`, `show_html_doc`), `radiant/radiant.hpp` + `radiant/browsing_session.cpp` (`session_navigate`, history stack, `BROWSE_HISTORY_MAX`), `radiant/radiant.hpp` + `radiant/frame_clock.cpp` (`RadiantFrameClock`), the CLI dispatch in `lambda/main.cpp` (`layout`/`render`/`view` subcommands), and the per-format loaders in `radiant/cmd_layout.cpp` (`load_html_doc`, `load_lambda_html_doc`, `load_svg_doc`, `load_xml_doc`, `load_text_doc`, `load_lambda_script_doc`, `load_markdown_doc`, `load_latex_doc`, `load_wiki_doc`) plus `dom_document_create` / `build_dom_tree_from_element` (defined in `lambda/input/css/dom_element.cpp`).
...
Radiant has three externally visible modes — `layout` (parse + style + layout, emit the view tree as JSON), `render` (rasterize/vectorize to SVG/PDF/PNG, [RAD_13](RAD_13_Render_Walk_Painters.md)), and `view` (open an interactive window). All three share one runtime object: the global `UiContext` in `window.cpp`. The shell's job is to construct that context, load a document into it, and — for `view` — run a frame loop until the window closes. Everything downstream (style, layout, paint) reads and writes through the same context.

The CLI entry points live in `lambda/main.cpp`: `strcmp(argv[1], "layout")` routes to `cmd_layout`, `"render"`/`"render-batch"` to `cmd_render_batch`, and `"view"` to `view_doc_in_window_with_events`. PDF, LaTeX, and graph sources select a native document-transform configuration (input type, package, public export) and pass typed options to it; they never become synthesized Lambda source. The design bias is deliberately single-window and single-document: the global `ui_context` lets GLFW's C callbacks reference it without a user-pointer indirection, at the cost of tabs/multi-document support ([§8](#8-known-issues--future-improvements)).
...
`view_doc_in_window_with_events_internal` is the driver. Setup order: `ui_context_init` → optional `event_sim_load` → `js_dom_set_ui_context(&ui_context)` → `js_dom_set_host_driven_loop(host_driven_loop)` where `host_driven_loop = (!headless || sim_ctx != nullptr)` → load the document (`load_doc_by_format`, or a configured native transform for optioned graph views) → conditionally initialize network support for HTTP documents (`network_downloader_init_shared` + `enhanced_cache_create`) → `session_create` → pre-layout resource discovery. It then splits into a headless synchronous path (running `event_sim_update` in a loop and pumping `js_event_loop_pump_nowait` between events) and the GUI frame loop.
...
`BrowsingSession` (`radiant.hpp`) owns **only navigation history**. Networking (the `NetworkThreadPool` and `EnhancedFileCache`) is created and owned by `window.cpp` and merely *referenced* by the session (`session->thread_pool`/`session->file_cache` are borrowed, freed by the caller — `session_create` at `browsing_session.cpp:74`, note at `browsing_session.cpp:98`). The history is a growable array of `HistoryEntry` (`radiant.hpp`: owned `Url*`, owned `char* title`, `float scroll_y`), with `history_count`/`history_index`/`history_capacity` and a hard cap `#define BROWSE_HISTORY_MAX 100` (`radiant.hpp`).
...
`show_html_doc` (`window.cpp:422`) is the session's loading hook: it calls `load_html_doc`, sets screen `output_scale` to 1, installs the document on `ui_context.document`, derives `raster_scale` from the current device scale (RSC7), ensures `DocState`, processes `@font-face`, lays out, and renders. Because it goes through `load_html_doc`, all format dispatch (below) is reachable from a navigation.
...
- **HTML / HTTP / default** — `load_lambda_html_doc`. The richest path. It loads source (memory/file/HTTP), converts non-UTF-8 charset, parses with the HTML5 parser, creates the `DomDocument`, builds the DOM tree, extracts viewport and `<base href>`, collects external/inline/`@import` stylesheets in document order, applies inline `style=""`, then applies the initial CSS cascade **before** load-time scripts so CSSOM reads see cascaded values. After `execute_document_scripts`, any JS DOM/class/style or `<style>` mutation triggers stylesheet re-collection and a post-script recascade that preserves JS inline style writes; inline event handlers are compiled after script execution. The legacy scripts-before-cascade order is still available with `RADIANT_SCRIPT_BEFORE_CASCADE=1`. JS scripting is detailed in [RAD_21](RAD_21_JS_Scripting_Integration.md).
...
- **`.ls` Lambda script** — `load_lambda_script_doc`. Allocates a heap `Runtime`, creates a retained result `Input` with `ui_mode=true`, sets `runtime->ui_mode=true` and `runtime->result_arena=result_input->arena`, runs the file with MIR, then interprets the result: a full `<html>` element is used directly; another element is wrapped in `html > body`; a scalar/text value is stringified into a `<div>`; an SVG element/string is wrapped as in-memory HTML; an HTML string is parsed. It builds the `DomDocument` from the retained input and stores `dom_doc->lambda_runtime` so the heap, JIT code, name pool, generated nodes and event-handler state stay live for an interactive script document.
- **PDF / LaTeX / graph** — `load_lambda_document_transform_doc` selects a `LambdaDocumentTransformConfig`, loads its actual shipped package file, parses the input using the configured type, then invokes the configured public export with a typed option map. PDF's package owns its default page cap; LaTeX and graph package entries own their view defaults. The retained runtime keeps generated nodes and custom layout callbacks alive. This follows the ordinary package/public-export boundary of **D7.2.1–D7.2.2**.
- **Markdown + math** (`.md`, `.markdown`) — `load_markdown_doc` parses Markdown, walks `<math>` nodes (parent, child index, source, display/inline mode), parses each source through the native `parse` primitive, and invokes `lambda.doc.math.math.render_math` directly with a typed display option. Generated elements are spliced directly into the retained Markdown tree; no Lambda source is constructed.
...
| `radiant/radiant.hpp` + `radiant/browsing_session.cpp` | `BrowsingSession`/`HistoryEntry`, `session_navigate`/`go_back`/`go_forward`, `BROWSE_HISTORY_MAX`, title extraction, scroll restoration. |
| `radiant/radiant.hpp` + `radiant/frame_clock.cpp` | `RadiantFrameClock` time base and next-timeout used by the main loop (owned by [RAD_16](RAD_16_Animation_Frame_Scheduling.md)). |
...
| `lambda/main.cpp` | CLI dispatch for `layout`/`render`/`view`. |
