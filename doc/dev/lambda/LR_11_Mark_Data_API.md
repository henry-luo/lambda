...
- **`ElementBuilder`** (`mark_builder.hpp:338`). Its constructor (`mark_builder.cpp:349`) allocates a `DomElement` (ui_mode) or a bare `elmt_arena` element, starts the element on its tag's shared root in the `Input`'s transition tree (`elmt_tree_root`, D3.4.3v2), falling back to a fresh private `TypeElmt` registered in `input->type_list` when the tag name is not pooled or the element budget is spent. `attr(...)` overloads pool the key via `createName` and delegate to `putToElement` → `elmt_put_tree`, which follows or mints the attribute's edge in the transition tree and falls back to the private-type `elmt_put`. `child()`/`text()`/`children()` `array_append` onto the element-as-list; in ui_mode `child()` re-wraps a plain `String` child as a fat `DomText` (`:447`–`461`). `final()` (`:531`) calls `elmt_finalize_shape` for shape-pool dedup; the child count lives only in the element's `List::length`.
- **`MapBuilder`** (`mark_builder.hpp:463`). Its constructor `arena_calloc`s a `Map` with the `&EmptyMap` placeholder type, replaced on first `put` (`mark_builder.cpp:509`–`521`). `put(...)` overloads route to `putToMap` → `map_put` (`:341`), caching the resolved `TypeMap*`. `final()` (`:592`) calls `map_finalize_shape`, now a no-op: `map_put` has already placed the map on its `Input`'s transition tree (D3.4.3v2).
...
Maps and elements share their types differently (**D3.4.3v2**). A map shares as it is built: each `put` follows the per-`Input` transition tree, so maps built by the same sequence of adds end on one `TypeMap` ([LR_03](LR_03_Value_and_Type_Model.md) §4), and `map_finalize_shape` is a no-op. An element's `final()` calls `elmt_finalize_shape`, which swaps the just-built attribute chain for the `ShapePool`'s copy; only elements left on a private type take this path — an element on the tree already shares its whole `TypeElmt`, built attribute by attribute through `elmt_put_tree`. The shape-construction primitive the editor uses is `ShapeBuilder` (§5), and the pool is owned by [LR_08](LR_08_Memory_and_GC.md).
...
`ShapeBuilder` (`shape_builder.hpp:23`) is the C primitive the editor uses to construct a deduplicated shape (`container_rebuild_with_new_shape`, and the tag rename); builders no longer use it, since maps share through the transition tree and `elmt_finalize_shape` calls the pool directly. It is a flat struct holding parallel `field_names[]`/`field_types[]` arrays capped at `SHAPE_BUILDER_MAX_FIELDS = 64` (`shape_builder.hpp:6`,`25`), a `field_count`, and the element-vs-map discriminator. `shape_builder_init_map`/`shape_builder_init_element` (`shape_builder.hpp:43`,`48`) set it up; `add_field` (`:60`) appends and replaces on name collision (returning false past the cap); `remove_field`/`has_field`/`get_field_type` query it; `import_shape` (`:99`) imports an existing `ShapeEntry` chain (silently truncating past 64, [LR11-4](../../../vibe/Lambda_Issue_Ledger.md)); `clear` resets it.

`shape_builder_finalize` (`shape_builder.hpp:121`, impl `shape_builder.cpp:158`) delegates to `shape_pool_get_map_shape`/`shape_pool_get_element_shape`, returning a **pool-owned, deduplicated** `ShapeEntry*` that the caller must not free. It is one of two sharing mechanisms: element attribute chains and editor rebuilds share `ShapeEntry` chains through the pool, while maps built by `map_put` share whole `TypeMap`s through the transition tree (D3.4.3v2, [LR_03](LR_03_Value_and_Type_Model.md) §4). The `ShapePool` itself, and how shapes interact with the GC and name pool, are owned by [LR_08 — Memory Management & Garbage Collection](LR_08_Memory_and_GC.md).
...
- **Type sharing.** Maps built the same way share one `TypeMap` through the per-`Input` transition tree (D3.4.3v2). Element attribute chains and `MarkEditor` rebuilds dedup through `ShapePool` + `shape_builder_finalize`; parsed element types share through the tree as maps do.
