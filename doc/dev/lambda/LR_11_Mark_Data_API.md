...
Recursion and width are bounded: `MAX_DEPTH = 2000` and `MAX_FIELD_COUNT = 10000` (`print.cpp:28`–`29`); past the depth limit it emits the literal `[MAX_DEPTH_REACHED]` (`:800`) and wide structures are clamped. The same file also carries the debug dump `print_ast_node` (the typed AST, `:1002`), which is a debugging facility, not part of the value-serialization contract.
