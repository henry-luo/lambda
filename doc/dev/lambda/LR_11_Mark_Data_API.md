...
Maps and elements share their types differently (**D3.4.3v2**). A map shares as it is built: each `put` follows the per-`Input` transition tree, so maps built by the same sequence of adds end on one `TypeMap` ([LR_03](LR_03_Value_and_Type_Model.md) §4), and `map_finalize_shape` is a no-op. An element's `final()` calls `elmt_finalize_shape`, which swaps the just-built attribute chain for the `ShapePool`'s copy; the `TypeElmt` itself stays private to the element until elements adopt the tree. The shape-construction primitive the editor uses is `ShapeBuilder` (§5), and the pool is owned by [LR_08](LR_08_Memory_and_GC.md).
...
- **Type sharing.** Maps built the same way share one `TypeMap` through the per-`Input` transition tree (D3.4.3v2). Element attribute chains and `MarkEditor` rebuilds dedup through `ShapePool` + `shape_builder_finalize`; element types themselves are due to move onto the tree.
