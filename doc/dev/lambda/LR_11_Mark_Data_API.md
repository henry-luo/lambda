...
- **`ElementBuilder`** (`mark_builder.hpp:338`). Its constructor (`mark_builder.cpp:349`) allocates a `DomElement` (ui_mode) or a bare `elmt_arena` element, starts the element on its tag's shared root in the `Input`'s transition tree (`elmt_tree_root`, D3.4.3v3), falling back to a fresh private `TypeElmt` registered in `input->type_list` when the tag name is not pooled or the element budget is spent. `attr(...)` overloads pool the key via `createName` and delegate to `putToElement` → `elmt_put_tree`, which follows or mints the attribute's edge in the transition tree and falls back to the private-type `elmt_put`. `child()`/`text()`/`children()` `array_append` onto the element-as-list; in ui_mode `child()` re-wraps a plain `String` child as a fat `DomText` (`:447`–`461`). `final()` (`:531`) has nothing left to do: the element's type came from the tree or is its own, and the child count lives only in the element's `List::length`.
- **`MapBuilder`** (`mark_builder.hpp:463`). Its constructor `arena_calloc`s a `Map` with the `&EmptyMap` placeholder type, replaced on first `put` (`mark_builder.cpp:509`–`521`). `put(...)` overloads route to `putToMap` → `map_put` (`:341`), caching the resolved `TypeMap*`. `final()` only returns the map: `map_put` has already placed it on its `Input`'s transition tree (D3.4.3v3).
...
`ShapeBuilder` (`shape_builder.hpp`) is the field list an editor rebuild is described with (`container_rebuild_with_new_shape`, and the tag rename): import the old shape, then add, retype or remove fields. It is a small handle over `(name, TypeId)` drafts grown from the caller's arena with no field cap (SCU10), plus the element-vs-map discriminator; `shape_builder_init_map`/`shape_builder_init_element` set it up, `add_field` appends and replaces on name collision, and `remove_field`/`has_field` query it. `import_shape` and `add_field` return `false` when the drafts cannot grow, and an edit then returns `ItemError`: a partial draft had silently rebuilt a container without some of its fields ([LR11-4](<../../../vibe/Lambda_Issue_Ledger (fixed).md#lr11-4>)). It makes no `ShapeEntry`: the rebuild turns the list into transition-tree steps, a kept field replaying its old record's identity, or into a chain the container owns when the tree declines (D3.4.3v3, [LR_03](LR_03_Value_and_Type_Model.md) §4).

There is no finalize step and no shape pool: the pool was retired with the element plan's P3 (2026-09-25).
...
Recursion is bounded: past `MAX_DEPTH = 2000` it emits the literal `[MAX_DEPTH_REACHED]`. Width is not: a 10,000-field cap had printed any wider map as `{[invalid map_type length]}` ([LR11-4](<../../../vibe/Lambda_Issue_Ledger (fixed).md#lr11-4>)); only a negative length is refused, as corrupt. The same file also carries the debug dump `print_ast_node` (the typed AST, `:1002`), which is a debugging facility, not part of the value-serialization contract.
...
- **Type sharing.** Maps built the same way share one `TypeMap` through the per-`Input` transition tree (D3.4.3v3). Parsed elements and `MarkEditor` rebuilds share the same way; nothing is pooled any more.
...
Moved to the central ledger: **[Lambda Core Runtime — Central Issue Ledger](../../../vibe/Lambda_Issue_Ledger.md)**, entries **LR11-1**, **LR11-7** and **LR11-8** (open), and **LR11-R1**, **LR11-R6**, **LR11-2** – **LR11-5** (resolved, in the [fixed archive](<../../../vibe/Lambda_Issue_Ledger (fixed).md>)). LR11-6 ("conservative safety analysis") was retired on 2026-09-10: the hard-coded gate functions it cited were dead code and are gone — see [LR12 §5](LR_12_Procedural_Runtime.md#5-the-static-safety-analyzer).
...
| `lambda/shape_builder.hpp`/`.cpp` | `(name, TypeId)` field list for editor rebuilds; arena-owned drafts, no cap. |
...
- [LR_08 — Memory Management & Garbage Collection](LR_08_Memory_and_GC.md) — the arenas, pools and `NamePool` that back the builder/reader/editor, and the GC interaction.
