...
3. **~~String-comparison inconsistency~~ (stale — verified 2026-07-16).** `fn_eq`, `fn_lt_scalar`/`fn_gt_scalar`, and the sort total order all compare strings by raw bytes and are mutually consistent. The utf8proc casefold comparators are markup-parser-only, and their Item-level wrappers have no callers ([LR_05 §Known Issues #3](LR_05_Strings_and_Vectors.md#9-known-issues--future-improvements)). Any future collation support must be an explicit opt-in governing equality and ordering together, not an operator change.
4. **Ordered comparison is deliberately partial; residual value-semantics items live in OI-1.** The scalar comparators enumerate numeric/datetime/string cases and return `BOOL_ERROR` for other types (bool/null included) — cross-family `<` is an error while cross-family `==` is `false`, the Python-style split. The formerly-cited equality gaps are re-scoped: `fn_eq` is value-correct and `array_num_eq` is fixed ([LR_03 §Known Issues #1](LR_03_Value_and_Type_Model.md#known-issues--future-improvements), [LR_05 §Known Issues #1](LR_05_Strings_and_Vectors.md#9-known-issues--future-improvements)); what remains is tracked as OI-1 in `vibe/Lambda_Issues_Outstanding.md` (VMap key eq/hash rank consistency, `decimal_cmp` failure-as-equality, `item_deep_equal` dedup).
...
- [LR_10 — Error Handling](LR_10_Error_Handling.md) — the `LambdaError` model, error codes, stack traces, and the compile-time postfix-`^`/braced-handler half of error handling.
