...
This document covers the runtime half of the story: the in-memory error representations, the propagation primitive, raising, formatting, GC integration and stack traces. The *surface syntax* that produces those runtime calls — the `T^E` return-type annotation, postfix `^` propagation, braced handlers, and the `ERR_UNHANDLED_ERROR` "must be handled" rule — is lowered entirely at compile time and is documented where that lowering lives ([LR_02](LR_02_Parsing_AST.md) for the AST, [LR_07](LR_07_MIR_Transpiler_JIT.md) for the emitted MIR). The retired destructure and prefix error-test forms are excluded by **S7.6.5v2**. The language-level semantics for the scripter are in `doc/Lambda_Error_Handling.md`.
...
**Errors are falsy.** `is_truthy` (`lambda-eval.cpp:154`) returns `BOOL_FALSE` for `LMD_TYPE_ERROR` (`:158`) just as it does for `LMD_TYPE_NULL`. Error tests use the explicit `is error` predicate; the retired prefix `^e` shorthand is historical only under **S7.6.5v2**.
...
Moved to the central ledger: **[Lambda Core Runtime — Central Issue Ledger](../../../vibe/Lambda_Issue_Ledger.md)**, entries **LR10-1 – LR10-6** (open/partial) and **LR10-R1 – LR10-R2** (resolved, Appendix A).
...
- [LR_02 — Parsing & AST Construction](LR_02_Parsing_AST.md) — where the `T^E` type, postfix `^` operator, braced handlers and the `ERR_UNHANDLED_ERROR` "must be handled" rule are lowered at compile time, and where `find_errors` runs in the parse stage.
