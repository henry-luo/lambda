...
Everything in the Lambda runtime — script values, parsed documents, JIT-produced temporaries — is a `Item`: a single 64-bit tagged word. This document is the map of that representation: how the type tag is packed, where each kind of value physically lives, how the JIT and the runtime move values in and out of their boxed forms, and how the container types are laid out so that one `get_type_id` dispatch handles all of them uniformly. The *memory* that backs these values (the GC heap, execution side stacks, and the name pool) is owned by [LR_08 — Memory Management & Garbage Collection](LR_08_Memory_and_GC.md); the *code that emits* the boxing operations is owned by [LR_06 — The C Transpiler](LR_06_C_Transpiler.md) and [LR_07 — The MIR Direct Transpiler & JIT](LR_07_MIR_Transpiler_JIT.md); the numeric, string and vector *operations* over these values live in [LR_04 — Numbers, Decimal & DateTime](LR_04_Numbers_Decimal_DateTime.md) and [LR_05 — Strings, Symbols & Vectors](LR_05_Strings_and_Vectors.md).
...
`TypeMap` (`lambda-data.hpp:246`) holds `length`, `byte_size` (the packed-struct size), the shape chain, and — for fast lookup — an open-addressing hash table `field_index` that lives out of line: pool-owned, allocated when first populated, and sized to the shape (a power of two, at least twice the fields; `typemap_hash_prepare`, `typemap_hash_lookup*`). Runtime JavaScript shapes additionally carry one immutable `const JsClassMeta*` selected before publication and preserved by transitions, as ruled by **D3.4.7**; the metadata has no `Item`, realm, GC, mutable-cache, or context-owned fields. The **shape chain remains authoritative** (last-writer-wins) whenever the hash table is unpopulated or saturated. `TypeElmt : TypeMap` (`:914`) adds the element's local `name`, its catalog `name_id`, a `content_list`, and namespace — `content_list` is a declared type's content pattern, matched against the children as a sequence pattern (S11.1.6v3), and NULL on every instance and literal type; `TypeObject : TypeMap` (`:501`) adds a `type_name`, a `base` for inheritance, a method list, and constraint hooks.

### Type sharing: literals, the transition tree, private types

Maps built the same way share one `TypeMap` (**D3.4.3v3**; the design and its history are in [`vibe/Lambda_Design_Shape_Transitions.md`](../../../vibe/Lambda_Design_Shape_Transitions.md)). Only the type is shared: each map keeps its values in its own data buffer. A map gets its type in one of four ways:
...
A path shares its entries, not only its types (D3.4.3v3): a child extends its parent's `ShapeEntry` chain in place while the parent still owns the tail, and shares its lookup table while the table has room, so a path of *n* fields holds *n* entries; a branch copies its prefix once. A type's fields therefore run from `shape` through `last`, and the chain may continue into a descendant's fields: walk them with `FOR_EACH_MAP_FIELD` / `typemap_next_field` (`lambda-data.hpp`), never by the link itself (`ShapeEntry::chain_next`; lint `no-raw-shape-chain-walk`), and a table hit on an entry past the type's length is a miss (`typemap_hash_entry_is_own`).

A shared type's field list (`typemap_is_shared_shape`) is never edited in place. An add with no usable edge clones first (`map_clone_typemap_for_mutation`), and runtime growth and type-changing writes build new types (`map_extend_open_shape`, `map_rebuild_for_type_change`; D3.4.5). The tree is bounded — 256 edges from the root, 16 from any other shape, 1,024 shapes per `Input` (`MAX_SHAPE_TRANSITIONS`, `MAX_SHAPE_GRAPH`) — and past a bound a map keeps a private type.
...
- **Shape chain + out-of-line hash table.** The chain is the authoritative ordered field list (cheap to extend, last-writer-wins); the hash table accelerates lookup once populated. It is allocated on first population rather than kept inline, because private types (one per parsed element) would otherwise each carry 256 bytes of empty slots. Interned names give pointer-equality fast paths.
...
- [LR_08 — Memory Management & Garbage Collection](LR_08_Memory_and_GC.md) — the heap, nurseries and name pool backing these values.
