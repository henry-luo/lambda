...
- **Numeric temporaries (int64, double, DateTime) use the raw number execution side-stack.** `push_l`/`push_d`/`push_k` bump its checked watermark and tag the stable payload address with `l2it`/`d2it`/`k2it`. Generated functions restore the watermark through one epilogue; escaping returns are converted to a scalar lane and rebuilt in the caller extent, while longer-lived containers and environments copy payloads into storage-owned tails. This region is separate from the collected GC data nursery; [LR_08](LR_08_Memory_and_GC.md) owns the lifetime rules.
...
- **Frame-scoped number boxing.** Float/int64/datetime temporaries use a raw bump side-stack, keeping allocation cheap while reclaiming each frame's temporaries at its single epilogue. Explicit scalar lanes and storage-owned tails handle values that escape that extent ([LR_08](LR_08_Memory_and_GC.md)).
...
| `lambda/lambda-mem.cpp` | Number side-stack boxing `push_l`/`push_d`/`push_k` and the `push_*_safe` double-box guards. |
