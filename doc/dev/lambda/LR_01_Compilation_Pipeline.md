...
`transpile_script` runs the compile phases: `lambda_parse_source` produces the Tree-sitter CST; parse errors are collected; a fresh `Input` base is allocated and copied into the `Transpiler`; `build_script` builds the typed AST; and `compile_script_as_mir_direct` does import registration + `transpile_mir_ast()` + `MIR_link()` and stores `jit_context`/`main_func` on the script. The internals of that call are owned by [LR_07](LR_07_MIR_Transpiler_JIT.md).
