...
> **Audience:** engine developers. **Convention:** `file:line` references drift; confirm against the cited symbol names. The C2MIR backend referenced historically below is retired; no supported build defines `LAMBDA_C2MIR`.
...
One structural fact frames the rest: **MIR Direct is the only supported code-generation path.** `transpile_script` routes execution to `compile_script_as_mir_direct`; the archived C-generation arm is wrapped in `#ifdef LAMBDA_C2MIR`, and both core and Jube configurations exclude its sources and macro. Older design notes describing a "two JIT path" world are historical.
