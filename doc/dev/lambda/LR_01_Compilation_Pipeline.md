...
This document owns the *orchestration* layer: the control flow that turns an invocation of the `lambda` binary into a transpiled, JIT-compiled, executed script and a printed result. It does **not** own the code generation itself — the AST → MIR lowering and the JIT mechanics belong to [LR_07 — The MIR Direct Transpiler & JIT](LR_07_MIR_Transpiler_JIT.md). The typed AST consumed here is produced by [LR_02 — Parsing & AST Construction](LR_02_Parsing_AST.md), the `Item`/`Container` values flowing through it are owned by [LR_03 — Value & Type Model](LR_03_Value_and_Type_Model.md), the GC heap the runner retains and roots is owned by [LR_08 — Memory Management & Garbage Collection](LR_08_Memory_and_GC.md), and the `run`-command procedural semantics are detailed in [LR_12 — The Procedural Runtime](LR_12_Procedural_Runtime.md).
...
`Heap` (`transpiler.hpp:10`) is the thin GC-heap wrapper — `pool`, `gc_heap*`, and a `result_root` GC slot that pins the current script result. `Script`/`Transpiler` are declared in `ast.hpp`; the `Transpiler` struct is **memcpy-compatible** with `Script` when loading a module. A `Script` carries `reference` (canonical path / synthetic id), `directory`, `source`, `ast_root`, `jit_context`, `main_func`, `const_list`, `type_list`, `index` (its slot in `runtime->scripts`), and the `is_loading`/`is_main` flags.
...
`transpile_script` selects the first-party C parser. `lambda_rd_reduce_ast()` allocates the `Input` owner on demand and reduces/binds directly into the typed AST through the span-native sink in `runtime/build_ast.cpp`; its `Transpiler` pass manager then runs `parse-build-bind` → `validate` → `index` before the same manager reaches const-fold, the direct MIR plan/lower/finalize-load operations, and `MIR_link()` (**D8.2.5**). Source reduction and binding remain coupled because declarations and scopes are required while later syntax is reduced; validation failure therefore retains diagnostics but cannot publish an indexed unit. The completed image is stored as `jit_context`/`main_func` on the script. The Tree-sitter grammar is not part of this path; the separate `lambda-cst` target is the syntax differential oracle. The internals of the MIR call are owned by [LR_07](LR_07_MIR_Transpiler_JIT.md).
...
- **Misc.** `serve` (`:3278`, a stub — see [LR01-2](../../../vibe/Lambda_Issue_Ledger.md)), `fetch` (`:3370`), `test-batch` (`:3501`), `js-test-batch` (`:3614`).
