...
`runner_setup_context` (`runner.cpp:1251`) builds the per-execution `EvalContext`: stack limit, pools, `type_list`, `consts`, decimal context, `schema_validator`, and the error/stack-trace fields. It then **reuses or creates** the GC heap, nursery, and name pool: if `runtime->heap` already exists it is adopted (`:1291`–`1297`), otherwise fresh resources are created and stored back on the `Runtime` for the next evaluation (`:1298`–`1313`). It also lazily creates `g_template_registry`.
...
- [LR_08 — Memory Management & Garbage Collection](LR_08_Memory_and_GC.md) — the retained GC heap, nursery, and root API used across evaluations.
