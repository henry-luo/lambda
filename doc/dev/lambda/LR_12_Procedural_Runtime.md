...
> **Last verified against tree:** 2026-09-10 *(safety-analyzer §5 corrected: the "conservative gate" was dead code)*
...
Procedural Lambda adds a **for-*statement*** distinct from the functional for-*expression*: `for i in a to b { arr[i] = v }` iterates for its side effects rather than building a result list. The AST node is built by `build_for_stam` (`build_ast.cpp`), which creates an `AstForNode` under `AST_NODE_FOR_EXPR` and marks the procedural form with `discard_result`; its iterator/query children use `AST_NODE_FOR_CLAUSE` (the former `AST_NODE_FOR_STAM` tag was retired by P1a/P1d). It inherits the proc context into its scope, builds the loop/let/where clauses via `build_for_clauses`, builds the `then` body, and types the whole thing `&TYPE_ANY`. The MIR lowering is `transpile_for` (`transpile-mir.cpp`) — its general loop structure (collection eval, `item_keys`, header/back-edge) is owned by [LR_07](LR_07_MIR_Transpiler_JIT.md); what matters here is the *body*.
...
`safety_analyzer.cpp`/`.hpp` answers one question about every user function: is it **tail-recursive**, and therefore eligible for the optimization that turns its recursion into a loop? Native stack-overflow protection is *not* a static gate here — it is the guard-page/signal handler owned by [LR_08](LR_08_Memory_and_GC.md) (`lambda-stack.cpp`), which needs no per-function emitted check.

**Correction (2026-09-10): the "conservative gate" this section used to describe was dead code.** Earlier revisions reported that `analyze_function_safety` did no analysis, that `function_needs_stack_check` was hard-`true` so every user function paid for a stack check, and that `function_is_tail_recursive` was hard-`false` so TCO was disabled. Those three functions existed, but **nothing ever called them** — at commit `c568f0f93` a tree-wide search for `function_needs_stack_check` matched only this document, `LR_11`, and the definition itself. Commit `8d44a6ca3` (2026-09-01) deleted the whole vestige (`SafetyAnalyzer`, its singleton factory, the `callback_sys_funcs_[]` table, and both gate functions) with no behavioural change, which is the proof they gated nothing. The record is archived as [LR12-R3](../../../vibe/Lambda_Issue_Ledger(fixed).md#lr12-r3).

**TCO is live on both tiers, and `should_use_tco` is the real gate.** What survives in the file is the analysis itself: `is_recursive_call` (matches a callee by resolved AST entry or by name), `has_tail_call` (walks `if`/`match`/`list`/`content`/`block`/`return` bodies for a recursive call in tail position), `should_use_tco` (requires a name, rejects closures, requires a tail call), `has_any_recursive_call`, `has_non_tail_recursive_call` (distinguishes tail from non-tail position), and `is_tco_function_safe` (true only when *all* recursion is tail recursion). MIR Direct consults `should_use_tco` in `transpile_func` (`transpile-mir.cpp:27906`) and, when it holds, emits the TCO label, the iteration counter, and the loop back-edge; the interpreter's planner consults the same predicate (`interp_plan.cpp:1693`) to mark tail calls for T0.

**The one function still unwired is `is_tco_function_safe`.** It computes exactly the safety conclusion the TCO loop would need to drop its iteration ceiling — "every recursive call here is in tail position, so after the goto transform this frame cannot grow" — and has no caller, so both tiers instead enforce a blanket counter (`LAMBDA_TCO_MAX_ITERATIONS` in the JIT, `LAMBDA_INTERP_TCO_MAX_ITERATIONS` in T0). Tracked as [LR07-13](../../../vibe/Lambda_Issue_Ledger.md#lr07-13).
...
- **Tail-position analysis decides TCO; the stack guard is dynamic.** Eligibility is a pure AST property (`should_use_tco`), consulted identically by the JIT and the interpreter's planner, so the two tiers cannot disagree about which functions become loops. Native stack exhaustion is caught dynamically by the guard-page handler rather than by a static per-function gate — which is why deleting the vestigial `function_needs_stack_check` changed nothing.
...
| `lambda/runtime/safety_analyzer.cpp` / `.hpp` | The tail-call analysis: `is_recursive_call`, `has_tail_call`, `should_use_tco` (the live TCO gate, read by both tiers), `has_non_tail_recursive_call`, and the still-uncalled `is_tco_function_safe`. |
