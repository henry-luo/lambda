...
> **Last verified against tree:** 2026-09-06 *(§5 trigger/adapt re-verified for the T20-5 nursery pacing; rest from the 2026-08-24 stamp)*
...
The header (`gc_heap.h:7`) names the design "Phase 5 Architecture: Dual-Zone Non-Moving Mark-and-Sweep". It is single-threaded and exact-rooted: native references must be published through registered slots/ranges or the execution root side-stack. It reclaims via mark-and-sweep — with a semispace-style copy compaction of one zone bolted on. Generation tag bits exist (`GC_GEN_NURSERY`/`GC_GEN_TENURED`, `gc_heap.h:37`–`38`), but the collector is effectively single-generation over the `all_objects` list; "tenuring" applies only to *data buffers* (copied nursery→tenured), never to objects.
...
It owns the **GC driver** `heap_gc_collect`, the callback `gc_data_alloc` invokes when the data zone crosses its threshold. The driver derives the live exact-root slice from `Context.side_root_base/top` and passes that slice to `gc_collect_with_root_region`; it never derives roots from native-stack or register contents.
...
The boxing and ownership entry points for this region, plus the counted GC scalar-cell fallback for ownerless persistent numeric Items, are described from the value-model side in [LR_03](LR_03_Value_and_Type_Model.md) §3 and normatively in `vibe/Lambda_Design_Stack_API.md` §15.
...
A cycle is **triggered** from two automatic sites, each paced by the same time budget (`gc_paced_floor`, Tune20 T20-5's 10% rule): (1) `gc_data_alloc` when nursery data-zone usage reaches `gc_threshold` (initially `GC_DATA_ZONE_THRESHOLD`, 75% of a 4 MB block); after each such cycle the threshold grows ×2 while the **mark phase's** share of wall time since the previous nursery collection exceeds 10% (×4 above 50%), capped at `GC_DATA_ZONE_THRESHOLD_CAP` (256 MB) — marking is the fixed per-collection cost a larger nursery amortizes, whereas compaction and the reset memset scale with the nursery itself, so a total-cost share would never converge for a churn-heavy script with a tiny live set; (2) the object-pressure trigger in `gc_heap_alloc`/`gc_heap_calloc_class`/`gc_heap_bump_alloc` when `total_allocated` (live + dead-unswept object bytes) reaches `object_threshold` (floor `GC_OBJECT_HEAP_THRESHOLD`, rebased to `max(2 × total_allocated, paced floor)` on the previous cycle's total cost). Both need `gc->collecting` clear and no deferred-collection scope. The callback path runs through `heap_gc_collect` (§3) into `gc_collect`, which sets `gc->collecting` to guard re-entrancy. `LAMBDA_GC_STATS=1` prints the final counters (collections, both thresholds, mark time) at exit on release builds.

**Mark.** Roots are gathered (§6), then `gc_mark_item` pushes each onto the `mark_stack` and `gc_trace_object` (`gc_heap.c`) traces the graph. Tracing is **precise per `TypeId`**: it walks Array/Element `items[]`, follows Map/Element shape chains up to each type's `last` (a transition-tree chain continues into a descendant's fields, D3.4.3v3; `LAMBDA_GC_OFF_TYPE_MAP_LAST`), reads Lambda `Function` closure storage, traces JS functions through the registered `gc_js_function_trace_fn`, traces `GC_TYPE_JS_ENV` Item halves while ignoring their raw scalar tails, and traces a `VMap` through its registered callback. Several per-type walks read struct fields at **hard-coded byte offsets**, which is fast but layout-fragile (§9). One lane is deliberately conservative (D4.3.4): a shaped field declared `null` has its eight-byte word marked through `gc_mark_possible_item`, because `fn_map_set`'s same-width fast path upgrades such a field to a container pointer in place while the shared literal shape stays `null` for its other instances, and a `null` lane after a nine-byte `any` lane sits at an offset the `gc_trace_data_words` sweep never visits (splay's node literal put `right` at offset 17; its right subtrees were freed while linked on every tier).

**Compact.** `gc_compact_data` copies live data buffers nursery→tenured and rewrites owner pointers (§4), then resets the nursery. The reset only rewinds the block cursors — it does **not** zero the reused memory. `gc_data_alloc` (and so `heap_data_alloc`/`heap_data_calloc`) zeroes at allocation time, right before the caller touches the buffer; `gc_data_alloc_uninit` / `heap_data_alloc_uninit` / `array_num_new_uninit` skip that pass for a caller that writes every byte before any read or GC safepoint (`fill`'s numeric/bool lanes, the varargs `array_*_fill` helpers). Zeroing at reset cost a memset of the whole nursery per collection on top of the callers' own initialization (brainfuck's 2.4 GB of `fill(30000, 0)` tapes were zeroed at reset, zeroed again by calloc, then written by fill).
...
**Adapt.** After sweeping, the **productivity rule** in `gc_collect` tunes the nursery threshold: if the cycle freed <40% of the nursery it grows the threshold 4×, <75% grows it 2×, capped at 256 MB — so unproductive collections are spread further apart rather than thrashing the cache. The time-budget pacing above is the complementary rule for the *productive* case: a script whose live graph keeps growing (havlak) frees ≥75% of a 3 MB nursery every time yet pays a full mark of the whole object heap per cycle; pacing on the mark share cut it from 27 to 5 collections.
...
## 7. Three-tier string allocation & name pool
...
| **Arena** | input parsers via `MarkBuilder` / `arena_alloc` | `Arena` | until arena reset/destroy (document scope) | Parser-built doc data. Not GC-tracked, no header. |
...
|---|---|
| `lib/gc/gc_heap.c` / `.h` | The collector engine: `gc_collect`, mark (`gc_mark_item`/`gc_trace_object`), compact (`gc_compact_data`), sweep (`gc_sweep`), allocators (`gc_heap_alloc`/`gc_heap_bump_alloc`/`gc_data_alloc`), exact root slot/range registries, adaptive threshold, finalization. |
...
| `lambda/mem_factory_rt.cpp` / `.h` | Factory that registers gc_heap / NamePool in the `MemContext` registry and installs release hooks. |
| `lambda/lambda-stack.cpp` / `.h` | Signal/SEH-based C-stack-overflow guard; it does not provide GC roots. |
