...
It owns side-stack binding/checking plus the root-registration wrappers around `gc_register_root`/`gc_register_root_range` (§5–§6). It owns number-home allocation for full-width `INT64`/`UINT64` and out-of-band `FLOAT`, caller-home adoption, destination/persistent rehoming support, and the interned single-char ASCII table. `push_k` is separate GC-owned datetime construction, not number-stack allocation. Finally it owns teardown finalization, which walks surviving objects and runs external finalizers at context end.
