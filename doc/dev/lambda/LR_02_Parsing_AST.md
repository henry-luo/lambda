...
The Lambda front end is a two-stage pipeline. Stage one is **parsing**: the first-party C lexer and recursive-descent/Pratt parser drive a syntax sink in `build_ast.cpp` that allocates each AST node (derived from a common `AstNode` base) as its production completes, with names unbound and typed `any`. Stage two is **resolution**: a pass over that tree, in the order its productions completed, binds names and annotates each node with the `Type*` inferred during construction. The Tree-sitter grammar is used separately by `lambda-cst` for differential verification; the normal compiler never walks its CST.
...
Normal Lambda files and modules use the first-party parser by default. The lexer is a streaming C17 scanner with bounded lookahead; recursive descent owns documents, declarations, statements, containers, and the type/path seams, while one Pratt table owns prefix, postfix, and infix expression binding. Each committed production immediately calls the syntax sink in `build_ast.cpp` and returns an `AstNode*`-backed value; the nodes it allocates are the retained AST, resolved in place afterwards. No Lambda CST or replacement syntax tree is retained. `lambda_rd_parse_syntax` and `lambda_rd_resolve_syntax` are the production adapters (`lambda_rd_reduce_ast` runs both); each fail-closes on an unsupported reduction or syntax error.
...
The C parser commits each reduction with a `SourceSpan` and already-built child values. `lambda_rd_parse_syntax` installs the syntax sink in `build_ast.cpp`, which builds declarations, expressions, containers, type patterns, paths, and procedure constructs as syntax nodes tagged with the production that built them (`AstNode::syntax_form`), and the `AstScript` root. `lambda_rd_resolve_syntax` then walks that tree, re-entering each construction scope where the parser reported it, and calls the shared span-based constructors for literals, names, scopes, type contracts, diagnostics, and semantic validation; a node whose kind depends on resolution is morphed in place.
...
- `AST_NODE_ASSIGN` → `AstAssignNode` (`ast-core.hpp`): `op`, `left/right`, and the optional Lambda target metadata. `AST_NODE_KEY_EXPR` / `AST_NODE_PARAM` remain `AstNamedNode` (`ast.hpp:388`): `String* name`, `AstNode* as`, `NameEntry* entry`. Legacy error-destructure state is not part of the AST; handlers use a separate current-error node/context.
- `AST_NODE_LOOP` → `AstLoopControlNode` (`ast-core.hpp`): `LoopForm form`, `init`, `test/cond`, `update`, `body`, and `NameScope* vars`; `WHILE`, `DO_WHILE`, and JavaScript C-style `FOR_C` are one condition-loop contract (**D8.2.2**, P1d).
- `AST_NODE_FOR_CLAUSE` → `AstLoopNode` (`ast.hpp`): iterator/query `for` clauses retain `name`, `index_name`, `as`, `LoopKeyFilter key_filter` (ALL/INT/SYMBOL), joins, and optionality. Its layout deliberately differs from `AstNamedNode`; the extra `index_name` sits before `as`, so the capture code casts to `AstLoopNode*` explicitly (**D8.2.4**, P1a/P1d).
...
- `AST_NODE_LET_STAM` / `PUB_STAM` / `TYPE_STAM` / `VAR_STAM` → the shared `AstVarDeclNode` (`ast-core.hpp`), with `declare`/`declarations` as one list slot; `AstLetNode` is an alias.
- `AST_NODE_FOR_EXPR` → `AstForNode` (`ast.hpp:447`): `loop`, `let_clause`, `where`, `AstGroupClause* group`, `order`, `limit`, `offset`, `then`, `NameScope* vars` — the SQL-like comprehension clauses. `AST_NODE_LOOP` is reserved for condition loops; `AST_NODE_FOR_CLAUSE` is the iterator edge (P1d, **D8.2.2**).
- `AST_NODE_IF_EXPR` → `AstIfNode` (`ast.hpp:459`, `cond`/`then`/`otherwise`); `AST_NODE_MATCH_EXPR` → `AstMatchNode` (`ast.hpp:472`, `scrutinee` + `AstMatchArm* first_arm`); `AST_NODE_RETURN_STAM` → `AstReturnNode` (`ast.hpp:486`); `AST_NODE_RAISE_STAM` / `RAISE_EXPR` → `AstRaiseNode` (`ast.hpp:492`).
- `AST_NODE_ASSIGN_STAM`, `AST_NODE_INDEX_ASSIGN_STAM`, and `AST_NODE_MEMBER_ASSIGN_STAM` → the shared `AstAssignNode` storage (`ast-core.hpp`), with `left/right` child ownership plus optional Lambda binding/compound-target metadata; the old `AstAssignStamNode` and `AstCompoundAssignNode` names are aliases only (P1c, **D8.2.2**/**D8.2.4**).
...
`build_identifier` (`build_ast.cpp:1812`) takes its type from the resolved `NameEntry->node->type`. Imports and params get special handling (preserving `TypeType` wrappers and `TypeParam::full_type` for direct struct access, `build_ast.cpp:1898`–`1910`; stripping `is_const`). Unresolved names are rewritten where possible. Math-module constants `pi`/`e` resolve first, because an import is a binding. Next, a bare name in a body whose `~` may stay implicit becomes `~.name` of the innermost current item (S10.1.7v2). The resolver's `in_that_clause` is on for such bodies: a `that` proviso, an object-level or type constraint, a match arm body, and a handler's value arm. It is off for a `|>`/`|:` body, a match pattern, and a write target, and each body restores the outer setting (`resolver_that_begin`/`resolver_that_end`). The rewrite never applies to a callee, or to a module or namespace prefix spelled as a member's object (`member_object_names_module`). Otherwise the type degrades to `TYPE_ANY`. The pipe-mode test `has_current_item_ref` treats a `~` that a nested body binds as bound, not free: it stops at match arms, a handler's value arm, and a nested pipe-family body.

`build_call_expr` (`build_ast.cpp:1358`) sets the return type from `SysFuncInfo::return_type` (system functions) or `TypeFunc::returned` (user functions); a `can_raise` callee yields `TYPE_ANY` because the error/value union is boxed. The resolution cascade is **builtin-module call → aliased-import call → sys-func → regular func-typed callee**, and crucially **user members/functions shadow sys-funcs** (user definitions are checked first, so user code can override built-ins). Pipe injection bumps the looked-up arg count via `tp->pipe_inject_args` and sets the `pipe_inject` flag. Argument/parameter compatibility is checked by `types_compatible_with_full` (`build_ast.cpp`): `ANY` accepts and passes everything, numeric coercions follow int→int64→float (with sized-int interplay), and a union type (`T | error`) matches either arm.
...
Moved to the central ledger: **[Lambda Core Runtime — Central Issue Ledger](../../../vibe/Lambda_Issue_Ledger.md)**, entries **LR02-1 – LR02-13** (open/partial; LR02-8 through LR02-12 were found during verification/implementation, not extracted from this section) and **LR02-R1 – LR02-R10** (resolved, Appendix A).
...
|---|---|
| `lambda/runtime/parse.c` | Production parser translation unit embedding the C lexer and recursive-descent/Pratt parser. |
...
| `lambda/runtime/ast.hpp` | `AstNode` base + concrete node structs, `AstNodeType` enum, `NameScope`/`NameEntry`/`CaptureInfo`/`NamespaceEntry`, `AstScript`. |
| `lambda/runtime/build_ast.cpp` | Direct span-based AST sink, per-construct semantic construction, type inference, scope/name resolution, import resolution, and capture analysis. |
