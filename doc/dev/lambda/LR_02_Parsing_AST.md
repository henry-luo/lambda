...
The **operator precedence table** (`grammar.js:139`–`173`, tight→loose) is `call > index > member > parent > primary > unary > ** > * / div % > + ++ - > relational > == != > to > & > set-exclude(!) > | union > is/in > and > or > pipe(| that) > pipe_file(|> |>>) > if > match > for > let > assign > assign_stam`. All binary operators are `prec.left` except `**` (right). A separate **type sublanguage** (`grammar.js:888`–`1109`) mirrors the value grammar for type expressions — `array_type [T]`, `map_type {k:T}`, `fn_type`, `occurrence_type T?/+/*`, `binary_type` (`| & !`), `constrained_type T that (…)`, `object_type`, and the string/symbol pattern grammar that is reused for the regex-like pattern definitions.
...
- **Inline literals.** Scalar literals have no dedicated node type — they are built inline in the switch: `SYM_INT` (`build_ast.cpp:7525`), `SYM_FLOAT`, `SYM_SIZED_INT`/`SYM_SIZED_FLOAT`, `SYM_NAMED_VALUE` (true/false → `LIT_BOOL`, else a float literal for `inf`/`nan`), `SYM_STRING`/`SYM_SYMBOL`/`SYM_BINARY` → `build_lit_node`, and `SYM_DATETIME`. `SYM_INT` **promotes to float** when the literal falls outside the 56-bit packed-int range `[INT56_MIN, INT56_MAX]` (`build_ast.cpp:7542`), because the tagged `Item` int payload is only 56 bits ([LR_03 §2](LR_03_Value_and_Type_Model.md)).
- **Empty-object pre-check.** `SYM_MAP` is intercepted: a single-identifier map `{TypeName}` that resolves to a known object type is built as an `AstObjectLiteralNode` (an empty object literal seeded with field defaults) rather than a map (`build_ast.cpp:7454`–`7488`). The *same* logic is duplicated in `build_primary_expr` (`build_ast.cpp:2699`–`2734`) — see §8.
...
Literals get either a canonical singleton (`LIT_INT`, `LIT_BOOL`, `LIT_NULL`) or a freshly-allocated `TypeFloat` / `TypeString` / `TypeDecimal` / `TypeDateTime` carrying a `const_index` into `tp->const_list` (built by `build_lit_float`, `build_lit_string`, `build_lit_decimal`, `build_lit_sized_integer`, `build_lit_datetime`, etc.). The int→float promotion at the 56-bit boundary happens both at the literal level (`build_primary_expr:2658`) and in `build_expr` (`build_ast.cpp:7542`).
...
- **`OPERATOR_DIV`** → always `LMD_TYPE_FLOAT` when both operands sit in the `INT..FLOAT` band, else `ANY` (`build_ast.cpp:3147`–`3160`). True division never narrows to int — this is also the FLOAT evidence the MIR-Direct parameter inference keys on ([LR_07 §5](LR_07_MIR_Transpiler_JIT.md)).
...
- **`AND` / `OR`** → `ANY` (the truthy idiom, not a plain boolean); **`TO`** → `RANGE`; **`PIPE`** / **`WHERE`** → the node becomes `AST_NODE_PIPE`, typed from the RHS (or `ARRAY` depending on `~` presence); **`PIPE_FILE`** (`|>` / `|>>`) → `AST_NODE_PIPE_FILE_STAM` (procedural only).

The promotion relies on the `TypeId` enum being ordered `LMD_TYPE_INT < INT64 < FLOAT < … < NUMBER` so that `std::max` picks the wider type — an *undocumented* dependency (see §8). **Decimal-result inference is present but commented out** for `DIV`/`ADD`/`SUB`/`MUL` (`build_ast.cpp:3150`–`3184`), so decimal promotion is currently disabled. The builder also performs static rewrites here: `expr is nan` is rewritten (`build_ast.cpp:3090`), a chained comparison `a < b < c` is desugared to `(a < b) and (b < c)` (`build_ast.cpp:3109`), and a literal `==`/`!=` between incompatible concrete types raises a compile-time warning (`build_ast.cpp:3221`–`3240`).
...
- **56-bit packed-int drives literal promotion.** Because `Item` packs a 56-bit int ([LR_03](LR_03_Value_and_Type_Model.md)), out-of-range integer literals are promoted to float *at the AST stage*, so codegen never has to handle an un-representable inline int.
...
2. **Numeric promotion relies on undocumented enum order.** `std::max(left_type, right_type)` (`build_ast.cpp:3171`/`3185`/`3255`, `build_if_expr:3481`) assumes the `TypeId` enum is ordered `INT < INT64 < FLOAT < … < NUMBER`. Reordering the enum in `lambda.h` would silently corrupt promotion — there is no static assertion guarding the dependency. (The enum is owned by [LR_03](LR_03_Value_and_Type_Model.md).)
3. **Decimal-result inference is disabled.** The decimal-promotion branches for `DIV`/`ADD`/`SUB`/`MUL` are present but commented out (`build_ast.cpp:3150`–`3184`), so an expression with a decimal operand is typed by the surrounding numeric rule rather than as a decimal.
...
9. **Duplicated empty-object-literal detection.** The `{TypeName}` → `AstObjectLiteralNode` pre-check is implemented twice — in `build_expr` (`build_ast.cpp:7454`–`7488`) and in `build_primary_expr` (`build_ast.cpp:2699`–`2734`) — so the two must be kept in sync by hand.
