...
- **GC heap** (`lib/gc/`) — dual-zone, **non-moving mark-and-sweep**. The *object zone* (size-class free lists) holds object structs, which **never move** — tagged pointers held by JIT code, envs, or either language stay valid across collections. The *data zone* (bump) holds variable-size buffers (`Map.data`, array items); it **is relocated** (copied to a tenured zone) at collection with owner-slot fixup — which is why naked data-zone pointers in JIT locals need precise, writable roots (the JIT root frames, [LR_07](lambda/LR_07_MIR_Transpiler_JIT.md)). Collection triggers on a data-zone threshold; roots = registered slots/ranges + conservative C-stack scan.
- **Numeric nursery** — a bump allocator backing boxed `FLOAT`/`INT64`/`DTIME` payloads (`push_d`/`push_l`/`push_k` in `lambda-mem.cpp`).
- **Mempool** — module-lifetime objects that are GC-*rooted* but not swept: `JsFunction`, closure envs, module-var arrays, `TypeMap`s.
...
- **JIT GC root frames:** generated code registers rootable locals in a thread-local frame stack (`heap_jit_gc_root_frame_*`) so precise roots exist across allocations; rooting is decided by *honest* local typing (packed scalars are not rooted) ([LR_07](lambda/LR_07_MIR_Transpiler_JIT.md), `vibe/Lambda_Issue_GC_Root (fixed).md`).
...
- **Values:** JS types are a projection onto `TypeId` ([JS_03](js/JS_03_Value_Model.md)). `js_make_number`: exact int56 → packed INT, else nursery-boxed double (preserving `-0.0`, avoiding the Symbol range).
...
- **Functions & calls:** `JsFunction` is pool-allocated (module lifetime); call arguments go through a single GC-rooted bump **args stack** (`js_args_push/save/restore`); top-level bindings live in an indexed **module-vars array** (per-module, GC-rooted); closures capture via env arrays (cheap indexed access) ([JS_05](js/JS_05_Functions_Closures.md), [JS_03 §7–8](js/JS_03_Value_Model.md)). Static dispatch (direct `MIR_CALL`) fires for `function` declarations, const-bound functions, and provably-monomorphic class methods; everything else goes through `js_call_function`.
...
4. **One document engine.** Radiant's `DomElement` trees are the shared substrate for Lambda's `layout`/`render`/`view` commands and JS's DOM — JS event handlers mutate the same nodes Radiant lays out, with lazy layout on metric queries ([JS_13](js/JS_13_Web_DOM.md), [Radiant design](Radiant_Layout_Design.md)).
...
Both engines are fast where inference keeps values **native** (pure numeric locals match or beat Node/V8 on several benchmarks) and slow where values fall back to the **boxed generic path**: boxed doubles are nursery allocations (the dominant cost on float-heavy code), property/method dispatch on non-shaped objects goes through C-runtime calls, and allocation-heavy workloads pay whole-heap mark-and-sweep. The optimization catalog and measured history are in [JS_15](js/JS_15_Performance.md); the current improvement roadmap (inline ICs, dense-double arrays, call-path slimming, stack-boxed scalars, root-frame elimination, float self-tagging) is in [`vibe/Lambda_Tuning_Proposal.md`](../../vibe/Lambda_Tuning_Proposal.md); benchmark snapshots live in `test/benchmark/Overall_Result*.md`.
