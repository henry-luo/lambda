...
**Document proxy & foreign docs.** Bare `document` resolves to a singleton branded VMap whose `host_data` is the active browsing-context `DomDocument*`; methods route through `js_document_proxy_method` -> `js_document_method` and properties through `js_document_proxy_get_property`. `document.implementation.createHTMLDocument`/`createDocument` build branded foreign-document VMaps; their get/set/method paths use the Radiant foreign-document bridge, which swaps the active document around document-proxy behavior when needed.

---

## 3. Element & document API dispatch

The VMap host-object gate in `js_property_get`/`js_property_set` is the primary entry from the ordinary pipeline. DOM nodes, Range/Selection, inline/computed style, CSSOM, and document/foreign-document proxies are recognized by native host brands before ordinary map property access; method calls arrive via `js_map_method`, which dispatches host receivers through their DOM/CSSOM/document bridges before any ordinary-method fallback.
