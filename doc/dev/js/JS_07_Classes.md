...
>
> **Primary sources:** `lambda/js/js_mir_context.hpp` (`JsClassEntry`, `JsClassMethodEntry`, `JsStaticFieldEntry`, `JsInstanceFieldEntry`, the `ctor_prop_*` arrays on `JsFuncCollected`, the collection caps), `lambda/js/js_mir_function_collection_class_inference.cpp` (class collection, `jm_scan_ctor_props`, `jm_find_class`), `lambda/js/js_mir_statement_lowering.cpp` (class-function emission, prototype/method install, constructor prologue/body/epilogue, `new C()` lowering), `lambda/js/js_mir_expression_lowering.cpp` (`jm_class_private_name`, `super.x` lowering), `lambda/js/js_runtime.cpp` (`js_construct_value`, `js_super_call_native`, brand helpers, active `new.target`), `lambda/js/js_class.h`, and `lambda/js/js_object_meta.h` (class metadata).
...
ES2022 classes are lowered to constructor functions, prototype objects, and
property descriptors. A class binding is a `JsFunction` with explicit call and
construct entries; the constructor's class/brand policy is carried by the
immutable metadata of its runtime shapes, while instance prototypes remain
ordinary objects. The public `prototype` property and internal prototype
resolver are separate, and no name/sentinel property grants callability.
...
The compiler distinguishes the **constructor function**, the public
`prototype` object, and the shape blueprint derived from the constructor body.
The constructor is a `JsFunction`; the prototype/shape objects carry their
own immutable metadata under **D3.4.7** rather than hidden class-map fields.
...
The prototype carries a non-enumerable `constructor` back-pointer
(`js_set_default_constructor_property`) and immutable class metadata. Built-in
prototype methods (e.g. `Array.prototype.map`) are installed as realm-local
properties when the intrinsic prototype is populated; they participate in the
same ordinary property walk described in [JS_06 §8](JS_06_Objects_Properties_Prototypes.md) and [JS_10 — Standard Built-in Library](JS_10_Builtins.md).

---

## 5. Inheritance, `super()` & `super.method()`

When a class has a resolved `superclass` `JsClassEntry`, the prototype's `[[Prototype]]` is linked to the parent's `prototype` (`js_set_prototype(last_proto, sp_obj)`, `js_mir_statement_lowering.cpp:4981`), while superclass and constructor-body state stays in the rooted `JsFunction` class carrier. Both tiers link heritage through the shared runtime decision `js_class_heritage_prototype_parent` (IsConstructor, then a valid `.prototype`; `null` yields a `null` prototype parent) and `js_set_class_constructor_parent` (a `null` heritage keeps the constructor's parent at `%Function.prototype%`); MIR reaches them through `jm_link_class_super_prototype`. An `extends <expr>` whose target is not a collected class (a builtin, a member expression, a binding holding `null`) takes this runtime-resolved path. `extends null` is recorded as the class's superclass, so the class is derived: an explicit `super()` and the implicit `super(...args)` both fail IsConstructor against the live `[[Prototype]]` (`%Function.prototype%` unless rebound).
...
3. let the parent's stored construct entry allocate its required internal slots and choose `activeNewTarget.prototype`; source-class constructors use the same function capability boundary;
...
**Brand checks.** Private access on an object that never went through the
class body throws `TypeError`, not `undefined`. Private keys are NamePool-owned
private identities; class metadata and explicit private-brand ownership are
kept separate from public property spelling under **D3.4.7**. Derived instances
are branded after `super()` by `js_init_class_instance_fields`.
...
`class MyArr extends Array {}` cannot start life as a plain `Map` — the instance needs the builtin's internal behaviour (exotic length, primitive slot, typed-array backing) *and* the subclass prototype. `js_construct_value` is the sole dynamic boundary: intrinsic constructors carry direct typed construct bodies, and the concrete body allocates its internal-slot-bearing instance using the explicit `newTarget` prototype. Bound and Proxy construct entries forward the same operand. Source classes are function values, so construction enters the ordinary capability path and delegates inherited builtin construction back into `js_construct_value` (**D6.2.2v2**).
...
4. **Private brand ownership.** Private names use NamePool private identity;
   public spelling prefixes cannot forge a class brand. Derived-class deferred
   initialization remains governed by `js_init_class_instance_fields`.
...
6. **Metadata boundary.** Class identity is resolved through immutable
   `TypeMap::js_meta` and `js_object_meta(Item)` under **D3.4.7**. Constructor
   and prototype relationships are ordinary properties/internal prototype
   state; spelling-based class probes and callable class Maps are retired.
...
| `lambda/js/js_runtime.cpp` | `js_constructor_create_object[_shaped[_cached]]`, common `js_construct_value`, `js_super_call_native`, brand helpers, constructor return rules, active `new.target`. |
