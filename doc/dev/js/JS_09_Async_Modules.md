...
At each `await`, lowering calls `js_async_prepare_await(value)`, routes its explicit rejection completion, reads the prepared PromiseResolve carrier through `js_async_get_resolved`, then emits `[awaited, next_state]` and returns. This is deliberately unconditional: the resumed input carries fulfillment or rejection at the same state label, preserving promise-job ordering for a settled value as well as a pending one under **D8.4.3v2**.

The caller side wires `js_async_context_create_mir` (allocates a `JsAsyncContext` plus a result promise), `js_async_start`, and `js_async_get_promise`. `js_async_drive` is the core: it calls the state-machine function, then on `next_state == -1` resolves the result promise through the resolution procedure (so a returned promise is adopted), on `-2` rejects it, and otherwise registers `js_async_resume_handler` / `js_async_reject_handler` on the pending promise via `js_promise_then` — these re-enter `js_async_drive` at the saved state when the awaited promise settles. A rejection follows the explicit completion ABI so the state machine's try/catch can observe it, per **D8.4.3v2**.
...
| `lambda/js/js_mir_function_class_lowering.cpp` | Phase-6 async state-machine lowering; `js_async_context_create_mir`/`start`/`get_promise` call emission. |
