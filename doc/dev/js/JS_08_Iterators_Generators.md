...
The loop obtains its iterator with `jm_emit_get_iterator` (`:3989`) and pushes it onto the `mt->for_of_iterators[32]` stack (`js_mir_context.hpp:375`) so nested loops can be closed by `break`/`continue` to an outer label. It then opens a **synthetic try context** (`mt->try_ctx_stack`, depth-bounded at 16) whose catch target is `l_iter_error` and whose end target is `l_forit_ret` (`:4087`). This try context is what makes IteratorClose happen on every abrupt exit, implementing ES §13.7.5.13:
...
- **throw from the body** → `l_iter_error` saves the routed ERROR Item, closes the iterator, preserves the original payload across any close-time failure, and re-throws it (`:4281`);
...
**Generator pool.** `JsGenerator` records are stored in a fixed array `js_generators[JS_MAX_GENERATORS]` with `JS_MAX_GENERATORS == 4096`; the generator object is a metadata-qualified Map-prefix carrier whose trailing `generator_index` selects the record. Slots are not recycled until the array is full, at which point the oldest `done` slot is reused.
...
Inside an async-generator body, `await` is lowered as a *second kind of yield point* in the same state machine — the `JS_AST_NODE_AWAIT_EXPRESSION` case allocates a state from the same `gen_yield_index` counter, with the await-count folded into the state budget. `js_async_prepare_await` publishes the PromiseResolve carrier that `js_async_get_resolved` passes to the generated suspension result; microtask resumption is owned by [JS_09 — Async, Promises & Modules](JS_09_Async_Modules.md). This document only notes that an async generator interleaves yield-states and await-states in one machine, under **D8.4.3v2**.
