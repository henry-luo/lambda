...
| `dns` | `lambda/module/node_net/node_dns_module.cpp` / `js_get_dns_namespace` | libuv `uv_getaddrinfo` | `lookup` (async), `lookupSync`, `resolve`/`resolve4` (delegate to A/AAAA `getaddrinfo`). No real DNS record types beyond A/AAAA. |
