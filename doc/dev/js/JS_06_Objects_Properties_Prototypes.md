...
- **`TypeMap`** (`lambda-data.hpp:242`) — the shape: `length` (field count), `byte_size`, the `ShapeEntry* shape` linked list (+ `last` tail), an **inline FNV-1a hash table `field_index[TYPEMAP_HASH_CAPACITY]`** with `TYPEMAP_HASH_CAPACITY == 32` (an accelerator; the shape chain remains authoritative when unpopulated or saturated), a lazily-built slot-indexed `slot_entries[]` (O(1) shaped access), `bool is_private_clone`, and a **`uint8_t js_class`** identity byte.
...
The hash table is last-writer-wins, capacity-32, and linear-probe (`lambda-data.hpp:280`). A full table still updates an existing key, but refuses new keys without evicting; shared `typemap_hash_lookup` falls back to a last-matching linear `ShapeEntry` walk when the table is unpopulated or saturated.
...
3. **Linear scans on hot paths.** Built-in method lookup is linear `strncmp` over each spec table (`js_runtime_builtin_registry.cpp:20`); the `TypeMap` hash is still fixed-32 (`lambda-data.hpp:280`), so saturated objects use the authoritative linear shape-chain fallback. This is now a performance debt, not a lookup-correctness gap. *Improvement:* sort + bsearch (or hash) the spec tables; grow `field_index`.
