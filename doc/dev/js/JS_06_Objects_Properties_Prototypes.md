...
The hash table is last-writer-wins, capacity-32, and linear-probe (`lambda-data.hpp:280`). A full table still updates an existing key, but refuses new keys without evicting; objects with >32 named fields fall back to a last-matching linear `ShapeEntry` walk in `js_map_get_fast`.
