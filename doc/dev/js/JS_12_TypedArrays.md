...
- **`JsArrayBuffer`** (`js_typed_array.h:35`) — `void* data` (heap byte buffer), `int byte_length`, `int max_byte_length`, and three flags: `detached`, `is_shared` (SharedArrayBuffer), `resizable`. Allocated by `js_arraybuffer_alloc` (`js_typed_array.cpp:1510`), which zero-fills via `mem_calloc`.
...
**The ArrayNum view.** Every TypedArray constructor calls `array_num_new_external_view` (`lambda-data-runtime.cpp:185`; e.g. from `js_typed_array_new` `js_typed_array.cpp:2109`, `js_typed_array_new_from_buffer` `:2174`): it allocates an `ArrayNum` (`lambda.h:640`) marked `is_view`/`is_mutable_view` whose `extra` slot holds an `ArrayNumShape` (`lambda.h:724`) — `base` (the ArrayBuffer's wrapper `Map`, kept for GC reachability), an element-unit `offset`, and a 1-D dims/strides table. The JS-visible geometry is **derived** from the view: `js_typed_array_stored_length` (`js_typed_array.cpp:394`) reads `view->length`, and `js_typed_array_stored_byte_offset` (`:386`) reads `shape->offset * elem_size`; `byteLength` is their product (`:399`). Because a resizable buffer's `resize` reallocates `ab->data` (§5), the view's cached `data` pointer can go stale — so **every public entry point calls `js_typed_array_refresh_arraynum_view` (`:442`) before touching storage** (`js_typed_array_get` `:2427`, `set` `:2442`, `length` `:2560`, `byte_length` `:2581`, `byte_offset` `:2588`, `current_data_ptr` `:2574`, and each bulk path). Refresh recomputes `view->data` from the live `ab->data + byte_offset` (`js_typed_array_current_data` `:435`), re-floors the length for length-tracking views (`js_typed_array_current_length` `:419`), and re-syncs `shape->offset`/`shape->base`.

**GC.** The typed-array `Map` is traced by `js_native_map_gc_trace` (`lambda-mem.cpp:76`): it recovers the `JsTypedArray*` from either the native-backed `Map.data` or the upgraded `__ta__` key (`gc_typed_array_from_map` `:65`), then marks `buffer_item` and the `view` object. Finalization frees the `JsTypedArray` record and, for buffers, the byte block (`gc_finalize_js_native_map` `:107`).
...
**Resizable buffers.** `ArrayBuffer.prototype.resize` (`js_arraybuffer_resize` `:1643`) is non-shared only, requires `resizable`, performs ToIndex on the new length **after** an entry check but before the detach re-check (so a coercing `valueOf` side effect is observed, per the spec note at `:1648`), rejects lengths above `max_byte_length`, then **allocates a fresh buffer** with `mem_calloc`, copies `min(old, new)` bytes, and reassigns `ab->data`. Because the data block moves, a TypedArray's `view->data` pointer becomes stale — this is exactly what the refresh discipline of §2 exists for: every access re-derives the pointer through `js_typed_array_current_data` (`:435`, which returns `ab->data + byte_offset` rather than any cached pointer) and re-syncs the ArrayNum view via `js_typed_array_refresh_arraynum_view` (`:442`). The exported `js_typed_array_current_data_ptr` (`:2570`) gives the MIR JIT the same live pointer.
...
**transfer.** `ArrayBuffer.prototype.transfer(newLength?)` and `transferToFixedLength(newLength?)` share `js_arraybuffer_transfer_impl` (`:1672`; wrappers `:1726`/`:1730`): both allocate a new buffer of `newLength` bytes (default `byteLength`), copy `min(srcByteLength, newLength)`, and **detach the source** via `js_arraybuffer_detach` (`:1863`). `transfer` preserves the source's `resizable`/`maxByteLength`; `transferToFixedLength` produces a non-resizable result. `js_arraybuffer_detach` zeroes `byte_length` and sets `detached = true`; existing TypedArrays observe the detach through their stored `ta->buffer` pointer.
...
DataView is a deliberate **carve-out from the ArrayNum unification**: it is a byte-oriented, endianness-aware window, so it keeps plain `byte_offset`/`byte_length` fields and raw pointer arithmetic throughout.
...
**The get/set methods** (`getInt8`…`getFloat64`, `getBigInt64`/`getBigUint64`, and the matching setters) dispatch by name in `js_dataview_method` (`js_typed_array.cpp:3109`). Each computes the system endianness once (`is_little_endian_system` `:3103`), reads the optional `littleEndian` argument (defaulting to big-endian per spec), loads/stores the raw bytes via `dv_ptr` (`:3085`, which bounds-checks against the **current** view length), and byte-swaps with `swap16`/`swap32`/`swap64` (`:3092`) only when the requested endianness differs from the host. The BigInt views read/write 64-bit lanes and round-trip through the same bigint helpers as the BigInt TypedArray lanes. All methods first run `dv_validate_or_throw` (`:3074`) so a detached or out-of-bounds view throws before touching memory.
...
`Buffer` is a thin Node-compatibility layer over `Uint8Array`, not a distinct exotic kind. `create_buffer` (`js_buffer.cpp:147`) calls `js_typed_array_new(JS_TYPED_UINT8, size)` and sets the resulting `JsTypedArray.is_buffer = true`; `Buffer.isBuffer` checks the `UINT8` element type plus the flag (`:915`), and the exotic get path routes a `Buffer`'s string-keyed methods to the Buffer prototype when `is_buffer` is set (`js_runtime.cpp:3343`). Because a `Buffer` is `MAP_KIND_TYPED_ARRAY`, string properties cannot be stored directly without triggering the native-backed upgrade, so Buffer identity is carried by the `is_buffer` flag rather than a marker property (`js_buffer.cpp:143`). The flag is also the **ArrayNum carve-out switch**: `js_typed_array_arraynum_view_matches`/`range_matches` (`js_typed_array.cpp:477`/`:482`) reject `is_buffer` arrays, so Buffer element and bulk access stays on the raw fallback paths rather than the ArrayNum accessors. `Buffer.alloc`/`from`/`concat`/`of` and the encode/decode methods (`toString`, `write`, base64/hex) live entirely in `js_buffer.cpp`; the full Node surface is documented in [JS_14 — Node Compatibility](JS_14_Node_Compat.md).
