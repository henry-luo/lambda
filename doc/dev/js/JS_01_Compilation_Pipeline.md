...
- `AstIndex` supplies dense node/function/scope/binding/class identity and parent/owner links. P2a–P2b removed the duplicate JS pointer index and synthetic fallback; P2c publishes common scopes, resolved `NameEntry` bindings, and class nodes, and MIR consumes binding IDs without compiler-time `js_scope_lookup()` or stale-scope repair. Count/fill collection, duplicate `JsFuncCollected::analysis`, shared use/definition edges, and JavaScript extension-scope publication remain open residue. **D8.2.4** requires one stable ID authority.
