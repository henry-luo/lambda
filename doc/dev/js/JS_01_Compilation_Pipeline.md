...
- `AstIndex` supplies dense node/function identity and parent/owner links. P2a–P2b removed the duplicate JS pointer index and synthetic fallback: all collected function lookups now use `AstFunctionId`/`func_by_id`, while count/fill collection, duplicate `JsFuncCollected::analysis`, and scope/binding identity remain open residue. **D8.2.4** requires one stable ID authority.
