...
- `AstIndex` supplies dense node/function identity and parent/owner links. P2a–P2b removed the duplicate JS pointer index and synthetic fallback; P2c makes MIR consume each identifier's builder-resolved `NameEntry` without compiler-time `js_scope_lookup()` or stale-scope repair. Count/fill collection, duplicate `JsFuncCollected::analysis`, and the dense scope/binding/class graph remain open residue. **D8.2.4** requires one stable ID authority.
