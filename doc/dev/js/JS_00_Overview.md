...
- **Benchmark pass rate is not yet 100%** — on the standard performance suites (AWFY, JetStream, R7RS, Larceny, Octane, beng, kostya) the release engine currently runs-and-passes ≈83% of the JS scripts (60/72); the gaps are four wrong-result correctness bugs (bounce, levenshtein, box2d, crypto-md5), an Octane driver that is not shipped plus two feature-path errors, and several heavy macro-benchmarks that time out. ([JS_15 §7](JS_15_Performance.md))
