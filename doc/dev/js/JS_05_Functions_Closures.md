...
`js_new_function` (`:151`) pool-allocates (JS functions are module-lifetime objects held in pool-allocated env arrays unreachable from GC roots, so they must not be collected), stamps the fields, binds `module_vars` to `js_active_module_vars`, and **caches by `func_ptr`** in a fixed 512-entry table (`:11`) so the same MIR function always yields the same `JsFunction*` — this is what makes `Foo.prototype = {…}` and `new Foo()` agree on identity. The cache is bypassed when a `with` environment is active (`:158`). `js_new_method_function` (`:178`) is the uncached variant for methods.
...
LambdaJS backs mutable closure capture with a **scope environment**: an `Item[]` array, GC-rooted, that holds the boxed values of every variable captured by a function's child closures. Because JS captures by reference, multiple sibling closures and the parent itself must all see the same storage.

<img alt="Closure environment lifecycle" src="diagram/d05_closure_env.svg" width="652">

**Allocation at parent entry.** When `fc->has_scope_env` (`jm_define_function`, `js_mir_function_class_lowering.cpp:2745`), the parent either **reuses** its incoming env (when `reuse_parent_env` holds and a compile-time re-check confirms no scope var is locally shadowed, `:2749`–`2790`) or **allocates** a fresh one via `js_alloc_env(scope_env_count)` (`:2792`). `js_alloc_env` (`js_runtime_function.cpp:222`) pool-allocates `count` `Item`s and registers the array as a GC root range so objects held in slots are traced. Each scope-env slot is populated from the current live value — re-read live from the parent's env when the var is itself a capture (`from_env`), so a sibling closure's mutation is visible to later grandchild closures (`:2815`–`2822`) — and the corresponding `JsMirVarEntry` is marked `in_scope_env` with its `scope_env_slot`/`scope_env_reg` (`:2828`–`2831`).

**Closure creation.** `jm_create_func_or_closure` (`js_mir_expression_lowering.cpp:11632`) chooses between two strategies for a capturing function. If the parent's scope env is live and every capture has a resolved slot — and not in a loop where a captured variable is `let`/`const` (which needs per-iteration binding, `:11677`) and not an NFE self-binding (`:11658`) — it emits `js_new_closure(func, pc, scope_env_reg, slot_count)` (`:11685`), giving the closure a **shared, parent-owned** env. Otherwise it falls back to a **copied dense env**: `js_alloc_env(env_alloc_size)`, copy each capture's current value into its slot (boxing natives, reading live from `from_env` parents, resolving `_js_this`/`_js_new.target`/module vars specially), then `js_new_closure(func, pc, env, size)` (`:11705`–`11765`). A self-reference slot is patched to the closure itself after creation (`:11767`). `js_new_closure` (`js_runtime_function.cpp:197`) pool-allocates the `JsFunction`, stores `env`/`env_size`, and binds `module_vars`.
