...
if [[ ! -f "Lambda_Cheatsheet.md" ]]; then
    echo -e "${RED}Error: Lambda_Cheatsheet.md not found in current directory${NC}"
    exit 1
fi

if [[ ! -f "_template_landscape.tex" ]]; then
    echo -e "${RED}Error: _template_landscape.tex not found${NC}"
    exit 1
fi

if [[ ! -f "_template_portrait.tex" ]]; then
    echo -e "${RED}Error: _template_portrait.tex not found${NC}"
    exit 1
fi
...
if pandoc Lambda_Cheatsheet.md \
    ...
    --template=_template_landscape.tex \
    --pdf-engine=xelatex \
...
if pandoc Lambda_Cheatsheet.md \
    -o _Lambda_Cheatsheet_Portrait.pdf \
    --template=_template_portrait.tex \
    --pdf-engine=xelatex \
    -V geometry:a4paper,portrait,margin=0.5in; then
    
    if [[ -f "_Lambda_Cheatsheet_Portrait.pdf" ]]; then
        PDF_SIZE=$(stat -f%z "_Lambda_Cheatsheet_Portrait.pdf" 2>/dev/null || stat -c%s "_Lambda_Cheatsheet_Portrait.pdf" 2>/dev/null || echo "unknown")
        echo -e "${GREEN}✓ Portrait PDF generated successfully${NC}"
        echo "  File: _Lambda_Cheatsheet_Portrait.pdf (${PDF_SIZE} bytes)"
    else
...
echo "  - _Lambda_Cheatsheet_Portrait.pdf (portrait, 3-column)"
echo
echo -e "${YELLOW}Tips:${NC}"
echo "  - To customize formatting, edit the _template_*.tex files"
echo "  - See _LaTeX_Templates_Guide.md for detailed customization options"
