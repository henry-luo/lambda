...
**Spec version:** 48.0.2 (2026-09-26)
...
- **S11.1.7*** **`none` is the empty type** — the bottom of the type lattice, as `any` is its top. It admits no value, `null` and errors included (the null type admits `null`; `none` admits nothing), so `none <: T` holds for every `T` and `T <: none` only when `T` is `none` (S11.1.4v2). A type operation (S10.1.1v2) **reduces** to `none` when its literal operands decide that nothing is admitted — scalar literals, and containers as the pattern their literal spells: `1 & 2`, `(1 | 2) ! (1 | 2)`, `int & "a"`, `null & int`, `[1] & [2]`, `{a: 1} & {a: 2}` — and `none` is the identity of `|` and `!` and absorbs `&`: `T | none` and `T ! none` are `T`, while `T & none` and `none ! T` are `none`. The reduction holds in value and type context alike (`type E = 1 & 2` names `none`), and a reduced form *is* its result: it prints as it, and under S5.5.2 `(3 & 4) == (1 & 2)` and `(int | none) == int`. *Informative:* the implementation reduces only what literal operands decide, so an operation that admits nothing for another reason, such as `int & string`, is left as written (Appendix A); a good implementation should detect one and warn, or reject it. As a declared type `none` is a contract no value passes (S11.4.1v3), with no rule of its own: a known source is a compile error (`let x: none = 5`, `fn f() none => 5`), and an unknown one fails the way its position fails for any type — a failed return is the call's error value, a failed declaration or argument stops. A function that only raises declares `none^E`. `none` is a base-type word (S16.10.1v2). *Why: the empty type needed a spelling distinct from the null type. `none` pairs with `any`, names what it admits, and implies no exception — a failed Lambda contract is an error value, not a throw.* [C6.5]
...
| S8.2.4v3 | **Ruled 2026-09-26 (USER); implemented 2026-09-26 on both tiers.** It reverts the array reading S8.2.4v2 of 2026-09-25, which was never built; the run itself had been conformant since 2026-09-23 (`fn_query`/`fn_child_query` finish through the list collapse and the checker types the result open). New in this ruling and now built: a list is stepped set-at-a-time — `child_query_collect` applies the step to each item, so a scalar item contributes nothing (`(1, "a", 2)[int]` had filtered to `(1, 2)`), while chains through element items already worked (`html[table][tr][td]` at two tables) — and `query_collect` applies `?`/`.?` to each item of a list instead of testing the items themselves; a `null` value never matches (`[1, null, 2]?(int | null)` had yielded three matches, the null hidden in printed output); `?` walks a range's values (`(1 to 3)?int` had found none); an index array is a gather and a bool array a mask over any sequence (`fn_index_select`: arrays, typed arrays, ranges, elements; `a[[1, 3, 5]]` had raised `the index array must be boolean`). Items nested in one another are not deduplicated: with no value identity (SI13), `html?<div>?<div>` reaches a doubly nested div once per enclosing one. The subscript defects found alongside are fixed with it: [LR07-32..36](<../vibe/Lambda_Issue_Ledger (fixed).md#lr07-32>). Fixtures `test/lambda/subscript_selection.ls` and `test/lambda/proc/subscript_last_scope.ls`, on all three tiers (`kTune27TierParity`) and in the forced-GC sweep; `query_kind.ls` now pins `type((1, "a", 2)[int])` as `null`. `make test-lambda-baseline` 5929/5929; `make test-radiant-baseline` 3692 passed, its one failure a Layout Page Suite runner error under the full parallel run that passed on two isolated reruns (51/51 measured, snapshot check clean). **Virtual carriers since later on 2026-09-26 (both tiers):** a type query walks a VArray, Velmt or VMap through `for`'s iteration helpers (`query_walk_virtual`: attribute values then children, a map's values, an array's items, D7.4.5v2), and a type key on a VMap is a query where `fn_index` had taken it as a map key and read `null`; the fixture's VMap section pins it, while VArray and Velmt, which need a host, have no fixture of their own. That walk allocates mid-query, and the forced-GC sweep caught `fn_query`/`fn_child_query` holding their result array unrooted, so both now root it with their operands (the older arms had simply never allocated between pushes). **Residue:** a gather over text is unruled and reads `null`. The inline `(int | null)` key, which had been a set union, is a type since S10.1.1v2 ([LR07-37](<../vibe/Lambda_Issue_Ledger (fixed).md#lr07-37>), fixed 2026-09-26). v1 (2026-09-23) closed [LR05-13](<../vibe/Lambda_Issue_Ledger (fixed).md#lr05-13>). Ruling record: [Expr_Query §4.1](../vibe/Lambda_Expr_Query.md). |
| S10.1.1v2 | **Ruled and implemented 2026-09-26 (USER; both tiers)**, replacing a same-day version that kept container set operations and collapsed decided type operations to values. `fn_union`/`fn_intersect`/`fn_exclude` (`lambda-eval.cpp`) always build a `TypeBinary` over their operands — a type operand as itself, a scalar as its literal type (`type_op_literal_type`, allocated in the runtime pool, since a type value is not GC-managed and outlives the value it was made from) — wrapped as a type value that `is`, the validator and the type query accept as they accept a type from syntax. The value set functions are `unique` and `intersect` (two operands or more) and `except` (two) in `lambda-vector.cpp`, built on `++` and unary `unique` so text keeps its kind and N-D arrays walk rows; the registry rows stop at four operands, and a longer call nests its first four at build time (`fold_set_function_operands`), which both functions allow because they fold in first-operand order. Before this, `1 | 2`, `1 | 1` and `int | null` were all `[]` on both tiers — the set operation walked a scalar's content, which is empty (S8.3.1v3) — which is what made `e[(int | null)]` select nothing ([LR07-37](<../vibe/Lambda_Issue_Ledger (fixed).md#lr07-37>)); `[1] | 2` was `[1]` for the same reason. Migration: `element_content_axes.ls` (`e \| f` → `unique(content(e), content(f))` and likewise), `test/std/core/operators/set_operators.ls` (to the functions, expected output unchanged), `list_kind_transform.ls` (the old list-kind line of `L \| (2, 5)` now pins the functions' array result) and `type_syntax_edges.ls` (E1 now pins the query boundary with a scalar match); no library code used the operators on containers. Fixture `test/lambda/type_set_operators_expr.ls`, on all three tiers (`kTune27TierParity`). **Completed later on 2026-09-26 (both tiers):** a container operand reads as its literal type — `type_op_operand_type` builds the pattern the value's literal spells (a bracket pattern of per-slot literal values or nested types, a map or element shape of per-field literal types, an element's children as its content section, a range value's range type), copied into the runtime pool so the type holds nothing of the GC heap — and bool (the value-carrying singletons of [LR03-31](<../vibe/Lambda_Issue_Ledger (fixed).md#lr03-31>)), datetime and binary scalars have literal types too. A union arm built from a type value in an expression is bare, and the validator refused every bare scalar kind but seven, so `int \| datetime` in an expression admitted no datetime (nor `date \| 1` a date, `decimal \| 1` a decimal) where `type T = int \| datetime` did: every scalar kind now validates as its wrapped form does (`validate_against_base_payload`), which also stopped the validator reading a sized type's size as a type kind (`5i16 is (i16 \| string)` was false) and made `date` and `time` test precision as `is` does ([LR13-12](<../vibe/Lambda_Issue_Ledger (fixed).md#lr13-12>)). Fixture `type_set_operators_expr.ls` gained a section for each; baseline 5954/5954 with the forced-GC sweep. **Residue:** a decimal operand has no runtime literal type — its libmpdec coefficient needs an owner that a runtime type value does not have — and a nominal object's literal pattern is unruled; both are operand errors. Decimal, datetime, binary, suffixed and negative literals are not type patterns in type position either ([LR02-30](../vibe/Lambda_Issue_Ledger.md#lr02-30)), so a datetime or binary literal type has only its expression spelling. An element is not walked by the set functions, as by unary `unique` — its children are `content(e)`. The static type of a type operation in an expression is `any`; the builder promotes only two explicit types (`int \| string`). Argument: [Formal2 C6.4](../vibe/Lambda_Semantics_Formal2.md). |
...
| S11.1.7 | **Ruled and implemented 2026-09-26 (USER; both tiers); partial for the literal decisions listed in the residue.** Container literals decide since the clarification of the same day (USER: "literal operands decide" includes containers, and the `int & string` sentence was implementation status, not a ruling), spec 47.2.0. `TYPE_NONE` is a compact meta type beside `number` and `integer` — the `LMD_TYPE_TYPE` tag, told apart by pointer identity — with the singleton type value `LIT_TYPE_NONE`; the word is in the three keyword tables (C lexer `base_types`, `grammar.js` `_base_type_kw`, the builder's `BASE_TYPE_NAMES`). The reduction is one function, `lambda_type_operation_reduced` (`type_contract.cpp`), that asks three-valued whether one operand admits all, none or only some of a literal operand's values: a scalar literal's (every value `==` to it, S11.2.1), a range's (by its bounds), or a container pattern's (part by part — slots, fields, tag and content — against a pattern of its own kind, by kind alone against any other type). A predicate, a string pattern or a run answers *unknown* and never reduces; so does whatever `is` could answer otherwise. The runtime type operators return its result — `none`, or the surviving operand — and the builder's three binary-type constructors give a reduced node its result as the type value, a survivor in a type_list slot of its own, which the JIT emits by that slot. The static boundary rejects a known source crossing into `none` (E201 at a declaration, E208 at a return) and defers an unknown one, and any `none` source. Fixed on the way, both older than `none`: `==` on type values compared the payload tag alone, so `number == integer` was `true` (and `none == type` would have been) — the compact meta types now compare by identity; and `is` tested a bare numeric literal type by its tag, so `2 is (none \| 1)` answered `true`. Migration: eight bindings named `none` in six fixtures. Fixtures `test/lambda/type_none.ls` (three tiers, `kTune27TierParity`), `negative/semantic/none_type_rejections.ls`, `negative/semantic/none_binding_barred.ls`; baseline 5947/5947, S16 harnesses C 354/354 and reference grammar 341/341. **Residue:** the implementation reduces only what literal operands decide, so `int & string` stays as written and nothing warns of it (the informative remark), and `<:` with an unreduced intersection as the candidate is incomplete (`(int & string) <: bool` is `false`). Some literal decisions answer *unknown*: a numeric literal against a numeric kind other than `number`, unless the kind is an integer kind and the literal has a fraction (`1.5 & int` is `none`, `1 ! float` stays); a datetime literal against `date` or `time`; a pattern with a run; the empty bracket pattern `[]` beyond its kind, since the validator admits every array into it ([LR13-14](../vibe/Lambda_Issue_Ledger.md#lr13-14)); and an element's attributes, since the validator never requires one (`<p> is <p a: 1>` is `true`, so `<p a: 1> & <p a: 2>` admits `<p>`), an unruled point. A range that union arms cover only together reduces since 2026-09-26 (`(1 to 3) ! (1 | 2 | 3)` is `none`, [LR03-21](<../vibe/Lambda_Issue_Ledger (fixed).md#lr03-21>)). Also: type equality compares normalized forms beyond the compact meta types since 2026-09-26 (S5.5.2, [LR03-29](<../vibe/Lambda_Issue_Ledger (fixed).md#lr03-29>), fixture `type_repr_equality.ls`); container patterns and function types still compare by kind ([LR03-33](../vibe/Lambda_Issue_Ledger.md#lr03-33)); `!none` (`any ! none`) reduces to `any` in every position — in a type position it always did, and the builder's value-position negation now reduces as well. Found on the way and fixed the same day: [LR03-30](<../vibe/Lambda_Issue_Ledger (fixed).md#lr03-30>), a one-literal alias (`type T = 1`) that was not a type value and a symbol alias that admitted nothing. Its fix also returns the shared `type` from `type(x)` of a type value, since the identity comparison above had made `type(int) == type` false (fixture `test/lambda/type_literal_alias.ls`, three tiers); a bool literal type carried no value and admitted both bools ([LR03-31](<../vibe/Lambda_Issue_Ledger (fixed).md#lr03-31>), fixed the same day, with [LR07-38](<../vibe/Lambda_Issue_Ledger (fixed).md#lr07-38>): the JIT skipped a one-value numeric literal contract). The container clarification's pass fixed [LR03-32](<../vibe/Lambda_Issue_Ledger (fixed).md#lr03-32>) — the reduction asked only a numeric literal's own value, so `1 ! int` and `(1 | 2) ! int` were `none` though `1.0 is 1`, and `t'2025-01-01' ! date` was `none` across precisions — and [LR13-13](<../vibe/Lambda_Issue_Ledger (fixed).md#lr13-13>), a literal union member that tested the TypeId before the value, so `1.0 is (1 | 2)` was `false`; it found [LR13-15](../vibe/Lambda_Issue_Ledger.md#lr13-15), a present `null` map field that only a run type admits. Fixtures: `type_none.ls` (container and literal sections, three tiers) and `type_literal_admission.ls`; baseline 5954/5954 (the forced-GC sweep included) and validator suites 154/154. Argument and prior art: [Formal2 C6.5](../vibe/Lambda_Semantics_Formal2.md). |
