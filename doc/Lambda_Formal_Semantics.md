...
**Spec version:** 45.0.0 (2026-09-26)
...
- **S10.1.1v2*** **`|`, `&` and `!` are the type operators, everywhere** — type expressions, match or-patterns, string patterns and value expressions alike (types are first-class); `|>` is the pipe. In a value expression the operands decide the operation. Two **containers** meet as value sets over their items: `|` the deduplicated union, `&` the intersection, `!` the exclusion, kind by S2.5.7v4 (`[1, 2] | [2, 3]` is `[1, 2, 3]`; the list case is SO48). Any other pairing — a type operand, or two scalars — is a **type operation**, a scalar reading as its literal type: `1 | 2` is the type admitting 1 or 2, `int | null` is `int?`, `data?(1 | 2)` finds the 1s and 2s. A type operation **collapses to a value only in expression context**, and only when a literal set decides its admitted set: exactly one value is that value (`1 | 1` is `1`, `(1 | 2) & 2` is `2`, `int & 5` is `5`, `(1 | 2) ! 1` is `2`) and none is `null` (`1 & 2`, `int & "a"`); otherwise it stays a type (`1 | 2`, `int ! 5`, `int & string` — no attempt is made to decide an infinite type). In **type context** — a `type` declaration, an annotation, a pattern — nothing collapses: `type t = 1 & 2` and `let v: 1 & 2` declare a type, as `type t = 1` declares the literal type and not the value `1`. *Why: a set of alternatives is an enum, which Lambda already writes as a type; a set value would need a kind that arrays already serve, and would make one spelling a type in an annotation and a value in an expression.* [C6, C6.4]
...
| S10.1.1v2 | **Ruled and implemented 2026-09-26 (USER; both tiers).** `fn_union`/`fn_intersect`/`fn_exclude` (`lambda-eval.cpp`) dispatch on their operands: two containers keep the value set operation; a type operand or two scalars take `type_set_operation`, which reads a scalar as its literal type (`type_op_literal_type`, allocated in the runtime pool, since a type value is not GC-managed and outlives the value it was made from), decides the admitted set through literal sets (`type_op_literal_set_type`) and collapses in expression context; an undecided operation builds a `TypeBinary` wrapped as a type value, which `is`, the validator and the type query accept as they accept a type from syntax. Type context is untouched: the type parser builds those types and never collapses (`type T1 = 1 & 2` admits nothing, `type T2 = 1 | 1` admits `1`). Before this, `1 | 2`, `1 | 1` and `int | null` were all `[]` on both tiers — the set operation walked a scalar's content, which is empty (S8.3.1v3) — which is what made `e[(int | null)]` select nothing ([LR07-37](<../vibe/Lambda_Issue_Ledger (fixed).md#lr07-37>)). Fixture `test/lambda/type_set_operators_expr.ls`, on all three tiers (`kTune27TierParity`). **Residue:** a runtime literal type exists for `null`, int, int64, float, string and symbol; a bool, decimal, datetime, binary or container operand of a type operation is an error until one is built for it. A scalar beside a container keeps the old set path, in which the scalar contributes no items (unruled). The static type of a type operation is `any`; the builder still promotes only two explicit types to a static type (`int | string`). Argument: [Formal2 C6.4](../vibe/Lambda_Semantics_Formal2.md). |
