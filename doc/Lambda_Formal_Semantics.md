...
**Spec version:** 47.1.1 (2026-09-26)
...
- **S11.1.7** **`none` is the empty type** — the bottom of the type lattice, as `any` is its top. It admits no value, `null` and errors included (the null type admits `null`; `none` admits nothing), so `none <: T` holds for every `T` and `T <: none` only when `T` is `none` (S11.1.4v2). A type operation (S10.1.1v2) **reduces** to `none` when its literal operands decide that nothing is admitted — `1 & 2`, `(1 | 2) ! (1 | 2)`, `int & "a"`, `null & int` — and `none` is the identity of `|` and `!` and absorbs `&`: `T | none` and `T ! none` are `T`, while `T & none` and `none ! T` are `none`. The reduction holds in value and type context alike (`type E = 1 & 2` names `none`), and a reduced form *is* its result: it prints as it, and under S5.5.2 `(3 & 4) == (1 & 2)` and `(int | none) == int`. An operation its literals do not decide stays as written — `int & string` admits nothing yet is not `none`, since only a literal operand makes emptiness a matter of listing values. As a declared type `none` is a contract no value passes (S11.4.1v3), with no rule of its own: a known source is a compile error (`let x: none = 5`, `fn f() none => 5`), and an unknown one fails the way its position fails for any type — a failed return is the call's error value, a failed declaration or argument stops. A function that only raises declares `none^E`. `none` is a base-type word (S16.10.1v2). *Why: the empty type needed a spelling distinct from the null type. `none` pairs with `any`, names what it admits, and implies no exception — a failed Lambda contract is an error value, not a throw.* [C6.5]
...
| S11.1.7 | **Ruled and implemented 2026-09-26 (USER; both tiers).** `TYPE_NONE` is a compact meta type beside `number` and `integer` — the `LMD_TYPE_TYPE` tag, told apart by pointer identity — with the singleton type value `LIT_TYPE_NONE`; the word is in the three keyword tables (C lexer `base_types`, `grammar.js` `_base_type_kw`, the builder's `BASE_TYPE_NAMES`). The reduction is one function, `lambda_type_operation_reduced` (`type_contract.cpp`): it lists a literal operand's values and asks the other operand about each, three-valued over the scalar lattice, so a predicate, a string pattern, an occurrence or a shape answers *unknown* and never reduces. The runtime type operators return its result — `none`, or the surviving operand — and the builder's three binary-type constructors give a reduced node its result as the type value, a survivor in a type_list slot of its own, which the JIT emits by that slot. The static boundary rejects a known source crossing into `none` (E201 at a declaration, E208 at a return) and defers an unknown one, and any `none` source. Fixed on the way, both older than `none`: `==` on type values compared the payload tag alone, so `number == integer` was `true` (and `none == type` would have been) — the compact meta types now compare by identity; and `is` tested a bare numeric literal type by its tag, so `2 is (none \| 1)` answered `true`. Migration: eight bindings named `none` in six fixtures. Fixtures `test/lambda/type_none.ls` (three tiers, `kTune27TierParity`), `negative/semantic/none_type_rejections.ls`, `negative/semantic/none_binding_barred.ls`; baseline 5947/5947, S16 harnesses C 354/354 and reference grammar 341/341. **Residue:** only literal-decided emptiness reduces, by the ruling (`int & string` stays as written), and `<:` with an unreduced intersection as the candidate is incomplete (`(int & string) <: bool` is `false`); type equality beyond `none` and the compact meta types still compares the payload tag alone, an S5.5.2 gap ([LR03-29](../vibe/Lambda_Issue_Ledger.md#lr03-29)); `!none` (`any ! none`) reduces to `any` in every position — in a type position it always did, and the builder's value-position negation now reduces as well. Only scalar literal sets decide emptiness: a container literal operand (`[1] & [2]`) is not reduced. Found on the way and fixed the same day: [LR03-30](<../vibe/Lambda_Issue_Ledger (fixed).md#lr03-30>), a one-literal alias (`type T = 1`) that was not a type value and a symbol alias that admitted nothing. Its fix also returns the shared `type` from `type(x)` of a type value, since the identity comparison above had made `type(int) == type` false (fixture `test/lambda/type_literal_alias.ls`, three tiers); a bool literal type carried no value and admitted both bools ([LR03-31](<../vibe/Lambda_Issue_Ledger (fixed).md#lr03-31>), fixed the same day, with [LR07-38](<../vibe/Lambda_Issue_Ledger (fixed).md#lr07-38>): the JIT skipped a one-value numeric literal contract). Argument and prior art: [Formal2 C6.5](../vibe/Lambda_Semantics_Formal2.md). |
