...
**Spec version:** 13.1.1 (2026-09-25)
...
- **DO23** Shape system opens: cross-`Input` type sharing — each `Input` grows its own transition tree (D3.4.3v2) — and shape serialization. *Closed 2026-09-25 with the shape pool's retirement:* the pool's hash-collision policy, its element-name-as-attribute-zero encoding, parse-vs-runtime interning (Shape_Pool Phase 5) and `CachedShape.ref_count`; `byte_offset`'s exclusion from identity is stated in D3.4.2v2.
