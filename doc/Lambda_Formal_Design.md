...
**Spec version:** 1.35.0 (2026-08-26)
...
- **D8.1.3v7*** LambdaJS has an explicit, boxed AST interpreter backend. A
  ...
  never replays through MIR. CommonJS/ES modules,
...
| D8.1.3v7 | Revised 2026-08-26: the LambdaJS AST tier admits the implemented synchronous P3 surface plus direct/indirect eval, `new.target`, public `super`, ordinary/arrow `arguments`, and private class elements. A traced class-private environment retains each evaluated class identity for fields, methods/accessors, static blocks, and nested closures; the existing private-key, field/brand, accessor, and private-`in` kernels remain the semantic authority. AST arrows capture their defining `new.target`, lexical home class, and nearest function `arguments` object; ordinary functions materialize the runtime object in their traced activation before parameter-default evaluation. Sloppy simple parameters use the existing mapped-alias helpers, while strict/non-simple functions use the existing unmapped object and its `callee` policy. Explicit derived constructors use the existing derived-`this` transition, live superclass construct capability, and post-super field initialization; super property reads/writes/calls use the same runtime reference path for classes and object methods. Direct eval deliberately delegates only the dynamic source to the established dynamic-code compiler, under the caller's shared EvalContext bridge, interpreter-cell writeback, eval-local var journal, and global lexical-to-slab synchronization; it never falls back or replays the enclosing AST script. `JsScript` remains in the common Runtime catalog and shares the canonical EvalContext/module-state slab with Lambda; AST-body function environments are precisely traced. `JS_EXECUTION_BACKEND=ast` remains explicit and fail-closed, and the default remains MIR while modules, shared T0/T1 environments, continuations, and AUTO policy are pending. Status and focused gates: `vibe/Lambda_Design_JS_Interpreter.md` and `vibe/impl/Lambda_Impl_JS_Interpreter.md`. |
