...
**Spec version:** 10.0.1 (2026-09-22)
...
- **D8.1.3v13*** LambdaJS uses its first-party C lexer and hybrid
  ...
  *(v12, 2026-09-19)* `JS_EXECUTION_BACKEND=auto` is an explicit AST-first
  ...
  top-level function declaration with local-only bindings is eligible. Any
  capture, module/global binding, property or call boundary, nested callable,
  class, compound literal/pattern, `this`, `arguments`, `new.target`, `eval`,
  or `with` pins that definition to AST. The satellite context is owned by the
  current `EvalContext` until final realm teardown. Thus an unsupported
...
- **D8.5.1v4*** **L1** is a persistent in-process `InputScriptCache` for
  ...
  cannot bypass the retained AST image. [D8.1.1v10, D8.1.3v13]
