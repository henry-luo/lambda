...
**Spec version:** 13.0.3 (2026-09-25)
...
| D3.4.3v2 | **Maps implemented; parsed elements and editor rebuilds since 2026-09-25; the pool's retirement not yet.** Maps built through `map_put` — every `MarkBuilder` parser and every JS object — share through the per-`Input` tree (`map_put_with_data_growth`, `input.cpp`). The edge table dates from 2026-06-25; the shared root and the bounds (256 edges from the root, 16 from any other shape, 1,024 shapes per `Input`) from 2026-09-09. Maps stopped interning through the shape pool on 2026-08-08 (`map_finalize_shape` is a no-op). Verification: removing 256 B from every `TypeMap` (A1v2, 2026-09-25) left the JSON parse peak at 191 MB while HTML fell from 315 to 233 MB; the shape budget took the test262 batch peak from 5,662 to 891 MB. **Elements (P1, 2026-09-25):** `ElementBuilder` starts every element on its tag's root (`elmt_tree_root`, a pool-owned table keyed by the pooled tag name and namespace) and `putToElement` adds attributes through the tree (`elmt_put_tree`); element nodes count against their own budget of 16,384 per `Input`, since a 13 MiB corpus of 77 real sites needs 5,093 of them. Peak RSS on a 13 MiB page corpus 109.6 → 86.3 MB and on a 13 MiB layout-test corpus (203K elements, 1,775 types) 184.0 → 120.4 MB, parse 1.43× faster; 17,230 HTML files, 99 test inputs and 84 formatter outputs identical to P0. **Residue:** runtime-built elements (`elmt_put` without an `Input`) and elements past the budget keep private types, and `elmt_finalize_shape` still pools those types' chains. Element literals already share. **Editor rebuilds (P2, 2026-09-25):** `container_rebuild_with_new_shape` replays the new field list through the tree from the container's root (`type_tree_root_like`, `type_tree_follow`), a kept field replaying its record's identity, so an edited map or element joins the type its fields already have; a declined tree leaves a type the container owns, with a chain of its own. It edits a type in place only when the container owns it (`is_private_clone`): the old guard also rewrote tree and literal types, and a GTest confirmed that an inline edit re-laid a sibling map's fields. Element rebuilds keep `name_id` and `ns`. The pool's retirement is planned ([Impl_Element_Type_Sharing](../vibe/impl/Lambda_Impl_Element_Type_Sharing.md) P3). The runtime/EvalContext shape pool (Shape_Pool Phase 5) is superseded rather than pending. Pointer: [Shape_Transitions §7–§8](../vibe/Lambda_Design_Shape_Transitions.md). |
