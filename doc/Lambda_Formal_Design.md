...
**Spec version:** 10.1.0 (2026-09-22)
...
- **D8.1.1v11*** First-party Lambda C lexer + hybrid recursive-descent/Pratt
  ...
  (**T1**). *(v11, 2026-09-22)* A file-backed static import closure uses one
  bounded, language-neutral work pool, not a series of dependency-depth
  batches. Parse-accurate discovery of an import immediately canonicalizes,
  deduplicates, and queues its child request; a completed child releases every
  direct parent that was waiting for it. Thus for `A -> B -> C` and
  `A -> D -> C`, discovering `B` and `D` queues their one shared `C` request,
  and completing `C` releases both `B` and `D`. A task never waits for a child
  task while occupying a pool worker; only a node whose direct dependencies
  completed may queue its AST build. The cache retains its ordinary
  single-flight behavior for requests from another closure. A worker owns a
  temporary `Runtime` and may publish only a retained AST template through
  ...
  evaluator. [D8.5.1v5] The REPL and legacy inspection tools
...
- **D8.1.3v18*** LambdaJS uses its first-party C lexer and hybrid
  ...
  *(v12, 2026-09-19)* `JS_EXECUTION_BACKEND=auto` is an explicit AST-first
  ...
  closure prebuilt under D8.1.1v11/D8.5.1v5, while an unsupported unit keeps
  ...
  interpretation. A selected MIR unit is not silently redirected to AST;
  compilation failure is reported without replay. This native-only contract
  does not remove other language profiles' MIR-interpreter support.
...
- **D8.2.5v2*** One typed pass manager owns build → bind → validate → index →
...
- **D8.5.1v5*** **L1** is a persistent in-process `InputScriptCache` for
  ...
  Static module closure prebuild is an AST-only cache producer. One bounded
  closure-wide task pool accepts every parse-accurately discovered static
  import immediately, canonicalizes and deduplicates requests, and queues
  child AST builds only after their direct imports publish. Completion notifies
  each direct dependent; no dependency-depth batch barrier or worker-side wait
  on a nested import is permitted. The cache retains its ordinary single-flight
  behavior for requests from another closure. Module initialization, execution
  state, and MIR compilation remain with the receiving runtime. Canonical file
  paths are used
  at both prebuild and runtime import boundaries, so a relative spelling
  cannot bypass the retained AST image. [D8.1.1v11, D8.1.3v18]
